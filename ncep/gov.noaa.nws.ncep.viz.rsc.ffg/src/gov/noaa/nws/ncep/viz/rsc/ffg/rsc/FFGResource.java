 ...
 * @version 1.0
 */
public class FFGResource extends
        AbstractNatlCntrsResource<FFGResourceData, MapDescriptor> implements
    ...
    // ffgData is a map from the zoneId to the data values and is used to avoid
    ...
    private class FrameData extends AbstractFrameData {
        ...
        public boolean updateFrameData(IRscDataObject rscDataObj) {
            if (!(rscDataObj instanceof FFGRscDataObj)) {
                System.out
                        .println("FFG:updateFrameData expecting FFGRscDataObj instead of: "
            ...
            // if zone is not in the list or if the new data is a better
            // timeMatch
            //
            if (existingFfgData == null) {
                ffgDataList.put(ffgRscData.zoneId, ffgRscData);
            } else if (timeMatch(rscDataObj.getDataTime()) < timeMatch(existingFfgData
                    .getDataTime())) {
                ffgDataList.put(ffgRscData.zoneId, ffgRscData);
            }
    ...
    protected IRscDataObject[] processRecord(Object pdo) {
        if (!(pdo instanceof FfgRecord)) {
            System.out
                    .println("FFG processRecord() : Expecting FfgRecord object instead of: "
    ...
    public void initResource(IGraphicsTarget grphTarget) throws VizException {
        ...
        stationTable = new StationTable(LocalizationManager.getInstance()
                .getFilename("countyZonesFile"));

        font = null; // use default for now

        cbarRscPair = ResourcePair
                .constructSystemResourcePair(new ColorBarResourceData(
                        ffgRscData.getColorBar()));
    ...
    public void paintFrame(AbstractFrameData frameData,
        ...
        // double kmPerPixel = (displayWidth /
        // paintProps.getCanvasBounds().width) / 1000.0;
        // get plotWidth

        // loop thru the ffg records
        // (This should be fast.)
        Collection<FFGRscDataObj> ffgDataValues = currFrameData.ffgDataList
                .values();
        ...
        for (FFGRscDataObj ffgData : ffgDataValues) {
            ...
            // get the pixel coord from the lat/lon and check if it is in the
            // paint extents
                ...
                for (int i = 0; i < colorBar.getNumIntervals(); i++) {
                    if (colorBar.isValueInInterval(i, paramVal, ffgUnits)) {
                        ...
                        if (ffgRscData.getDisplayValues()) {
                            grphTarget.drawString(
                                    font,
                                    Float.toString(paramVal), // ffgData.zoneId,
                                    zonePix[0], zonePix[1], 0.0,
                                    TextStyle.NORMAL, drawColor,
                                    HorizontalAlignment.CENTER, 0.0);
                        }
                        // TODO : Draw the selected Symbol.
                        else {
                            grphTarget.drawString(font, "X", zonePix[0],
                                    zonePix[1], 0.0, TextStyle.NORMAL,
                                    drawColor, HorizontalAlignment.CENTER, 0.0);
                        }
                    }
                }
            }
