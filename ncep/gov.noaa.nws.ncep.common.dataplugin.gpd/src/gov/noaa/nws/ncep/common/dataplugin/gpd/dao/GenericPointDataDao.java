...
public class GenericPointDataDao extends PointDataPluginDao<GenericPointDataRecord> {
	...
	public String[] getKeysRequiredForFileName() {
		return new String[] { GenericPointDataConstants.DB_REFTIME_NAME , GenericPointDataConstants.DB_PROD_NAME, GenericPointDataConstants.DB_MASTER_LEVEL_NAME};
	}
	...
	/*
	 * (non-Javadoc)
	 * @see com.raytheon.uf.edex.pointdata.PointDataPluginDao#getPointDataFileName(com.raytheon.uf.common.dataplugin.PluginDataObject)
	 * Chin: in GPD implementation this is only called from Purge
	 * 
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see com.raytheon.uf.edex.pointdata.PointDataPluginDao#getPointDataFileName(java.util.Map)
	 * return a full HDF5 point data file name (including path)
	 * Chin:::This is called when requesting data 
	 * Currently one report is defined with only one master level, therefore file path will be like this.
	 * /awips2/edex/data/hdf5/gpd/productName/gpd-productName-yyyy-mm-dd-HH-MM.h5 
	 * E.g. /awips2/edex/data/hdf5/gpd/sib1/gpd-sib1-2013-05-08-19-00.h5 
	 * Note:::future ???
	 * If, we want to defined more than one master level for one report type, then file path will like this,
	 * /awips2/edex/data/hdf5/gpd/reportName/masterLevelName/gpd-reportName-masterLevelName-yyyy-mm-dd-HH-MM.h5 
	 * E.g. /awips2/edex/data/hdf5/gpd/sib1/MB/gpd-sib1-MB-2013-05-08-19-00.h5
	 *
	 */
	@Override
	public String getPointDataFileName(Map<String, Object> dbResults) {
		String reportname = (String)dbResults.get(GenericPointDataConstants.DB_PROD_NAME);
		//String lmName= (String)dbResults.get(GenericPointDataConstants.DB_MASTER_LEVEL_NAME);
		String dateStr = hdfFileDateFormat.format(dbResults.get(GenericPointDataConstants.DB_REFTIME_NAME));
		String filename = 
				PLUGIN_HDF5_DIR
				+  reportname + File.separator 
				//+ lmName + File.separator  
				+this.pluginName+"-"+reportname+/*"-"+lmName+*/dateStr+".h5";
		//System.out.println("GenericPointDataDao getPointDataFileName2 called and return: "+ filename);

		return filename;
	}
	/*
	 * (non-Javadoc)
	 * @see com.raytheon.uf.edex.pointdata.PointDataPluginDao#getFullFilePath(com.raytheon.uf.common.dataplugin.PluginDataObject)
	 * return a full HDF5 point data file name (including path)
	 * This is called when decoding data 
	 */
	@Override
	public File getFullFilePath(PluginDataObject persistable) {
		File file;
		GenericPointDataRecord rec = (GenericPointDataRecord)persistable;
		String directory = PLUGIN_HDF5_DIR /*+ File.separator + persistable.getPluginName()
                + File.separator*/
				+ rec.getProductInfo().getName();

		Date refTime = ((PluginDataObject) persistable).getDataTime().getRefTime();
		String dateStr = hdfFileDateFormat.format(refTime);
		String fileName = persistable.getPluginName()+ "-"+rec.getProductInfo().getName()+/*"-"+rec.getReportType().getMasterLevel().getName()+*/dateStr+".h5";
		file = new File(directory
				+ File.separator
				+ fileName);
		//System.out.println("GenericPointDataDao getFullFilePath return "+ file.getPath()+" "+ file.getName());
		return file;
	}
	/*
	...
	//look up target location in gpd_location table. If not present and if createLocation = TRUE, insert it to table.
	public boolean lookupGpdLocation(ObStation location, boolean createLocation) {
		...
		try {
			...
			Criterion nameCrit = Restrictions.eq("stationId", location.getStationId());
			crit.add(nameCrit);
			Criterion nameCrit1 = Restrictions.eq("catalogType", location.getCatalogType());
			crit.add(nameCrit1);
			//querying...
			List<?> vals = crit.list();
			...
			logger.error(
					"lookupGpdLocation:Error occurred looking up [" + location.getStationId()
					+ "]", e);
			status = false;
			if (trans != null) {
				try {
					...
					logger.error("lookupGpdLocation: Error occurred rolling back transaction", e);
				}
			}
		} finally {
			if (sess != null) {
				try {
					...
					logger.error("lookupGpdLocation: Error occurred closing session", e);
				}
			}
		}
	...
	//look up target parm in parameter table. If not present and if createParm = TRUE, insert it to table.
	public boolean lookupParameter(Parameter parm, boolean createParm) {
		...
		try {
			...
			Criterion nameCrit = Restrictions.eq("abbreviation", parm.getAbbreviation());
			crit.add(nameCrit);
			//querying...
			List<?> vals = crit.list();
			if (vals.size() <= 0 ){
			...
			logger.error(
					"lookupParameter: Error occurred looking up parm[" + parm.getAbbreviation()
					+ "]", e);
			status = false;
			if (trans != null) {
				try {
					...
					logger.error("lookupParameter: Error occurred rolling back transaction", e);
				}
			}
		} finally {
			if (sess != null) {
				try {
					...
					logger.error("lookupParameter: Error occurred closing session", e);
				}
			}
		}
	...
	//look up target masterLvl in level_master table. If not present and if createMl = TRUE, insert it to table.
	public boolean lookupMasterlevel(MasterLevel masterLvl, boolean createMl) {
		...
		try {
			...
			logger.error(
					"lookupMasterlevel: Error occurred looking up parm[" + masterLvl.getName()
					+ "]", e);
			status = false;
			if (trans != null) {
				try {
					...
					logger.error("lookupMasterlevel: Error occurred rolling back transaction", e);
				}
			}
		} finally {
			if (sess != null) {
				try {
					...
					logger.error("lookupMasterlevel: Error occurred closing session", e);
				}
			}
		}
	...
	private boolean mergeProductInfoParameterList(GenericPointDataProductInfo targetProdInfo, GenericPointDataProductInfo sourceProdInfo){
		...
		/*for(Parameter p: srcParmList){
			System.out.println("srcA p="+p.getAbbreviation());
		}
		for(Parameter p: tarParmList){
			System.out.println("tarA p="+p.getAbbreviation());
		}*/
		for (int index = srcParmList.size()-1; index >=0 ; index--){
			...
			if(found == false){
				...
				merged = true;
				//System.out.println("not found add newPM="+ newPm.getAbbreviation());
			}
			/*else {
				System.out.println("found srcP="+pm.getAbbreviation());
			}*/
		}
		//System.out.println("merged = "+ merged);
		/*for(Parameter p: srcParmList){
			System.out.println("srcB p="+p.getAbbreviation());
		}
		for(Parameter p: tarParmList){
			System.out.println("tarB p="+p.getAbbreviation());
		}*/
		return merged;
	}
	// 1. look up target product in gpd_productinfo table. If not present and if createReport = TRUE, insert it to table.
	// 2. A complete product is returned using the contents found in DB, as user may just use product name in input XML file for 
	// decoding request, when a same product information had been already saved in DB earlier.
	// 3. If prodInfo contains new parameters, if version number is 0, than update DB, otherwise, reject the update
	// 4. copy input prod's "number of level" to return prodInfo
	public GenericPointDataProductInfo lookupUpdateGpdProdInfo(GenericPointDataProductInfo prod, boolean createProd, int prodVersion) {
		...
		try {
			...
			Criteria crit = sess.createCriteria(GenericPointDataProductInfo.class);
			...
			if (vals.size() > 0) {
				...
				GenericPointDataProductInfo dbProdInfo = (GenericPointDataProductInfo) vals.get(0);
				//check to see if there are new parameters, and merge then to parameter list in dbProdInfo
				boolean merged = mergeProductInfoParameterList(dbProdInfo,prod);
				if(merged == true ){
					//if there are new parameters and product version number is 0, then update product to DB
					if( prodVersion == 0){
				...
				if(status== true && lookupMasterlevel(prod.getMasterLevel(), true) == false){
					status = false;
				}
				if(status){
			...
			logger.error(
					"lookupGpdReportType: Error occurred looking up GenericPointDataReporttype[" + prod.getName()
					+ "]", e);
			status = false;
			if (trans != null) {
				try {
					...
					logger.error("lookupGpdReportType: Error occurred rolling back transaction", e);
				}
			}
		} finally {
			if (sess != null) {
				try {
					...
					logger.error("lookupGpdReportType: Error occurred closing session", e);
				}
			}
		}
	 ...
	 */
	public GenericPointDataProductInfo updateProductInfo(GenericPointDataProductInfo prod){
		Session sess = null;
		boolean status=true;
		Transaction trans = null;
		...
		try {
			...
			if(status){
				...
				if (vals.size() > 0) {
					...
					System.out.println("updateProductInfo: new parameter array size="+rval.getParameterLst().size());
				}
			...
			logger.error(
					"updateProductInfo: Error occurred looking up product [" + prod.getName()
					+ "]", e);
			...
			if (trans != null) {
				try {
					...
					logger.error("updateProductInfo: Error occurred rolling back transaction", e);
				}
			}
		} finally {
			if (sess != null) {
				try {
					...
					logger.error("updateProductInfo: Error occurred closing session", e);
				}
			}
		}
		return rval;
	...
	/* TBM...Chin delet this leter...not used
	public boolean lookupGpdDataUri(String dataUri) {
		boolean status= true;
        Session sess = null;
        Transaction trans = null;
        try {
            ...
            if (trans != null) {
                try {
                    ...
                    logger.error("lookupGpdDataUri: Error occurred rolling back transaction", e);
                }
            }
        } finally {
            if (sess != null) {
	...
	public   GenericPointDataProductInfo getGpdProdInfo(String prodName){
		...
		if(prodName!= null){
			...
			if (vals.size() > 0) {
				try {
					//to avoid LazyInitializationException, we have to take care of Collection before
	...
	/*
	 * Chin::::
	 * Get point data container for one or multiple stations.
	 * Based on queryKey.
	 * If BY_STN_ID, then stnId required, and is querying one station
	 * If BY_SLAT_SLON, then slat/slon required, and is querying one station
	 * If BY_REPORT_NAME, then both not required, and is querying all stations that meet prodInfo and refTime query constraints
	 */
	private PointDataContainer getPointDataContainer(Date refTime, GenericPointDataQueryKey quertKey, String stnId, float slat, float slon, GenericPointDataProductInfo prodInfo, int productVersion) throws Exception{
		 ...
		 */
		//1st:: add return fields from HDF5. They are the parameter list defined in a GPD report
		...
		returnParametersString.append(","+GenericPointDataConstants.HDF5_LEVEL_VALUE);
		returnParametersString.append(","+GenericPointDataConstants.HDF5_NUM_LEVEL);
		returnParametersString.append(","+GenericPointDataConstants.HDF5_STN_ID); 
		System.out.println("gpd dao hdf5 parameterlist="+returnParametersString.toString());

		//2nd:: add return fields form DB. the parameter name need to be defined in 
		//gov.noaa.nws.ncep.edex.plugin.gpd/res/pointdata/gpddb.xmlquertKey
		// for example::
		returnParametersString.append(","+GenericPointDataConstants.DB_STN_CATALOGTYPE);
		...
		// parameters defined in /gov.noaa.nws.ncep.edex.plugin.gpd/utility/common_static/base/path/gpdPathKeys.xml
		// AND those returned by dao.getKeysRequiredForFileName()
		// will be added automatically when calling PointDataQuery.execute()

		//PointDataQuery.setParameters() is to set return fields from both DB and HDF5			
	...
	/* TBM...Chin delet this leter...not used
	public  GenericPointDataProductContainer getGpdProduct(Date refTime,  String prodName,boolean useSpecifiedProductVersion, int productVersion)throws Exception{
		...
		if(useSpecifiedProductVersion== false){
			//find the latest version if there is one. Otherwise, use user specified version number
			productVersion = getGpdProductLatestVersion( refTime, prodName);
		}
		PointDataContainer pdc = getPointDataContainer( refTime,GenericPointDataQueryKey.BY_REPORT_NAME, null,0,0,  prodInfo, productVersion);
		if (pdc == null) {
		...
		for (int i = 0; i < pdc.getCurrentSz(); i++) {
			...
			Set<String> parameters = new HashSet<String>(pdv.getContainer().getParameters());
			for(String parm: parameters){
				System.out.println("parm ="+parm);
			}
			String stnId= null;
			if (parameters.contains(GenericPointDataConstants.HDF5_STN_ID)){
				stnId = pdv.getString(GenericPointDataConstants.HDF5_STN_ID);
				System.out.println("stnid= "+ stnId);
				//stnId value is retrieved already, so drop it here
				parameters.remove(GenericPointDataConstants.HDF5_STN_ID);
			}
			else
				continue; //stnId not available, no need to continue on this PDV.
			int numLevel=0;
			...
			if (parameters.contains(GenericPointDataConstants.HDF5_LEVEL_VALUE)){
				...
				Number[]  num =  pdv.getNumberAllLevels(GenericPointDataConstants.HDF5_LEVEL_VALUE);//pdv.getNumberAllLevels(parm,numLevel); 
				for(Number n: num){
					System.out.println("level value="+n.floatValue());
					if(n.floatValue() == GenericPointDataConstants.GPD_INVALID_FLOAT_VALUE)
						//Not a valid level, skip it
						continue; 
			...
			if (parameters.contains(GenericPointDataConstants.DB_STN_CATALOGTYPE)){
				stnCatalogType = pdv.getInt(GenericPointDataConstants.DB_STN_CATALOGTYPE);
			...
			if (parameters.contains(GenericPointDataConstants.DB_SLAT)){
				slat = pdv.getFloat(GenericPointDataConstants.DB_SLAT);
			...
			if (parameters.contains(GenericPointDataConstants.DB_SLON)){
				slon = pdv.getFloat(GenericPointDataConstants.DB_SLON);
			...
			stnPd.setSlon(slon);
			for(String parm: parameters){
				//these parameters are data parameters and should be 2 dimensional float value per design
				//If a new "meta" data is queried, then we should take care of that data specifically before here.
				Number[]  num = pdv.getNumberAllLevels(parm);//,numLevel);
				System.out.println("parm ="+parm);
				for(int j=0; j< numLevel ; j++){
					Number n = num[j];
					System.out.println(" value="+n.floatValue());
					GenericPointDataParameter gpdParm = new GenericPointDataParameter(parm,n.floatValue());
	 ...
	 */
	public  GenericPointDataProductContainer getGpdProduct(Date refTime, GenericPointDataQueryKey key, String stnId, float slat, float slon, String prodName,boolean useSpecifiedProductVersion, int productVersion)throws Exception{
		GenericPointDataProductInfo prodInfo = getGpdProdInfo( prodName);
		if (prodInfo == null) {
			System.out.println("report is not in DB");
			return null;
		...
		PointDataContainer pdc =  getPointDataContainer( refTime, key, stnId, slat, slon, prodInfo, productVersion);
		if (pdc == null) {
			System.out.println("pdc is null");
			return null;
		}
		...
		for (int i = 0; i < pdc.getCurrentSz(); i++) {
			...
			if (parameters.contains(GenericPointDataConstants.HDF5_LEVEL_VALUE)){
				...
				if(numLevel >1){
					...
					for(Number n: num){
						...
						if(count > numLevel)
							break;
						//System.out.println("Level " +count+ " value="+n.floatValue());
						GenericPointDataLevel gpdLevel = new GenericPointDataLevel();
						gpdLevel.setLevelValue(n.floatValue());
						levelList.add(gpdLevel);
					}
					...
					GenericPointDataLevel gpdLevel = new GenericPointDataLevel();
					gpdLevel.setLevelValue(pdv.getFloat(GenericPointDataConstants.HDF5_LEVEL_VALUE));
					levelList.add(gpdLevel);
				}
				//level value is retrieved already, so drop it here
				parameters.remove(GenericPointDataConstants.HDF5_LEVEL_VALUE);
			...
			for(String parm: parameters){
				if(numLevel>1){
					...
					GenericPointDataParameter gpdParm = new GenericPointDataParameter(parm,pdv.getFloat(parm));
	 ...
	 * 
	 */
	public  List<GenericPointDataStationProduct> getGpdStationProduct(List<Date> refTimeList, GenericPointDataQueryKey key, String stnId, float slat, float slon, String prodName)throws Exception{
		GenericPointDataProductInfo prodInfo = getGpdProdInfo( prodName);
		if (prodInfo == null) {
		...
		for(Date refTime: refTimeList){
			...
			System.out.println(refTime.toString() +" pdc CurrentSz()="+pdc.getCurrentSz());
	...
	/* TBM...Chin delet this leter...not used
	public  GenericPointDataStationProduct getGpdStationProduct(Date refTime, GenericPointDataQueryKey key, String stnId, double slat, double slon, String reportName,boolean useSpecifiedProductVersion, int productVersion)throws Exception{
		GenericPointDataProductInfo report = getGpdProdInfo( reportName);
		if (report == null) {
			System.out.println("report is not in DB");
			return null;
		}
		if(useSpecifiedProductVersion== false){
			//find the latest version if there is one. Otherwise, use user specified version number
			productVersion = getGpdProductLatestVersion( refTime, reportName);
		}
		PointDataContainer pdc;
		pdc = getPointDataContainer( refTime, key, stnId, slat, slon, report, productVersion);

		if (pdc == null) {
			System.out.println("pdc is null");
			return null;
		}
		//for a single station product query, the pdc.getCurrentSz() should always be 1
		System.out.println("pdc CurrentSz()="+pdc.getCurrentSz());
		GenericPointDataStationProduct stnPd= null;
		for (int i = 0; i < pdc.getCurrentSz(); i++) {
			PointDataView pdv =pdc.readRandom(i);
			...
			stnPd.setProductName(reportName);
			stnPd.setRefTime(refTime);
			stnPd.setLevelLst(levelList);
			stnPd.setProductVersion(productVersion);
			stnPd.setSlat(rtnslat);
	...
	public int getGpdProductLatestVersion( Date refTime, String prodName){
		...
		Criteria crit = sess.createCriteria(GenericPointDataRecord.class);
		//System.out.println("getGpdProductLatestVersion input reportName="+ prodName);
		...
		if(vals.size() > 0) {
			for(int i=0; i < vals.size() ; i++){
				//to avoid LazyInitializationException, we have to take care of Collection before
				// closing session. 
				GenericPointDataRecord rec = (GenericPointDataRecord) vals.get(i);
	...
	/**
	 *  Chin note: copy from PluginDao, modified code to get GPD HDF5 file path correctly.
	 *  
	...
	public int purgeDataByRefTime(Date refTime,
		...
		int results = 0;
		PurgeLogger.logInfo("Enter purgeDataByRefTime refTime=" + refTime +" trackHdf5="+trackHdf5+ " trackToUri="+trackToUri, pluginName);
		...
		if((hdf5FileToUriPurged != null)){
			for (String key: hdf5FileToUriPurged.keySet())
			{
				...
				PurgeLogger.logInfo("starting purgeDataByRefTime hdf5FileToUriPurged map key=" + key, pluginName);
				if(pairLst!=null){
					for(String val: pairLst){
						PurgeLogger.logInfo("starting purgeDataByRefTime hdf5FileToUriPurged map val=" + val, pluginName);
					}
				}
			}
		}
		...
		if ((productKeys != null) && (productKeys.size() > 0)) {
			for (Map.Entry<String, String> pair : productKeys.entrySet()) {
				...
				PurgeLogger.logInfo(" purgeDataByRefTime product map key=" + pair.getKey()+" value="+ pair.getValue(), pluginName);
			}
		}
			...
			if ((pdos != null) && !pdos.isEmpty()) {
				...
				if (trackHdf5 && (hdf5FileToUriPurged != null)) {
					for (PluginDataObject pdo : pdos) {
						...
						String directory = PLUGIN_HDF5_DIR + rec.getProductInfo().getName();

						String dateStr = hdfFileDateFormat.format(refTime);
						String fileName = this.pluginName+ "-"+rec.getProductInfo().getName()+dateStr+".h5";
						String file = directory+ File.separator+ fileName;
						PurgeLogger.logInfo(++loopCount+" purgeDataByRefTime file=" + file, pluginName);
						if (trackToUri) {
		...
		if((hdf5FileToUriPurged != null)){
			...
			{
				List<String> pairLst = hdf5FileToUriPurged.get(key);
				PurgeLogger.logInfo("leaving purgeDataByRefTime hdf5FileToUriPurged map key=" + key, pluginName);
				if(pairLst!=null){
					for(String val: pairLst){
						PurgeLogger.logInfo("leaving purgeDataByRefTime hdf5FileToUriPurged map val=" + val, pluginName);
					}
				}
			}
		}
