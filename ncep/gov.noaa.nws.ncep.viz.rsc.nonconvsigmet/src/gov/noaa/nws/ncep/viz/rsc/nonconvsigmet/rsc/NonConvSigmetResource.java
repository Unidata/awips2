...
public class NonConvSigmetResource extends
        AbstractNatlCntrsResource<NonConvSigmetResourceData, IMapDescriptor>
    ...
    private class FrameData extends AbstractFrameData {
        ...
        public boolean updateFrameData(IRscDataObject rscDataObj) {
            // NonConvSigmetRecord nonConvSigmetRecord = (NonConvSigmetRecord)
            // pdo;
            // NonConvSigmetRscDataObject nonConvSigmetData =
            // getNonConvSigmetData( nonConvSigmetRecord );
            ...
            if (nonConvSigmetData != null) {
                ...
                if (nonConvSigmetData.hazardType == HazardType.CANCEL
                    ...
                    if (existingNonConvSigmetData == null
                            || nonConvSigmetData.issueTime
                                    .greaterThan(existingNonConvSigmetData.issueTime)) {
    ...
    Map<HazardType, Boolean> hazardEnable = new EnumMap<HazardType, Boolean>(
            HazardType.class);

    Map<SeriesName, Boolean> seriesEnable = new EnumMap<SeriesName, Boolean>(
            SeriesName.class);
    ...
    protected IRscDataObject[] processRecord(Object pdo) {
        ...
        NonConvSigmetRscDataObject ncSigmetRscDataObj = getNonConvSigmetData((NonConvSigmetRecord) pdo);
    ...
    public void paintFrame(AbstractFrameData frameData, IGraphicsTarget target,
        ...
        if (currFrameData != null) {
            ...
            for (NonConvSigmetRscDataObject nonConvSigmetData : nonConvSigmetDataValues) {
                ...
                // if (activeFrameTime.compareTo(nonConvSigmetData.startTime) <
                // 0 ||
                // activeFrameTime.compareTo(nonConvSigmetData.endTime) >= 0)
                // continue;
                ...
                if (hazardEnable.get(nonConvSigmetData.hazardType)
                    ...
                    for (int i = 0; i < nonConvSigmetData.numPoints; i++) {
                        ...
                        if (currentPoint == null)
                            continue; // gracefully skip over omitted points
                    ...
                    if (textLocation != null) {
                        ...
                        if (ncRscData.nameNumberEnable) { // sequence ID; for
                                                          // example, "OSCAR 1"
                            labelList.add(nonConvSigmetData.sequenceID);
                        }

                        if (ncRscData.flightLevelEnable) { // altitude upper
                                                           // bound
                        ...
                        if (ncRscData.symbolEnable) { // drawn independently of
                                                      // text block, but
                                                      // sequenced here
                            ...
                            Symbol symbol = new Symbol(
                                    ...
                                    false, // clear
                                    coordinateSymbolLocation, category,
                                    symbolType);
                            // ...and display it
                        ...
                        if (ncRscData.flightLevelEnable) { // altitude lower
                                                           // bound
                            // Note we skip both lines if this option disabled,
                            // but if enabled and no value, we draw blank line
    ...
    // to
    // the frames. processRecords has saved off the cancellations and
    // corrections into the
    // modify and cancelLists. We will use this list to correct or cancel the
    // times in the
    ...
    protected boolean preProcessFrameUpdate() {
        ...
        // order
        // of bulletins isn't assured, necessary to wait until all possible
        // 'victims'
        // are in place first.) Note that this doesn't remove any data records;
        // it
        ...
        // assured,
        // was necessary to wait until all possible 'victims' are in place
        // first.)
        // Note that this doesn't remove any data records; it just adjusts end
        // times.
        // Cancellations are processed after corrections, since a single SIGMET
        // could have been corrected/amended shortly after issue, and then
        // cancelled
        // hours later at phenomena termination; in this case we need to make
        // sure
        // the cancellation time supersedes the correction time, and not the
        // reverse.
    ...
    private NonConvSigmetRscDataObject getNonConvSigmetData(
        ...
        // NonConvSigmetRecord
        // from the database, but it's distilled down a bit to only the stuff
        // we'll need for
        // paint() later.
        ...
        if (nonConvSigmetData.hazardType == HazardType.UNKNOWN
                ...
                && (nonConvSigmetRecord.getBullMessage().contains(" BLDU ")
                        || nonConvSigmetRecord.getBullMessage()
                                .contains(" DS ") || nonConvSigmetRecord
                        .getBullMessage().contains(" SS "))) {
        ...
        nonConvSigmetData.eventTime = new DataTime(
                nonConvSigmetRecord.getStartTime(), new TimeRange(
                        nonConvSigmetRecord.getStartTime(),
                        nonConvSigmetRecord.getEndTime()));
        nonConvSigmetData.endTime = new DataTime(
                nonConvSigmetRecord.getEndTime());
        nonConvSigmetData.correction = nonConvSigmetRecord
                .getCorrectionRemarks().equalsIgnoreCase("COR")
                || nonConvSigmetRecord.getCorrectionRemarks().equalsIgnoreCase(
                        "AMD");
