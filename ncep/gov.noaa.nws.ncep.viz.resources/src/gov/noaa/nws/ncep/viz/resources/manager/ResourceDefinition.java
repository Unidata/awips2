...
public class ResourceDefinition implements ISerializableObject, IAlertObserver,
        Comparable<ResourceDefinition> {
    ...
    public ResourceDefinition() {
        resourceDefnName = "";
        resourceCategory = ResourceCategory.NullCategory; // new
                                                          // ArrayList<String>();
        subTypeGenerator = "";
        rscTypeGenerator = "";
    ...
    // isEnabled used to be stored here so instead of changing the code that
    // called this method, just implement it here.
    public boolean isEnabled() {
        ResourceDefinitionFilter rdf;
        try {
            rdf = ResourceDefnsMngr.getInstance().getResourceDefnFilter(
                    resourceDefnName);
        } catch (VizException e) {
            return false;
        }
        // TODO : somewhere there should be logic to determine the default
        // enabled state of a RD that doesn't have an entry in the filters file.
        // Here? or in mngr?
    ...
    public NcDisplayType[] getSupportedDisplayTypes() {
        ...
        try {
            ...
            if (rscData instanceof AbstractNatlCntrsRequestableResourceData) {
                ...
                System.out.println("??? Rsc Impl " + rscImplementation
                        + "has non-NC resource class");
            }
        } catch (InstantiationException e) {
            System.out.println("Error instantiating class ("
                    + implClass.getName() + ") for resource: "
                    + rscImplementation);
    ...
    public List<String> getUnconstrainedParameters() {
        HashMap<String, RequestConstraint> constrMap = getInventoryConstraintsFromParameters(
    ...
    public HashMap<String, RequestConstraint> getInventoryConstraintsFromParameters(
        ...
        HashMap<String, RequestConstraint> inventoryConstraints = new HashMap<String, RequestConstraint>();

        inventoryConstraints.put("pluginName", new RequestConstraint(
                getPluginName()));
    ...
    public void disableInventoryUse() throws VizException {
        ...
        //
        if (isRequestable()
                && (getSubTypeGenParamsList().length > 0 || !getRscTypeGenerator()
                        .isEmpty())) {
    ...
    public boolean usesInventory() {
        ...
        return isRequestable() && !isPgenResource()
    ...
    public void validateResourceParameters() throws VizException {
        ...
        if (rscImplementation == "ModelFcstGridContours"
            ...
            System.out
                    .println("Sanity Check: GRID rsc has a non-forecast timelineGenMethod???");
        }
        ...
        List<String> genParamsList = new ArrayList<String>(
    ...
    // if an alias
    public NcInventoryDefinition createNcInventoryDefinition()
            throws VizException {

        // Each resource implementation defines parameters which are used to
        // constraint the data read from the database. (e.g. the modelName
        // for Grids).

        // These parameters ultimately must be given values in order to
        // instantiate a Resource. The parameters may come from:
        ...
        //
        // At this point, we only have values for (1), while values for
        // (2) and (3) are defined later when the user selects the full
        // ResourceName (i.e. sub-type & attrSet).
        // So parameters/constraints for (2) and (3) must be kept in the
        // inventory.
        //
        // The inventory has 'baseConstraints' which all data in the
        // inventory must pass. And it has 'inventoryConstraints' which
        // will define the constraints that each node at a given level
        // must pass.
        //
        HashMap<String, ResourceParamInfo> rscImplParams = ResourceExtPointMngr
                .getInstance().getResourceParameters(rscImplementation);
        ...
        // inventory.
        // This will be all the request constraint params that don't have a
        // specified value for this resource (such as any type or sub-type
        // generating params and any parameters that are specified in an
        // attribute set; e.g. radar product codes and satellite imageTypes).
        //
        ...
        baseConstraints.put("pluginName",
                new RequestConstraint(getPluginName()));
        // inventoryConstraints.put( "pluginName", new RequestConstraint(
        // getPluginName() ) );
        ...
        // loop thru all of the request constraint parameters defined for the
        // implementation and add it to either the base constraints or the
        ...
        for (ResourceParamInfo prmInfo : rscImplParams.values()) {
            if (prmInfo.getParamType() == ResourceParamType.REQUEST_CONSTRAINT) {
                ...
                if (paramValues.containsKey(prmInfo.getParamName())) {
                    ...
                    String prmValue = paramValues.get(prmInfo.getParamName())
                            .trim();

                    // if the parameter value is a wildcard or a non-constant
                    // value (e.g. a list of reportTypes), then we will need
                    // to put this parameter in the inventory
                    //
                    if (prmValue.indexOf(',') != -1) {
                        inventoryParamNames.add(prmInfo.getConstraintName());
                        baseConstraints.put(prmInfo.getConstraintName(),
                                new RequestConstraint(prmValue,
                                        ConstraintType.IN));
                ...
                } else { // if there is no parameter value given add to the
                         // inventoryConstrains
                    inventoryParamNames.add(prmInfo.getConstraintName());
            ...
            // sanity check : if the parameter is not given in the attribute set
            // then we won't be able to instantiate the resourceData.
            // These should be NON_EDITABLE_ATTRIBUTES
            else if (prmInfo.getParamType() == ResourceParamType.IMPLEMENTATION_PARAM
                    && (prmInfo.getDefaultValue() == null || prmInfo
                            .getDefaultValue().isEmpty())) {

                if (!paramValues.containsKey(prmInfo.getParamName())) {
                    throw new VizException("Error initializing rscDefn "
                            + getResourceDefnName()
                            + ": a value is not specified for parameter "
                            + prmInfo.getParamName());
                }
            }
        }

        // Make sure that the inventory tree is ordered with the pluginName
        // first followed by the generating params and any defined in the
        // attrSets and with the dataTimes as end nodes.
        //
        // inventoryParamNames = new ArrayList<String>(
        // inventoryConstraints.keySet() );
        ...
        Collections.sort(inventoryParamNames, new Comparator<String>() {
            ...
            public int compare(String s1, String s2) {
                ...
                // radar has elevation and productCode parameters in the
                // attrSets
        ...
        return new NcInventoryDefinition(inventoryName, baseConstraints,
                inventoryParamNames);
    }
    ...
    public ArrayList<String> getGeneratedTypesList() throws VizException {
        ...
        if (!usesInventory()) {
            ...
            // {
            // out.println("RscDefn "+ getResourceDefnName() +
            // " has a generating type "+
        ...
        //
        Map<String, RequestConstraint> searchConstraints = getInventoryConstraintsFromParameters(getResourceParameters(true));

        // Create an NcInventoryRequest script to get query the ncInventory for
        // the types.
        //
        // String inventoryName =
        ...
        // TODO : Need to change this to return the constraint field instead of
        // the generator parameter. Til then all parameters that generate
        ...
        for (String rsltStr : rsltsArray) {
            ...
            generatedTypesList.add(getResourceDefnName() + ":" + rsltStr);// genTypesArr[
                                                                          // genTypesArr.length-1]
                                                                          // );
        }
    ...
    public ArrayList<String> generatedSubTypesList() throws VizException {
        ...
        // Note that if the inventory is at first enabled and then disabled,
        // generatedSubTypesList will have been populated by the inventory
        // and not queryGeneratedTypes. Also, any new sub types ingested
        ...
        // The parameters used to define the data that gets stored in the
        // inventory. This will be all the request constraint params that
        // don't have a specified value for this resource. This will be
        // any type or sub-type generating params and any parameters that
        // are specified in an attribute set. (e.g. radar product codes
        // and satellite imageTypes.)
        //
        Map<String, RequestConstraint> searchConstraints = getInventoryConstraintsFromParameters(getResourceParameters(true));

        // get the names of the db fields which may be different than the
        // resource's parameter names.
        ...
        for (int i = 0; i < subTypeParamNames.length; i++) {
            ...
            if (rscImplParams.containsKey(subTypeParam)) {
                queryParams[i] = rscImplParams.get(subTypeParam)
                        .getConstraintName();
            }
        }

        // Create an NcInventoryRequest script to get query the ncInventory
        // for the types.
        //
        NcInventoryRequestMsg reqMsg = NcInventoryRequestMsg.makeQueryRequest();
        reqMsg.setInventoryName(inventoryAlias);
        ...
        if (!(rslts instanceof String[])) {
            ...
            throw new VizException(
                    "Inventory Request Error: String[] response expecting instead of "
        ...
        for (Object queryRslt : rsltsArray) {
            ...
            if (queryResults.length != queryParams.length) {
                System.out
                        .println("sanity check: number of requested params doesn't match returned values");
                continue;
            }
    ...
    //
    public List<String> queryInventoryParameter(ResourceName rscName,
            String invPrm) throws VizException {
        if (!getInventoryEnabled()) {
        ...
        // LocalizationManager.getInstance().getCurrentUser() + ":" +
        // getResourceDefnName();
        // this will create constraints for all of the REQUEST_CONSTRAINT
        // parameters
        // specified in the RD, AS, and generated type/subType in the name.
        //
        HashMap<String, RequestConstraint> searchConstraints = getInventoryConstraintsFromParameters(ResourceDefnsMngr
    ...
    private void addTimesCacheEntry(ResourceName rscName) throws VizException {
        ...
        // possible resources.
        // rscNamesList =
        // ResourceDefnsMngr.getInstance().getAllSelectableResourceNamesForResourcDefn(
        // this );

        // for( ResourceName rName : rscNamesList ) {
        ResourceName rName = rscName;

        Map<String, RequestConstraint> resourceConstraints = getInventoryConstraintsFromParameters(ResourceDefnsMngr
                .getInstance().getAllResourceParameters(rName));
        ...
        if (!availTimesCache.containsKey(resourceConstraints)) {
            availTimesCache.put(resourceConstraints, new DataTimesCacheEntry(
                    resourceConstraints));
        }
        // }

        // There can be more than 1 entry in the cache for each RD. Generated
        // types/sub-types for example, or for cases where the attribute sets
        // contain request constraint parameters (i.e. synop reportTypes or
        // mcidas imageTypes.)

        // I pulled 4 out of my arse. Not sure what the best number is.
        // Even though the inventory is very quick, getLatestTime() can get
        // called many times by the LabelProvider when showing the attrSet
        // List and there can be a slight delay for example with all (40)
        // radar products.
        //
        //
        if (availTimesCache.size() == 4) {
            for (DataTimesCacheEntry uriRefreshCallback : availTimesCache
                    .values()) {
    ...
    public DataTime getLatestDataTime(ResourceName rscName) throws VizException {
        ...
        // if times are cached for these constraints, and if the times
        // haven't expired, then just return the cached times.
        //
        ...
        if (latestTime == null) {
            ...
            if (!usesInventory()
                    || (getInventoryEnabled() && !isInventoryInitialized())) {
    ...
    public List<DataTime> getDataTimes(ResourceName rscName)
        ...
        } else if (getInventoryEnabled() && !isInventoryInitialized()) {
            throw new VizException("Inventory Not Initialized.");
        }
        ...
        if (!availTimesCache.containsKey(resourceConstraints)) {
            // if there is no entry in the cache for this resourceName, add an
            // entry and possibly add it to the URICatalog
        ...
        DataTimesCacheEntry cachedTimesEntry = availTimesCache
        ...
        try {
            ...
            if (getInventoryEnabled()) {
                ...
                reqMsg.setReqConstraintsMap((HashMap<String, RequestConstraint>) resourceConstraints);
                reqMsg.setUniqueValues(true);
                ...
                for (int i = 0; i < rsltsList.length; i++) {
                    dataTimeArr[i] = (rsltsList[i] == null ? new DataTime(
    ...
    public String dumpInventory() throws VizException {
        if (!usesInventory() || !getInventoryEnabled()) {
            throw new VizException("Inventory not enabled.");
        } else if (!isInventoryInitialized()) {
            throw new VizException("Inventory Not Initialized.");
        }
        ...
        if (!(rslts instanceof String)) {
            ...
            throw new VizException(
                    "Inventory Dump Request Error: String response expecting instead of "
                            + rslts.getClass().getName());
        }

        long t02 = System.currentTimeMillis();

        out.println("Inventory Dump for " + resourceDefnName + " took "
                + (t02 - t01) + "msecs ");
        return (String) rslts;
    }
    ...
    public void queryGeneratedTypes() throws VizException {
        ...
        try {
            ...
            //
            // int numSubTypeGenParams = getSubTypeGenParamsList().length;
            // String queryParam = getSubTypeGenParamsList()[
            // numSubTypeGenParams-1 ];

            HashMap<String, RequestConstraint> requestConstraints = getInventoryConstraintsFromParameters(getResourceParameters(false));

            // requestConstraints.put( "pluginName", new RequestConstraint(
            // getPluginName() ) );

            if (!getRscTypeGenerator().isEmpty()) {

                String genTypesRslts[] = CatalogQuery.performQuery(
                        getRscTypeGenerator(), requestConstraints);
    ...
    // if the inventory is disabled then this will be called to update the
    // generating types or subTypes from the ingested URIs
    @Override
    public void alertArrived(Collection<AlertMessage> alertMessages) {

        HashMap<String, RequestConstraint> baseConstraints = getInventoryConstraintsFromParameters(getResourceParameters(false));

        for (AlertMessage alrtMsg : alertMessages) {
            Map<String, Object> uriAttrValues = new HashMap<String, Object>(
                    alrtMsg.decodedAlert);
            ...
            for (String attrName : uriAttrValues.keySet()) {
                ...
                if (attribsObj instanceof String
                    ...
                    // alrtMsg.dataURI +
                    // "\n     Attribute "+ attrName +" has a space : '"+
                    // (String)attribsObj +"'" );
                    // }
                }
            }

            // check to see if this URI matches the 'base' constraints (ie.
            // satelliteName, modelName) for this RD.
            boolean useUri = true;
            ...
            if (!getRscTypeGenerator().isEmpty()) {
                if (!uriAttrValues.containsKey(getRscTypeGenerator())) {
                    out.println("??? dataUriNotification, " + alrtMsg.dataURI
                            + ", doesn't contain TypeGenerator,"
                            + getRscTypeGenerator());
                    useUri = false;
                } else {
                    String genType = getResourceDefnName() + ":"
                            + uriAttrValues.get(getRscTypeGenerator());
            ...
            for (String genPrm : getSubTypeGenParamsList()) {
                ...
                if (!uriAttrValues.containsKey(genPrm)) {
                    out.println("??? dataUriNotification, " + alrtMsg.dataURI
                            + ", doesn't contain SubTypeGenerator," + genPrm);
                    useUri = false;
                }
            }
