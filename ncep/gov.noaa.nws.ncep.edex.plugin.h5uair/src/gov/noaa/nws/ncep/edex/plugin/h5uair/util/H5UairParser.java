...
public class H5UairParser {
     ...
     * @return
     */
	public static void getLevels(String codeMessage, H5UairRecord record) {
	...
	public static void processTTAACC(String[] codeGroupAr, int cgSize, 
			Boolean windKnot, H5UairRecord record) {
			...
			if ( pp.equals("88") || pp.equals("77") || pp.equals("66") ) {
				...
				if ( range < cgSize ) {
					...
					if ( pres >= topWind ) {
						...
						wspeed = WindGroup.getSped();
						wdir = WindGroup.getDrct();
						if ( wdir > 360 ) {
	...
	public static void processTTBBDD(String[] codeGroupAr, 
			int cgSize, Boolean windKnot, H5UairRecord record) {
	...
	public static void processTropopause(String[] codeGroupAr, int index, 
			Boolean topwindFlag, Boolean windKnot, H5UairRecord record, Boolean above) {
	...
	public static void processMaximumWindShear(String[] codeGroupAr, 
			int index, int cgSize, Boolean windKnot, H5UairRecord record) {
	...
	public static void processLiftedIndex(String[] codeGroupAr, 
			int index, Boolean windKnot, H5UairRecord record) {
		...
		if ( codeGroupAr[i].equals("10194") ) {
			...
			WindGroup.WindField(codeGroupAr[i+1], windKnot);
			wspeedSurface = WindGroup.getSped();
			wdirSurface = WindGroup.getDrct();

			// Decode wind group from 5000ft to 10000ft
			WindGroup.WindField(codeGroupAr[i+2], windKnot);
			wspeedAbove = WindGroup.getSped();
			wdirAbove = WindGroup.getDrct();
			
			H5LiftedIndex li = null;
			li = new H5LiftedIndex();
			li.setLiTemp(liTemp);
	 ...
	 * @return
	 */
	public static void addLevels(H5UairRecord record, float pres, float geoHeight,
			float temp, float dwpt, float drct, float sped) {
		
		H5ObsLevels level = null;
		level = new H5ObsLevels();
		level.setPres(pres);
	 ...
	 * @return
	 */
	public static void addTrop(H5UairRecord record, float pres,
			float temp, float dwpt, float drct, float sped) {
		
		H5Tropopause trop = null;
		trop = new H5Tropopause();
		trop.setPres(pres);
		trop.setTemp(temp);
		trop.setDwpt(dwpt);
		trop.setDrct(drct);
