...
import gov.noaa.nws.ncep.common.dataplugin.uair.LiftedIndex;
...
import gov.noaa.nws.ncep.edex.plugin.uair.util.WindGroup;
import gov.noaa.nws.ncep.common.tools.IDecoderConstantsN;
import java.util.regex.Matcher;
import java.util.regex.Pattern;
import java.util.*;

public class UairParser {
     ...
     * @return
     */
	public static void getLevels(String codeMessage, UairRecord record) {
		...
		while (sc.hasNext()) {
			...
			//System.out.println("code group=" + codeGroup);
			codeGroupAr[cgSize] = codeGroup;
			cgSize++;
		}
		...
		default: {
	    	System.out.println("\n DATA TYPE is INVALID and Will NOT process****");
		}
		}
	}
	...
	public static void processTTAACC(String[] codeGroupAr, int cgSize, 
			Boolean windKnot, UairRecord record) {
		...
		int level = 0;
		int topwind = IDecoderConstantsN.INTEGER_MISSING;
		
		String dataType = record.getDataType();
		String stationNumber = record.getStationNumber();

		if ( dataType.substring(2,4).equals("CC") ) {
			above = true;
		}
		if ( dataType.substring(0,2).equals("XX") || dataType.substring(0,2).equals("UU") ) {
			topwind = ShipMobile.getTopwind();
			if ( dataType.substring(0,2).equals("XX") ) {
				drop = true;
			}
		} else {
			topwind = TimeGroup.getTopwind();
		}
		if ( topwind == IDecoderConstantsN.INTEGER_MISSING ) {
			topwindFlag = true;
		}
		...
		while ( i < cgSize && ! endrpt ) {
			...
			if ( pp.equals("88") || pp.equals("77") || pp.equals("66") ) {
				...
				if ( range < cgSize ) {
					...
					PressureHeightGroup.PressureHeightField(codeGroupAr[i], above, level, stationNumber, dataType, record);
					pres = PressureHeightGroup.getPressure();
					height = PressureHeightGroup.getHeight();

					// Decode temperature group
					TempGroup.TempField(codeGroupAr[i+1]);
					temp = TempGroup.getTemperature();
					dewpTemp = TempGroup.getDewpointTemp();

					if ( pres >= topWind ) {
						// Decode wind group
						WindGroup.WindField(codeGroupAr[i+2], windKnot);
						wspeed = WindGroup.getWindSpeed();
						wdir = WindGroup.getWindDirection();
						if ( wdir > 360 ) {
					...
					//System.out.println("pres=" + pres + " temp=" + temp +
					//		" dewptemp=" + dewpTemp + " wdir=" + wdir + " wspeed=" + wspeed
					//		+ " height=" + height );
				}
				i = i + 3;
				level++;
	...
	public static void processTTBBDD(String[] codeGroupAr, 
			int cgSize, Boolean windKnot, UairRecord record) {
			...
			if ( codeGroupAr[i].equals("31313")) {
				...
				if ( range < cgSize ) {
					...
					if ( ! wind ) {
						...
						// Decode wind group
						WindGroup.WindField(codeGroupAr[i+1], windKnot);
						wspeed = WindGroup.getWindSpeed();
						wdir = WindGroup.getWindDirection();
					}
		...
		while ( i < cgSize && ! endrpt ) {
			...
			if ( ppIndicator.equals("44") || ppIndicator.equals("55")) {
				...
				if ( skip.equals("///")) {
					...
					int onetothree = Integer.parseInt(codeGroupAr[i].substring(2,3));
					...
					if ( onetothree > 0 && onetothree <= 3) {
						...
						for (int index = 0; index < onetothree; index++) {
							...
							// Decode wind group
							if ( i < cgSize ) {
								WindGroup.WindField(codeGroupAr[i], windKnot);
								wspeed = WindGroup.getWindSpeed();
								wdir = WindGroup.getWindDirection();
							}
		...
		while ( i < cgSize && ! endrpt ) {
			...
			if ( ! pressflag ) {
				while ( ! missing && ( noWind < 3 ) ) {
					int index = noWind + 3;
					String unit = codeGroupAr[i].substring(index-1,index);
					if ( unit.equals(("/")) ) {
						missing = true;
						break;
					}
					...
					if ( range < cgSize ) {
						// Decode wind group.
						WindGroup.WindField(codeGroupAr[i+noWind+1], windKnot);
						wspeed = WindGroup.getWindSpeed();
						wdir = WindGroup.getWindDirection();
					}
	...
	public static void processTropopause(String[] codeGroupAr, int index, 
			Boolean topwindFlag, Boolean windKnot, UairRecord record, Boolean above) {
	...
	public static void processMaximumWindShear(String[] codeGroupAr, 
			int index, int cgSize, Boolean windKnot, UairRecord record) {
		...
		if ( ppIndicator.equals("77") || ppIndicator.equals("66")) {
			...
			MaxWind mwind = null;
			mwind = new MaxWind();
			mwind.setPressure(pres);
			mwind.setWindDirection(wdir);
			mwind.setWindSpeed(wspeed);
	...
	public static void processLiftedIndex(String[] codeGroupAr, 
			int index, Boolean windKnot, UairRecord record) {
		...
		if ( codeGroupAr[i].equals("10194") ) {
			...
			WindGroup.WindField(codeGroupAr[i+1], windKnot);
			wspeedSurface = WindGroup.getWindSpeed();
			wdirSurface = WindGroup.getWindDirection();

			// Decode wind group from 5000ft to 10000ft
			WindGroup.WindField(codeGroupAr[i+2], windKnot);
			wspeedAbove = WindGroup.getWindSpeed();
			wdirAbove = WindGroup.getWindDirection();
			
			LiftedIndex li = null;
			li = new LiftedIndex();
			li.setLiTemp(liTemp);
			li.setLoWindDir(wdirSurface);
			li.setLoWindSpeed(wspeedSurface);
			li.setHiWindDir(wdirAbove);
			li.setHiWindSpeed(wspeedAbove);
	 ...
	 * @param geoHeight The input geoHeight
	 * @param temp The input temp
	 * @param dwpt The input dew point temperature.
	 * @param windDirection The input wind direction
	 * @param windSpeed The input wind speed
	 * @return
	 */
	public static void addLevels(UairRecord record, float pressure, float geoHeight,
			float temp, float dwpt, float windDirection, float windSpeed) {
		
		ObsLevels level = null;
		level = new ObsLevels();
		level.setPressure(pressure);
		level.setGeoHeight(geoHeight);
		level.setTemp(temp);
		level.setDwpt(dwpt);
		level.setWindDirection(windDirection);
		level.setWindSpeed(windSpeed);
	 ...
	 * @return
	 */
	public static void addTrop(UairRecord record, float pressure,
			float temp, float dwpt, float windDirection, float windSpeed) {
		
		Tropopause trop = null;
		trop = new Tropopause();
		trop.setPressure(pressure);
		trop.setTemp(temp);
		trop.setDwpt(dwpt);
		trop.setWindDirection(windDirection);
		trop.setWindSpeed(windSpeed);
