...
public class PressureHeightGroup {
	...
	public static void PressureHeightField(String presGroup, Boolean above, 
			int level, String stationNumber, String dataType, UairRecord record) {
		
		pressure = IDecoderConstantsN.UAIR_FLOAT_MISSING;
		height = IDecoderConstantsN.UAIR_FLOAT_MISSING;
		int iabove = 0;
		int ipres = IDecoderConstantsN.INTEGER_MISSING;
		int ihhh = IDecoderConstantsN.INTEGER_MISSING;
		Boolean drop = false;
		Boolean ship = false;
		...
		if ( presGroup.length() == 5 && ( level < 12 )) {
			...
			if (pp.equals(pressValue[iabove][level])) {
				...
				if (! above) {
					 ...
					 */
					if ( ipres != IDecoderConstantsN.INTEGER_MISSING ) {
						ipres = ipres * 10;
						if ( ipres == 0 ) {
							ipres = 1000;
						}
					 ...
					 */
					if ( ihhh != IDecoderConstantsN.INTEGER_MISSING ) {
						height = ihhh;
				...
				if ( pp.equals("99") && ! above ) {
					...
					if ( ! drop && stationNumber != null ) {			
						height = IDecoderConstantsN.INTEGER_MISSING;
