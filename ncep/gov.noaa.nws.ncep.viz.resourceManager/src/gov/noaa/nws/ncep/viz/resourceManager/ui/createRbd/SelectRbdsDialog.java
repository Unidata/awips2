...
/**
 * Dialog displayed from RBD Mngr window when the 'Import RBD...' is selected.
 * This lets the user choose which RBD or which active editor to modify.
 * 
 * <pre>
 * SOFTWARE HISTORY
...
public class SelectRbdsDialog extends Dialog {
    ...
    public SelectRbdsDialog(Shell parShell, String titleStr,
        ...
        dlgTitle = titleStr;// (importSinglePane ? "Import Pane" :
                            // "Import Display" );
        ...
        pane_combo.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                ...
                if (seldRbdsList.size() == 1) {
                    // Note that if selecting panes is allowed, multi-select is
                    // not and so there will only be one RBD in the list
    ...
    private void setSeldSpfGroup(String newSpfGroup) {
        ...
        spfNameCombo.setItems(SpfsManager.getInstance().getSpfNamesForGroup(
                seldSpfGroup));
    ...
    private void setSeldSpfName(String newSpfName) {
        ...
        try {
            rbdBndls = SpfsManager.getInstance().getRbdsFromSpf(seldSpfGroup,
                    seldSpfName, false); // don't resolve Latest Cycle Times
    ...
    private void setSelectedRBDs() {
        StructuredSelection sel_rbds = (StructuredSelection) rbdLviewer
                .getSelection();
        if (sel_rbds.isEmpty()) {
            seldRbdsList.clear();
            ...
            if (seldRbdsList.size() == 1) {
                ...
                for (int paneIndx = 0; paneIndx < seldRbd.getPaneLayout()
                        .getNumberOfPanes(); paneIndx++) {
                    // for( int c=0 ; c<seldRbd.getPaneLayout().getColumns() ;
                    // c++ ) {
                    pane_combo.add(seldRbd.getPaneLayout()
                            .createPaneId(paneIndx).toString());
                    // }
                }
    ...
    private ArrayList<AbstractRBD<?>> getRbdsFromAllDisplays() {
        ...
        for (AbstractEditor ncDisplay : allNcDisplays) {
            try {
                AbstractRBD<?> rbdFromDisplay = AbstractRBD
                        .createRbdFromEditor(ncDisplay);

                rbdFromDisplay.setIsDefaultRbd(false);
