...
public class RbdViewComposite extends Composite {
    ...
    public RbdViewComposite(Composite parent) {
        ...
        rscLviewer = new ListViewer(topComp, SWT.SINGLE | SWT.BORDER
        ...
        rscLviewer.setContentProvider(new IStructuredContentProvider() {
            ...
            public Object[] getElements(Object inputElement) {
                ...
                for (int paneIndx = 0; paneIndx < selRbd.getPaneLayout()
                        .getNumberOfPanes(); paneIndx++) {
                    INcPaneID paneId = selRbd.getPaneLayout().createPaneId(
                            paneIndx);

                    INatlCntrsRenderableDisplay disp = selRbd
                            .getDisplayPane(paneId);
                    AbstractDescriptor mapDescr = (AbstractDescriptor) disp
                            .getDescriptor();

                    boolean showPane = (!viewSelectedPane || (viewSelectedPane && (paneId
                            .compareTo(selRbd.getSelectedPaneId()) == 0)));

                    // show this pane only if its a single pane or if this is
                    // the
                    if (showPane) {
                        ...
                        PredefinedArea area = (PredefinedArea) disp
                                .getInitialArea();

                        if (area.getSource() == AreaSource.PREDEFINED_AREA) {
                            rscNames.add("Predefined Area "
                                    + area.getAreaName());
                        } else {
                        ...
                        for (ResourcePair rp : mapDescr.getResourceList()) {
                            if (rp.getResourceData() instanceof INatlCntrsResourceData) {
                                ...
                                String rscName = "";
                                if (ncRsc instanceof GroupResourceData) {
                                    rscName = ((GroupResourceData) ncRsc)
                                            .getGroupName();
                                } else {
                                    rscName = ncRsc.getResourceName()
                                            .toString();
                                }
