 ...
 * ------------ ----------  ----------- --------------------------
 * 09/20/2011   458			sgurung	    Main pdo class
 * 09/23/2011   458			sgurung	    Converted to HDF5
 * 09/29/2011               sgurung     Added reportType
 * 10/19/2011               sgurung     Set dataTime with correct valid period
 * 10/26/2011               sgurung     Added probable parameters (for TEMPO/PROB groups). 
 * 										Split change groups into hourly records.
...
@XmlRootElement
public class NcTafRecord extends PluginDataObject implements ISpatialEnabled, IDecoderGettable, IPointData, IPersistable {
    ...
    public static float findTafCeilingFromLayers(Set<NcTafSkyCover> skyCov) {
     ...
     public static NcTafRecord[] split(NcTafBulletinRecord bulletin) {
        ...
        if ( bulletin.getChangeGroups () != null ) {
    		Iterator<NcTafChangeGroup> chgGrps = bulletin.getChangeGroups().iterator();
    		if (chgGrps != null) {
     ...
     public static List<NcTafRecord> splitToHourlyRecords(List<NcTafRecord> records) {
