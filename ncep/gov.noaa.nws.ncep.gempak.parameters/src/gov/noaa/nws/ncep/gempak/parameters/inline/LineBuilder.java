 ...
 *  07-Jun-2010    186       Archana.S   Renamed the package as
...
public class LineBuilder {
    ...
    **/
	...
	private List<LinePatterns> linePatternsList;
	
@SuppressWarnings("unused")
    
    /** An instance of the <code> LinePatterns </code> enumeration*/
	private LinePatterns       linePattern;
	
    /** The default line pattern is SOLID*/
    private LinePatterns       defaultLinePattern;
	
    /**A HashMap of the Integer values and the corresponding <code> LinePatterns </code> */
    protected HashMap<Integer,LinePatterns> linePatternValuesHashMap;
    
    /**The<code> LinePatterns </code> enumeration*/
	protected enum LinePatterns{
									SOLID(1),
									SHORT_DASHED(2),
									MEDIUM_DASHED(3),
	...
	protected LineBuilder(){
        ...
        defaultColor                      = 1; 
        defaultLinePattern                = LinePatterns.SOLID;
        defaultLineWidth                  = 1;
		...
		linePatternsList                  = new ArrayList<LinePatterns>();
		...
		linePatternValuesHashMap = new HashMap<Integer,LinePatterns>();
		/**
		 *@see <code>LineBuilder.LinePatterns </code>
		 **/
		linePatternValuesHashMap.put(1, LinePatterns.SOLID);
		linePatternValuesHashMap.put(2, LinePatterns.SHORT_DASHED);
		linePatternValuesHashMap.put(3, LinePatterns.MEDIUM_DASHED);
		linePatternValuesHashMap.put(4, LinePatterns.LONG_DASH_SHORT_DASH);
		linePatternValuesHashMap.put(5, LinePatterns.LONG_DASH);
		linePatternValuesHashMap.put(6, LinePatterns.LONG_DASH_THREE_SHORT_DASHES);
		linePatternValuesHashMap.put(7, LinePatterns.LONG_DASH_DOT);
		linePatternValuesHashMap.put(8, LinePatterns.LONG_DASH_THREE_DOTS);
		linePatternValuesHashMap.put(9, LinePatterns.MEDIUM_DASH_DOTS);
		linePatternValuesHashMap.put(10,LinePatterns.DOTTED);		
	 ...
	 * The method <code>setLinePatternsList</code> accepts a list of Integers as input.
	 * For each member of the list, it checks if a corresponding member of the <code>LinePatterns</code> 
	 * enumeration exists in the HashMap and if so, it adds this LinePattern to the list of LinePatterns.
	 */
	public void setLinePatternsList(List<Integer> linePatternList) {
		int currentIndex;
		int patternType;
		...
		for(currentIndex = 0;currentIndex < linePatternList.size();currentIndex++){
			...
			if(this.linePatternValuesHashMap.containsKey(patternType)){
	    		 this.linePatternsList.add(currentIndex,
	...
	/**
	 * @return List of <code>LinePatterns</code> - linePatternsList
	 */
	public List<LinePatterns> getLinePatternsList() {
		return linePatternsList;
	}
		...
		if (lineAttributesList.size() > 0) {
			switch (lineAttributeType) {
				...
				case 1:
					...
					this.setLinePatternsList(lineAttributesList);
	 ...
	 *@param List of LinePatterns - patternList
	...
	protected void setDefaultAttributesForLinesWithMissingAttributes(List<Integer>        colorList,
		...
		this.addDefaultLinePatternsAttribute(numPatternElementsToBeAdded);
	 ...
	 *<p>
	 *The method <code>addDefaultLinePatternsAttribute</code> is invoked by the method
	 * <code>setDefaultAttributesForLinesWithMissingAttributes</code>, with an input 
	 * parameter that decides the number of default elements to be added to the list
	 * of line patterns.  
	 **/
	private void addDefaultLinePatternsAttribute(int numDefaultPatternsToAdd){
		int count = 0;
		while(count < numDefaultPatternsToAdd){
			this.getLinePatternsList().add(this.defaultLinePattern);
			count++;
		}
	}
