...
//	}
	...
	public PgenSelectHandler( AbstractPgenTool tool, AbstractEditor mapEditor, PgenResource resource,
					AttrDlg attrDlg){
    ...
    public boolean handleMouseDown(int anX, int aY, int button) { 
    	...
    	if ( button == 1 ) {
    		...
    		if ( pgenrsc.getSelectedDE() == null ) ptSelected = false;
    		...
    		if ( ptSelected || pgenrsc.getSelectedDE() != null ) {
    			...
    			if ( pgenrsc.getSelectedDE() instanceof SinglePointElement
    					&& pgenrsc.getDistance(pgenrsc.getSelectedDE(), loc) > pgenrsc.getMaxDistToSelect() ) {
    				ptSelected = false;   //prevent SPE from moving when selecting it and then click far away and hold to  move.
    			}
    			
    			if ( !(pgenrsc.getSelectedDE() instanceof SinglePointElement) && !ptSelected ){
    				firstDown = loc;
    			}
    	...
    	//	AbstractDrawableComponent adc = drawingLayer.getNearestComponent( loc );
    		...
    		if ( elSelected != null && elSelected.getParent() != null && 
    			...
    			adc = pgenrsc.getNearestComponent( loc, new AcceptFilter(), true );       			
    		}
...
//		    AbstractDrawableComponent adc = drawingLayer.getNearestComponent( loc, new AcceptFilter(), true );
    		...
    		if ( tool instanceof PgenContoursTool ) {
    			...
    			DECollection dec = ((PgenContoursTool)tool).getCurrentContour();
    			if ( dec != null ){
    				elSelected = pgenrsc.getNearestElement( loc, (Contours)dec );
    				if ( elSelected instanceof MultiPointElement ) { 
    					//ptSelected could be set to true if there is a SiglePointElement near the click, which is not part of the contour.
    					ptSelected = false;
    				}
    			}
    			...
    			pgenType = "Contours";
 		        ...
 		        if ( adc != null && adc instanceof Jet && tool instanceof PgenSelectingTool) {
    			    ...
    			    if(  adc.getPrimaryDE() == elSelected){
    			        ...
    			        if ( ((Jet)adc).getSnapTool() == null ){
    			            ((Jet)adc).setSnapTool( new PgenSnapJet(pgenrsc.getDescriptor(), mapEditor, null));
    			        }
    			    }
   		        }
            ...
            if ( pgCategory != null ){
            	...
            	if ( attrDlg != null && !(attrDlg instanceof ContoursAttrDlg && tool instanceof PgenContoursTool)) {
            	...
            	if ( attrDlg instanceof JetAttrDlg && tool instanceof PgenSelectingTool ){
            		((JetAttrDlg)attrDlg).setJetDrawingTool((PgenSelectingTool)tool);
            		((JetAttrDlg)attrDlg).updateSegmentPane();
            		if ( ((PgenSelectingTool)tool).getJet().getSnapTool() == null ){
            			((PgenSelectingTool)tool).getJet().setSnapTool( new PgenSnapJet(pgenrsc.getDescriptor(), mapEditor, (JetAttrDlg)attrDlg));
    				}
            	}	
    ...
    public boolean handleMouseDownMove(int x, int y, int button) { 
    	...
    	if ( dontMove  && pgenrsc.getSelectedDE() != null) return true;
    	//  Check if mouse is in geographic extent
    	Coordinate loc = mapEditor.translateClick(x, y);
    	//if ( loc == null ) return false;

    	DrawableElement tmpEl = pgenrsc.getSelectedDE();        	       	
    	if( PgenUtil.isUnmovable(tmpEl) ) return false;
    	...
    	if (loc != null && inOut == 1) {
    		...
    		if ( pgenrsc.getDistance(tmpEl, loc) > pgenrsc.getMaxDistToSelect() && !ptSelected ) {
    		//	if (( firstDown != null && pgenrsc.getDistance(tmpEl, firstDown) > drawingLayer.getMaxDistToSelect() && !ptSelected) ||
    		//			tmpEl instanceof SinglePointElement ){
    			if ( firstDown != null && pgenrsc.getDistance(tmpEl, firstDown) < pgenrsc.getMaxDistToSelect() ){
    				firstDown = null;
    			}
    			else {
    				return false;
    		...
    		if (tmpEl instanceof SinglePointElement ){
    			...
    			if ( tmpEl instanceof Jet.JetBarb ){
    				...
    				if ( jt != null ){
           			...
           			pgenrsc.resetADC( tmpEl.getParent() );  // reset display of the DECollecion.
			...
			if ( el != null ){
				...
				if ( el instanceof SinglePointElement ){
					...
					if ( oldLoc != null ) {
						...
						pgenrsc.resetElement(el);   // reset display of this element
		        	...
		        	if ( tool instanceof PgenSelectingTool && el instanceof Jet.JetLine && 
		        			((PgenSelectingTool)tool).getJet().getPrimaryDE() == el){
						Jet newJet = ((PgenSelectingTool)tool).getJet().copy();
						pgenrsc.replaceElement(((PgenSelectingTool)tool).getJet(), newJet);
