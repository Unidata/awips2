...
public class ContoursAttrDlg extends AttrDlg implements IContours, SelectionListener, ILine {
	 ...
	 */
    private static enum ContourDrawingStatus {
        DRAW_LINE, DRAW_SYMBOL, DRAW_CIRCLE
    }
	...
	LineTypeSelectionDlg		lineTypePanel = null;
	SymbolTypeSelectionDlg		symbolTypePanel = null;
	...
	/**
	 * Creates a Contours attribute dialog if the dialog does not exist 
	...
	public Control createDialogArea(Composite parent) {
	    ...
	    layout2.horizontalSpacing = 20;
	    lineComp.setLayout( layout2 );
        		    
        Composite closelineComp = new Composite( lineComp, SWT.NONE ); 
	    closelineComp.setLayout( new GridLayout( 3, false ) );

	    drawLineBtn = new Button( closelineComp, SWT.CHECK );;
        drawLineBtn.setToolTipText("Check to start drawing a line");
		drawLineBtn.addSelectionListener( new SelectionAdapter() {
	        public void widgetSelected(SelectionEvent event) {    			
                    setDrawingStatus( ContourDrawingStatus.DRAW_LINE );
	            }
	        }); 
        drawLineBtn.setSelection( true );              
        ...
        editlineComp.setLayout( new GridLayout( 2, false ) );
		...
		lineAttrBtn.addListener(SWT.MouseDown, new Listener(){
			 ...
			 public void handleEvent(Event event) {
				 try {
					 if ( lineAttrDlg == null ){
						 lineAttrDlg = new ContourLineAttrDlg(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell());
					 }
                     ...
                     if ( de != null && de.getParent() != null && de.getParent() instanceof ContourLine ) {
                    	 ...
                    	 if ( lineTemplate == null ) {
                    		 lineTemplate = (Line)contoursAttrSettings.get( retrieveDefaultLineType() );
        ...
        if ( numOfQuickSymbols == 1 ) {
        	...
        	layoutm.horizontalSpacing = 2;
        }

        quickSymbolComp.setLayout( layoutm );	    
       	
	    Composite quickSymbolTypeComp = new Composite( quickSymbolComp, SWT.NONE ); 
	    quickSymbolTypeComp.setLayout( new GridLayout( 2, false ) );

	    drawSymbolBtn = new Button( quickSymbolTypeComp, SWT.CHECK );;
        drawSymbolBtn.setToolTipText("Check to start drawing a symbol");
		drawSymbolBtn.addSelectionListener( new SelectionAdapter() {
	        public void widgetSelected(SelectionEvent event) {    			
	    ...
	    quickComp.setLayout( new GridLayout( 3, false ) );
			...
			if (  quickSymbolType.get( str ) && ii < MAX_QUICK_SYMBOLS ) {
		        ...
		        btn.addListener( SWT.MouseDown, new Listener() {
				    ...
				    public void handleEvent(Event event) {
				    	...
				    	if ( btnClicked == activeQuickSymbolBtn ) {
				    		openSymbolPanel( );	
				    	}
				    	else {
					        ...
					        for ( Button b : quickSymbolBtns ) {
					            if ( b ==  activeQuickSymbolBtn ) {
					            	DrawableElement de = (SinglePointElement)contoursAttrSettings.get( b.getData().toString() );														
					            	setButtonColor( b, defaultButtonColor, de.getColors()[0] );				    
					            }
					            else {
						             setButtonColor( b, activeButtonColor, defaultButtonColor );			     
					            }
					        }			     
				        }
				    }
		...
		DrawableElement de = (SinglePointElement)contoursAttrSettings.get( activeQuickSymbolBtn.getData().toString() );
		setButtonColor( activeQuickSymbolBtn, defaultButtonColor, de.getColors()[0] );				    
		
        Composite editSymbolAttrComp = new Composite( quickSymbolComp, SWT.NONE );   	
        editSymbolAttrComp.setLayout( new GridLayout( 2, false ) );
			 ...
			 public void handleEvent(Event event) {
				 try {
                     ...
                     if ( de != null && de instanceof Symbol && 
                    	 ...
                    	 minmaxTemplate = (Symbol)contoursAttrSettings.get( 
                    			               activeQuickSymbolBtn.getData().toString() );
                   	     if ( minmaxTemplate == null ) {
                             minmaxTemplate = new Symbol( null, new Color[] {Color.green},
                      			    2.0F, 2.0, true, null, "Symbol",  getActiveSymbolObjType() );     	
	    ...
	    layoutc.horizontalSpacing = 36;
	    circleComp.setLayout( layoutc );
        	
	    Composite circleTypeComp = new Composite( circleComp, SWT.NONE ); 
	    circleTypeComp.setLayout( new GridLayout( 3, false ) );

	    drawCircleBtn = new Button( circleTypeComp, SWT.CHECK );;
	    drawCircleBtn.setToolTipText("Check to start drawing a circle");
		drawCircleBtn.addSelectionListener( new SelectionAdapter() {
	        public void widgetSelected(SelectionEvent event) {    			
...
//      	        openSymbolPanel();
	    ...
	    hideCircleLabelBtn.setToolTipText("Check to hide the label for this circle");
	    
        Composite editCircleComp = new Composite( circleComp, SWT.NONE );   	
        editCircleComp.setLayout( new GridLayout( 2, false ) );
	    ...
	    textGrp.setLayout( new GridLayout( 1,  false ) );
        ...
        labelTxt = new Text( textValueComp,  SWT.SINGLE | SWT.BORDER );                        
        labelTxt.setLayoutData( new GridData( 50, 15 ) );
        ...
        editTextComp.setLayout( new GridLayout( 2, false ) );
			 ...
			 public void handleEvent(Event event) {
				 try {
					 if ( labelAttrDlg == null ){
						 labelAttrDlg = new LabelAttrDlg(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell());
					 }
                     ...
                     if ( de != null ) {
                    	 if ( de.getParent() instanceof ContourLine 
                    		 && ((ContourLine)(de.getParent())).getLabels().size() > 0 ) {
                        	 labelAttrDlg.setAttrForDlg( (IAttribute)((ContourLine)(de.getParent())).getLabels().get(0) );
                    	 }
                    	 else if ( de.getParent() instanceof ContourMinmax 
                    		 && ((ContourMinmax)(de.getParent())).getLabel() != null ) {
                        	 labelAttrDlg.setAttrForDlg( (IAttribute)((ContourMinmax)(de.getParent())).getLabel() );
                    	 }
                    	 else if ( de.getParent() instanceof ContourCircle 
                        		 && ((ContourCircle)(de.getParent())).getLabel() != null ) {
                            	 labelAttrDlg.setAttrForDlg( (IAttribute)((ContourCircle)(de.getParent())).getLabel() );
                         }
                     }
                     else {
                    	 
                         if ( labelTemplate == null ) {
        	                 labelTemplate = (gov.noaa.nws.ncep.ui.pgen.elements.Text)contoursAttrSettings.get( "General Text" );                       	 
                    	 }                        
        ...
        // Create a composite to display contour levels created on the value of CINT.      			
        ...
        editContourLineComp.setLayout( new GridLayout( 3, false ) );
        editContourLineComp.setLayoutData( new GridData(SWT.CENTER, SWT.DEFAULT, false, false) );		
        ...
        addLineBtn = new Button( editContourLineComp, SWT.PUSH );;
        addLineBtn.setText( ADD_CONTOURLINE );
        addLineBtn.setToolTipText( "Add a new contour component" ); 
        addLineBtn.addSelectionListener( this );
		setButtonColor( addLineBtn, defaultButtonColor );
...
//		}
	...
	private void createLabelBtns( Composite comp, boolean firstTime ) {
		...
		if ( firstTime ) {
			labelGrp = new Group( comp, SWT.SHADOW_ETCHED_OUT );
			labelGrp.setLayout( new GridLayout( 4, true ) );
		}
		else { 
		...
		if ( !firstTime ) this.getShell().pack();
	    ...
	    if ( cd.isCINTStringParsed() ) {
	    	for ( String dbl : cd.getContourLabelsForZoomLevel( CINT.FIRST_ZOOM_LEVEL ) ) {
	...
	public void widgetSelected( SelectionEvent e ) {
		...
		if ( e.widget instanceof Button ) {
			...
			if ( btnName.equals( SELECT_CONTOURLINE ) ) {
			    ...
			    PgenUtil.setSelectingMode( currentContours );

			}
			
			/*
			 * Draw and add a new contour line to the current contours
			 * and the notify the drawing tool.
			 */
			else if ( btnName.equals( ADD_CONTOURLINE ) ) {
								
			    PgenUtil.loadContoursTool( currentContours );
			...
			/*
			 * Remove the selected contour line or min/max from the contours
			 * and the notify the drawing tool.
			 */
				...
				if ( de != null && 
		            ...
		            if ( ((DECollection)oldContours).size() <= 1 ) {
				        ...
				        /*
				         *  Remove the selected contour line and copy 
				         *   over other non-selected contour lines.
				         */			
    ...
    public boolean drawContourLine() {    	
	    ...
	    return drawLineBtn.getSelection();
		           
    }
		
	/**
	 * Updates the selected contours and contour line, then redraws the PGEN layer.
	 */
	public void okPressed(){

		/*
		 * Create a new Contours as well as a new ContourLine with a new line and a label.
		 */
		DrawableElement de = drawingLayer.getSelectedDE();
		if ( de != null && de.getParent() != null && 
			...
			/*
			 *  Replace the selected contour line with a new contour line and copy 
			 *  over other non-selected contour lines.
			 */			
	        while ( iterator.hasNext() ) {    				        					        					        	
	        	...
	        	if ( oldAdc.equals( de.getParent() ) ) {
	                ...
	                if ( oldAdc instanceof ContourCircle ) {
	                    ...
	                    ((ContourCircle)newAdc).updateLabelString(new String[]{ getLabel() });        	        
	                    ((ContourCircle)newAdc).getLabel().setHide( hideCircleLabel() );        	        
	                }
	                    ...
	                    ((ContourLine)newAdc).getLine().setPgenType( getContourLineType() );        	        
	                    ((ContourLine)newAdc).getLine().setClosed( lineClosedBtn.getSelection() );        	        
       	        
		            	int nlabels = ((ContourLine)newAdc).getNumOfLabels();
	        	        if ( nlabels != getNumOfLabels() ) {
		            		
		            	    ((ContourLine)newAdc).updateNumOfLabels( getNumOfLabels() );
		            	    
		            	    if ( newEl instanceof gov.noaa.nws.ncep.ui.pgen.elements.Text ) {
		            	    	if ( ((ContourLine)newAdc).getLabels().size() > 0 ) {
		            	    		newEl = ((ContourLine)newAdc).getLabels().get( 0 );
		            	    	}
		            	    	else {
		            	    		drawingLayer.removeSelected();
		            	    		newEl = null;
	        	        ...
	        	        ((ContourLine)newAdc).updateLabelString(new String[]{ getLabel() }); 
	                    ...
	                    ((ContourMinmax)newAdc).getSymbol().setPgenCategory( getActiveSymbolClass() );
	                    ((ContourMinmax)newAdc).getSymbol().setPgenType( getActiveSymbolObjType() );
	                	
	                    ((ContourMinmax)newAdc).updateLabelString(new String[]{ getLabel() });
	                }
	...
	/**
	 * @param lineTemplate the lineTemplate to set
	 */
	...
	public IAttribute getMinmaxTemplate() {
		if ( minmaxAttrDlg != null && minmaxAttrDlg.getShell() != null ) {
			...
			return (IAttribute)contoursAttrSettings.get( activeQuickSymbolBtn.getData().toString() );
		}
	}

	/**
	 * @param minmaxTemplate the minmaxTemplate to set
	 */
	...
	private class LabelAttrDlg extends TextAttrDlg {
		...
		public void okPressed() {						
			 ...
			 */            
			labelTemplate = (gov.noaa.nws.ncep.ui.pgen.elements.Text)new DrawableElementFactory().create(DrawableType.TEXT, 
	...
	/**
	 * Updates the selected contours' label attributes, then redraws the PGEN layer.
	 */
	...
	private class ContourLineAttrDlg extends LineAttrDlg {
		...
		public void okPressed(){			
			 ...
			 */
			lineTemplate = (gov.noaa.nws.ncep.ui.pgen.elements.Line)new DrawableElementFactory().create(DrawableType.LINE, 
					this, "Line", "LINE_SOLID", (Coordinate)null, null);
	...
	/**
	 * Updates the selected contours' line attributes, then redraws the PGEN layer.
	 * Note: if 
	 */
	        ...
	        while ( iterator.hasNext() ) {    				        					        					        	
	        	...
	        	if ( newAdc instanceof ContourLine ) {
	        	    ...
	        	    if ( newEl != null && oldAdc.equals( de.getParent() ) ) {	                	        		
	        		    ...
	        		    ((ContourLine)newAdc).getLine().setClosed( isClosedLine() );	
    ...
    private void retrieveIconType() {    	
		...
		/* Bet all Combo Symbols
		 * Combo symbol's class "ComboSymbol" and symbol/marker's class
		 * is "Symbol", switching between them requires more detailed
		 * handling - so we do not use combos for now.
		 */
    }
    ...
    public boolean drawSymbol() {    	
	    ...
	    return drawSymbolBtn.getSelection();
    ...
    /**
     *  Set the active symbol's type and image to a selected one.
     *  If not found in the list, force to be the first one.
     */
    public void setActiveSymbol( DrawableElement elem ) {    			    	
    	...
    	for ( Button btn : quickSymbolBtns ) {
			...
			if ( symboltype.equals( btn.getData().toString() ) ) {								
                btn.setToolTipText( symbolItemMap.get( symboltype ).getAttribute( "label") );
    ...
    public String getActiveSymbolClass() {    	  	
	    return symbolItemMap.get( getActiveSymbolObjType() ).getAttribute( "className" );
	   			     ...
	   			     public void handleEvent(Event event) {
	   		        	 ...
	   		        	 if ( wids != null ) {
	   		       	         for ( int kk = 0; kk < wids.length; kk++ ) {
	   		       	             setButtonColor( (Button)wids[kk], defaultButtonColor );
	   		        	    } 
	   		        	 }
	   				     
	   		        	 String objstr = event.widget.getData().toString();
	   				     selectedType = objstr;	 
	   				     setButtonColor( (Button)event.widget, activeButtonColor );
	...
	private void openSymbolPanel() {
		...
		if ( symbolTypePanel == null ) {
		    ...
		    symbolTypePanel  = new SymbolTypeSelectionDlg( sh, objs, activeQuickSymbolBtn, 
		    		                        "Select Contours Quick Access Symbol" );
		}
	...
	private class ContourMinmaxAttrDlg extends LabeledSymbolAttrDlg {
		...
		public void okPressed(){			
			 ...
			 */
			minmaxTemplate = (gov.noaa.nws.ncep.ui.pgen.elements.Symbol)new DrawableElementFactory().create(DrawableType.SYMBOL, 
					(IAttribute)this, "Symbol", getActiveSymbolObjType(), (Coordinate)null, null);
			contoursAttrSettings.put( getActiveSymbolObjType(), minmaxTemplate );
			setButtonColor( activeQuickSymbolBtn, defaultButtonColor, this.getColors()[0] );
	...
	private void updateMinmaxAttributes() {
		...
		if ( de == null || de.getParent() == null || !(de.getParent() instanceof ContourMinmax ) ) {
			return;
		}
	        ...
	        while ( iterator.hasNext() ) {    				        					        					        	
	        	...
	        	if ( newAdc instanceof ContourMinmax ) {
	        	    if ( updateAllMinmaxAttr() ) {	        		
	        	        Symbol oneSymb = (Symbol) ((ContourMinmax)newAdc).getSymbol();
	        	    ...
	        	    if ( newEl != null && oldAdc.equals( de.getParent() ) ) {	                	        		
	        		    ...
	        		    ((ContourMinmax)newAdc).getSymbol().update( minmaxTemplate );
	        	    }	
	        	}
    ...
    private void setButtonColor( Button btn, Color clr ) {
		...
		btn.setBackground( new org.eclipse.swt.graphics.Color( this.getShell().getDisplay(), 
    ...
    private void setButtonColor( Button btn, Color fclr,  Color bclr ) {
		...
		btn.setBackground( new org.eclipse.swt.graphics.Color( this.getShell().getDisplay(), 
                           bclr.getRed(), bclr.getGreen(), bclr.getBlue() ) );

		btn.setForeground( new org.eclipse.swt.graphics.Color( this.getShell().getDisplay(), 
                fclr.getRed(), fclr.getGreen(), fclr.getBlue() ) );
   }
    
    /**
     *  Set the drawing status (LINE/MINMAX/CIRCLE)
     */    
    private void setDrawingStatus( ContourDrawingStatus st ) {
		
    	switch ( st ) {
		
		    case DRAW_SYMBOL: 
			    drawLineBtn.setSelection( false );
			    drawCircleBtn.setSelection( false );
			    drawSymbolBtn.setSelection( true );
			break;
		    
		    case DRAW_CIRCLE: 
		        drawLineBtn.setSelection( false );
		        drawSymbolBtn.setSelection( false );
			    drawCircleBtn.setSelection( true );
			break;
	...
	/**
	 * Get a list of fast access symbols defined in for ContoursInfo.xml
	 * If none of them is selected, the "H" and "L" will be selected by default.
	 * @param 
	 * @return
	 */
	private LinkedHashMap<String, Boolean> getQuickSymbols(){
		...
		String xpath = ContoursInfoDlg.CNTRINFO_XPATH + "[@name='QuickSymbols']";
	...
	/**
	 * Retrieve default settings from "settings.tbl" for contour line, symbol, label,
	 * and circle.
	 * 
	 * @param 
	 * @return
...
//	    	contoursAttrSettings.put( "Circle", retrieveDefaultSettings( "Circle" ) );		    
		    ...
		    if ( adc != null && adc instanceof Contours ) {
		    	...
		    	if ( cline != null && cline.size() > 0 ) {
		    		...
		    		if ( cline.get(0).getLabels() != null &&
		    			...
		    			labelFound = true;
		    			contoursAttrSettings.put( cline.get(0).getLabels().get(0).getPgenType(), 
		    					                  cline.get(0).getLabels().get(0).copy() );
		    		}	   	    			   	    		
		    	}
		    	
		    	List<ContourMinmax> csymbols = ((Contours)adc).getContourMinmaxs();
		    	if ( csymbols != null && csymbols.size() > 0 ) {
                    for ( ContourMinmax cmx : csymbols ) {
                    	contoursAttrSettings.put( cmx.getSymbol().getPgenType(), 
                    			                  cmx.getSymbol().copy() );
                        if ( !labelFound ) {
                        	contoursAttrSettings.put( cmx.getLabel().getPgenType(), 
      			                                      cmx.getLabel() );  
                        	labelFound = true;
                        }
                    }
		    ...
		    if ( !lineFound ) {
			    Line dln = new Line( null, new Color[]{ Color.red }, 
					     ...
					     FillPattern.SOLID, "Lines", "LINE_SOLID" );                            
			    
			    contoursAttrSettings.put( "LINE_SOLID", dln );
		    }
		    
		    if ( !labelFound ) {
			    gov.noaa.nws.ncep.ui.pgen.elements.Text txt = new gov.noaa.nws.ncep.ui.pgen.elements.Text( null, 
	       			    "Courier", 14.0f, TextJustification.CENTER,
	                     null, 0.0, TextRotation.SCREEN_RELATIVE, 
	                     new String[]{ "text" },
