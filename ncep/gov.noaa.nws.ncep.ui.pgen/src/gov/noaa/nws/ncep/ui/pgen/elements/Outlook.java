...
public class Outlook extends Contours {
	...
	/**
	 * If lines have not been sorted, sort line in each group so that the line with
	 * east-most starting point be the first line.
	 * If lines have been sorted, move the first line to the back of the list.  
	 */
		...
		if ( !linesSorted ){
			...
			while( it.hasNext() ){
				 ...
				 if ( adc.getName().equalsIgnoreCase( Outlook.OUTLOOK_LINE_GROUP )){
					 	ListIterator<AbstractDrawableComponent> listIt = ((DECollection)adc).getComponentListIterator();
					 	...
					 	while ( listIt.hasPrevious() && lastLn == null){
					 		...
					 		if ( adc1.getName().equalsIgnoreCase(Outlook.OUTLOOK_LABELED_LINE)){
					 			Iterator<AbstractDrawableComponent> it1 = ((DECollection)adc1).getComponentIterator();
					 			while (it1.hasNext() ){
	...
	/**
	 * Sort lines in every group so that in each group the line with
	 * the east-most starting point is the first in the list. 
	 */
	private void orderLines(){

		//Loop through every group and put all lines in the group into a array list.
		...
		while( it.hasNext() ){
			 ...
			 if ( adc.getName().equalsIgnoreCase( Outlook.OUTLOOK_LINE_GROUP )){
					...
					Iterator<AbstractDrawableComponent> itGrp = ((DECollection)adc).getComponentIterator();
					while ( itGrp.hasNext() ){
						 AbstractDrawableComponent labeledLine = itGrp.next();
						 Iterator<AbstractDrawableComponent> itLabeledLine = ((DECollection)labeledLine).getComponentIterator();
	...
	/**
	 * Implementation of a comparator class for sorting lines by
	 * the east-most starting point.
	 * @author bingfan
	 *
	 */
	private class EastMost implements Comparator<DECollection>{
		...
		public int compare( DECollection labeledLn1, DECollection labeledLn2) {
			 ...
			 Iterator<AbstractDrawableComponent> itLabeledLine1 = ((DECollection)labeledLn1).getComponentIterator();
			 Line ln1 = null;
			 ...
			 Iterator<AbstractDrawableComponent> itLabeledLine2 = ((DECollection)labeledLn2).getComponentIterator();
			 Line ln2 = null;
			 while( itLabeledLine1.hasNext() ){
	...
	/**
	 * Add the input line into a group so all the lines in the group will
	 * be showed as continued lines when creating the text message.
	 * @param ln
	 * @param grp
	 */
	public void addLineToGroup(Line ln, DECollection grp){
		if ( ln != null && this.contains(ln) && (grp==null || this.contains(grp))){
			...
			if (ln.getParent().getParent().equals(this)) {
			...
			else if ( ln.getParent().getParent().getName().equalsIgnoreCase( Outlook.OUTLOOK_LINE_GROUP)
					&& grp != null ){
				...
				Iterator<DrawableElement> it = ((DECollection)ln.getParent().getParent()).createDEIterator();
				int lns = 0;
				while ( it.hasNext() ){
		...
		while( it.hasNext() ){
			...
			if ( adc.getName().equalsIgnoreCase(Outlook.OUTLOOK_LABELED_LINE)){
				...
				if ( txt == null ){
					...
					lblInfo += txt.getText()[0] + lineBreaker;
	...
	public Outlook createContours( int nContours, int[] nContourPts, 
		...
		for ( int mm = 0; mm < nContours; mm++ ) {
	        ...
	        if ( clr != null ) oneLine.setColors( new Color[]{ clr } );
            ...
            for ( int ii = 0; ii < nLabels; ii++ )	{
    	        ...
    	        if ( clr != null ) oneText.setColors( new Color[]{ clr } );
	...
	/**
	 *   @return a list of ContourLines converted from "outlookLine"s
	 *           in an Outlook.
	 */
	private void findContourLines( DECollection dec,
			                       ArrayList<ContourLine> lines ) {
		
		if ( lines == null ) {
			lines = new ArrayList<ContourLine>();
		}
		
		Iterator<AbstractDrawableComponent> iterator = dec.getComponentIterator();
