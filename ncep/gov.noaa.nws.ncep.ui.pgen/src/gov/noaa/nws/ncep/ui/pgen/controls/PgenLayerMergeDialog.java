...
/**
 * This class creates a dialog to allow the use import/merge layers from a selected
 * activity to the current activity's layers.
 * 
 * <pre>
 * SOFTWARE HISTORY
...
public class PgenLayerMergeDialog extends Dialog {
	 ...
	 */
	private static enum LayerActions { NO_ACTION, 
		                               ADD_AS_NEW_LAYER, 
	 ...
	 * @throws VizException
	 */
	public PgenLayerMergeDialog( Shell parShell, Product prodIn, String fileIn ) throws VizException {
	...
	private void initializeComponents() {
	    ...
	    curActComp.setLayoutData( new GridData(SWT.LEFT, SWT.DEFAULT, true, false) );
	    ...
	    selActComp.setLayoutData( new GridData(SWT.LEFT, SWT.DEFAULT, true, false) );
	    ...
	    layersComp.setLayoutData( new GridData(SWT.LEFT, SWT.DEFAULT, true, false) );
	    ...
	    for ( Layer lyr : incomingActivity.getLayers() ) {
		    ...
		    Combo layerActCombo = new Combo(layersGrp , SWT.DROP_DOWN | SWT.READ_ONLY );
            
		    boolean sameLayerExist = layerExist( lyr.getName() );
		    layerActCombo.add( "Take no action                                                                                   " );
		    ...
		    layerActCombo.add( "Replace content of 'ActiveLayer' and rename to '" +  lyr.getName() + "'" );
		    ...
		    layerActCombo.add( "Merge content into 'ActiveLayer' and rename to '" +  lyr.getName() + "'" );
	    ...
	    if ( incomingActivity.getLayers().size() > 1 ) {
	        ...
	        Combo bulkCombo = new Combo(layersGrp , SWT.DROP_DOWN | SWT.READ_ONLY );
    	    ...
    	    switch ( LayerActions.values()[ index ] ) {
              ...
              case NO_ACTION:
 		       ...
 		       case REPLACE_LIKE_NAME_LAYER:
 	              	 replaceMergeLayer( activeAct.getLayer( lyr.getName() ), lyr, true, null );
		    	   break;
 		       ...
 		       case MERGE_INTO_LIKE_NAME_LAYER:
 	              	 replaceMergeLayer( activeAct.getLayer( lyr.getName() ), lyr, false, null );
 		    	   break;
 		    	   
 		       case MERGE_INTO_ACTIVE_LAYER:
 	              	 replaceMergeLayer( drawingLayer.getActiveLayer(), lyr, false, null );
 		    	   break;
	...
	/*
	 * Add input layer to an activity - make sure the layer name won't conflict with
	 * existing layers.
	 */
	...
	/*
	 * Replace or merge the content of one layer with the content from another layer
	 * 
	 ...
	 *  
	 *  1. One existing layer can receive only one "Replace" action from incoming layers.
	 *  2. One existing layer can receive only one "Rename" action from incoming layers.
	 *  
	...
	private boolean conflictActions() {
		...
		for ( Layer lyr : layerComboMap.keySet() ) {
    	    ...
    	    switch ( LayerActions.values()[ index ] ) {
              ...
              case NO_ACTION:
 		       ...
 		       case REPLACE_LIKE_NAME_LAYER:
	              	if ( drawingLayer.getActiveLayer() == 
	              		 drawingLayer.getActiveProduct().getLayer( lyr.getName() ) ) {
		...
		if ( conflict ) {
	   	    MessageDialog confirmOpen = new MessageDialog( 
	   	    		...
	   	    		"Conflicting actions to import layers", null, 
	   	    		msg,
