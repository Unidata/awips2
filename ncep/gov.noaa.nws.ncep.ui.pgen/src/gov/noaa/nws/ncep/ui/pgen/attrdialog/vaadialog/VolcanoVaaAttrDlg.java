...
public class VolcanoVaaAttrDlg extends AttrDlg implements ISigmet {
    ...
    public void setAttrForDlg(IAttribute ia) {
        ...
        /*
         * String corr = volcano.getCorr();//-----------nmap2 does NOT do this,
         * so comment out if(corr != null){ btnCorr.setSelection(true);
         * txtCorr.setEnabled(true); txtCorr.removeVerifyListener(tvl);//tvl
         * added already during creation txtCorr.setText(corr);
         * txtCorr.addVerifyListener(tvl); setTxtChange(txtAdNo, false); }
         */

        // ---product type: from Edit

        comboType.setEnabled(true);
        comboType.setText(volcano.getProduct()); // in legacy, only type is NOT
                                                 // reflected 2010-03-25
    ...
    public void createButtonsForButtonBar(Composite parent) {
        ...
        this.mouseHandlerName = null;// --- 2010-03-10 for VaaCloudDlg not
                                     // showing buttons when creating
        ...
        this.getButton(IDialogConstants.OK_ID).setLayoutData(
                new GridData(ctrlBtnWidth, ctrlBtnHeight));
        this.getButton(FORMAT_ID).setLayoutData(
                new GridData(ctrlBtnWidth, ctrlBtnHeight));
        this.getButton(RESET_ID).setLayoutData(
    ...
    public void createArea4(Group top) {
         ...
         * lblInfoSour.setLayoutData(new
         * GridData(GridData.VERTICAL_ALIGN_BEGINNING));
         * 
         * listInfoSour = new org.eclipse.swt.widgets.List(top, SWT.MULTI |
         * SWT.BORDER | SWT.V_SCROLL); GridData gData = new GridData(300,96);
         * gData.horizontalSpan = 7; listInfoSour.setLayoutData( gData);
         * listInfoSour.setItems( this.getInfoSourItems() );//Multi-selecting:
    ...
    private String getElevText() {
        ...
        return elevFootMeterTxt;// *///TODO: elevFootMeterTxt needs to be
                                // assigned a value ??
    }
    ...
    public String getRemarks() {
        String rmk = Volcano.getNoNullTxt(volcano.getRemarks()); // rmk == null
                                                                 // ? "" : rmk;
    ...
    public void copyAttrToVolcano() {
        ...
        if (this.fromSelection)
            volcano.setProduct(comboType.getText()); // TEST/RESUME can be set
                                                     // already
    ...
    private Product getVaaProduct(Volcano vol) {
        /*
         * List<Product> prods =
         * PgenSession.getInstance().getPgenResource().getProducts(); Product
         * volProd = null; for(Product p : prods){ List<Layer> lyrList =
         * p.getLayers(); for(Layer lyr : lyrList){
         * List<AbstractDrawableComponent> list = lyr.getDrawables();
         * for(AbstractDrawableComponent adc : list){
         * 
         * if( adc instanceof Volcano && volcano.getName().equals(
         * ((Volcano)adc).getName())){ volProd = p; } } } }
         */
    ...
    private String getNormalizedTxt(String s) {
        ...
        String[] txt = s.split("\n");
        int start = 0, end = 0; // VOLCANO Layer text added on top, others
                                // bottom
        for (int i = 0; i < txt.length; i++) {
            if (txt.length > 3) {// TODO: remove the hard coded words, see
                                 // vaaAllHeader.xslt
