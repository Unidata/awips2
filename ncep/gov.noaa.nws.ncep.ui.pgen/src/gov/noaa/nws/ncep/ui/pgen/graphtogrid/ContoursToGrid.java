...
//        	logger.warn( "Invalid input for kx;ky - default to 63;28" );
        ...
        /*
         * Find a coordinate transformation between world and the custom grid space.
         */
        CoordinateTransform gtrans = new CoordinateTransform( proj, garea, kx, ky );
        ...
        */
        String bound = getParamValues( gridParameters, "BOUNDS" );                
   	    Contours bndContours = contoursFromBounds( bound, gtrans, hist, gridPts, kx, ky );
	     ...
	     * 
	     * 1. Convert the text label to float using the CATMAP.
	     * 2. Add bound lines as part of the Contours.
	     * 3. Extend the lines to the boundary of the GAREA
	     * 
	     * Note that bound lines is also processed here but it won't be
	     * smoothed and extended since they are closed lines with smoothing 
	     * level 0.
	     * 
	     */	    		    
	    ...
	    for ( ContourCircle cc : ccircle ) {
        	ArrayList<Coordinate> pts = generateArcPoints( (Arc)cc.getCircle(), 20.0  );
        	if ( pts.size() > 1 ) {
        	    ContourLine ncl = new ContourLine( pts, true, cc.getLabelString() , 1 );
        	    extContours.add( ncl );
        	}
        }
...
//		drawBoundsAndGrid( bndContours, hist, kx, ky, gridPts );
	     ...
	     * 
	     * Note that if no extension is required (bounds is empty or 
	     * the line is a closed line), fi_ext and fj_ext should use the 
	     * the original line points.
	     * 
...
//		}
        ...
        /*
         * Convert 2-D arrays into 1-D to call the C driver.
         * First do the original lat/lon points
         */       
       	int ntotal = 0;
        ...
        for ( ContourMinmax cmm : cminmax ) {
        	Coordinate p = ((SinglePointElement)(cmm.getSymbol())).getLocation();
         	mmlon[ ii ] = (float)p.x;
        	mmlat[ ii ] = (float)p.y;
		 ...
		 */       
        g2gNative.g2g_compute( grid, hist, kx, ky, nlines, 
        		npoints, latPts, lonPts, norigpts, smthLat, smthLon,
        	    nextpts, extLat, extLon, linevalues, ismth, iclosed,
        		mmnum, mmlat, mmlon, mmfi, mmfj, mmvalue,
        	    catmap, histgrd, discrete, dlines, gglims, edgeopts );   
...
//        	logger.debug( "GLEVEL is not provided");
        ...
        gdatim = new String ( PgenUtil.calendarToGempakDattim( extContours.getTime1() ) + 
        										extContours.getForecastHours() );
...
//        	String workingDir = PgenUtil.CURRENT_WORKING_DIRECTORY;
        ...
        if ( mt.find() ) {
            ...
            if ( !grdfile.exists() ) {
            	msg = new String( "The PATH does not exist, please create it first" );
            }      	
        }
        ...
        if ( msg != null ) {
        	...
        	MessageDialog msgDlg = new MessageDialog( 
					...
					"Warning", null, "Fail to write to " + fullFile + " because:\n\n" + msg,
...
/*
        ...
        logger.debug( "gdoutf="+gdoutf);
        logger.debug( "cpyfil="+cpyfil);       
        logger.debug( "anlyss="+anlyss);
        logger.debug( "maxgrd="+maxgrd);
        logger.debug( "gparm="+gparm);
        logger.debug( "glevel="+glevel);       
        logger.debug( "gvcord="+gvcord);
        logger.debug( "gdattim="+gdatim);
...
//        			logger.debug( "Invalid contour interval - " + e );
        ...
        Contours gridContours = ((Contours)currentGraph).createContours( nContours, nContourPts, 
                         latlons, cntStrings, Color.green );
	...
	/**
     *  Bound processing - By default, set the grid points inside a bound as BOUNDED 
     *  and their values as RMISSD (-9999).  However, the assignments are reversed
     *  if the "inout" flag is explicitly set to "false" by the user.  In that 
     *  case, the grid points outside a bound are set as BOUNDED and their values 
     *  as RMISSD (-9999).
     *  	     
     *  Note: need to access the "hist" array as it is an FORTRAN array (column first).
     */
	private Contours contoursFromBounds( String bnds, CoordinateTransform gtrans,
		...
		for ( BoundPolygon bpoly : boundPolys ) {
		    ...
		    for ( MultiPolygon mpoly : bpoly.getBoundPolygons() ) {
			    ks++;
			    /*
			     * Mask the grid points inside the bound as bounded.
			     * If the inout is set to false, set the grid point
			     * outside the bound as BOUNDED.
			     */
			    for ( int jj = 0; jj < ky; jj++ ) {				
				    for ( int ii = 0; ii < kx; ii++ ) {
     ...
     *  
     *  1. A closed line labeled "-9999.0" is treated as a bound - the grid points 
     *     inside such a bound are set as "BOUNDED" and values as "RMISSD". 
	...
	private void checkSpecialBounds( Contours cnt, float[] hist, int kx, int ky,
			                         CoordinateTransform gtrans, Coordinate[] grdPts,
			                         CatMap cmap ) {
		...
		for ( ContourLine cline : cntline ) {
        	...
        	if ( ln.isClosedLine() && ( lblValue == G2GCommon.RMISSD ||
        		...
        		CoordinateSequence sequence = new CoordinateArraySequence( coords.toArray( cd ) );
	...
	private void drawBoundsAndGrid( Contours cnt, float[] hist, int kx, int ky,
		...
		/*
	     * Build grid points as a a collection of markers - for debug. 
	     * if a grid point is BOUNDED, it is drawn as red, otherwise, blue.
	     */		    
		...
		for ( int jj = 0; jj < ky; jj++ ) {				
		    for ( int ii = 0; ii < kx; ii++ ) {
		    	...
		    	if ( hist[ ii + jj * kx ] == G2GCommon.BOUNDED ) {        
		            nde.add( new Symbol(null, new Color[] { Color.red }, 0.5f, 0.5, 
		            	    false, grdPts[ ii + jj * kx ], "Marker", "FILLED_BOX" ) );
				}
				else {
	                nde.add(new Symbol(null, new Color[] { Color.blue }, 0.5f, 0.5, 
	       			     false, grdPts[ ii + jj * kx ], "Marker", "FILLED_BOX" ) );
	...
	private  ArrayList<Coordinate> generateArcPoints( Arc arc, double interval ) {
        ...
        int numpts = (int)(Math.round(arc.getEndAngle() - arc.getStartAngle() + 1.0 ) / increment) ;
