...
public class WatchBox extends MultiPointElement implements IWatchBox{
	...
	public static enum WatchShape { NS, EW, ESOL };
	public static String dirs[] = {"N","NNE","NE","ENE","E","ESE","SE","SSE",
	...
	public AbstractDrawableComponent copy() {
		...
		if ( fillColor != null ){
			newWatchBox.setFillColor(new Color(fillColor.getRed(),
										fillColor.getGreen(),
										fillColor.getBlue()));
		}
		/*
	...
	public String getSpec(){
		...
		spec = String.format("%1$5.2f%2$10.2f", linePoints.get(0).y, linePoints.get(0).x) 
		+ "   " 
		...
		+ "   "
		+ getRelative(linePoints.get(4),getNearestVor(linePoints.get(4)))
		+"\n"
		+ "ORIENT: " + boxShape.toString()
		+ "  - HALF WIDTH: " + String.format("%1$4.0f", getHalfWidth()/PgenUtil.SM2M)
		+ "sm (" + Math.round(getHalfWidth()/PgenUtil.NM2M/5.0)*5 + " nm)\n"
	...
	/**
	 * Return a string that indicates the relative location from
	 * the input location to the input station
	 * 
	 * @param pt
	 * @param st
	 * @return
     ...
     * @param point - input location
     * @param lnPt1 - first point of the line
     * @param lnPt2 - second point of the line
     * @param intrsctPt - the nearest point on the line
    ...
    /**
     * Make a copy of the input list and set the 
     * county to it
     * @param cl
     */
	...
	/**
	 * Return a list of WFOs(String) which are in the
	 * watch box  
	 * @return
	 */
			...
			for(SPCCounty cnty: countyList ){
				...
				if ( wfo != null ) {
					for ( int ii = 0; ii < wfo.length(); ii+=3 ){
						...
						if ( !wfos.contains(wfoStr)) wfos.add( wfoStr );
					}
				}
	...
	/**
	 * Return a list of state names(String), which are in
	 * the watch box
	 * @return
	 */
	...
	/**
	 * Add all counties of an CWA into the county list
	 * of the watch box
	 * @param cwa
	 */
	...
	/**
	 * get a list of inactive counties inside of any polygon
...
//	}
    ...
    /**
     * Snap the input location onto the input anchor point
     * This function takes an input location and an anchor point,
     * returns a location along a 16-pt compass direction with 
     * a distance rounded to  the nearest 5 statute miles from the 
     * anchor point.                                     
     * @param anchor
     * @param point
     * @return
     ...
     * @param pt - input location
     * @param anchorList - a list of anchor points
    ...
    /**
     * Generate a watch box's eight points from the two input locations,
     * half width of the watch box, and the shape of the watch box 
     * @param ws: shape of the watch box
     * @param halfWidth: half width of the watch box 
     * @param point1:	first input location 
     * @param point2:	second input location 
     * @return
     */
    public static  ArrayList<Coordinate> generateWatchBoxPts(WatchShape ws, double halfWidth, 
    	...
    	if ( ws == WatchShape.NS ){
    		...
    		if ( Math.abs(point1.x-point2.x) < 0.0001 ){
    			...
    			dir = 180 - Math.atan((point2.y-point1.y)/
     ...
     * @param ptIdx - the point being edited
     * @param loc - new location of the editing point.
    ...
    /**
     * Calculate the new half width of the watch box being edited.
     * Re-compute the locations of point 0 and point 4.
     * @param wb 	- watch box being edited.
     * @param ptIdx	- the index of point being edited.
     * @param loc	- the new location of the editing point.
     * @param newWbPt0	- new location for point 0
     * @param newWbPt4	- new location for point 4
	...
	public List<String> getNearbyWFOs(){
		...
		for(SPCCounty cnty : counties ){
			...
			if ( wfo != null ) {
				for ( int ii = 0; ii < wfo.length(); ii+=3 ){
					String wfoStr = wfo.substring(ii, wfo.length()>ii+3?ii+3:wfo.length());
					if ( !wfos.contains(wfoStr) && !nWFOs.contains(wfoStr)) nWFOs.add( wfoStr );
				}
			}
	...
	/**
	 * Add a new status
	 * @param fromLine - 'from line' string
	 * @param dNum - discussion number
	 * @param vTime - valid time
	 * @param eTime - expiration time
	 * @param name - forecaster name
	 */
	
    public void addStatus(String fromLine, int dNum,
			Calendar vTime, Calendar eTime, String name){
    	if ( statusHistory == null ){
    		statusHistory = new ArrayList<WatchStatus>();
    	}
    	
    	statusHistory.add(new WatchStatus(fromLine, dNum,
    					vTime, eTime, name));
		...
		if ( cntyList != null && !cntyList.isEmpty() ){
			...
			for( SPCCounty cnty : cntyList ){
				...
				cntyInfo += String.format("%1$-7s%2$-5s%3$-12s%4$6.2f%5$8.2f%6$7s%7$5s", 
						...
						cnty.getCentriod().y, cnty.getCentriod().x, cnty.getFips(), cnty.getWfo());
				cntyInfo += "\n";
			}
		}
		else {
    ...
    public class WatchStatus{
    	...
    	private WatchStatus(String fromLine, int dNum,
    				Calendar vTime, Calendar eTime, String name){
    		this.fromLine = fromLine;
