...
public class TcaAttrDlg extends AttrDlg implements ITca, SelectionListener {
    ...
    private void createStormInfoArea(Group g1) {
        ...
        /*
         * Down arrow button used to decrement storm advisory number in text
         * field
         */
    ...
    private void createBreakpointTools(Group g2) {
        ...
        advisoryTypes.setLayoutData(new GridData(GridData.HORIZONTAL_ALIGN_FILL
                | GridData.GRAB_HORIZONTAL));
        ...
        breakpointTypes.setLayoutData(new GridData(
        ...
        bkpt1Field.setLayoutData(new GridData(GridData.HORIZONTAL_ALIGN_FILL
                | GridData.GRAB_HORIZONTAL));
        ...
        bkpt2Field.setLayoutData(new GridData(GridData.HORIZONTAL_ALIGN_FILL
                | GridData.GRAB_HORIZONTAL));
        ...
        specialGeogTypes.setLayoutData(new GridData(
                GridData.HORIZONTAL_ALIGN_FILL | GridData.GRAB_HORIZONTAL));
    ...
    /*
     * create widgets for the section of the dialog that lists current
     * breakpoints
     */
    private void createBreakpointList(Group g3) {
         ...
         */
        breakpointList = new List(g3, SWT.SINGLE | SWT.BORDER | SWT.H_SCROLL
                | SWT.V_SCROLL);
    ...
    /*
     * Resets the advisory information section of the TCA dialog to default
     * values. Usually called when no advisory is selected
     */
    protected void resetAdvisoryInfo() {

        // Apply and Delete buttons are not available when no advisory is
        // selected
        apply.setEnabled(false);
        deleteSegment.setEnabled(false);
    ...
    /*
     * Update the advisory info section of the dialog with values in the
     * currently selected advisory
     */
     ...
     * selected in the breakpoint list, and that the advisory info is updated in
     * the dialog
     */
    ...
    /**
     * Called when no advisory is selected. Ensures that nothing is selected in
     * the breakpoint list, and that default values are set in the dialog
     */
    ...
    protected void buttonPressed(int buttonId) {
        ...
        if (buttonId == SAVE_ID) {
            ...
            if (stormNameField.getText().isEmpty()) {
                ...
                MessageDialog messageDlg = new MessageDialog(PlatformUI
                ...
                MessageDialog messageDlg = new MessageDialog(PlatformUI
                        .getWorkbench().getActiveWorkbenchWindow().getShell(),
                        "Warning", null, msg.toString(), MessageDialog.ERROR,
                        new String[] { "OK" }, 0);
                messageDlg.open();
            } else {
                dataURI = tcaTool.saveAdvisory();
                if (dataURI != null) {
                    getButton(SAVE_ID).setEnabled(false);
         ...
         */
            ...
            TcaTextMessageDlg textMessage = new TcaTextMessageDlg(
                    this.getShell());

            textMessage.setOutputFilename(generateVTECFilename());
            textMessage.setDataURI(dataURI);
    ...
    /*
     * creates a new string with user input changes for a storm advisory number;
     * then checks to see if it is a valid advisory number.
     */
    ...
    /*
     * generate the name of the file used to store the TCV message based on the
     * storm info
     */
    ...
    /*
     * Replace an advisory with the given advisory in the watch/warning list and
     * the breakpoint list. Update the advisory info section of the dialog
     */
    ...
    public void widgetSelected(SelectionEvent e) {
        ...
        if (e.widget instanceof Button) {
            ...
            /*
             * Remove the selected advisory from the list of advisories,
             * breakpoint list, and the notify the drawing tool.
             */
            else if (b.getText().equals(DELETE_SEGMENT)) {
                int idx = breakpointList.getSelectionIndex();
                if (idx != -1) {
                    setTextLocation(advisories.get(idx).getSegment()
                            .getBreakpoints().get(0).getLocation());
