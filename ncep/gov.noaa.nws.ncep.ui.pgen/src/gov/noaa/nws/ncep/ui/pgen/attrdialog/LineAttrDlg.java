...
public class LineAttrDlg  extends AttrDlg implements ILine {
	...
	protected static enum ChkBox { COLOR, WIDTH, SMOOTH, PATTERN_SIZE, CLOSE, FILL, FILL_PATTERN, LABEL };
	...
	/**
	 * Creates a line attribute dialog if the dialog does not exist 
	...
	public void setColor( Color[] colors ){
		...
		for (int j=0; j < colors.length; j++ ) {
			Color clr = colors[j];
			csList.get(j).setColorValue(new RGB(clr.getRed(), clr.getGreen(), clr.getBlue()));
		}
	...
	private void setFillPattern(FillPattern fp){
		if (fp != null && fillPatternCbo != null ) fillPatternCbo.select(fp.ordinal());
	}
...
*/		
	...
	private void enableChkBoxes(boolean flag){
		...
		for ( ChkBox chk : ChkBox.values()){
			if ( chkBox[chk.ordinal()] != null )chkBox[chk.ordinal()].setVisible(flag);
		}
	...
	private void setAllChkBoxes(){
		...
		for ( ChkBox chk : ChkBox.values()){
			if ( chkBox[chk.ordinal()] != null )chkBox[chk.ordinal()].setSelection(true);
		}
	}
