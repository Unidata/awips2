 ...
 * 08/14        #1020       J. Wu      Override "handleShellCloseEvent()" to a simple close().
 * </pre>
 * 
 * @author M. Gao
 */
...
public class TrackExtrapPointInfoDlg extends AttrDlg {
    ...
    public void setTrack(Track _track, int unitComboSelectedIndex,
        ...
        if (roundDirComboSelectedIndex > 0)
            ...
            directionText.setText("Dir: " + doubleValurFormater(dir, 2)
                    + " deg.");

        /*
         * fill time, latitude and longitude info for the last two initial
         * points
         */
    ...
    private void fillExtraPointInfo(TrackPoint[] trackPoints) {
        /*
         * The first point of the extra track points is the last point of the
         * initial points thus, it needs to be skipped.
         */
        if (trackPoints == null || trackPoints.length < 2)
            return;
    ...
    private void setInitPointTimeText(TrackPoint trackPoint,
            Text initTimeTextObject) {
    ...
    private void setInitPointLatText(TrackPoint trackPoint,
    ...
    private void setInitPointLonText(TrackPoint trackPoint,
    ...
    private void initializeComponents(Composite topComposite, Composite parent) {
        ...
        speedText = createTextfieldWithoutLabel(topComposite, SWT.SINGLE
                | SWT.BORDER, DEFAULT_TEXT_WIDTH, DEFAULT_TEXT_HEIGHT, false);
        directionText = createTextfieldWithoutLabel(topComposite, SWT.SINGLE
                | SWT.BORDER, DEFAULT_TEXT_WIDTH, DEFAULT_TEXT_HEIGHT, false);
        ...
        firstInitPointTimeText = createTextfieldWithoutLabel(topComposite,
                SWT.SINGLE | SWT.BORDER, DEFAULT_TEXT_WIDTH / 2,
                DEFAULT_TEXT_HEIGHT, false);
        ...
        extraPointInfoText = new Text(topComposite, SWT.BORDER | SWT.MULTI
                | SWT.H_SCROLL | SWT.V_SCROLL);
        GridData gridData = new GridData(DEFAULT_SCOLL_TEXT_WIDTH,
                DEFAULT_SCOLL_TEXT_HEIGHT);
    ...
    private void drawSingleLabelSpanColumns(Composite parentComposite,
            int spanColumns, String labelString) {
        Label label = new Label(parentComposite, SWT.LEFT);
        label.setText(labelString);
        // make the label to fill the a number of columns, the actual number is
        // determined by spanColumns
     ...
     * 
     * For this window, just need to close it.
     * 
