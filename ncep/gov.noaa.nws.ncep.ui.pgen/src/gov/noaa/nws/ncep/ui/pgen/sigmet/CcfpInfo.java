...
public class CcfpInfo {
	 ...
	 */
	public static String convertXml2Txt(String xmlFileName, String xltFileName){
        String res = "";
	 ...
	 * @param it: Issue time String
	 * @param vt: Valid time String
	 ...
	 * @param prds: Product list to be filtered
	 * @param it: Issue time String
	 * @param vt: Valid time String
		...
		for(Product p : prds){
			for(Layer l : p.getLayers()){
				for(AbstractDrawableComponent adc : l.getDrawables()){
					...
					{Ccfp c=(Ccfp)adc;Sigmet s=c.getSigmet();s.setEditableAttrStartTime(it);s.setEditableAttrEndTime(vt);c.setCollectionNameWithSigmet(s);	sigs.add( (Sigmet) ((Ccfp)adc).getSigmet().copy());		}				//sigs.add(((Sigmet)adc).copy());
				}
			}
		}
	 ...
	 * @param sig: the Sigmet with text
...
//		String conf = "CONF "+ ( sigConf==null ? "LOW" : sigConf.contains("49")? "LOW":"HIGH") ;
...
//		return new String[]{tops,gwth,conf,cvrg};//sb.toString();
	...
	public static ArrayList<Coordinate> getDirMostPts(String dir, Coordinate[] coors, IMapDescriptor md){
		...
		try{
			...
			gc.setDirection( DIR_AZI_MAP.get(dir), 70*1852);//30 nautical miles
	 ...
	 * @param dir:   the direction of the speed
	 * @param coors: the Coordinate of the CCFP
	 * @param md: 	 the MapDescriptor 
	 ...
	 * @param sig: the Sigmet of which the Centroid to be calculated
	 ...
	 * @param sig: the Sigmet for which the check to be performed
	...
	public static boolean isCrossingLon180(Sigmet sig){
		...
		LineString lon180 = f2.createLineString(new Coordinate[]{new Coordinate(180,90),new Coordinate(180,-90)});
		
		GeometryFactory f3 = new GeometryFactory();
		LineString lon180x = f3.createLineString(new Coordinate[]{new Coordinate(-180,90),new Coordinate(-180,-90)});
	 ...
	 * @param ctxt: the Coordinate of the Text
	 * @param sig:  the Sigmet 
	...
	public static double[] getCcfpTxtAziDir(Coordinate ctxt, Sigmet sig){
		...
		GeodeticCalculator gc = new GeodeticCalculator(PgenSession.getInstance().getPgenResource().getCoordinateReferenceSystem());//DefaultEllipsoid.WGS84);
		...
		//save the Arrow head and Text Coordinates: required by vgf-xml converter
	 ...
	 * @param sig: the Sigmet with Area
	 * @param pts: the Point's Coordinate
	...
	public static boolean isPtsInArea(AbstractSigmet sig, Coordinate pts){	
		if (sig==null || sig.getLinePoints()==null || sig.getLinePoints().length<3 || pts==null){
			return false;
		}
	...
	public static Coordinate getSigCentroid2(Sigmet sig, IMapDescriptor md){
		...
		if(sig==null || sig.getLinePoints()==null || sig.getLinePoints().length<2) 
			return null; 
	...
	public static boolean isPtsInArea2(AbstractSigmet sig, Coordinate pts, IMapDescriptor md){	
		...
		return (dpts[0][0]>=xset.first())&&(dpts[0][0]<=xset.last())&&(dpts[0][1]>=yset.first())&&(dpts[0][1]<=yset.last());
	}
	
	/**
	 * Save the CCFP Text and Arrow End Points to the Sigmet
	 * @param ctxt:		Text Coordinate;
	 * @param cArrw:	Arrow head Coordinate:
	 * @param sig:		The Sigmet element.
	 */
	...
	/**
	 * calculate Text Arrow Azimuth and distance;
	 * for the convenience of CCFP vgf-xml converter.
	 * @param sigmet
	 */
	...
	public static boolean isTxtArrwExst(Sigmet sigmet){
		...
		if( fline != null && (! fline.isEmpty()) && ftext != null && (! ftext.isEmpty()) ){
			
			if( PGEN_TYPE_CCFP.equalsIgnoreCase(fline.trim()) && ftext.contains(TEXT_SEPERATOR) )
				return true;		
		}
