...
//        	productDefaults = new ArrayList< HashMap<String, String> >( productNames.size() );
    ...
    private void createBasicInfo( Composite comp ) {
		...
		if (  cntAttrDlg instanceof ContoursAttrDlg  )  {
			Contours curCnt = ((ContoursAttrDlg)cntAttrDlg).getCurrentContours();
			String fcsthr = "";
			if ( curCnt != null ) {
    ...
    private void createAdvancedInfo( Composite comp ) {
	    ...
	    createPanelFromList( advancedGrp, "Grid Navigation", 
	    				GraphToGrid.getGridCalcParams(), paramText, 9 );
	    createPanelFromList( advancedGrp, "Grid Display", 
	    				GraphToGrid.getGridDisplayParams(), paramText, 1 );
	    createPanelFromList( advancedGrp, "Grid Output", 
	    				GraphToGrid.getGridOutputParams(), paramText, 9 );
...
//	                }
    ...
    private HashMap<String, String> retrievePrdMap( String prdName ) {
		...
		if ( advancedGrp != null && paramText != null ) {               		    		
            for ( Text txt : paramText ) {
    			currentProductParams.put( txt.getData().toString(), txt.getText() );    			
    		}   		   		
    	}
	...
	/**
	 * @param cntAttrDlg the cntAttrDlg to set
	 */
...
//	        	logger.warn( "Invalid input for kx;ky - default to 63;28" );
			...
			/*
			 *  Note: extend the line only, so the label of the line doesn't matter 
			 *  and thus we do not need CatMap here.
			 */			
	...
	private HashMap<String, String> generateParameters(  ) {
		...
		HashMap<String, String> prm = new HashMap<String, String>( currentProductParams );
	   	...
	   	gparm = dlg.getParm();
	   	level = dlg.getLevel();
	   	gdoutf = new String( gparm.toLowerCase() + "_" + dlg.getTime1().get(Calendar.YEAR) + 
	                 gdt.substring(2, 6) + gdt.substring(7, 9) + 
		               fcstCombo.getText() + ".grd" );
