...
public class CcfpTimeDlg extends AttrDlg{
	...
	/**
	 * this String MUST be identical to ccfpTimes.xml's Time element issue attribute
	 */
...
//------------------------------------------------------------------added 201001 	
	...
	private String[] parseCcfpTimesFile(){
		...
		for(int i=0;  i<nlist.getLength(); i++){
			...
			for(int j=0; j<nnMap.getLength(); j++){
				...
				Node nAttr = nnMap.item(j);//each attribute of an element, like: issue, valid1, or valid2
	...
	public void handleTime(ArrayList<String> list, String hour, boolean isIssueTime, String itime){
		...
		Boolean hourOnly = null, today = true, tomorrow = false;;
				...
				if( isIssueTime ){
					String day = (fh+100) < ch ? getDayOrHour( tomorrow ) : getDayOrHour( today );//fh+100: from legacy code
					list.add( day+"_"+hour );		
					issueTimeDayMap.put(itime, day);
				}else{
					list.add( (fh < ih ? getDayOrHour( tomorrow ) : issueTimeDayMap.get(itime))+"_"+hour );
				}
	...
	public String getDayOrHour(Boolean isToday){
		...
		return isToday == null 
				? sdft.format(today.getTime())//today's hour
				: isToday 
