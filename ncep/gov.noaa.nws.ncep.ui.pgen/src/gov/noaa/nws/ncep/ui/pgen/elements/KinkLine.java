...
public class KinkLine extends Line implements IKink {
	 ...
	 * @param arrowHeadType
	 */
	public KinkLine(Coordinate[] range, Color[] colors,
			...
			String pgenType, String pgenCategory, double kinkPosition,
			ArrowHeadType arrowHeadType) {
	...
	public void update(IAttribute iattr) {
		...
		if ( iattr instanceof IKink ){ 
			...
			coords.add(attr.getStartPoint());
			coords.add(attr.getEndPoint());
			this.setLinePoints( coords );
		}
	}
