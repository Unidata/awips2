 ...
 * are valid JTS geometries.  If an invalid Geometry is found, an attempt is made to
 * create one or more valid ones from the invalid geometry.
 * @author sgilbert
 *
 */
public class ContourValidator {
	 ...
	 * @param contours GeometryCollection containing Polygons and LineStrings
	 * @return
	 */
	 ...
	 * If not, an attempt is made to create valid Geometrys from the invalid one.
	 * @param cntr
	 * @return
	 */
		...
		if ( cntr instanceof Polygon ) {
			...
			if ( ! cntr.isValid() ) {
				...
				case TopologyValidationError.RING_SELF_INTERSECTION:
					/*
					 * self intersection.  try to separate into two diff polygons at pt of intersection
					 */
					//System.out.println(cntr);
					List<Geometry> subs = splitPolygonAt( cntr, op.getValidationError().getCoordinate() );
					geoms.addAll(subs);
					break;
	...
	private static List<Geometry> splitLineString(Geometry cntr) {

		Coordinate[] tempCoords = null;
		Coordinate[] coords = cntr.getCoordinates();
		List<Geometry> tempList = new ArrayList<Geometry>();
		List<Geometry> newCntrs = new ArrayList<Geometry>();
		GeometryFactory gf = new GeometryFactory();
		...
		for ( int j=0; j < coords.length; j++ ) {
			if ( coords[j].equals2D( intx ) ) indexes.add(j);
		}
		...
		if ( indexes.size() < 2 ) {
			...
			for ( int j=0; j < coords.length-1; j++ ) {
				//clist.add( coords[j], true);
				if ( !coords[j].equals2D(intx)  && !coords[j+1].equals2D(intx)) {
					if ( CGAlgorithms.isOnLine(intx, new Coordinate[]{ coords[j], coords[j+1]} )) clist.add(intx, true);
				}
			...
			for ( int j=0; j < tempCoords.length; j++ ) {
				if ( tempCoords[j].equals2D( intx ) ) indexes.add(j);
			}
		...
		CoordinateList c1 = new CoordinateList( CoordinateArrays.extract( tempCoords, 0, indexes.get(0)));
		c1.add( CoordinateArrays.extract( tempCoords, indexes.get(1)+1, tempCoords.length-1), false);
		//c1.closeRing();
		Geometry g = gf.createLineString(c1.toCoordinateArray());
		if (g!=null) tempList.add( g );
		
		CoordinateList c2 = new CoordinateList( CoordinateArrays.extract( tempCoords, indexes.get(0), indexes.get(1) ) );
		g = createPolygon(c2.toCoordinateArray());
		if (g!=null) tempList.add( g );
		
		for ( Geometry geom : tempList ) {
			newCntrs.addAll( validateContour(geom) );
		}
	...
	/*
	 * Try to split up a polygon into two separate polygons at (or near) the given point
	 */
	private static List<Geometry> splitPolygonAt(Geometry geom, Coordinate point) {
		 ...
		 */
		for ( int j=0; j < coords.length; j++ ) {
			if ( coords[j].equals2D( point ) ) indexes.add(j);
		}
		
		if ( indexes.size() == 1 ) {
			/*
			 * if point appears only once, check to see if polygon
			 * self intersects at a different point
			 */
			...
			if ( ! glist.isEmpty() ) {
				...
				Geometry g = createPolygon( coords );
				if (g!=null) tempList.add( g );
			}
		}
			 ...
			 */
			if ( indexes.get(0)==0 && indexes.get(1)==coords.length-1 ) {    // point is at endpoints
				List<Geometry> glist = trySplitPolygon( coords );
				if ( ! glist.isEmpty() ) tempList.addAll( glist );
			}
			else {
				/*
				 * create two polygons from original
				 */
				CoordinateList c1 = new CoordinateList( CoordinateArrays.extract( coords, 0, indexes.get(0)));
				c1.add( CoordinateArrays.extract( coords, indexes.get(1)+1, coords.length-1), false);
				c1.closeRing();
				Geometry g = createPolygon(c1.toCoordinateArray());
				if (g!=null) tempList.add( g );

				CoordinateList c2 = new CoordinateList( CoordinateArrays.extract( coords, indexes.get(0), indexes.get(1) ) );
				g = createPolygon(c2.toCoordinateArray());
				if (g!=null) tempList.add( g );
			}
		}
	...
	private static List<Geometry> trySplitPolygon(Coordinate[] coords) {
		...
		IsSimpleOp op = new IsSimpleOp(ls);
		if ( op.isSimple() ) return tempList;             // does not self intersect
		...
		if ( indexes.size() < 2 ) {
			/*
			 * intersection point appears only once in coordinate list.
			 * check each segment, and add intersection point to that segment if
			 * they are collinear.
			 */
			CoordinateList clist = new CoordinateList();
			clist.add(coords[0], true);
	...
	private static Geometry createPolygon(Coordinate[] coords) {
		...
		if ( (env.getHeight() == 0) || (env.getWidth() == 0) ) return null;    // collinear points
