...
public class IntlSigmetResource extends
        AbstractNatlCntrsResource<IntlSigmetResourceData, IMapDescriptor>
    ...
    private enum WeatherHazardType {
        ...
        WIND, FZRA, TEST, CANCEL, OTHER, UNKNOWN, NIL
    }
    ...
    public IntlSigmetResource(IntlSigmetResourceData resourceData,
        ...
        // condensedIntligLinkedHashMap = new HashMap<String,
        // CondensedIntlSigmetRecord>(0);
    ...
    public void paintFrame(AbstractFrameData frameData,
        ...
        if (frameData != null) {
            ...
            for (IntlSigmetRscDataObj condensedISIG : condensedISIGCollection) {
                ...
                // activeFrameTime.compareTo(condensedISIG.endTime) > 0)
                // continue;
                ...
                ArrayList<WeatherHazardType> weatherHarzardList = condensedISIG
                        .getWeatherHazardEnumList();

                /*
                 * Retrieve the symbol specific attributes such as symbol size,
                 * width, pattern to be used etc, for all the weather hazards in
                 * the current international sigmet
                 */
                condensedISIG.symbolAttributeSubsetList = condensedISIG
                        .generateListOfSymbolAttributesForAllWeatherHazards(weatherHarzardList);
                ...
                condensedISIG.polygonVertexPixelCoordList = this
                        .generatePixelCoordinateListFromWorldCoordinateList(condensedISIG.polygonVertexWorldCoord);
                if (condensedISIG.polygonVertexPixelCoordList != null) {
                    ...
                    if (polygonVertexPixelCoordListSize > 1
                            && !isLocationLookUpFailed) {
                        this.drawPolygon(graphicsTarget,
                                condensedISIG.polygonVertexPixelCoordList,
                                polygonLineColor, polygonLineWidth, lineStyle);

                        /*
                         * Get the pixel coordinate of the lowest vertex of the
                         * polygon to render the text
                         */
                        textLocation = condensedISIG
                                .getLabelLocation(condensedISIG.polygonVertexPixelCoordList);

                        /*
                         * Get the centroid of the polygon in world coordinates
                         * to render the symbol
                         */
                         ...
                         * symbol at that point, but convert it first to world
                         * coordinates
                         */
                        ...
                        if (condensedISIG.getDistance() != IDecoderConstantsN.INTEGER_MISSING) {
                            ...
                            /*
                             * delta is used to draw the marker at the center of
                             * the circle
                             */
                        ...
                        /*
                         * This is retained from legacy code: For a single point
                         * (isolated) TEST SIGMET the label's text is preceded
                         * with the character 'T'.
                         */
                        if (weatherHarzardList.size() > 0
                                && weatherHarzardList.get(0).equals(
                                        WeatherHazardType.TEST)) {
                            labelList.add("T");
                        }

                        /*
                         * The location of the label should be the same as that
                         * of the single vertex, but with a small x and y offset
                        ...
                        /*
                         * The question marks are added to ensure thatthe user
                         * understands this is a case of location look up
                         * failure
                         */
                        labelList.add("??");
                        tempSymbolLocationWorldCoord = new Coordinate(
                    ...
                    if (intlSigmetResourceDataObj.symbolEnable) {
                        ...
                        /*
                         * If the symbol location is not null and the list of
                         * symbols and the list of weather hazards are not
                         * empty..
                         */
                        if (tempSymbolLocationWorldCoord != null
                            ...
                            Color[] symbolColor = { new Color(
                                    firstSymbolAttributes.getSymbolColor().red,
                            ...
                            for (int i = 0; i < numSymbolsToDisplay; i++) {
                                 ...
                                 * displayed in the Southern Hemisphere, and if
                                 * it is either a hurricane or a tropical storm,
                                 * then different symbols are used than if it
                                 * were to be rendered in the Northern
                                 * Hemisphere.
                                 */
                                ...
                                /*
                                 * (Non-Javadoc) For the weather hazards listed
                                 * below, instead of symbols, a text
                                 * representation of the same is rendered within
                                 * a box. (Note): The decision to render
                                 * WIND/WTSPT hazards as text (since no
                                 * equivalent symbols exist) is an improvement
                                 * over legacy, which does not render them at
                                 * all. For WIND/WTSPT, the text is rendered in
                                 * the color white, since there is no predefined
                                 * color.
                                 */
                                ...
                                if (isDrawText) {
                                     ...
                                     * calculation shown below are ad-hoc and
                                     * notvery accurate since there is no
                                     * specific scaling factorthat exists
                                     * between the symbols and the fonts used in
                                     * CAVE
                                     */
                                    float symbolSizeInNMAP = (float) (7 * symbolSize + 2.5);
                                    float fontSize = symbolSizeInNMAP;
                                    Text symbolText = new Text(null, "Courier",
                                            symbolSizeInNMAP,
                                ...
                                /*
                                 * To get the x-offset and y-offset to display
                                 * more than one symbol without overlapping
                                 */
                                ...
                                symbolCoordinate = new Coordinate(worldArr[0],
                                        worldArr[1]);
                            }
                    ...
                    if (intlSigmetResourceDataObj.nameOrNumberEnable) {
                         ...
                         * sequenceNumber are not null, display them together.
                         * If the message id alone is not null, display it, but
                         * if it is null, don't display the sequence number.
                         */
                        if (condensedISIG.getMessageID() != null) {
                            String locMessageId = condensedISIG.getMessageID();
                            /*
                             * (Non-Javadoc) This is retained from legacy code:
                             * If any of the sigmets (KZNY/KZMA/KZHU/TJZS) in
                             * the Atlantic are encountered, the label is
                             * preceded with the character 'A'.
                             */
    ...
    private void drawCircleWithMarker(IGraphicsTarget graphicsTarget,
            PixelCoordinate circleCenterPixelCoord, double radius,
            double delta, RGB polygonLineColor, int polygonLineWidth)
            throws VizException {
    ...
    private void drawPolygonSurroundingLine(IGraphicsTarget graphicsTarget,
            ...
            int distance, String polygonExtent) throws VizException {
        if (polygonVertexWorldCoord != null
            ...
            if ((polygonExtent == null) || polygonExtent.isEmpty()
                ...
                if ((polygonExtent != null && !(polygonExtent.isEmpty()))
                        && (polygonExtent.contains("WI")
                                || polygonExtent.contains("WTN") || polygonExtent
                                .contains("WITHIN"))) {
     ...
     */
    private ArrayList<PixelCoordinate> generatePixelCoordinateListFromWorldCoordinateList(
         ...
         */
        ArrayList<PixelCoordinate> pixelCoordList = new ArrayList<PixelCoordinate>(
                0);
    ...
    private void drawPolygon(IGraphicsTarget graphicsTarget,
            ArrayList<PixelCoordinate> polygonCoordinatesList,
            RGB polygonLineColor, int polygonLineWidth, LineStyle lineStyle)
            throws VizException {
    ...
    private IntlSigmetRscDataObj getCondensedIntlSigmetRecord(
        ...
        condensedIntlSigmetRecord.FRAME_DATE_FORMAT.setTimeZone(TimeZone
                .getTimeZone("GMT"));

        condensedIntlSigmetRecord.eventTime = new DataTime(
                completeIntlSigRecord.getStartTime(), new TimeRange(
        ...
        condensedIntlSigmetRecord.endTimeText = new String(
                condensedIntlSigmetRecord.FRAME_DATE_FORMAT
                        .format(condensedIntlSigmetRecord.endTime
        ...
        condensedIntlSigmetRecord.weatherHazardEnumList = condensedIntlSigmetRecord
                .generateWeatherHazardEnumList(completeIntlSigRecord
                        .getHazardType());
        ...
        condensedIntlSigmetRecord.polygonVertexWorldCoord = condensedIntlSigmetRecord
                .generateWorldCoordinatesForPolygonFromLatLonArray(condensedIntlSigmetRecord
                        .getArrayLatLonPoints());
    ...
    private class IntlSigmetRscDataObj implements IRscDataObject {
        ...
        String intensity; // example: "NC" (NC - No Change -- not currently
                          // displayed)
        ...
        final SimpleDateFormat FRAME_DATE_FORMAT = new SimpleDateFormat(
                "dd/HHmm");
        ...
        private Coordinate getCentroidInWorldCoordinates(
            ...
            if (vertexListInPixel != null && vertexListInPixel.size() > 0) {
                ArrayList<Coordinate> locCoordArray = new ArrayList<Coordinate>(
                        0);
                ListIterator<PixelCoordinate> it = vertexListInPixel
                        .listIterator();
                while (it.hasNext()) {
                    PixelCoordinate currLoc = it.next();
                    locCoordArray.add(new Coordinate(currLoc.getX(), currLoc
                            .getY()));
                }
         ...
         */
        private void computeAlternateLocationForSymbol(
                String sigmetIssueOffice, String omwoName, String atsuName) {
            /* Code to get the coordinates of the issueOffice. */
            String queryStr1 = "select station_id, latitude, longitude from stns.intlsig";

            Double stnLatitude = (double) IDecoderConstantsN.FLOAT_MISSING; // TODO:
                                                                            // Should
                                                                            // we
                                                                            // create
                                                                            ...
                                                                            // instead
                                                                            // ?
            Double stnLongitude = (double) IDecoderConstantsN.FLOAT_MISSING; // TODO:
                                                                             // Should
                                                                             // we
                                                                             // create
                                                                             // IDecoderConstantsN.DOUBLE_MISSING
                                                                             ...
                                                                             // ?
            try {
                List<Object[]> stationTableInfo = NcDirectDbQuery.executeQuery(
                        queryStr1, "ncep", QueryLanguage.SQL);
                ...
                for (Object[] stationObjectArray : stationTableInfo) {
                    ...
                    if (stnCode.equals(sigmetIssueOffice)) {
                        ...
                        this.setAlternateSymbolLocationLatitude(stnLatitude
                                .doubleValue());
                        this.setAlternateSymbolLocationLongitude(stnLongitude
                                .doubleValue());
                        break;
                ...
                stnLatitude = (double) IDecoderConstantsN.FLOAT_MISSING; // TODO:
                                                                         // Should
                                                                         // we
                                                                         // create
                                                                         // IDecoderConstantsN.DOUBLE_MISSING
                                                                         // instead
                                                                         // ?
                stnLongitude = (double) IDecoderConstantsN.FLOAT_MISSING; // TODO:
                                                                          // Should
                                                                          // we
                                                                          // create
                                                                          // IDecoderConstantsN.DOUBLE_MISSING
                                                                          ...
                                                                          // ?
                System.out
                        .println("Unable to retrieve the latitude and longitude of "
         ...
         *         otherwise.
         */
        private ArrayList<Coordinate> generateWorldCoordinatesForPolygonFromLatLonArray(
                LatLonPoint[] localLatLonPointArray) {
            ArrayList<Coordinate> polygonWorldCoordList = new ArrayList<Coordinate>(
                    0);
            ...
            if (!this.isLocationLookUpFailed() && localLatLonPointArray != null
                ...
                for (LatLonPoint currentPoint : localLatLonPointArray) {
                    ...
                    polygonWorldCoordList.add(new Coordinate(latLon[0],
                            latLon[1]));
                }
            }
        ...
        private boolean isSIGMETInAtlantic(String issueOff, String omwo,
            ...
            if (issueOff != null
                    && ((issueOff.contains("KZMA"))
                            || (issueOff.contains("KZNY"))
                            || (issueOff.contains("KZHU")) || (issueOff
                            .contains("TJZS")))) {
                inAtlantic = true;
            }

            if (omwo != null
                    && ((omwo.contains("KZMA")) || (omwo.contains("KZNY"))
                            || (omwo.contains("KZHU")) || (omwo
                            .contains("TJZS")))) {
                inAtlantic = true;
            }

            if (atsu != null
                    && ((atsu.contains("KZMA")) || (atsu.contains("KZNY"))
                            || (atsu.contains("KZHU")) || (atsu
                            .contains("TJZS")))) {
                inAtlantic = true;
            }
        ...
        ArrayList<WeatherHazardType> generateWeatherHazardEnumList(String hazard) {
            ArrayList<WeatherHazardType> enumWeather = new ArrayList<WeatherHazardType>(
                    0);
            /*
        ...
        private ArrayList<SymbolAttributesSubSet<RGB, Integer, Float, Boolean, String>> generateListOfSymbolAttributesForAllWeatherHazards(
            ...
            ArrayList<SymbolAttributesSubSet<RGB, Integer, Float, Boolean, String>> localSymbolAttributeSubsetList = new ArrayList<SymbolAttributesSubSet<RGB, Integer, Float, Boolean, String>>(
                    0);
            ...
            for (WeatherHazardType currentHazard : listOfWeatherHazards) {
                ...
                case TS:/* Thunderstorm */
                    ...
                    localSymbolAttributeSubsetList
                            .add(new SymbolAttributesSubSet<RGB, Integer, Float, Boolean, String>(
                ...
                case HU:/* Hurricane */
                    ...
                    localSymbolAttributeSubsetList
                            .add(new SymbolAttributesSubSet<RGB, Integer, Float, Boolean, String>(
                                    ...
                                    "HURRICANE_NH", // change to HURRICANE_SH if
                                                    // location is in Southern
                                                    // hemisphere
                                    0)); // Hurricance does not have a line
                ...
                case TR:/* Tropical Storm */
                    ...
                    localSymbolAttributeSubsetList
                            .add(new SymbolAttributesSubSet<RGB, Integer, Float, Boolean, String>(
                                    ...
                                    "TROPICAL_STORM_NH", // change to
                                                         // TROPICAL_STORM_NH if
                                                         // location is in
                                                         // Southern hemisphere
                                    0)); // Tropical Storm does not have a line
                                         // width associated with it.
                ...
                case TD:/* Tropical Depression */
                    ...
                    localSymbolAttributeSubsetList
                            .add(new SymbolAttributesSubSet<RGB, Integer, Float, Boolean, String>(
                                    ...
                                    "TROPICAL_DEPRESSION", // refer
                                                           // symbolPatterns.xml
                                    0)); // Tropical Depression does not have a
                                         // line width associated with it.
                ...
                case TC:/* Tropical Cyclone */
                    ...
                    localSymbolAttributeSubsetList
                            .add(new SymbolAttributesSubSet<RGB, Integer, Float, Boolean, String>(
                                    ...
                                    "TROPICAL_CYCLONE", // refer
                                                        // symbolPatterns.xml
                                    0)); // Tropical Cyclone does not have a
                                         // line width associated with it.
                ...
                case WS:/* Wind Shear */
                    ...
                    localSymbolAttributeSubsetList
                            .add(new SymbolAttributesSubSet<RGB, Integer, Float, Boolean, String>(
                                    ...
                                    "ASTERISK", /*
                                                 * Actually wind shear does not
                                                 * have any symbol associated
                                                 * with it
                                                 */
                ...
                default: /* TODO Check if this is applicable for TEST sigmets */
                    localSymbolAttributeSubsetList
                            .add(new SymbolAttributesSubSet<RGB, Integer, Float, Boolean, String>(
                                    new RGB(255, 255, 255), 2, 1.5f, true,
                                    "ASTERISK", 2));
                    break;
                }
            }
    ...
    public IRscDataObject[] processRecord(Object isigObj) {
        if (!(isigObj instanceof IntlSigmetRecord)) {
            System.out
                    .println("IntlSigmentResource.processRecord: object is not a "
                            + "IntlSigmetRecord: "
    ...
    private class FrameData extends AbstractFrameData {
        ...
        // private ArrayList<IntlSigmetRecord> intlsigmetCancelList; move to
        // resource class

        /**
         * Overridden method to determine if the International SIGMET belongs to
         * the current frame
         * 
         * @param dataObj
         *            - The International SIGMET record
         * @return true if the SIGMET belongs to the current frame and false
         *         otherwise
         */
        // @Override
        ...
        //
        // // return false if the sigmet end time is before the frame start time
        // or
        ...
        public FrameData(DataTime time, int frameInterval) {
            ...
            condensedIntligLinkedHashMap = new LinkedHashMap<String, IntlSigmetRscDataObj>(
                    0);
        }
        ...
        public boolean updateFrameData(IRscDataObject rscDataObj) {
            if (!(rscDataObj instanceof IntlSigmetRscDataObj)) {
                System.out
                        .println("IntlSigmetResource:updateFrameData() processing.....\n"
            ...
            // }
            // else{
            // IntlSigmetRscDataObj condensedIntlSigmetRecord =
            // getCondensedIntlSigmetRecord(isigRecord);

            /*
             * (Non-Javadoc) Since the messageId and the sequenceNumber may be
             * null in some international sigmets,the issue time is also added
             * to create a key
             */
            ...
            if (this.condensedIntligLinkedHashMap.isEmpty()
                ...
                this.condensedIntligLinkedHashMap
                        .put(key, intlSigmetRscDataObj);
            }
            // }
