...
public class H5ScdParser {
     ...
     * 			
     */
    public static void processH5Scd(String message, H5ScdRecord record) {
    	/*
    	...
    	String h5scdreport = null;
//    	Boolean suspectTimeFlag = false;
    	String suspectTimeFlag = "false";
    	while (m.find()) {
    		h5scdreport = m.group().replaceAll("\r\r\n", " ").trim();
    		h5scdreport = UtilN.removeExtraWhiteSpaces(h5scdreport);

    		/*
    		 * Set raw report and station ID.
    		 */
    		record.setReport (h5scdreport);   	    		
    		String[] xxx = h5scdreport.split(" ", 20);
...
//    				suspectTimeFlag = true;
    		 ...
    		 */
    		indx = h5scdreport.indexOf (" 8");
    		if ( indx >= 0 ) {
    			try {
    				record.setCFRT(Integer.parseInt(h5scdreport.substring(indx+2,indx+3)));
    			} catch (Exception e ) {
    				/* do nothing */
    			}
    			try {
    				record.setCFRL(Integer.parseInt(h5scdreport.substring(indx+3,indx+4)));
    			} catch (Exception e ) {
    				/* do nothing */
    			}
    			try {
    				record.setCTYL(Integer.parseInt(h5scdreport.substring(indx+4,indx+5)));
    			} catch (Exception e ) {
    				/* do nothing */
    			}
    			 ...
    			 */
    			try {
    				if ( h5scdreport.substring(indx+3,indx+7).equals("0000")) {
    					int cbas = 9;
    					record.setCBAS(cbas);
    				} else {
    					record.setCBAS(Integer.parseInt(h5scdreport.substring(indx+5,indx+6)));
    				}
    			} catch (Exception e ) {
    				/* do nothing */
    			}
    			try {
    				record.setCTYM(Integer.parseInt(h5scdreport.substring(indx+6,indx+7)));
    			} catch (Exception e ) {
    				/* do nothing */
    			}
    			try {
    				record.setCTYH(Integer.parseInt(h5scdreport.substring(indx+7,indx+8)));
    			} catch (Exception e) {
    				/* do nothing */
    			}
    		}
    		 ...
    		 */
    		indx = h5scdreport.indexOf (" 931");
    		if ( indx >= 0 ) {
    			try {
    				int snew=Integer.parseInt(h5scdreport.substring(indx+4,indx+7));
    		 ...
    		 */
    		indx = h5scdreport.indexOf (" 933");
    		if ( indx >= 0 ) {
    			try {
    				int weqs =Integer.parseInt(h5scdreport.substring(indx+4,indx+7));
    		 ...
    		 */
    		indx = h5scdreport.indexOf (" 4/");
    		if ( indx >= 0 ) {
    			try { 
    				int snow=Integer.parseInt(h5scdreport.substring(indx+3,indx+6));
    		 ...
    		 */
    		indx = h5scdreport.indexOf (" 40");
    		int indx1 = h5scdreport.indexOf (" 41");
    		if ( indx > 0 || indx1 > 0) {
    			try { 
    				if ( indx1 > 0 ) {
    					indx = indx1;
    				}
    				int sign=Integer.parseInt(h5scdreport.substring(indx+2,indx+3));
    				int tmax=Integer.parseInt(h5scdreport.substring(indx+3,indx+6));
    				if ( sign == 1 ) tmax = - tmax;
    				record.setTDXC((float)(tmax/10.F));
    				sign=Integer.parseInt(h5scdreport.substring(indx+6,indx+7));
    				int tmin=Integer.parseInt(h5scdreport.substring(indx+7,indx+10));
    				if ( sign == 1 ) tmin = - tmin;
    		 ...
    		 */
    		indx = h5scdreport.indexOf (" 6");
    		if ( indx >= 0 ) {
    			try {
    				int p06i=Integer.parseInt(h5scdreport.substring(indx+2,indx+6));
    		 ...
    		 */
    		indx = h5scdreport.indexOf (" 98");
    		if ( indx >= 0 ) {
    			try {
    				int msun=Integer.parseInt(h5scdreport.substring(indx+3,indx+6));
    				record.setMSUN(msun);
    			} catch (Exception e) {
    				/* do nothing */
    			}
    		 ...
    		 */
    		indx = h5scdreport.indexOf (" 24/931");
    		if ( indx >= 0 ) {
    			try {
    				int s24i=Integer.parseInt(h5scdreport.substring(indx+7,indx+10));
    		 ...
    		 */
    		indx = h5scdreport.indexOf (" 7");
    		if ( indx >= 0 ) {
    			try {
    				int p24i=Integer.parseInt(h5scdreport.substring(indx+2,indx+6));
