...
public class GridIndicesDisplay implements IRenderable {
    ...
    public GridIndicesDisplay(RGB color, IMapDescriptor descriptor, ISpatialObject gridLocation) {
...
//    	int interv = (int) ((interval0 + 15) / 15.0);
    	...
    	if (interv > 4) interv = 4;
    	
    	double centerX = 0.5 * (screenExtentInPixels.getMinX() + screenExtentInPixels.getMaxX());
    	double centerY = 0.5 * (screenExtentInPixels.getMinY() + screenExtentInPixels.getMaxY());
    	...
    	for (int i = 0; i < gridDims[0]; i++) {
    		for (int j = 0; j < gridDims[1]; j++) {
    			...
    			try {
    				...
    				double dy = (centerY - c.asPixel(descriptor.getGridGeometry()).y);
