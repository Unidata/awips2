...
public class GridRelativeHiLoDisplay implements IRenderable {
	    ...
	    public GridRelativeHiLoDisplay (HILOBuilder hiloBuild, HILORelativeMinAndMaxLocator hiloLocator, TextStringParser markerStr,
	    		TextStringParser valueStr, IMapDescriptor descriptor, ISpatialObject gridLocation) {
...
//	    	display ();
	     ...
	     * Draw Highs
	     */
	    private void paintHighs (IGraphicsTarget target, PaintProperties paintProps ) throws VizException {
	    	
	    	if (  gridPointHiSymbolSet != null ) {
	    		DisplayElementFactory df = new DisplayElementFactory (target, this.descriptor);
				ArrayList<IDisplayable> elements = df.createDisplayElements(gridPointHiSymbolSet, paintProps);
	     ...
	     * Draw Lows
	     */
	    private void paintLows (IGraphicsTarget target, PaintProperties paintProps ) throws VizException {
	    	
	    	if ( gridPointLoSymbolSet != null ) {
	    		DisplayElementFactory df = new DisplayElementFactory (target, this.descriptor);
				ArrayList<IDisplayable> elements = df.createDisplayElements(gridPointLoSymbolSet, paintProps);
				for (IDisplayable each : elements)
				{
					each.draw(target);
					each.dispose();
	     ...
	     * Draw High character
	     */
	    private void drawHiCharacter ( IGraphicsTarget target, PaintProperties paintProps ) throws VizException {
	    	...
	    	if ( hiloBuild.getSymbolHi().length() > 0 && tmp1.length > 0 ) {
    			...
    			IFont font = target.initializeFont(getFontName(textMarkerString.getTextFont()), 
    					textMarkerString.getTextSize(), new IFont.Style[] { IFont.Style.BOLD });
    			...
    			for ( int i = 0; i < tmp1.length ; i ++ ) {
    				...
    				try {
    					...
    					target.drawString(font, text, d[0], d[1], 0.0, TextStyle.NORMAL,
    							hiloBuild.getColorHi(), HorizontalAlignment.CENTER, VerticalAlignment.MIDDLE,
    							0.0);
	     ...
	     * Draw Low character
	     */
	    private void drawLoCharacter ( IGraphicsTarget target, PaintProperties paintProps ) throws VizException {
	    	...
	    	if ( hiloBuild.getSymbolLo().length() > 0 && tmp1.length > 0 ) {
    			...
    			for ( int i = 0; i < tmp1.length ; i ++ ) {
    				...
    				try {
    					...
    					target.drawString(font, text, d[0], d[1], 0.0, TextStyle.NORMAL,
    							hiloBuild.getColorLo(), HorizontalAlignment.CENTER, VerticalAlignment.MIDDLE,
    							0.0);
    				} catch (TransformException e) {
    					// TODO Auto-generated catch block
    					e.printStackTrace();
	    ...
	    private void plotGridHiValue ( IGraphicsTarget target ) throws VizException {
    		...
    		if ( tmp1.length > 0 ) {
    			...
    			IFont font = target.initializeFont(getFontName(textValueString.getTextFont()), 
    					textValueString.getTextSize(), new IFont.Style[] { IFont.Style.BOLD });
    		    if ( offY > 0.0 ) {
    		    	charSize              = target.getStringBounds(font, "N");
    			...
    			for ( int i = 0; i < tmp1.length ; i ++ ) {
    				...
    				try {
    					...
    					target.drawString(font, text, d[0], d[1]+offY, 0.0, TextStyle.NORMAL,
    							hiloBuild.getColorHi(), HorizontalAlignment.CENTER, VerticalAlignment.MIDDLE,
    							0.0);
    				} catch (TransformException e) {
    					// TODO Auto-generated catch block
	    ...
	    private void initHiLoSymbolSet () {
	    	 ...
	    	 */
	    	if ( (tmp1.length > 0 ) && 
	    		( (hiloBuild.getSymbolHiType() == 2) || (hiloBuild.getSymbolHiType() == 3 ) )) {
	    		if ( hiloBuild.getSymbolHiType() == 2 ) {
	    			//set marker
	    			tmp = "2/" + hiloBuild.getSymbolHiMarkerNumber() + "/" + textMarkerString.getTextWidth();
	    		...
	    		markerColor = new Color(hiloBuild.getColorHi().red, hiloBuild.getColorHi().green, hiloBuild.getColorHi().blue);
	    		...
	    		gridPointHiSymbolSet = new SymbolLocationSet (
	    				null,
	    				colors,
	    				textMarkerString.getTextWidth(),
	    				textMarkerString.getSymbolMarkerSize(),
	    				false,
	    				locations1,
	    				catType,
	    				symbolName);
	    		symbolHiHight = gridPointHiSymbolSet.getSymbol().getSizeScale()*charPix;
	    	}
	    	...
	    	if ( (tmp1.length > 0 ) && 
	    			( (hiloBuild.getSymbolLoType() == 2) || (hiloBuild.getSymbolLoType() == 3) )) {
	    		if ( hiloBuild.getSymbolLoType() == 2 ) {
	    			//set marker
	    			tmp = "2/" + hiloBuild.getSymbolLoMarkerNumber() + "/" + textMarkerString.getTextWidth();
	    			marker = new MARKER(tmp);
	    			symbolName = marker.getMarkerName();
	    			catType = "Markers";
	    		}
	    		...
	    		markerColor = new Color(hiloBuild.getColorLo().red, hiloBuild.getColorLo().green, hiloBuild.getColorLo().blue);
	    		...
	    		gridPointLoSymbolSet = new SymbolLocationSet (
	    				null,
	    				colors1,
	    				textMarkerString.getTextWidth(),
	    				textMarkerString.getSymbolMarkerSize(),
	    				...
	    				catType,
	    				symbolName);
	    		symbolLoHight = gridPointLoSymbolSet.getSymbol().getSizeScale()*charPix;
	    	}
	    ...
	    private String getSymbolName ( int symbolNum ) {
	    	...
	    	String []name = { "SQUARE","FILLED_SQUARE","CIRCLE", "FILLED_CIRCLE","TRIANGLE_SPCL" ,"FILLED_TRIANGLE_SPCL",
	    			"DIAMOND_SPCL","FILLED_DIAMOND_SPCL","STAR_SPCL","FILLED_STAR_SPCL","HIGH_PRESSURE_H","LOW_PRESSURE_L",
	    			"FILLED_HIGH_PRESSURE_H","FILLED_LOW_PRESSURE_L","SINGLE_BRACKET","BOTTOM_HALF_OF_BRACKET",
	    			"TOP_HALF_OF_BRACKET"};
	    ...
	    private void display () {
	    	int i;
	    	System.out.println ( "    HIGH -- Color\t\t[" + hiloBuild.getColorHi().red + " " + hiloBuild.getColorHi().green + " " + hiloBuild.getColorHi().blue + "]");
			...
			System.out.println ( "    LOW -- Color\t\t[" + hiloBuild.getColorLo().red + " " + hiloBuild.getColorLo().green + " " + hiloBuild.getColorLo().blue + "]");
			...
			System.out.println ( "            Text String: size[" + textMarkerString.getTextSize() + ";" +textMarkerString.getSymbolMarkerSize()+ "] font[" + textMarkerString.getTextFont() + "] width[" +
					textMarkerString.getTextWidth() + "] HW Flag [" + textMarkerString.getTextHWFlag() + "]");
			System.out.println ( "            Value String: size[" + textValueString.getTextSize() + ";" + textValueString.getSymbolMarkerSize()+ "] font[" + textValueString.getTextFont() + "] width[" +
					textValueString.getTextWidth() + "] HW Flag [" + textValueString.getTextHWFlag() + "]");
			System.out.println ( "==================================================" );
	    }
}
