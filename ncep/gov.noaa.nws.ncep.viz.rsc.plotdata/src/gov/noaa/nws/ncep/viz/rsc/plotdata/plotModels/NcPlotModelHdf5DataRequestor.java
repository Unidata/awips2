...
public class NcPlotModelHdf5DataRequestor {
    ...
    // to hold the values from the pointDataContainer. 
...
//	 Map< AbstractMetParameter, RequestConstraint > condMetParamReqConstraintMap = null;
    public void queueStationsForHdf5Query(DataTime dt , Collection<Station> listOfStations ){
    	...
    	timeLogger.append("About to query Hdf5 data for frame: " + dt.toString() + "\n");
    ...
    public NcPlotModelHdf5DataRequestor( PlotModel plotModel, String level, Map<String, RequestConstraint> constraintMap, 
    		                             IPointInfoRenderingListener listener, double initialPlotDensity, ConditionalFilter cf){
					...
					if( thisPlotParamDefn != null ){
						if( thisPlotParamDefn.getDeriveParams()  != null ){
							...
							String dbPrmName = thisPlotParamDefn.getDbParamName();
							  MetParameterFactory.getInstance().alias( thisPlotParamDefn.getMetParamName(), 
									  dbPrmName );
                              AbstractMetParameter condColoringParam = MetParameterFactory.getInstance().
                              createParameter(  thisPlotParamDefn.getMetParamName(), thisPlotParamDefn.getPlotUnit() );
	...
	/**
	 * Filters the stations assuming that the conditional filter has parameters that are available in the plot model
	 * @param dataTime
	 * @param stationSet
	 */
            ...
            for( PlotParameterDefn eachPlotParamDefn : listOfAllPlotParamDefnsForThisPlugin ){
            	 ...
            	 if( condFilterMap.containsKey( plotParamName ) ){
            		 ...
            		 if( eachPlotParamDefn.getDeriveParams() != null ){
            			   ...
            			   MetParameterFactory.getInstance().alias( eachPlotParamDefn.getMetParamName(), 
            					                                    eachPlotParamDefn.getDbParamName() );
            			   condMetParam = MetParameterFactory.getInstance().
						   createParameter(  eachPlotParamDefn.getMetParamName(), eachPlotParamDefn.getPlotUnit() );
    			...
    			if(plotPrmDefn.getMetParamName().compareTo(metPrm.getMetParamName()) == 0){
    			       ...
    			       for( String condPlotParamName : condPlotParamNameSet ){
    			           if( condPlotParamName.compareTo(plotParamName) == 0 ){
    			        	   ...
    			        	   AbstractMetParameter condMetParam =  MetParameterFactory.getInstance().
    			        		   ...
    			        		   if(!condMetParam.hasStringValue())
    			        			   ...
    			        			   condMetParam.setStringValue(metPrm.getStringValue());
       							      ...
       							      if( plotTrim != 0 ) {
       							       ...
       							       boolean result = condMetParam.hasStringValue() ? reqConstraint.evaluate(formattedPlotString) 
       									                 : reqConstraint.evaluate(Double.parseDouble(formattedPlotString));
	...
	private void establishPlotParamDefnToMetParamMappings() throws VizException{
    	...
    	if(listOfAllPlotParamDefnsForThisPlugin != null && !listOfAllPlotParamDefnsForThisPlugin.isEmpty()){
    	  ...
    	  timeLogger.append(" establishPlotParamDefnToMetParamMappings() took "+ (t1 - t0)/1000000 + " ms\n");	
    	}
...
//		if(metParamNameToDbNameMap != null && !metParamNameToDbNameMap.isEmpty())
//			metParamNameToDbNameMap.clear();
       ...
       determineParameterNamesForHdf5Query();
 	  long t1 = System.nanoTime();
	  timeLogger.append(" updateListOfParamsToPlotFromCurrentPlotModel() took "+ (t1 - t0)/1000000 + " ms\n");
    }
    ...
    public void dispose(){
    	timeLogger.append("\n Invoking NcPlotModelHdf5DataRequestor.dispose()\n");
...
//		System.out.println("ParamsToPlot KeySet: "+paramsToPlot.keySet());
		long t1 = System.nanoTime();
		timeLogger.append("addToParamsToPlot() took "  + (t1 - t0)/1000000 + " ms\n");
	}
	...
	private AbstractMetParameter addToDerivedParamsList(String[] deriveParams, PlotParameterDefn plotPrmDefn ) {
		...
		long t1 = System.nanoTime();
		timeLogger.append("addToDerivedParamsList() took " + (t1 - t0)/1000000 + " ms for " + derivedMetParam.getMetParamName() + "\n");
    ...
    private void determineDBParamNamesForDerivedParameters(){
    	...
    	long t1 = System.nanoTime();
    	timeLogger.append("getListOfDbParamNamesForDerivedMetParamsArgs() " + (t1 - t0)/1000000 + " ms\n");
    ...
    private void determineParameterNamesForHdf5Query(){
	    ...
	    if (this.derivedParamsList != null 
	    		&&!this.derivedParamsList.isEmpty()){
		...
		long t1 = System.nanoTime();
		timeLogger.append("determineParameterNamesForHdf5Query() took " + (t1 - t0)/1000000 + " ms\n");
    }
    ...
    private  Collection<Station>  requestUpperAirData( List<Station> listOfStationsRequestingForData ){
		...
		timeLogger.append("\nrequestUpperAirData()-->sndingQuery.query() took "+ (t005-t004)/1000000 + " ms");		
        ...
        if( sndingCube != null && sndingCube.getRtnStatus() == QueryStatus.OK ) {
        	...
        	synchronized(listOfSoundingProfiles){
                ...
                /*Clear the existing list of parameters to plot in each station - to guarantee
                 * an updated list if there is a re-query for parameters by editing the plot model*/
...
//	                	 paramsToPlot = new HashMap<String, AbstractMetParameter>(paramsToPlot);
...
//                    	}
                   ...
                   for( AbstractMetParameter derivedParam : derivedParamsList ) {
                	   try {
                		 ...
                		 if( paramsToPlot.containsKey(derivedParam.getMetParamName()) ){
                      		      //if ( !currentStation.listOfParamsToPlot.contains( clonedDerivedPrm ))
                ...
                /*Validate the station against conditionally derived MetParameters*/
                if(condFilterMap != null && !condFilterMap.isEmpty() ){
                   synchronized(setOfCondDerivedMetParamNames){	
                     for( String condMetParamName : setOfCondDerivedMetParamNames ){
                	      ...
                	      for( AbstractMetParameter condDerivedParamToCheck : derivedParamsList ){
                	           	if( condDerivedParamToCheck.getMetParamName()
                	           			 .compareTo(condMetParamName) == 0 ){
                ...
                /*Process the conditional parameter(s) (if any) for the station*/
                if ( setOfCondColoringParamNames != null && !setOfCondColoringParamNames.isEmpty() ){
                	...
                	synchronized(setOfCondColoringParamNames){
                	  for ( String condColorParamName : setOfCondColoringParamNames ){
                		...
                		currentStation = processConditionalParameterForEachStation(dbMetParamColl, 
                				                                                   currentStation, 
                				                                                   condColorParamName);
                		
                		currentStation = processConditionalParameterForEachStation(derivedParamsList,
                				                                                   currentStation, 
                				                                                   condColorParamName);
                	  }
                	}
                ...
                /*Evaluate the station against the conditional filter 
...
//    			     if( index == 0 ){
//    			         System.out.println("\n for the frame " + time.toString() + "dt[0] = " + dt[0].toString());
//    			     }
    			     ...
    			     if( plotProp.hasDistinctStationId ){
    			          ...
    			          timeConstraint.addToConstraintValueList(currentStation.info.dataTime.toString());
    			     }
    			     else
    			    	  rc.addToConstraintValueList(currentStation.info.dataURI);
    			     
    			     stationMap.put( formatLatLonKey(      currentStation.info.latitude,
            ...
            if( plotProp.hasDistinctStationId ){
    			    ...
    			    if( dt.length > 1 && !dt[0].equals(dt[1])){
...
//            System.out.println("About to query Hdf5 data for frame: " + time.toString() + "\n");
//                    System.out.println("HDF5 query map = " + map);
    			    boolean displayStationPlot = false;
    			    long t0 = System.nanoTime();
    			    PointDataContainer pdc = null;
    			    		pdc = DataCubeContainer.getPointData(plugin, this.parameters, null, map);
    			    long t1 = System.nanoTime();
    			    timeLogger.append("DataCubeContainer.getPointData() took " + (t1 - t0)/1000000 + " ms \n" );
    			    int pdcSize = -1;
                    if(pdc == null ){
    	            ...
    	            for( int uriCounter = 0; uriCounter < pdcSize ; uriCounter++ ) {
    	                ...
    	                String key = new String( formatLatLonKey( pdv.getFloat(latDbName), 
    	                ...
    	                for( String dbPrm : setOfDBParamNamesForHdf5Query ) {
    	                	...
    	                	/*Set the value for Met parameters from the corresponding database value*/
                            setMetParamFromPDV(metPrm, pdv, dbPrm, dataTime);    	                	
    	                	
//                            if( metPrm.getMetParamName().compareTo("StationID") == 0 ){
//                            	if(metPrm.getStringValue().compareTo(currentStation.info.stationId) != 0 ){
...
//                            }
    	                ...
    	                /*Validate the station against a conditional derived MetParameter*/
    	                if(condFilterMap != null && !condFilterMap.isEmpty() ){
    	                    for( String condMetParamName : setOfCondDerivedMetParamNames ){
    	                	  synchronized(derivedParamsList){
    	                	     for( AbstractMetParameter condDerivedParamToCheck : derivedParamsList ){
    	                ...
    	                synchronized( metParamsToDisplay ){
    	                  try{	
    	                   for ( AbstractMetParameter metParam  : metParamsToDisplay ){
    	                		/*Creating a fresh copy of the met parameter seems to be the only way
    	                		 *that each station retains a unique set of values as queried (or derived). 
    	                		 *Otherwise all stations in a frame get the MetParameter values of the last station being processed
    	                		 *since the list currentStation.listOfParamsToPlot references the AbstractMetParameter
    	                		 *values from paramsToPlot*/
...
//    	                   System.out.println( "\nFor frame: " + time.toString() + " stn dataTime: " 
...
//    	                		             + currentStation.listOfParamsToPlot);
    	                ...
    	                synchronized( paramsToPlot ){
    	                   ...
    	                   synchronized( pkeySet ){
    	                	   try{
    	                        for( String prmToPlotKey : pkeySet ){
    	                        	//System.out.println("paramsToPlotKey = " + prmToPlotKey);
    	                	         AbstractMetParameter prmToPlot = paramsToPlot.get( prmToPlotKey );
    	                	         if( prmToPlot != null ){
    	                	             prmToPlot.setValueToMissing();
    	                	             paramsToPlot.put( prmToPlot.getClass().getSimpleName(), prmToPlot );
    ...
    private final class GetDataTask implements Runnable{
		...
		public void run() {
        	if(levelStr == null )
		     ...
		     timeLogger.append("\nFinished getting data for " + stationsWithData.size()
		    		           + " stations in " + (t1 - t0)/1000000 + " ms for frame: " + time.toString() );
