 ...
 *     is a scaling factor with a default of 3.  It defines a moving search area
 *     where extrema are found.                                 
 *                                                                      
 * <p>
 *<pre>
 * SOFTWARE HISTORY
...
public class HILORelativeMinAndMaxLocator {
	...
	protected enum HILORelativeMinAndMaxLocatorErrCodes{
				NO_ERROR(0),
				INVALID_GRID_VALUES(1),
				TOO_MANY_GRIB_POINTS(2);
	...
	public HILORelativeMinAndMaxLocator ( float [] gridData, int col, int row, int radius, boolean interp,
			                              int maxHi, int maxLo, float maxLowerBd , float maxUpperBd,
		...
		for ( j = gridRadius + 1; j <=  gridRow - gridRadius; j ++ ) {
			for ( i = gridRadius + 1; i <= gridCol - gridRadius; i ++ ) {
				...
				if ( ! checkGridMissingValues (grid[cidx])){
					...
					while ((jj <= (j + gridRadius)) && (ok)) {
						...
						while ((ii <= (i+gridRadius))&&(ok)) {
							...
							if ((ii != i || jj != j) && 
								...
								if ( mm == 0 ) {
									if ( grid[cidx] > grid[cidx1] ) ok = false;
								}
								else {
									if ( grid[cidx] < grid[cidx1] ) ok = false;
								}
								if ( !checkGridPointValuesDiff(grid[cidx],grid[cidx1])) alleq = false;
							
								if ( ok && gridInterp ){
									if ( (ii == i - 1) && (jj == j) ) qxm1 = grid[cidx1];
									if ( (ii == i + 1) && (jj == j) ) qxp1 = grid[cidx1];
									if ( (jj == j - 1) && (ii == i) ) qym1 = grid[cidx1];
									if ( (jj == j + 1) && (ii == i) ) qyp1 = grid[cidx1];
								}
							}
							ii ++;
						}
					...
					if ( ok && (!alleq) && 
					     ((checkGridPointValuesDiff(min,max)) || 
					      ((grid[cidx] >= min)&&( grid[cidx] <= max)))) {
						xq = i;
						yq = j;
						vq = grid[cidx];
						if ( gridInterp && (!checkGridMissingValues(qxm1))&&
								(!checkGridMissingValues(qxp1))&&(!checkGridMissingValues(qym1))&&
	...
	private void chkClusterExist ( int icntpt, boolean [] keep, int [] idofcl) {
		...
		while ( iptrcl <= numOfClusters ) {
			...
			for ( int iptcnt = 1; iptcnt <= numOfClusters; iptcnt++) {
				if ( keep[iptcnt-1]) {
					if ( (Math.abs(xExtr[idofcl[iptrcl-1]-1] - xExtr[iptcnt-1]) <= gridRadius) &&
							(Math.abs(yExtr[idofcl[iptrcl-1]-1] - yExtr[iptcnt-1]) <= gridRadius)){
						numOfClusters ++;
	...
	public void display () {
		...
		if ( isHiLoRelativeMinAndMaxLocated ) {
			...
			System.out.println ( "    Radius:" + gridRadius + " interp:" + gridInterp );
			System.out.println ( "    Max number of highs:" + gridMaxHi + " Max number of lows:" + gridMaxLo );
			System.out.println ( "    Lower Bound on Max:" + gridLowerMax + " Upper Bound on Max:" + gridUpperMax );
			System.out.println ( "    Lower Bound on Min:" + gridLowerMin + " Upper Bound on Min:" + gridUpperMin );
		}
	}
}
