...
public class ProductConverter {
	...
	/*
	 *  Convert a XML file Products object to a list of PGEN in-memory Product objects
	 */
	public static List<Product> convert ( Products filePrds ) {
	   
	    List<Product> prd = new ArrayList<Product>();
	   
	    for (gov.noaa.nws.ncep.ui.pgen.file.Product fPrd : filePrds.getProduct() ) {
	...
	/*
	 *  Convert a list of XML file Layer objects to a list of PGEN in-memory Layer objects
	 */	
	private static List<Layer> convertFileLayers ( 
	    ...
	    for ( gov.noaa.nws.ncep.ui.pgen.file.Layer fLayer : flayers ) {
		    ...
		    lyr.setColor( new Color( fLayer.getColor().getRed(), fLayer.getColor().getGreen(),
		    		                 fLayer.getColor().getBlue(),  fLayer.getColor().getAlpha() ) );
    ...
    private static List<AbstractDrawableComponent> convert ( 
    	    ...
    	    for ( gov.noaa.nws.ncep.ui.pgen.file.Line fLine : elem.getLine() ) {	    
    		    ...
    		    for ( Point pt : fLine.getPoint() ) {
       		    ...
       		    Line line = new Line( null, clr,
       		    		fLine.getLineWidth(), fLine.getSizeScale(), fLine.isClosed(), 
    	...
    	if ( !elem.getSymbol().isEmpty() ) {
    		...
    		for ( gov.noaa.nws.ncep.ui.pgen.file.Symbol fSymbol : elem.getSymbol() ) {
    		    ...
    		    int nn = 0;
        	    for (gov.noaa.nws.ncep.ui.pgen.file.Color fColor : fSymbol.getColor() ) {
    		        clr[nn++] = new Color( fColor.getRed(), fColor.getGreen(),
    		        		               fColor.getBlue(),  fColor.getAlpha() ); 	
    		    }
    	...
    	if ( !elem.getText().isEmpty() ) {
    		...
    		for ( gov.noaa.nws.ncep.ui.pgen.file.Text fText : elem.getText() ) {
    		    ...
    		    Text text = new Text( (Coordinate[])null, fText.getFontName(), 
    		    		              ...
    		    		              (new Coordinate( loc.getLon(), loc.getLat() ) ),    		    		              
    		    		              fText.getRotation(), 
  			    					  TextRotation.valueOf( fText.getRotationRelativity() ), 
  			    					  st,
    		...
    		for ( gov.noaa.nws.ncep.ui.pgen.file.AvnText aText : elem.getAvnText() ) {
    		    ...
    		    AvnText text = new AvnText( (Coordinate[])null, aText.getFontName(), 
    		    		              ...
    		    		              AviationTextType.valueOf(aText.getAvnTextType()), 
  			    					  ...
  			    					  aText.getSymbolPatternName(), aText.getPgenCategory(), aText.getPgenType() );
    	...
    	if ( !elem.getMidCloudText().isEmpty() ) {
    		...
    		for ( gov.noaa.nws.ncep.ui.pgen.file.MidCloudText mText : elem.getMidCloudText() ) {
    		    ...
    		    MidCloudText text = new MidCloudText( (Coordinate[])null, mText.getFontName(), 
    		    		              mText.getFontSize(), 
    	...
    	if ( !elem.getArc().isEmpty() ) {   	    	    
    	    ...
    	    for ( gov.noaa.nws.ncep.ui.pgen.file.Arc fArc : elem.getArc() ) {	    
    		    ...
    		    for ( Point pt : fArc.getPoint() ) {
       		    ...
       		    Arc arc = new Arc( null, clr[ 0 ],
       		    		fArc.getLineWidth(), fArc.getSizeScale(), fArc.isClosed(), 
       		    		fArc.isFilled(), fArc.getSmoothFactor(), 
				        FillPattern.valueOf( fArc.getFillPattern() ), fArc.getPgenType(),
    	...
    	des.addAll(TrackConverter.getTrackElementListByTrackBeanList(elem.getTrack())); 
    	...
    	if ( !elem.getVector().isEmpty() ) {
    		...
    		for ( gov.noaa.nws.ncep.ui.pgen.file.Vector fVector : elem.getVector() ) {
    		    ...
    		    Vector vector = new Vector( null, clr,
    				         fVector.getLineWidth(), fVector.getSizeScale(), 
    				         fVector.isClear(), (new Coordinate( loc.getLon(), loc.getLat() ) ),
    				         vtype, fVector.getSpeed(), fVector.getDirection(),
    	...
    	if ( !elem.getTCA().isEmpty() ) {
    		...
    		for ( gov.noaa.nws.ncep.ui.pgen.file.TCA ftca : elem.getTCA() ) {
	         ...
	         /*   VectorType vtype = null;
				...
				advTime.set(xmlCal.getYear(), xmlCal.getMonth()-1, xmlCal.getDay(), 
    	...
    	if ( !elem.getDECollection().isEmpty() ) {
    		...
    		for ( gov.noaa.nws.ncep.ui.pgen.file.DECollection fdec : elem.getDECollection() ) {
     	...
     	if ( !elem.getWatchBox().isEmpty() ) {
     		for ( gov.noaa.nws.ncep.ui.pgen.file.WatchBox fwb : elem.getWatchBox()){
    	...
    	if ( !elem.getOutlook().isEmpty() ) {
    		...
    		for ( gov.noaa.nws.ncep.ui.pgen.file.Outlook fotlk : elem.getOutlook() ) {
    	...
    	if ( !elem.getSigmet().isEmpty() ) {   	    	    
    	    ...
    	    for ( gov.noaa.nws.ncep.ui.pgen.file.Sigmet fSig : elem.getSigmet() ) {	    
    		    ...
    		    for ( Point pt : fSig.getPoint() ) {
       		    ...
       		    Sigmet sigmet = new Sigmet( null, clr,
       		    		fSig.getLineWidth(), fSig.getSizeScale(), fSig.isClosed(), 
       		    		fSig.isFilled(), sigmetPoints, 
    	...
    	if (!elem.getGfa().isEmpty()) {
			...
			for (gov.noaa.nws.ncep.ui.pgen.file.Gfa fgfa : elem.getGfa()) {
				...
				for (gov.noaa.nws.ncep.ui.pgen.file.Color fColor : fgfa.getColor()) {
					clr[nn++] = new Color(fColor.getRed(), fColor.getGreen(), fColor.getBlue(),
							fColor.getAlpha());
				}
				...
				if(fgfa.getTop() != null && fgfa.getBottom() != null) {
					gfa.setGfaValue(Gfa.TOP_BOTTOM, fgfa.getTop() + "/" + fgfa.getBottom());
				}
    	...
    	if ( !elem.getVolcano().isEmpty() ) {   	    	    
    	    ...
    	    for ( gov.noaa.nws.ncep.ui.pgen.file.Volcano fVol: elem.getVolcano() ) {	 
	...
	public static Products convert ( List<Product> prds ) {
	    ...
	    for ( Product prd : prds ) {
		    ...
		    gov.noaa.nws.ncep.ui.pgen.file.Product p = 
	    		             new gov.noaa.nws.ncep.ui.pgen.file.Product();
    ...
    /*
	 *  Convert a list of PGEN in-memory Layer objects into a list of XML file Layer objects
	 */	
	private static List<gov.noaa.nws.ncep.ui.pgen.file.Layer> convertLayers ( List<Layer> layers ) {
		   
		List<gov.noaa.nws.ncep.ui.pgen.file.Layer>  flyrs = 
			                       new ArrayList<gov.noaa.nws.ncep.ui.pgen.file.Layer>();
	   	   
	    for ( Layer lyr : layers ) {
	    	gov.noaa.nws.ncep.ui.pgen.file.Layer l = 
	    		             new gov.noaa.nws.ncep.ui.pgen.file.Layer();
	    
	    	if ( lyr.getName() != null ) {
		        l.setName( lyr.getName() );	
	        }
	    	
	    	gov.noaa.nws.ncep.ui.pgen.file.Color clr = 
...
*/
	 ...
	 /*
	 *  Convert a list of in-memory PGEN  DrawableElement objects into a list of XML 
	 *  file DrawableElement objects
	 */	
			...
			if ( adc instanceof DrawableElement ){
				...
				if ( de instanceof Line ) {
					...
					if ( de instanceof Arc ) {
						gov.noaa.nws.ncep.ui.pgen.file.Arc arc = 
							new gov.noaa.nws.ncep.ui.pgen.file.Arc();
						...
						gov.noaa.nws.ncep.ui.pgen.file.Gfa fgfa = 
							new gov.noaa.nws.ncep.ui.pgen.file.Gfa();
						...
						if (e.getAttribute(GfaRules.WORDING) != null) {
							...
							fgfa.setFromCondsDvlpg(GfaRules.replacePlusWithCycle(w
									.getFromCondsDvlpg(), e.getGfaCycleHour()));
							fgfa.setFromCondsEndg(GfaRules.replacePlusWithCycle(w
									.getFromCondsEndg(), e.getGfaCycleHour()));
							fgfa.setCondsContg(GfaRules.replacePlusWithCycle(w.getCondsContg(), e
									.getGfaCycleHour()));
							fgfa.setOtlkCondsDvlpg(GfaRules.replacePlusWithCycle(w
									.getOtlkCondsDvlpg(), e.getGfaCycleHour()));
						...
						gov.noaa.nws.ncep.ui.pgen.file.Sigmet sigmet = 
							new gov.noaa.nws.ncep.ui.pgen.file.Sigmet();
						...
						sigmet.setEditableAttrStartTime(		((Sigmet) de).getEditableAttrStartTime());
						...
						sigmet.setEditableAttrPhenomName(		((Sigmet) de).getEditableAttrPhenomName());
						sigmet.setEditableAttrPhenomLat(		((Sigmet) de).getEditableAttrPhenomLat());
						sigmet.setEditableAttrPhenomLon(		((Sigmet) de).getEditableAttrPhenomLon());
						sigmet.setEditableAttrPhenomPressure(	((Sigmet) de).getEditableAttrPhenomPressure());
						sigmet.setEditableAttrPhenomMaxWind(	((Sigmet) de).getEditableAttrPhenomMaxWind());
						...
						sigmet.setEditableAttrPhenomSpeed(		((Sigmet) de).getEditableAttrPhenomSpeed());
						sigmet.setEditableAttrPhenomDirection(	((Sigmet) de).getEditableAttrPhenomDirection());
						sigmet.setEditableAttrLevel(			((Sigmet) de).getEditableAttrLevel());
						sigmet.setEditableAttrLevelInfo1(		((Sigmet) de).getEditableAttrLevelInfo1());
						sigmet.setEditableAttrLevelInfo2(		((Sigmet) de).getEditableAttrLevelInfo2());
						sigmet.setEditableAttrLevelText1(		((Sigmet) de).getEditableAttrLevelText1());
						sigmet.setEditableAttrLevelText2(		((Sigmet) de).getEditableAttrLevelText2());
						...
						gov.noaa.nws.ncep.ui.pgen.file.Line line = 
							new gov.noaa.nws.ncep.ui.pgen.file.Line();
					...
					gov.noaa.nws.ncep.ui.pgen.file.Symbol symbol = 
						new gov.noaa.nws.ncep.ui.pgen.file.Symbol();
					...
					gov.noaa.nws.ncep.ui.pgen.file.AvnText atext = 
						new gov.noaa.nws.ncep.ui.pgen.file.AvnText();	
					...
					gov.noaa.nws.ncep.ui.pgen.file.MidCloudText mtext = 
						new gov.noaa.nws.ncep.ui.pgen.file.MidCloudText();	
					...
					gov.noaa.nws.ncep.ui.pgen.file.Text text = 
						new gov.noaa.nws.ncep.ui.pgen.file.Text();	
					...
					gov.noaa.nws.ncep.ui.pgen.file.Vector vector = 
						new gov.noaa.nws.ncep.ui.pgen.file.Vector();
					...
					gov.noaa.nws.ncep.ui.pgen.file.TCA tca = 
						new gov.noaa.nws.ncep.ui.pgen.file.TCA();
			...
			//		tca.setAdvisoryTime( tcaEl.getAdvisoryTime() );
					...
					try {
						xmlCal = DatatypeFactory.newInstance().newXMLGregorianCalendar(
								...
								advTime.get(Calendar.MILLISECOND),
								0
								);
	...
	/**
	 * Convert a DECollection to a format that can be used by jaxb and saved to XML files
	 * @param dec
	 * @return
	 */
	private static gov.noaa.nws.ncep.ui.pgen.file.DECollection convertDECollection2XML ( DECollection dec ) {
		
		gov.noaa.nws.ncep.ui.pgen.file.DECollection fdec = 
            new gov.noaa.nws.ncep.ui.pgen.file.DECollection();
	...
	private static Jet convertXML2Jet(gov.noaa.nws.ncep.ui.pgen.file.DECollection dec){
		...
		gov.noaa.nws.ncep.ui.pgen.file.DrawableElement elem = dec.getDrawableElement();
		...
		if (elem.getDECollection() != null ){
			for ( gov.noaa.nws.ncep.ui.pgen.file.DECollection fdec : elem.getDECollection() ) {
				if ( fdec.getCollectionName().equalsIgnoreCase("WindInfo")){
					...
					gov.noaa.nws.ncep.ui.pgen.file.DrawableElement de = fdec.getDrawableElement();
	 ...
	 * @return
	 */
	private static JetLine convertJetLine( gov.noaa.nws.ncep.ui.pgen.file.Line jetLine, Jet jet ){
		...
		JetLine newLine = jet.new JetLine( null, clr,
				jetLine.getLineWidth(), jetLine.getSizeScale(), jetLine.isClosed(), 
				jetLine.isFilled(), linePoints, 
				jetLine.getSmoothFactor(), 
	 ...
	 * @return
	 */
	private static JetHash convertJetHash(gov.noaa.nws.ncep.ui.pgen.file.Vector fVector, Jet jet){
		Color[] clr = new Color[ fVector.getColor().size() ];
		int nn = 0;
		for (gov.noaa.nws.ncep.ui.pgen.file.Color fColor : fVector.getColor() ) {
			clr[nn++] = new Color( fColor.getRed(), fColor.getGreen(),
		...
		JetHash hash = jet.new JetHash( null, clr,
				fVector.getLineWidth(), fVector.getSizeScale(), 
				fVector.isClear(), (new Coordinate( loc.getLon(), loc.getLat() ) ),
				vtype, fVector.getSpeed(), fVector.getDirection(),
				fVector.getArrowHeadSize(), fVector.isDirectionOnly(),
	 ...
	 * @return
	 */
	private static JetBarb convertJetBarb(gov.noaa.nws.ncep.ui.pgen.file.Vector fVector, Jet jet){
		Color[] clr = new Color[ fVector.getColor().size() ];
		int nn = 0;
		for (gov.noaa.nws.ncep.ui.pgen.file.Color fColor : fVector.getColor() ) {
			clr[nn++] = new Color( fColor.getRed(), fColor.getGreen(),
		...
		JetBarb barb = jet.new JetBarb( null, clr,
				fVector.getLineWidth(), fVector.getSizeScale(), 
				fVector.isClear(), (new Coordinate( loc.getLon(), loc.getLat() ) ),
				vtype, fVector.getSpeed(), fVector.getDirection(),
				fVector.getArrowHeadSize(), fVector.isDirectionOnly(),
	 ...
	 * @return
	 */
	private static JetText convertJetFL(gov.noaa.nws.ncep.ui.pgen.file.Text fText, Jet aJet){
		Color[] clr = new Color[ fText.getColor().size() ];
		int nn = 0;
		for ( gov.noaa.nws.ncep.ui.pgen.file.Color fColor : fText.getColor() ) {
			clr[nn++] = new Color( fColor.getRed(), fColor.getGreen(),
		...
		JetText text = aJet.new JetText( (Coordinate[])null, fText.getFontName(), 
				fText.getFontSize(), 
				TextJustification.valueOf( fText.getJustification() ),
				(new Coordinate( loc.getLon(), loc.getLat() ) ),    		    		              
				fText.getRotation(), 
	 ...
	 * @return
	 */
    private static gov.noaa.nws.ncep.ui.pgen.file.Contours convertContours2XML( Contours cnt ) {

	    gov.noaa.nws.ncep.ui.pgen.file.Contours contours = 
		                      new gov.noaa.nws.ncep.ui.pgen.file.Contours();
	    ...
	    try {
	        xmlCal = DatatypeFactory.newInstance().newXMLGregorianCalendar(
			    ...
			    cntTime.get(Calendar.MILLISECOND),
			    0 );
	    } catch (DatatypeConfigurationException e) {
	        e.printStackTrace();
	    }
	    ...
	    try {
	        xmlCal2 = DatatypeFactory.newInstance().newXMLGregorianCalendar(
			    ...
			    cntTime2.get(Calendar.MILLISECOND),
			    0 );
	    } catch (DatatypeConfigurationException e) {
	        e.printStackTrace();
	    }
	 ...
	 * @return
	 */
    private static Contours convertXML2Contours( gov.noaa.nws.ncep.ui.pgen.file.Contours cnt ) {
    	...
    	if (xmlCal != null)  
    	cntTime.set( xmlCal.getYear(), xmlCal.getMonth()-1, xmlCal.getDay(), 
    				 xmlCal.getHour(), xmlCal.getMinute(), xmlCal.getSecond() );
    	...
    	if (xmlCal2 != null) 
    	cntTime2.set( xmlCal2.getYear(), xmlCal2.getMonth()-1, xmlCal2.getDay(), 
    				 xmlCal2.getHour(), xmlCal2.getMinute(), xmlCal2.getSecond() );
    	contours.setTime2( cntTime2 );
    	
    	for ( gov.noaa.nws.ncep.ui.pgen.file.DECollection fdec : cnt.getDECollection() ) {
			
    		if ( fdec.getCollectionName().equals( "ContourLine" ) ) {
    		    ContourLine contourLine = new ContourLine();
    		
    		    List<AbstractDrawableComponent> delist = convert( fdec.getDrawableElement() );
     ...
     * @return
     */
    private static gov.noaa.nws.ncep.ui.pgen.file.WatchBox convertWatchBox2XML( WatchBox wb ){
	    gov.noaa.nws.ncep.ui.pgen.file.WatchBox fwb  = 
	    	new  gov.noaa.nws.ncep.ui.pgen.file.WatchBox();
	    	...
	    	if ( issueTime != null ){
	    		...
	    		try {
	    			xmlIssueCal = DatatypeFactory.newInstance().newXMLGregorianCalendar(
	    					...
	    					issueTime.get(Calendar.MILLISECOND),
	    					0
	    			);
	    		} catch (DatatypeConfigurationException e) {
	    			e.printStackTrace();
			...
			if ( expTime != null ){
				...
				try {
					xmlExpCal = DatatypeFactory.newInstance().newXMLGregorianCalendar(
							...
							expTime.get(Calendar.MILLISECOND),
							0
					);
				} catch (DatatypeConfigurationException e) {
					e.printStackTrace();
	    	...
	    	if ( fColor != null ){
	    		gov.noaa.nws.ncep.ui.pgen.file.Color fill = 
				new gov.noaa.nws.ncep.ui.pgen.file.Color();
	    	...
	    	for ( Station stn : wb.getAnchors()){
	    		fwb.getAnchorPoints().add(stn.getStid() + " " + stn.getState() + " " + stn.getStnname());
	    	}
	    			...
	    			if ( statusValidTime != null ){
	    				...
	    				try {
	    					xmlCal = DatatypeFactory.newInstance().newXMLGregorianCalendar(
	    							...
	    							statusValidTime.get(Calendar.DAY_OF_MONTH),
	    							statusValidTime.get(Calendar.HOUR_OF_DAY),
	    							statusValidTime.get(Calendar.MINUTE),
	    							statusValidTime.get(Calendar.SECOND),
	    							statusValidTime.get(Calendar.MILLISECOND),
	    							0
	    					);
	    				} catch (DatatypeConfigurationException e) {
	    			...
	    			if ( statusExpTime != null ){
	    				...
	    				try {
	    					xmlCal = DatatypeFactory.newInstance().newXMLGregorianCalendar(
	    							...
	    							statusExpTime.get(Calendar.DAY_OF_MONTH),
     ...
     * @return
     */
    private static WatchBox convertXML2WatchBox( gov.noaa.nws.ncep.ui.pgen.file.WatchBox fwb ){
			...
			if ( xmlIssueCal != null ){
				issueTime.set(xmlIssueCal.getYear(), xmlIssueCal.getMonth()-1, xmlIssueCal.getDay(), 
					    xmlIssueCal.getHour(), xmlIssueCal.getMinute(), xmlIssueCal.getSecond());
			...
			if ( xmlExpCal != null ){
				expTime.set(xmlExpCal.getYear(), xmlExpCal.getMonth()-1, xmlExpCal.getDay(), 
					    xmlExpCal.getHour(), xmlExpCal.getMinute(), xmlExpCal.getSecond());
		    ...
		    if ( fwb.getFillColor() != null ){
		    	gov.noaa.nws.ncep.ui.pgen.file.Color fColor = fwb.getFillColor().getColor();
		    	if ( fColor != null ){
		     ...
		     */
		    String fileDir = ProductConverter.class.getProtectionDomain().getCodeSource().getLocation().toString(); 
	    	...
	    	for ( String str : fwb.getAnchorPoints()){
	    		...
	    		if ( !str.equalsIgnoreCase("") )
	    			anchors[nn++] = (new StationTable(fileDir+"table/spcwatch.xml")).getStation(StationField.STID, str.substring(0,3));    	
	    	}
	    	...
	    	for ( String str : fwb.getCounties()){
	    		...
	    		county[n] = (new StationTable(fileDir + "table/mzcntys.xml")).getStation(StationField.STNM, str);
	    		if (county[n] != null) {
	    			//convert Staton to County
	    			SPCCounty wbCounty = new SPCCounty(
	        			county[n].getStnnum(), //getFips(),
	        			county[n].getStnname(),
	        			county[n].getWfo(),
	        			"",  //county[n].getStid(), //ugcId,
	        			county[n].getState(),
	        			county[n].getCountry(),
	        			"", 
	        			new Coordinate(county[n].getLongitude(), county[n].getLatitude()),
	        			null, false
	        			/*county[n].getzoneName(),
	        			county[n].centroid(),
	        			county[n].shape,
	        			county[n].marineZone*/);
...
//	    						if ( !cnty.getFips().isEmpty() && str.equals(cnty.getFips())){
//		    						wb.addCounty(cnty);
//		    						it.remove();
//		    						break;
//		    					}
//	    					}
//	    					else {
//	    						if ( !cnty.getUgcId().isEmpty() && str.contains(cnty.getUgcId())){
//		    						wb.addCounty(cnty);
//		    						it.remove();
//		    						break;
//		    					}
...
//	    	}
	    	...
	    	for ( Status fws : fwb.getStatus() ){
	    		...
	    		Calendar statusValidTime = Calendar.getInstance( TimeZone.getTimeZone("GMT") );
	    		XMLGregorianCalendar xmlStatusValidCal = fws.getStatusValidTime();
	    		if ( xmlStatusValidCal != null ){
	    			statusValidTime.set(xmlStatusValidCal.getYear(), xmlStatusValidCal.getMonth()-1, xmlStatusValidCal.getDay(), 
	    					xmlStatusValidCal.getHour(), xmlStatusValidCal.getMinute(), xmlStatusValidCal.getSecond());
	    		}

	    		Calendar statusExpTime = Calendar.getInstance( TimeZone.getTimeZone("GMT") );
	    		XMLGregorianCalendar xmlStatusExpCal = fws.getStatusExpTime();
	    		if ( xmlStatusExpCal != null ){
	    			statusExpTime.set(xmlStatusExpCal.getYear(), xmlStatusExpCal.getMonth()-1, xmlStatusExpCal.getDay(), 
	    					xmlStatusExpCal.getHour(), xmlStatusExpCal.getMinute(), xmlStatusExpCal.getSecond());
	    		}
	 ...
	 * @return
	 */
    private static gov.noaa.nws.ncep.ui.pgen.file.Outlook convertOutlook2XML( Outlook otlk ) {

	    gov.noaa.nws.ncep.ui.pgen.file.Outlook fotlk = 
		                      new gov.noaa.nws.ncep.ui.pgen.file.Outlook();
	 ...
	 * @return
	 */
    private static Outlook convertXML2Outlook( gov.noaa.nws.ncep.ui.pgen.file.Outlook fotlk ) {
    	...
    	for ( gov.noaa.nws.ncep.ui.pgen.file.DECollection fdec : fotlk.getDECollection() ) {
    		//for non-grouped labeled lines
			if ( fdec.getCollectionName().equalsIgnoreCase(Outlook.OUTLOOK_LABELED_LINE)){
				DECollection dec = new DECollection(Outlook.OUTLOOK_LABELED_LINE);
				List<AbstractDrawableComponent> delist = convert( fdec.getDrawableElement() );
				for ( AbstractDrawableComponent de : delist ) {
	    			de.setParent( dec );
	    			dec.add(de);
			...
			else if ( fdec.getCollectionName().equalsIgnoreCase( Outlook.OUTLOOK_LINE_GROUP ) ) {
				DECollection grp = new DECollection( Outlook.OUTLOOK_LINE_GROUP );
				//add all labeled lines
				for (gov.noaa.nws.ncep.ui.pgen.file.DECollection labeledLine : fdec.getDrawableElement().getDECollection()){
					DECollection lblLine = new DECollection(Outlook.OUTLOOK_LABELED_LINE);
					List<AbstractDrawableComponent> des = convert( labeledLine.getDrawableElement() );
    ...
    public static Volcano convertXML2Volcano(gov.noaa.nws.ncep.ui.pgen.file.Volcano fVol){
...
//	    boolean isNoneDrawable = Arrays.asList(VaaInfo.ProductInfo.getProduct(VaaInfo.LOCS[1])).contains( fvp );	    
	    ...
	    if( ! isNoneDrawable )	    vol.setPoints(volPoints);
    ...
    public static gov.noaa.nws.ncep.ui.pgen.file.Volcano convertVolcano2XML(Volcano vol){
    	gov.noaa.nws.ncep.ui.pgen.file.Volcano fVol = 
    		new gov.noaa.nws.ncep.ui.pgen.file.Volcano();
    	String vp = vol.getProduct() == null ? null : vol.getProduct().trim();
    	boolean isNoneDrawable = Arrays.asList(VaaInfo.ProductInfo.getProduct(VaaInfo.LOCS[1])).contains( vp );
    	...
    	if ( ll != null ) {
    			...
    			for (gov.noaa.nws.ncep.ui.pgen.file.DECollection lblDec : elem
    				...
    				if (lblDec.getCollectionName().equalsIgnoreCase("Label")) {
    					...
    					if (lblDe.getMidCloudText() != null) {
    						...
    						for (gov.noaa.nws.ncep.ui.pgen.file.MidCloudText aText : lblDe.getMidCloudText() ) {
    							...
    							MidCloudText text = new MidCloudText((Coordinate[]) null, aText.getFontName(), 
    														aText.getFontSize(),
    														TextJustification.valueOf(aText.getJustification()), 
    														new Coordinate(loc.getLon(), loc.getLat()),
    														aText.getCloudTypes(), aText.getCloudAmounts(), 
    														aText.getTurbulenceType(), aText.getTurbulenceLevels(),
    														aText.getIcingType(), aText.getIcingLevels(),
    														aText.getTstormTypes(), aText.getTstormLevels(), 
    														FontStyle.valueOf(aText.getStyle()), clr[0], aText.getPgenCategory(),
    														aText.getPgenType());
    					...
    					if (lblDe.getAvnText() != null) {
    						...
    						for (gov.noaa.nws.ncep.ui.pgen.file.AvnText aText : lblDe.getAvnText() ) {
    		...
    		if (elem.getLine() != null) {
    			...
    			for (gov.noaa.nws.ncep.ui.pgen.file.Line fLine : elem.getLine()) {
    				...
    				Line line = new Line(null, clr, fLine.getLineWidth(), fLine
    						.getSizeScale(), fLine.isClosed(), fLine.isFilled(),
    						linePoints, fLine.getSmoothFactor(), FillPattern
    						.valueOf(fLine.getFillPattern()), fLine
    						.getPgenCategory(), fLine.getPgenType());
    ...
    private static gov.noaa.nws.ncep.ui.pgen.sigmet.Ccfp convertXML2Ccfp(gov.noaa.nws.ncep.ui.pgen.file.DECollection dec){
	    ...
	    gov.noaa.nws.ncep.ui.pgen.sigmet.Ccfp ccfp = new gov.noaa.nws.ncep.ui.pgen.sigmet.Ccfp(dec.getCollectionName());
	    ccfp.setSigmet(sig);
    	return (gov.noaa.nws.ncep.ui.pgen.sigmet.Ccfp)convertXML2LabeledLine(dec, ccfp);
    }
    ...
    private static void handleCcfpText(gov.noaa.nws.ncep.ui.pgen.file.DrawableElement lblDe, Label lbl){
    	...
    	for (gov.noaa.nws.ncep.ui.pgen.file.Text aText : lblDe.getText() ) {
    		...
    		Text text = new Text(null, aText.getFontName(), aText.getFontSize(),
    				TextJustification.valueOf(aText.getJustification()), new Coordinate(loc.getLon(), loc.getLat()),
    				0.0, TextRotation.SCREEN_RELATIVE, aText.getTextLine().toArray(new String[]{}),
    				FontStyle.valueOf(aText.getStyle()), clr[0], 0, 0,
    				true, DisplayType.BOX, aText.getPgenCategory(), aText.getPgenType() );
