...
public class PRLibrary {
    ...
    /***
     * Computes TPWN, the numeric weather code for temporary/
     * probability/vicinity weather, with probability 30 weather taking
     * precedence over vicinity weather and vicinity weather taking precedence
     * over temporary weather.
     * 
     * @param twnm
     *            - Temporary weather code
     * @param vwnm
     *            - Vicinity weather code
     * @param pprb
     *            - Probability for forecast change indicator
    ...
    /***
     * Computes prAllWeatherNumericCode, the numeric weather code for prevailing
     * temporary/ probability/vicinity weather, with probability 30 weather
     * taking precedence over vicinity weather and vicinity weather taking
     * precedence over temporary weather and temporary weather taking precedence
     * over prevailing weather.
     * 
     * @param wnum
     *            - Prevailing numeric weather code
     * @param twnm
     *            - Temporary weather code
     * @param vwnm
     *            - Vicinity weather code
    ...
    public static float prAwnm(float wnum, float twnm, float vwnm, float pprb) {
        ...
        return (MissingValueTester.isDataValueMissing(prAllWeatherNumericCode) ? wnum
                : prAllWeatherNumericCode);
    }

    /***
     * Computes a standard abbreviated 3-digit display of pressure containing
     * the tens and units digits and the first digit after the decimal point.
     * The input is multiplied by 10, truncated, and the original thousand and
     * hundred digits are dropped
     * 
     * @param pmsl
     *            - the pressure in mb
    ...
    /***
     * Computes the WMO fractional cloud cover from the input numeric total
     * cloud cover
     * 
     * @param cfrt
     *            - Numeric total cloud cover
     * @return RMISSD if the numeric total cloud cover is equal to RMISSD and
     *         the WMO fractional cloud cover otherwise
     */
     ...
     * 
     * Coverage values are ordered as follows: 0 no clouds 1 clear 6 thin
     * scattered 2 scattered 7 thin broken 3 broken 8 thin overcast 4 overcast9
     * 9 partially obscured 5 obscured
     * 
     * @param clcl
    ...
    public static float prClct(float clcl, float clcm, float clch) {
        /* Sanity check */
        if (clcl == GempakConstants.RMISSD || clcm == GempakConstants.RMISSD
    ...
    /***
     * Computes the numeric cloud coverage from the combined cloud height and
     * coverage
     * 
     * @param comx
     *            - the combined cloud height (either at the low/medium/high
     *            cloud level expressed in hundreds of feet) and coverage.
    ...
    /***
     * This method computes the lowest ceiling from ceil, chc1, chc2 and chc3.
     * if there is no ceiling, get the lowest layer from chc1, chc2 and chc3.
     * 
     * @param ceil
    ...
    public static float prClhx(float comx) {
        ...
        // return ( comx > 10000 ? ( ( (int) (comx) - 10000) /10 ) : (int)
        // comx);
        return (comx > 10000 ? (((int) (comx) - 10000) / 10)
                : (int) (comx / 10.));
    }
     ...
     * 
     * @param clcx
     *            - the numeric cloud coverage for clouds ( one of
     *            Low/Mid/High/Total cloud coverage)
    ...
    public static float prCloa(float clcx) {
        ...
        return ((cloud != null && cloud.length > 0 && cloud.length >= icld) ? cloud[icld]
                : GempakConstants.RMISSD);
    }

    /***
     * Computes the combined low, mid and high cloud coverage The following
     * equation is used: cmbc = clcl*100 + clcm*10 + clch
     * 
     * @param clcl
     *            - low cloud coverage
     * @param clcm
     ...
     * @param chc3
     *            - Cloud height & coverage 3
     * @return the computed combined height and numeric sky coverage for high
     *         clouds
     */
    public static float prComh(float chc1, float chc2, float chc3) {
        ...
        int icovx = -1;
        /*
         * (Non-Javadoc) Check for thin obscured combined with cloud height and
        ...
        if (ihght > 178) {
            prcomh = chc1 % 10000;
            icovr = (int) prCtcf((ichc1 % 10));
        }

        /* Check 2nd report */
        if (!MissingValueTester.isDataValueMissing(chc2)) {
            ihght = (int) (chc2 / 10);
            icovx = (int) prCtcf((chc2 - (ihght * 10)));
        ...
        if (!MissingValueTester.isDataValueMissing(chc3)) {
            ihght = (int) (chc3 / 10);
            icovx = (int) prCtcf((chc3 - (ihght * 10)));
            if (ihght > 178 && icovx > icovr) {
                prcomh = chc3;
            }
        }
    ...
    /***
     * Computes the combined height and numeric sky coverage for clouds, whose
     * height is lesser than 63000 feet
     * 
     * @param chc1
     *            - Cloud height & coverage 1
     * @param chc2
    ...
    /***
     * Computes the combined height and numeric sky coverage for mid-level
     * clouds, whose height is greater than 63000 feet but lesser than 179,000
     * feet.
     * 
     * @param chc1
     *            - Cloud height & coverage 1
    ...
    public static float prComm(float chc1, float chc2, float chc3) {
        ...
        if (!MissingValueTester.isDataValueMissing(chc1)) {
            ...
            /*
             * Check for thin obscured combined with cloud height and coverage
             * and get height code
             */
            ichc1 = (chc1 >= 10000 ? ((int) chc1 - 10000) : (int) chc1);
            ihght = ichc1 / 10;
    ...
    /***
     * Computes the highest cloud height and coverage from the low, mid and high
    ...
    /***
     * Computes the combined cloud height and coverage from the cloud height
     * (clhx) and the cloud coverage (clcx)
     * 
     * @param clhx
     *            - Cloud height
     * @param clcx
    ...
    public static float prComx(float clhx, float clcx) {
        ...
        if (clcx == 1)
            prcomx = clcx;
        /*
         * Check for obscured clouds with missing height; return cloud -
         * coverage only.
         */
        else if (((clcx == 5) || (clcx == 9))
                && MissingValueTester.isDataValueMissing(clhx)) {
            prcomx = clcx;
        }
    ...
    /***
     * Computes the cloud type symbol number from the synoptic code cloud type
     * number
     * 
     * @param ctyh
     *            - Synoptic code for high cloud
     * @return The cloud type symbol number after adding 20 to ctyh if ctyh lies
     *         between 1 and 9 and 0.0 otherwise
     */
     ...
     * 
     * @param ctyl
     *            - Synoptic code for low cloud
     * @return The cloud type symbol number as ctyl if ctyl lies between 1 and 9
     *         and 0.0 otherwise
     */
     ...
     * 
     * @param ctym
     *            - Synoptic code for mid cloud
     * @return The cloud type symbol number after adding 10 to ctym if ctym lies
     *         between 1 and 9 and 0.0 otherwise
     */
    ...
    /***
     * Computes the cloud type symbol number for the first level at which clouds
     * are reported
     * 
     * @param ctyl
     *            - Synoptic code for low cloud
    ...
    /***
     * Computes the maximum cloud cover from the combined height/coverage for
     * low, mid and high clouds
     * 
     * @param chc1
     *            - Combined height and coverage for low clouds
     * @param chc2
     *            - Combined height and coverage for mid clouds
     * @param chc3
     *            - Combined height and coverage for high clouds
     * @return the maximum cloud cover
     */
     ...
     * 
     * @param clct
     *            - is the numberic total cloud cover
     ...
     * @param hvalue
     *            the value to divide
     * @return the input value divided by 100, if hvalue is not missing and
     *         RMISSD (-9999.0) otherwise
     */
    public static float prD100(float hvalue) {
        return (!MissingValueTester.isDataValueMissing(hvalue) ? hvalue / 100
                : GempakConstants.RMISSD);
    }
     ...
     *         both tmpx and dwpx are valid values and RMISSD ( - 9999.0)
     *         otherwise.
     */
    public static float prDdep(float tmpx, float dwpx) {
        return ((!MissingValueTester.isDataValueMissing(tmpx)
                && !MissingValueTester.isDataValueMissing(dwpx) && dwpx <= tmpx) ? (tmpx - dwpx)
                : GempakConstants.RMISSD);
    }
    ...
    public static float prTmcf(float tmpc) {
        return (!MissingValueTester.isDataValueMissing(tmpc) ? (tmpc * GempakConstants.RPRM) + 32
                : GempakConstants.RMISSD);
    }
    ...
    public static float prTmfc(float tmpf) {
        return (!MissingValueTester.isDataValueMissing(tmpf) ? (tmpf - 32)
                / GempakConstants.RPRM : GempakConstants.RMISSD);
    }
     ...
     * @return the equivalent temperature in Kelvin if tmpf is valid or RMISSD (
     *         -9999.0) otherwise
     */
     ...
     * @return the equivalent temperature in Farenheit if tmpk is valid or
     *         RMISSD ( -9999.0) otherwise
     */
    public static float prTmkf(float tmpk) {
        return (!MissingValueTester.isDataValueMissing(tmpk) ? (prTmkc(tmpk) * GempakConstants.RPRM) + 32
                : GempakConstants.RMISSD);
    }
     ...
     * @param tmpc
     *            - temperature in Celsius
     * @return the equivalent temperature in Kelvin if tmpc is valid and RMISSD
     *         (-9999.0) otherwise
     */
    public static float prTmck(float tmpc) {
        return (!MissingValueTester.isDataValueMissing(tmpc) ? (tmpc + GempakConstants.TMCK)
                : GempakConstants.RMISSD);
    }
     ...
     * @return the equivalent temperature in Celsius if tmpk is valid and RMISSD
     *         (-9999.0) otherwise
     */

    public static float prTmkc(float tmpk) {
        return (!MissingValueTester.isDataValueMissing(tmpk) ? (tmpk - GempakConstants.TMCK)
                : GempakConstants.RMISSD);
    }
     ...
     *         input values, if they exist and RMISSD (-9999.0) otherwise.
     */
     ...
     *            temperature)
     * @return the dewpoint in the same units as tmpx and dpdx if both input
     *         parameters are valid and RMISSD ( - 9999.0) otherwise
     */
    public static float prDwdp(float tmpx, float dpdx) {
        return ((!MissingValueTester.isDataValueMissing(tmpx) && !MissingValueTester
                .isDataValueMissing(dpdx)) ? tmpx - dpdx
                : GempakConstants.RMISSD);
    }
     ...
     * @param pres
     *            - the pressure (in mb)
     * @return the dewpoint (in Celsius), if both the input values are valid and
     *         RMISSD (-9999.0) otherwise
     */
    public static float prDwpt(float rmix, float pres) {
        ...
        if (!MissingValueTester.isDataValueMissing(rmix)
                && !MissingValueTester.isDataValueMissing(pres) && (rmix > 0)
                && (pres > 0)) {
            ...
            vaporPressure = vaporPressure
                    / (1.001f + ((pres - 100.0f) / 900.0f) * .0034f);

            /* Calculate dewpoint */
            prDwpt = (float) (Math.log(vaporPressure / 6.112) * 243.5 / (17.67 - Math
    ...
    /**
     * Computes the equivalent speed in knots from the input speed in
     * meters/second using the equation: SKNT = SPED * 1.9425
     * 
     * @param sped
     *            - speed in meters/second
     * @return the speed in knots if sped is not missing and RMISSD (-9999.0)
     *         otherwise.
     */
    public static float prMskn(float sped) {
        return (!MissingValueTester.isDataValueMissing(sped) ? (sped * GempakConstants.METERS_PER_SEC_TO_KNOTS)
                : GempakConstants.RMISSD);
    }

    /**
     * Computes the equivalent speed in meters/second from the input speed in
     * knots using the equation: SPED = SKNT / 1.9425
     * 
     * @param sknt
     *            - speed in knots
     * @return the speed in knots if sknt is not missing and RMISSD (-9999.0)
     *         otherwise.
     */
    public static float prKnms(float sknt) {
        return (!MissingValueTester.isDataValueMissing(sknt) ? (sknt * GempakConstants.KNOTS_TO_METERS_PER_SEC)
                : GempakConstants.RMISSD);
    }
     ...
     * @param sknt
     *            - speed in knots
     * @return the speed in miles/hour if sknt is not missing and RMISSD
     *         (-9999.0) otherwise.
     */
    public static float prKnmh(float sknt) {
        return (!MissingValueTester.isDataValueMissing(sknt) ? (sknt * GempakConstants.KNOTS_TO_MILES_PER_HOUR)
                : GempakConstants.RMISSD);
    }
     ...
     * @param smph
     *            - speed in miles/hour
     * @return the speed in knots if smph is not missing and RMISSD (-9999.0)
     *         otherwise.
     */
    public static float prMhkn(float smph) {
        return (!MissingValueTester.isDataValueMissing(smph) ? (smph * GempakConstants.MILES_PER_HOUR_TO_KNOTS)
                : GempakConstants.RMISSD);
    }

    /**
     * Determines the height in meters which corresponds to a given code figure
     * from WMO Code Table 1677. If the given code figure is found to be
     * invalid, then RMISSD is returned.
     * 
     * @param hhString
     *            - the code figure from WMO table 1677
    ...
    public static float prHcdm(String hhString) {
        ...
        try {
            ...
            if ((ihh >= 0) && (ihh <= 50))
                prhcdm = ihh * 30f;
            else if ((ihh >= 56) && (ihh <= 80)) // what about numbers from
                                                 // 51-56?
                prhcdm = (ihh - 50f) * 300f;
             ...
             * RMISSD, since there is no clear way to set a single output value
             * for the ranges represented by such code figures.
             */
    ...
    public static float prHeat(float tmpf, float relh) {
        ...
        if (!MissingValueTester.isDataValueMissing(tmpf)
                && !MissingValueTester.isDataValueMissing(relh)) {
            /*
             * If the temperature is less than 40 degrees, set the heat index to
             * the temperature
             */
            if (tmpf <= 40.0f)
                prheat = tmpf;
                ...
                /*
                 * Compute a simple heat index. If the value is less than 80 deg
                 * F use it
                 */
                ...
                if (prheat >= 80.0) {
                    float t2 = tmpf * tmpf;
                    float r2 = relh * relh;
                    prheat = (float) (-42.379 + 2.04901523 * tmpf + 10.14333127
                            * relh - 0.22475541 * tmpf * relh - 0.00683783 * t2
                            - 0.05481717 * r2 + 0.00122874 * t2 * relh
                            + 0.00085282 * tmpf * r2 - 0.00000199 * t2 * r2);
                    /*
    ...
    public static float prMhgt(float hb, float pb, float pt, float scale) {
        if (!MissingValueTester.isDataValueMissing(hb)
                ...
                && !MissingValueTester.isDataValueMissing(scale) && (pt > 0)
                && (pb > 0)) {
    ...
    /**
     * Translates a WMO pressure tendency string into the pressure tendency code
     * * 1000 + the magnitude of the change in tenths of millibars.
     * 
     * @param ctend
     *            - the WMO pressure tendency string
    ...
    public static float prP03d(String ctend) {
        if (ctend != null && !ctend.isEmpty()) {
            ...
            /* 888 */
            // if ( cc.charAt( 0 ) > '0' && cc.charAt( 0 ) > '8' && (
            // cc.compareTo( "999" ) != 0 ) ) {
            if (cc.charAt(0) >= '0' && cc.charAt(0) <= '8'
                    && (cc.compareTo("999") != 0)) {
     ...
     * @param selv
     *            - Station elevation in meters
     * @return the pressure in millibars if nont of the input values are missing
    ...
    public static float prPalt(float altm, float selv) {
        ...
        if (!MissingValueTester.isDataValueMissing(altm)
            ...
            float expo = (GempakConstants.GRAVTY
                    / (GempakConstants.GAMUSD * GempakConstants.RDGAS) * 1000.0f);

            /* Calculate pressure */
            prpalt = (float) (altm * Math.pow((1 - (hgtk
                    * GempakConstants.GAMUSD / (GempakConstants.TMCK + 15))),
                    expo));
        }
        return prpalt;
    }
     ...
     *         the input values are valid or RMISSD (-9999.0) otherwise
     */
    public static float prThte(float pres, float tmpc, float dwpc) {
        float prthte = GempakConstants.RMISSD;
        if (!MissingValueTester.isDataValueMissing(pres)
                && !MissingValueTester.isDataValueMissing(tmpc)
                && !MissingValueTester.isDataValueMissing(dwpc) && (pres > 0)) {
            ...
            if (!MissingValueTester.isDataValueMissing(rmix)) {
                ...
                // e = ( float ) ( ( ( 3.376 / tlcl ) - 0.00254 ) * ( rmix * ( 1
                // + 0.81 * 0.001 * rmix ) ) );
                e = ((3.376f / tlcl) - 0.00254f)
                        * (rmix * (1 + 0.81f * 0.001f * rmix));
     ...
     *         input values are valid and RMISSD ( -9999.0) otherwise
     */
    public static float prTlcl(float tmpc, float dwpc) {
        ...
        if (!MissingValueTester.isDataValueMissing(tmpc)
            ...
            prtlcl = (float) ((800 * (dwpk - 56) / (800 + (dwpk - 56)
                    * Math.log(tmpk / dwpk))) + 56);
    ...
    public static float prTmpk(float pres, float thta) {
        ...
        if (!MissingValueTester.isDataValueMissing(pres)
                && !MissingValueTester.isDataValueMissing(thta) && (pres >= 0)) {
            prtmpk = (float) (thta * (Math.pow(pres / 1000f,
                    GempakConstants.RKAPPA)));
        }
    ...
    public static float prPmsl(float pres, float tmpc, float dwpc, float selv) {
        ...
        if (!MissingValueTester.isDataValueMissing(pres)
            ...
            float tVave = tv + (deltaV / 2);
            float mathFormula = (GempakConstants.GRAVTY * selv)
                    / (GempakConstants.RDGAS * tVave);
            /* 8888888888888 */
            System.out.println(" selv=" + selv + " tVave=" + tVave
                    + " mathForm=" + mathFormula);
            /* 888 */
    ...
    /**
     * Computes the parcel pressure from THTE the equivalent potential
     * temperature ( in Kelvin ) and and TMPC the parcel temperature at PRES on
     * a specified moist adiabat (THTE). The computation is an iterative
     * Newton-Raphson technique of the form: x = x(guess) + [ f( x ) - f(
     * x(guess) ) ] / f'( x(guess) )
     * 
     * f' is approximated with finite differences f' = [ f( x(guess) + 1 ) - f(
     * x(guess) ) ] / 1 Convergence is not guaranteed for extreme input values.
     * If the computation does not converge after 100 iterations, the missing
     * data value will be returned.
     * 
     * @param thte
    ...
    public static float prPmst(float thte, float tmpk) {
        ...
        if (!MissingValueTester.isDataValueMissing(thte)
                && !MissingValueTester.isDataValueMissing(tmpk) && (thte > 0)) {
            /* Set convergence and initial guess of pressure */
            float epsi = 0.01f;
            float tmpc = prTmkc(tmpk);
            float pgdn = (float) (1000 * Math.pow((tmpk / thte),
                    GempakConstants.AKAPPA));
            /*
             * (Non-Javadoc) Set a limit of 100 iterations. Compute tedn and
             * teup, the thte's at one degree above the guess temperature
             */
            for (int index = 0; index < 100; index++) {
                ...
                /* Check thte */
                if (MissingValueTester.isDataValueMissing(tedn)
                        || MissingValueTester.isDataValueMissing(teup))
                    break;
                else {
     ...
     * @param thta
     *            - potential temperature ( in Kelvin )
     * @return the station pressure ( in mb ) if both the inputs are valid and
    ...
    public static float prPres(float tmpc, float thta) {
        ...
        if (!MissingValueTester.isDataValueMissing(tmpc)
                ...
                && (tmpc > -GempakConstants.TMCK) && (thta > 0)) {
            float tmpk = prTmck(tmpc);
            prpres = (float) (1000 * Math.pow(tmpk / thta,
                    1 / GempakConstants.RKAPPA));
        }
        return prpres;
    }
    ...
    /**
     * Computes the packed wind speed and direction from DRCT and SPED using the
     * equation: PSPD = JDRCT * 500 + JSPED where JDRCT = NINT ( DRCT / 5 )
     * JSPED = NINT ( SPED )
     * 
     * @param drct
     ...
     * @param sped
     *            - Wind speed
     * @return the packed wind speed and direction if both inputs are valid and
     *         RMISSD ( -9999 ) otherwise. It is in the form DDFFF, where DD is
     *         the wind direction in tens of degrees, and FFF is either the wind
     *         speed or wind speed plus 500, depending on the unit digit of
     *         direction rounded to the nearest 5 degrees.
     * 
     */
     ...
     * @return the pressure tendency symbol code if the input is valid or RMISSD
     *         ( -9999.0 ) otherwise.
     */
    public static float prPtsy(float p03d) {
        // return ( !MissingValueTester.isDataValueMissing(p03d) ? ( ( int ) (
        // p03d / 1000 ) ) * 1000 + 999 : GempakConstants.RMISSD );
        return (!MissingValueTester.isDataValueMissing(p03d) & !(p03d < 0)
                & !(p03d >= 9000) ? ((int) (p03d / 1000)) * 1000 + 999
                : GempakConstants.RMISSD);
    }

    /**
     * Converts the numeric WMO weather code for past weather reported from an
     * automatic station (WMO code table 4531) to the corresponding numeric WMO
     * weather code for past weather reported from a manned station (WMO code
     * table 4561).
     * 
     * @param pwwa
     *            - the auto station past weather code
     * @return the manned station past code if the input is valid and RMISSD (
     *         -9999 ) otherwise.
     */
    ...
    /**
     * Computes the quotient for parameter ratios using the equation: PR_QUOT =
     * X / Y (For computing QPF/Watch Threshold)
     * 
     * @param x
     *            - Numerator
     * @param y
     *            - Denominator
     * @return the quotient if both inputs are valid and the denominator is non
     *         zero. Returns RMISSD ( -9999 ) otherwise
     */
    public static float prQuot(float x, float y) {
        if (!MissingValueTester.isDataValueMissing(x)
                && !MissingValueTester.isDataValueMissing(y) && (y != 0)) {
            return (x / y);
        }
        return GempakConstants.RMISSD;
    }
     ...
     *         value of the vapor pressure computed is > ( 1* e^(-30)), or
     *         RMISSD (-9999) otherwise
     */
    public static float prRhdp(float tmpc, float relh) {
        ...
        if (!MissingValueTester.isDataValueMissing(tmpc)
            ...
            if (!MissingValueTester.isDataValueMissing(vaps)) {
                ...
                // if ( vapr > ( Math.pow(Math.E, -30) ) ){//legacy checks for
                // 1.E-30
                if (vapr >= (Math.pow(Math.E, -30))) {// legacy checks for
                                                      // 1.E-30
                    prrhdp = (float) (243.5 * (Math.log(6.112) - Math.log(vapr)) / (Math
                            .log(vapr) - Math.log(6.112) - 17.67));

                    /*
                     * If the dew-point is less than -190 degrees C, it is
                     * treated as missing data Note: Legacy documents it but
                     * does not implement it. However, in CAVE, it was decided
                     * to implement it.
                     */
    ...
    /**
     * Computes the abbreviated standard altimeter code SALI from the altimeter
     * setting in inches ALTI. SALI is an abbreviated altimeter code in inches
     * which contains the unit digit and the first two digits after the decimal
     * points. ALTI is multiplied by 100 truncated, and the original tens digit
     * dropped. The following equation is used: SALI = NINT ( MOD ( ALTI, 10 ) *
     * 100 )
     * 
     * @param alti
     *            - the altimeter code in inches
     * @return the abbreviated standard altimeter code from the input altimeter
     *         code (if it exists) and RMISSD (-9999) otherwise.
     */
    public static float prSali(float alti) {
        ...
        if (!MissingValueTester.isDataValueMissing(alti)) {
            ...
            /*
             * Include the tenths and hundredths digit: Multiply by 100 and
             * round it off to the nearest integer
             */
            aalt *= 100;
            prsali = Math.round(aalt);
        }
    ...
    /**
     * Computes the standard abreviated altimeter code from the altimeter
     * setting in inches, after converting it to millibars ALTM. Then ALTM is
     * multiplied by 10, truncated, and the original thousand and hundred digits
     * are dropped. The following equation is used: SALT = NINT ( MOD ( ALTM,
     * 100 ) * 10 )
     * 
     * @param alti
     *            - the altimeter setting in inches
     * @return the abbreviated standard altimeter code from the input altimeter
     *         setting (if it exists) and RMISSD (-9999) otherwise.
     */
    ...
    /**
     * Computes the scale height in a layer which can then be used to compute
     * the moist hydrostatic height. The folloiwng equation is used: SCLH = (
     * RDGAS / GRAVTY ) * TAV TAV = average virtual temperature in layer = (
     * TVIRTB + TVIRTT ) / 2 TVIRTB = virtual temperature at bottom TVIRTT =
     ...
     * @return the scale height ( in meters ) if all the input data exist and
     *         RMISSD (-9999) otherwise.
     */
    public static float prSclh(float tb, float tt, float tdb, float tdt,
            float pb, float pt) {
    ...
    /**
     * Combines the sky coverage symbol number with the wind speed and direction
     * 
     * @param skyc
     *            - Sky coverage
     * @param drct
     ...
     * @return The packed speed and direction if none of the input values are
     ...
     * @param vWnd
     *            - V component of velocity
     * @return the computed windspeed if both inputs are valid and RMISSD (
     *         -9999) otherwise
     */
    public static float prSped(float uWnd, float vWnd) {
        ...
        if (!MissingValueTester.isDataValueMissing(uWnd)
            ...
            prsped = (float) (Math
                    .sqrt((Math.pow(uWnd, 2) + Math.pow(vWnd, 2))));
        }
        return prsped;
    }

    /**
     * Computes a standard height used on upper-air charts. For data below 500
     * mb, the standard height is the last three digits of the height. For data
     * at and above 500 mb, the height is the last three digits of the height in
     * decameters.
     * 
     * @param pres
     *            - Pressure in millibars
     * @param hght
     *            - Height in meters
     * @return the abbreviated height, if both inputs are valid and RMISSD (
     *         -9999) otherwise.
     */
    public static float prStdz(float pres, float hght) {
        ...
        if (!MissingValueTester.isDataValueMissing(pres)
            ...
            /*
             * ( Non-Javadoc ) This computation matches the method's
             * description, as provided in the Javadoc. The method's description
             * needs to be interpreted from a meteorologist's point of view as
    ...
    public static float prThta(float tmpc, float pres) {
        ...
        if (!MissingValueTester.isDataValueMissing(tmpc)
            ...
            prthta = (float) (tmpk * Math.pow((1000 / pres),
                    GempakConstants.RKAPPA));
        }
        return prthta;
    }
     ...
     *         both input values are greater than zero. Otherwise, it returns
     *         RMISSD ( -9999 ).
     */
    public static float prZalt(float altm, float pres) {
        float przalt = GempakConstants.RMISSD;
        if (!MissingValueTester.isDataValueMissing(altm)
                && !MissingValueTester.isDataValueMissing(pres) && (altm > 0)
                && (pres > 0)) {
            ...
            float expo = (gamma * GempakConstants.RDGAS)
                    / GempakConstants.GRAVTY;
    ...
    /**
     * Converts the GEMPAK weather code to the WMO weather code, which is used
     * to plot weather symbols.
     * 
     * @param wnum
     *            - GEMPAK weather code
    ...
    public static float prNsym(float wnum) {
        ...
        // float wcod = ptWcod ( wnum ); /*Convert weather number to character
        // weather code*/
        // prnsym = ptWsym ( wcod ); /*Convert weather character code to weather
        // symbol number*/
        return prnsym;
    }
    ...
    public static class RZLL {
         ...
         */
        public void prRzll(float instltdg, float instlndg, float inrange,
                float inazim, float inhght) {
            if (!MissingValueTester.isDataValueMissing(instltdg)
                ...
                if (elev > 0.2618f)
                    ...
                    mathFormula1 = (float) ((1 - (Math.pow(elev, 2) / 2)) - range
                            * elev / radp);
                ...
                // mathFormula2 ) );
                mathFormula2 = (float) ((Math.pow(cx, 2)
                        / (2 * Math.pow(rad, 2)) * Math.tan(stlat)));
                xlat = (stlat + (cy / rad) - mathFormula2);
