...
public class CloudHeightDialog extends Dialog { 
	...
	private Unit<? extends Length>      distWorkingUnits = SI.METER; // NonSI.MILE; // CHECK this
	private Unit<? extends Temperature> tempWorkingUnits = SI.CELSIUS; // from SatResource
	private Unit<? extends Length>      hghtWorkingUnits = SI.METER; // from sounding models and data
    ...
    public void createDialog( Composite parent ) {
        ...
        snd_src_txt = new Text( top_form, SWT.SINGLE | SWT.BORDER | SWT.READ_ONLY );
        snd_src_txt.setText("");
        fd = new FormData( 100, 20 );
        ...
        snd_time_txt = new Text( top_form, SWT.SINGLE | SWT.BORDER | SWT.READ_ONLY );
        ...
        dist_units_combo.addSelectionListener( new SelectionAdapter() {
   			public void widgetSelected(SelectionEvent e) {
   				...
   				distUnitsConverter = distWorkingUnits.getConverterTo( optsDlg.getSndDistUnits() );
   				
   				// convert the value currently displayed
   				// (Or we could convert back to the working units and then call setSoundingDataDistance 
   				convertDisplayedValue( dist_txt, prevUnits, optsDlg.getSndDistUnits() );
   			}
        });
        ...
        temp_units_combo.addSelectionListener( new SelectionAdapter() {
        	public void widgetSelected(SelectionEvent e) {			
   				Unit<? extends Temperature> prevUnits = optsDlg.getTemperatureUnits();
        		...
        		tempUnitsConverter = tempWorkingUnits.getConverterTo( optsDlg.getTemperatureUnits() );

        		// convert the value currently displayed
   				// (Or we could convert back to the working units and then call setSoundingDataDistance 
   				convertDisplayedValue( temp_txt, prevUnits, optsDlg.getTemperatureUnits() );
        	}
        });
        ...
        // if this order changes then need to change code that saves selection to optsDlg
        ...
        hght_units_combo.addSelectionListener( new SelectionAdapter() {
        	public void widgetSelected(SelectionEvent e) {			
        		Unit<? extends Length> prevUnits = optsDlg.getCloudHeightUnits(); 
        		...
        		hghtUnitsConverter = hghtWorkingUnits.getConverterTo( optsDlg.getCloudHeightUnits() );

        		// convert the value currently displayed
   				// (Or we could convert back to the working units and then call setSoundingDataDistance 
   				convertDisplayedValue( hght1_txt, prevUnits, optsDlg.getCloudHeightUnits() );

   				// if there are alternate cloud heights then parse the previous values and 
   				// add them to the alt cloud heights list
   		    	try {
   		    		UnitConverter unitCnvtr = prevUnits.getConverterTo( hghtWorkingUnits );
        ...
        alt_cloud_lvls_list = new List( top_form, SWT.SINGLE | SWT.BORDER | SWT.V_SCROLL );

        fd = new FormData( 100, 44 ); // 44 is minimum to show 2 lines without scrollbars.
    ...
    public boolean convertDisplayedValue( Text txt_wid, Unit<?> oldUnits, Unit<?> newUnits ) {
...
//        getNearestStation( new Coordinate(lon, lat));
    ...
    private void init() {
    	...
    	if( optsDlg == null ) {
    		optsDlg = new CloudHeightOptionsDialog( shell, "Cloud Height Options", this );
    	}
    }
...
//    		dist_txt.setText("");
    ...
    // input values are in Ft and mb. The height will be converted to the units selected
    ...
    public void addAltCloudHeight( double workingHght, double pres ) {
		try {
	    	...
	    	alt_cloud_lvls_list.add( String.format("%-6s / %-6.1f mb", "Error", pres ) );
        	System.out.println(e.getMessage());
    	}
    }
