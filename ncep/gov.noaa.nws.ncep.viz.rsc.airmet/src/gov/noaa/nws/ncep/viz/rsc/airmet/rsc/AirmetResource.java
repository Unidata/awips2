 ...
 * @version 1.0
 */
public class AirmetResource extends
        AbstractNatlCntrsResource<AirmetResourceData, IMapDescriptor> implements
        INatlCntrsResource {
    ...
    private class FrameData extends AbstractFrameData {
        ...
        public boolean updateFrameData(IRscDataObject rscDataObj) {
            ...
            if (existingAirmetDataObject == null
                ...
                // airmetData.issueTime.greaterThan(
                // existingAirmetDataObject.issueTime) ) {

                // if( existingAirmetDataObject != null ) {
                // System.out.println("Updating "+ keyStr +"from "+
                // existingAirmetDataObject.updateNum +" to " +
    ...
    protected IRscDataObject[] processRecord(Object recordObject) {
        ...
        if (!(recordObject instanceof AirmetRecord)) {
            System.out
                    .println("AirmetResource.processRecord: object must be an AirmetRecord???");
        ...
        for (AirmetReport aRep : amRec.getAirmetReport()) {
            ...
            if (aRep.getCancelFlag() == 1) {
                ...
                if (canceledAirmets.get(keyStr) != null) {
                    canceledAirmets
                            .put(keyStr, new DataTime(aRep.getEndTime()));
                }
    ...
    public void paintFrame(AbstractFrameData frameData, IGraphicsTarget target,
        ...
        for (AirmetRscDataObject airmetData : airmetDataSet) {
            ...
            boolean symIsText = false; // if true the symbolType will be
                                       // drawn as a Text string instead of a
                                       // symbol.
            ...
            case TURB:
                ...
                if (!airmetData.isFreezingLevel() && // sanity check ; shouldn't
                                                     // be freezing level
            ...
            case LOW_LVL_WIND_SHEAR:
                ...
                // symbolWidth =
                // airmetResourceData.getLowLevelWindShearSymbolWidth();
            ...
            if (enabled) {
                ...
                if (airmetResourceData.symbolEnable && symbolType != null) {
                    if (symIsText) {
                        ...
                        Rectangle2D symCharSize = target.getStringBounds(
                                symFont, "N");
                        ...
                        currLblCoord.addToY(charPixHeight); // approx symbol
                                                            // size
                        ...
                        Coordinate symCoords = new Coordinate(
                                worldSymbolLoc[0], worldSymbolLoc[1]);
    ...
    private AirmetRscDataObject getAirmetRscDataObject(
        ...
        try {
            if (airmetSection.getHazardType().equals("INSTRUMENT FLIGHT RULES")) {
            ...
            } else if (airmetSection.getHazardType().equals(
                ...
                System.out.println("Unrecognized AIRMET Hazard Type "
            ...
            System.out.println("Error parsing flight level: "
                    + airmetSection.getFlightLevel1() + " or "
                    + airmetSection.getFlightLevel2());
        ...
        if (numPoints > 1) {
            ...
            for (AirmetLocation airmetLocation : airmetSection
                ...
                LatLonPoint point = new LatLonPoint(
                        ...
                        airmetLocation.getLongitude(), LatLonPoint.INDEGREES);
                int index = airmetLocation.getIndex() - 1;
                if (airmetData.points[index] != null) {
                    System.out
                            .println("Error reading Airmet Locations: duplicate Index found??");
                    return null;
                }
    ...
    private IFont getSymbolFontSize(int symSize, IGraphicsTarget grphTarget) {
        ...
        // final float symSizeMapping[] = { 1f, 8f, 14f, 15f, 16f, 17f, 18f,
        // 19f, 20f, 21f, 22f };

        if (symbolFonts[symSize] == null) {
            // float fontSize = symSizeMapping[symSize];
            float fontSize = (symSize == 1 ? 8f : (symSize == 2 ? 15f
                    : symSize + 14f));
        ...
        for (IRscDataObject rscDataObj : newRscDataObjsQueue) {
            ...
            if (canceledAirmets.containsKey(keyStr)) {
                ...
                if (cancelTime != null
                    ...
                    System.out.println("Airmet " + keyStr
                            + " has been canceled.");
                }
            }
        }
