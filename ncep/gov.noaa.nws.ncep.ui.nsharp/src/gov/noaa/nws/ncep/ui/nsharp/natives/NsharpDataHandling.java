...
public class NsharpDataHandling {
	...
	/*************************************************************/
	{
		...
		{
			if((sndLys.get(i).getPressure() == pres) && (sndLys.get(i).getWindDirection() != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA) &&
					(sndLys.get(i).getWindSpeed() != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA))
				return ucomp( sndLys.get(i).getWindDirection(), sndLys.get(i).getWindSpeed() );
			if((sndLys.get(i).getPressure() < pres) && (sndLys.get(i).getWindDirection() != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA))
			{
				above = i; ok=1; break;
			}
		}
		if( ok == 0 ) return NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA;
		...
		if( above == below)
		{
			return ucomp( sndLys.get(above).getWindDirection(), sndLys.get(above).getWindSpeed() );
		}

		/* ----- Now we need to interpolate to get the Wind ----- */
		nm1 = sndLys.get(below).getPressure() - pres;
		ubot = ucomp( sndLys.get(below).getWindDirection(), sndLys.get(below).getWindSpeed() );
		utop = ucomp( sndLys.get(above).getWindDirection(), sndLys.get(above).getWindSpeed());
		return ubot - (nm1 / (sndLys.get(below).getPressure() - sndLys.get(above).getPressure()))
		* (ubot - utop);
	}
	...
	/*************************************************************/
	{
		...
		for (i = 0; i < sndLys.size(); i++)
		{
			if((sndLys.get(i).getPressure() == pres) && (sndLys.get(i).getWindDirection() != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA) &&
					(sndLys.get(i).getWindSpeed() != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA))
				return vcomp( sndLys.get(i).getWindDirection(), sndLys.get(i).getWindSpeed() );
			if((sndLys.get(i).getPressure() < pres) && (sndLys.get(i).getWindDirection() != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA))
			{
				above = i; ok=1; break;
			}
		...
		if( above == below)
		{
			return vcomp( sndLys.get(above).getWindDirection(), sndLys.get(above).getWindSpeed() );
		}

		/* ----- Now we need to interpolate to get the Wind ----- */
		nm1 = sndLys.get(below).getPressure() - pres;
		vbot = vcomp( sndLys.get(below).getWindDirection(), sndLys.get(below).getWindSpeed());
		vtop = vcomp( sndLys.get(above).getWindDirection(), sndLys.get(above).getWindSpeed() );
		return vbot - ((nm1 / (sndLys.get(below).getPressure() - sndLys.get(above).getPressure()))
				* (vbot - vtop));
	}
	...
	/*************************************************************/
		...
		for (i = sndLys.size() - 1; i > -1; i--)
		{
			if((sndLys.get(i).getPressure() >= pres) && (sndLys.get(i).getTemperature() != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA))
			{
				below = i; ok=1; break;
			}
		}
		...
		nm1 = sndLys.get(above).getTemperature()- sndLys.get(below).getTemperature();
		nm2 = Math.log( sndLys.get(below).getPressure() / sndLys.get(above).getPressure() );
	...
	private static float i_dwpt ( float pres, List<NcSoundingLayer> sndLys ){
		...
		for (i = 0; i < sndLys.size(); i++)
		{
			if((sndLys.get(i).getPressure() == pres) && (sndLys.get(i).getDewpoint() != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA) && (sndLys.get(i).getPressure() != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA))
			{
				/* no need to interpolate if we have the level and dewpoint exists */
				return sndLys.get(i).getDewpoint();
			}
			if((sndLys.get(i).getPressure() < pres) && (sndLys.get(i).getDewpoint() != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA))
			{
				above = i; ok=1; break;
			}
		}
	...
	/*************************************************************/
		...
		for (i = 0; i < sndLys.size(); i++)
		{
			...
			if((sndLys.get(i).getGeoHeight() == hght) && (sndLys.get(i).getPressure() != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA)){
				index = i;
		...
		nm2 = sndLys.get(above).getGeoHeight() - sndLys.get(below).getGeoHeight();
		nm3 = Math.log( sndLys.get(above).getPressure() / sndLys.get(below).getPressure() );
		index = below;
		return (float)(sndLys.get(below).getPressure() * Math.exp(( nm1 / nm2) * nm3));
	}
	...
	private static float i_hght ( float pres , List<NcSoundingLayer> sndLys, int index){
		...
		for (i = sndLys.size() - 1; i > -1; i--)
		{
			if((sndLys.get(i).getPressure() >= pres) && (sndLys.get(i).getGeoHeight() != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA))
			{
				below = i; ok=1; break;
			}
		}
		...
		nm1 = sndLys.get(above).getGeoHeight() - sndLys.get(below).getGeoHeight();
		nm2 = Math.log( sndLys.get(below).getPressure() / sndLys.get(above).getPressure() );
		nm4 = Math.log(sndLys.get(below).getPressure() / pres );
		index = below;
        ...
        return new Comparator<NcSoundingLayer>() {
            ...
            public int compare(NcSoundingLayer layerA, NcSoundingLayer layerB) {
                ...
                if (layerA != layerB) {
                    ...
                    retValue = Double.compare(layerB.getGeoHeight(), layerA
                            .getGeoHeight());
                    
                    if(retValue == 0 ){
                    	// the two layers have same height. sort them by pressure
                    	retValue = Double.compare(layerA.getPressure(), layerB
                                .getPressure());
                    
                    	if(retValue == 0 ){
                    	// the two layers have same height. sort them by wind
                    		retValue = Double.compare(layerB.getWindSpeed(), layerA
                    				.getWindSpeed());
                    	}
                    }
                }
 ...
 // compare two layers based on reverse pressure,
    ...
    private static List<NcSoundingLayer> replaceNanSoundingData(List<NcSoundingLayer> soundingLys) {
    ...
    public static List<NcSoundingLayer> removeInvalidSoundingData(List<NcSoundingLayer> soundingLys) {
		...
		for (NcSoundingLayer layer : sndLysLst) {
			...
			else if (layer.getWindSpeed() < 0 ){
				removingItemList.add(i);
			}
			else*/ if (layer.getGeoHeight() < -50){
				removingItemList.add(i);
			}
    ...
    public static List<NcSoundingLayer> updateObsSoundingDataForShow(List<NcSoundingLayer> soundingLys, float stnElv) {
    	List<NcSoundingLayer> sndLysLst = soundingLys;
    	List<Integer> removingItemList = new ArrayList<Integer>();
		int i =0 ;
		...
		for (NcSoundingLayer layer : sndLysLst) {
			//if( (layer.getPressure() < 100 )&&(layer.getPressure() >=0 ))
				//System.out.println("pressure below 100 "+ layer.getPressure() + " height " + layer.getGeoHeight() );
			if (layer.getWindDirection() < 0 )
				layer.setWindDirection(NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA);
			...
			if((layer.getPressure() == NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA )&& (layer.getGeoHeight()== NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA ))
				removingItemList.add(i);
		...
		//sort layers based on pressure, height, wind and list them from highest pressure 
		Collections.sort(sndLysLst,reverseHeightPressureWindComparator());
		

		/*
		 * remove duplicate data with same pressure, or same hieght or pressure less than 100
		 */
		//create duplicate list
		removingItemList.clear();
		for ( i = 0; i < sndLysLst.size()-1; i++) {
			if(((sndLysLst.get(i).getPressure() != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA) &&
				(sndLysLst.get(i).getPressure() == sndLysLst.get(i+1).getPressure()))||
			   ((sndLysLst.get(i).getGeoHeight() != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA) &&
				 (sndLysLst.get(i).getGeoHeight() == sndLysLst.get(i+1).getGeoHeight())) )
			{
		...
		for (SoundingLayer soundingLy : presLst){
			System.out.println(" pressure "+ soundingLy.getPressure() + " height "+ soundingLy.getGeoHeight() );
		}
		System.out.println("height list");
		for (SoundingLayer soundingLy : heightLst){
			System.out.println(" pressure "+ soundingLy.getPressure() + " height "+ soundingLy.getGeoHeight() );
		}
		System.out.println("final list");
		for (NcSoundingLayer soundingLy : finalLst){
			System.out.println(" pressure "+ soundingLy.getPressure() + " height "+ soundingLy.getGeoHeight()
					+" temp "  +soundingLy.getTemperature() + " windS " + soundingLy.getWindSpeed());
		}
		*/
		...
		for (NcSoundingLayer soundingLy : heightLst){
				//round up height
				soundingLy.setGeoHeight((float)Math.rint(soundingLy.getGeoHeight()));
				int index=0;
				...
				if(pressure != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA){
					...
					//System.out.println("interpolated pressure "+ soundingLy.getPressure() + " height "+ soundingLy.getGeoHeight() );
		...
		for ( i = 0; i < finalLst.size()-1; i++) {
			if(finalLst.get(i).getPressure() == finalLst.get(i+1).getPressure())
			{
				removingItemList.add(i+1);
				continue;
			}			
		...
		//Interpolates missing temperature, dewpoint, wind speed and direction using pressure
		...
		for (NcSoundingLayer soundingLy : finalLst){
			...
			//because of i_wndu() and i_wndv() algorithm..we should call i_wspd() first, then i_wdir()
			if (soundingLy.getWindSpeed() == NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA) {
				...
				if(interpolatedValue != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA)
					...
					// can not interpolate its wind speed, marked as invalid data
					removingItemList.add(i); 
					i++;
					continue;
				}
			...
			if (soundingLy.getWindDirection() == NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA) {
				...
				if(interpolatedValue != NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA)
					...
					// can not interpolate its wind direction, marked as invalid data
					removingItemList.add(i); 
					i++;
					continue;
				}
		...
		//remove any invalid data still having missing data or pressure below 100
		Collections.reverse(removingItemList);
		for ( i = 0; i < removingItemList.size(); i++){	
			//System.out.println("removing "+ sndLys.get(dupItemList.get(i).intValue()).getPressure());
			finalLst.remove(removingItemList.get(i).intValue());	
		}		
		
		/* 
		 * Chin's NOTE:
		 * native nsharp extend sounding data to add 50 and 75 mb layers.
		 * It uses 150mb layer's temp, dew, wind dir and wind speed. and interpolate
		 * height only.
		 * see xtnd_sndg() at readdata.c for original c code.
		 * 
		 */
		if(finalLst.size() > 2){
			if( found75 == false) {
				...
				soundingLy.setOmega(NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA);
				//interpolate height for 75mb layer
				above1 = finalLst.size()-1;
				above2 = finalLst.size()-2;
				nm1 = finalLst.get(above1).getGeoHeight() - finalLst.get(above2).getGeoHeight();
				nm2 = Math.log( finalLst.get(above2).getPressure() / finalLst.get(above1).getPressure() );
				nm4 = Math.log( finalLst.get(above2).getPressure() / 75 );
				soundingLy.setGeoHeight((float)(finalLst.get(above2).getGeoHeight() + (( nm4 / nm2) * nm1)));
				if(found50 == true){
		...
		//			" temp "  +soundingLy.getTemperature() + " windS " + soundingLy.getWindSpeed());
		//}
		//System.out.println("final size " + finalLst.size() + " stnElv " + stnElv);
		return finalLst;
		
    }
    //This api is called when user wnat to show raw data
    public static List<NcSoundingLayer> sortObsSoundingDataForShow(List<NcSoundingLayer> soundingLys, float stnElv) {
    	List<NcSoundingLayer> sndLysLst = soundingLys;
    	List<Integer> removingItemList = new ArrayList<Integer>();
		int i =0 ;
		//reset missing data to NSHARP_NATIVE_UNVALID_DATA
		...
		//			" temp "  +soundingLy.getTemperature() + " windS " + soundingLy.getWindSpeed());
		//}
		//System.out.println("final size " + sndLysLst.size() + " stnElv " + stnElv);
		return sndLysLst;
		
    }
    public static List<NcSoundingLayer> organizeSoundingDataForShow(List<NcSoundingLayer> soundingLys, float gndElv) {
		...
		//remove under ground layer(s). Note: There may be more than one under ground layer.
		boolean found75 = false, found50=false;
		for (int i =0; i < soundingLys.size(); i++) {
			NcSoundingLayer layer = soundingLys.get(i);
			//System.out.println("layer "+ (i+1) + " pressure= "+ layer.getPressure()+ " H= " + layer.getGeoHeight()+ " temp= "+
			//		layer.getTemperature() + " dewpt = " + layer.getDewpoint());
			if(layer.getGeoHeight() == NsharpNativeConstants.NSHARP_NATIVE_INVALID_DATA || layer.getGeoHeight() < gndElv){
				removingItemList.add(i);				
			} else if (layer.getPressure() < 100 && layer.getPressure()!= 50f && layer.getPressure()!= 75f) {
				removingItemList.add(i);
			}
		...
		for (int i = 0; i < removingItemList.size(); i++){	
			//System.out.println("removing "+ soundingLys.get((removingItemList.get(i).intValue())).getPressure());
		...
		if(soundingLys.size() > 2){
			if( found75 == false) {
				...
				nm1 = soundingLys.get(above1).getGeoHeight() - soundingLys.get(above2).getGeoHeight();
				nm2 = Math.log( soundingLys.get(above2).getPressure() / soundingLys.get(above1).getPressure() );
				nm4 = Math.log( soundingLys.get(above2).getPressure() / 75 );
				soundingLy.setGeoHeight((float)(soundingLys.get(above2).getGeoHeight() + (( nm4 / nm2) * nm1)));
				if(found50 == true){
