...
public class NsharpEditDataDialog extends Dialog {
	...
	private void createDialogContents( Composite parent) {
		...
		NsharpSkewTResource rsc=null;
		if(NsharpSkewTEditor.getActiveNsharpEditor()!= null)
			rsc = NsharpSkewTEditor.getActiveNsharpEditor().getNsharpSkewTDescriptor().getSkewtResource();	
	...
	private void handleEditing(){
		...
		try{
			...
			if((dp > tp) || winDir >= 360.0 ){
				...
				case NEW_LEVEL:
					skewtRsc.addNewLayer(tp, dp, winSpd, winDir, press);
					break;
				case SELECTED_LEVEL:
					skewtRsc.updateLayer(selIndex,tp, dp, winSpd, winDir, press);
					break;
				default:
					return;
				}	
