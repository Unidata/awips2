...
c   12/15/03 version - original
     
      implicit none

      real  To, Tdo, T, Td, dT, Toff
      real  D, Dav, Dmax, Dmin, Dcont
      real  DavC,davcm,sig,svr,davcb
      real  UPMAXV, UPMAXT, 
     * ZBAS, TBAS, TOPT,TOPZ, CAPE, SHEAR, ESI, ESIMAX
      character*8 cdate
      integer idate, IC, report, i, nofroze
...
C     CONVERT INTEGER FILE NAME TO CHARACTER EQUIVALENT
     ...
     *      TOPZ, CAPE, SHEAR, cdate, nofroze)

            D=D/2.54
            if(d.ge.1.95) sig=sig+1
            if(d.ge.0.75) svr=svr+1
            ESI=(CAPE*SHEAR)*0.001
c	    print *, 'cape= ',cape, '  shear= ', shear, 'esi= ', esi
	    if((UPMAXV .gt. 7.0).and.(nofroze.eq.1)) then
		DavC = DavC + D
		IC= IC + 1
		if(D. lt. Dmin) Dmin = D
	    endif
      print *,'IC= ',IC, ' UPMAXV= ',UPMAXV, ' D= ',D,' DavC= ',DavC
	    
            if(D. gt. Dmax) Dmax = D
	    IF(ESI.GE.ESIMAX) ESIMAX=ESI
	    enddo
           enddo  
...
c      if temperature is less than -85, set to -85
c      This fixes NSHARP missing data flag, which makes value -9999.0 
...
CJL      IF(NTRAIN.EQ.1)WRITE(2,'(i10,2X,I5,''  CLOUDTOP ENTRAINMENT'')') 
CJL     *                           IDATUM,ISTASIE
CJL      IF(NTRAIN.EQ.2)WRITE(2,'(i10,2X,I5,''  LATERAL ENTRAINMENT'')')
CJL     *                           IDATUM,ISTASIE
      IF(NTRAIN.EQ.1)WRITE(2,*) IDATUM,ISTASIE, '  CLOUDTOP ENTRAINMENT'
      IF(NTRAIN.EQ.2)WRITE(2,*) IDATUM,ISTASIE, '  LATERAL ENTRAINMENT'
      WRITE(2,'(''MAX TEMP    DEW-POINT'')')
CJL      WRITE(2,'(3X,F4.1,9X,F4.1)')TMAX-273.16,TDOU-273.16
      write(2,*) TMAX-273.16,TDOU-273.16
      WRITE(2,
     *'('' CAPE  SHEAR  RICHSON  TYPECLD  PSEUDO'')')
CJL      WRITE(2,'(F7.1,3X,F6.3,5X,F6.0,4X,9X,I2,6X,F6.1)')
CJL     *               CAPE,SHEAR1*1000.0,RICH,ITIPWLK,PSEUDO
      write(2,*) CAPE,SHEAR1*1000.0,RICH,ITIPWLK,PSEUDO
      WRITE(2,
...
*** SKRYF DIE WOLK SE WAARDES UIT
      DO 200 I=1,JTEL+1
CJL      WRITE(2,
CJL     * '(F5.0,F8.1,4F6.1,F7.1,F8.1, F6.1)')(WOLKDTA(I,KK),KK=1,8)
      write(2,*) (WOLKDTA(I,KK),KK=1,8)
...
***************  5. BEREKEN DIE HITTE BUDGET VAN KORREL  **************
     ...
     *           DGMI,GMW,GMI,DI,SEKDEL,ITIPE,P)
      IF(MOD(INT(SEK/SEKDEL),INT(60.0/SEKDEL)).EQ.0)
     *WRITE(4,71)TS,TC,D,P,FW,Z,V,VU,VT,SEK,ITIPE
71    FORMAT(F5.1,' ',F5.1,' ',F8.5,' ',F5.0,' ',F4.2,' ',
     *       F6.0,' ',F7.1,' ',F7.1,' ',F7.1,' ',F6.0,
     *       ' ',I2)

c      IF(SEK/100*100.EQ.SEK)
c       IF(MOD(DINT(SEK/SEKDEL),DINT(300/SEKDEL)).EQ.0)
