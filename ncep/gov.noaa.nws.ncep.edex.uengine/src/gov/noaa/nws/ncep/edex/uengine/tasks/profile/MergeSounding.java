...
//System.out.println("TTBB:");
		for (NcSoundingLayer l: ttbb){
	//System.out.println("P="+l.getPressure()+"H="+l.getGeoHeight()+"T="+l.getTemperature()+"D="+l.getDewpoint()+"Wd="+l.getWindDirection()+"Ws="+l.getWindSpeed());
		}
//System.out.println("PPAA:");
		for (NcSoundingLayer l: ppaa){
	//System.out.println("P="+l.getPressure()+"H="+l.getGeoHeight()+"T="+l.getTemperature()+"D="+l.getDewpoint()+"Wd="+l.getWindDirection()+"Ws="+l.getWindSpeed());
		}
//System.out.println("PPBB:");
		for (NcSoundingLayer l: ppbb){
	//System.out.println("P="+l.getPressure()+"H="+l.getGeoHeight()+"T="+l.getTemperature()+"D="+l.getDewpoint()+"Wd="+l.getWindDirection()+"Ws="+l.getWindSpeed());
		}*/
...
//System.out.println ( "sndataSize in mergeUairSounding(), after calling getSurfaceData(): " + sndata.size() );				
		if ( isNumber(level) >= 0 ) {			
			if ( equal(0.f, Float.valueOf(level.trim()).floatValue()) ||
					equal(sl.getPressure(), Float.valueOf(level.trim()).floatValue())) {
...
//System.out.println ( "sndataSize in mergeUairSounding(), after calling mergeMandatory(): " + sndata.size() );
		...
		if ( isNumber(level) >= 0 ) {			
			for ( int kk = 0; kk < sndata.size(); kk++) {
				if ( equal(Float.valueOf(level.trim()).floatValue(), sndata.get(kk).getPressure())) {
			...
			if (layer.getDewpoint() == RMISSD) {
				...
				if (rh == RMISSD || temp == RMISSD ) {
					...
					if(vapr < Math.exp(-30))
						...
						dwpc = (float) (243.5 * ( Math.log(6.112) - Math.log(vapr)) / (Math.log(vapr) - Math.log(6.112)-17.67));
						layer.setDewpoint(dwpc);
...
//System.out.println("From scaleHeight: " );
	...
	public NcSoundingLayer getSurfaceData(List<NcSoundingLayer> man, 
			List<NcSoundingLayer> ttbb, List<NcSoundingLayer> ppbb, float elevation ) {
		float pres = RMISSD;
		...
		if (man.size() < 1) {
			for (int k = 0; k < NPARMS; k++) {
				...
				sl_sfc.setTemperature(RMISSD);
				sl_sfc.setDewpoint(RMISSD);
				sl_sfc.setWindDirection(RMISSD);
				sl_sfc.setWindSpeed(RMISSD);
				sl_sfc.setGeoHeight(RMISSD);
				sl_sfc.setOmega(RMISSD);
				return sl_sfc;
			}
		} else {
			...
			pres = man.get(0).getPressure();
			if (pres > 1080.) {
				sl_sfc.setPressure(RMISSD);
			} else {
				sl_sfc.setPressure(pres);
			}		
		...
		if (ttbb.size() >= 1 && ttbb.get(0).getPressure() != RMISSD
				&& ttbb.get(0).getTemperature() != RMISSD) {
			pman = sl_sfc.getPressure();
		...
		if (checkWindData(ppbb)) {
			if (ppbb.size() > 0 && ppbb.get(0).getGeoHeight() == 0.
					&& ppbb.get(0).getWindDirection() != RMISSD) {
				sl_sfc.setWindDirection(ppbb.get(0).getWindDirection());
				sl_sfc.setWindSpeed(ppbb.get(0).getWindSpeed());
			}

		} else {
			if (ppbb.size() > 0 && ppbb.get(0).getPressure() != RMISSD
					&& ppbb.get(0).getWindDirection() != RMISSD) {
				pman = sl_sfc.getPressure();
	...
	public void mergeMandatory(
			List<NcSoundingLayer> man_a, List<NcSoundingLayer> man_c,
		...
		if ( man_a.size() > 0 ) {
			for (int kk = 0; kk < man_a.size(); kk++) {
				...
				if (pres < plast && pres != RMISSD
						&& ( man_a.get(kk).getTemperature() != RMISSD
								|| man_a.get(kk).getWindDirection() != RMISSD)) {
...
//	           //System.out.println("tlev = " + tlev );
			if (newblock) {
				if (sndata.get(nlev).getGeoHeight() != RMISSD
					...
					blev = nlev;
					newblock = false;	
 //                   double h = sndata.get(nlev).getGeoHeight();
//                    if ( h < 0 && h != -9999 )
...
//						double h = sndata.get(kk).getGeoHeight();
//						if ( h < 0 && h != RMISSD  )
		...
		for (int kk = 0; kk < sig_wa.size(); kk++) {
			...
			if (!found) {
				if ( ( sig_wa.get(kk).getWindDirection() != RMISSD
						&& sig_wa.get(kk).getPressure() != RMISSD )) {
...
//System.out.println("From constructPpbbHeight(): " );
		...
		if (tlev == (sndata.size()-1) ) {
			...
			float scale;
			float tb, tdb;
			float tt, tdt, mhght = 0.f;
			pb = sndata.get(tlev).getPressure();
			zb = sndata.get(tlev).getGeoHeight();
			tb = sndata.get(tlev).getTemperature();
			tdb = sndata.get(tlev).getDewpoint();
			...
			for (int kk = tlev+1; kk < sndata.size(); kk++) {
				if (sndata.get(kk).getGeoHeight() == RMISSD) {
					...
					sndata.get(kk).setGeoHeight(mhght);
					if (mhght != RMISSD) {
						pb = pt;
						zb = zt;
						tb = tt;
	...
	public static class reverseSortByPressure implements Comparator<NcSoundingLayer> {
		public int compare(NcSoundingLayer l1, NcSoundingLayer l2) {
	 ...
	 */
	public List<NcSoundingLayer> constructPpbbPressure(List<NcSoundingLayer> sndata) {
		
		if ( sndata.size() <= 2) return sndata;
		...
		if (tlev == (sndata.size()-1) || tlev == IMISSD ) {
			...
			for (int kk = tlev+1; kk < sndata.size(); kk++) {
				if (sndata.get(kk).getPressure() == RMISSD) {
					...
					sndata.get(kk).setPressure((float)(pb*(Math.pow(pt/pb,rmult))));
					pb = pt;
					zb = zt;
				}
			}
	...
	public List<NcSoundingLayer> reOrderSounding(List<NcSoundingLayer> sndata) {
		...
		float tt, td, dd, ff;
		int klev = 0;
		if ( sndata.size() <= 1) return sndata;
	...
	public List<NcSoundingLayer> addUnderGround(List<NcSoundingLayer> man, 
		...
		if (sndata.get(0).getPressure() == RMISSD || man.size() < 1 || sndata.size() <= 1) {
			return sndata;
		}
	 ...
	 */
	public List<NcSoundingLayer> getSingLevel (float pres, List<NcSoundingLayer> sndata) {
		NcSoundingLayer sl =  new NcSoundingLayer();
		List<NcSoundingLayer> sls =  new ArrayList<NcSoundingLayer>();
		sndata = removeUnderGround(sndata);
		if ( sndata.size() <= 1) return setMissing(); //Chin: check size again, after remove under ground lavel, size changed
	...
	public void addDataToList(int index,
			List<NcSoundingLayer> indat, List<NcSoundingLayer> outdat) {
		NcSoundingLayer sl = new NcSoundingLayer();
	 ...
	 */
	public List<NcSoundingLayer> setMissing () {
		List<NcSoundingLayer> outdat = new ArrayList<NcSoundingLayer>();
		NcSoundingLayer sl = new NcSoundingLayer();
