...
public class NcSoundingDrv  {
	...
	private double[][] latLonArray; // e.g. at nth element, lat=[n][0], lon=[n][1] 
	...
	public void setLatLons(double[] latLons) {
		...
		if(latLons.length >0){
			...
			for(int i=0, j=0; i < latLons.length; i=i+2){
				...
				//System.out.println("i=" + i+ " j=" + j+" latlons = "+ latLons[i+1] + ","+ latLons[i]);
				j++;
			}
			stnIdArr=null; //stnId and coordinateArray (or latLonArray) should be mutual exclusive
		}
	}
	...
	public Object getSoundingStnInfoCol() throws Exception {
		...
		if (sndType.equals(ObsSndType.NCUAIR.toString()) ||
			...
			stnInfoCol = PfcSoundingQuery.getPfcSndStnInfoCol(sndType, timeLine,refTimeStr);
		} 
	...
	/*
	 * CHin: 2012 Feb 13: Support PFC sounding with one lat/lon location only.....
	 * Query PFC sounding data in one shot. to see performce difference
	 *
	public Object getSoundingDataByRangeTimeArray() throws Exception {
		...
		Coordinate[]  coorArray = new Coordinate[1];
		Coordinate latLon = new Coordinate();
		latLon.y= lat;
		latLon.x= lon;
		coorArray[0]=latLon;
		//List<NcSoundingProfile> pfs2=PfcSoundingQuery.getPfcSndDataBySoundTimeRangeArray(lat, lon, null, refTimeStr, rangeTimeStringLst, sndType, SndQueryKeyType.LATLON);
		List<NcSoundingProfile> pfs=PfcSoundingQuery.getPfcSndDataGeneric(coorArray, null, refTimeStr, rangeTimeStringLst, sndType,level);
		...
		cube.setSoundingProfileList(pfs);
		returnedObject = cube;
	 ...
	 *  
	 * Chin's Note @ 02/27/2012
	 * This API is currently only used for query grid and bufrua data by Nsharp.
	 */
	private Object getSoundingDataByLatLonArray() throws Exception {
		...
		if(sndType.equals(MdlSndType.ANY.toString()) ) {
			...
			if(latLonArray!= null){
				...
				{
					double lat = latLonArray[i][0];
					double lon = latLonArray[i][1];
					for (int j=0; j< timeLineStrList.size(); j++){
						String timeStr = timeLineStrList.get(j);
						pf = MdlSoundingQuery.getMdlSndData( lat, lon, timeStr, validTimeStartStr, pluginName, modelName );
				...
				soundingProfileList =MdlSoundingQuery.getMdlSndDataProfileList(latLonArray,timeLineStrList.get(0), validTimeStartStr, pluginName, modelName);
			}
		}
				...
				for(Calendar timeCal :rangeTimeCalLst){
					if ( merge == 0 ) {
						...
						if(dataType.equals(DataType.ALLDATA.toString()))
							pf = ObservedSoundingQuery.getObservedSndBufruaAllData(lat, lon,stnId, timeCal,  sndQuery);
						else
							pf = ObservedSoundingQuery.getObservedSndBufruaData(lat, lon, stnId, timeCal, dataType, sndQuery);
						...
						//*System.out.println ( " Request merged data at lat="+ lat+" lon="+lon+ " refT="+ timeCal.getTime().toGMTString());

						ttaa = ObservedSoundingQuery.getObservedSndBufruaData(lat, lon, stnId, timeCal,"TTAA", sndQuery).getSoundingLyLst();
						ttbb = ObservedSoundingQuery.getObservedSndBufruaData(lat, lon, stnId, timeCal,  "TTBB", sndQuery).getSoundingLyLst();
						ttcc = ObservedSoundingQuery.getObservedSndBufruaData(lat, lon, stnId, timeCal,  "TTCC", sndQuery).getSoundingLyLst();
						ttdd = ObservedSoundingQuery.getObservedSndBufruaData(lat, lon, stnId, timeCal, "TTDD", sndQuery).getSoundingLyLst();
						//ppaa = ObservedSoundingQuery.getObservedSndBufruaData(lat, lon, stnId, timeCal, "PPAA", sndQuery).getSoundingLyLst();
						ppbb = ObservedSoundingQuery.getObservedSndBufruaData(lat, lon, stnId, timeCal,  "PPBB", sndQuery).getSoundingLyLst();
						//ppcc = ObservedSoundingQuery.getObservedSndBufruaData(lat, lon, stnId, timeCal,  "PPCC", sndQuery).getSoundingLyLst();
						ppdd = ObservedSoundingQuery.getObservedSndBufruaData(lat, lon, stnId, timeCal, "PPDD", sndQuery).getSoundingLyLst();
						wmax_a = ObservedSoundingQuery.getObservedSndBufruaData(lat, lon, stnId, timeCal,  "MAXWIND_A", sndQuery).getSoundingLyLst();
						wmax_c = ObservedSoundingQuery.getObservedSndBufruaData(lat, lon, stnId, timeCal,  "MAXWIND_C", sndQuery).getSoundingLyLst();
						trop_a = ObservedSoundingQuery.getObservedSndBufruaData(lat, lon, stnId, timeCal,  "TROPOPAUSE_A", sndQuery).getSoundingLyLst();
						trop_c = ObservedSoundingQuery.getObservedSndBufruaData(lat, lon, stnId, timeCal,  "TROPOPAUSE_C", sndQuery).getSoundingLyLst();
						pf = ObservedSoundingQuery.getObservedSndStnInfo(lat, lon,stnId,sndType, timeCal,sndQuery);
	...
	/* Chin:: 2/21/2012
	 * Only NCUair and PFC (modelsounding DB) query is supported now
	 * Use generic query API
	 * Mainly used by CAVE Resource plotting
	...
	public Object getSoundingData2Generic() throws Exception {
		...
		if (sndType.equals(ObsSndType.NCUAIR.toString())) {
			...
			//for (int j=0; j< rangeTimeStringLst.size(); j++){
				...
				uairRecordArrList = ObservedSoundingQuery.getObservedSndNcUairDataGeneric(coordinateArray, stnIdArr, rangeTimeStringLst,rangeTimeArr);
				long t004 = System.currentTimeMillis();
				System.out.println("getObservedSndNcUairDataGeneric query took "+(t004-t003)+"ms");
				if(uairRecordArrList!=null && uairRecordArrList.size()>0){
					...
					System.out.println("getSoundingData2Generic total sounding time merging for "+uairRecordArrList.size()+" profiles took "+(t006-t005)+"ms");
				}				
			//}
		}			
		else if(sndType.equals(PfcSndType.NAMSND.toString()) ||
				...
				List<NcSoundingProfile> listReturned = PfcSoundingQuery.getPfcSndDataGeneric(null, stnIdArr,refTimeStr, rangeTimeStringLst,sndType,level);
	...
	public Object getSoundingDataGeneric() throws Exception {
		...
		if (sndType.equals(ObsSndType.NCUAIR.toString())) {
			...
			if(uairRecordArrList!=null && uairRecordArrList.size()>0){
				//long t005 = System.currentTimeMillis();
				soundingProfileList=processQueryReturnedNcUairData( uairRecordArrList,  useNcSoundingLayer2);
				//long t006 = System.currentTimeMillis();
				...
				List<NcSoundingProfile> listReturned = PfcSoundingQuery.getPfcSndDataGeneric(coordinateArray, stnIdArr,refTimeStr, rangeTimeStringLst,sndType,level);
				soundingProfileList.addAll(listReturned);
	...
	*/
	...
	/* Chin: this is not completed yet....
	 * When use point data query for bufrua is supported, then we have to work on this one.
	 * 
	 */
	private List<NcSoundingProfile>  processQueryReturnedBufruaData(List<UAObs[]> uairRecordArrList, boolean useNcSndLayer2){
		List<NcSoundingProfile>  soundingProfileList = new ArrayList<NcSoundingProfile>(0);
		/*
		if ( merge == 0 ) {
			...
			for(UAObs[] recordArray:uairRecordArrList){
				...
				if(useNcSndLayer2== true){
					...
					if(recordArray != null && recordArray.length >0){			
						...
						for(int k=0; k< recordArray.length; k++){
							UAObs record= recordArray[k];
							if(record.getReportType() == NcSoundingLayer.dataTypeMap.get("TTAA")){
								ttaa = getSndLayersFromNcUairRecordObsLevel(record);
								trop_a = getSndLayersFromNcUairRecordTrop(record);
								wmax_a = getSndLayersFromNcUairRecordMaxw(record);
							}
				...
				if(pf != null && (pf.getSoundingLyLst2().size()>0||pf.getSoundingLyLst().size()>0 )) {
					//System.out.println(" pf is not null, so adding a profile to the list of NcSoundingProfiles ");
					soundingProfileList.add(pf);
					pf = null;
				}
	...
	private List<NcSoundingProfile>  processQueryReturnedNcUairData(List<NcUairRecord[]> uairRecordArrList, boolean useNcSndLayer2){
		List<NcSoundingProfile>  soundingProfileList = new ArrayList<NcSoundingProfile>(0);
		for(NcUairRecord[] recordArray:uairRecordArrList){
			...
			if ( merge == 0 ) {
				...
				if(useNcSndLayer2== true){
					...
					if(recordArray != null && recordArray.length >0){
						...
						//System.out.println(" input lat="+lat+" pf's lat="+pf.getStationLatitude()+" elv="+pf.getStationElevation()+" stnId="+pf.getStationId());
						if(useNcSndLayer2)
							sls = ms2.mergeUairSounding(level,ttaa,ttbb,ttcc,ttdd,ppaa,ppbb,ppcc,ppdd,trop_a,trop_c,wmax_a,wmax_c,pf.getStationElevation()); 
	...
	/*
	 * Convert sounding data saved in NcSoundingLayer list to NcSoundingLayer2 list
	 * remove NcSoundingLayer data to have a smaller size for sending back to client
	 */
	private void convertPfcNcSoundingLayerToNcSoundingLayer2(List<NcSoundingProfile> pfLst){
		for(NcSoundingProfile pf: pfLst){
			...
			for (NcSoundingLayer level : pf.getSoundingLyLst())
			{
				...
				try {
					...
					airTemp.setValue( new Amount ( level.getTemperature(), SI.CELSIUS ) );
					soundingLy2.setTemperature(airTemp);

					DewPointTemp dewPoint = new DewPointTemp();
					dewPoint.setValue( new Amount (level.getDewpoint(), SI.CELSIUS ) );
					soundingLy2.setDewpoint(dewPoint);

					PressureLevel pressure = new PressureLevel();
					pressure.setValue( new Amount (level.getPressure(), NcUnits.MILLIBAR ));
					soundingLy2.setPressure(pressure);

					WindDirection windDirection = new WindDirection();
					windDirection.setValue(level.getWindDirection(), NonSI.DEGREE_ANGLE);
	...
	private void sysPrintProfileLayer2(List<NcSoundingProfile> pfs){
		System.out.println("-----------------------------------------------------------------\n sysPrintProfileLayer2: profile size ="+ pfs.size());
		for(int i =0; i < pfs.size();i++){
			if(pfs.get(i).getStationId().indexOf('K') == 0){
			System.out.println("pf"+i+ " stn="+pfs.get(i).getStationId()+" lat/lon="+ pfs.get(i).getStationLatitude()+"/"+pfs.get(i).getStationLongitude());
			if(pfs.get(i).getSoundingLyLst2().size() >0){
				System.out.println(" temp="+pfs.get(i).getSoundingLyLst2().get(0).getTemperature()+" dewp="+pfs.get(i).getSoundingLyLst2().get(0).getDewpoint()+" press="+
						pfs.get(i).getSoundingLyLst2().get(0).getPressure() + " height="+pfs.get(i).getSoundingLyLst2().get(0).getGeoHeight()+
						" windSp="+pfs.get(i).getSoundingLyLst2().get(0).getWindSpeed()+" windDir="+pfs.get(i).getSoundingLyLst2().get(0).getWindDirection()+
						" omega="+pfs.get(i).getSoundingLyLst2().get(0).getOmega());
			}
			}
		}
	}
