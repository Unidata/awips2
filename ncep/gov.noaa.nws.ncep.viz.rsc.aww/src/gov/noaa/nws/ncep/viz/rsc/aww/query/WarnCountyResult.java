...
public class WarnCountyResult {
	...
	// Don't need the state but keep for now since it shouldn't slow us down much at all.
	// Could also do w/o the countyname if we stick with using the name from the StationsTable.
	...
	/**
	 * TODO : This needs to be changed to be able to query more fips when new aww records are ingested. Currently there
	 * is a bug when auto update is triggered for aww's that have counties that were not in the list of original fips to query.
	 * 	
	 */	
