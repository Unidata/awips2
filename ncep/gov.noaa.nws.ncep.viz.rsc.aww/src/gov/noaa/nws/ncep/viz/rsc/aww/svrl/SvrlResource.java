...
import gov.noaa.nws.ncep.viz.rsc.aww.query.SvrlCountyQueryResult;
...
implements     INatlCntrsResource, IStationField  { 
	...
	public class SvrlData implements  IRscDataObject{
		...
		List<String>		countyUgc,countyNames,stateNames;//T456  //To get all the counties for the warning
	...
	private ArrayList<SvrlData> getAwwtData( AwwRecord awwRecord) {
		...
		List<SvrlData> svrlDataList = new ArrayList<SvrlData>();
				...
				for (AwwUgc awwugcs : awwUgc) {
					...
					// Why not just get the reportType from the record?
...
//					}
					...
					svrlStatusData.reportType = AwwReportType.getReportType( awwRecord.getReportType() );
	...
	public void paintFrame( AbstractFrameData frameData, 
		...
		for( SvrlData svrlData : svrlDataValues ) {
			...
			if(svrlRscData.getColorCodeEnable()){
				...
				if( svrlData.reportType == AwwReportType.SEVERE_THUNDERSTORM_WATCH ) { //).equalsIgnoreCase("SEVERE_THUNDERSTORM_WATCH")){
				...
				else if ( svrlData.reportType == AwwReportType.TORNADO_WATCH_OUTLINE_UPDATE ) { //).equalsIgnoreCase("TORNADO_WATCH_OUTLINE_UPDATE")){
