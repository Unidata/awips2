...
//	}
	...
	public void paintFrame( AbstractFrameData frameData, 
		...
		if( areaChangeFlag ) { 
			areaChangeFlag = false; postProcessFrameUpdate(); 
...
//	for(FfaRscDataObj eachFFAData : fdvList){//T456
		...
		for( FfaRscDataObj eachFFAData : ffaDataValues ) {
			...
			AwwReportType rptyp = AwwReportType.getReportType( eachFFAData.reportType );
			...
			if( rptyp == AwwReportType.FLASH_FLOOD_WATCH ) {
				//FFAConstant.FLASH_FLOOD_WATCH.equalsIgnoreCase(eachFFAData.reportType)){
				...
				//FFAConstant.FLOOD_WATCH/*.FLASH_FLOOD_STATEMENT*/.equalsIgnoreCase(eachFFAData.reportType)){//T456
