...
public class EditSatelliteAttrsDialog extends AbstractEditResourceAttrsDialog {
    ...
    public Composite createDialog(Composite topComp) {
        ...
        if (colorMapNameAttr == null
                || colorMapNameAttr.getAttrClass() != String.class) {
            System.out
                    .println("colorMapName is null or not of expected class String?");
            return null;
        }
        if (colorBarAttr == null
                || colorBarAttr.getAttrClass() != ColorBarFromColormap.class) {
            System.out
                    .println("colorBar is null or not of expected class ColorBarFromColormap?");
            return null;
        }
        ...
        final String availColorMaps[] = ColorMapUtil
                .listColorMaps(cmapCategory);

        final Combo colorMapNamesCombo = new Combo(topComp, SWT.READ_ONLY
                | SWT.DROP_DOWN);
        ...
        for (int c = 0; c < availColorMaps.length; c++) {
            if (availColorMaps[c].equals(colorMapNameAttr.getAttrValue()
                    .toString())) {
        ...
        if (seldColorMapIndx == -1) {
            System.out.println("The current colorMap '"
                    + colorMapNameAttr.getAttrValue().toString()
                    + "' was not found.");
            seldColorMapIndx = 0;
        ...
        colorMapNamesCombo.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                // update the selected color map name in the attrSet and in the
                // colorbar editor.
                colorMapNameAttr.setAttrValue(availColorMaps[colorMapNamesCombo
                        .getSelectionIndex()]);
                setColorBarFromColorMap(availColorMaps[colorMapNamesCombo
                        .getSelectionIndex()]);
        ...
        // usually just the options (length, orientation..) are initialized in
        // the colorbar and a colormap
    ...
    public void setColorBarFromColorMap(String cmapName) {
        ColorMap colorMap;
        try {
            colorMap = (ColorMap) ColorMapUtil.loadColorMap(cmapCategory,
                    cmapName);
        ...
        (editedColorBar).setColorMap(colorMap);
    }
