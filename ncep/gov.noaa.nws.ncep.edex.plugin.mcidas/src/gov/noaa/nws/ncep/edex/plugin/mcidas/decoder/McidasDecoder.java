...
public class McidasDecoder extends AbstractDecoder {
	...
	public PluginDataObject[] decode(byte[] data) throws Exception {
		...
		if (byteArrayToInt (area, 0, 0) == 0) {
			...
			/*
			 *  Check endians for 2nd word, if it is not 4, i.e., big endian, swapping bytes.
			 */
			...
			/*
			 *  Number of bytes each element (1, 2 or 4)
			 *  	int zdim = byteArrayToInt (area, 40, endian);
			 */			
			...
			/*
			 * Maximum number of bands per scan line
			 * 		int zres = byteArrayToInt (area, 52, endian);
			 */
			...
			/*
			 *  User project number under which the area is created
			 *  	int task = byteArrayToInt (area, 60, endian);
			 */
			...
			/*
			 * Get and set image type, e.g., VIS, IR, IR2 from satellite name and 
			 * image type number
			 */
			int imageTypeNumber = byteArrayToInt (area, 72, endian);
			record.setImageTypeNumber(imageTypeNumber);
			if ( imageTypeNumber <= 0 ) imageTypeNumber = -1;
			String imageType=dao.getImageType(Integer.toString(sid), 
					Integer.toString(imageTypeNumber)).get(0).getImageType();
           
			/* String memo = 
			 *	byteArrayToString(area,96,endian) + byteArrayToString(area,100,endian) +
			 *	byteArrayToString(area,104,endian) + byteArrayToString(area,108,endian) +
			 *	byteArrayToString(area,112,endian) + byteArrayToString(area,116,endian) +
			 *	byteArrayToString(area,120,endian) + byteArrayToString(area,124,endian) +
			...
			/* 
			 * Get and set the area name from AFN.  If area name has a "|", parse the file
			 * and the 1st part is the group name for the satellite.  The 2nd part is the
			 * area name.	
			 */
			...
			if ( areaName.contains("|")) {
				...
				/*
				 * Handle special cases for duplicate area file numbers (see Design document)
				 */						
			...
			/* 
			 * Area file number
			 * 		int filno = byteArrayToInt (area, 128, endian);
			 */
			...
			/*
			 * Navigation offset: byte offset to the start of the navigation block
			 *
			 */
			int navoff = byteArrayToInt (area, 136, endian);

			/*
			 * Validity code: if these bytes are non-zero, they must match the first four 
			...
			/*
			 * Actual starting scan
			 * 		int ascan = byteArrayToInt (area, 188, endian);
			 * 
			 * Line prefix documentation section length in bytes
			 *		int predoc = byteArrayToInt (area, 192, endian);
			 *
			 * Line prefix calibration section length in bytes
			 *  	int precal = byteArrayToInt (area, 196, endian);
			 * 
			 * Line prefix level map section length in bytes
			 *		int prelvl = byteArrayToInt (area, 200, endian);
			 *
			 * Image source type: "VISR', "VAS', 'AAA', ERBE', "AVHR'
			 * 		String srctyp = byteArrayToString (area, 204, endian);
			 */
			...
			/* Processing type
			 *		int prctyp = byteArrayToInt (area, 212, endian);
			 *
			 * POES signal type
			 *		int sigtyp = byteArrayToInt (area, 216, endian);
			 *
			 * POES ascending/descending 
			 *		int phase = byteArrayToInt (area, 220, endian);
			 *
			 * Original source srctyp
			 * 		String orgtyp = byteArrayToString (area, 224, endian);
			 */
			...
			/*
			 * For map coverage compliance: 1: Mecator (MERC), 3: Lamber Conformal (LAMB), 
			 * 5: Polar Steoreographic (PS)
			 */	
			int resolution = 0;
			Integer iproj = 0;
			...
			/*
			 * Set pixel/grid spacing and earth radius and eccentricity. For TANC, 
			 * n5 is the standard latitude.
			 */
			...
			/*
			 * Eccentricity
			 * 		int n8 = byteArrayToInt (navigation, 28, endian);
			 ...
			 */
				...
				if (navtyp.equals ("LAMB")) {
					...
					/*
					 *  McIDAS uses Earth Radius 6371.1 (RADIUS).  Navigation block km per pixel
					 *  scaled by 10000., convert to meters
					 */
					re = RADIUS;
				...
				if (psi1 == psi2) {
					...
					double tmp2 = Math.log (Math.tan(psi2/2.) / Math.tan(psi1/2.));
					ccone = tmp1 / tmp2;
				}

				/*
				 * Compute lat/lon of the lower-left corner. Sing = 1/-1 denotes NH/SH
				 */
				double dxp = 1. - rxp;
				double dyp = 1. - ryp;
				double rm = dx * Math.sqrt((dxp*dxp + dyp*dyp));
				double tmp = ccone / (re * Math.sin (psi1));
				double arg = Math.pow (rm * tmp, 1. / ccone) * Math.tan(psi1/2.);
			...
			try {
				if ( iproj <= 5 ) {
					mapCoverage = McidasSpatialFactory.getInstance().getMapCoverage (iproj,
							nx, ny, dx, dy, clon, stdlat1, stdlat2, lllat, lllon, urlat, urlon);
				}
				else {
					// non-remapped Navigations
					mapCoverage = McidasSpatialFactory.getInstance().getMapCoverage (iproj,
							nx, ny, clon, ulelem, ulline, xres, yres, navigation);
				}
			} catch (Exception e) {
				...
				if ( iproj <=5 ) {
					...
					buf.append("Error getting or constructing SatMapCoverage for Navigation Type "+proj).append("\n");
				}
	...
	private Calendar convertJulianToCalendar(int julian, int hhmmss) {
		...
		/*
		 * The Julian day format is nYYDDD where n = 1, year > 2000 and n = 0, year
		 * is prior to 2001. For example, 109244 -> 2009/244.
		 */
		if (julian > 100000) {
	...
	public static void main(String[] args) throws Exception {
		File infile = new File("/export-3/ncosrvnfs/images/satellite/GOES-W/Full_Disk/IR/IR_20100810_0900");
		...
		dmtf.scanForPlugins();
		McidasDecoder dec = new McidasDecoder();
		dec.decode(payload);
	}
	*/
