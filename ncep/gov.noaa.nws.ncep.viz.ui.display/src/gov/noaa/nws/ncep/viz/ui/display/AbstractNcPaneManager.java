...
//  protected int displayId;
    ...
    // a flag to indicate that resources other than those in the default RBD have been loaded.
    // The primary and currently only use for this is that if false, this editor is considered 
    // empty or available for loading new data.
    
    // Currently it is only the resources that are checked and not the sync flags or the area.
    //
    ...
    protected boolean isHide = false; // means loop rsc on, and Hide btn displayed
    ...
    // We are using the selectedPanes map from PaneManager but the keys are bogus
...
//    protected ArrayList<IDisplayPane> selectedPanesList = null;
    ...
    protected class NcPaneMouseHandler extends InputAdapter {
        ...
        public boolean handleMouseMove(int x, int y) {
            ...
            if (lastHandledPanes.length > 1
                ...
                geoSync = NcEditorUtil.arePanesGeoSynced( (AbstractNcEditor)paneContainer );
            }
    ...
    // The following methods are based on the NcDisplayType but these can be overridden
    // 
    // 
    ...
    public INatlCntrsRenderableDisplay createNcRenderableDisplay( INcPaneID pid ) {    	
    	try {
			...
			if( rendDispObj instanceof INatlCntrsRenderableDisplay ) {
				...
				Class<?> descrClass = Class.forName( 
									getDescriptorClass( rendDispClassName, pid) );
                ...
                if (e.button == 1 || e.button == 3) {
                    ...
                    if (paneContainer instanceof AbstractNcEditor) {
                        NcEditorUtil.selectPane( 
                        		(AbstractNcEditor)paneContainer, thisPane, radioBehaviour);
                    }
                }
            }
        });
    ...
    public void setSelectedPane(String action, IDisplayPane pane) {
        ...
        if (!selectedPanes.values().contains(pane) ) {
        	...
        	if( selectedPanes.containsKey( keyStr ) ) {
        		System.out.println("Sanity check in NcPaneManager.setSelectedPane. A non-unique key was created???");
        		return;
        	}
...
//        System.out.println("NCPaneManager selecting pane " + pane.hashCode());
    ...
    public IDisplayPane[] getSelectedPanes(String action) {
        ...
        IDisplayPane seldPanes[] = selectedPanes.values().toArray(new IDisplayPane[0]);
    ...
    protected IDisplayPane addRenderableDisplayToPane(
            IRenderableDisplay renderableDisplay, Composite canvasComp, INcPaneID pid) {

    	// sanity check 
    	// the pane id index should match the index into the displayPanes array.
    	if( pid != null &&
    		paneLayout.getPaneIndex( pid ) != displayPanes.size() ) {
    		System.out.println("??? in addRenderableDisplayToPane. "+
    ...
    public IDisplayPane addPane(IRenderableDisplay renderableDisplay) {
        ...
        if( renderableDisplay instanceof INatlCntrsRenderableDisplay ) {
    		paneId = ((INatlCntrsRenderableDisplay)renderableDisplay).getPaneId();
    	}
    	else {
    		System.out.println("??? addPane() to non-INatlCntrsRenderableDisplay display pane");
        }
...
//        parentComposite = null;
    ...
    public void setAutoUpdate(boolean autoUpdate) {
        ...
        for (IDisplayPane pane : dispPanes) {
        	((INatlCntrsDescriptor)pane.getDescriptor()).setAutoUpdate(autoUpdate);
        }
    }
