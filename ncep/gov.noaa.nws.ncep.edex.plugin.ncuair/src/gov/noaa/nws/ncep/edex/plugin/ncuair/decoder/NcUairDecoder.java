...
public class NcUairDecoder extends AbstractDecoder {
     ...
     */
    public NcUairDecoder(String name) throws DecoderException {
    	pluginName = name;
    }
	...
	public ObStation getStationInfo(Integer wmoIndex) {
	    ...
	    if(dao != null) {
	    	List<?> obs = null;
	    	try {
		        obs = dao.queryBySingleCriteria("wmoIndex", wmoIndex.toString());	
	    	} catch (DataAccessLayerException e) {
        		System.out.println("Error while quering for wmoIndex: "+wmoIndex);
        		return null;
        	}
    ...
    public PluginDataObject[] decode(byte[] data, Headers headers) throws DecoderException {
       ...
       long curTime = System.currentTimeMillis();
    	...
    	if ( stationNumber != null ) {
    		...
    		if (station != null && (station.getStationId() != null && station.getStationId().length() > 0)) {
                SurfaceObsLocation obsLoc = new SurfaceObsLocation(station.getStationId());
                if (station.getGeometry()!=null) {
                	...
                	logger.info("Latitude/Longitude (the_geom) missing for Station Number [" + stationNumber + "]. Record discarded.");
                	record = null;
                }                   
                   		
            } else {
            	// Couldn't find stnum (wmoindex) in spatial table
            	logger.info("Station Number not found [" + stationNumber + "]. Record discarded.");
            	record = null;
            }
    		if ( record !=null ){
	    		 // Get time group
    	...
    	if ( ! nil && record !=null) {
    		 ...
    		 if ( reportMatcher.find()) {
    			 ...
    			 if ( shipMatcher.find()) {
    				 ...
    				 SurfaceObsLocation obsLoc = new SurfaceObsLocation(NcUairShipMobile.getStnId());
    	             obsLoc.assignLocation(NcUairShipMobile.getXlat(), NcUairShipMobile.getXlon());
    ...
    public PluginDataObject[] decodeBatch(byte[] data, Headers headers) throws DecoderException {
    	byte[] messageData = null;
    	PluginDataObject[] returnObjects = null;
    	String traceId = null;
    	WMOHeader hd;
    	Boolean nil=false;
    	System.out.println("Nc uair decodeBatch(), raw data size= "+ data.length); 
    	//long curTime = System.currentTimeMillis();
    	if (headers != null) {
    		/*
    		 * traceId equals to the file name
    	...
    	while( sep.hasNext() ){
        		...
        		ObStation station = getStationInfo(Integer.parseInt(stationNumber));
        		 ...
        		 if (station != null && (station.getStationId() != null && station.getStationId().length() > 0)) {
                    ...
                    if (station.getGeometry()!=null) {
                    	...
                    	// lat/lon (the_geom/upperairgeom) missing in spatial table
                    	logger.info("Latitude/Longitude (the_geom) missing for Station Number [" + stationNumber + "]. Record discarded.");
                    	continue;
                    }                   
    		...
    		if (record != null && record.getLocation() != null && record.getLocation().getStationId() == null) {
    		...
    		if ( record != null ){
    			if ( obsTime != null ) {
        			if(Math.abs(obsTime.getTime().getDate() - issueTime.getTime().getDate()) >= 2 ){
        				//Chin, not a good record, should just skip it
        				System.out.println("Nc uair record bad issue time " + issueTime.getTime().toString() + " obs Time "+obsTime.getTime().toString());
        				continue;
        			}
    	...
    	//long enqueueTime = System.currentTimeMillis();
    	//double latency = (  enqueueTime-curTime);
    	//System.out.println("end of Nc uair decode, spent time="+ latency + "records size="+recordList.size()); 
