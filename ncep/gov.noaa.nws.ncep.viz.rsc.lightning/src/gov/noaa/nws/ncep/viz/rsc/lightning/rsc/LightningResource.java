...
public class LightningResource extends
        AbstractNatlCntrsResource<LightningResourceData, MapDescriptor>
    ...
    private class FrameData extends AbstractFrameData {
        ...
        public boolean updateFrameData(IRscDataObject rscDataObj) {
            ...
            // ((DfltRecordRscDataObj)rscDataObj).getPDO();
            // BinLightningRecord ltngRec = (BinLightningRecord) pdo;
            // DataTime ltngTime = ltngRec.getDataTime();
            //
            // // look up the uri in the map created during the preprocessing
            // pass.
            // String uri = ltngRec.getDataURI();
            // List<LtngStrikeDataObj> strikes =
            // strikeMap.get(ltngRec.getDataURI());
            //
            // // TODO : Do we need to check if this uri/pdo has already been
            // processed
    ...
    private ArrayList<LtngStrikeDataObj> getStrikeDataFromHDF(Object[] pdoList) {
        ...
        // ArrayList<LtngStrikeDataObj> ltngStrikesList = new
        // ArrayList<LtngStrikeDataObj>();
        ...
        for (File hdf5File : hdf5FileMap.keySet()) {
            ...
            try {
                ...
                // records for this
                // URI and then fill in the strikeMap with all of the strikes
                // for this URI.
                for (String uri : uris) {
                    for (IDataRecord hdf5Rec : hdf5Recs) {
                        if (hdf5Rec.getGroup().equals(uri)) {
                            List<LtngStrikeDataObj> strikeList = strikeMap
                                    .get(uri);
                            ...
                            if (strikeList == null) {
                                ...
                                // init with the number of strikes for this
                                // record.
                                // NOTE: all of the records for this uri
                                // must(will) have the same
                            ...
                            for (int s = 0; s < hdf5Rec.getSizes()[0]; s++) {
                                LtngStrikeDataObj strikeInfo = strikeList
                                        .get(s);
                                if (hdf5Rec.getName().equals("intensity")) {
                                    ...
                                    // }
                                    // } else if(
                                    // hdf5Rec.getName().equals("strikeCount") )
                                    // {
                                    // strikeInfo.lon = ((FloatDataRecord)
                                    // hdf5Rec).getFloatData()[s];
    ...
    protected IRscDataObject[] processRecord(Object pdo) {
        if (pdo instanceof LtngStrikeDataObj) {
            ...
            System.out
                    .println("LTNG:processRecord expecting LtngStrikeDataObj or "
    ...
    public void paintFrame(AbstractFrameData frameData,
        ...
        if (!needsUpdate && !currFrameData.ltngStrikeShapesList.isEmpty()
            ...
            for (LtngStrikeDataObj strike : currFrameData.ltngStrikes) {
                ...
                // System.out.println("Intensity = "+ Integer.toString(
                // strike.intensity ) );
                ...
                if (extent.contains(worldLoc)) {
                    ...
                    long endTime = currFrameData.getFrameTime().getRefTime()
                            .getTime()
                            / 1000
                            - (long) (ltngRscData.getColorBar().getIntervalMin(
                                    0) * 60);
                    ...
                    for (int i = 0; i < ltngRscData.getColorBar()
                        ...
                        // this colorInterval.
                        // if we are coloring by history then we will check if
                        // the strike is within
                        // the given time interval and if we are coloring by
                        // intensity then we will
                        ...
                        if ((!ltngRscData.getColorByIntensity()
                            ...
                            // System.out.println("Strike time is "+
                            // strike.getDataTime().toString() );
                            // System.out.println("is in Interval " +
                            // Integer.toString(i) + " : " +
                            // new DataTime( new Date(beginIntervalTime*1000)
                            // ).toString() +" to " +
        ...
        // code from d2d's lightningResource to draw the strike counts in the
        // upper left corner.
        //
        ...
        // TextStyle.BLANKED, ltngRscData.getColor(), HorizontalAlignment.RIGHT,
        // null);
        // }
        //
        // if( ltngRscData.getEnableNegativeStrikes() ) {
        // grphTarget.drawString(null, noIntensityCount + " 0 Strikes",
        // extent.getMinX()
        // + (1500 * zoomLevel),
        // extent.getMinY() + (1000 * zoomLevel), 0.0,
        // TextStyle.BLANKED, ltngRscData.getColor(), HorizontalAlignment.RIGHT,
