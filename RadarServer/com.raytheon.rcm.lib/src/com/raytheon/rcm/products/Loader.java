...
public class Loader {
    static List<RadarProduct> loadRadarInfoData(Scanner fs) {
        ...
        while (fs.hasNext()) {
            ...
            try {
                ...
                if (ls.next().trim().equalsIgnoreCase("y")) // TODO: same as
                                                            // above?
                    rp.params.add(Param.STORM_SPEED_DIR);
                if (ls.next().trim().equalsIgnoreCase("y"))
            ...
            case 35:
            case 36: // 35 and 36 are missing from SPG ICD Table IIa
            case 37:
            case 38:
            case 41:
            case 57:
            ...
            case 141:
                ...
                /*
                 * This is special. For the TDWR, it does *not* have an
                 * elevation, only a mini-volume. So create two products with
                 * code 149.
                 */
            case 149:
    ...
    public static void loadElevationInfo(Scanner fs,
            HashMap<Sel, int[]> staticInfo, Collection<VCPInfo> vcpInfo) {
        ...
        while (fs.hasNext()) {
            ...
            try {
                ...
                if (m.matches())
                    ...
                    vcp = ElevationInfo.OTR_CODE;
                else if (id.equals("RMR"))
                    vcp = ElevationInfo.RMR_CODE;
                else if (id.equals("OTR-TDWR"))
                    vcp = ElevationInfo.OTR_TDWR_CODE;
                else if (id.equals("RMR-TDWR"))
                    vcp = ElevationInfo.RMR_TDWR_CODE;
                else
                    continue;
    ...
    public static void loadTdwrElevationInfo(Scanner fs,
        ...
        HashMap<String, HashSet<Integer>> allElevs = new HashMap<String, HashSet<Integer>>();
        ...
        while (fs.hasNext()) {
            ...
            if (skipComments(ls))
                continue;
            /*
             * We currently only need to know the set of unique angles, not the
             * whole scan strategy.
             */
            elevs.clear();
            try {
        ...
        for (Map.Entry<String, HashSet<Integer>> e : allElevs.entrySet()) {
            ...
            Sel sel = new Sel(e.getKey(), ElevationInfo.OTR_CODE);
            staticInfo.put(sel, elevsArray);
        }
    }
}
