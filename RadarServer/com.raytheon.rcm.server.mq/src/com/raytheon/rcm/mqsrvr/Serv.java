...
import com.raytheon.rcm.ccmmgr.CCMManager;
...
public class Serv {
    ...
    public Serv(RadarServer server) {
        ...
        for (RadarEventListener l : server.getListeners()) {
            ...
            } else if (l instanceof CCMManager) {
                ccmManager = (CCMManager) l;
            }
        }
        ...
        if (ccmManager == null) {
            Log.warn("No CCM manager is registered.  Remote clients cannot sent CCMs.");
        }
    }
    ...
    public Collection<RadarConfig> getAllRadarConfigs() {
        ...
        Collection<RadarConfig> result = new ArrayList<>();
    ...
    public Map<String, RadarStatus> getAllRadarStatus() {
        HashMap<String, RadarStatus> result = new HashMap<>();
        Configuration config = server.getConfiguration();
    ...
    public void sendOTRs(List<String> radarIDs, List<Request> requests,
    ...
    public String sendRpsList(List<String> radarIDs, int vcp,
        ...
        RpsList rpsList = new RpsList(-1, vcp, requests.toArray(new Request[0]));
        return sendRpsListCommon(radarIDs, rpsList, vcp, store);
    }

    public String sendRpsListData(List<String> radarIDs, int vcp, byte[] data,
            boolean store) {
        // TODO: INVALID_OP_MODE
        RpsList rpsList;
        try {
            rpsList = RcmUtil.parseRpsListData(data, -1, vcp);
        } catch (IOException e) {
            // TODO: ...
            return e.toString();
    ...
    public String sendCcmRequest(String radarID, CcmRequestDefinition ccmRequest) {
        return ccmManager.sendCcmRequest(radarID, ccmRequest);
    }
