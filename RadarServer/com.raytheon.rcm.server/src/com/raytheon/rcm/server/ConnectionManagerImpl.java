...
public class ConnectionManagerImpl extends RadarEventAdapter implements
        ConnectionManager, StatusManager {
    ...
    private static class Status implements StatusManager.RadarStatus {
        ...
        void setUnconnected() {
            ...
            currentGSM = currentCPM = currentPTL = null;
        }
    ...
    protected HashMap<LinkType, InterfaceStatus> ifcSt = new HashMap<LinkType, InterfaceStatus>();
    protected ArrayDeque<String> connectReqQueue = new ArrayDeque<String>();

    private ConnectionManagerImpl() {
    }

    public ConnectionManagerImpl(RadarServer radarServer,
    ...
    public void handleRadarEvent(RadarEvent event) {
        ...
        if (event.getType() == RadarEvent.CONNECTION_UP) {
            ...
            case Message.COMMAND_PARAMETER_MESSAGE:
                status.currentCPM = status.lastCPM = msg;
                break;
        ...
        if (status != null && rc != null) {
            ...
            if (status.state == State.CONNECTED
                ...
                if (linkType != null) {
                    ...
                    synchronized (ifcStatus) {
                        /*
                         * Logic/design shortcut: If a dedicated connection,
                         * just ignore the line count. TCP connections are
                         * 'unlimited' anyway and there are no longer dedicated
                         * X25 connections.
                         * 
                    ...
                    Log.errorf("Link type for radar '%s' not configured",
                            radarID);
                }
            }
        } else {
     ...
     * 
     * @param msg
     *            A Nexrad message. If either the source or destination ID is
     *            zero, it will be changed to the appropriate value.
     *            <code>msg</code> must not be immutable in this case as its
    ...
    public void sendMessageToRadar(String radarID, byte[] msg) {
        ...
        if (status != null) {
            ...
            if (buf.getShort(14) == 0)
                buf.putShort(14, (short) config.getConfigForRadar(radarID)
                        .getNexradID());
