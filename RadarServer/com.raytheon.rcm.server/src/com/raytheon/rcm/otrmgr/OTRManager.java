...
public class OTRManager extends RadarEventAdapter {
    ...
    protected class RadarStatus {
        ...
        /**
         * isConnected+goGsm: because there is not ConnectionManager.isConnected
         */
        ...
        public void handleMessage(byte[] messageBuffer) {
            ...
            if (code == Message.GSM) {
                ...
                while (i.hasNext()) {
                    ...
                    if (r.sent && r.sequence == rr.sequence) {
                         ...
                         * Message): These two errors indicate the product will
                         * be sent in the next scan.
                         */
                        if ((rr.errorCode & ~(RequestResponse.AVAILABLE_NEXT_SCAN | RequestResponse.OTR_PROCESS_FAULTED)) != 0) {
                            r.nExpected--;
                            // TODO: correct logic
                            if (r.nExpected < 1) {
                                i.remove();
            ...
            if (requests.isEmpty()) {
                /*
                 * The only reason we contact dial radars is for OTRs so this is
                 * okay for now.
                 */
        ...
        public void sendRequests(Iterable<Request> requests, OTRHandler handler) {
            ...
            synchronized (this.requests) {
                ...
                Lsrc: for (Request src : requests) {
                    Filter filter = Filter.getFilterForCode(src.productCode);
                    for (Req r : this.requests) {
                        /*
                         * This could be a "merge" rather than a "filter".
                         * Although exact comparison of fields such as elevation
                         * and repeat count produce safe results, it does not
                         * filter out all possible duplicates.
                         */
                        if (filter.requestsEqual(src, r.request, null/*
                                                                      * TODO:
                                                                      * ...
                                                                      */,
                                RcmUtil.getRadarType(getRadarConfig()))) {
                            if (handler != null)
            ...
            if (isReady()) {
                ...
                synchronized (this.requests) {
                    for (Req r : requests) {
                        if (!r.sent) {
                            r.sent = true;
                            r.sequence = r.request.sequence = Sequence
                                    .next(radarID);
                            r.timeSent = now;
                            r.calculateNExpected();
                            Log.eventf("Sending request %s to %s", r.request,
                                    radarID);
                ...
                byte[] msg = ProductRequest.encode(requestsToSend
                        .toArray(new Request[0]));
                radarServer.getConnectionManager().sendMessageToRadar(radarID,
                        msg);
            } else {
        ...
        private class Req {
            ...
            public String toString() {
                return String.format(
                        ...
                        request, sent, sent ? new Date(timeSent) : "",
                        sequence, nExpected);
            }
            ...
            public void calculateNExpected() {
                 ...
                 * how many elevations will be returned. Note AWIPS 1 does not
                 * do this and disconnects (from dial radars) after the first
                 * product is received.
                 */
                if (MessageInfo.isElevationBasedProduct(request.productCode)
                    ...
                    if (lastGSM != null) {
                        ...
                        Log.warnf(
                                "Do not have the latest GSM for '%s'.  Cannot determine how many products to expect for multi-elevation OTR",
                                radarID);
                    }
                }
    ...
    /*
     * public static final int SUCCESS = 1; public static final int FAILED = 2;
     * public static final int TIMED_OUT = 3; public static final int PRR = 4;
     *
     * interface Handler { void handleRequestResult(Object cookie, String
     * radarID, Request request, int result, byte[] msg); }
     */
    ...
    public void sendOneTimeRequests(Iterable<String> radarIDs,
            Iterable<Request> requests, OTRHandler handler) {
    ...
    protected RadarStatus getRadarStatus(String radarID, boolean create) {
        ...
        if (rs == null && create) {
            RadarConfig rc = radarServer.getConfiguration().getConfigForRadar(
                    radarID);
            if (rc == null) {
                Log.errorf("Request to send OTR to unconfigured radar '%s'",
                        radarID);
                return null;
            }
    ...
    public void handleRadarEvent(RadarEvent event) {
        /*
         * need to create status so we can track state
         */
        RadarStatus rs = getRadarStatus(event.getRadarID(), true/* false */);
        /*
         * ... less necessary if we have ConnMgr.isConnected ... not necessary
         * if we have ConnMgr.getRadarState().getCurrentGsm()
         */
        if (rs == null)
            return;
