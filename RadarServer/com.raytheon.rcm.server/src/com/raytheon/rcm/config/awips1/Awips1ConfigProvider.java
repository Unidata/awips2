...
/**
 * Builds a {@link com.raytheon.rcm.config.Configuration} instance based on a
 * set of configuration files in the AWIPS 1 format and directory structure.
 *
 * The root directory of the AWIPS 1 directory structure must be specified by
 * the com.raytheon.rcm.config.awips1.resourceRoot property. It is also
...
public class Awips1ConfigProvider implements ConfigurationProvider {
    ...
    public Configuration getConfiguration() {
        ...
        try {
            f = new File(new File(rootRel(fxaData), "workFiles"),
                    "RadarStorage.StateInfo");
            Scanner s = new Scanner(f);
            try {
                if (s.nextInt() == 0)
    ...
    private void tryAddProdList(StandardProductDistInfoDB db, boolean isTDWR,
            String fileName) {
        try {
            readProdListFile(getIfsResource(fileName), db, isTDWR);
        } catch (Exception e) {
            Log.errorf("Could not load product dist info file '%s': %s",
                    fileName, e);
        }
    }

    public static void readProdListFile(File f, StandardProductDistInfoDB db,
            boolean isTDWR) throws NoSuchElementException,
        ...
        try {
            while (fs.hasNext()) {
                ...
                try {
                    ...
                    Awips1ProdDistInfoBuilder.addProdListEntry(f, messageCode,
                            elev, isTDWR, info, db);
    ...
    private void readNationalRpsLists() {
        ...
        wsrLists[0] = getNatRpsList(GSM.OP_MODE_CLEAR_AIR,
    ...
    private void findRpsLists() {
        ...
        for (String name : radars.keySet()) {
            ...
            RpsList[] nats = nationalRpsLists.get(RcmUtil.getRadarType(rc));
            if (nats != null)
        ...
        try {
            while (fs.hasNext()) {
                ...
                try {
                    ...
                    case 2:
                        /*
                         * TCP dedicated or X.25 dedicated (but the latter is
                         * obsolete)
                         */
    ...
    protected void tryReadOTRFile(String fileName, boolean backup,
            boolean dedicated) {
        try {
    ...
    protected void readOTRFile(File f, boolean backup, boolean dedicated)
        ...
        try {
            while (fs.hasNextLine()) {
                ...
                while (nRpgs-- > 0) {
                     ...
                     */
                    String fullName = getFullName(radarNamelet, linespeed);
                    /* maybe return Radarconfig then.. */
                    if (fullName != null) {
