 ...
 * Jan 13, 2016 5201       dlovely     Allow double digit major version numbers.
 * 
 * </pre>
 * 
 * @author bkowal
...
public class Main {
    ...
    private void scanEclipsePlugin(final String id,
        ...
        if (resourcePaths.isEmpty()) {
            throw new Exception("Failed to find plugin with id: " + id +
                        " required by feature: " + containingBuildFeature + "!");
        }
    ...
    private List<Path> getEclipseResource(final String id, Path containingPath)
            throws Exception {
        final String resourceGlob = id + "_[0-9]*";
        List<Path> resourcePaths = new ArrayList<>(1);
        try (DirectoryStream<Path> directoryStream = Files.newDirectoryStream(
                containingPath, resourceGlob)) {
