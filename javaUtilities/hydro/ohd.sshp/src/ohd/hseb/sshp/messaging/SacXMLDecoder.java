...
public class SacXMLDecoder
{
	...
	private void execute()
	{
	    ...
	    Set<String> filesNotToProcessAgainSet = new HashSet<String>();
		...
		while ( executeProgram )
		{
			...
			int validFileCount = localFileArray.length - filesNotToProcessAgainSet.size();
			
			if (validFileCount > 0)
			{
			    for (File file : localFileArray)
			    {

			        if (! filesNotToProcessAgainSet.contains(file.getAbsoluteFile()))
			        {

			            timer1.start();

			            _logger.log( "Processing " + file.getName() );

			            processFile(builder, file, filesNotToProcessAgainSet);	    

			            _logger.log( "Finished Processing " + file.getName() );

			            timer1.stop( "Finished processing " + file.getName() + " in " );
			        }
			        else
			        {
			            _logger.log( "Skipping " + file.getName() +  "because it could not be moved earlier.  Do not want to process it again." );

			        }

			    } //end for
			}
			else  // validFileCount <= 0  (No files found)  
			{
	...
	private boolean processFile(SAXBuilder builder, File file, Set<String> filesNotToProcessAgain)
	{
	    ...
	    try
        {
            ...
            _doc = builder.build( fullFilePath );

            parseXMLFile();

            moveFile(shortFileName, fullFilePath, filesNotToProcessAgain);
        
        }
        catch ( JDOMException e )
        {
            _logger.log( "     " + fullFilePath + " is not a valid XML file." );
            filesNotToProcessAgain.add(fullFilePath);
            success = false;
        }
        ...
        catch ( IOException e )
        {
            ...
            _logger.log( "     " + fullFilePath + " encountered an IOException -->" );
            e.printStackTrace(_logger.getPrintWriter());
            _logger.log("end IOException message");
	...
	private void moveFile(String shortFileName, String fullFilePath, Set filesUnableToBeMovedSet )
	{
	    
	    File backupInputFile = new File( _archiveDir + "/" + shortFileName);
           
	    File inputFile = new File( fullFilePath );
        boolean successfulRename = inputFile.renameTo( backupInputFile );
        if (successfulRename)
        {
            _logger.log( "Success!  Moved " + fullFilePath + " to " + backupInputFile);
        }
        else
        {
            _logger.log( "Error! - Unable to move " + fullFilePath + " to " + backupInputFile);
            filesUnableToBeMovedSet.add(fullFilePath);
        }
...
//	-----------------------------------------------------------------------------     
	...
	private void processRunoffTimeSeries( Element e )
	{
		...
		if ( process )
		{
			...
			//only save this time series to the database if the duration is correct
			if (descriptor.getDur() == 1001)
			{
