...
public class ProcessDataLimitsDifferencesDialog extends ProcessDifferencesDialog
{
    public ProcessDataLimitsDifferencesDialog( JFrame frame, RaxSyncDataMgr syncDataMgr )
    {
        super( frame, "Process DataLimits", "DataLimits", syncDataMgr, new StringPadder( 25, 35 ), new Dimension( 650, 550 ) );
        _keyString = "(PE | Dur | IDur | MonthDayStart)";
    }
    ...
    protected String getSelectionComboBoxString( RecordDifference recordDiff )
    {
        ...
        RaxDataLimitsRecord record = (RaxDataLimitsRecord) recordDiff.getRaxRecord();
        StringPadder padder = new StringPadder( 4, 8 );

        cbString = PEManager.getPEFromPe1Pe2( record.getPe1(), record.getPe2() ) + 
                   " | " + record.getDur() + " | " + 
                   padder.getFrontPaddedString( record.getIdur() ) + 
    ...
    protected void refreshRecordPanel()
    {
        ...
        if ( _selectedRecordDiff != null )
        {
            ...
            DataLimitsRecord ihfsRecord = (DataLimitsRecord) _selectedRecordDiff.getIhfsRecord();
            RaxDataLimitsRecord raxRecord = (RaxDataLimitsRecord) _selectedRecordDiff.getRaxRecord();
        ...
        {
            _recordPanel = new JListPanel( "", recordStringList, false, Color.GRAY,
                    false, false, null );
            _recordPanel.setBorder( new TitledBorder( new LineBorder( Color.GRAY), headerString ) );
    ...
    public static void main( String[] args )
    {
        ...
        ProcessDataLimitsDifferencesDialog test = new ProcessDataLimitsDifferencesDialog( frame, syncDataMgr );
        test.displayGUI();
    }
