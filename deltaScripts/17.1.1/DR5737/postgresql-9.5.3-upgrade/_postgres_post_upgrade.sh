...
echo -e "\nINFO: Setting up awipsadmin db user"
...
"${PSQL}" --port="${TEMP_PORT}" --db=postgres --user=awipstmp << EOF
    begin;
    alter role awips rename to awipsadmin;
    alter role awipstmp with nologin;
    commit;
EOF
"${PSQL}" --port="${TEMP_PORT}" --db=postgres --user=awipsadmin << EOF
    begin;
    alter role awipsadmin with password 'awips';
    drop role awipstmp;
    commit;
EOF
