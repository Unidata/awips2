...
########################################################################
#
# checkAndUpdateHydroFormatters.py
#
# Description: With DCS22296, several hydro formatter files in WarnGen
# and GFE have been removed. This script will notify you of any overrides
# that exist and ask if you wish to backup and delete them. This script
# will also update any override to warngen/config.xml and remove the
# hydro entries corresponding to the deleted base-level files.
#
# Backups will be stored in /tmp/DCS_22296_YYYYMMDD_HH where
...
backupDir = f"/tmp/DCS22296_{currentTime}"
...
gfeFormattersPath = os.path.join("gfe", "textproducts", "templates", "product")
gfeTextProductsPath = os.path.join("gfe", "userPython", "textProducts")
warnGenFormattersPath = os.path.join("warngen")

# Files to find and delete
gfeFiles = ["Hazard_FFA_MultiPil.py", "Hazard_FFA_MultiPil_Local.py"]
warnGenFileHeaders = ["arealFloodAdvisory", "arealFloodAdvisoryFollowup",
                      "arealFloodWarning", "arealFloodWarningFollowup",
                      "burnScarFlashFloodWarning", "burnScarFlashFloodWarningFollowup",
                      "flashFloodWarning", "flashFloodWarningFollowup",
                      "impactBurnScarFlashFloodWarning", "impactBurnScarFlashFloodWarningFollowup",
                      "impactFlashFloodWarning", "impactFlashFloodWarningFollowup",
                      "impactNonConvectiveFlashFloodWarning", "impactNonConvectiveFlashFloodWarningFollowup",
                      "nonConvectiveFlashFloodWarning", "nonConvectiveFlashFloodWarningFollowup"]

############################
# GFE formatter checking
############################
# Walk through the remaining levels and move files
gfeCommonRootDirs = glob(os.path.join(commonStaticRootDir, "*", "*", gfeFormattersPath))
for gfePath in gfeCommonRootDirs:
    for gfeFile in gfeFiles:
        # Build path to it and its md5 companion
        tmpPath = os.path.join(gfePath, gfeFile)
        md5Path = tmpPath + ".md5"
        # If it exists, ask user if the wish to backup and delete it
        if os.path.exists(tmpPath):
            print("=========================================")
            print(f"{gfeFile} is deleted at the base-level and is no longer used.")
            confirmDelete = confirmMessage(tmpPath)
            if confirmDelete:
                fileBackup =  backupFile(tmpPath, backupDir)
                if fileBackup:
                    deletePath(tmpPath)
                    if os.path.exists(md5Path):
                        deletePath(md5Path)
            else:
               print(f"{tmpPath} not deleted")

gfeCaveRootDirs = glob(os.path.join(caveStaticRootDir, "*", "*", gfeTextProductsPath))
for gfePath in gfeCaveRootDirs:
    tmpPath = os.path.join(gfePath, "Hazard_FFA_*")
    matchingFiles = sorted(glob(tmpPath))
    if matchingFiles:
        print("=========================================")
        print("The Hazard_FFA_* template is deleted at the base-level and is no longer used.")
        confirmDelete = confirmMessage(tmpPath)
        if confirmDelete:
            ...
            print(f"{numMatchingFiles} files associated with Hazard_FFA_* not deleted")
                ...
                if child.tag in ["mainWarngenProducts", "otherWarngenProducts"]:
                    ...
                    for entry in entryList:
                        ...
                        if "/" in entry:
                            ...
                            if fileName in warnGenFileHeaders:
                                print(f" No {fileName} files exist at the base-level, removing this entry from the {child.tag} tag.")
                                modifiedEntry = True
                                continue
