...
# from upgrade_postgis.sh and not by itself.
#
# Author: tgurney
...
get_postgis_ver () {
    "${psql}" --db "$1" -U $db_admin_user -Atc "
        select substring(extversion from 1 for 5) s from pg_extension where extname = 'postgis';
        "
}
...
get_legacy_error_count () {
    "${psql}" --db "$1" -U $db_admin_user -Atc "
        select count(*) c from pg_proc where probin like '%postgis%' and probin not like '%-3%';
        "
}

uninstall_legacy () {
    echo "INFO: Uninstalling legacy functions for $1"
    "${psql}" --db "$1" -U $db_admin_user -f "${postgis_scripts_dir}"/uninstall_legacy.sql >/dev/null 2>&1
    echo "INFO: Done uninstalling legacy functions for "$1""
...
for db in ${databases} ; do
    ...
    if [[ "${has_extension}" == "y" ]]; then
        ...
        if [[ "${ver}" != "${new_postgis_ver_long}" ]]; then
            echo "Upgrading ${db}"
            alter_extensions "${db}"
            ver=$(get_postgis_ver "${db}")
            if [[ "${ver}" != "${new_postgis_ver_long}" ]]; then
