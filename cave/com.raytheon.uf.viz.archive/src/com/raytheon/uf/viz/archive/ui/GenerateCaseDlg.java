 ...
 * ------------- -------- --------- --------------------------------------------
 * Jun 03, 2013  1966     rferrel   Initial creation
 * Aug 16, 2013  2225     rferrel   Change structure of copy to include archive
 *                                  and category directory and implementation of
 *                                  compression.
 ...
 * Apr 03, 2014  2862     rferrel   Logic for shared locking of top level
 *                                  directories.
 * Apr 25, 2018  7025     tgurney   Log failed files and continue copying
...
public class GenerateCaseDlg extends CaveSWTDialog {
    ...
    /**
     * The case's destination directory.
     */
    ...
    private final List<IJobChangeListener> jobListeners = new ArrayList<>();
     ...
     * @param splitSize
     */
    protected GenerateCaseDlg(Shell parentShell, File caseDir,
    ...
    private class GenerateJob extends Job {
         ...
         */
        private static final long LOCK_RETRY_TIME = 2
                * TimeUtil.MILLIS_PER_MINUTE;

        /** Files/directories needing plug-in locks in order to copy. */
        private final Map<CopyInfo, Map<String, List<File>>> caseCopyMap = new HashMap<>();
        ...
        protected IStatus run(IProgressMonitor monitor) {
            ...
            try {
                ...
                for (DisplayData displayData : sourceDataList) {
                    ...
                    List<File> files = archiveManager
                            .getDisplayFiles(displayData, startCal, endCal);
                    ...
                    for (File source : files) {
                        ...
                        if (allowCopy) {
                            ...
                            try {
                                ...
                                statusHandler.warn("Failed to copy "
                ...
                statusHandler.handle(Priority.PROBLEM, e.getLocalizedMessage(),
                        e);
                setStateLbl("Failed to Create: " + caseName,
                        caseDir.getAbsolutePath() + "\n"
                ...
                if (caseCopy != null) {
                    ...
                    if (statusHandler.isPriorityEnabled(Priority.INFO)) {
                        String message = String.format("Case %s took %s.",
                                caseDir.getName(), TimeUtil.prettyDuration(
        ...
        private void waitForLocks()
                throws CaseCreateException, InterruptedException {
            int retryCount = 0;
            ...
            while (caseCopyMap.size() > 0) {
                if (updateStatus) {
                    ...
                    tooltip.append("Waiting to finish ")
                            .append(caseCopyMap.size());
            ...
            if (caseCopyMap.size() > 0) {
                ...
                try {
                    ...
                    while (copyInfoIter.hasNext()) {
                        ...
                        setStateLbl(
                                copyInfo.archive + " | " + copyInfo.category,
    ...
    private static class CopyMove implements ICaseCopy {
        ...
        public void copy(File source) throws CaseCreateException {
            String relativePath = source.getAbsolutePath()
                    .substring(startRelativePath);
    ...
    private static class CompressCopy implements ICaseCopy {
         ...
         */
        protected final HashSet<File> tarDirFile = new HashSet<>();

        /**
         * Buffer to use for reading in a file.
         */
        protected final byte[] buffer = new byte[(int) (32
         ...
         * @throws CaseCreateException
         */
        private void addTarFiles(File[] files)
                ...
                if (file.isDirectory()) {
                    ...
                    if (name.endsWith(hdfExt)) {
                        File destination = new File(
                                ...
                                file.getName());
                        name = destination.getAbsolutePath()
                                .substring(startRelativePath);
                    }
                    TarArchiveEntry entry = new TarArchiveEntry(file, name);
         ...
         */
        protected void checkFit(File file)
                throws IOException, ArchiveException {
        ...
        protected void addParentDir(File file) throws IOException {
            ...
            if (parent != null && !tarDirFile.contains(parent) && (parent
                    .getAbsolutePath().length() > startRelativePath)) {
                addParentDir(parent);
    ...
    private static class CompressAndSplitCopy extends CompressCopy {
         ...
         */
        private static final long BACK_OFF_BYTES = 5 * SizeUtil.BYTES_PER_KB;
        ...
        protected File getTarFile() {
            fileCnt++;
            String name = String.format("%s_%03d%s", categoryDirName, fileCnt,
