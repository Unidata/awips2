 ...
 * Feb 10, 2021  8290     randerso  Add workaround for Eclipse bug causing CAVE
 *                                  crash.
 * Feb 19, 2021  8290     randerso  Fix range check in SetData event handler
 *
 * </pre>
 *
 * @author lvenable
...
public class ArchiveTableComp extends Composite {
    ...
    private static final String CATEGORY_COL_LABEL = "Data Set";

    /** Column label when table displaying select all information. */
    private static final String SEL_ALL_COL_LABEL = "Archive | Category | Data Set";

    /** Column to display label information. */
    private static final int LABEL_COL_INDEX = 0;
     ...
     */
    private final List<IModifyListener> modifiedListeners = new CopyOnWriteArrayList<>();
    ...
    private void createTable() {
        ...
        Rectangle trim = table.computeTrim(0, 0, 0, table.getItemHeight() * 10);
        gd.heightHint = trim.height - trim.y;
        ...
        table.addListener(SWT.SetData, new Listener() {
            ...
            public void handleEvent(Event event) {
                ...
                if (index >= 0 && index < tableData.length) {
                    ...
                    item.setText(
                            new String[] { label, displayData.getSizeLabel() });
    ...
    private void sortColumn(Event e) {
        ...
        int index = eventColumn == table.getColumn(LABEL_COL_INDEX)
                ? LABEL_COL_INDEX
                : SIZE_COL_INDEX;
    ...
    protected void populateTable(String archiveName, String categoryName,
        ...
        showSelectAll = false;
        table.getColumn(LABEL_COL_INDEX).setText(CATEGORY_COL_LABEL);
    ...
    protected void populateSelectAll(List<DisplayData> displayDatas) {
        showSelectAll = true;
        table.getColumn(LABEL_COL_INDEX).setText(SEL_ALL_COL_LABEL);
        populateTable(displayDatas);
    }
    ...
    private void populateTable(List<DisplayData> displayDatas) {
        ...
        TableColumn sizeColumn = table.getColumn(SIZE_COL_INDEX);
        /*
         * Avoid CAVE crash when packing a column when no rows are present
         * https://bugs.eclipse.org/bugs/show_bug.cgi?id=571103
         */
        if (tableData.length > 0) {
            sizeColumn.pack();
        } else {
            sizeColumn.setWidth(0);
        }

        GC gc = new GC(table);
        int w = gc.textExtent(" 9999.9GB").x;
        if (sizeColumn.getWidth() < w) {
            sizeColumn.setWidth(w);
        }
        gc.dispose();

        Rectangle trim = table.computeTrim(0, 0, 0, 0);
        table.getColumn(LABEL_COL_INDEX).setWidth(table.getClientArea().width
                - trim.width + trim.x - sizeColumn.getWidth());
