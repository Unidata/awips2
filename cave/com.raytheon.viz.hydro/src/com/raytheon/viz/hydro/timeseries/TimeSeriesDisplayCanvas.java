 ...
 *                                     also, the flood stage value to the graph
 *                                     when no data available
 * Jul 22, 2010  5967       mpduff     Implemented toggling on/off of traces via
 ...
 *                                     y-axis max was greater than the current
 *                                     y-axis max
 * Jan 26, 2011  5557       bkowal     created a new function that would draw
 *                                     the time series canvas within specified
 *                                     boundaries.
 ...
 *                                     interface is not locked up. Extracted the
 *                                     data retrieval logic from the function
 *                                     that updates the interface with the data
 *                                     retrieved.
 * Feb 07, 2011  6288       lbousaid   Added validGraph flag and findGraph
 *                                     arrayList to list if the graph has trace
 *                                     or no
 * Feb 04, 2011  7758/7759             Added checkbox to pop up menu and added
 *                                     check box selection event to the handler
 * Apr 13, 2011  9006       jpiatt     Corrected getFcstData select statement to
 *                                     have uppercase parameters.
 * Apr 18, 2011  8963       jpiatt     Adjusted call to scaling manager for
 *                                     y-axis.
 * Apr 25, 2011  7759       djingtao   Modify getDataForGraph(), getFcstData()
 *                                     and misc to allow correct graph display
 *                                     when "show latest Forecast only" is on or
 *                                     off
 * Jun 01, 2011  9499       djingtao   update getFcstData()
 * Jun 29, 2011  9669       lbousaidi  removed page flickering during the graph
 *                                     display
 ...
 * Aug 10, 2011  10457      djingtao   allow red rubberband box to be draw for
 *                                     setMissing in Edit
 ...
 *                                     sampling algorithm
 * Nov 06, 2012  15459      lbousaidi  update data when page/up or page/down is
 *                                     pressed without having to click in graph
 *                                     button again.
 * Nov 06, 2012  15400      lbousaidi  Changed logic in buildPointString
 *                                     routine, added discharge2stage to display
 *                                     stage value, also added checking for
 *                                     rating curve for both stage and
 *                                     discharge.
 * Nov 13, 2012  15416      lbousaidi  added a check when the colorname is null
 *                                     and a call to getGroupModeColor
 * Jan 09, 2012  15493      lbousaidi  added code to delete data while zooming
 *                                     when you draw a box
 * Jan 16, 2013  15695      wkwock     Fix popup menu
 * Apr 24, 2013  1921       mpduff     Fix zoom reset to only reset the "active"
 *                                     graph
 * May 06, 2013  1976       mpduff     Refactored Hydro time series data access.
 ...
 * Jul 18, 2017  18467      xwei       Fix IndexOutOfBoundsException error when moving points of a selected trace
 * Jan 04, 2018  6744       mduff      Don't scale Q* PEs to flood stage.
 * Jan 15, 2018  6971       mduff      Set the min/max point data for all points, not only when zoomed out.
 *
 * </pre>
 *
 * @author lvenable
 *
 */
public class TimeSeriesDisplayCanvas extends TimeSeriesGraphCanvas
    ...
    private final TimeSeriesDisplayDlg dialog;
    ...
    private final NumberFormat twoDecimalFormat = new DecimalFormat("0.00");

    /** A reference to this class */
    private final Canvas tsCanvas;
     ...
     */
    private final List<List<Region>> listRegionList = new ArrayList<>();

    /**
     * List of regions for points for each trace
     */
    private final List<List<Region>> pointList = new ArrayList<>();
     ...
     */
    private final List<Integer> setMissingList = new ArrayList<>();
     ...
     */
    private final int[] editPts = new int[6];
    ...
    public TimeSeriesDisplayCanvas(final TimeSeriesDisplayDlg dialog,
            Composite parent, GraphData graphData, Date beginDate, Date endDate,
            boolean groupMode) {
    ...
    public void getDataForGraph() {
        try {
            ...
            // iterating over it
            List<TraceData> iterList = new ArrayList<>();
    ...
    protected void drawCanvas(GC gc) {
            ...
            if (!inDataRetreival) {
                ...
                if ((pageInfoList != null) && (!pageInfoList.isEmpty())) {
                    String title = pageInfoList.get(dialog.getCurrentPage())
                            .getTitle();
                    if (title != null) {
                        gc.drawText(title, 3, fontHeight);
                    }
                }
            } else {
                gc.drawText("Waiting for data . . .", 3, 3, true);
            }
        }
        ...
        try {
            ...
            statusHandler.handle(Priority.ERROR, "Error retrieving river names",
                    e);
            siteLabel = "";
        }
        ...
        if (floodStage == null) {
            ...
            if ("-9999.0".equals(floodStage) || floodStage.isEmpty()
        ...
        if (!hasData) {
            ...
            gc.drawRectangle(graphBorderLeft, graphBorderTop, graphAreaWidth,
                    graphAreaHeight);
            int tmpX = canvasWidth / 2
                    - NO_DATA_AVAILABLE.length() / 2 * fontWidth;
            ...
            gc.drawText(NO_DATA_AVAILABLE, tmpX, tmpY);
            gc.drawText(siteLabel + " fs=" + floodStage, graphBorderLeft,
                    graphBorderTop - fontHeight * 2);
            ...
            setForegroundColor(gc, SWT.COLOR_CYAN);
            if (pointString != null) {
                gc.drawText(pointString, graphBorderLeft - 55, fontHeight * 2);
            }
            ...
            gc.drawText(siteLabel + " fs=" + floodStage, graphBorderLeft - 10,
                    graphBorderTop - fontHeight * 2);

            int index = graphBorderLeft - fontWidth;
            // If labels run off the right of the canvas then need to stack them
            boolean stackLabels = false;

            /*
             * This should start as 2 because the first stack will be above a
             * line
             */
            int stackCount = 2;
            Point labelStart = new Point(0, 0);

            // store the label to be plotted on the gc legend later
            List<String> noDataLabels = new ArrayList<>();
            ...
            for (int j = 0; j < traceArray.size(); j++) {
                ...
                if (j == 0) {
                    ...
                    if ((graphData.getDerivepp() != null)
                            && graphData.getDerivepp()
                                    .equalsIgnoreCase(HydroConstants.OFF)) {
                ...
                if (traceSelected && (selectedTraceId == j)
                    ...
                    gc.drawText("Active Trace:  " + dataString, canvasWidth / 2,
                            graphBorderTop / 2);
                }
                ...
                if (td.getPe().equalsIgnoreCase(HydroConstants.PP)
                    ...
                    List<Region> ppointList = new ArrayList<>();
                    for (int i = 0; i < pointArray.length; i++) {
                        pointArray[i].setPixelX(x2pixel(graphData,
                                pointArray[i].getX().getTime()));
                        if (pointArray[i]
                                .getY() != HydroConstants.MISSING_VALUE) {
                            pointArray[i].setPixely(
                                    y2pixel(graphData, pointArray[i].getY()));
                        } else {
                            pointArray[i].setPixely(bottomBorder - 2);
                        }

                        /* Top left point of bar */
                        int x = pointArray[i].getPixelX() + graphBorderLeft;

                        if ((x < graphBorderLeft)
                                || (x > graphBorderLeft + graphAreaWidth)) {
                            continue;
                        }

                        int x2 = x;
                        x = x2pixel(graphData,
                                pointArray[i].getX().getTime() - 3_600_000)
                                + graphBorderLeft;
                        int y = pointArray[i].getPixelY() + graphBorderTop;
                        ia[0] = x;
                        ia[1] = y;
                        barWidth = x2 - x;
                        if (barWidth < 1) {
                ...
                if (graphData.getTraces().size() > 1) {
                    if (td.getLineData() != null
                            && td.getLineData().length > 0) {
                        if (td.isTraceOn()) {
                            if (stackLabels
                                    || ((dataString.length() * fontWidth)
                                            + index > rightBorder)) {
                                labelStart = getLabelLocation(index, dataString,
                                        stackCount);
                                stackCount++;
                                stackLabels = true;
                            } else {
                                labelStart.x = index;
                                labelStart.y = graphBorderTop - fontHeight;
                            }
                            gc.drawText(dataString, labelStart.x, labelStart.y);
                            if (!stackLabels) {
                                index += (dataString.length() + 2) * fontWidth;
                            }
                        }
                    } else {
                    ...
                    if (graphData.getTraceData(0).getPe().startsWith("Q")) {
                        ...
                        if (graphData.getTraceData(0).getYmax() >= 10_000) {
                            if (td.isTraceOn()) {
                                gc.drawText(
                                        dataString + "  min="
                                        ...
                                        graphBorderLeft + (dataString.length()
                                                * fontWidth * j),
                                        graphBorderTop - fontHeight);
                            }
                        } else {
                            if (td.isTraceOn()) {
                                gc.drawText(dataString + "  min="
            ...
            for (int i = 0; i < noDataLabels.size(); i++) {
                ...
                if (stackLabels || ((labelString.length() * fontWidth) + 50
                        + index > canvasWidth)) {
                    labelStart = getLabelLocation(index, labelString,
                            stackCount);
                    stackCount++;
                    stackLabels = true;
                } else {
                ...
                gc.drawText(labelString, labelStart.x, labelStart.y);
                if (!stackLabels) {
                    index += (labelString.length() + 2) * fontWidth;
                }
            }
            ...
            graphAreaRectangle = new Rectangle(graphBorderLeft, graphBorderTop,
            ...
            if (pe.equalsIgnoreCase(PC) && graphData.getShowpp()) {
                ...
                gc.drawText("DERIVED PP", index, graphBorderTop - fontHeight);
            }
    ...
    private Point getLabelLocation(int index, String dataString,
            int stackCount) {

        return new Point(rightBorder - dataString.length() * fontWidth,
                graphBorderTop - (stackCount * fontHeight));
    }
    ...
    private void findGraphMinMax(GraphData gd) {
        ...
        for (int i = 0; i < gd.getTraces().size(); i++) {
            ...
            if (td != null && td.isTraceOn()) {
                ...
                if (points != null) {
                    ...
                    List<TimeSeriesPoint> pointListZoomed = new ArrayList<>();
                    List<TimeSeriesPoint> pointList = new ArrayList<>();

                    /* Delete the specified point */
                    if ((!deleteList.isEmpty()) && (i == selectedTraceId)) {
                        ...
                        td.setTsData(pointList.toArray(
                                new TimeSeriesPoint[pointList.size()]));
                    ...
                    /* Set missing */
                    if ((!setMissingList.isEmpty()) && (i == selectedTraceId)) {
                        for (int j = 0; j < points.length; j++) {
                    ...
                    if ((insertedPoint != null) && (i == selectedTraceId)) {
                        ...
                        for (int j = 0; j < points.length; j++) {
                            ...
                            if (insertedPoint.getX().before(points[j].getX())) {
                                ...
                                pointList.add(points[j]);
                                pointAdded = true;
                            } else if (points[j].getX()
                                    .before(insertedPoint.getX())) {
                                pointList.add(points[j]);
                                if ((j + 1 < points.length) && points[j + 1]
                                        .getX().after(insertedPoint.getX())) {
                                    pointList.add(insertedPoint);
                                    pointAdded = true;
                                }
                            }
        ...
        if (zoomed) {
            if (!dialog.isZoomAction() && dialog.isSelectZoom()) {
                ...
                Date xMin = pixel2x(gd, rubberBandX1 - graphBorderLeft);
                Date xMax = pixel2x(gd, rubberBandX2 - graphBorderLeft);
        ...
        for (TraceData trace : gd.getTraces()) {
            if (trace.getPe().startsWith("H")
                    || trace.getPe().startsWith("h")) {
                useFloodStage = true;
                break;
            }
        }
    ...
    private ForecastData createPoint(TraceData td, TimeSeriesPoint point) {
        ...
        if (!td.getDur().isEmpty()) {
            data.setDur(TimeSeriesUtil.convertDur2Short(
                    td.getDur().toUpperCase().toCharArray()[0]));
        } else {
            data.setDur(0);
        }
    ...
    private void getData(TraceData td) {
        ...
        List<TimeSeriesPoint> pointsbak = new ArrayList<>();
        lid = td.getLid();
        ...
        } catch (VizException | ClassNotFoundException e) {
        ...
        td.setPreviousTsData(
                pointsbak.toArray(new TimeSeriesPoint[pointsbak.size()]));
    }
    ...
    private void popupMenu() {
        ToggleTimeSeriesDlg ttsd = ToggleTimeSeriesDlg.getInstance(getShell(),
                getTraceList(), this);
        ttsd.open();
    }
    ...
    private String buildPointString(int x, int y) {
        StringBuilder sb = new StringBuilder();
        Date xValue = pixel2x(graphData, x - graphBorderLeft);
    ...
    private void handleMouseMoveEvent(MouseEvent e) {
        if (mouseDown && !pointSelected && !dialog.isDelete()
            ...
            if (mouseDown) {
                ...
                setMissingRect = new Rectangle(setMissingX1, setMissingY1,
                        e.x - setMissingX1, e.y - setMissingY1);
                redraw();
            } else {
                if (precipPE) {
                    List<Region> ppl = precipPointList.get(selectedTraceId);
            ...
            if (pointSelected && mouseDown) {
                ...
                tsCanvas.redraw(graphBorderLeft, fontHeight * 2, extent.x,
                        extent.y, true);
                redraw();
            } else {
     ...
     * 
     */
    private static int getIndex(List<List<Region>> pointList, int x, int y) {
    ...
    private void handleMouseUpEvent(MouseEvent e) {
        ...
        if (dialog.isZoomAction()) {
            ...
            insertedPoint.setX(pixel2x(graphData, e.x - graphBorderLeft));
        ...
        if (createRegions) {
            ...
            for (TraceData td : traceList) {
                if (td.isTraceOn()) {
                    ...
                    List<Region> pointRegionList = new ArrayList<>();
    ...
    private void setPointArray(TraceData td) {
        ...
        List<TimeSeriesPoint> zoomedPointList = new ArrayList<>();

        int[] dataPts = new int[pointArray.length * 2];
        List<Integer> dataPtList = new ArrayList<>();
        int dataIndex = 0;
        int zoomDataIndex = 0;
        ...
        for (int i = 0; i < pointArray.length; i++) {
            if (pointArray[i].getY() != HydroConstants.MISSING_VALUE) {
                if (!zoomed) {
                    dataPtList.add(graphBorderLeft + x2pixel(graphData,
                            pointArray[i].getX().getTime()));
                    pointArray[i].setPixelX(dataPtList.get(dataIndex));
                    dataIndex++;
                    dataPtList.add(graphBorderTop
                            + y2pixel(graphData, pointArray[i].getY()));
                    ...
                    if ((pointArray[i].getX().after(graphData.getXMin())
                            && pointArray[i].getX().before(graphData.getXMax()))
                            && ((pointArray[i].getY() > graphData.getYmin())
                                    && (pointArray[i].getY() < graphData
                                            .getYmax()))) {
                        zoomedPointList.add(pointArray[i]);
                        al.add(graphBorderLeft + x2pixel(graphData,
                                pointArray[i].getX().getTime()));
                        pointArray[i].setPixelX(al.get(zoomDataIndex));
                        zoomDataIndex++;
                        al.add(graphBorderTop
                                + y2pixel(graphData, pointArray[i].getY()));
    ...
    void cancelEdit() {
        TimeSeriesPoint[] tpa = graphData.getTraceData(selectedTraceId)
                .getPreviousTsData();
        if (tpa != null) {
            List<TimeSeriesPoint> origTpa = new ArrayList<>(tpa.length);
    ...
    private int getFcstData(TraceData traceData) {
        ...
        try {
            ...
            String where = String.format(" WHERE lid = '%s' "
                    ...
                    traceData.getTs().toUpperCase(),
                    Integer.parseInt(traceData.getDur()),
                ...
                for (Fcstheight row : results) {
                    ...
                    if ((validTime.getTime() >= beginDate.getTime())
                        ...
                        if (basisTime.getTime() != prevBasisTime.getTime()) {
                            ...
                            ntraces++;
                            traceData.setTsData(points.toArray(
                                    new TimeSeriesPoint[points.size()]));
    ...
    private boolean isFloodLineDisplay() {
        return ((dialog.getBatchDataAndCategoriesMI().getSelection()
                || dialog.getBatchDataOnlyShowCatMI().getSelection())
                || (dialog.getScaleStagesDataOnlyShowCategoreMI().getSelection()
                        || dialog.getScaleStagesDataAndCategoriesMI()
                                .getSelection()));
    }
