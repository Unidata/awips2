	...
	protected void drawYAxis(GC gc, GraphData gd, String label) {
		...
		for (int i = 0; i < numberTicks; i++) {
			...
			if (ratingCurveExist) {
				...
				gc.drawText(dischargeFormat.format(value) + "", labelX,
							labelY, true);			
			}
		...
		if (showPP)
		{
		   for (int i = 0; i < numberTicks2; i++)
		   {
			...
			int[] tick2 = { leftBorder, topBorder + showPPDatay, leftBorder - dx,
					topBorder + showPPDatay };				       
			tick2 = new int[4];
			tick2[0] = rightBorder;
...
//			    }
	...
	private void labelRightAxis(GC gc, String pe, double maxDischarge) {
		...
		if (ratingCurveExist) {
			...
			//no label needed based on A1, just use different tick mark
	...
	protected void displayFloodCatLines(GC gc, GraphData graphData) {
		...
		/* Action stage/flow */
		if ((graphData.getActionStage() >= 0) || (graphData.getActionFlow() >= 0)) {
    		...
    		if (graphData.getTraceData(0).getPe().toUpperCase().startsWith("H")) {
    			...
    			y = GRAPHBORDER
    					+ graphAreaHeight
    					- (lowerAxis - y2pixel(graphData, graphData.getActionFlow()));
    		}
		...
		if ((graphData.getFloodStage() >= 0) || (graphData.getFloodFlow() >= 0)) {
    		...
    		if (graphData.getTraceData(0).getPe().toUpperCase().startsWith("H")) {
    			y = GRAPHBORDER
    					+ graphAreaHeight
    					- (lowerAxis - y2pixel(graphData, graphData.getFloodStage()));
    		} else {
    			y = GRAPHBORDER
    					+ graphAreaHeight
    					- (lowerAxis - y2pixel(graphData, graphData.getFloodFlow()));
    		}
		...
		if ((graphData.getMinorStage() >= 0) || (graphData.getMinorFlow() >= 0)) {
    		...
    		if (graphData.getTraceData(0).getPe().toUpperCase().startsWith("H")) {
    			y = GRAPHBORDER
    					+ graphAreaHeight
    					- (lowerAxis - y2pixel(graphData, graphData.getMinorStage()));
    		} else {
    			y = GRAPHBORDER
    					+ graphAreaHeight
    					- (lowerAxis - y2pixel(graphData, graphData.getMinorFlow()));
    		}
		...
		/* Moderate stage/flow */
		if ((graphData.getModerateStage() >= 0) || (graphData.getModerateFlow() >= 0)) {
    		gc.setForeground(new Color(parentComp.getDisplay(), HydroUtils
    				.getColor("Red")));
		...
		if ((graphData.getMajorStage() >= 0) || (graphData.getMajorFlow() >= 0)) {
    		...
    		if (graphData.getTraceData(0).getPe().toUpperCase().startsWith("H")) {
    			y = GRAPHBORDER
    					+ graphAreaHeight
    					- (lowerAxis - y2pixel(graphData, graphData.getMajorStage()));
    		} else {
    			y = GRAPHBORDER
    					+ graphAreaHeight
    					- (lowerAxis - y2pixel(graphData, graphData.getMajorFlow()));
    		}
	...
	protected void drawPcBars(GC gc, GraphData graphData, TraceData td) {
		...
		//adjust the yminChange and ymaxChange to newyminChange and newymaxChange
		adjust_pcymax();
		...
		for (int i = 0; i < pointArray.length; i++) {
			...
			pointArray[i].setPixelX(x2pixel(graphData, pointArray[i].getX()
					.getTime()));
		//	pointArray[i].setPixely(y2pixel(graphData, pointArray[i].getY()));
		//	pointArray[i + 1].setPixely(y2pixel(graphData,
		//			pointArray[i + 1].getY()));
			
		//	pointArray[i].setPixely(secondaryY2pixel(graphData, pointArray[i].getY()));
