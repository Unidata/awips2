 ...
 * Jan 19, 2011  5281     lbousaidi  make the graph window larger data was too
 *                                   compressed.
 * Jan 24, 2011  7799     bkowal     ensured that "Data Only" would be the
 *                                   default selection under both the Graph -
 *                                   Scale Stages sub-menu and the Options -
 *                                   Batch Scale Stages sub-menu.
 * Jan 26, 2011  5557     bkowal     finished the implementation of and enabled
 *                                   "reverse video" printing. Updated the
 *                                   printing capability so that it would place
 *                                   the entire time series within the printable
 *                                   area of the page.
 * Mar 04, 2011  7644     lbousaid   fixed Zoom in feature
 * May 30, 2012  14967    wkwock     fix insert deleted data to rejecteddata
 *                                   table
 ...
 * Jan 30, 2012  15459    mpduff     Redmine 1560 - Make graph canvases redraw
 *                                   on page up/down.
...
public class TimeSeriesDisplayDlg extends CaveSWTDialog {
                ...
                if (save) {
                    ...
                    if (insertList.size() > 0) {
                        try {
                            ...
                            mb.setMessage(
                                    "An error occured during insert.  Not all points were inserted.");
                            mb.open();
    ...
    private void createDisplayCanvas() {
        ...
        if (groupInfo.isGroupSelected()) {
            ...
            PageInfo[] pageArray = pageList
                    .toArray(new PageInfo[pageList.size()]);
            ...
            for (int i = 0; i < pageArray.length; i++) {
                ...
                for (int j = 0; j < graphArray.length; j++) {
                    ...
                    if (!gd.getShowcat()) {
                        ...
                        String showCat = AppsDefaults.getInstance()
                                .getToken("timeseries_showcat");
    ...
    public void saveCanvas(String filename) {
        ...
        for (int i = 0; i < canvasList.size(); i++) {
            ...
            Image image = new Image(display,
                    canvasList.get(i).getBounds().width,
                    canvasList.get(i).getBounds().height);
            GC gc = new GC(image);
    ...
    private void print() {
        ...
        if (printerData != null) {
            for (int i = 0; i < canvasList.size(); i++) {
                ...
                Display display = canvasList.get(i).getDisplay();
                Image image = new Image(display, printer.getClientArea().height,
                        printer.getClientArea().width);
                GC gc = new GC(image);

                canvasList.get(i).resizeGraph(
                ...
                // an image.
                ImageData destData = this
                        .rotateTimeSeriesImage(image.getImageData());
                image.dispose();
                ...
                canvasList.get(i).resizeGraph(originalBounds);
