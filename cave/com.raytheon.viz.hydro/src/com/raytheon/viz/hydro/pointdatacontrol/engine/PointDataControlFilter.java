...
public class PointDataControlFilter {
     ...
     *            The report list
     */
    public static List<GageData> filterReportsAndAddInfo(List<GageData> repList) {
        ...
        for (int i = 0; i < repList.size(); i++) {
            ...
            if (locpdc != null) {
                ...
                if ((pcOptions.getQueryMode() == QueryMode.AD_HOC_MODE
                    ...
                    for (int k = 0; k < pcOptions.getDataSourceChosenCount(); k++) {
                        if (pcOptions.getDataSourcesChosen()[k]
                                .equalsIgnoreCase(PDCConstants.OBSERVER_STRING)) {
                            if (locpdc.isDcp()) {
                                useReport = true;
                            }
                            ...
                            if (locpdc.isDcp()) {
                                ...
                                .equalsIgnoreCase(PDCConstants.UNDEFINED_STRING)) {
                            if (locpdc.isDcp()
                                    && locpdc.isObserver()
                                    && ((locpdc.getTelemType() != null) && (locpdc
                                            .getTelemType().length() == 0))) {
                                useReport = true;
                            }
                        } else {
                            if (pcOptions.getDataSourcesChosen()[k]
                ...
                if (pcOptions.getFcstptsOnly() == 1) {
                    if (!locpdc.getDispClass().contains(
                            PDCConstants.FCSTPT_STNCLASS)) {
                        useReport = false;
                    }
                }
                ...
                if ((pcOptions.getFilterByHSA() == 1) || !useReport) {
                    if (useReport) {
                        if (!pcOptions.getHsaList().contains(
                                PDCConstants.ALL_AREAS)
                                && !pcOptions.getHsaList().contains(
                                        locpdc.getHsa())) {
                            useReport = false;
                        }
                    }
                ...
                if (isFilterdOutByValue(pcOptions.getValueFilterOperation(),
                        pcOptions.getValueFilterValue(), repList.get(i)
                                .getValue())
                        || !useReport) {
                    useReport = false;
                }
                ...
                if (isFilterdOutByValue(pcOptions.getElevFilterOperation(),
                        pcOptions.getElevFilterValue(), repList.get(i)
                                .getElevation())
                        || !useReport) {
                    useReport = false;
                }
