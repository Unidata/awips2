    ...
    public static List<GageData> processValueOption(List<GageData> repList,
        ...
        if ((valueType == ValueType.TYPE_VALUE.getValueType())
            ...
            for (int i = 0; i < repList.size(); i++) {
                RiverStat riverStat = dataManager.getRiverStatus(repList.get(i)
                        .getLid());
        ...
        if ((valueType == ValueType.TYPE_VALUE.getValueType())
            ...
            if (pcOptions.getSelectedAdHocElementString().equals("HG")) {
                ...
                peType = QR;
            } else if (pcOptions.getSelectedAdHocElementString().startsWith(
                    "Pr")) {
                peType = PRIMARY;
            } else {
                return repList;
            ...
            for (int i = 0; i < repList.size(); i++) {
                ...
                case HG:
                    ...
                    if (repList.get(i).getValue() != PDCConstants.MISSING_VALUE) {
                        flow = RatingUtils.stage2discharge(repList.get(i)
                                .getLid(), repList.get(i).getValue());
                ...
                case QR:
                     ...
                     */
                    if (repList.get(i).getValue() != PDCConstants.MISSING_VALUE) {
                        stage = RatingUtils.discharge2stage(repList.get(i)
                                .getLid(), repList.get(i).getValue());
        ...
        if ((valueType == ValueType.TYPE_DEPART.getValueType())
             ...
             */
            for (int i = 0; i < repList.size(); i++) {
                RiverStat rsInfo = dataManager.getRiverStatus(repList.get(i)
                        .getLid());
