...
/**
 * A system resource which draws the color bar in D2D for resources which have
 * the {@link ColorMapCapability}.
 * 
 * <pre>
 * 
 * SOFTWARE HISTORY
 ...
 * @author mschenke
 */
public class D2DColorBarResource
        extends AbstractVizResource<GenericResourceData, IDescriptor> {

    private static RGB COLOR = ColorFactory.getInstance()
            .getColor(D2DColorBarResource.class.getName() + "Color");
    ...
    protected void initInternal(IGraphicsTarget target) throws VizException {
        colorBarFont = target
                .initializeFont(D2DColorBarResource.class.getName() + "Font");
    ...
    private void paintResource(IGraphicsTarget target,
        ...
        if (rsc.hasCapability(BlendableCapability.class)) {
            ...
            // Assume 2 resources
            if (rl.size() > 0) {
                paintResource(target, paintProps, rl.get(0), ColorBarLoc.LEFT);
            }
            if (rl.size() > 1) {
                paintResource(target, paintProps, rl.get(1), ColorBarLoc.RIGHT);
            }
        }
    }
    ...
    private void paintLeftColorBar(IGraphicsTarget target,
        ...
        ImagingCapability cap = rsc.hasCapability(ImagingCapability.class)
                ? rsc.getCapability(ImagingCapability.class) : null;
    ...
    private void paintLabels(IGraphicsTarget target, PaintProperties paintProps,
            double x1, double x2, double yPos, double zPos,
            List<LabelEntry> labels) throws VizException {
        if (labels != null) {
            List<LabelEntry> modifiedLabels = modifiedMap.get(labels);
            if (modifiedLabels == null) {
                modifiedLabels = new ArrayList<>();
                modifiedMap.put(labels, modifiedLabels);
                List<Integer> numericIndecies = new ArrayList<>();
                ...
                if (numericIndecies.size() < 3
                    ...
                    if (zeroIndex != -1 && numZeros == 1) {
                        ...
                        if (zeroIndex == 0 || zeroIndex == values.size() - 1) {
                            ...
                            j += inc;
                            boolean good = true;
                            for (int count = 2; count < values.size()
                                    && good; count++, j += inc) {
                            ...
                            if (good) {
                                ...
                                if (zeroIndex == 0) {
                                    zeroLabel = new LabelEntry(
                                            s + (prev > 0 ? "" : "-"),
                                            zeroLabel.getLocation());
                                    modifiedLabels.add(zeroLabel);
                                    ++i;
                                    for (int k = 1; k < numericIndecies
                                            .size(); ++k, ++i) {
                                        if (prev > 0) {
                                            modifiedLabels.add(labels.get(
                                                    numericIndecies.get(k)));
                                        } else {
                                            LabelEntry le = labels.get(
                                                    numericIndecies.get(k));
                                            String l = le.getText()
                                                    .substring(1);
                                            modifiedLabels.add(new LabelEntry(l,
                                                    le.getLocation()));
                                        }
                                    }
                                } else {
                                    ...
                                    for (int k = 0; k < numericIndecies.size()
                                            - 1; ++k, ++i) {
                                        if (prev > 0) {
                                            modifiedLabels.add(labels.get(
                                                    numericIndecies.get(k)));
                            ...
                            if (leftSame) {
                                ...
                                if (rightSame && rightPrev != leftPrev) {
                                    ...
                                    LabelEntry zeroLabel = labels.get(
                                            numericIndecies.get(zeroIndex));
            ...
            List<DrawableString> drawables = new ArrayList<>(
                    modifiedLabels.size());
