 ...
 * Aug 02, 2017 6374       njensen     Use common_static instead of cave_static
 * Apr 06, 2018 6718       bsteffen    Track last edit.
 * Apr 12, 2018 6994       bsteffen    Prevent concurrent loading.
 * 
 * </pre>
 * 
 * @author rferrel
...
public class PointsDataManager implements ILocalizationFileObserver {
    ...
    private static final UnitConverter NM_TO_METERS = USCustomary.NAUTICAL_MILE
    ...
    private final LinkedList<PointRequest> requestList;
    ...
    private PointsDataManager() {
        ...
        userCtx = pathMgr.getContext(LocalizationType.COMMON_STATIC,
                LocalizationLevel.USER);

        pointsDir = pathMgr.getLocalizationFile(userCtx,
                AWIPSTOOLS + IPathManager.SEPARATOR + site
                        + IPathManager.SEPARATOR + POINTS_DIR);
        userToolsDir = pathMgr.getLocalizationFile(userCtx,
                AWIPSTOOLS + IPathManager.SEPARATOR + site);
        userToolsDir.addFileUpdatedObserver(this);

        childrenKeyMap = new ConcurrentHashMap<>();
        groupDeleteMap = new ConcurrentHashMap<>();
    ...
    public Collection<String> getVisiblePointNames() {
        Collection<String> visiblePoints = new ArrayList<>();
        for (String name : getPointsMap().keySet()) {
            Point point = points.get(name);
            if (!point.isGroup()
                    && point.getHidden() == PointFieldState.FALSE) {
    ...
    /**
     * Get the name of the point that was most recently changed with this
     * instance of {@link PointsDataManager}.
     * 
     * @return the name of the last edited point.
     */
     ...
     * @param point
     */
    private void storePoint(ILocalizationFile dir, Point point) {
    ...
    private void storePoint(ILocalizationFile dir, Point point,
            String fileName) {
        ILocalizationFile lFile = pathMgr.getLocalizationFile(userCtx,
                dir.getPath().trim() + IPathManager.SEPARATOR + fileName);
    ...
    private Map<String, Point> getPointsMap() {
        ...
        if (points.isEmpty()) {
            ...
            if (d2dPoint != null) {
                if (childrenKeyMap.get(d2dKey).isEmpty()) {
                    createDefaultPoints();
                    processRequests();
                }
            } else {
                ...
                ILocalizationFile d2dDir = pathMgr.getLocalizationFile(userCtx,
                        path);
    ...
    private Coordinate loadHome() {
        ILocalizationFile lFile = pathMgr.getLocalizationFile(
     ...
     */
    private synchronized boolean loadPoints() {
        boolean doRequest = false;
        if (!points.isEmpty()) {
            return doRequest;
        }
        ...
        ILocalizationFile[] files = pathMgr.listFiles(userCtx, path,
    ...
    private void doNodeState(IPointNode node) {
        ...
        while (!children.isEmpty() && unknownCnt < 2) {
     ...
     * @return point
     */
    private Point loadPoint(ILocalizationFile lFile) {
        Point point = null;
        ...
        if (point != null) {
            StringBuilder sb = new StringBuilder(lFile.toString());
     ...
     * @return lFile
     */
    private ILocalizationFile getGroupDir(Point point) {
        String path = (pointsDir.getPath()
                + point.getGroup().replace(' ', PointUtilities.DELIM_CHAR));
        ILocalizationFile dir = pathMgr.getLocalizationFile(userCtx, path);
        return dir;
    }
    ...
    public List<IPointNode> getChildren(IPointNode node, boolean allGroups) {
        ...
        List<IPointNode> children = new ArrayList<>();
        if (childrenKeyList != null) {
            if (allGroups) {
                ...
                for (String key : childrenKeyList) {
                    ...
                    if (!child.isGroup() || !childrenKeyMap
                            .get(getPointKey((Point) child)).isEmpty()) {
                        children.add(child);
                    }
                }
            }
     ...
     * @return name
     */
    private String getPointName(ILocalizationFile lFile) {
        String name = null;
        StringBuilder sb = new StringBuilder();
     ...
     * @return key
     */
    private String getPointKey(ILocalizationFile lFile) {
        String key = null;
        if (!lFile.isDirectory()) {
     ...
     * @return
     */
    public boolean renameGroup(final IPointNode srcNode,
    ...
    private IPointNode createGroup(String parent, String name) {
        ...
        /*
         * lFile.exits() can return true even when directory doesn't exist. Get
         * a list of the parent's sub-directories and see if group's directory
         * is in it.
         */
        ILocalizationFile[] files = pathMgr.listFiles(userCtx, parent,
    ...
    public String[] getGroupList() {
        ILocalizationFile[] files = pathMgr.listFiles(userCtx,
                pointsDir.getPath().trim(), new String[] {}, true, false);
    ...
    public Coordinate getCoordinateOnCircle(Coordinate coor, double radius,
        ...
        return new Coordinate(GC.getDestinationGeographicPoint().getX(),
    ...
    private boolean checkPoint(FileUpdatedMessage message, String fileName) {
        ...
        default:
            statusHandler.handle(Priority.DEBUG,
                    "Unhandled change type: " + message.getChangeType());
        }
        return stateChange;
    }
    ...
    private boolean checkGroup(FileUpdatedMessage message) {
        ...
        case DELETED:
            if (foundGroup != null) {
                if (!childrenKeyMap.get(key).isEmpty()) {
     ...
     * @param fileName
     */
    private void homeLocationFileUpdated() {
        if (home != null) {
            loadHome();

            JobPool pool = new JobPool("Home Location Update",
                    homeListeners.size(), true);
    ...
    private void storePoint(Point point) {
        ILocalizationFile dir = getGroupDir(point);
     ...
     * @throws IOException
     */
    private void marshalPointToXmlFile(Point point, ILocalizationFile lFile)
     ...
     * @throws IOException
     */
    private Point unmarshalPointFromXmlFile(ILocalizationFile lFile)
            throws LocalizationException, IOException {
