...
public class QCTStations {
    ...
    private void qcTLoop(int j, int k, List<Station> temperature_stations,
            int numTstations) {
        ...
        for (m = 0; m < max_tstations; m++) {
            ...
            for (ii = 0; ii < mpe_dqc_max_temp_neighbors; ii++) {
                ...
                if (k == 4) {
                    temp = (float) ((DailyQcUtils.tdata[j].tstn[i].tlevel2[k].data
                            + (temperature_stations.get(m).max[isom]
                ...
                if (k == 5) {
                    temp = (float) ((DailyQcUtils.tdata[j].tstn[i].tlevel2[k].data
                            + (temperature_stations.get(m).min[isom]
            ...
            double fvalu = fdata / fdist;
            DailyQcUtils.tdata[j].tstn[m].tlevel2[k].estimate = (short) fvalu;

            fdif = Math
                    .abs(DailyQcUtils.tdata[j].tstn[m].tlevel2[k].data - fvalu);
    ...
    private void qcT6Loop(int j, int k, List<Station> temperature_stations,
            int numTstations) {
        int isom = DailyQcUtils.isom;
        int mpe_dqc_max_temp_neighbors = DailyQcUtils.mpe_dqc_max_temp_neighbors;
        int max_tstations = numTstations;
        ...
        for (int m = 0; m < max_tstations; m++) {
            ...
            for (int ii = 0; ii < mpe_dqc_max_temp_neighbors; ii++) {

                int i = temperature_stations.get(m).index[ii];
                ...
                temp = (float) (DailyQcUtils.tdata[j].tstn[i].tlevel2[k].a
                        * dist);
            ...
            b = a * (DailyQcUtils.tdata[j].tstn[m].tlevel2[4].data
