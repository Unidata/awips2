 ...
 * May 23, 2018 7312       smanoj      Fixed netCDF file permissions issues.
 * 
 * </pre>
 * 
 * @author snaples
...
public class WriteDQCNetCDFGrids {
    private static final transient IUFStatusHandler statusHandler = UFStatus
            .getHandler(WriteDQCNetCDFGrids.class);
    ...
    public void write_dqc_netcdf_grids(String fname_nc, int pnum,
        ...
        if (gcount == 1) {
            ...
            if (dqc_lonorigin.length() != 0) {
                try {
                    ...
                    statusHandler.handle(Priority.ERROR,
                            "Error parsing token for mpe_dqc_lonorigin. ", e);
                }
            } else {
                lonOrigin[0] = 0;
            }
            ...
            try {
                ...
                statusHandler.handle(Priority.ERROR,
                        "Error in write_dqc_netcdf_grids: Error creating netCDF file",
                        e);
                return;
            } catch (Throwable e) {
                statusHandler.handle(Priority.ERROR,
                        "Error creating new netCDF file. ", e);
            }
            ...
            cdf_grid_id.addAttribute(
                    new Attribute("projectionType", ga.projectionType));

            cdf_grid_id.addAttribute(
                    new Attribute("descriptiveName", ga.descriptiveName));
            ...
            cdf_grid_id
                    .addAttribute(new Attribute("rateDependent", data_ratedep));
            ...
            try {
                ...
                statusHandler.handle(Priority.ERROR,
                        "ERROR creating file " + ncfile.getLocation(), e);
            }
        ...
        if (gcount == num_period_qc) {
            ...
            try {
                ...
                // Open up the write permission of the netCDF files for the
                // GROUP.
                File gridfile = new File(fname_nc);
                if (!(Files.getPosixFilePermissions(gridfile.toPath()))
                        .equals(FilePermissionHelper.POSIX_FILE_SET)) {
                    FilePermissionHelper.applyFilePermissions(gridfile.toPath(),
                            FilePermissionHelper.POSIX_FILE_SET);
                }

            } catch (InvalidRangeException e) {
                statusHandler.handle(Priority.ERROR,
                        "ERROR: in write_dqc_netcdf_grids: InvalidRangeException in writing cdf_grid. "
                                + e.getMessage(),
                        e);
            } catch (IOException e) {
                statusHandler.handle(Priority.ERROR,
                        "Error redefining netCDF file, Exiting.... "
                                + e.getMessage(),
                        e);
                return;
            }
