 ...
 * May 10, 2018  7184      bkowal      Initialize the data quality to Verified instead of Screened.
 * Nov 12, 2018  DR# 20672 jlinahan    Fix for MPE froze and gives a UELE
 * 
 * </pre>
 * 
 * @author snaples
            ...
            try {
                ...
                bad: while (in.ready()) {
                    ...
                    for (k = startk; k < maxk; k++) {
                        ...
                        // Initialize the quality flag to 8 (Verified)
                        dqc.pdata[i].stn[j].frain[k].qual = 8;
                        /*
                         * TODO: why? the qual code is set based on the
                         * information in the level 2 file below. Is there some
                         * unreported parsing error that prevents the data from
                         * being read properly and thus the quality flag is
                         * never updated to something other than 0 (Screened)?
                         * Ideally, in the future these text files that the data
                         * is read from will be replaced by a database that will
                         * make it possible to track when data was changed, was
                         * it a user edit, a system edit, and allow for merging
                         * of user edits and system edits, etc.
                         */
                        ...
                        if (buf.indexOf('.') < 0
                            ...
                            if (p < 0) {
                                ...
                                if (p+3 >= buf.length()){
                                    System.out.println("Record " + q + " is missing characters after decimal point");
                                    val = buf;
                                } else {
                                    ...
                                    if (auto_dailyqc_flag == false) {
                                        ...
                                        if (dqc.pdata[i].stn[j].sflag[k] != 1) {
                                            ...
                                            if (qual != 5 && qual != 6
                                                ...
                                                System.out.println(
                                                        "Data set bad level 1 overwriting level 2 "
                                                                + precip_stations
                                                                        .get(j).hb5
                                                                + ", "
                                                                + precip_stations
