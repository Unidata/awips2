...
public class CollaborationPaneManager extends PaneManager {
    ...
    public void initializeComponents(IDisplayPaneContainer container,
        ...
        gl.marginWidth = 0;
        comp.setLayout(gl);
        noDisplayLabel = new Label(comp, SWT.NONE);
        noDisplayLabel
                .setText("The session leader is not viewing a shared display.");
        noDisplayLabel.setLayoutData(new GridData(SWT.CENTER, SWT.CENTER, true,
                true));
        composite.layout();
    }
    ...
    public IDisplayPane activateDisplay(int displayId,
        ...
        if (activeData != null) {
            ...
            noDisplayLabel.setVisible(false);
            ((GridData) noDisplayLabel.getLayoutData()).exclude = true;
        }
        setExclude(data, false);
        activeData = data;
        composite.layout();
        activatedPane = currentMouseHoverPane = activeData.pane;
        displayPanes.clear();
        displayPanes.add((VizDisplayPane) activatedPane);
        return activeData.pane;
    }

    public void deactivateDisplays() {
        if (activeData != null) {
            setExclude(activeData, true);
            activeData = null;
        }
