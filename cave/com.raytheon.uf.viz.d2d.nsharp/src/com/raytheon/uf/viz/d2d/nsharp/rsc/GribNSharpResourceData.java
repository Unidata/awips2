...
public class GribNSharpResourceData extends D2DNSharpResourceData {
    ...
    private static final transient IUFStatusHandler statusHandler = UFStatus.getHandler(GribNSharpResourceData.class);
    ...
    protected NcSoundingCube getSoundingCube(NsharpStationInfo stnInfo) {
        ...
        NcSoundingCube cube = NcSoundingQuery.mdlSoundingQueryByLatLon(refTimeStr, validTimeStr, latLon, "grid",
                getSoundingType(), false, "-1", gridInterpolation);
            ...
            if (profileList.getStationElevation() == NcSoundingProfile.MISSING) {
                if (surfaceElevation == NcSoundingProfile.MISSING) {
                    try {
                        ...
                        surfaceElevation = (float) topoQuery.getHeight(coordinate);
                        // Set missing data points to 0
