...
public abstract class AbstractGraph implements IGraph {
    ...
    protected static void normalizeAxis(double[] minMax) {
        ...
        double inc = Math.abs(minMax[1] - minMax[0]) / 10;
        if (inc < 1.25) {
            // use 1.5 increments at least
            inc = 1.5;
        } else {
            // otherwise calculate based on 5's
            inc = (Math.ceil(((inc + .5) / 5)) * 5);
        }
        // how many axes are between the min/max?
        int axes = (int) Math.round(Math.abs(minMax[1] - minMax[0]) / inc);
        int axesToX = (11 - axes) / 4 * 2;
        double val = Math.floor(minMax[0] / inc) * inc;
        double newMin = val - ((axesToX) * inc);
        double newMax = newMin + (10 * inc);
        // Never cross the zero line.
        if (newMin < 0 && minMax[0] > 0) {
            newMax -= newMin;
            newMin = 0;
        }
        if (newMax > 0 && minMax[1] < 0) {
            newMin -= newMax;
            newMax = 0;
        }
        minMax[0] = newMin;
    ...
    /**
     * Given a distance determine some incrmeent that evenly divides the space
     * into roundish numbers
     * 
     * @param distance
     * @return
     */
    protected static double determineIncrement(double distance) {
        double inc = Math.pow(10, (int) Math.log10(distance));
        if (distance / inc > 4.5) {
            return inc;
    ...
    protected void paintDataTimeUnits(IGraphicsTarget target,
        ...
        for (IGraphLabel<DataTime> xLabel : xLabels) {
            ...
            if (xLabel.getResource().getProperties().isVisible() == false) {
                continue;
            }
