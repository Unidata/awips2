...
public class SiteMonitor implements IRequestCompleteListener<Map<?, ?>> {
    ...
    /**
     * For DR14717: check if Metar is outdated and turn indicator labels gray
     * when Metar is outdated
     */
    ...
    protected Map<String, MonitorToolTip> tooltipMap = new HashMap<>();
    ...
    private Map<String, String> alertTimeMap;

    public Map<String, String[]> tempoMap;
     ...
     */
    private static final Map<String, String[]> ALERT_KEY_MAP = new HashMap<>();
    ...
    public SiteMonitor(Composite parent, TafSiteComp parentSiteComp,
            ...
            Map<String, String> alertTimeMap/* DR 14570 */,
    ...
    private void initMonitorLabels(String monitorLabelsStr) {
        ...
        if (monitorLabelsStr != null) {
             ...
             * present.
             */
            int minArraySize = (tmpLabels.length < monitorLabels.length)
                    ? tmpLabels.length : monitorLabels.length;
    ...
    private void cleanupMsg(String text, StringBuilder mtrtafinfo,
        ...
        if (text != null) {
            if (text.indexOf(":METAR:") > 0) {
                 ...
                 * the same metar/taf info line. Keep one copy of it and strip
                 * it off of each category entry.
                 */
                for (String lne : text.split("\n")) {
                    ...
                    if (metarPos > 0) {
                        ...
                        if (!fltcat) {
                            fltcat = true;
                            mtrtafinfo
                                    .append(lne.substring(metarPos + 1).trim())
                                    .append("\n");
                        }
                    } else {
            ...
            if (obj != null) {
                ...
                if (thisMonitor.equals("MetarMonitor")) {
                    ...
                    if (mtrdcdMap != null) {
                        Map<?, ?> mtrItimeMap = (Map<?, ?>) mtrdcdMap
                                .get("itime");
                        parentSiteComp.setLatestMtrTime(
                                ((Number) mtrItimeMap.get("value")).longValue()
                                        * 1000);
                    }
                }
                ...
                for (Object key : keys) {
                    ...
                    if (label != null && !label.isDisposed()) {
                        ...
                        int severity = ((Number) valueMap.get("severity"))
                                .intValue();
                        ...
                        /**
                         * DR14717: Metar monitor indicators should turn gray
                         * when Metar is outdated
                         */
                        if (parentSiteComp.getLatestMtrTime() > 0) {
                            if ((currentTime > (parentSiteComp
                                    .getLatestMtrTime()
                                    + TafSiteComp.METAR_TIMEOUT_4HR10MIN))
                                    && (thisMonitor.equals("MetarMonitor")
                                            || thisMonitor.equals(
                                                    "PersistMonitor"))) {
                                /**
                                 * both Current observation monitoring
                                 * indicators and persistence indicators should
                                 * turn gray
                                 */
                                GRAY_LABEL = true;
                                msg = "METAR outdated";
                            } else if ((currentTime > (parentSiteComp
                                    .getLatestMtrTime()
                                    + TafSiteComp.METAR_TIMEOUT_2HR))
                        ...
                        if ((parentSiteComp.getLatestMtrTime() < 0)
                                && thisMonitor.equals("PersistMonitor")) {
                            parentSiteComp
                                    .setPersistMonitorProcessedFirst(true);
                        }
                        ...
                        if (alertMap != null && severity >= 2
                            ...
                            List<String> tempoAlertValues = new ArrayList<>();// 20120711
                            // DR 14570: based on A1 Python code in
                            // TafViewer.highlight()
                            long tsys = SimulatedTime.getSystemTime().getTime()
                                    .getTime();
                            long tfrom = ((Number) ((Map<?, ?>) ((Map<?, ?>) oncl
                                    .get("prev")).get("time")).get("from"))
                                            .longValue()
                                    * 1000;
                            ...
                            for (Map<?, ?> map : group) {
                                tto = ((Number) ((Map<?, ?>) ((Map<?, ?>) map
                                        .get("prev")).get("time")).get("to"))
                                                .longValue()
                                        * 1000;
                                if (time < tto) {
                                    // for TEMPO highlight
                                    String[] keyArray = map.keySet()
                                            .toArray(new String[] {});
                                    for (String s : keyArray) {
                                        if (s.equals("ocnl")) {
                                            long oFrom = ((Number) ((Map<?, ?>) ((Map<?, ?>) map
                                                    .get("ocnl")).get("time"))
                                                            .get("from"))
                                                                    .longValue()
                                                    * 1000;
                                            long oTo = ((Number) ((Map<?, ?>) ((Map<?, ?>) map
                                                    .get("ocnl")).get("time"))
                                                            .get("to"))
                                                                    .longValue()
                                                    * 1000;
                                    ...
                                    obs = (Map<?, ?>) map.get("prev");
                                    break;
                                }
                            }

                            // for getting correct line using time
                            Map<?, ?> obsTimeMap = (Map<?, ?>) obs.get("time");

                            for (String tafKey : tafKeys) {
                                // DR 14570 20120711
                                Map<?, ?> tempoAlert = (tempo == null) ? null
                                        : (Map<?, ?>) tempo.get(tafKey);
                                if (tempoAlert != null) {
                                    tempoAlertValues.add(
                                            (String) tempoAlert.get("str"));
                                } // END 20120711
                            ...
                            tempoMap.put((String) key, tempoAlertValues.toArray(
                                    new String[tempoAlertValues.size()]));// 20120711
                            if (alertTimeMap != null) {
                                // DR 14570
                                alertTimeMap.put((String) key,
                                        (String) obsTimeMap.get("str"));
                            }
                ...
                if (this.getMonitorClassName().equals("MetarMonitor")) {
                    ...
                    long tafTime = ((Number) tafItimeMap.get("value"))
                            .longValue() * 1000;
                    parentSiteComp.setTafTime(tafTime,
                            tafItimeMap.get("str").toString());
                    ...
                    if (mtrdcdMap != null) {
                        Map<?, ?> mtrItimeMap = (Map<?, ?>) mtrdcdMap
                                .get("itime");
                        long mtrTime = ((Number) mtrItimeMap.get("value"))
                                .longValue() * 1000;
