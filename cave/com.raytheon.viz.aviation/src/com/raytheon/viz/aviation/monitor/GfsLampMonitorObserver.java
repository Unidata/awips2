...
import com.raytheon.viz.aviation.guidance.TugViewer;
import com.raytheon.viz.aviation.guidance.ViewerTab;
import com.raytheon.viz.aviation.observer.TafMonitorDlg;

/**
 * Watches incoming GFSLAMP (bufrmosLAMP) alerts to see if they apply to any
 ...
 * @version 1.0
 */
public class GfsLampMonitorObserver extends MonitorObserver implements
        IAlertObserver {

    public static String pluginName = "bufrmosLAMP";

    public GfsLampMonitorObserver(TafMonitorDlg dlg) {
        super(dlg);
    }
    ...
    public void alertArrived(Collection<AlertMessage> alertMessages) {
        ...
        if (siteIDs.size() > 0) {
            for (ViewerTab tab : dialog.getViewerTabList()) {
                if (tab instanceof com.raytheon.viz.aviation.guidance.TugViewer) {
                    TugViewer tug = (TugViewer) tab;
                    if ("gfslamp".equals(tug.getModel())) {
                        tab.alertSites(siteIDs);
                    }
                } else if (tab instanceof com.raytheon.viz.aviation.guidance.MosViewer) {
                    MosViewer mos = (MosViewer) tab;
                    if ("gfslamp".equals(mos.getModel())) {
                        tab.alertSites(siteIDs);
                    }
                }
            }
