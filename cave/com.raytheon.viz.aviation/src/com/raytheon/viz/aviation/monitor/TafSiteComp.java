 ...
 * Apr 29, 2019 7591        tgurney     NotifyAudioManager.playFile() changed
 *                                      signature
 * </pre>
 *
 * @author lvenable
...
public class TafSiteComp {
     ...
     */
    private static final long METAR_TIMEOUT = (1L * 60L + 5L) * 60L * 1000L;

    /**
     * DR14717: When Metar is 2 hours old, the persistence indicators turn gray.
     * This is the timeout in milliseconds for 2 hours.
     */
    public static final long METAR_TIMEOUT_2HR = 2L * 60L * 60L * 1000L;
     ...
     * plus 10 minutes.
     */
    public static final long METAR_TIMEOUT_4HR10MIN = (4L * 60L + 10L) * 60L
            * 1000L;
     ...
     * minute grace period.
     */
    private static final long TAF_TIMEOUT = (6L * 60L + 40L) * 60L * 1000L;
    ...
    private void createTafMetarTimeComp(ResourceConfigMgr configMgr) {
        ...
        Label tafLbl = new Label(tafMtrComp, SWT.NONE);
        tafLbl.setText("TAF");
        ...
        Label mtrLbl = new Label(tafMtrComp, SWT.NONE);
        mtrLbl.setText("MTR");
    ...
    private void createEditorButtonsComp(ResourceConfigMgr configMgr) {
        ...
        Button amdBtn = new Button(btnStatusComp, SWT.PUSH);
        ...
        gd.minimumWidth = btnMinWidth;
        // gd.widthHint = btnMinWidth; // DR 15606
        Button rtdBtn = new Button(btnStatusComp, SWT.PUSH);
        ...
        gd.minimumWidth = btnMinWidth;
        // gd.widthHint = btnMinWidth; // DR 15606
        Button corBtn = new Button(btnStatusComp, SWT.PUSH);
    ...
    public void tafArrived(TafRecord taf, boolean doBlinkCheck) {
        ...
        if (taf == null) {
            VizApp.runAsync(() -> {
                if (!siteIdBtn.isDisposed() && !tafTimeLbl.isDisposed()) {
        ...
        if (updatable) {
            for (int i = 0; i < monitorArray.size(); i++) {
                ...
                if (m.getMonitorClassName().equals(monitor)) {
                    ...
                    if ("MetarMonitor".equals(monitor)
                            && stationName
                                    .equals(tveDlg.getCurrentViewerStation())
                            && metarMontiorIndex >= 0) {
    ...
    public void setTafTime(long tafTime, String timestamp) {
        if (!tafTimeLbl.isDisposed()) {
            ...
            if (!"HH:MM".equals(mtrTimeLbl.getText())) {
    ...
    public void setMetarTime(long metarTime, String timestamp) {
        if (!mtrTimeLbl.isDisposed()) {
            ...
            if (!"HH:MM".equals(tafTimeLbl.getText())) {
                siteIdBtn.setBackground(getBackgroundColor());
            }
        }
    }
    ...
    public void updateSeverity(int severity) {
        ...
        List<?> raiseValues = Arrays
                .asList(configMgr.getComboValues(ResourceTag.NotifyRaise));
        int index2 = raiseValues.indexOf(
                configMgr.getResourceAsString(ResourceTag.NotifyRaise));

        List<?> playValues = Arrays
                .asList(configMgr.getComboValues(ResourceTag.NotifyPlay));
        int index3 = playValues
                .indexOf(configMgr.getResourceAsString(ResourceTag.NotifyPlay));
        if (!parent.getShell().isFocusControl() && 0 < index2
                && index2 < severity) {
            setActive();
        }
        ...
        if (0 < index3 && index3 < severity) {
            ...
            if (soundFile != null && !"None".equals(soundFile)) {
                try {
                    statusHandler.debug("playFile: play");
                    NotifyAudioManager.getInstance().playFile(soundFile);
                } catch (Exception e) {
                    statusHandler.debug("Failed to play sound", e);
    ...
    private void setActive() {
        ...
        statusHandler.debug("setActive() " + fmat.format(date) + " "
                + avnMonitor.getText());
    ...
    class WaitForMonitors extends Job {
        ...
        protected IStatus run(IProgressMonitor monitor) {
            ...
            VizApp.runAsync(() -> tveDlg.updateSettings(TafSettings.UPDATE_VIEW,
                    stationName));
