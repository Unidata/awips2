 ...
 *
 */
public class MosViewer extends ViewerTab
        implements IRequestCompleteListener<String[]> {

    /**
     * Tool tip for values in the VIS/CVIS rows.
     */
    private static final Map<String, String> visDict = new HashMap<>();
    static {
        visDict.put("1", "<1/2SM");
     ...
     */
    private static final Map<String, String> cigDict = new HashMap<>();
    static {
        cigDict.put("1", "<200ft");
     ...
     */
    private static final Map<String, String> qpfDict = new HashMap<>();
    static {
        qpfDict.put("1", "<0.1\"");
    ...
    protected void createTopControls(ResourceConfigMgr configMgr) {
        ...
        routineChk.setSelection(
                configMgr.getResourceAsBoolean(ResourceTag.ShowRoutine));
        ...
        probabilitiesChk.setSelection(
                configMgr.getResourceAsBoolean(ResourceTag.ShowProbs));
    ...
    public void requestComplete(String[] newGuidance) {
        ...
        if (!tableRdo.isDisposed() && tableRdo.getSelection()) {
            genToolTips = true;
            if (!probabilitiesChk.isDisposed()
                    && !probabilitiesChk.getSelection()) {
                removeProbs = true;
            }
        }
    ...
    private void generateToolTips() {
        ...
        List<HeaderTextToolTip> tipsList = new ArrayList<>();
        int offset = 0;
        int end = -1;
        ...
        while (offset >= 0 && offset < text.length()) {
            ...
            if (line.startsWith("VIS") || line.startsWith("CVIS")) {
                ...
                if ("VIS".equals(lastCategory)) {
                    tip = visDict.get(index);
                    if (tip == null) {
                        tip = visDefault;
                    }
                    tipsList.add(
                            new HeaderTextToolTip(offset, offset + 5, tip));
                } else if ("CIG".equals(lastCategory)) {
                    tip = cigDict.get(index);
                    if (tip == null) {
                        tip = cigDefault;
                    }
    ...
    public int generateGuidance(String siteID) {
        ...
        if (allChk.getSelection() && tableRdo.getSelection()) {
            msgStatComp.setMessageText(
                    "Table display for all sites is not supported",
                    allChk.getDisplay().getSystemColor(SWT.COLOR_RED).getRGB());
            return cnt;
        }
        List<String> siteIDs;
        ...
        for (String sID : siteIDs) {
            ...
            byte[] siteObj = getCacheSiteObj(sID, sIDtag);
            if (siteObj == null) {
