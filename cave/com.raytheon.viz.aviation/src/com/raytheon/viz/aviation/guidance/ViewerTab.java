...
public abstract class ViewerTab extends Composite {
     ...
     */
    public abstract void generateCache(List<String> siteIDs);
    ...
    public void queueCacheRequests(final int cnt,
        ...
        Thread thread = new Thread(() -> {
            PythonGuidanceJob.getInstance().waitForCacheRequests(cacheRequests);
            // Update tab if still current and waiting for this request
            if (!ViewerTab.this.isDisposed() && isCurrentTab()
                    && generateGuidanceCount.get() == cnt) {
                VizApp.runAsync(() -> generateGuidance(siteID));
            }
        });
        thread.start();
    }
     ...
     */
    public byte[] getCacheSiteObj(String siteID, String tag) {
