 ...
 * Mar 27, 2008  1033     grichard  Changed hours to num hours.
 * Apr 10, 2008  934      grichard  Populated site lists with icaos.
 * Jun 18, 2008  1119     lvenable  Updated for wind rose drawing.
 ...
 * Jul 23, 2012  15169    zhao      Use Combo for 'Month' and 'Number of Months'
 *                                  & disabled site controls while drawing
 * Oct 04, 2012  1229     rferrel   Changes for non-blocking WindRoseConfigDlg.
 * Oct 08, 2012  1229     rferrel   Made non-blocking.
 * Oct 15, 2012  1229     rferrel   Changes for non-blocking HelpUsageDlg.
 * Dec 03, 2013  16754    zhao      Modified printImage()
...
public class WindRosePlotDlg extends CaveSWTDialog {
    private static final IUFStatusHandler statusHandler = UFStatus
            .getHandler(WindRosePlotDlg.class);
        ...
        configureMI.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent event) {
                if (configDlg == null || configDlg.getShell() == null
                    ...
                    configDlg = new WindRoseConfigDlg(shell,
                            windRoseConfigData);
                    configDlg.open();
                } else {
    ...
    private void createSiteControls(Composite parentComp) {
        ...
        for (int i = 1; i <= 12; i++) {
            monthCbo.add(Integer.toString(i));
        }
        ...
        for (int i = 1; i <= 12; i++) {
            numMonthsCbo.add(Integer.toString(i));
        }
    ...
    public void disposed() {
        WindRoseDataMgr.getInstance().cancelPythonTask();
        if (waitCursor != null) {
            waitCursor.dispose();
        }
        super.disposed();
    ...
    private void generateWindRoseHeader() {
        ...
        if (numMonthsCbo.getSelectionIndex() == 0) {
            ...
            if (numMonthsCbo.getSelectionIndex()
                    + monthCbo.getSelectionIndex() > 11) {
        ...
        if (flightCatCbo.getItem(flightCatCbo.getSelectionIndex())
                .compareTo("All") != 0) {
            header.append(
                    flightCatCbo.getItem(flightCatCbo.getSelectionIndex()));
    ...
    private void printImage() {
        ...
        if (printerData != null) {
            ...
            int destX = (screenDPI.x * bounds.width
                    - printerDPI.x * imageData.width) / screenDPI.x / 2;
            if (destX < 0) {
                destX = 0;
            }
            int destY = (screenDPI.x * bounds.height
                    - printerDPI.x * imageData.height) / screenDPI.x * 80 / 100
                    / 2;
            if (destY < 0) {
                destY = 0;
            }
    ...
    private void saveKml() {
        ...
        try {
            ...
            if (!"".equals(image_name)) {
                ...
                kmlFilePath = file.getParent() + "/" + station
                        + "_Wind_Rose.kml";
                ...
                sb.append("  <Document>\n");
                sb.append(
                        "    <name>" + station + " Wind Rose Overlay</name>\n");
                ...
                try (FileWriter writer = new FileWriter(file);
                    ...
                    statusHandler.error(
                            "Error writing to kml file: " + kmlFilePath, e);
                }
            }
        } catch (ConfigurationException | IOException e) {
            statusHandler.error("Error loading configuration", e);
