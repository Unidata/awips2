 ...
 * Jul 29, 2008  1342     grichard  Get calendar instance in UTC time zone.
 * Sep 12, 2008  1444     grichard  Accommodate separate message logs.
 * Nov 18, 2010  6701     rferrel   Now uses the same wxPlotCfg as
 *                                  WeatherPlotDataManager.
 * Apr 28, 2011  8065     rferrel   Use cache data.
 * Oct 02, 2012  1229     rferrel   Made dialog non-blocking.
 * Oct 10, 2012  1229     rferrel   Changes for non-blocking TimeSelectorDlg.
 * Oct 15, 2012  1229     rferrel   Changes for non-blocking HelpUsageDlg.
 * Nov 26, 2012  1298     rferrel   Non-blocking dialog code cleanup.
 * Aug 12, 2013  2256     lvenable  Disposed of masterImage.
 * Mar 19, 2014  2925     lvenable  Added dispose checks for runAsync.
 * Mar 11, 2020  8044     randerso  Save configuration to USER level instead of
 *                                  wherever it was found to avoid attempting to
 *                                  write to BASE level. Code cleanup.
 *
 * </pre>
 *
 * @author lvenable
...
public class WeatherPlotDialog extends CaveSWTDialog {
     ...
     * Formatted UTC.
     */
    private SimpleDateFormat sdfUTC = new SimpleDateFormat(
     ...
     */
    private static final String[] FILTER_NAMES = { "Bitmap (*.bmp)",
            "JPEG (*.jpg)", "PNG (*.png)" };

    /**
     * Array of image filters.
     */
    private static final String[] FILTER_EXTS = { "*.bmp", "*.jpg", "*.png" };
    ...
    private void createTopControls(ResourceConfigMgr configMgr) {
        ...
        zoomSpinner.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                scrolledComp.setMinSize(
                        graphComp.computeSize(SWT.DEFAULT, SWT.DEFAULT));
                graphComp.layout();
            }
        });
        ...
        Button displayBtn = new Button(controlsComp, SWT.PUSH);
        ...
        // Times button
        gd = new GridData(buttonWidth, SWT.DEFAULT);
        Button timesBtn = new Button(controlsComp, SWT.PUSH);
        ...
        timesBtn.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent event) {
                if (timeDlg == null) {
                    ...
                    timeDlg.addCloseCallback(new ICloseCallback() {
        ...
        // Print button
        gd = new GridData(buttonWidth, SWT.DEFAULT);
        Button printBtn = new Button(controlsComp, SWT.PUSH);
        ...
        // Close button
        gd = new GridData(buttonWidth, SWT.DEFAULT);
        Button closeBtn = new Button(controlsComp, SWT.PUSH);
        ...
        // Help button
        gd = new GridData(buttonWidth, SWT.DEFAULT);
        Button helpBtn = new Button(controlsComp, SWT.PUSH);
    ...
    private void saveImage() {
        ...
        gc.drawText("Visibility", 513,
                (20 - fontHeight) / 2 + 1 + cigCanvasComp.getHeight() + 20);
    ...
    private void createDataSourcesControls(ResourceConfigMgr configMgr) {
        dataSourceCheckBoxes = new ArrayList<>();
    ...
    private void toggleDataSource(String dataSource, boolean status) {
        ...
        ILocalizationFile lf = pm.getLocalizationFile(ctx, LocalizationUtil
                .join("aviation", "config", "gui", "WxPlotCfg.xml"));

        try (SaveableOutputStream out = lf.openOutputStream()) {
            JAXB.marshal(wxPlotCfg, out);
            out.save();
        } catch (IOException | LocalizationException e) {
            statusHandler.error("Unable to save plot configuration to " + lf,
                    e);
        }
    }
    ...
    private void createScrolledComposite(ResourceConfigMgr configMgr) {
        scrolledComp = new ScrolledComposite(mainComp,
