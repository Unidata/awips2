 ...
 * Mar  9, 2020 8048        tgurney     Fix casting of Python int
 * </pre>
 *
 * @author lvenable
 *
...
public class WeatherWindCanvasComp extends WeatherCanvasComp {
    ...
    private void drawWindBarb(GC gc, int xc, int yc, int ff, Object ddObj) {
        ...
        if (ddObj instanceof java.lang.String) {
            ...
            dd = ((Number) ddObj).doubleValue();
            if (dd == 0) {
                r *= 2;
            }
        } else {
            if (ddObj == null) {
                statusHandler.error("dd is null",
                        new IllegalArgumentException());
            } else {
                statusHandler.error(
                        "Unknown type of dd: " + ddObj.getClass().getName(),
                        new IllegalArgumentException());
            }
            return;
        }
        ...
        if (dataMgr != null) {
            ...
            if (tafs != null) {
                for (Map<String, Object> taf : tafs) {
                    long from = ((Number) taf.get("from")).longValue();
                    int ff = ((Number) taf.get("ff")).intValue();
                    ...
                    tdelta /= 3600000.0;
                    int xc = (int) (Math.round(tdelta * timeLineXCoordSpace)
                            + x0);
        ...
        if (dataMgr != null) {
            ...
            if (mtrs != null) {
                for (Map<String, Object> mtr : mtrs) {
                    long from = ((Number) mtr.get("from")).longValue();
    ...
    private void drawModel(GC gc, List<Map<String, Object>> data) {
        if (data != null) {
            ...
            for (Map<String, Object> record : data) {
                ...
                long from = ((Number) record.get("from")).longValue();
                int ff = ((Number) record.get("ff")).intValue();
