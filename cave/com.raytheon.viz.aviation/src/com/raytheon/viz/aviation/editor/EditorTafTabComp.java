 ...
 * Feb 27, 2018 7120        tgurney     Stop text box from resizing after send
 * Feb 28, 2018 6870        tgurney     Fix the sticky Alt key on Alt+Tab,
 *                                      stop tracking modifier key state and
 *                                      use the provided state mask instead
 *
 * </pre>
 *
 * @author lvenable
...
public class EditorTafTabComp extends Composite {
     ...
     */
    public static final String ISSUE_TIME = "\\d{6}Z \\d{4}/";
    ...
    private void createTopControls(ResourceConfigMgr configMgr) {
        ...
        gd.widthHint = 90;
        Button clearBtn = new Button(controlsComp, SWT.PUSH);
    ...
    private void createTafEditorTextControl(ResourceConfigMgr configMgr) {
        ...
        tafEditorTxt = new StyledText(editorTextComp,
                SWT.BORDER | SWT.MULTI | SWT.V_SCROLL | SWT.H_SCROLL);
        tafEditorTxt.setEditable(true);
        tafEditorTxt.setLayoutData(gd);
        configMgr.setTextEditorFontAndColors(tafEditorTxt);

        undoStack = new ArrayList<>(UNDO_STACK_SIZE);
        redoStack = new ArrayList<>(UNDO_STACK_SIZE);
        ...
        tafEditorTxt.addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                boolean shft = (e.stateMask & SWT.SHIFT) == SWT.SHIFT;

                // Set flag if Alt pressed in combination with another key
                if (alt && e.keyCode != SWT.ALT) {
                    altPlusKey = true;
                }
                ...
                if (ctrl && !shft && e.keyCode == SWT.ARROW_UP) {
                    ...
                    if (currentLine == 0) {
                        ...
                        while (currentLine > 0 && !tafEditorTxt
                                .getLine(currentLine).trim().endsWith("=")) {
                            currentLine--;
                        }
                ...
                if (!shft && ctrl && e.keyCode == SWT.ARROW_DOWN) {
                    ...
                    if (currentLine == (tafEditorTxt.getLineCount() - 1)) {
                        ...
                        while (currentLine < (tafEditorTxt.getLineCount() - 1)
                                && !tafEditorTxt.getLine(currentLine)
                                        .endsWith("=")) {
                            currentLine++;
                        }
                        ...
                        int numLines = Math.round(
                                tafEditorTxt.getBounds().height / lineHeight);
                        int topLine = tafEditorTxt.getTopIndex();

                        if ((currentLine - topLine) > numLines) {
                            if (currentLine == (tafEditorTxt.getLineCount()
                                    - 1)) {
                                currentLine++;
                            }
        ...
        tafEditorTxt.addExtendedModifyListener(new ExtendedModifyListener() {
            ...
            public void modifyText(ExtendedModifyEvent e) {
                ...
                if (modifyFlag) {
                    ...
                    HashMap<String, Object> undoData = new HashMap<>();
    ...
    private void amendIt(String amd) {
        tafEditorTxt
                .setText(tafEditorTxt.getText().replaceAll("TAF COR", "TAF"));
        tafEditorTxt
                .setText(tafEditorTxt.getText().replaceAll("TAF AMD", "TAF"));
        tafEditorTxt.setText(
                tafEditorTxt.getText().replaceAll("TAF", ("TAF" + " " + amd)));
    ...
    public void updateTimes() {
        tafEditorTxt.setText(tafEditorTxt.getText().replaceAll(ISSUE_TIME,
    ...
    public void undo() {
        if (!undoStack.isEmpty()) {
            HashMap<String, Object> undoData = undoStack
    ...
    public void redo() {
        if (!redoStack.isEmpty()) {
            HashMap<String, Object> redoData = redoStack
