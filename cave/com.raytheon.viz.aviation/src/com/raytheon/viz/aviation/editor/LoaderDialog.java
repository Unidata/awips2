...
public class LoaderDialog extends CaveSWTDialog {
    ...
    private static final IUFStatusHandler statusHandler = UFStatus
            .getHandler(LoaderDialog.class);

    private static final Pattern REXP_RAMD = Pattern
            .compile("\\s+AMD\\s(NOT\\sSKED|LTD\\sTO)([\\s\\w/]+)?");

    private static final Pattern DDPAT = Pattern.compile("[Dd][Dd1]");

    private static final Pattern D2PAT = Pattern.compile("[dD]2");
    ...
    protected void initializeComponents(Shell shell) {
        ...
        try {
            ...
            statusHandler.error("Error getting site configuration", e);
        }
    ...
    private void createLeftSideControls(Composite mainComp,
        ...
        int index = initFromCbo
                .indexOf(configMgr.getDataAsString(ResourceTag.LoadOrder));
    ...
    private String[] getTafTemplate(String ident, String bbb) {
        ...
        try {
            ...
            ILocalizationFile lFile = config.getTemplateFile(ident, tmplHour);
            java.util.List<String> tmplLines = new ArrayList<>();
            StringBuilder contents = new StringBuilder();
            try (BufferedReader input = new BufferedReader(
                    new InputStreamReader(lFile.openInputStream()))) {
                String line = null;
                ...
                if (!Pattern.matches(pattern, firstLine)) {
                    throw new ConfigurationException(
                            "Invalid pattern in template");
                }
                ...
                if (routineTaf) {
                    ...
                    // Assume issue time contains the day before. (Could be
                    // previous
                    // month day)
                ...
                // Issue time
                tokens[1] = LoaderDialog.DDPAT.matcher(tokens[1])
                        .replaceAll(value);

                // Forecast hours
                tokens[2] = LoaderDialog.DDPAT.matcher(tokens[2])
                        .replaceAll(d1);
                tokens[2] = LoaderDialog.D2PAT.matcher(tokens[2])
                        .replaceAll(d2);
                ...
                while (!tmplLines.isEmpty()) {
                    contents.append(tmplLines.remove(0))
                            .append(TafUtil.LINE_BREAK);
                }
    ...
    private void apply() {
        ...
        for (int i = 0; i < selected.length; i++) {
            ...
            if ((taf != null && !"merge".equals(order))) {
                // latest
        ...
        tveDlg.populateEditorTab(sb.toString().trim(), wmoId, siteId, issueTime,
                bbb);

        try {
            ITafSiteConfig config = TafSiteConfigFactory.getInstance();
            String collectivePil = config.getProductCollectivePil(
                    productList.getItem(productList.getSelectionIndex()));

            if (collectivePil != null && !collectivePil.isEmpty()) {
