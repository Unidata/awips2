 ...
 * @version 1.0
 */
public class ClimateDataMenuDlg extends CaveSWTDialog implements
        IClimateDataMenuDlg {
    ...
    private void displayFixClimateFilesDialog() {
        if ((fixClimateFilesDlg == null)
                || (fixClimateFilesDlg.getShell() == null)
    ...
    private void displayGenerateScriptDialog() {
        if ((generateScriptsDlg == null)
            ...
            generateScriptsDlg.addCloseCallback(new ICloseCallback() {
    ...
    public void updateMonitor(final String msg) {
        if (!isDisposed()) {
            getDisplay().asyncExec(new Runnable() {
                ...
                public void run() {
                    if (!isDisposed()) {
                        fileAssessST.setText(fileAssessST.getText().trim()
                                + "\n" + msg);
                        ...
                        if (lineCount > numLines) {
                            fileAssessST
                                    .setTopIndex((lineCount - numLines) + 1);
                        }
                    }
                }
            });
    ...
    public void checkSite() {
        ...
        getDisplay().asyncExec(new Runnable() {
            ...
            @Override
            public void run() {
                if (!identList.isDisposed()
                        && (identList.getSelectionCount() > 0)
