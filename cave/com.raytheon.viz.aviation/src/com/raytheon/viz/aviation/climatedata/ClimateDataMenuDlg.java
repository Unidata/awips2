 ...
 * Jun 16, 2016 5693       rferrel     Added Fix Climate Files option.
...
public class ClimateDataMenuDlg extends CaveSWTDialog
        implements IClimateDataMenuDlg {
    ...
    private FixClimateFilesDlg fixClimateFilesDlg;
    ...
    protected void disposed() {
        ClimateDataManager.getInstance().cancelPythonTask();
        textFont.dispose();
    ...
    private void createCommandsMenu(Menu menuBar) {
        ...
        MenuItem fixClimateFilesMI = new MenuItem(commandsMenu, SWT.NONE);
        fixClimateFilesMI.setText("&Fix Climate Files...");
        fixClimateFilesMI.addSelectionListener(new SelectionAdapter() {

            @Override
            public void widgetSelected(SelectionEvent e) {
                displayFixClimateFilesDialog();
            }
        });
    }
            ...
            public void widgetSelected(SelectionEvent event) {
                if (mustCreate(usageDlg)) {
                    ...
                    String helpText = "This dialog is used to create and append climatology data files.\n\n"
                            ...
                            + "\n\tFix Climate Files:    Opens a dialog showing sites with HDF5 files."
                            + "\n\t\t\t\t\tAllows the user to select sites to filter out unwanted"
                            + "\n\t\t\t\t\tdata from the files." + "\n\nTools:"
    ...
    private void createLeftSideControls(Composite parentComp) {
        ...
        siteMetarCtrlComp.setLayoutData(new GridData());
    ...
    private void createCenterControls(Composite parentComp) {
        ...
        gd.widthHint = 400;
        gd.heightHint = 150;
        siteInfoList = new List(infoComp,
        ...
        gd.heightHint = 200;
        gd.horizontalSpan = 2;
        fileAssessST = new StyledText(centerComp,
    ...
    private void handleIdentsSelection() {
        ...
        java.util.List<String> idents = new ArrayList<>();
        ...
        getSiteInfoList(idents, timeout);
    }
    ...
    private void setHeaderInformation(StyledText headerST) {
        ...
        sb.append("\n");
        sb.append(
                String.format(bottomFmt, "Site", "USAF-WBAN", "NCDC", "Local"));
    ...
    private void displayFixClimateFilesDialog() {
        if (fixClimateFilesDlg == null || fixClimateFilesDlg.getShell() == null
                || fixClimateFilesDlg.isDisposed()) {
            fixClimateFilesDlg = new FixClimateFilesDlg(shell, this);
            fixClimateFilesDlg.open();
        } else {
            fixClimateFilesDlg.bringToTop();
        }
    }
    ...
    private void displayGenerateScriptDialog() {
        if (generateScriptsDlg == null || generateScriptsDlg.getShell() == null
            ...
            generateScriptsDlg.addCloseCallback(returnValue -> genScriptsBtn
                    .setEnabled((Boolean) returnValue));
    ...
    private void getSiteInfoList(java.util.List<String> idents, int timeout) {
        ClimateDataManager dataMgr = ClimateDataManager.getInstance();
        setWait(true);
        dataMgr.getIdnum(idents, timeout, this);
    }
        ...
        VizApp.runAsync(() -> {
            ...
            if (list != null) {
                for (java.util.List<String> idList : list) {
                    ...
                    sb1.append(ident);
                    sb1.append(String.format("%15s", String.format("%s-%s",
                            idList.get(0), idList.get(1))));
                    sb1.append(String.format("%12s", String.format("%s-%s",
                            idList.get(2), idList.get(3))));
                    sb1.append(String.format("%12s", String.format("%s-%s",
                            idList.get(4), idList.get(5))));
                    String item = sb1.toString();
                ...
                sb2.append(
                        String.format("%s   -- NO INFO AVAILABLE --", ident));
                siteInfoList.add(sb2.toString());
                assessDataBtn.setEnabled(false);
            }
        });
    }
    ...
    private void assessData() {
        ...
        java.util.List<String> itemList = new ArrayList<>(Arrays.asList(items));
        setWait(true);
        scriptsBtn(false);
        processBtn(false);
        validateBtn(false);
    ...
    private void saveLog() {
        ...
        try {
            dlg.setFilterPath(ClimateDataManager.getIshFilePath() + "/tmp/");
    ...
    public void updateMonitor(final String msg) {
        if (!isDisposed()) {
            getDisplay().asyncExec(() -> {
                if (!isDisposed()) {
                    fileAssessST.setText(
                            fileAssessST.getText().trim() + "\n" + msg);
                    // Scroll the visible area if necessary
                    int lineHeight = fileAssessST.getLineHeight();
                    int numLines = Math.round(
                            fileAssessST.getBounds().height / lineHeight);
    ...
    private ClimateHistoryData getObsHistoryFromInv() {
        Map<String, java.util.List<StationData>> dataMap = new LinkedHashMap<>();
        ...
        try {
            ...
            File invFile = new File(
                    ClimateDataManager.getIshFilePath() + "/isd-inventory.txt");

            try (FileReader fr = new FileReader(invFile);
            ...
            for (String str : siteInfoList.getItems()) {
                ...
                for (String line : lines) {
                    if (startsWithPat.matcher(line).matches()) {
                        ...
                        ObsGraphData yearData = new ObsGraphData(year,
                                obsCount);
                        stationData.addObsGraphData(yearData);
                    }
                }
                ...
                if (stationDataArray == null) {
                    stationDataArray = new ArrayList<>();
                }
    ...
    public void checkSite() {
        ...
        getDisplay().asyncExec(() -> {
            if (!identList.isDisposed() && identList.getSelectionCount() > 0
                    && siteInfoList.getItemCount() > 0
                    && siteInfoList.getItem(0).contains("NO INFO AVAILABLE")) {
                String site = identList.getSelection()[0];
                MessageBox confirmDeleteMB = new MessageBox(shell,
    ...
    public void executeStart() {
        VizApp.runSync(() -> setWait(true));

    }

    @Override
    public void executeDone() {
        VizApp.runSync(() -> setWait(false));
    }
}
