 ...
 * Jul 02, 2018  6641      njensen      Use List interface instead of ArrayList, cleanup
 * 
 * </pre>
 * 
 * @author lvenable
...
public class SourceComp extends Composite {
     ...
     */
    private static final int MARGIN_VAL = 3;
     ...
     * County data map.
     */
    private Map<Long, ValueNameIdData> countyDataMap;

    /**
     * County data array.
     */
    private java.util.List<ValueNameIdData> countyDataArray;

    /**
     * Basin data map.
     */
    private Map<Long, ValueNameIdData> basinDataMap;

    /**
     * Basin data array.
     */
    private java.util.List<ValueNameIdData> basinDataArray;
     ...
     */
    private java.util.List<String> sourceNamesArray;
    ...
    private void init() {
        ...
        gl.marginWidth = 0;
        gl.marginHeight = MARGIN_VAL;
        gl.marginRight = MARGIN_VAL;
        gl.marginLeft = MARGIN_VAL;
        this.setLayout(gl);
    ...
    private void initData() {
        ...
        countyDataMap = new HashMap<>();
        countyDataArray = new ArrayList<>();

        basinDataMap = new HashMap<>();
        basinDataArray = new ArrayList<>();
    }
    ...
    private void createSourceControls() {
        ...
        durationList = new List(sourceComp,
        ...
        if (!firstSource) {
            ...
            gd.widthHint = 35;
            Button removeSourceBtn = new Button(selectCtrlComp, SWT.PUSH);
        ...
        gd = new GridData(SWT.FILL, SWT.DEFAULT, true, false);
        Button selectBtn = new Button(selectCtrlComp, SWT.PUSH);
    ...
    private void createCountyList() {
        ...
        countyList = new List(countyComp,
                SWT.BORDER | SWT.MULTI | SWT.V_SCROLL);
        countyList.setLayoutData(gd);
        countyList.setFont(listFont);

        gd = new GridData(SWT.CENTER, SWT.DEFAULT, true, false);
        Button removeCountyBtn = new Button(countyComp, SWT.PUSH);
    ...
    private void createBasinList() {
        ...
        gd = new GridData(SWT.CENTER, SWT.DEFAULT, true, false);
        Button removeBasinBtn = new Button(basinComp, SWT.PUSH);
        ...
        removeBasinBtn.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                removeSelectedListItems(basinList, basinDataArray,
                        basinDataMap);
                actionCB.setModified(true);
            }
        });
    ...
    private void loadDurationList(String displayName) {
        ...
        for (String name : srcNames) {
            if (sourceDisplayMap.get(name).getDisplayName()
                    .equals(displayName)) {
                durationList
                        .add(sourceDisplayMap.get(name).getDurationHrsString());
            }
        }
    ...
    private void populateSourceComboDurationList() {
        /*
         * Since the source name is the unique identifier it will be the key in
         * the the source display map. The value in the map is
         * SrcDisplayDurationData which contains the display name for the source
         * combo box and the duration hour for the duration hour list control.
         *
         * Example:
         * 
        ...
        sourceNamesArray = new ArrayList<>();
        ...
        for (String sourceName : sourceNamesArray) {
            ...
            if (sourceCbo.indexOf(dispName) < 0) {
                sourceCbo.add(dispName);
            }
        }
        ...
        if (this.sourceCompData == null) {
            ...
            for (Entry<String, SrcDisplayDurationData> entry : sourceDisplayMap
                    .entrySet()) {
                String sourceName = entry.getKey();
                if (sourceCompData.getSourceName().equals(sourceName)) {
                    selectedSource = sourceName;
                    String dispName = entry.getValue().getDisplayName();
        ...
        if (sourceDisplayMap.containsKey(selectedSource)) {
            double duration = sourceDisplayMap.get(selectedSource)
                    .getDurationHrs();
    ...
    private void removeSelectedListItems(List listControl,
            java.util.List<ValueNameIdData> array,
            Map<Long, ValueNameIdData> map) {
        ...
        if (indexes.length > 1) {
            ...
            String selection = listControl
                    .getItem(listControl.getSelectionIndex());
     ...
     */
    private void addUpdateData(java.util.List<ValueNameIdData> newArray,
            List listControl, java.util.List<ValueNameIdData> array,
            Map<Long, ValueNameIdData> map) {
        ValueNameIdData valNameIdData;

        for (ValueNameIdData newData : newArray) {
            if (map.containsKey(newData.getId())) {
     ...
     */
    private void addData(java.util.List<ValueNameIdData> newArray,
            List listControl, java.util.List<ValueNameIdData> array,
            Map<Long, ValueNameIdData> map) {
    ...
    public void setSelectedBackground(boolean flag) {
        if (flag) {
            ((GridLayout) this.getLayout()).marginRight = MARGIN_VAL;
            ((GridLayout) this.getLayout()).marginLeft = MARGIN_VAL;
     ...
     *            County or Basin.
     */
    public void addUpdateNewData(java.util.List<ValueNameIdData> newArray,
        ...
        if (updateType == UpdateType.UPDATE) {
            if (type == GuidSectType.COUNTY) {
                ...
                addUpdateData(newArray, basinList, basinDataArray,
                        basinDataMap);
            }
     ...
     *            Array of new data.
     */
    public void addCountyBasinData(java.util.List<ValueNameIdData> newArray) {
        for (ValueNameIdData newData : newArray) {
    ...
    public SourceCompData getSourceData() {
        ...
        java.util.List<ValueNameIdData> allCntyBasinData = new ArrayList<>();
    ...
    public void setSourceName(String srcName) {
        if (sourceDisplayMap.containsKey(srcName)) {
            ...
            durationList
                    .select(durationList.indexOf(sddd.getDurationHrsString()));
        }
    }
    ...
    public String getSourceName() {
         ...
         */
        String selectedSourceCboName = sourceCbo
                .getItem(sourceCbo.getSelectionIndex());
        String selectedDuration = durationList
