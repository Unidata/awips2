 ...
 * Jun 21, 2018       6621 njensen     Get upper hour range val from server
 * 
 * </pre>
 * 
 * @author randerso
...
public class TimeDurScale extends Canvas implements PaintListener {
    ...
    private class MouseListener extends MouseAdapter implements
        ...
        public void mouseMove(MouseEvent e) {
            if (dragging) {
                ...
                if (hourVal < LOWER_HOUR_RANGE_VAL) {
                    hourVal = LOWER_HOUR_RANGE_VAL;
     ...
     */
    private static final int LOWER_HOUR_RANGE_VAL = 0;
    ...
    public TimeDurScale(Composite parent, ITimeDurationAction actionCallback) {
        ...
        int hoursAvailable = 24;
        try {
            hoursAvailable = (Integer) RequestRouter
                    .route(new FFMPGetDefaultPurgeHourRequest());
        } catch(Exception e) {
            statusHandler
                    .error("Error determining time range, defaulting to 24 hours",
                            e);
        }
        this.upperHourRangeVal = validateHours(hoursAvailable);
    ...
    private void resize() {
        ...
        pixPerHour = (timeBarRect.width - thumbRect.width)
                / (double) (upperHourRangeVal - LOWER_HOUR_RANGE_VAL);
    ...
    private void drawTimeSlider(GC gc) {
        // draw tick marks
        int i = 0;
        for (double hour = LOWER_HOUR_RANGE_VAL; hour <= upperHourRangeVal; hour += 1.0) {
    ...
    private void calcDisplayNumbers(int barWidth) {
        int totalDur = upperHourRangeVal - LOWER_HOUR_RANGE_VAL;
        int labelInc = 1;
        ...
        for (int i = 0; i < numLabels - 1; i++) {
            labelValues[i] = LOWER_HOUR_RANGE_VAL + labelInc * i;
        }
