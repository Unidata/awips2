...
public class FFFGForceUtil {
    ...
    public FFFGForceUtil(FFMPResource resource, String guidType) {
        ...
        List<SourceCompData> sourceCompList = fdm.getUserXMLData();
        if ((sourceCompList == null) || (sourceCompList.isEmpty())) {
            sourceCompList = fdm.getMasterXMLData();
        }
        List<String> forcedSourceList = new ArrayList<>();
    ...
    private ForceUtilResult forceIt(FFMPTemplates ft, FFMPBasin cBasin,
        ...
        List<Long> forcedPfafList = new ArrayList<>();
        ...
        if (pfafList != null && !pfafList.isEmpty()) {
            if ((sliderTime >= src1Hr) && (sliderTime <= src2Hr)) {
                ...
                if (sliderTime == src1Hr) {
                    ...
                    forcedPfafList = this.getForcedBasins(
                            sourceXML1.getSourceName(), pfafList, ft);
                    if ((sourceXML2 != null) && (!forced)
                            && ((forcedPfafList == null)
                                    || (forcedPfafList.isEmpty()))) {
                        if (cBasin.getAggregated()) {
                            forced = fdm.isForced(sourceXML2.getSourceName(),
                                    cBasin.getPfaf());
                        } else {
                ...
                // isn't
                if ((!forced) && (forcedPfafList.isEmpty())) {
                    if (!cBasin.getAggregated()) {
     ...
     * @return
     */
    private List<Long> getForcedBasins(String source, List<Long> pfafList2,
            FFMPTemplates ft) {
        FFFGDataMgr fdm = FFFGDataMgr.getInstance();
        List<Long> forcedList = new ArrayList<>();
        long prevCtyFips = 0l;
    ...
    public float getAvgForcedValue(List<Long> pfafList, List<Long> forcedPfafs,
        ...
        if (!interpolation.isInterpolate()) {
            ...
            return getInterpolatedForcedValue(interpolation, templates,
                    forcedPfafs, INTERPOLATION_TYPE.AVG) / forcedPfafs.size();
        }
    }
    ...
    public float getMaxForcedValue(List<Long> pfafList, List<Long> forcedPfafs,
        ...
        if (!interpolation.isInterpolate()) {
            ...
            return getInterpolatedForcedValue(interpolation, templates,
                    forcedPfafs, INTERPOLATION_TYPE.MAX);
        }
    }
    ...
    public List<Float> getForcedGuidValues(List<Long> pfafList,
        ...
        List<Float> guidList = new ArrayList<>();
        if (pfafList != null) {
            if (huc.equals(FFMPRecord.ALL) || huc.equals(FFMPRecord.COUNTY)) {
                ...
                for (Long pfaf : pfafList) {
                    ...
                    float val = getAvgForcedValue(pl, forcedPfafs,
                            ffmpGuidanceInterpolation, guidSourceExpiration,
                            ft);
                    guidList.add(val);
                }
            } else {
                // Only add aggregate value to guidList for numbered hucs
                // (second pfaf in list for forcing).
    ...
    public Float getInterpolatedForcedValue(FFMPTemplates templates,
        ...
        if (source1.equals(source2)) {
            ...
            FFFGDataMgr dman = FFFGDataMgr.getInstance();
            if (!dman.isExpired()) {
                adjValue2 = dman.adjustValue(adjValue2, source2, cBasinPfaf,
                        0l);
            }
            ...
            if ((interpolate.getHour(source1) == 0) || (source1.equals(source2)
                    && (interpolate.getHour(source2) == 1))) {
                Double ratio = new Double(ratioOffset);
                if (ratio.equals(.25)) {
                    guidance = (float) (.62 * value2);
                } else if (ratio.equals(.5)) {
            ...
            FFFGDataMgr dman = FFFGDataMgr.getInstance();
            if (!dman.isExpired()) {
                adjValue1 = dman.adjustValue(adjValue1, source1, cBasinPfaf,
                        0l);
            }
