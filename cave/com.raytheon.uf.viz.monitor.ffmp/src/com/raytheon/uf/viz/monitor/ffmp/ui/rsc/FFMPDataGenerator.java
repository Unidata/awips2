...
public class FFMPDataGenerator {
                ...
                if (!resource.isWorstCase() || resource.getHuc().equals("ALL")
                    ...
                    if (cBasin.getValues().size() > 0) {
                        ...
                        if (resource.getTime() > 0.00) {
                            ...
                            qpe = cBasin.getAccumValue(time, recentTime,
                    ...
                    for (String guidType : guidBasins.keySet()) {
                        ...
                        if ((guidBasin != null)
                            ...
                            if ((cBasin.getPfaf() == 31055)
                                    || (cBasin.getPfaf() == 2071284100000l)) {
                                System.out.println("Fouglas");
                            }
                            ...
                            if (((forcedPfafs.size() > 1)) && forced) {
                                ...
                                guidance = resource.getGuidanceValue(
                                        ffmpGuidBasin, recentTime, guidType);
                ...
                if (!resource.isWorstCase() || resource.getHuc().equals("ALL")
                    ...
                    if ((qpeBasin != null)
                        ...
                        qpe = qpeBasin.get(cBasin.getPfaf()).getAccumValue(
                                time, recentTime, expirationTime, isRate);
                        // time, recentTime, expirationTime, isRate);
                        trd.setTableCellData(2, new FFMPTableCellData(
                                FIELDS.QPE, qpe));
                    } else {
    ...
    private FFMPTableRowData getMaxValue(FFMPTableRowData trd, FFMPBasin cBasin) {
        ...
        if (cBasin instanceof FFMPVirtualGageBasin) {
                ...
                if (virtualBasin != null) {
                    trd.setTableCellData(
                            ...
                            new FFMPTableCellData(FIELDS.RATE, virtualBasin
                                    .get(cBasin.getPfaf()).getValue(recentTime)));
                } else {
                ...
                for (String guidType : guidBasins.keySet()) {
                    ...
                    if (guidBasin != null) {
                        ...
                        FFMPGuidanceBasin basin = ((FFMPGuidanceBasin) guidBasin
                                .get(cBasin.getPfaf()));
                        guidance = resource.getGuidanceValue(basin, recentTime,
                                guidType);
            ...
            if (pfafs.size() > 0) {
                if (rateBasin != null) {
                    rate = rateBasin.getMaxValue(pfafs, recentTime);
                    trd.setTableCellData(1, new FFMPTableCellData(FIELDS.RATE,
                            rate));
                } else {
                    trd.setTableCellData(1, new FFMPTableCellData(FIELDS.RATE,
                ...
                if (qpeBasin != null) {
                    qpe = qpeBasin.getAccumMaxValue(pfafs, time, recentTime,
                            expirationTime, isRate);
                    trd.setTableCellData(2, new FFMPTableCellData(FIELDS.QPE,
                            qpe));
                } else {
                    ...
                    if ((guidBasin != null)
                        ...
                        if (resource.isWorstCase()) {
                            ...
                            FFMPGuidanceBasin basin = (FFMPGuidanceBasin) guidRecords
                                    ...
                                    .get(cBasin.getPfaf());
                            guidance = resource.getGuidanceValue(basin,
                                    recentTime, guidType);
                        }
                ...
                if ((rateBasin != null)
                    ...
                    trd.setTableCellData(
                            ...
                            new FFMPTableCellData(FIELDS.RATE, rateBasin.get(
                                    cBasin.getPfaf()).getValue(
                                    monitor.getRateWindow().getBeforeTime(),
                                    monitor.getRateWindow().getAfterTime())));
                } else {
                    trd.setTableCellData(1, new FFMPTableCellData(FIELDS.RATE,
                            Float.NaN));
                }
                if ((qpeBasin != null)
                        && (qpeBasin.get(cBasin.getPfaf()) != null)) {
                    qpe = qpeBasin.get(cBasin.getPfaf()).getAccumValue(time,
                            recentTime, expirationTime, isRate);
                    trd.setTableCellData(2, new FFMPTableCellData(FIELDS.QPE,
                            qpe));
                } else {
                    trd.setTableCellData(2, new FFMPTableCellData(FIELDS.QPE,
    ...
    private FIELDS getBaseField() {
        ...
        if (monitor.getQpfWindow() == null) {
            monitor.setQpfWindow(monitor.getTimeWindow(qpfSource,
                    resource.getTableTime(), resource.getSiteKey()));
        }
        if (monitor.getQpeWindow() == null) {
            monitor.setQpeWindow(new FFMPTimeWindow(resource.getTableTime(),
                    recentTime));
        }
        if (monitor.getRateWindow() == null) {
            monitor.setRateWindow(monitor.getTimeWindow(resource.getProduct()
                    .getRate(), recentTime, resource.getSiteKey()));
        }
        ...
        if (resource.isWorstCase() || (resource.centeredAggregationKey != null)) {
            ...
            rateRecord = monitor.getRateRecord(product, siteKey, dataKey,
                    product.getRate(), recentTime, huc, true);
            qpeRecord = monitor.getQPERecord(product, siteKey, dataKey,
                    product.getQpe(), time, huc, true);
            qpfRecord = monitor.getQPFRecord(product, siteKey, dataKey, null,
                    time, huc, true);
            guidRecords = monitor.getGuidanceRecords(product, siteKey, time,
                    huc, true);
            virtualRecord = monitor.getVirtualRecord(product, siteKey, dataKey,
                    product.getVirtual(), time, huc, true);
        } else {
            rateRecord = monitor.getRateRecord(product, siteKey, dataKey,
                    product.getRate(), recentTime, resource.getHuc(), true);
            qpeRecord = monitor.getQPERecord(product, siteKey, dataKey,
                    product.getQpe(), time, resource.getHuc(), true);
            qpfRecord = monitor.getQPFRecord(product, siteKey, dataKey, null,
                    time, resource.getHuc(), true);
            guidRecords = monitor.getGuidanceRecords(product, siteKey, time,
                    resource.getHuc(), true);
            if (resource.getHuc().equals("ALL")) {
                virtualRecord = monitor.getVirtualRecord(product, siteKey,
                        dataKey, product.getVirtual(), time, resource.getHuc(),
                        true);
            }
            huc = resource.getHuc();
        }
