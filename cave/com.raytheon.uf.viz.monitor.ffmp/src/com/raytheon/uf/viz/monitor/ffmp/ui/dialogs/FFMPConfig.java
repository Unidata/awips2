...
public class FFMPConfig {
    ...
    private void init() {
        threshMgrMap = new EnumMap<>(
                ThreshColNames.class);

        thresholdLookup = new HashMap<>();
    ...
    public String getBasinTrendPlotColorName(String itemName) {
        List<FFMPTableColumnXML> tcXmlArray = ffmpCfgBasin
                .getTableColumnData();
    ...
    public Color getBasinThresholdColor(String colName, double val) {
        if (thresholdLookup.containsKey(colName)) {
    ...
    public ThresholdManager getThresholdManager(String threshName) {
        if (thresholdLookup.containsKey(threshName)) {
    ...
    public void createAttributesDlgData(String siteKey) {
        List<FFMPTableColumnXML> columnData = ffmpCfgBasin
                .getTableColumnData();
    ...
    public void setVisibleColumns(AttributesDlgData attrData) {
        ...
        Map<String, Boolean> guidanceMap = attrData.getGuidanceList();
        String list = String.join(",", guidanceMap.keySet());
    ...
    public int getStartSortIndex(String siteKey) {
        ...
        ProductXML productXml = sourceConfigManager.getProductByPrimarySourceName(name);

        List<String> guidDispNames = productXml.getAvailableGuidanceDisplayNames();

        String guidRankSource = null;
        if (guidDispNames.size() > 1) {
