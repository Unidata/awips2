 ...
 * Sep 14, 2017  20297    lshi      FFMP AlertViz errors when changing layers
 *                                  and opening basin trend graphs
 * Mar 26, 2018  7251     mduff     Added the missing data text like A1 had.
 * Aug 13, 2018  6670     randerso  Removed unused appName field. Code cleanup.
 *
 * </pre>
 *
 * @author lvenable
...
public abstract class FFMPTable extends Composite {
    ...
    private static final String NO_DATA_MSG = getMissingDataMsg();
    ...
    private Composite missingDataComp;
    ...
    private boolean missingCompDisplayed = false;
    ...
    private void createTableItems() {
        ...
        List<FFMPTableColumnXML> ffmpTableCols = ffmpCfgBasin
                .getTableColumnData();
        ...
        List<FFMPTableRowData> rowArray = tableData.getTableRows();
        ...
        if (indexArray.isEmpty() && !initialLoad) {
            if (!missingCompDisplayed) {
                createMissingDataComposite();
            }
        } else {
            if (missingDataComp != null) {
                missingDataComp.dispose();
                missingCompDisplayed = false;
            }
        }
    ...
    private void createMissingDataComposite() {
        missingDataComp = new Composite(this, SWT.NONE);
        GridData gd = new GridData(SWT.FILL, SWT.FILL, true, false);
        GridLayout gl = new GridLayout(1, false);
        missingDataComp.setLayout(gl);
        missingDataComp.setLayoutData(gd);

        gd = new GridData(SWT.FILL, SWT.FILL, true, false);
        Label missingDataLbl = new Label(missingDataComp, SWT.NONE);
        missingDataLbl.setText(NO_DATA_MSG);
        missingDataLbl.setLayoutData(gd);
        missingCompDisplayed = true;
    }

    private static String getMissingDataMsg() {
        String msg = "NO DATA Retrieved.\n";
        msg = msg.concat("This could be due to:\n");
        msg = msg.concat(
                "- the filter value for the attribute used for ranking\n");
        msg = msg.concat(
                "- no FFG for an FFG-based attribute used for ranking\n");
        msg = msg.concat(
                "- an insufficient Duration or a Duration with a large gap\n");
        msg = msg.concat(
                "- or it could mean that there is truly no data available.\n\n");
        msg = msg.concat("If you believe there is data, try the following:\n");
        msg = msg.concat(
                "- Adjust the filter value (Click the Thresholds button)\n");
        msg = msg.concat("- Change the rank attribute to qpe\n");
        msg = msg.concat(
                "- Adjust the Duration (adjust the Time Duration slider)");
        return msg;
    }
