 ...
 * Nov 29, 2012   1353     rferrel   Make dialog non-blocking. Changes for
 *                                   non-blocking AboutDlg. Changes for
 *                                   non-blocking AcknowledgmentsDlg. Changes
 *                                   for non-blocking HelpDlg. Changes for
 *                                   non-blocking RetrieveMergeDlg. Changes for
 *                                   non-blocking LoadSaveDeleteSelectDlg.
 * Mar 28, 2013   1790     rferrel   Bug fix for non-blocking dialogs.
 * Apr, 15, 2013  1911     dhladky   dialog wouldn't open every time to retrieve
 *                                   file.
 * May 05, 2014   3109     lvenable  Removed code that sets the cursor on Cave
 *                                   because it doesn't need to be set.
 ...
 * @author lvenable
 */
public class FFFGDlg extends CaveSWTDialog
        implements ISourceCompAction, IFFFGData {
    ...
    protected void initializeComponents(final Shell shell) {
        ...
        Thread dataRetrieveThread = new Thread(dlgData);
    ...
    private void createLeftSideControls() {
        ...
        expireTimeSpnr.setMaximum(100_000);
        ...
        Button sortCountyNameRdo = new Button(countyComp, SWT.RADIO);
        ...
        Button sortCountyIdRdo = new Button(countyComp, SWT.RADIO);
        ...
        gd.horizontalSpan = 2;
        Button addUpdateCountyBtn = new Button(countyComp, SWT.PUSH);
        ...
        Button sortBasinNameRdo = new Button(basinComp, SWT.RADIO);
        ...
        gd = new GridData(listWidth, listHeight);
        basinNameList = new List(basinComp,
                SWT.BORDER | SWT.MULTI | SWT.V_SCROLL);
        basinNameList.setLayoutData(gd);
        basinNameList.setFont(listFont);

        gd = new GridData(listWidth, listHeight);
        basinIdList = new List(basinComp,
                SWT.BORDER | SWT.MULTI | SWT.V_SCROLL);
        ...
        gd = new GridData(SWT.CENTER, SWT.DEFAULT, true, false);
        gd.horizontalSpan = 2;
        Button addUpdateBasinBtn = new Button(basinComp, SWT.PUSH);
    ...
    public void setStatusMsg(int status, String message) {
        if (message == null) {
            ...
            statusLbl.setText(
                    statusFormat.format(now.getTime()) + ": " + message);
    ...
    private void applyDataAction() {
        ...
        if (expireTimeSpnr.getSelection() == 0) {
            ...
            sb.append(
                    "Setting the expiration time to zero will cause the forced ");
        ...
        java.util.List<SourceCompData> srcCompData = getSourceCompData();
        java.util.List<java.util.List<FFFGBasinIdXML>> basinList = new ArrayList<>();
        for (SourceCompData scd : srcCompData) {
            java.util.List<FFFGBasinIdXML> list = new ArrayList<>();
            java.util.List<ValueNameIdData> dataList = scd.getCountyBasinData();
            for (ValueNameIdData vni : dataList) {
                // Get the basins for each county
                java.util.List<Long> pfafs = templates
                        .getAllAggregatePfafs(vni.getId(), "COUNTY");
                for (long pfaf : pfafs) {
    ...
    private long calculateExpTimeInMillis() {
        ...
        long expTimeInMillis = SimulatedTime.getSystemTime().getTime().getTime()
                + expHoursInMillis;
     ...
     */
    private java.util.List<SourceCompData> getSourceCompData() {
        java.util.List<SourceCompData> srcCompData = new ArrayList<>();
    ...
    private void addUpdateSourceData(List nameList, List idList,
        ...
        for (int i = 0; i < selIndexes.length; i++) {
            ...
            if (selectedSrc.containsID(id, type)) {
        ...
        java.util.List<ValueNameIdData> newDataArray = new ArrayList<>();
        ValueNameIdData valNameIdData;

        StringBuilder srcMsg = new StringBuilder();
        for (int i = 0; i < selIndexes.length; i++) {
            if (type == GuidSectType.COUNTY) {
                // The ID for county needs to be the GID. The ID list control
                // is a hybrid so we need the GID instead.
                valNameIdData = new ValueNameIdData(ffgValueTF.getText().trim(),
                        nameList.getItem(selIndexes[i]),
                        counties.getCounties().get(selIndexes[i]).getGid(),
                        type);
                srcMsg.append(" {").append(ffgValueTF.getText().trim())
                        .append(" ").append(nameList.getItem(selIndexes[i]))
                        .append(" ").append(counties.getCounties()
                                .get(selIndexes[i]).getGid())
                        .append("} ");
            } else {
                // For the basins, the read ID is in the ID list control.
                valNameIdData = new ValueNameIdData(ffgValueTF.getText().trim(),
                        nameList.getItem(selIndexes[i]),
                        basinListData.getBasins().get(selIndexes[i]).getPfaf(),
                        type);
                srcMsg.append(" {").append(ffgValueTF.getText().trim())
                        .append(" {").append(nameList.getItem(selIndexes[i]))
                        .append("} ").append(basinListData.getBasins()
                                .get(selIndexes[i]).getPfaf())
                        .append("}");
            }
        ...
        if (selIndexes.length <= 2) {
            setStatusMsg("Inserted " + srcMsg.toString() + " successfully.");
        } else {
    ...
    private void updateBasinCountLabel() {
        if (!showBasinsFromCountyChk.getSelection()) {
        ...
        if (selectedCounties == 0) {
            ...
            int basins = basinNameList.getItemCount();
            basinCountLbl.setText(
                    basins + " basin(s) in " + selectedCounties + " counties");
        }
    }
    ...
    private boolean validateFFGValue() {
        ...
        if (!ffgText.matches("^[+-]?\\d*\\.?\\d*")) {
            StringBuilder msg = new StringBuilder();
            msg.append("An incorrect FFG value was entered.\n");
            msg.append("The FFG value must follow these rules:\n\n");
            msg.append(
                    "* Only numbers (including decimals) can be entered in.\n");
            msg.append(
                    "* A '+' in front the number will add the number to the FFG value.\n");
            msg.append(
                    "* A '-' in front the number will subtract the number to the FFG value.\n");
            msg.append(
                    "* Just a number will override the existing FFG value.\n\n");
        ...
        if ((ffgVal < -9.0) || (ffgVal == 0.0) || (ffgVal > 9.0)) {
            StringBuilder msg = new StringBuilder();
            msg.append(
                    "The FFG value must be between -9.0 and 9.0 and cannot be zero.\n");
            displayMessageBox("Warning", msg.toString(), SWT.ICON_WARNING);
            return false;
        }
    ...
    private boolean validateSources() {
         ...
         */
        Map<String, Object> srcNameMap = new HashMap<>();
        for (SourceComp sc : sourceCompArray) {
            if (!srcNameMap.containsKey(sc.getSourceName())) {
    ...
    private void populateCountyLists() {
        ...
        java.util.List<FFMPCounty> countyArray = counties.getCounties();
    ...
    private void populateBasinLists(boolean sorted) {
        if (!showBasinsFromCountyChk.getSelection()) {
            if ((!allBasinsAreLoaded) || (basinIdList.getItemCount() == 0)
                    || sorted) {
                java.util.List<FFMPBasinNameIdData> basinArray = new ArrayList<>();

                basinNameList.removeAll();
                basinIdList.removeAll();

                java.util.List<FFMPCounty> countyArray = counties.getCounties();
                java.util.List<Long> pfafArray;
            ...
            for (int i = 0; i < indexes.length; i++) {
                ...
                for (Long pfaf : pfafArray) {
                    ...
                    if (basinMetaData == null) {
                        statusHandler.debug("Meta Data is null!");
                    }

                    basinData = new FFMPBasinNameIdData(basinMetaData.getPfaf(),
                            basinMetaData.getStreamName());
                    basinArray.add(basinData);
                }
            }
    ...
    private FFMPBasins.BasinSort getBasinSort() {
        if (sortBasinIdRdo.getSelection()) {
    ...
    private void retrieveSavedData() {
        ...
        if (isDialogClear() && (fileNameLbl.getText().trim().length() == 0)) {
            ...
            if (retMergeDlg == null) {
                ...
                retMergeDlg.addCloseCallback(new ICloseCallback() {
    ...
    private void doRetrieveSavedData(RetrieveMergeAction action,
        ...
        fdm.loadUserFFFGData(lfile.getFile().getName());
         ...
         */
        java.util.List<SourceCompData> tmpScdArray = fdm.getUserXMLData();
        java.util.List<SourceCompData> scdArray;

        if (tmpScdArray.isEmpty()) {
            StringBuilder sb = new StringBuilder();
        ...
        if (scdArray.size() != sourceCompArray.size()) {
            StringBuilder sb = new StringBuilder();
            sb.append(
                    "There was an error creating new Source controls for the\n");
        ...
        for (int i = 0; i < scdArray.size(); i++) {
            ...
            if (sc.isValidSourceName(scd.getSourceName())) {
                ...
                sb.append(
                        "' is an invalid source name.  This source name will be skipped.");
                displayMessageBox("Invalid Source Name", sb.toString(),
                        SWT.ICON_WARNING);
            }
        }

        String filename = lfile.getFile().getName();
        if (filename.equals(fdm.getFFFGMasterFileName())) {
            fileNameLbl.setText(filename + "  (read-only)");
        } else {
        ...
        updateFileStatusLabel(false);
        packLabels();
        setStatusMsg("Retrieved " + filename + " successfully.");
    }
     ...
     *            Number of sources.
     */
    private void createSourcesForExistingData(
            java.util.List<SourceCompData> scdArray) {
    ...
    private void doSaveFileAs(LocalizationFile fileName) {
        ...
        this.setStatusMsg(
                "Saved as " + fdm.getUserFileName() + " successfully. ");
    }
    ...
    private void doDeleteFile(LocalizationFile lfile) {
        FFFGDataMgr fdm = FFFGDataMgr.getInstance();
        String name = lfile.getFile().getName();

        try {
            if (name.equals(fdm.getUserFileName())) {
                fdm.deleteUserXMLData(name);
                fileNameLbl.setText("");
                updateFileStatusLabel(false);
            }
            lfile.delete();
            ...
            statusHandler.handle(Priority.PROBLEM,
                    "Unable to delete file \"" + lfile.toString() + "\"", e);
    ...
    private java.util.List<SourceCompData> mergeData(
            java.util.List<SourceCompData> retrievedDataArray,
            java.util.List<SourceCompData> displayDataArray) {
        java.util.List<SourceCompData> newDataArray = new ArrayList<>(
                displayDataArray);
        ...
        for (SourceCompData retrievedScd : retrievedDataArray) {
            ...
            for (int i = 0; i < newDataArray.size(); i++) {
                if (newDataArray.get(i).getSourceName()
                    ...
                    newDataArray.get(i)
                            .mergeAreaFFG(retrievedScd.getAreaFFGValue());
    ...
    private java.util.List<SourceCompData> mergeOverwriteData(
            java.util.List<SourceCompData> retrievedDataArray,
            java.util.List<SourceCompData> displayDataArray) {
        java.util.List<SourceCompData> newDataArray = new ArrayList<>(
                displayDataArray);
        ...
        for (SourceCompData retrievedScd : retrievedDataArray) {
            ...
            for (int i = 0; i < newDataArray.size(); i++) {
                if (newDataArray.get(i).getSourceName()
                    ...
                    foundMatchingScd = true;
                    newDataArray.get(i)
                            .setAreaFFGValue(retrievedScd.getAreaFFGValue());
    ...
    public void addNewSource(SourceCompData scd) {
         ...
         */
        if (sourceCompArray.size() == MAX_SOURCES) {
    ...
    public void removeSource(SourceComp srcComp) {
        ...
        sourceCompArray.get(sourceCompArray.size() - 1)
    ...
    private void updateFileStatusLabel(boolean isModified) {
        if (isModified && (fileNameLbl.getText().trim().length() > 0)) {
            fileNameStatusLbl.setBackground(
    ...
    private boolean isDialogClear() {
        java.util.List<SourceCompData> srcCompDataList = getSourceCompData();
