...
/**
 * 
 * Duration Hours Scale Composite
 * 
 * <pre>
 *
 * SOFTWARE HISTORY
 ...
 * ------------ ---------- ----------- --------------------------
 * ????                                Initial creation
 * Jun 21, 2018 6641       njensen     Get upperVal from server
...
public class DurHoursScaleComp extends Composite {
    ...
    private static final IUFStatusHandler statusHandler = UFStatus
            .getHandler(DurHoursScaleComp.class);
    ...
    private void init() {
        FFMPSourceConfigurationManager fscm = FFMPSourceConfigurationManager
                .getInstance();
        List<String> guidSourceNames = fscm.getGuidanceSourceNames();

        FFTIDataManager fdm = FFTIDataManager.getInstance();

        lowerVal = Double.MAX_VALUE;
        for (String sourceName : guidSourceNames) {
        ...
        try {
            upperVal = (Integer) RequestRouter.route(req);
        } catch (Exception e) {
            statusHandler.error(
                    "Error getting default FFMP purge rule, defaulting slider to 24 hours",
                    e);
        }

        // get the smallest QPF value
        List<String> qpfSourceNames = fscm.getQPFSourceNames();
    ...
    private void createControls() {
        ...
        scaleValueCanvas
                .setLayoutData(new GridData(CANVAS_WIDTH + 20, CANVAS_HEIGHT));
        ...
        Canvas scaleRangeCanvas = new Canvas(this, SWT.DOUBLE_BUFFERED);
        scaleRangeCanvas
                .setLayoutData(new GridData(CANVAS_WIDTH + 20, CANVAS_HEIGHT));
    ...
    private void drawScaleValueCanvas(GC gc) {
        ...
        int newXCoord = (int) Math
                .round((this.timeDurHours - lowerVal) * pixPerInc);
