 ...
 * ------------- -------- --------- --------------------------------------------
 *                                  Initial creation
 * Dec 05, 2012  1353     rferrel   Convert to CaveSWTDialog and made
 *                                  non-blocking.
 * Jun 21, 2018  6641     njensen   Size buttons based on char width
...
public class FFTIControlDlg extends CaveSWTDialog {
     ...
     */
    private static final String[] TAB_TITLES = new String[] { "Setting 1",
    ...
    private void initializeComponents() {
        try {
            ...
            statusHandler.handle(Priority.ERROR, "FFTI cannot load", e);
        }
    }
    ...
    private void createCwaControls() {
        ...
        // initialize the CWAs
        List<String> cwaList = fftiDataMgr.getCwaList();
        if ((cwaList != null) && (!cwaList.isEmpty())) {
    ...
    private Set<Integer> getDuplicates() {
        Set<Integer> duplicateLst = new HashSet<>();
        for (int i = 0; i < fftiTabFolder.getItemCount(); i++) {
            ...
            for (int j = (i + 1); j < fftiTabFolder.getItemCount(); j++) {
                SettingComp nextItem = (SettingComp) fftiTabFolder.getItem(j)
                        .getControl();
                if (thisItem.getSelectedAttribType()
                        .endsWith(nextItem.getSelectedAttribType())
                        ...
                        && (thisItem.getQpeSrc().length > 0
                                && nextItem.getQpeSrc().length > 0
                                && thisItem.getQpeSrc()[0]
                                        .equals(nextItem.getQpeSrc()[0]))
                        && (thisItem.getQpfSrc().length > 0
                                && nextItem.getQpfSrc().length > 0
                                && thisItem.getQpfSrc()[0]
                                        .equals(nextItem.getQpfSrc()[0]))
                        && (thisItem.getGuidSrc().length > 0
                                && nextItem.getGuidSrc().length > 0
                                && thisItem.getGuidSrc()[0]
    ...
    protected void saveAllSettings() {
        ...
        List<FFTISettingXML> settings = new ArrayList<>();
    ...
    private void createSettingTabs() {
        ...
        if (fftiDataMgr.getSettingList().size() == 0) {
            ...
            settingTab.setText(TAB_TITLES[0]);
            ...
            for (int i = 0; i < fftiDataMgr.getSettingList().size(); i++) {
                FFTISettingXML fftiSetting = fftiDataMgr.getSettingList()
                        .get(i);
                TabItem settingTab = new TabItem(fftiTabFolder, SWT.NONE);
                settingTab.setText(TAB_TITLES[i]);
    ...
    private void addSettingTab() {
        // Add tab
        TabItem settingTab = new TabItem(fftiTabFolder, SWT.NONE);
        settingTab.setText(TAB_TITLES[fftiTabFolder.getItemCount() - 1]);
        settingTab.setControl(new SettingComp(fftiTabFolder));

        updateTabSettings();

        fftiTabFolder.getParent().pack(true);
    ...
    private void renameTabs() {
        ...
        for (int i = 0; i < tabItemArray.length; i++) {
            tabItemArray[i].setText(TAB_TITLES[i]);
        }
    }
    ...
    public List<String> getCWASelectionIDs() {
        List<String> cwaIds = new ArrayList<>();
