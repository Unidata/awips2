...
public class FFMPRowGenerator implements Runnable {
    ...
    public void run() {
        ...
        if (cBasin instanceof FFMPVirtualGageBasin) {
            ...
            List<String> excludedVGBLids = FFMPTemplateConfigurationManager
                    .getInstance().getExcludedVGBs().getLids();

            String lid = vgBasin.getLid();

            // DR-11250 get rid of excluded
            if (lid != null && (!excludedVGBLids.contains(lid))) {
                ...
                if (huc.equals(FFMPRecord.ALL)
                    ...
                    List<Long> pfafs = ft.getAggregatePfafs(cBasin.getPfaf(),
                            siteKey, domain);
    ...
    private String getDisplayName(FFMPBasin basin) {
        ...
        try {
            ...
            // aggregations
                ...
                List<Long> pfafs = ft.getAggregatePfafs(basin.getPfaf(),
                        siteKey, huc);
                if (!pfafs.isEmpty()) {
            ...
            statusHandler.handle(Priority.WARN,
                    "No display name for basin." + basin.getPfaf(), e);
        }
        return name;
    }

    private FFMPTableRowData getMaxValue(FFMPTableRowData trd, FFMPBasin cBasin) {
        List<DomainXML> domainList = FFMPRunConfigurationManager.getInstance()
                .getDomains();
        List<DomainXML> activeDomains = new ArrayList<>();
        ...
        List<Long> pfafs = ft.getAggregatePfafs(cBasin.getPfaf(), siteKey, huc,
                activeDomains);
        ...
        if (cBasin instanceof FFMPVirtualGageBasin) {
            ...
            if (!pfafs.isEmpty()) {
                ...
                for (String guidType : guidBasins.keySet()) {
                    ...
                    List<Long> forcedPfafs = new ArrayList<>();
                ...
                if ((qpfBasin != null)
                    ...
                    qpf = qpfBasin.get(cBasin.getPfaf()).getMaxValue(
    ...
    private void processGuidance(FFMPTableRowData trd, FFMPBasin cBasin,
            List<Long> pfafs, Float qpe) {
        Float guidance;
        Float ratioValue;
        Float diffValue;
        int i = 0;
        for (Entry<String, FFMPBasinData> entry : guidBasins.entrySet()) {
            guidance = Float.NaN;
            diffValue = Float.NaN;
            ratioValue = Float.NaN;
            String guidType = entry.getKey();
            FFFGForceUtil forceUtil = forceUtils.get(guidType);
            forceUtil.setSliderTime(sliderTime);

            FFMPBasinData guidBasin = entry.getValue();
    ...
    private float getForcedAvg(String domain, FFMPBasin cBasin, String guidType) {
        ...
        List<Long> pfafList = new ArrayList<>();
        float guidance = Float.NaN;
