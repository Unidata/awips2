 ...
 * Aug 06, 2021  22528    smoorthy    Added server name field to dialog. Also, add 
 *                                    proxy address to JMSConnectionInfo
 * </pre>
 *
 * @author bsteffen
 * @version 1.0
...
public class ThinClientConnectivityDialog extends ConnectivityPreferenceDialog {
    ...
    public ThinClientConnectivityDialog(boolean checkAlertViz) {
        ...
        serverName = store.getString(ThinClientPreferenceConstants.P_PROXY_SERVERNAME);
    }
    ...
    protected void createTextBoxes(Composite textBoxComp) {
        ...
        serverNameLabel = new Label(textBoxComp, SWT.RIGHT);
        serverNameLabel.setText("Server Name:");
        GridData gd = new GridData(SWT.RIGHT, SWT.CENTER, false, true);
        serverNameLabel.setLayoutData(gd);

        serverNameText = new Text(textBoxComp, SWT.BORDER);
        serverNameText.setText(serverName == null ? "": serverName);
        serverNameText.setBackground(getTextColor(isLocalizationGood()));
    ...
    protected void applySettings() {
        ...
        thinStore.setValue(ThinClientPreferenceConstants.P_PROXY_SERVERNAME, serverName);
    ...
    public boolean validate() {
        ...
        if (proxyAddress != null && proxyAddress.length() > 0 && serverName != null && serverName.length()>0) {

            //add the host/servername pair for HttpClient
            try {
                URI proxyURI = new URI(proxyAddress);
                String host = proxyURI.getHost();
                HttpClient.getInstance().setHostHeader(host, serverName);
                validateServices();
                validatePypies();
            } catch (URISyntaxException e) {
                //if this exception results, fail validation
                status="Address could not be parsed as a valid URI";
                statusHandler.debug(status, e);
                appendDetails(
        ...
        if (serverNameText != null && !serverNameText.isDisposed()) {
            serverNameText
                    .setBackground(getTextColor(servicesGood && pypiesGood));
        }
    ...
    private void validateJms(boolean hasEdexConnection) {
        if (hasEdexConnection) {
            try {
                ...
                response.getJmsConnectionInfo().setProxyAddress(proxyAddress);
