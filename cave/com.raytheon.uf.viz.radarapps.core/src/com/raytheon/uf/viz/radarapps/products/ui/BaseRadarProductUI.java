 ...
 * May 20, 2016 DR18622    kshrestha   Removed duplicates and sorted OTR elevations
 *                                     list for active radar
 * 2016-04-01   DR17885    jdynina     Fix elevation list retrieval for RMR & OTR
 * 2016-06-07   DR17885    dfriedman   Validate elevation when changing RPG/product
 * </pre>
 *
 * @author ?
 * @version 1.0
...
public abstract class BaseRadarProductUI {
        ...
        if (currentPossibleProducts.size() > 0) {
            ...
            for (Param p : first.params) {
                if (oldProd != null && oldProd.params.contains(p)) {
                    ...
                    if (p == Param.ELEVATION) {
                        changeRequestElevationParameter();
                    }
                } else
                    setParamDefault(p);
            }
    ...
    /* If necessary, change the request's elevation parameter to
     * conform to what is valid for the current product/RPG.
     */
    private void changeRequestElevationParameter() {
        RadarProduct prod = getProductInfo().getPoductForCode(
                request.productCode);
        if (prod != null && prod.params.contains(Param.ELEVATION)) {
            if (!isElevationValid(request)) {
                if (findRequestElevationSelectionIndex() >= 0) {
                    int[] elevAngles = getElevationAngles();
                    int firstAngle = 0;
                    if (elevAngles != null) {
                        firstAngle = elevAngles[0];
                    }
                    switch (request.getElevationSelection()) {
                    case Request.ALL_ELEVATIONS:
                        if (request.getElevationAngle() == 0) {
                            // Should not happen.
                            setParamDefault(Param.ELEVATION);
                        } else {
                            request.selectAllElevations(firstAngle);
                        }
                        break;
                    case Request.SPECIFIC_ELEVATION:
                        request.setElevationAngle(firstAngle);
                        break;
                    case Request.LOWER_ELEVATIONS:
                        request.selectLowerEelevations(firstAngle);
                        break;
                    case Request.N_ELEVATIONS:
                        request.selectLowerCuts(1);
                        break;
                    }
                } else {
                    setParamDefault(Param.ELEVATION);
    ...
    /*
     * Returns the possible elevation selection types for the current radar
     * type.
     *
     * It is also used to determine the type currently selected in the UI. If
     * it becomes possible for this method to be called when the UI is not in
     * sync with the radar type, the method must be refactored.
     */
    ElevationSelection[] getElevationSelections() {
        if (radarType == RadarType.TDWR)
            return tdwrElevationSelections;
        else
            return wsrElevationSelections;
    }
    ...
    private void createElevationUI(Composite rows) {
        ...
        for (ElevationSelection es : getElevationSelections())
            cmb.add(es.label);
    ...
    protected void onElevationAngleSelected() {
        if (elevationAnglesCombo != null && elevationTypeCombo != null) {
            ...
            if (angles == null || angles.length == 0 || es == null)
                return;
    ...
    public void setRadar(RadarConfig rc, GSM gsm) {
        ...
        if ( liveElevationAngles != null) {
           HashSet<Integer> set = new HashSet<Integer>(liveElevationAngles.length);
           for (int e : liveElevationAngles)
               set.add(e);

           liveElevationAngles = new int[set.size()];

           int i = 0;
           for (int e : set)
               liveElevationAngles[i++] = e;

           Arrays.sort(liveElevationAngles);
        }
    ...
    private int[] getElevationAngles() {
        ...
        if (radarID != null && radarID.equals(liveRadarID) && liveVCP == vcp)
            ...
            if (usage == Usage.RMR)
                elev_vcp = ElevationInfo.RMR_CODE;

            if (radarType != null)
                if (usage == Usage.RMR && radarType == RadarType.TDWR)
                    elev_vcp = ElevationInfo.RMR_TDWR_CODE;
                else if (usage == Usage.OTR && radarType == RadarType.TDWR)
                    elev_vcp = ElevationInfo.OTR_TDWR_CODE;

            return ElevationInfo.getInstance()
                    .getUniqueElevations(radarID, elev_vcp);
    }
    ...
    private int findElevationSelection(ElevationSelection es) {
        ElevationSelection[] elevationSelections = getElevationSelections();
    ...
    private void setElevationAngleComboFromRequest() {
        ...
        if (elevationAnglesCombo != null && es != null) {
            ...
            if (es == ElevationSelection.LOWEST_N) {
                ...
                int i = getIndexOfElevationAngle(angle);
    ...
    private int getIndexOfElevationAngle(int angle) {
    ...
    private boolean isElevationValid(Request request) {
        if (findRequestElevationSelectionIndex() < 0) {
            return false;
        }
        switch (request.getElevationSelection()) {
        ...
        case Request.SPECIFIC_ELEVATION:
        case Request.LOWER_ELEVATIONS:
            return getIndexOfElevationAngle(request.getElevationAngle()) >= 0;
        case Request.N_ELEVATIONS: {
            int[] elevAngles = getElevationAngles();
            return request.getElevationAngle() >= 1 && elevAngles != null
                    && request.getElevationAngle() <= elevAngles.length;
        }
        }
        return false;
    }
    ...
    private ElevationSelection getElevationSelectionFromUI() {
        if (elevationTypeCombo != null) {
            ...
            ElevationSelection[] elevationSelections = getElevationSelections();
            if (i >= 0 && i < elevationSelections.length) {
