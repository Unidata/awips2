...
public class RadarApps {
    ...
    public static String chooseRpg(Shell shell, Boolean dedicatedRestriction,
            RadarType typeRestriction, boolean alwaysPrompt) {
        // Could have automatically updating list, but do we really want that?
        // have to filter anyway for dedicated...
        ConfigReply reply = (ConfigReply) getRcmSystem().sendCheckedAndHandled(
                new ReqObj.GetRadarConfig(), shell);
        if (reply == null)
            return null;
    ...
    public static float[] getRadarLocation(String radar) {
        ...
        List<Object[]> rows;
        try {
            rows = DirectDbQuery.executeQuery(query, METADATA_DB,
                    QueryLanguage.SQL);
