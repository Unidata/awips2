...
public class FcstDataDetermWindow implements Subject
{
	...
	{		 
	    ...
	    Dimension d = new Dimension(1275, 920);
    ...
    {
    	if(locationTextField.getText().length() > 0 || numElevZonesTextField.getText().length() > 0 ||
    	   qpfTextField.getText().length() > 0 ||
    	   consumptiveUseTextField.getText().length() > 0 || channelLossTextField.getText().length() > 0 ||
    	   qtfTextField.getText().length() > 0 || qzfTextField.getText().length() > 0)
    	{
    		activateClearButton();
    	}
    	else
	...
	private class SaveActionListener implements ActionListener
    {
        public void actionPerformed(ActionEvent e) 
        {
			...
			try
			{
				...
				{
					if(Short.valueOf(numElevZonesTextField.getText()) > 0)
					{
						record.setNum_elev_zones(Short.valueOf(numElevZonesTextField.getText()));
					}
					else
					{
			...
			try
			{
				...
				{
					record.setHours_qpf(Short.valueOf(qpfTextField.getText()));
				}
			}
			catch(NumberFormatException ne)
			{
			...
			try
			{
				...
				{
					record.setHours_qtf(Short.valueOf(qtfTextField.getText()));
				}
				else
				{
					null_or_not = 1;
			...
			try
			{
				...
				{
					record.setHours_qzf(Short.valueOf(qzfTextField.getText()));
				}
				else
				{
					null_or_not = 1;
...
//			lhvmLogger.log(" Delete record"+ record.toString());
			...
			for(i=0;i<lhvmRowDataList.size();i++)
    		{
    			...
    			qpfTextField.getText().equalsIgnoreCase(String.valueOf(currentHoursQpfFromRowsInsideLoop)) &&
