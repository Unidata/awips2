...
public class PdcTslDataManager {
     ...
     * ParamCode paramCode) { String header =
     * "PdcTslDataManager.getPreferredFcstTypeSourceByIngestFilter(): ";
     * 
     * String newTypeSource = null;
     * 
     * IngestFilterTable table = new IngestFilterTable(_db);
     ...
     * IngestFilterRecord record = null;
     * 
     * String whereClause = "WHERE lid = '" + lid + "' AND pe = '" +
     * paramCode.getPe() + "'" + "AND dur = 0 " + "AND ts like 'F%' " +
     * " order by ts_rank ASC" ;
     * 
     * try { List recordList = table.select(whereClause); if (recordList.size()
     * > 0) { int i = 0; //for (int i = 0; i < recordList.size(); i++) { record
     * = (IngestFilterRecord) recordList.get(i); newTypeSource = record.getTs();
     ...
     * //
     * ------------------------------------------------------------------------
     * ------------- public String getPreferredFcstTypeSourceByFcstHeight(String
     * lid, ParamCode paramCode) { String header =
     * "PdcTslDataManager.getPreferredFcstTypeSource(): ";
     * 
     * String newTypeSource = null;
     * 
     * FcstHeightTable table = new FcstHeightTable(_db);
     ...
     * 
     * String whereClause = "WHERE lid = '" + lid + "' AND pe = '" +
     * paramCode.getPe() + "'" + "AND dur = 0 " +
     * " order by basistime DESC, validtime ASC" ;
     * 
     * try { List recordList = table.selectNRecords(whereClause,1); if
     * (recordList.size() > 0) { int i = 0; //for (int i = 0; i <
     * recordList.size(); i++) { record = (FcstHeightRecord) recordList.get(i);
     * newTypeSource = record.getTs(); } } } catch(SQLException e) {
     * logSQLException(e); }
     * 
     * System.out.println(header + " newTypeSource = " + newTypeSource);
     * 
    ...
    public RegularTimeSeries loadPrecipData(String locationId,
            int durationInHours) {
        String header = "PdcTslDataManager.loadPrecipData(): ";
        RegularTimeSeries timeSeries = null;

        if (locationId.equalsIgnoreCase("MAX")) {
            timeSeries = loadPrecipDataForMaximumInArea(locationId,
                    durationInHours);
        } else if (locationId.equalsIgnoreCase("AVG")) {
            timeSeries = loadPrecipDataForAverageInArea(locationId,
                    durationInHours);
        } else {
            timeSeries = loadPrecipDataForRealLocation(locationId,
                    durationInHours);
        }
    ...
    public RegularTimeSeries loadPrecipDataForRealLocation(String locationId,
        ...
        for (int i = 0; i < list.size(); i++) {
            RegularObsTimeSeries regularObsTimeSeries = (RegularObsTimeSeries) list
                    .get(i);
    ...
    // -------------------------------------------------------------------------------------
    public RegularTimeSeries loadPrecipDataForMaximumInArea(String locationId,
            int durationInHours) {
        ...
        if (regularObsTimeSeriesList.size() > 0) {
            regularObsTimeSeries = (RegularObsTimeSeries) regularObsTimeSeriesList
                    .get(0);
            maxRegularObsTimeSeries = regularObsTimeSeries;
        }

        List maxTimeValuePairList = maxRegularObsTimeSeries
        ...
        for (int i = 1; i < regularObsTimeSeriesList.size(); i++) {
            regularObsTimeSeries = (RegularObsTimeSeries) regularObsTimeSeriesList
                    .get(i);
            List timeValuePairList = regularObsTimeSeries
                    .getTimeValuePairList(true);
            ...
            for (int j = 0; j < timeValuePairList.size(); j++) {
                ...
                TimeValuePair maxPair = (TimeValuePair) maxTimeValuePairList
                        .get(j);
                double maxValue = maxPair.getValue();

                if (value < MAX_REASONABLE_PRECIP) {
                    if (value > maxValue) {
                        String timeString = DbTimeHelper
                                .getDateTimeStringFromLongTime(time);

                        System.out.println(header + " new max = " + value
                                + " at "
    ...
    public RegularTimeSeries loadPrecipDataForAverageInArea(String locationId,
            int durationInHours) {
        // String header =
        // "PdcTslDataManager.loadPrecipDataForAverageInArea(): ";
        RegularTimeSeries timeSeries = null;
        ...
        if ((regularObsTimeSeriesList != null)
                && (regularObsTimeSeriesList.size() > 0)) {
            regularObsTimeSeries = (RegularObsTimeSeries) regularObsTimeSeriesList
                    .get(0);
            averageRegularObsTimeSeries = new RegularObsTimeSeries(
                    regularObsTimeSeries);

            averageTimeValuePairList = averageRegularObsTimeSeries
                    .getTimeValuePairList(true);
        } else {
            return timeSeries;
        }
        ...
        for (int i = 0; i < regularObsTimeSeriesList.size(); i++) {
            ...
            for (int j = 0; j < timeValuePairList.size(); j++) {
                ...
                if ((value < MAX_REASONABLE_PRECIP) && (value >= 0.0)) {
                    TimeValuePair totalPair = (TimeValuePair) averageTimeValuePairList
                            .get(j);
                    if (i == 0) // first iteration only, init the previous
                                // totals to Zero
                    {
                        previousValue = 0;
                    } else {
        ...
        for (int i = 0; i < averageTimeValuePairList.size(); i++) {
            TimeValuePair pair = (TimeValuePair) averageTimeValuePairList
                    .get(i);
            long time = pair.getDateTime();
        ...
        // just to be sure
        averageRegularObsTimeSeries
                .setTimeValuePairList(averageTimeValuePairList);
    ...
    public RegularTimeSeries getRegularTimeSeries(
        ...
        long timeStepIntervalInMillis = obsTimeSeries
                .getTimeStepIntervalInMillis();
        long timeStepIntervalInHours = timeStepIntervalInMillis
                / MILLIS_PER_HOUR;
        MeasuringUnit unit = MeasuringUnit.inches;

        String startTimeString = DbTimeHelper
                .getDateTimeStringFromLongTime(startTime);
        String endTimeString = DbTimeHelper
                .getDateTimeStringFromLongTime(endTime);

        RegularTimeSeries regularTimeSeries = new RegularTimeSeries(startTime,
                endTime, (int) timeStepIntervalInHours, unit);
    ...
    public IrregularTimeSeries loadObservedStageTimeSeries(String locationId,
        ...
        String whereClause = " WHERE lid = '" + locationId + "'"
                + " AND pe = '" + paramCode.getPe() + "'" + " AND ts = '"
                + paramCode.getTypeSource() + "'" + " AND dur = "
                + paramCode.getIhfsDur() + " AND extremum = '"
                + paramCode.getExtremum() + "'" + " AND quality_code >= "
                + _questionable_bad_threshold + "";
    ...
    public IrregularTimeSeries loadObservedPrecipPcTimeSeries(
        ...
        IrregularTimeSeries timeSeries = new IrregularTimeSeries(unit);
        ...
        String whereClause = " WHERE lid = '" + locationId + "'"
                + " AND pe = 'PC' " + " AND ts = '" + paramCode.getTypeSource()
                + "'" + " AND dur = 0 " + " AND quality_code >= "
                + _questionable_bad_threshold + " AND obstime >= '"
                + startTimeString + "'";
    ...
    // ------------------------------------------------------------------------
    public IrregularTimeSeries loadObservedTemperatureTimeSeries(
            String locationId, ParamCode paramCode) {
    ...
    // -----------------------------------------------------------------------------------
    public IrregularTimeSeries loadRelativeHumidityTimeSeries(
            String locationId, ParamCode paramCode) {
    ...
    public IrregularTimeSeries loadObservedSnowTimeSeries(String locationId,
            ParamCode paramCode) {
    ...
    public IrregularTimeSeries loadObservedDewpointTimeSeries(
            String locationId, ParamCode paramCode) {
    ...
    public IrregularTimeSeries loadFcstStageTimeSeries(String locationId,
        ...
        String whereClause = " WHERE lid = '" + locationId + "'"
                ...
                + paramCode.getIhfsDur() + " AND extremum = '"
                + paramCode.getExtremum() + "'" + " AND quality_code >= "
                + _questionable_bad_threshold + " ORDER BY basistime DESC ";
        // " AND obstime >= '" + startTimeString + "'";
    ...
    public Location getLocationInfo(String locationId) {
        ...
        for (int i = 0; i < locPDCViewList.size(); i++) {
            ...
            location.setLat(Double.parseDouble(decimalFormat
                    .format(locPDCRecord.getLat())));
            location.setLon(Double.parseDouble(decimalFormat
                    .format(locPDCRecord.getLon())));
    ...
    private String getPrecipFileName(int durationInHours) {
        String precipFileName = _preprocessedFilePath + "/Precip"
                + durationInHours + "Hour.dat";
