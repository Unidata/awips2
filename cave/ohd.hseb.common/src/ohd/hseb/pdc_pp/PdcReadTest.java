...
public class PdcReadTest extends TestCase {
    ...
    private static TimeValuePair[] _24HourCorrectTimeValuePairArray = {
            ...
            tvp("2007-01-09 12:00:00", 9.0), tvp("2007-01-10 12:00:00", 10.0) };

    // 3 hour variables
    private long _3HourStartTime = _3HourCorrectTimeValuePairArray[0]
            .getDateTime();

    private String _3HourStartTimeString = DbTimeHelper
            .getDateTimeStringFromLongTime(_3HourStartTime);

    private long _3HourEndTime = _3HourCorrectTimeValuePairArray[9]
            .getDateTime();

    private String _3HourEndTimeString = DbTimeHelper
            .getDateTimeStringFromLongTime(_3HourEndTime);

    // 24 hour variables
    private long _24HourStartTime = _24HourCorrectTimeValuePairArray[0]
            .getDateTime();

    private String _24HourStartTimeString = DbTimeHelper
            .getDateTimeStringFromLongTime(_24HourStartTime);

    private long _24HourEndTime = _24HourCorrectTimeValuePairArray[9]
            .getDateTime();

    private String _24HourEndTimeString = DbTimeHelper
            .getDateTimeStringFromLongTime(_24HourEndTime);
    ...
    public PdcReadTest() {
        ...
        long time = DbTimeHelper
                .getLongTimeFromDateTimeString("2007-01-01 12:00:00");
        System.out.println("time = " + time);

        time = DbTimeHelper
                .getLongTimeFromDateTimeString(" 2007-01-10 12:00:00");
        System.out.println("time = " + time);
    ...
    private RegularObsTimeSeries createTimeSeries(String lid,
        ...
        RegularObsTimeSeriesDescriptor descriptor = getRegularObsTimeSeriesDescriptor(
                lid, startTimeString, endTimeString);
    ...
    private void createCorrectTimeSeries() {
        _correct3HourTimeSeries = createTimeSeries("TEST1", 3,
                _3HourCorrectTimeValuePairArray);
        _correct24HourTimeSeries = createTimeSeries("TEST1", 24,
                _24HourCorrectTimeValuePairArray);
    }

    // --------------------------------------------------------------------------------------------------------

    public void innerCFileRead(int timeStepIntervalInHours, int durationCode,
        ...
        List readTsList = _dataMgr.readPrecipTimeSeriesListFromCGeneratedFile(
                timeStepIntervalInHours, durationCode);
        ...
        boolean readAndCorrectTimeSeriesEqual = regularTimeSeriesEqual(readTs,
                correctTimeSeries);
        ...
        else // size is same, need to check some more
        {
            ...
            for (int i = 0; i < list1.size(); i++) {
                ...
                if (!regularTimeSeriesEqual(ts1, ts2)) {
                    result = false; // something is different, so the lists are
                    // not the same
                    break;
