...
public class PDCPreprocessorDataMgr {
    ...
    private int _precipHoursToReadWhenCaching = 48; // currently, must be a
    ...
    public PDCPreprocessorDataMgr(String baseConnectionString, Logger logger,
            String preprocessedFilePath, long minutesBeforeTopOfHour,
            long minutesAfterTopOfHour, boolean debug) {
        super();
    ...
    private void getAppsDefaults() {
        ...
        _cacheRefreshAgeMaxInMillis = cacheRefreshAgeInMinutes
                * MILLIS_PER_MINUTE;

        System.out.println("_cacheRefreshAgeMax = "
                + _cacheRefreshAgeMaxInMillis);
    ...
    /**
     * createLidMap() - Creates a Hashmap of Location objects. Used for the
     * RegularObsTimeSeriesDescriptor objects
     */
    public void createLidMap() {
        ...
        if (_DEBUG) {
            System.out
                    .println("Size of recordslist = " + locPDCViewList.size());
            System.out.flush();
        }
        ...
        for (int i = 0; i < locPDCViewList.size(); i++) {
            ...
            double formattedLat = getFormattedDouble(locPDCRecord.getLat(), 2,
                    DbTable.getNullDouble(), 0.0);
            location.setLat(formattedLat);

            double formattedLon = getFormattedDouble(locPDCRecord.getLon(), 2,
                    DbTable.getNullDouble(), 0.0);
            ...
            if ((fq == MISSING) && (fs != MISSING)
                    && (location.getRatingCurve() != null)) {
                location.setFloodFlow(location.getRatingCurve()
                        .getDischargeFromStage(fs));
            }
         ...
         * RiverstatRecord riverStatRecord = (RiverstatRecord)
         * riverStatRecordList.get( i );
         * 
         * Location selectedLoc = (Location) _locationHashMap.get(
         * riverStatRecord.getLid() );
         * 
        ...
        for (int i = 0; i < stationClassList.size(); i++) {
            StnClassRecord stnClassRecord = (StnClassRecord) stationClassList
                    .get(i);

            Location selectedLoc = (Location) _locationHashMap
                    .get(stnClassRecord.getLid());
        ...
        if (_DEBUG) {
            System.out.println("push HashMap finished in "
                    + putHashMapTimer.getElapsedTime() + " millis");
            System.out.println("parseTimer finished in "
                    + parseTimer.getElapsedTime() + " millis");
            System.out.println("ratingCurveTimer finished in "
                    + ratingCurveTimer.getElapsedTime() + " millis");
    ...
    private double getFormattedDouble(double originalValue,
            int decimalPlacesToMaintain, double missingValue,
            double missingValueReplacement) {
        double value = missingValueReplacement;

        if (originalValue == missingValue) {
            originalValue = missingValueReplacement;
        } else {
            value = MathHelper.roundToNDecimalPlaces(originalValue,
    ...
    public Set getRiverStationLidSet() {
        ...
        for (int i = 0; i < locationKeyList.size(); i++) {
            ...
            if (location == null) {
                System.err.println("no location found for " + lid
                        + " in getRiverStationLidSet");
                continue;
            }
    ...
    public RegularObsTimeSeries getDischargeTimeSeriesFromHeightTimeSeries(
        ...
        RegularObsTimeSeriesDescriptor descriptor = new RegularObsTimeSeriesDescriptor(
                obsTimeSeries.getDescriptor());

        Location location = (Location) _locationHashMap
                .get(descriptor.getLid());

        if (location == null) {
            System.err.println("no location found for " + descriptor.getLid()
                    + " in getDischargeTimeSeriesFromHeightTimeSeries");
        ...
        if (!wrongPe && ratingCurve != null && ratingCurve.exists()) {
            ...
            newTimeSeries = new RegularObsTimeSeries(descriptor, 1,
            ...
            for (int i = 0; i < timeValuePairList.size(); i++) {
                TimeValuePair timeValuePair = (TimeValuePair) timeValuePairList
                        .get(i);
                if (timeValuePair != null) {
                    TimeValuePair newTimeValuePair = new TimeValuePair(
                            timeValuePair.getDateTime());

                    dischargeValue = ratingCurve
                            .getDischargeFromStage(timeValuePair.getValue());
                    dischargeValue = MathHelper.roundToNDecimalPlaces(
                            dischargeValue, 2);
                    newTimeValuePair.setValue(dischargeValue);

                    // dischargeString = decimalFormat.format(
                    // ratingCurve.getDischargeFromStage(
                    // timeValuePair.getValue() ) );
                    // newTimeValuePair.setValue( Double.parseDouble(
                    // dischargeString ) );

                    newTimeSeries
                            .addTimeValuePairIfBetterMatch(newTimeValuePair);
                }
            }
        }
    ...
    public RegularObsTimeSeries getPercentFloodFlowTimeSeriesListFromFlowStorageTimeSeries(
            RegularObsTimeSeries obsTimeSeries) {
        RegularObsTimeSeries newTimeSeries = null;
        RegularObsTimeSeriesDescriptor descriptor = obsTimeSeries
                .getDescriptor();
        Location location = (Location) _locationHashMap
                .get(descriptor.getLid());
        // DecimalFormat decimalFormat = new DecimalFormat( "0.00" );
        ...
        if (location == null) {
            System.err
                    .println("no location found for "
                            + descriptor.getLid()
                            + " in getPercentFloodFlowTimeSeriesListFromFlowStorageTimeSeries");
            return newTimeSeries;
        }
                ...
                if (timeValuePair != null) {
                    ...
                    if (!timeValuePair.isMissingValue()) {
                        percentFloodFlow = timeValuePair.getValue()
                                / location.getFloodFlow();
                        percentFloodFlow = 100.0 * MathHelper
                                .roundToNDecimalPlaces(percentFloodFlow, 2);

                        // String valueString = decimalFormat.format(
                        // timeValuePair.getValue() / location.getFloodFlow() );
                        // newTimeValuePair.setValue( Double.parseDouble(
                        // valueString ) );

                        if ((percentFloodFlow >= 0.0)
                                && (percentFloodFlow <= 100.0)) {
    ...
    public void loadRatingCurves() {
        ...
        IntegerHolder ratingShiftIndexHolder = new IntegerHolder(
                ratingShiftIndex);
        ...
        try {
            ...
            allRatingShiftRecordList = ratingShiftTable
                    .select("WHERE active = 'T' ORDER BY lid, date desc");
            allRiverstatRecordList = riverstatTable.select("ORDER BY lid");

            for (int i = 0; i < allRatingRecordList.size(); i++) {
                RatingRecord ratingRecord = (RatingRecord) allRatingRecordList
                        .get(i);
                // you have at least one rating curve that you have dealt with
                // and you are still going
                if ((prevLid != null)
                        && (!ratingRecord.getLid().equalsIgnoreCase(prevLid))) {
                    singleStationShiftList = getStationRatingShiftRecordListFromRatingShiftRecordList(
                            prevLid, allRatingShiftRecordList,
                            ratingShiftIndexHolder);
                    riverStatRecord = getRiverStatRecordFromRiverStatRecordList(
                            prevLid, allRiverstatRecordList,
                            riverStatIndexHolder);
                    // process old
                    loadRatingCurveByRecordList(singleStationRatingRecordList,
                            singleStationShiftList, riverStatRecord, prevLid);
    ...
    private void loadRatingCurveByRecordList(List ratingRecordList,
            List ratingShiftList, RiverstatRecord riverStatRecord,
            String locationId) {
        ...
        if (location != null) {
            ...
            System.out
                    .println(header
                            ...
                            + locationId
                            + " +  Run set_stnclass.LX to update station classifications.");
        }
     ...
     * @param lid
     *            - location id
     * @param allRatingShiftRecordList
     *            - Complete list of ratingShiftRecords sorted by lid
     * @param ratingShiftIndexHolder
     *            - Index holder for the current ratingShift Index
     * @return Single station ratingShiftRecordList
     */
    private List getStationRatingShiftRecordListFromRatingShiftRecordList(
            String lid, List allRatingShiftRecordList,
        ...
        for (int i = ratingShiftIndexHolder.getValue(); i < allRatingShiftRecordList
                .size() && !done; i++) {
            ...
            } else if (found) // prevlid does not match current lid and lid has
            // already been found so we've found all the
            // ratingshifts for the lid
            {
                done = true;
            }
        }
     ...
     * @param lid
     *            - location id
     * @param allRiverStatRecordList
     *            - Complete list of RiverstatRecords
     * @param riverStatIndexHolder
     *            - Index holder for the current riverstat Index
     */
    private RiverstatRecord getRiverStatRecordFromRiverStatRecordList(
            String lid, List allRiverStatRecordList,
            IntegerHolder riverStatIndexHolder) {
        RiverstatRecord record = null;

        boolean found = false;

        for (int i = riverStatIndexHolder.getValue(); i < allRiverStatRecordList
                .size() && !found; i++) {
    ...
    /**
     * createShefDurMap() - Creates a Hashmap of Shef durations. Used for
     * creating PEDTSEPs when writing out the data to .DAT files
     */
    ...
    /**
     * getCachedNonPrecipObsTimeSeriesList - Queries the database and returns a
     * List of Observed Time Series.
     * 
     * @param tableName
     ...
     */
    private List readNonPrecipObsTimeSeriesListFromCachedFile(
            String fullFilePath) {
    ...
    public List getNonPrecipObsTimeSeriesListUsingCache(String tableName,
        ...
        String fullFilePath = _preprocessedFilePath + "/cached" + tableName
                + ".dat";
        ...
        System.out.println(header + "for table = " + tableName
                + ", numHoursToRetrieveFromDb = " + numHoursToRetrieveFromDb);

        dbTimer.start();
        dbTimeSeriesList = getNonPrecipObsTimeSeriesListFromDb(tableName,
                endTime, numHoursToRetrieveFromDb);
        dbTimer.stop(header + "db read within cache read took ");

        long newStartTime = getNewStartTime(endTime, 1, numHoursToPreprocess);

        resultantTimeSeriesList = TimeSeriesListMgr.mergeTimeSeriesLists(
                dbTimeSeriesList, fileTimeSeriesList, newStartTime, endTime);
        ...
        writeTimer
                .stop(header + "writing to data file within cache read took ");
    ...
    public List getPrecipObsTimeSeriesListFromCache(int timeStepIntervalInHours) {
        ...
        String fullFilePath = _preprocessedFilePath + "/cached"
                + timeStepIntervalInHours + "HourPrecip.dat";
    ...
    private void deleteOldCacheFiles() {
        ...
        if (fileArray != null) {
            ...
            for (int i = 0; i < fileArray.length; i++) {
                ...
                if (fileName.startsWith("cached") && fileName.endsWith(".dat")) {
                    file.delete();
                }
    ...
    private boolean isCachedDataCurrentEnough(List fileTimeSeriesList,
            int cacheHours) {
        /*
         * This examines the times of the data in the cache.
         * shouldCacheBeAccepted() examines the last time that the cache itself
         * was completely recreated from scratch.
         */
        String header = "PDCPreprocessorDataMgr.isCachedDataCurrentEnough() :";
        boolean result = false;

        if (fileTimeSeriesList != null) // if there is no file, then it is not
        // current enough
        {
            RegularObsTimeSeries firstFileTs = (RegularObsTimeSeries) fileTimeSeriesList
                    .get(0);
            ...
            // " currentTime = " +
            // DbTimeHelper.getDateTimeStringFromLongTime(currentTime) );
    ...
    // -------------------------------------------------------------------------
    /**
     * getNonPrecipObsTimeSeriesListFromDb - Queries the database and returns a
    ...
    public List getNonPrecipObsTimeSeriesListFromDb(String tableName,
        ...
        NonPrecipObsTableManager table = new NonPrecipObsTableManager(_db,
                tableName);
        List obsTimeSeriesList = null;

        if (numHoursToPreProcess > 0) {
            try {
                obsTimeSeriesList = table.select(tableName,
                        numHoursToPreProcess, endTime);
            } catch (SQLException e) {
                logSQLException(e);
            }
        }
    ...
    public RegularObsTimeSeries getDepthAboveFSTimeSeries(
            RegularObsTimeSeries obsTimeSeries) {
        RegularObsTimeSeries newTimeSeries = null;
        Location location = (Location) _locationHashMap.get(obsTimeSeries
                .getDescriptor().getLid());
        ...
        if (location.isRiverStation()) {
            newTimeSeries = new RegularObsTimeSeries(
                    obsTimeSeries.getDescriptor(), 1, hourlyTimeSlotPolicy);
            ...
            for (int i = 0; i < timeValuePairList.size(); i++) {
                ...
                if (timeValuePair != null) {
                    TimeValuePair newTimeValuePair = new TimeValuePair(
                            timeValuePair.getDateTime());
                    if ((floodStage == MISSING)
                            || (timeValuePair.getValue() == MISSING)) {
                        newTimeValuePair.setValue(MISSING);
                    } else {
                        newTimeValuePair.setValue(timeValuePair.getValue()
                                - floodStage);
                    }
                    newTimeSeries
                            .addTimeValuePairIfBetterMatch(newTimeValuePair);
            ...
            for (int i = 0; i < timeValuePairList.size(); i++) {
                ...
                if (timeValuePair != null) {
                    newTimeValuePair = new TimeValuePair(
                            timeValuePair.getDateTime(), MISSING);
                    newTimeSeries
                            .addTimeValuePairIfBetterMatch(newTimeValuePair);
                }
            }
    ...
    public RegularObsTimeSeries getDeltaTimeSeries(
            RegularObsTimeSeries obsTimeSeries) {
        DecimalFormat decimalFormat = new DecimalFormat("0.00");
        ...
        RegularObsTimeSeries newTimeSeries = new RegularObsTimeSeries(
                obsTimeSeries.getDescriptor(), 1, hourlyTimeSlotPolicy);
        ...
        for (long indexTime = descriptor.getStartTime(); indexTime <= descriptor
                .getEndTime(); indexTime += MILLIS_PER_HOUR) {
            TimeValuePair timeValuePair = (TimeValuePair) timeValuePairMap
                    .get(new Long(indexTime));
            if (timeValuePair != null) {
                TimeValuePair newTimeValuePair = new TimeValuePair(
                        timeValuePair.getDateTime());
                TimeValuePair previousTimeValuePair = (TimeValuePair) timeValuePairMap
                        .get(new Long(previousDayTime));

                if ((timeValuePair == null) || (previousTimeValuePair == null)
                        || (previousTimeValuePair.getValue() == MISSING)
                        || (timeValuePair.getValue() == MISSING)) {

                } else {
                    double newValue = timeValuePair.getValue()
                            - previousTimeValuePair.getValue();

                    newTimeValuePair.setValue(Double.parseDouble(decimalFormat
                            .format(newValue)));
                    newTimeSeries
                            .addTimeValuePairIfBetterMatch(newTimeValuePair);
                }
    ...
    public void executePDCPrecipPP(long endTime, int precipHoursToReadFromDB) {
        ...
        String commandString = dirString + "/run_pdc_precip_pp" + " "
                + (endTime / 1000) + " " + precipHoursToReadFromDB;

        if ((OS.indexOf("nt") > -1) || (OS.indexOf("windows 2000") > -1)
                || (OS.indexOf("windows xp") > -1)) // Checking to see if
        // running on Linux or
        // Windows
        {
        } else {
    ...
    public List getInstantaneousPrecipTimeSeriesList(long endTime) {
        ...
        {
            instantPrecipInputFileName = _preprocessedFilePath
                    + "/geninstantprecip.out";
        }

        try {
            File instantPrecipFile = new File(instantPrecipInputFileName);

            BufferedReader _reader = new BufferedReader(new FileReader(
                    instantPrecipFile));
            ...
            while ((line = _reader.readLine()) != null) // reads the next line
            // and checks if it's
            // null
            {
                RegularObsTimeSeriesDescriptor descriptor = new RegularObsTimeSeriesDescriptor();
                ...
                IrregularObsTimeSeries obsTimeSeries = new IrregularObsTimeSeries(
                        descriptor, 1);
                ...
                for (long indexTime = startTime; indexTime < endTime; indexTime += MILLIS_PER_HOUR) {
                    ...
                    timeValuePair.setValue(Double
                            .parseDouble(tokenizedLine[counter++]));
    ...
    public List readPrecipTimeSeriesListFromCGeneratedFile(
        ...
        try {
            ...
            // null
            {
                ...
                descriptor.setPe(tokenizedLine[1]); // commented out because I
                // want acumm. precip to
                // display as PP
                ...
                RegularObsTimeSeries obsTimeSeries = new RegularObsTimeSeries(
                        descriptor, timeStepIntervalInHours, timeSlotPolicy);
    ...
    private void logException(Exception exception) {
        _logger.log("ERROR = " + exception.getMessage());
        exception.printStackTrace(_logger.getPrintWriter());
    ...
    // ------------------------------------------------------------------------
    public PrecipListHolder processPrecipDataWithCaching(long endTime,
        ...
        int hoursToReadWhenCaching = _precipHoursToReadWhenCaching; // Note,
        // MUST be a
        // multiple
        // of 24
        // hours
        int numHoursToRetrieveUsingCRoutines = 0;
        // read in cached Precip Data into all 4 Lists of RegularObsTimeSeries
        // objects
        ...
        List cached1HourPrecipTimeSeriesList = getPrecipObsTimeSeriesListFromCache(1);

        List cached3HourPrecipTimeSeriesList = getPrecipObsTimeSeriesListFromCache(3);

        List cached6HourPrecipTimeSeriesList = getPrecipObsTimeSeriesListFromCache(6);

        List cached24HourPrecipTimeSeriesList = getPrecipObsTimeSeriesListFromCache(24);
        ...
        if ((isCachedDataCurrentEnough(cached1HourPrecipTimeSeriesList,
                hoursToReadWhenCaching))
                && (isCachedDataCurrentEnough(cached3HourPrecipTimeSeriesList,
                        hoursToReadWhenCaching))
                && (isCachedDataCurrentEnough(cached6HourPrecipTimeSeriesList,
                        hoursToReadWhenCaching))
                && (isCachedDataCurrentEnough(cached24HourPrecipTimeSeriesList,
                        hoursToReadWhenCaching))) {
        ...
        // retrieve fresh data and merge it with the data previously retrieved
        // from the cache.
        if (numHoursToPreprocess > 0) {
            ...
            cCodeTimer.stop(header
                    + "Running the C Precip preprocessor alone took");

            instTimer.start();
            instantPrecipTimeSeriesList = getInstantaneousPrecipTimeSeriesList(endTime);
            precipListHolder
                    .setInstantPrecipTimeSeriesList(instantPrecipTimeSeriesList);
            ...
            long newStartTime = 0;
            oneHourPrecipTimeSeriesList = getMergedPrecipTimeSeriesList(
                    endTime, 1, numHoursToPreprocess, 1001,
                    cached1HourPrecipTimeSeriesList);
            precipListHolder
                    .setOneHourPrecipTimeSeriesList(oneHourPrecipTimeSeriesList);

            threeHourPrecipTimeSeriesList = getMergedPrecipTimeSeriesList(
                    endTime, 3, numHoursToPreprocess, 1003,
                    cached3HourPrecipTimeSeriesList);
            precipListHolder
                    .setThreeHourPrecipTimeSeriesList(threeHourPrecipTimeSeriesList);

            sixHourPrecipTimeSeriesList = getMergedPrecipTimeSeriesList(
                    endTime, 6, numHoursToPreprocess, 1006,
                    cached6HourPrecipTimeSeriesList);
            precipListHolder
                    .setSixHourPrecipTimeSeriesList(sixHourPrecipTimeSeriesList);

            twentyFourHourPrecipTimeSeriesList = getMergedPrecipTimeSeriesList(
                    endTime, 24, numHoursToPreprocess, 2001,
                    cached24HourPrecipTimeSeriesList);
            precipListHolder
                    .setTwentyForHourPrecipTimeSeriesList(twentyFourHourPrecipTimeSeriesList);

            mergingTimer.stop(header
                    + "merging the 1,3,6, and 24 hour time series took");
    ...
    // ------------------------------------------------------------------------
    private List getMergedPrecipTimeSeriesList(long endTime,
            int timeStepInHours, int numHoursToPreprocess, int durationCode,
        ...
        freshDataList = readPrecipTimeSeriesListFromCGeneratedFile(
                timeStepInHours, durationCode);

        newStartTime = getNewStartTime(endTime, timeStepInHours,
        ...
        String newStartTimeString = DbTimeHelper
                .getDateTimeStringFromLongTime(newStartTime);
        String newEndTimeString = DbTimeHelper
                .getDateTimeStringFromLongTime(newEndTime);

        System.out.println(header + " " + timeStepInHours
                + "-hour timestep,  newStartTime = " + newStartTimeString
                + " newEndTime = " + newEndTimeString);

        mergedDataList = TimeSeriesListMgr.mergeTimeSeriesLists(freshDataList,
                cachedPrecipTimeSeriesList, newStartTime, newEndTime);
    ...
    // ------------------------------------------------------------------------
    long getNewStartTime(long endTime, int timeStepIntervalInHours,
            int numHoursToPreprocess) {
        long newStartTime = 0;
        long hoursBack = 0;
        ...
        // long millisBackInTime = numPeriods * timeStepIntervalInHours *
        // MILLIS_PER_HOUR;
        ...
        long timeStepIntervalInMillis = timeStepIntervalInHours
                * MILLIS_PER_HOUR;
    ...
    long getNewEndTime(long origEndTime, int timeStepIntervalInHours) {
        ...
        if (timeStepIntervalInHours != 24) {
            // System.out.println(header + "orig end time was " +
            // DbTimeHelper.getDateTimeStringFromLongTime(origEndTime));
            endTime = TimeHelper.truncateTimeInMillisToNearestHour(origEndTime,
                    timeStepIntervalInHours);
            endTime += timeStepIntervalInHours * MILLIS_PER_HOUR;
        }

        // System.out.println(header + "new time is " +
        // DbTimeHelper.getDateTimeStringFromLongTime(endTime));
        else // if (timeStepIntervalInHours == 24)
        {
            long twelveHourEndTime = TimeHelper
                    .truncateTimeInMillisToNearestHour(origEndTime, 12);
            long twentyFourHourEndTime = TimeHelper
                    .truncateTimeInMillisToNearestHour(origEndTime, 24);
    ...
    // -------------------------------------------------------------------------
    public void writeIrregularObsDataToFile(List obsTimeSeriesList,
            String fileName) {
        PDCFileWriter writer = new PDCFileWriter(_locationHashMap, _shefDurMap,
                _shefDurToDurcodeMap, _preprocessedFilePath);
        writer.writeIrregularObsDataToFile(obsTimeSeriesList, fileName);
    }

    public void writeRegularObsDataToFile(List obsTimeSeriesList,
            String fileName) {
        PDCFileWriter writer = new PDCFileWriter(_locationHashMap, _shefDurMap,
                _shefDurToDurcodeMap, _preprocessedFilePath);
    ...
    public void testPDCFiles() {
        ...
        PDCFileReaderSlow reader = new PDCFileReaderSlow(fullFilePath,
                _shefDurToDurcodeMap);
