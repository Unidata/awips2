...
public class PDCPreprocessor {
    ...
    private String _preprocessedFilePath = null; // directory where the
                                                 // processed files are placed
                                                 // in

    private String _logDirName = null; // directory where the log will be placed
                                       // in
    ...
    /**
     * initialize( String[] args ) - Used to process the command line arguments
    ...
    private void initialize(String[] args) {
        ...
        StringBuffer baseConnectionStringBuffer = new StringBuffer(); // gotten
                                                                      // from
                                                                      // the
                                                                      // command
                                                                      // line
        ...
        _logger.log("Starting PDC Preprocessor - "
                + DbTimeHelper.getDateTimeStringFromLongTime(System
                        .currentTimeMillis()));

        _logger.log(header + "JDBC connection string = "
                + baseConnectionStringBuffer.toString());

        _dataMgr = new PDCPreprocessorDataMgr(baseConnectionStringBuffer
                .toString(), _logger, _preprocessedFilePath, _lowerWindow,
                _upperWindow, _debug);
    ...
    /**
     * processObsData() - Processes the Observed data and calls appropriate
     * methods from the Data Manager to write the data out to the .dat files.
     */
    public void processObsData() {
        ...
        _dataMgr.writeIrregularObsDataToFile(_instantPrecipTimeSeriesList,
                "PrecipInstant");

        _dataMgr.writeRegularObsDataToFile(_hourlyPrecipTimeSeriesList,
                "Precip1Hour");
        _dataMgr.writeRegularObsDataToFile(_threeHourPrecipTimeSeriesList,
                "Precip3Hour");
        _dataMgr.writeRegularObsDataToFile(_sixHourPrecipTimeSeriesList,
                "Precip6Hour");
        _dataMgr.writeRegularObsDataToFile(_twentyFourHourPrecipTimeSeriesList,
                "Precip24Hour");

        writePrecipDataFileTimer
                .stop("Writing all of the precip data files took");
    }
    ...
    static public void printFreeMemory(String message) {
        ...
        System.out.println(message + " Free memory = " + freeMemory
    ...
    private void handleHeightData() {
        ...
        _heightTimeSeriesList = _dataMgr
                .getNonPrecipObsTimeSeriesListUsingCache("Height",
        ...
        addMissingRiverTimeSeries(_heightTimeSeriesList,
                getRegularObsTimeSeriesDescriptor("HG", "RZ", "Z", (short) 0,
                        getNonPrecipEndTime(), _heightHours, true), true);
        ...
        _dataMgr.writeRegularObsDataToFile(_depthAboveFloodStageTimeSeriesList,
                "HeightAboveFloodStage");
    ...
    private void handleFlowAndStorageData() {
        _lakeTimeSeriesList = _dataMgr.getNonPrecipObsTimeSeriesListUsingCache(
                "Lake", getNonPrecipEndTime(), _heightHours);
        _dischargeTimeSeriesList = _dataMgr
                .getNonPrecipObsTimeSeriesListUsingCache("Discharge",
                        getNonPrecipEndTime(), _heightHours);
        // addMissingTimeSeries( _lakeTimeSeriesList,
        // getRegularObsTimeSeriesDescriptor( "LS", "RZ", "Z", (short) 0,
        // getNonPrecipEndTime(), _heightHours ), true );

        // adds discharge data that was derived from height data, as needed (if
        // there was no data for it)
        incorporateConvertedHeightData(_heightDerivedDischargeTimeSeriesList,
                _dischargeTimeSeriesList);

        addMissingTimeSeries(_dischargeTimeSeriesList,
                getRegularObsTimeSeriesDescriptor("QR", "RZ", "Z", (short) 0,
                        getNonPrecipEndTime(), _heightHours, true), true);
        // addMissingTimeSeries( _dischargeTimeSeriesList,
        // getRegularObsTimeSeriesDescriptor( "QT", "RZ", "Z", (short) 0,
        // getNonPrecipEndTime(), _heightHours, true ), true );
        ...
        _dataMgr.writeRegularObsDataToFile(_flowStorageTimeSeriesList,
                "FlowStorage");
        ...
        _dataMgr.writeRegularObsDataToFile(_percentFloodFlowTimeSeriesList,
                "PercentFloodFlow");
    ...
    private void incorporateConvertedHeightData(
            List derivedDischargeTimeSeriesList, List dischargeTimeSeriesList) {
        ...
        for (int i = 0; i < derivedDischargeTimeSeriesList.size(); i++) {
            RegularObsTimeSeries derivedDischargeTimeSeries = (RegularObsTimeSeries) derivedDischargeTimeSeriesList
                    .get(i);

            RegularObsTimeSeriesDescriptor derivedDescriptor = derivedDischargeTimeSeries
                    .getDescriptor();

            String derivedLid = derivedDescriptor.getLid();

            boolean foundMatch = false;
            for (int j = lastFoundIndex; !foundMatch
                    && j < dischargeTimeSeriesList.size(); j++) {
                RegularObsTimeSeries dischargeTimeSeries = (RegularObsTimeSeries) dischargeTimeSeriesList
                        .get(j);

                RegularObsTimeSeriesDescriptor descriptor = dischargeTimeSeries
                        .getDescriptor();
            ...
            if (!foundMatch) // it wasn't in the list, so add it to the list to
                             // add later
            // we don't want to screw up the iteration through the
            // dischargeTimeSeriesList
            {
        ...
        for (int i = 0; i < timeSeriesToAddList.size(); i++) {
            RegularObsTimeSeries timeSeries = (RegularObsTimeSeries) timeSeriesToAddList
                    .get(i);
    ...
    private void handleTemperatureData() {
        ...
        dbTimer.start();
        _temperatureTimeSeriesList = _dataMgr
                .getNonPrecipObsTimeSeriesListFromDb("Temperature",
                        getNonPrecipEndTime(), _temperatureHours);
        ...
        addMissingTimeSeries(_temperatureTimeSeriesList,
                getRegularObsTimeSeriesDescriptor("TA", "RZ", "X", (short) 0,
                        getNonPrecipDailyEndTime(), _temperatureHours, false),
                false);
        ...
        _dataMgr.writeRegularObsDataToFile(_minTempTimeSeriesList,
                "TemperatureMin");
        _minTempTimeSeriesList = null;
        System.gc();
        printFreeMemory(header + "after  _minTempTimeSeriesList = null");

        _dataMgr.writeRegularObsDataToFile(_maxTempTimeSeriesList,
                "TemperatureMax");
        ...
        _dataMgr.writeRegularObsDataToFile(_deltaTempTimeSeriesList,
                "TemperatureDelta");
        _dataMgr.writeRegularObsDataToFile(_deltaDewPointTimeSeriesList,
                "DewPointDelta");
    ...
    private void handleWindData() {
        ...
        windTimer.start();
        _windTimeSeriesList = _dataMgr.getNonPrecipObsTimeSeriesListUsingCache(
                "Wind", getNonPrecipEndTime(), _windHours);
        ...
        _dataMgr.writeRegularObsDataToFile(_windSpeedTimeSeriesList,
                "WindSpeed");
        _dataMgr.writeRegularObsDataToFile(_windDirectionTimeSeriesList,
                "WindDirection");
    ...
    private void handleSnowData() {
        ...
        snowTimer.start();
        _snowTimeSeriesList = _dataMgr.getNonPrecipObsTimeSeriesListUsingCache(
        ...
        _dataMgr.writeRegularObsDataToFile(_snowWaterTimeSeriesList,
                "SnowWaterEquivalent");
        _dataMgr.writeRegularObsDataToFile(_deltaSnowWaterTimeSeriesList,
                "SnowWaterEquivalentDelta");

    }

    private void handleWeatherData() {
        _weatherTimeSeriesList = _dataMgr
                .getNonPrecipObsTimeSeriesListUsingCache("Weather",
        ...
        _dataMgr.writeRegularObsDataToFile(_relativeHumidityTimeSeriesList,
                "RelativeHumidity");
    ...
    private void addAllMissingTimeSeries() {
        addMissingTimeSeries(_heightTimeSeriesList,
                getRegularObsTimeSeriesDescriptor("HG", "RZ", "Z", Short
                        .parseShort("0"), getNonPrecipEndTime(), _heightHours,
                        true), true);
        addMissingTimeSeries(_temperatureTimeSeriesList,
                getRegularObsTimeSeriesDescriptor("TA", "RZ", "Z", Short
                        .parseShort("0"), getNonPrecipEndTime(),
                        _temperatureHours, true), true);
        addMissingTimeSeries(_temperatureTimeSeriesList,
                getRegularObsTimeSeriesDescriptor("TA", "RZ", "X", Short
                        .parseShort("0"), getNonPrecipEndTime(),
                        _temperatureHours, false), false);
        addMissingTimeSeries(_temperatureTimeSeriesList,
                getRegularObsTimeSeriesDescriptor("TA", "RZ", "N", Short
                        .parseShort("0"), getNonPrecipEndTime(),
                        _temperatureHours, false), false);
        addMissingTimeSeries(_temperatureTimeSeriesList,
                getRegularObsTimeSeriesDescriptor("TD", "RZ", "Z", Short
                        .parseShort("0"), getNonPrecipEndTime(),
                        _temperatureHours, true), true);
        addMissingTimeSeries(_windTimeSeriesList,
                getRegularObsTimeSeriesDescriptor("UD", "RZ", "Z", Short
                        .parseShort("0"), getNonPrecipEndTime(), _windHours,
                        true), true);
        addMissingTimeSeries(_windTimeSeriesList,
                getRegularObsTimeSeriesDescriptor("US", "RZ", "Z", Short
                        .parseShort("0"), getNonPrecipEndTime(), _windHours,
                        true), true);
        addMissingTimeSeries(_snowTimeSeriesList,
                getRegularObsTimeSeriesDescriptor("SW", "RZ", "Z", Short
                        .parseShort("0"), getNonPrecipEndTime(), _snowHours,
                        true), true);
        addMissingTimeSeries(_weatherTimeSeriesList,
                getRegularObsTimeSeriesDescriptor("XR", "RZ", "Z", Short
                        .parseShort("0"), getNonPrecipEndTime(), _weatherHours,
                        true), true);
        addMissingTimeSeries(_lakeTimeSeriesList,
                getRegularObsTimeSeriesDescriptor("LS", "RZ", "Z", Short
                        .parseShort("0"), getNonPrecipEndTime(), _heightHours,
                        true), true);
        addMissingTimeSeries(_dischargeTimeSeriesList,
                getRegularObsTimeSeriesDescriptor("QR", "RZ", "Z", Short
                        .parseShort("0"), getNonPrecipEndTime(), _heightHours,
                        true), true);
        addMissingTimeSeries(_dischargeTimeSeriesList,
                getRegularObsTimeSeriesDescriptor("QT", "RZ", "Z", Short
                        .parseShort("0"), getNonPrecipEndTime(), _heightHours,
                        true), true);
    }
    ...
    private void addMissingTimeSeries(List obsTimeSeriesList,
            RegularObsTimeSeriesDescriptor descriptor, boolean isHourly,
            Set lidSet) {
        ...
        if ((descriptor.isMissing() == true)
                && (descriptor.getExtremum().equals("X"))) {
            System.out.println(header + "descriptor =  " + descriptor);
        }

        getLidSetTimer.start();
        partialLidSet = getLidSetFromObsTimeSeriesList(obsTimeSeriesList,
                descriptor.getPe(), descriptor.getExtremum());
        ...
        for (int i = 0; i < missingLidList.size(); i++) {
            ...
            RegularObsTimeSeries obsTimeSeries = createMissingTimeSeries(lid,
                    new RegularObsTimeSeriesDescriptor(descriptor), isHourly);
    ...
    private void addMissingTimeSeries(List obsTimeSeriesList,
            RegularObsTimeSeriesDescriptor descriptor, boolean isHourly) {
        addMissingTimeSeries(obsTimeSeriesList, descriptor, isHourly,
                _completeLidSet);
    ...
    private RegularObsTimeSeries createMissingTimeSeries(String lid,
        ...
        if (isHourly) {
            obsTimeSeries = new RegularObsTimeSeries(descriptor, 1,
                    new HourlyTimeSlotPolicy());
        } else {
            obsTimeSeries = new RegularObsTimeSeries(descriptor, 24,
                    new DailyTimeSlotPolicy());
        }

        /*
         * 
         * for ( long indexTime = descriptor.getStartTime(); indexTime <
         * descriptor.getEndTime(); indexTime +=
         * PDCPreprocessorDataMgr.MILLIS_PER_HOUR ) { TimeValuePair
         * timeValuePair = new TimeValuePair( indexTime,
         * PDCPreprocessorDataMgr.MISSING );
    ...
    // --------------------------------------------------------------------------------------------
    private RegularObsTimeSeriesDescriptor getRegularObsTimeSeriesDescriptor(
            String pe, String ts, String extremum, short duration,
            long endTime, int hoursToProcess, boolean adjustByOneHour) {
        ...
        descriptor.setEndTime(TimeHelper
                .roundTimeInMillisToNearestHour(endTime));
        ...
        descriptor.setStartTime(descriptor.getEndTime()
                - ((hoursBack) * PDCPreprocessorDataMgr.MILLIS_PER_HOUR));

        return descriptor;
    }

    // --------------------------------------------------------------------------------------------
    public void addTimeSeriesToOrderedList(List obsTimeSeriesList,
        ...
        index = Collections.binarySearch(obsTimeSeriesList, newTimeSeries,
                comparator); // -4
    ...
    // --------------------------------------------------------------------------------------------
    public void addTimeSeriesToOrderedList_orig(List obsTimeSeriesList,
            RegularObsTimeSeries newTimeSeries) {
        RegularObsTimeSeries obsTimeSeries = null;
        String newLid = newTimeSeries.getDescriptor().getLid();
        int index = -1;
        ...
        for (int i = 0; i < obsTimeSeriesList.size(); i++) {
            ...
            if (newLid.compareToIgnoreCase(obsTimeSeries.getDescriptor()
                    .getLid()) < 0) {
                index = i;
                found = true;
                break;
        ...
        {
            obsTimeSeriesList.add(index, newTimeSeries);
        } else // descriptorlist is either empty or descriptor belongs at the
               // end of the list
        {
            index = 0;
    ...
    /**
     * 
     * @param obsTimeSeriesList
     *            - list of ObsTimeSeries objects
     * @return Set of lids in the obsTimeSeriesList
     */
    private Set getLidSetFromObsTimeSeriesList(List obsTimeSeriesList,
            String pe, String extremum) {
        Set lidSet = new HashSet();

        for (int i = 0; i < obsTimeSeriesList.size(); i++) {
            RegularObsTimeSeries obsTimeSeries = (RegularObsTimeSeries) obsTimeSeriesList
                    .get(i);
            if ((obsTimeSeries.getDescriptor().getPe().equalsIgnoreCase(pe))
                    && (obsTimeSeries.getDescriptor().getExtremum()
                            .equalsIgnoreCase(extremum))) {
    ...
    /**
     * Takes the temperature Timeseries List and extracts 24 hour min/max, Air
     * temperature, and Dewpoint timeseries
     * 
     */
    private void processTemperatureTimeSeriesList() {
        ...
        if (_temperatureTimeSeriesList != null) {
            for (int i = 0; i < _temperatureTimeSeriesList.size(); i++) {
                RegularObsTimeSeries obsTimeSeries = (RegularObsTimeSeries) _temperatureTimeSeriesList
                        .get(i);
                ...
                if ((pe.equalsIgnoreCase("TA"))
                    ...
                    // descriptor.setStartTime( startTime + 24 *
                    ...
                    _deltaTempTimeSeriesList.add(_dataMgr
                            .getDeltaTimeSeries(obsTimeSeries));

                } else if (pe.equalsIgnoreCase("TD")) {
                    _deltaDewPointTimeSeriesList.add(_dataMgr
                            .getDeltaTimeSeries(obsTimeSeries));
    ...
    /**
     * takes the Snow Timeseries list and extracts all Timeseries with a PE of
     * "SW"
     * 
     */
    private void processSnowTimeSeriesList() {
        ...
        if (_snowTimeSeriesList != null) {
            for (int i = 0; i < _snowTimeSeriesList.size(); i++) {
                RegularObsTimeSeries obsTimeSeries = (RegularObsTimeSeries) _snowTimeSeriesList
                        .get(i);
                ...
                if (pe.equalsIgnoreCase("SW")) // Snow Water equivalent
                {
                    ...
                    _deltaSnowWaterTimeSeriesList.add(_dataMgr
                            .getDeltaTimeSeries(obsTimeSeries));
                }
            }
        }
    ...
    /**
     * processFlowStorageTimeSeriesList() - Processes the
     * dischargeTimeSeriesList and retrieves the valid flowStorage timeseries
     * 
     */
    private void processFlowStorageTimeSeriesList() {
        ...
        if (_dischargeTimeSeriesList != null) {
            for (int i = 0; i < _dischargeTimeSeriesList.size(); i++) {
                RegularObsTimeSeries obsTimeSeries = (RegularObsTimeSeries) _dischargeTimeSeriesList
                        .get(i);
                ...
                if ((pe.equalsIgnoreCase("QR")) || (pe.equalsIgnoreCase("QT"))) {
                    addTimeSeriesToOrderedList(_flowStorageTimeSeriesList,
                            obsTimeSeries);
        ...
        if (_lakeTimeSeriesList != null) {
            for (int i = 0; i < _lakeTimeSeriesList.size(); i++) {
                RegularObsTimeSeries obsTimeSeries = (RegularObsTimeSeries) _lakeTimeSeriesList
                        .get(i);
    ...
    /**
     * takes the Weather Timeseries list and extracts all Timeseries with a PE
     * of "XR" (Relative Humidity)
     * 
     */
    private void processWeatherTimeSeriesList() {
        ...
        if (_weatherTimeSeriesList != null) {
            for (int i = 0; i < _weatherTimeSeriesList.size(); i++) {
                RegularObsTimeSeries obsTimeSeries = (RegularObsTimeSeries) _weatherTimeSeriesList
                        .get(i);
    ...
    /**
     * Takes the Wind Timeseries and extracts the Wind Speed and Wind Direction
     * Time Series
     * 
     */
    private void processWindTimeSeriesList() {
        ...
        if (_windTimeSeriesList != null) {
            for (int i = 0; i < _windTimeSeriesList.size(); i++) {
                RegularObsTimeSeries obsTimeSeries = (RegularObsTimeSeries) _windTimeSeriesList
                        .get(i);
    ...
    /**
     * Takes all of the Height Timeseries and divides it into the stage/pool and
     * depth_above_flood_stage timeserieslist. Also creates a flowstorage
     * timeseries list
     * 
     */
    private void processHeightTimeSeriesList() {
        ...
        if (_heightTimeSeriesList != null) {
            for (int i = 0; i < _heightTimeSeriesList.size(); i++) {
                RegularObsTimeSeries obsTimeSeries = (RegularObsTimeSeries) _heightTimeSeriesList
                        .get(i);
                pe = obsTimeSeries.getDescriptor().getPe();

                if (pe.equalsIgnoreCase("HG") || pe.equalsIgnoreCase("HP")
                        || pe.equalsIgnoreCase("HT")) {
                    _stagePoolTimeSeriesList.add(obsTimeSeries);
                    _depthAboveFloodStageTimeSeriesList.add(_dataMgr
                            .getDepthAboveFSTimeSeries(obsTimeSeries));
                    derivedDischargeTimeSeries = _dataMgr
                            .getDischargeTimeSeriesFromHeightTimeSeries(obsTimeSeries);
                    if (derivedDischargeTimeSeries != null) {
                        _heightDerivedDischargeTimeSeriesList
                                .add(derivedDischargeTimeSeries);
                    }
                }
            }
        }
    ...
    /**
     * Takes the floodStorageTimeSeriesList and creates the
     * percentFloodFlowTimeSeriesList
     * 
     */
    private void processFloodFlowPercentageTimeSeriesList() {
        ...
        if (_flowStorageTimeSeriesList != null) {
            for (int i = 0; i < _flowStorageTimeSeriesList.size(); i++) {
                RegularObsTimeSeries obsTimeSeries = (RegularObsTimeSeries) _flowStorageTimeSeriesList
                        .get(i);
                if (obsTimeSeries != null) {
                    // pe = obsTimeSeries.getDescriptor().getPe();

                    RegularObsTimeSeries percentFloodFlowTimeSeries = _dataMgr
                            .getPercentFloodFlowTimeSeriesListFromFlowStorageTimeSeries(obsTimeSeries);
    ...
    /**
     * Runs the PDC Precip Preprocessor (C code) and then takes the generated C
     * code and creates the instant/six hour/24 hour precip timeseries lists.
     * 
     */
    private void processPrecipData() {
        if (_precipHours > 0) {
            _dataMgr.executePDCPrecipPP(getPrecipEndTime(), _precipHours);
            _instantPrecipTimeSeriesList = _dataMgr
                    .getInstantaneousPrecipTimeSeriesList(getPrecipEndTime());

            _hourlyPrecipTimeSeriesList = _dataMgr
                    .get1HourPrecipTimeSeriesListFromCFile();
            _threeHourPrecipTimeSeriesList = _dataMgr
                    .get3HourPrecipTimeSeriesListFromCFile();
            _sixHourPrecipTimeSeriesList = _dataMgr
                    .get6HourPrecipTimeSeriesListFromCFile();
            _twentyFourHourPrecipTimeSeriesList = _dataMgr
                    .get24HourPrecipTimeSeriesListFromCFile();
        }
    }

    private void processPrecipDataWithCaching(int numHoursToPreprocess) {
        PrecipListHolder holder = _dataMgr.processPrecipDataWithCaching(
                getPrecipEndTime(), numHoursToPreprocess);
        ...
        _threeHourPrecipTimeSeriesList = holder
                .getThreeHourPrecipTimeSeriesList();
        _sixHourPrecipTimeSeriesList = holder.getSixHourPrecipTimeSeriesList();
        _twentyFourHourPrecipTimeSeriesList = holder
                .getTwentyFourHourPrecipTimeSeriesList();
    }

    /**
     * 
     * @param baseConnectionStringBuffer
    ...
    private void processCommandLineArguments(
            StringBuffer baseConnectionStringBuffer, String[] args) {
        if (args.length < 1) {
            System.err
                    .println("Usage: ohd.hseb.PDCPreprocessor.PDCPreprocessor connection_string logFileDir ");
            System.exit(0);
        }
