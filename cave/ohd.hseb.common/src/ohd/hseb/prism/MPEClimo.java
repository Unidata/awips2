...
public class MPEClimo implements MPEConstants
{
   ...
   static public void main ( String args [ ] )
   {
      ...
      // The user must supply the prefix of the national prism file.  The
      // national prism file is assumed to end with the month number.  For example
      ...
      if (args.length < 2 )
      {
    	  System.out.println( "Must provide the prefix of the input prism file");
    	  System.out.println( "and the cropped, output prism file.");
    	  System.out.print ( "Usage: MPEclimo input_prism_file_prefix ");
    	  System.out.println ("output_prism_file_prefix <t|p>");
    	  System.exit(1);
      }
      
      // The user may suppy a flag indicating whether temperature or
      // precipitation PRISM xmrg files are to be created.
      if ( args.length == 3 )
      {
    	  if ( args [ 2 ].charAt(0) == 't' || args [ 1 ].charAt(0) == 'T')
    	  {
      ...
      try
      {
         ...
         for ( int month = 0; month < month_array.length; ++ month )
         {
            ...
            file = prism_output_path + "/" + args[1] + "_"+ month_name_array [ month ];
