 ...
 **/
package com.raytheon.uf.viz.vtec;
 ...
 *                                  Added getNextEtn method with parameter to
 *                                  specify activeTableMode
 ...
 * May 23, 2019  20825    ryu       Added regex to extract segments from VTEC product.
 *
 * </pre>
 *
 * @author bwoodle
...
public class VtecUtil {
    ...
    public static final Pattern SEGMENT_REGEX = Pattern.compile(
            "\n\n([A-Z]{2}[CZ][0-9]{3})[->](?:[A-Z0-9\\->]*\n)+(?:[^\\$]*\n)+\\$\\$");
    ...
    /**
     * Parses the specified product text for all VTEC lines and returns them in
     * the form of {@link VtecObject} instances.
     *
     * @param message
     *            The product text.
     * @return A {@code Collection} of all the {@code VtecObject} instances that
     *         were found in the supplied product text.
     */
    public static Collection<VtecObject> parseMessageForAllVtec(
            String message) {
        Collection<VtecObject> rval = new ArrayList<>();
        Matcher m = VTEC_REGEX.matcher(message);
        while (m.find()) {
            rval.add(new VtecObject(m.group()));
        }
