 ...
 * Sep  5, 2018  7039        tgurney     Make file name handling more robust
 * </pre>
 *
 *
 * @author Chip Gobs
...
public class PostAnalysisManager {
    ...
    private static final Pattern XMRG_FILE_NAME_PATTERN = Pattern
            .compile("xmrg[0-9]+z");

    private static Map<String, NamedColorUseSet> colorUseSetMap = new HashMap<>();
    ...
    public static String getSelectedDateString() {
        ...
        handler.debug(
                header + " my reformatted selected Date = " + dateString2);
    ...
    public long getTimeFromFileName(String fileName) {
        long longTime = -1;
        if (fileName != null) {
            String timeString = fileName.substring(4, 14);
            Date shiftedDate = null;
            SimpleDateFormat utcSdf2 = new SimpleDateFormat(
                    HydroConstants.QPE_DATE_FORMAT.toPattern());
            ...
            try {
                ...
                handler.warn(
                        "Failed to parse time from file name: " + fileName);
            }
    ...
    private class FileNameComparator implements Comparator<String> {
        ...
        private boolean useAscendingOrder = true;
        ...
        public int compare(String o1, String o2) {
            ...
            long time1 = getTimeFromFileName(fileName1);
            long time2 = getTimeFromFileName(fileName2);
    ...
    public List<String> getListOfAvailableXmrgFiles() {
        ...
        List<String> filteredFileNameList = filterFileNames(fileNameArray);
    ...
    private List<String> filterFileNames(String[] fileNameArray) {
        ...
        for (String element : fileNameArray) {
            if (!XMRG_FILE_NAME_PATTERN.matcher(element).matches()) {
                continue;
            }
            long fileTime = getTimeFromFileName(element);
            if ((fileTime > startTime) && (fileTime <= endTime)) {
                filteredFileNameList.add(element);
            }
        }
    ...
    public double computeObservedWeight(int i, int j, double logRHat,
        ...
        /*
         * Find the distance to the nearest precip station from this HRAP bin at
         * [i][j]
         */
        ...
        return Math.exp(estimatedScale * logRHat * nearestDistance);
    ...
    private void printDimensions(String text, double[][][] threeDArray) {
        ...
        String message = String.format(
                text + "array of dimensions [%d][%d][%d]", dim1, dim2, dim3);
    ...
    public double[][] convertTo2DDoubleArray(short[][] precipArray,
        ...
        // xmrg reads from south to north
        if (exchangeXY) {
            ...
            if (flipYOrientation) {
                for (int row = 0; row < rowCount; row++) {
                    ...
                    for (int col = 0; col < colCount; col++) {
                        doubleArray[col][targetRow] = precipArray[row][col];
                    }
                }
            } else {
                for (int row = 0; row < rowCount; row++) {
                    for (int col = 0; col < colCount; col++) {
                        doubleArray[col][row] = precipArray[row][col];
                    }
                }
            }
            ...
            if (flipYOrientation) {
                for (int row = 0; row < rowCount; row++) {
                    ...
                    for (int col = 0; col < colCount; col++) {
                        ...
                        doubleArray[targetRow][col] = precipArray[row][col];
                    }
                }
            } else {
                for (int row = 0; row < rowCount; row++) {
                    for (int col = 0; col < colCount; col++) {
                        doubleArray[row][col] = precipArray[row][col];
                    }
                }
            }
        }
        ...
        if (exchangeXY) {
            ...
            if ((flipYOrientation) && (!flipXOrientation)) {
                ...
                for (int row = 0; row < rowCount; row++) {
                    for (int col = 0; col < colCount; col++) {
                        ...
                        doubleArray[targetCol][row] = precipArray[row][col]
                                * scaleFactor;
                    }
                }
            } else if ((flipXOrientation) && (flipYOrientation)) {
                for (int row = 0; row < rowCount; row++) {
                    ...
                    for (int col = 0; col < colCount; col++) {
                        int targetCol = (colCount - 1) - col;
                        doubleArray[targetCol][targetRow] = precipArray[row][col]
                                * scaleFactor;
                    }
                }
            } else {
            ...
            if ((flipYOrientation) && (!flipXOrientation)) {
                ...
                for (int row = 0; row < rowCount; row++) {
                    ...
                    for (int col = 0; col < colCount; col++) {
                        int targetCol = (colCount - 1) - col;
                        doubleArray[row][targetCol] = precipArray[row][col]
                                * scaleFactor;
                    }
                }
            } else if ((flipXOrientation) && (flipYOrientation)) {
                for (int row = 0; row < rowCount; row++) {
                    ...
                    for (int col = 0; col < colCount; col++) {
                        int targetCol = (colCount - 1) - col;
                        doubleArray[targetRow][targetCol] = precipArray[row][col]
                                * scaleFactor;
                    }
                }
            } else {
    ...
    public double[][] readGridData(String filePath, boolean flipYOrientation,
        ...
        try {
            ...
            if (fileLength > 0) {
                file1.load();
                this.extent = file1.getHrapExtent();
                ...
                handler.debug(
                        "Zero length xmrg file: " + file1.getFile().getPath());
            }
        } catch (IOException e) {
            handler.warn("Failed to read grid data from xmrg file: "
                    + file1.getFile().getPath(), e);
        }
    ...
    public double[][] readAsciiXmrgGridData(String filePath,
        ...
        // units as read from the file are hundredths of inches
    ...
    private boolean[][] getMisBin() throws Exception {
         ...
         * checks RadClim, radar climatology to determine if there are
         * sufficient radar returns for specific bins under the coverage
         * umbrella In this PostAnalysis version of the misbin concept, EVERY
         * bin under the theoretical coverage umbrella is considered to be
         * covered
         */
        ...
        // 66 is approximately half the distance of the 131 * 131 radar grid
        final int maxDistanceSquared = 66 * 66;
        ...
        if (haveRadarLoc) {
            ...
            /*
             * don't have a point array, so set the misbin values all to true
    ...
    public Coordinate[] getRadarLatLonCoordinateArray() {
        Coordinate[] radarCoordinateArray = null;
        ...
        try {
            ...
            for (int i = 0; i < rs.size(); i++) {
                ...
                try {
                    ...
                    handler.debug(
                            "Invalid lat/lon in radar coordinate array (lat: "
                                    + Objects.toString(lat) + " , lon: "
                                    + Objects.toString(lon) + ")",
                            e);
                    lat = 0.0;
                    lon = 0.0;
                }
            ...
            handler.warn("Failed to get radar coordinate array", e);
    ...
    private Point getRadarPointFromLatLon(double lat, double lon)
        ...
        Coordinate hrapCoordinate = HrapUtil
                .latLonToHrap(new Coordinate(lon, lat));
    ...
    public static NamedColorUseSet createNamedColorUseSet(String colorUseName) {
        ...
        int duration = TimeUtil.SECONDS_PER_DAY;
        ...
        NamedColorUseSet namedColorUseSet1 = new NamedColorUseSet(colorUseName,
                colorUseName, thresholdValues, colorNameArray, missingColorName,
                lowValueColorName, duration);
     ...
     *
     * @param accum24HourPrecip
     *            reverse order MAXY, MAXX
     * @param gageGrid
     * @param mergeArray
     * @param ratioArray
     * @param disaggArray
     * @param estimatedScale
     * @param rhat
     * @throws Exception
     */
    public void mergeData(double accum24HourPrecip[][], double gageGrid[][],
            double mergeArray[][], double ratioArray[][],
        ...
        for (i = 0; i < MAXX; i++) {
            for (j = 0; j < MAXY; j++) {
                ...
                if (gageOnlyGridValueInInches < 0.0000001
                        && qpeInches < 0.0000001) {
                    // if both are 0, don't convert, since already 0.0
                    mergeArray[i][j] = 0.0;
                } else if (!misbin[j][i]) {
                    /*
    ...
    public float getRhat() throws Exception {
        ...
        handler.debug(
                "PostAnalysisManager.getRhat():  = " + parameters.getRhat());
