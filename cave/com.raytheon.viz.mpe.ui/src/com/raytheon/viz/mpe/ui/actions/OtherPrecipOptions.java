...
/**
 * Precip options used by variousparts of MPE.NNN
 * 
 * <pre>
 * 
 * SOFTWARE HISTORY
 ...
 *                                     has been resolved.
 * May 10, 2018 7131       mduff       Moved code to MPEDisplayManager.
...
public class OtherPrecipOptions {
    ...
    public void chg_precip_time(int data) {
        ...
        if (MPEDisplayManager.pcpn_time_step == 0) {
            timePos = DailyQcUtils.pcpn_time;
        } else {
            timePos = 4;
        }
        ...
        if (MPEDisplayManager.pcpn_time_step == 0) {
            timePos = DailyQcUtils.pcp_flag;
        } else {
            timePos = 40 + DailyQcUtils.pcpn_day;
        }
        ...
        if (DailyQcUtils.pcp_in_use[timePos] == -1) {
            QcPrecipOptionsDialog.dataSet.clear();
        ...
        if (points_flag == 1 && DailyQcUtils.pcp_in_use[timePos] == -1) {
            i = 0;
        ...
        timePos = 100 + DailyQcUtils.pcp_flag;
        ...
        if (MPEDisplayManager.pcpn_time_step == 1) {
            ...
            if ((DailyQcUtils.pcp_flag != 0
                    && (DailyQcUtils.pcp_in_use[timePos] == 1
                            || DailyQcUtils.pcp_in_use[timePos - 1] == 1))
                    || (DailyQcUtils.pcp_flag == 0
                            && DailyQcUtils.pcp_in_use[timePos] != -1)) {
        ...
        long offset = (DailyQcUtils.pcpn_day * SECONDS_PER_DAY);
        ...
        if (clientdata == 0) {
            ...
            if (DailyQcUtils.pdata[DailyQcUtils.pcpn_day].used[4] != 0) {
                ...
                dbuf = String.format("%s%s_%04d%02d%02d", dqc.grid_file,
                        DailyQcUtils.TIME_FILE[2][4], tmtime.get(Calendar.YEAR),
            ...
            for (int m = 0; m < 4; m++) {
                ...
                if (m < 2) {
                    old_time = DailyQcUtils.pdata[DailyQcUtils.pcpn_day].data_time;
                    tmtime.setTime(old_time);
                    tmtime.add(Calendar.SECOND, -SECONDS_PER_DAY);
                } else {
                    old_time = DailyQcUtils.pdata[DailyQcUtils.pcpn_day].data_time;
                    tmtime.setTime(old_time);
                }
                ...
                dbuf = String.format("%s%s_%04d%02d%02d", dqc.grid_file,
                        DailyQcUtils.TIME_FILE[2][m], tmtime.get(Calendar.YEAR),
                        tmtime.get(Calendar.MONTH) + 1,
                        tmtime.get(Calendar.DAY_OF_MONTH));
            ...
            for (m = 0; m < 4; m++) {
                ...
                dbuf = String.format("%s%s_%04d%02d%02d", dqc.zgrid_file,
                        DailyQcUtils.Z_TIME_FILE[dqcTimeStringIndex][m],
            ...
            if (DailyQcUtils.tdata[DailyQcUtils.pcpn_day].used[4] != 0) {
                ...
                dbuf = String.format("%s%s_%04d%02d%02d", dqc.tgrid_file,
                        DailyQcUtils.T_TIME_FILE[dqcTimeStringIndex][4],
                        tmtime.get(Calendar.YEAR),
                        tmtime.get(Calendar.MONTH) + 1,
                        tmtime.get(Calendar.DAY_OF_MONTH));
            ...
            if (DailyQcUtils.tdata[DailyQcUtils.pcpn_day].used[5] != 0) {
                ...
                dbuf = String.format("%s%s_%04d%02d%02d", dqc.tgrid_file,
                        DailyQcUtils.T_TIME_FILE[dqcTimeStringIndex][5],
                        tmtime.get(Calendar.YEAR),
                        tmtime.get(Calendar.MONTH) + 1,
                        tmtime.get(Calendar.DAY_OF_MONTH));
            ...
            for (int m = 0; m < 4; m++) {
                ...
                dbuf = String.format("%s%s_%04d%02d%02d", dqc.tgrid_file,
                        DailyQcUtils.T_TIME_FILE[dqcTimeStringIndex][m],
                        tmtime.get(Calendar.YEAR),
                        tmtime.get(Calendar.MONTH) + 1,
                        tmtime.get(Calendar.DAY_OF_MONTH));
            ...
            timePos = 150 + DailyQcUtils.pcp_flag;
