...
public class BadGagesDlg extends AbstractMPEDialog {
    ...
    private void populateGageList() {
        java.util.List<String> dataArray = MPEDataManager.getInstance()
                .readBadGageList();
        numBadGages = dataArray.size();
    ...
    private void okAction() {
        ...
        if (numBadGages == 0) {
            return;
        } else if (deleteGages.isEmpty()) {
            MessageBox msg = new MessageBox(this.getShell(), SWT.ICON_WARNING);
            msg.setText("No Bad Gages");
            msg.setMessage("There are no gages marked for deletion.");
            msg.open();
            return;
        }
