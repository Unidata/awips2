 ...
 * Jan 27, 2025 2038346    mapeters    Remove single-pol fields, consolidate
...
public class PrecipFieldsPopulator extends FieldsPopulator {
    ...
    private static final ILocalizationPathObserver appsDefaultsObserver = file -> {
        synchronized (LOCK) {
            initialize();

            /*
             * Clear the manager's contribution items so that they are reloaded
    ...
    private static void initialize() {
        synchronized (LOCK) {
            ...
            // Create textMap
            Map<DisplayFieldData, MenuData> textMapTemp = new LinkedHashMap<>();
            ...
            menuItems = textMapTemp.keySet().toArray(new DisplayFieldData[0]);
            textMap = textMapTemp;
        }
    }
