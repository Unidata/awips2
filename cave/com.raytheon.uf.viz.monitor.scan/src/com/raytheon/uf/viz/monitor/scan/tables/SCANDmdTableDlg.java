 ...
 * Aug 16, 2018  6740     tgurney    Fix alarm clearing
 *
 * </pre>
 *
 * @author lvenable
 */
public class SCANDmdTableDlg extends AbstractTableDlg
        implements IAttributeUpdate, IThresholdUpdate, IDisplayFilterUpdate,
    ...
    private void createTopControls() {
        ...
        linkToFrameChk
                .setSelection(dmdConfigMgr.getScanDmdCfgXML().getLinkToFrame());
        ...
        tipText.append(
                "Deactivate to include in the SCAN table all cells and\n");
        ...
        cwaFilterChk.setSelection(
                dmdConfigMgr.getScanDmdCfgXML().getFilterOption());
        ...
        alarmBtn.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                if (alarmsDlg == null || alarmsDlg.isDisposed()) {
                    ...
                    alarmsDlg.addCloseCallback(new ICloseCallback() {
    ...
    private void displayFilterDialog() {
        if (displayFilterDlg == null || displayFilterDlg.isDisposed()) {
            ...
            displayFilterDlg.addCloseCallback(new ICloseCallback() {
    ...
    private void updateElevation() {
        ...
        elevationLbl.setText(
                "Elevation: " + String.format("%3.1f", currentTrueElevation));
    }
    ...
    private void updateDefineActiveTrendMenu() {
        ...
        SCANConfig scancfg = SCANConfig.getInstance();
        Map<String, String> trendMap = scancfg.getTrendConfigMgr(scanTable)
                .getTrendSetMap();
    ...
    private void displayAttributesDialog() {
        if ((attributeDlg == null || attributeDlg.isDisposed())
                || (attributeDlg.getParent().isDisposed())) {
    ...
    public void sortedColumn(String columnName) {
        if (columnName.compareTo(DMDTable.STRANK.getColName()) == 0) {
            rankBtn.setText(
                    "Rank: " + SCANConfig.getInstance().getDefaultName());
            return;
        }
    ...
    public void notify(IMonitorEvent me) {
        if (me.getSource() instanceof IMonitor) {
            ...
            if ((time != null) && scan.isInstantiated()) {
                ...
                if (!getLinkToFrame(ScanTables.DMD.name())) {
                ...
                if ((currentTime != null) && (currentElevation != null)
                        && currentTime.equals(
                                scan.getMostRecent(scanTable.name(), site)
                                        .getRefTime())
                 ...
                 */
                if (dmdTableComp.timeHeightDisplayed()) {
                    dmdTableComp.updateTimeHeightGraph(scan, site, time);
                }

                // closes the alarm dialog if new data comes in or user switches
                // frame
                if (getLinkToFrame(scanTable.name()) || currentTime.equals(scan
                        .getMostRecent(scanTable.name(), site).getRefTime())) {
                ...
                if ((EditorUtil.getActiveVizContainer() != null)
                    ...
                    if (mgr.getAlertedAlarms(site, scanTable).size() > 0) {
                        alarmBtn.setVisible(
                                mgr.hasUnclearedAlarms(site, scanTable));
                        addAlarmTimer();
                    } else {
                        alarmBtn.setVisible(false);
                    }
