...
public class SCANTableCellData {
    ...
    public SCANTableCellData(ScanTables scanTable, String columnName,
        ...
        if ((this.scanTable == ScanTables.TVS) && (this.columnName
                .compareTo(TVSTable.TYPE.getColName()) == 0)) {
            if (this.cellText.trim().endsWith("TVS") == true) {
                backgroundColor = Display.getCurrent()
                        .getSystemColor(SWT.COLOR_MAGENTA);
                return;
            }
        } else if ((this.scanTable == ScanTables.MESO) && (this.columnName
                .compareTo(MESOTable.TVS.getColName()) == 0)) {
        ...
        } else if ((this.scanTable == ScanTables.DMD) && (this.columnName
                .compareTo(DMDTable.TVS.getColName()) == 0)) {
            ...
            if (scanTable == ScanTables.DMD) {
                this.cellText = scanCfg.getDmdClassification(
                        String.valueOf(Math.round(value)));
            } else {
                this.cellText = scanCfg.getMesoClassification(
                        String.valueOf(Math.round(value)));
            }
            return;
            ...
            if (this.cellText.matches("[0-9.]+") == true) {
                ...
                try {
                    ...
                    value = Double.valueOf(
                            cellText.substring(0, cellText.length() - 1));
