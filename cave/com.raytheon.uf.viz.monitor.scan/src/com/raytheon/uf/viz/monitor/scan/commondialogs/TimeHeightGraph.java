...
public class TimeHeightGraph {
    ...
    private void createTimeLabels() {
        ...
        if ((graphData != null) && !graphData.isEmpty()) {
            ...
            if ((timeList != null) && (timeList.size() > 0)) {
                int secondsFirst = (int) (double) timeList.get(0);
                int seconds = (int) (double) timeList.get(timeList.size() - 1);

                // filter out invalid SAILS entries with times in the past
                if (secondsFirst > seconds) {
                    seconds = secondsFirst;
                }
                if (highSeconds < seconds) {
                    highSeconds = seconds;
                } else if (highSeconds > seconds) {
                    seconds = highSeconds;
                }
    ...
    public void setGraphData(TreeMap<Long, DMDTableDataRow> data) {
        ...
        for (Long l : graphData.keySet()) {
            ...
            // Toss the invalid SAILS times
            DMDTableDataRow tmpData = graphData.get(l);
            ArrayList<Double> times = tmpData.getTimeHeightTimes();
            if ((times != null) && (!times.isEmpty())) {
                Calendar tmpCal = TimeUtil.newGmtCalendar();
                tmpCal.setTimeInMillis(l);
                int secondsFirst = (int) (double) times.get(0);
                int seconds = (int) (double) times.get(times.size() - 1);
                int secPastMid = getSecondsPastMidnight(tmpCal);
                if ((secondsFirst > seconds) && (seconds < secPastMid)) {
                    invalidTimes.add(l);
                    continue;
                }
            }
