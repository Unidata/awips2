 ...
 * Dec 23, 2011  13608    mgamazay  Updated populateIdentCombo so the drop down
 *                                  menu shows the current feature ident instead
 *                                  of being blank.
...
public class TimeHeightDlg extends CaveSWTDialog implements ITimeHeightInfo {
     ...
     */
    private final NavigableMap<Long, DMDTableDataRow> graphData;
    ...
    private void populateAttributesCombo() {
        ...
        Map<String, String> trendAttrNames = scanCfg
                .getTimeHeightAttributeUnits(scanTable);

        for (Entry<String, String> entry : trendAttrNames.entrySet()) {
            attrList.add(entry.getKey());
            attrCbo.add(entry.getKey() + " (" + entry.getValue() + ")");
        }
    ...
    private void handleIdentAttributeComboAction() {
        ...
        NavigableMap<Long, DMDTableDataRow> data = timeHeightCB
    ...
    public void displayMessage() {
        if (this.msgBox == null || msgBox.isDisposed()) {
            ...
            msgBox.addCloseCallback(new ICloseCallback() {
