 ...
 * Jul 10, 2018 6673       tgurney      Cleanup
 *
 * </pre>
 *
 * @author lvenable
...
public class SCANConfig {
    ...
    private final Color midColor = new Color(Display.getDefault(), 221, 221,
            34);

    private final Color lowerColor = new Color(Display.getDefault(), 222, 222,
            222);

    private final Color defaultColor = new Color(Display.getDefault(), 153, 153,
            153);
    ...
    private final Color rankColor = new Color(Display.getDefault(), 154, 80,
            76);
    ...
    private final Color cellForegroundColor = new Color(Display.getDefault(), 0,
            0, 0);
    ...
    private void init() {
        ...
        setupDmdClassificationMap(dmdConfigMgr
                .getArrtibuteXML(DMDTable.STRANK.getColName()).getUpper());
        setupMesoClassificationMap(mesoConfigMgr
                .getArrtibuteXML(MESOTable.MDASR.getColName()).getUpper());
    }
    ...
    private void setupTrendIntFormatMaps() {
         ...
         * CELL
         */
        cellTrendGraphIntFormatMap = new HashMap<>();
         ...
         * DMD
         */
        dmdTrendGraphIntFormatMap = new HashMap<>();
    ...
    public void setupDmdClassificationMap(double upperVal) {
         ...
         * Classification Map
         */
        dmdClassificationFormatMap = new HashMap<>();
    ...
    public void setupMesoClassificationMap(double upperVal) {
         ...
         * Classification Map
         */
        mesoClassificationFormatMap = new HashMap<>();
    ...
    public boolean displayAsIntTable(ScanTables scanTable, String attrName) {
        if (scanTable == ScanTables.CELL) {
            if (cellIntFormatMap.containsKey(attrName)) {
                return false;
            }
        } else if (scanTable == ScanTables.DMD) {
            if (dmdIntFormatMap.containsKey(attrName)) {
                return false;
            }
        } else if (scanTable == ScanTables.MESO) {
            if (mesoIntFormatMap.containsKey(attrName)) {
                return false;
            }
        } else if (scanTable == ScanTables.TVS) {
            if (tvsIntFormatMap.containsKey(attrName)) {
                return false;
            }
        }
    ...
    public boolean displayAsIntTrend(ScanTables scanTable, String attrName) {
        if (scanTable == ScanTables.CELL) {
            if (cellTrendGraphIntFormatMap.containsKey(attrName)) {
                return false;
            }
        } else if (scanTable == ScanTables.DMD) {
            if (dmdTrendGraphIntFormatMap.containsKey(attrName)) {
                return false;
            }
        }
    ...
    public Color getColorThresholdDialogColor(ScanTables scanTable,
         ...
         */
        if (cfgMgr.getArrtibuteXML(colName).getColored()) {
    ...
    public boolean[] getVisibleColumns(ScanTables scanTable) {
        ...
        List<SCANAttributesXML> columnData = getAttributes(scanTable);
     ...
     */
    public List<SCANAttributesXML> getAttributes(ScanTables scanTable) {
        AbsConfigMgr cfgMgr = getAbsConfigMgr(scanTable);
     ...
     * @param scanTable
     *            Scan table.
     * @return Map of column names that are available (visible).
     */
    public Map<String, Object> getAvailRankColumns(ScanTables scanTable) {
        Map<String, Object> availRankMap = new HashMap<>();
    ...
    public int getDefaultRankColumnIndex(ScanTables scanTable) {
        String defaultRank = getDefaultRank(scanTable);
        if (defaultRank != null && !defaultRank.isEmpty()
                && !DEFAULT_NAME.equalsIgnoreCase(defaultRank)) {
            switch (scanTable) {
            case CELL:
                CELLTable ct = CELLTable.valueOf(defaultRank.toUpperCase());
                return ct.ordinal();
            case DMD:
                DMDTable dt = DMDTable.valueOf(defaultRank.toUpperCase());
                return dt.ordinal();
            case MESO:
                MESOTable mt = MESOTable.valueOf(defaultRank.toUpperCase());
                return mt.ordinal();
            case TVS:
                TVSTable tt = TVSTable.valueOf(defaultRank.toUpperCase());
                return tt.ordinal();
            }
        }
        return -1;
    ...
    public StormCellConfig getStormCellConfig() {
        ...
        StormCellConfig scanDisplayCfg = new StormCellConfig(symsCircleHigh,
                ...
                radHigh, arrowMode, arrowConversion, attrName, upperVal, midVal,
                lowerVal, linkToFrame);
