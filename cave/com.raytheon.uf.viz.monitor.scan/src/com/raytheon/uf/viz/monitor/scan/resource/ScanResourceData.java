 ...
 * Dec 04, 2013  2592     lvenable  Added check to ensure the PluginDataObject
 *                                  array has at least one element.
 * Aug 01, 2018  6685     randerso  Fix NPE when DMD table is loaded with no
 *                                  DMD records in the database.
 *
 * </pre>
 *
 * @author dhladky
...
public class ScanResourceData extends AbstractRequestableResourceData {
    private static final IUFStatusHandler statusHandler = UFStatus
            .getHandler(ScanResourceData.class);
    ...
    public Map<DataTime, ScanRecord> dataObjectMap;
    ...
    protected AbstractVizResource<?, ?> constructResource(
        ...
        if (objects.length > 0) {
            ...
            List<String> uris = getScan()
                    .getAvailableUris(ScanTables.valueOf(tableType), icao);
            try {
                ...
                List<ScanRecord> recordsToLoad = new ArrayList<>(uris.size());
                for (String uri : uris) {
                ...
                if (tableType.equals(ScanTables.DMD.name())) {
                    if (dataObjectMap != null) {
                        for (ScanRecord record : dataObjectMap.values()) {
                            getScan().setTableData(icao, record.getTableData(),
                                    /*
                                     * TODO: This should be the volume scan
                                     * time, but
                                     * {Radar,Scan}Record.getVolScanTime is
                                     * actually the radar product generation
                                     * time.
                                     */
                                    record.getDataTime().getRefTime(),
                                    record.getTilt(),
                ...
                if ((getScan().getDialog(ScanTables.valueOf(tableType),
                        icao) != null)
                        && !getScan()
                    ...
                    if (time != null) {
                        getScan().updateDialog(ScanTables.valueOf(tableType),
                                icao, time.getRefTime(), time.getRefTime(),
    ...
    public void populateRecords(ScanRecord[] records) throws VizException {
        Map<File, Set<ScanRecord>> fileMap = new HashMap<>();
        for (ScanRecord record : records) {
    ...
    public DataTime[] getAvailableTimes() throws VizException {
        if (tableType.equals(ScanTables.DMD.name())) {
            ...
            List<DataTime> dataList = new ArrayList<>();
