...
public class SCANAlarmAlertManager {
     ...
     * @return the instance
     */
    public static synchronized SCANAlarmAlertManager getInstance(String site) {
        if (instance == null) {
    ...
    public void addSite(String site) {
        ...
        if (!alertedAlarmsMap.containsKey(site)) {
            Map<ScanTables, Set<AlertedAlarms>> siteAlertedAlarmsSet = Collections
                    .synchronizedMap(new HashMap<>());
            siteAlertedAlarmsSet.put(ScanTables.DMD, new HashSet<>());
            siteAlertedAlarmsSet.put(ScanTables.CELL, new HashSet<>());
            alertedAlarmsMap.put(site, siteAlertedAlarmsSet);
        }

        if (!idents.containsKey(site)) {
            Map<ScanTables, List<String>> siteIdents = new HashMap<>();
            siteIdents.put(ScanTables.CELL, new ArrayList<>());
            siteIdents.put(ScanTables.DMD, new ArrayList<>());
    ...
    private SCANAlarmAlertManager() {
        ...
        if (scheduledAlarmsMap == null) {
            scheduledAlarmsMap = Collections.synchronizedMap(new HashMap<>());
        }

        if (alertedAlarmsMap == null) {
            alertedAlarmsMap = Collections.synchronizedMap(new HashMap<>());
        }
        ...
        for (ScheduledAlarms alarm : scheduledAlarmsMap.get(site)
            ...
            if (alarm.type == AlarmType.AbsVal) {
                for (SCANTableRowData scanData : data.getTableRows()) {
                    if (!idents.get(site).get(tableType).contains(
                            scanData.getTableCellData(0).getCellText())) {
                        addIdent(site, tableType,
                                scanData.getTableCellData(0).getCellText());
     ...
     * @param alarm
     */
    public void getScanTableDiff(SCANTableData data, ScanTableData<?> otherData,
            ScanTables tableType, int length, ScanDataGenerator sdg,
            ScheduledAlarms alarm, SCANConfig config, Date latestTime) {
        ...
        for (SCANTableRowData currentRow : data.getTableRows()) {
            if (!idents.get(site).get(tableType)
                ...
                addIdent(site, tableType,
                        currentRow.getTableCellData(0).getCellText());
            } else if (containsAlarm(site, tableType, alarm.type, alarm.colName,
                    currentRow.getTableCellData(0).getCellText())) {
                continue;
            }
            // grab the whole row
            ...
            for (SCANTableRowData previousRow : scanOtherData.getTableRows()) {
                ...
                if (currentTableCells[0].getCellText()
                        .equals(previousTableCells[0].getCellText())) {
                    ...
                    for (int i = 0; i < currentRow.getNumberOfCellData(); i++) {
                        ...
                        // tested
                        if (currentTableCells[i].getColumnName()
                                .equals(alarm.colName)) {
    ...
    public void updateScheduledAlarm(String site, ScanTables tableType,
            String name, AlarmType type, double value) {
        ScheduledAlarms alarm = new ScheduledAlarms(name, type, value);
        for (ScheduledAlarms theAlarm : scheduledAlarmsMap.get(site)
                .get(tableType)) {
     ...
     * @param col
     */
    public void addAlertedAlarm(String site, ScanTables tableType, String ident,
            String colName, AlarmType type, int row, int col, Date validTime) {
        Set<AlertedAlarms> alarms = alertedAlarmsMap.get(site).get(tableType);
        if (alarms.isEmpty()) {
    ...
    public boolean hasUnclearedAlarms(String site, ScanTables tableType) {
    ...
    private void clearOldAlarms(String site, ScanTables type, Date latestTime) {
        List<AlertedAlarms> clearList = new ArrayList<>();
    ...
    public class ScheduledAlarms {
        ...
        public ScheduledAlarms(String colName, AlarmType type, double value) {
