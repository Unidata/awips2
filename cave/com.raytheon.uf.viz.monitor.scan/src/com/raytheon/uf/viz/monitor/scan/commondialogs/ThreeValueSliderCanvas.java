...
public class ThreeValueSliderCanvas {
    ...
    private final int CANVAS_HEIGHT = 60;

    /*
     * Bar information.
     */
    private final int barBottomYCoord = 55;
    ...
    private final int barYCoord = 45;
    ...
    /*
     * Mid arrow, label and values
     */
    private Region midRegion;

    private int[] midPtArray = new int[] { 0, 0 };

    private int midArrowXCoord = 100;

    private Rectangle midLblRect;

    private String midStr;

    private final int midLblYCoord = 14;

    private Color midColor;

    private boolean moveMid = false;

    private Double midDisplayVal = 0.0;
    ...
    private final int lowerLblYCoord = 27;
    ...
    public ThreeValueSliderCanvas(Composite parentComp, double min,
            double range, double inc, double startingUpperVal,
            double startingMidVal, double startingLowerVal, ScanTables scanTable) {
        this.parentComp = parentComp;

        display = this.parentComp.getDisplay();

        setValuesNoRedraw(min, range, inc, startingUpperVal, startingMidVal,
                startingLowerVal);
    ...
    private void init(ScanTables scanTable) {
        canvasBgColor = new Color(display, 142, 122, 110);
        upperColor = display.getSystemColor(SWT.COLOR_RED);
        midColor = display.getSystemColor(SWT.COLOR_YELLOW);
        if (scanTable == ScanTables.DMD) {
            lowerColor = display.getSystemColor(SWT.COLOR_GREEN);
        } else {
            lowerColor = display.getSystemColor(SWT.COLOR_WHITE);
        }
        blackColor = display.getSystemColor(SWT.COLOR_BLACK);
        if (scanTable == ScanTables.DMD) {
            subColor = display.getSystemColor(SWT.COLOR_WHITE);
        } else {
            subColor = display.getSystemColor(SWT.COLOR_GRAY);
        }
        ...
        canvas.addMouseListener(new MouseAdapter() {
            ...
            public void mouseDown(MouseEvent e) {
                if (upperRegion.contains(e.x, e.y) == true
                    ...
                    mouseDown = true;
                    moveUpper = true;
                } else if (midRegion.contains(e.x, e.y) == true
                        || midLblRect.contains(e.x, e.y) == true) {
                    mousePt.x = e.x;
                    mousePt.y = e.y;
                    mouseDown = true;
                    moveMid = true;
    ...
    private void drawCanvas(GC gc) {
        ...
        gc.setBackground(subColor);
        gc.fillRectangle(barXCoord, barYCoord, barWidth, barHeight);

        gc.setBackground(lowerColor);
        gc.fillRectangle(lowerArrowXCoord, barYCoord, barWidth + 25
                - lowerArrowXCoord, barHeight);

        gc.setBackground(midColor);
        gc.fillRectangle(midArrowXCoord, barYCoord, barWidth + 25
                - midArrowXCoord, barHeight);

        gc.setBackground(upperColor);
    ...
    private void updateUpperArrow(GC gc) {
        ...
        upperPtArray = new int[] { upperArrowXCoord, 42, upperArrowXCoord + 4,
    ...
    private void updateMidArrow(GC gc) {
        midRegion.subtract(midPtArray);
        midPtArray = new int[] { midArrowXCoord, 42, midArrowXCoord + 4,
                barBottomYCoord, midArrowXCoord - 4, barBottomYCoord };
        midRegion.add(midPtArray);

        gc.setBackground(midColor);
        gc.fillPolygon(midPtArray);
        gc.setForeground(blackColor);
        gc.drawPolygon(midPtArray);
    }

    private void updateMidLabel(GC gc) {
        gc.setForeground(midColor);
        midStr = calcDisplayString(midDisplayVal);
        int lblXCoord = midArrowXCoord - (textWidth * midStr.length() / 2);
        gc.drawString(midStr, lblXCoord, midLblYCoord, true);
        midLblRect.x = lblXCoord;
        midLblRect.y = midLblYCoord;
        midLblRect.width = textWidth * midStr.length();
        midLblRect.height = textHeight;
    }

    private void updateLowerArrow(GC gc) {
        lowerRegion.subtract(lowerPtArray);
        lowerPtArray = new int[] { lowerArrowXCoord, 42, lowerArrowXCoord + 4,
    ...
    private void handleMouseMove(MouseEvent e) {
        ...
        if (moveUpper == true) {
            ...
            if (upperArrowXCoord > 225) {
                ...
                upperArrowXCoord = 25;
            } else if (upperArrowXCoord < midArrowXCoord) {
                upperArrowXCoord = midArrowXCoord;
            }

            upperDisplayVal = calcDisplayValue(upperArrowXCoord);
        } else if (moveMid == true) {
            int xDiff = e.x - mousePt.x;
            midArrowXCoord += xDiff;

            if (midArrowXCoord > 225) {
                midArrowXCoord = 225;
            } else if (midArrowXCoord < 25) {
                midArrowXCoord = 25;
            } else if (midArrowXCoord < lowerArrowXCoord) {
                midArrowXCoord = lowerArrowXCoord;
            } else if (midArrowXCoord > upperArrowXCoord) {
                midArrowXCoord = upperArrowXCoord;
            }

            midDisplayVal = calcDisplayValue(midArrowXCoord);
            ...
            if (lowerArrowXCoord > 225) {
                ...
                lowerArrowXCoord = 25;
            } else if (lowerArrowXCoord > midArrowXCoord) {
                lowerArrowXCoord = midArrowXCoord;
            }
    ...
    private void setValuesNoRedraw(double min, double range, double inc,
            double startingUpperVal, double startingMidVal,
            double startingLowerVal) {
        this.minValue = min;
        this.rangeValue = Math.round(range);
        this.incValue = inc;
        ...
        midArrowXCoord = calcValueToBarXCoord(startingMidVal);
    ...
    public void setValues(double min, double range, double inc,
            double startingUpperVal, double startingMidVal,
            double startingLowerVal) {
        setValuesNoRedraw(min, range, inc, startingUpperVal, startingMidVal,
                startingLowerVal);
