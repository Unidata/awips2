...
/**
 * Provides the metadata and constructor for Radar mosaics.
 * 
 * <pre>
 * 
 * SOFTWARE HISTORY
...
public class RadarMosaicResourceData extends AbstractRequestableResourceData {
    ...
    private class ResourceConstructJob implements Runnable {
        ...
        public void run() {
            try {
                AbstractVizResource subRsc = rp.getResourceData()
                        .construct(rp.getLoadProperties(), descriptor);
    ...
    public AbstractVizResource<?, ?> construct(LoadProperties loadProperties,
        ...
        List<ResourceConstructJob> jobList = new ArrayList<>();
        for (ResourcePair rp : resourceList) {
            rp.getLoadProperties()
                    .overrideCapabilities(loadProperties.getCapabilities());
    ...
    public void update(Object updateData) {
        if (updateData instanceof PluginDataObject[]) {
            Map<AbstractResourceData, List<PluginDataObject>> updates = new HashMap<>();
