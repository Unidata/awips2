...
public class SailsFrameCoordinator extends FrameCoordinator implements
        ...
        if (isEnabled()) {
            ...
            for (RadarDataTime time : volumeScan) {
                if (validator.isValid(time)) {
                    int index = getBestFrameIndex(frames, dataIndex, time,
                            validator);
                    if (index >= 0) {
                        return index;
                    }
     ...
     * @return the index of the frame that should displayed or -1 if none of the
     *         frames in the volume scan at that angle are overlapping with the
     *         previous frame.
     */
    private int getBestFrameIndex(DataTime[] frames, int previousIndex,
         ...
         */
        RadarDataTime startTime = nextByLevel.floor(previousDataTime);
        if (startTime == null) {
            /*
             * There are no times on the new level before the previous time so
             * just start at the previous time.
             */
            startTime = previousDataTime;
        }

        RadarDataTime endRange = previousByLevel.higher(previousDataTime);
        if (endRange == null) {
            /*
             * There is no time after the previous time so extend the range 10
             * minutes which is approximately the length of the longest volume
             * scan.
             */
            Calendar endCal = previousDataTime.getRefTimeAsCalendar();
            endCal.add(10, Calendar.MINUTE);
            endRange = new RadarDataTime(new DataTime(endCal));
        }

        NavigableSet<RadarDataTime> nextSet = nextByLevel.subSet(startTime,
                true, endRange, false);
        if (nextSet.isEmpty()) {
            return -1;
    ...
    private static <T extends DataTime> NavigableSet<T> filterByLevel(
        ...
        NavigableSet<T> filtered = new TreeSet<>(REF_TIME_COMPARATOR);
        for (T time : times) {
