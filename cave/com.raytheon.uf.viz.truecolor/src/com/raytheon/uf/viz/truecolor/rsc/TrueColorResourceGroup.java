                    ...
                    if (error == null) {
                        ...
                        if (ci == null) {
                            ...
                            params.setColorMapMax((float) ci.getRangeMax());
                                ...
                                params.getColorMapUnit())) {
                            params.setDisplayUnit(ci.getUnit());
                            UnitConverter converter = params
                                    .getDisplayToColorMapConverter();
                            params.setColorMapMin((float) converter.convert(ci
                                    .getRangeMin()));
                            params.setColorMapMax((float) converter.convert(ci
                                    .getRangeMax()));
                        } else {
        ...
        for (Channel c : Channel.values()) {
            ...
            if (dcr != null) {
                ...
                if (dcr.resource.hasCapability(ColorMapCapability.class)
                    ...
                    ColorMapParameters cmp = dcr.resource.getCapability(
                    ...
                    if (ri.charAt(0) == ('-'))
                        rirall = "-"
                                + (ri.substring(1) + "x").replaceAll("[^\\d.]",
                                        " ");
                    else
                        rirall = (ri + "x").replaceAll("[^\\d.]", " ");
                    try {
                        double value = Double.parseDouble(rirall.substring(0,
                                rirall.indexOf(" ")));
