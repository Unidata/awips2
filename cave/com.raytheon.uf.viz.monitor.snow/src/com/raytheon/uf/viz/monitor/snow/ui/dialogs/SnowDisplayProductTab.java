 ...
 *
 */
public class SnowDisplayProductTab extends TabItemComp
        implements IUpdateDisplayProduct {
    private SnowDisplayProductEditDlg productEditDlg;

    private List<SnowDisplayProductData> snowDataArray;
    ...
    private void createDataArray() {
        snowDataArray = new ArrayList<>();

        SnowThresholdMgr stm = SnowThresholdMgr.getInstance();

        String xmlKey;

        List<String> areaList = stm.getCfgMgr().getAreaList();
        for (String areaID : areaList) {
            ...
            sdpd.setBlizWrnVisR(
                    stm.getThresholdValue(duKey, threshKeyR, areaID, xmlKey));
            sdpd.setBlizWrnVisY(
                    stm.getThresholdValue(duKey, threshKeyY, areaID, xmlKey));

            xmlKey = SnowDisplay.SNOW_DISP_PROD_BLIZZ_WIND_SPEED.getXmlKey();
            sdpd.setBlizWrnWindSpdR(
                    stm.getThresholdValue(duKey, threshKeyR, areaID, xmlKey));
            sdpd.setBlizWrnWindSpdY(
                    stm.getThresholdValue(duKey, threshKeyY, areaID, xmlKey));

            xmlKey = SnowDisplay.SNOW_DISP_PROD_BLIZZ_GUST_SPEED.getXmlKey();
            sdpd.setBlizWrnGustSpdR(
                    stm.getThresholdValue(duKey, threshKeyR, areaID, xmlKey));
            sdpd.setBlizWrnGustSpdY(
                    stm.getThresholdValue(duKey, threshKeyY, areaID, xmlKey));

            xmlKey = SnowDisplay.SNOW_DISP_PROD_BLIZZ_PEAK_WIND.getXmlKey();
            sdpd.setBlizWrnPeakWindR(
                    stm.getThresholdValue(duKey, threshKeyR, areaID, xmlKey));
            sdpd.setBlizWrnPeakWindY(
                    stm.getThresholdValue(duKey, threshKeyY, areaID, xmlKey));
            ...
            sdpd.setFrzPrecipTempR(
                    stm.getThresholdValue(duKey, threshKeyR, areaID, xmlKey));
            sdpd.setFrzPrecipTempY(
                    stm.getThresholdValue(duKey, threshKeyY, areaID, xmlKey));

            xmlKey = SnowDisplay.SNOW_DISP_PROD_FRZ_HOURLY_PRECIP.getXmlKey();
            sdpd.setFrzPrecipHrlyPrcpR(
                    stm.getThresholdValue(duKey, threshKeyR, areaID, xmlKey));
            sdpd.setFrzPrecipHrlyPrcpY(
                    stm.getThresholdValue(duKey, threshKeyY, areaID, xmlKey));
            ...
            sdpd.setHvySnowSnincrHrR(
                    stm.getThresholdValue(duKey, threshKeyR, areaID, xmlKey));
            sdpd.setHvySnowSnincrHrY(
                    stm.getThresholdValue(duKey, threshKeyY, areaID, xmlKey));

            xmlKey = SnowDisplay.SNOW_DISP_PROD_HSW_SNINCR_TOTAL.getXmlKey();
            sdpd.setHvySnowSnincrTotR(
                    stm.getThresholdValue(duKey, threshKeyR, areaID, xmlKey));
            sdpd.setHvySnowSnincrTotY(
                    stm.getThresholdValue(duKey, threshKeyY, areaID, xmlKey));

            xmlKey = SnowDisplay.SNOW_DISP_PROD_HSW_SNOW_DEPTH.getXmlKey();
            sdpd.setHvySnowDepthR(
                    stm.getThresholdValue(duKey, threshKeyR, areaID, xmlKey));
            sdpd.setHvySnowDepthY(
                    stm.getThresholdValue(duKey, threshKeyY, areaID, xmlKey));
    ...
    protected void editDataAction() {
        ...
        if (productEditDlg == null) {
            productEditDlg = new SnowDisplayProductEditDlg(
                    getParent().getShell(), sdpd, this);
            productEditDlg.open();
            productEditDlg = null;
        }
    }
