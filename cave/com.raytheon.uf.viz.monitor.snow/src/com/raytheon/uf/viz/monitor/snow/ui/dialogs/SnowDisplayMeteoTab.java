 ...
 *
 */
public class SnowDisplayMeteoTab extends TabItemComp
    ...
    protected void populateTable() {
        ...
        for (int i = 0; i < snowDataArray.size(); i++) {
            ...
            item.setText(5, formatTableData(tmpVisStr));

            visVal = sdmd.getVisY();
            tmpVisStr = rangeUtil.getVisString((int) visVal);
            item.setText(6, formatTableData(tmpVisStr));

            /* SLP */
            appendIntData(item, 7, sdmd.getSlpR(), sdmd.getSlpY());

            /* Hourly Precipitation */
            appendDecimalData(item, 9, sdmd.getHrPrecipR(),
                    sdmd.getHrPrecipY());
