...
public class TafSiteInfoEditorDlg extends CaveSWTDialog {
     ...
     */
    private static final String[] START_HOURS = { "00", "06", "12", "18" };
        ...
        saveBtn.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent event) {
                if (validateData()) {
                    ...
                    } catch (ConfigurationException | LocalizationException
                            | IOException e) {
                ...
                if (isSiteIdValid()) {
                    try {
                        ...
                        if (mustCreate(editorDlg)) {
                            ...
                            editorDlg.addCloseCallback(new ICloseCallback() {
        ...
        try {
            for (String runway : runways) {
                ...
                if (runwayInt < 0 || runwayInt > 360) {
                    throw new IllegalArgumentException("runway value "
                            + runwayInt + " not in range 0-360");
                }
            }
        } catch (Exception e) {
            isValid = false;
    ...
    private boolean isRadarHeightValid(String text) {
        ...
        if (text.isEmpty() && radarsTxt.isEmpty()) {
            return true;
        } else if (text.isEmpty() && !radarsTxt.isEmpty()
                || !text.isEmpty() && radarsTxt.isEmpty()) {
            return false;
        }
    ...
    private boolean isProfilerHeightValid(String text) {
        ...
        if (text.isEmpty() && profilersTxt.isEmpty()) {
            return true;
        } else if (text.isEmpty() && !profilersTxt.isEmpty()
                || !text.isEmpty() && profilersTxt.isEmpty()) {
            return false;
        }
    ...
    private boolean isMetarValid() {
        ...
        for (String metar : metars) {
            if (!metar.trim().matches("^[A-Z[0-9]]{4}$")) {
                isValid = false;
                break;
            }
        }
    ...
    private boolean isProfilerValid() {
        ...
        if (profilersTF.getText().isEmpty()) {
            return true;
        }
    ...
    private boolean isVsbyValid() {
        ...
        try {
            for (String vsby : vsbys) {
                ...
                if (d < 0.0 || d > 10.0) {
                    throw new IllegalArgumentException("visibility value " + d
                            + " not in range 0-10");
                }
    ...
    private boolean isCigValid() {
        ...
        try {
            for (String cig : cigs) {
                ...
                if (i < 0 || i > 30_000) {
                    throw new IllegalArgumentException("ceiling value " + i
                            + " not in range 0-30000");
                }
    ...
    private boolean isLatitudeValid() {
        ...
        try {
            ...
            if (lat < -90.0 || lat > 90.0) {
                throw new IllegalArgumentException("latitude value " + lat
                        + " not in range -90 - 90");
            }
        } catch (Exception e) {
            isValid = false;
    ...
    private boolean isLongitudeValid() {
        ...
        try {
            ...
            if (lon < -180.0 || lon > 180.0) {
                throw new IllegalArgumentException("longitude value " + lon
                        + " not in range -180 - 180");
            }
        } catch (Exception e) {
            isValid = false;
        }
