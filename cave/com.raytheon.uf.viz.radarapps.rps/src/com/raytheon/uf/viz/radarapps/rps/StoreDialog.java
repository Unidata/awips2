...
public class StoreDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		ArrayList<String> rpgs = new ArrayList<String>();
		{
			...
			ConfigReply reply = (ConfigReply)
				RadarApps.getRcmSystem().sendCheckedAndHandled(new ReqObj.GetRadarConfig(), 
					getParentShell());
			if (reply == null)
				return null;
		...
		TableViewer fTableViewer = new TableViewer(c, SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL | SWT.BORDER);
        ...
        if (listEditor.getRadarID() != null)
        	fTableViewer.setSelection(new StructuredSelection(Arrays.asList(listEditor.getRadarID())));
		...
		{
			Collection<VCPInfo> allVcps = ElevationInfo.getInstance().getVcpInfo();
			for (VCPInfo vi : allVcps) {
				if (typeRestriction != null && (typeRestriction == RadarType.TDWR) !=
						listEditor.isTdwrVcp(vi.vcp))
					continue;
				vcps.add(vi);
			}
		}
		fTableViewer = new TableViewer(c, SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL | SWT.BORDER);
        fTableViewer.setContentProvider(new ArrayContentProvider());
        fTableViewer.setLabelProvider(new LabelProvider());
        fTableViewer.addSelectionChangedListener(new ISelectionChangedListener() {
        	public void selectionChanged(SelectionChangedEvent event) {
        ...
        if (listEditor.getVcp() != -1) {
        	for (VCPInfo vi : ElevationInfo.getInstance().getVcpInfo()) {
        		if (vi.vcp == listEditor.getVcp()) {
                	fTableViewer.setSelection(new StructuredSelection(Arrays.asList(vi)));
                	break;
        		}
        	}
        }
	...
	protected void okPressed() {
		...
		selectedRadarIDs = new ArrayList<String>((List<String>) sel.toList());		
