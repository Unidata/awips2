...
import com.raytheon.viz.pointdata.rsc.wind.WindPlotRenderable.Barb;
 ...
 * Jun 18, 2018  6601     bsteffen  When similar resources are loaded, only
 *                                  sample the closest.
 * Mar 14, 2019  7713     tjensen   Remove hard coded of wind barb config values
 * </pre>
 *
 * @author bsteffen
 */
public class WindPlotResource
    ...
    public String inspect(ReferencedCoordinate coord) throws VizException {
        ...
        try {
            Coordinate pixelLoc = coord.asPixel(descriptor.getGridGeometry());
            Barb thisBarb = getClosestBarb(pixelLoc);

            ResourceList resourceList = descriptor.getResourceList();
            List<? extends WindPlotResource> similarResources = resourceList
                    .getResourcesByTypeAsType(getClass());
            Map<String, RequestConstraint> thisMM = resourceData
                    .getMetadataMap();
            double thisDist = Double.NEGATIVE_INFINITY;
            if (thisBarb != null) {
                thisDist = pixelLoc.distance(thisBarb.getLocation());
            } else if (similarResources.get(0) == this) {
                return "NO DATA";
            } else {
                return null;
            }
            for (WindPlotResource similarResource : similarResources) {
                if (similarResource == this) {
                    continue;
                }
                Map<String, RequestConstraint> similarMM = similarResource
                        .getResourceData().getMetadataMap();
                if (similarMM.equals(thisMM)) {
                    Barb similarBarb = similarResource.getClosestBarb(pixelLoc);
                    if (similarBarb != null) {
                        double similarDist = pixelLoc
                                .distance(similarBarb.getLocation());
                        if (similarDist < thisDist) {
                            return null;
                        }
                    }
                }
            }
            return thisBarb.getText();
        } catch (TransformException | FactoryException e) {
            throw new VizException("Unable to transform point for inspect.", e);
        }
    }

    protected Barb getClosestBarb(Coordinate pixelLoc) {
        ...
        return frame.getRenderable().getClosestBarb(pixelLoc);
    }
    ...
    protected void loadData(DataTime dataTime, WindPlotRenderable renderable,
        ...
        try {
            ...
            for (int i = 0; i < container.getAllocatedSz(); i += 1) {
                ...
                double magnitude = config.getMagnitude().getNumericValue(view);
