 ...
 * Oct 12, 2016   5631     bkowal      Cleanup. Eliminated warnings.
 * 
 * </pre>
 * 
 * @author mpduff
...
public class HpeLabelResource
        extends AbstractVizResource<HpeLabelResourceData, MapDescriptor>
        ...
        if (type == ChangeType.DATA_REMOVE) {
            if (object instanceof DataTime) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (key.getDate()
                            .equals(((DataTime) object).getRefTime())) {
                        iter.remove();
                    }
                }
            }
        ...
        if (rscList != null) {
            ...
            if (!list.isEmpty()) {
                double[] pixel = paintProps.getView()
                        .getDisplayCoords(new double[] { 125, 100 }, target);
                ...
                for (D2DGridResource rsc : list) {
                    ...
                    if (HpeUtils.isHpe(currentGridRec)) {
                        ...
                        String text = getText(
                                currentGridRec.getDataTime().getRefTime(),
                                currentGridRec.getSecondaryId());
                        if (text != null) {
                            sb.append(text);
                        }
                    }
    ...
    /*
     * TODO: resolve duplication with:
     * com.raytheon.uf.viz.monitor.ffmp.ui.rsc.FFMPResource.HpeSourceDataJob.
     */
