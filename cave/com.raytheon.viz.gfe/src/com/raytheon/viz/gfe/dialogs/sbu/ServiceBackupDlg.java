 ...
 * Mar 20, 2013  1447     dgilling  Port troubleshooting mode changes from A1 DR
 *                                  21404, some code cleanup.
 ...
 * Jun 10,2014   17401    lshi
 * Feb 18, 2015  4300     randerso  Completely rewritten to support multi-site
 *                                  backup
...
public class ServiceBackupDlg extends CaveJFACEDialog
    ...
    public ServiceBackupDlg() throws GFEException {
        ...
        this.statusDialogs = new ConcurrentHashMap<>();

        Boolean isAuthorized = checkPermission(
                PermissionUtils.buildPermissionString("gfe", "serviceBackup"));
        if (!isAuthorized) {
    ...
    private void populateSites() {
        primarySites = new ArrayList<>(CheckPrimary.getPrimarySites());
        Collections.sort(primarySites);

        this.activatedBackupSites = new TreeSet<>();

        List<String> backupPartners = new ArrayList<>();
        try {
        ...
        List<String> knownSites = new ArrayList<>();
        try {
            @SuppressWarnings("unchecked")
            ...
            if (sr.isOkay()) {
                ...
                statusHandler.error(
                        "Error retreiving known sites list: " + sr.message());
            }
        } catch (VizException e) {
        ...
        allSites = new ArrayList<>(knownSites.size() + backupPartners.size());
            ...
            if (response.isOkay()) {
                ...
                for (Entry<String, Collection<ServiceBackupJobStatus>> entry : backupStatus
                    ...
                    for (ServiceBackupJobStatus status : entry.getValue()) {
                        if ("svcbuMode".equals(status.getJobName()) && status
                                .getJobStatus().equals(JobProgress.SUCCESS)) {
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        Label label = new Label(startupActionsGroup,
        ...
        enterCombo = new Combo(startupActionsGroup,
                SWT.DROP_DOWN | SWT.READ_ONLY);
        layoutData = new GridData(SWT.FILL, SWT.DEFAULT, true, false);
        enterCombo.setLayoutData(layoutData);
        enterCombo.setToolTipText(
                "Select a failed site you wish to start backing up");
        enterCombo.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                enterButton.setToolTipText(
                        "Enter service backup for " + enterCombo.getText());
        ...
        importGridsCombo = new Combo(startupActionsGroup,
                SWT.DROP_DOWN | SWT.READ_ONLY);
        layoutData = new GridData(SWT.FILL, SWT.DEFAULT, true, false);
        importGridsCombo.setLayoutData(layoutData);
        importGridsCombo.setToolTipText(
                "Select a failed site from which to import forecast grids");
        ...
        startGFECombo = new Combo(startupActionsGroup,
                SWT.DROP_DOWN | SWT.READ_ONLY);
        layoutData = new GridData(SWT.FILL, SWT.DEFAULT, true, false);
        ...
        startGFECombo.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                startGFEButton.setToolTipText(
                        "Start GFE Sesson for " + startGFECombo.getText());
        ...
        exportFailedGridsCombo = new Combo(exitActionsGroup,
                SWT.DROP_DOWN | SWT.READ_ONLY);
        layoutData = new GridData(SWT.FILL, SWT.DEFAULT, true, false);
        exportFailedGridsCombo.setLayoutData(layoutData);
        exportFailedGridsCombo.setToolTipText(
                "Select a failed site to export forecast grids");
        exportFailedGridsCombo.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                exportFailedGridsButton.setToolTipText(
                        "Export grids to  " + exportFailedGridsCombo.getText());
        ...
        exitCombo.setToolTipText(
                "Select a failed site you wish to stop backing up");
        exitCombo.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                exitButton.setToolTipText(
                        "Exit service backup for " + exitCombo.getText());
        ...
        exportLocalGridsCombo = new Combo(localActionGroup,
                SWT.DROP_DOWN | SWT.READ_ONLY);
        layoutData = new GridData(SWT.FILL, SWT.DEFAULT, true, false);
        ...
        exportLocalGridsCombo.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                exportLocalGridsButton.setToolTipText(
                        "Export grids for " + exportLocalGridsCombo.getText()
        ...
        exportLocalGridsCombo.setItems(
                primarySites.toArray(new String[primarySites.size()]));
        ...
        try {
            isAuthorized = checkPermission(PermissionUtils
                    .buildPermissionString("gfe", "exportConfig"));
        ...
        if (isAuthorized) {
            ...
            exportConfigCombo = new Combo(localActionGroup,
                    SWT.DROP_DOWN | SWT.READ_ONLY);
            layoutData = new GridData(SWT.FILL, SWT.DEFAULT, true, false);
            ...
            exportConfigCombo.setItems(
                    primarySites.toArray(new String[primarySites.size()]));
            if (primarySites.size() == 1) {
                exportConfigCombo.setText(primarySites.get(0));
                exportConfigButton.setToolTipText(
                        "Export grids for " + exportConfigCombo.getText()
                                + " to the central server");
                exportConfigButton.setEnabled(true);
            }
        }
    ...
    private void updateDialog() {
        ...
        String[] items = activatedBackupSites.isEmpty() ? new String[] { "WWW" }
                : activatedBackupSites
                        .toArray(new String[activatedBackupSites.size()]);
        Arrays.sort(items);
    ...
    private List<String> getBackupSites() {
        ...
        ArrayList<String> backupSites = new ArrayList<>(allSites);
    ...
    private void enterBackup(final String failedSite) {
        ...
        tasks.add(new ServerRequestSbuTask("cleanup",
                "Removing %s's config and data",
                new CleanupSvcBuRequest(failedSite)));
        ...
        tasks.add(new SiteActivationSbuTask(failedSite, ACTIVATIONTYPE.ACTIVATE,
                "activatingServer", "Activating GFE server for %s"));
    ...
    private void exitBackup(final String failedSite) {
        ...
        boolean confirmed = MessageDialog.openQuestion(getShell(),
                "Confirm Exit", "Have you contacted " + failedSite
    ...
    private void exportConfigToCentralServer(String localSite) {
        ...
        tasks.add(new ServerRequestSbuTask("exportConfig",
                ...
                new ExportConfRequest(localSite)));
