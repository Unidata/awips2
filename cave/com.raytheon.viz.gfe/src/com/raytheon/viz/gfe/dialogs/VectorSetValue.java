 ...
 * Feb 20, 2012  346      dgilling  Create PickupChangedJob to fix Invalid
 *                                  Thread Access exceptions in
 *                                  pickupValueChanged.
 * Apr 09, 2018  7262     randerso  Added getDirValue()
 *
 * </pre>
 *
 * @author randerso
...
public class VectorSetValue extends AbstractSetValue
        implements IVectorModeChangedListener, IPickupValueChangedListener {
    ...
    private void setParm(Parm parm) {
        ...
        GridData layoutData = new GridData(SWT.CENTER, SWT.DEFAULT, true,
                false);
    ...
    protected void dirChanged() {
        ...
        VectorWxValue pickupValue = (VectorWxValue) parm.getParmState()
                .getPickUpValue();
        float floatVal = getDirValue(dirField.getText());
    ...
    private float getDirValue(String compass) {
        float value;
        if ("N".equals(compass)) {
            value = 0f;
        } else if ("NNE".equals(compass)) {
            value = 22.5f;
        } else if ("NE".equals(compass)) {
            value = 45f;
        } else if ("ENE".equals(compass)) {
            value = 67.5f;
        } else if ("E".equals(compass)) {
            value = 90f;
        } else if ("ESE".equals(compass)) {
            value = 112.5f;
        } else if ("SE".equals(compass)) {
            value = 135f;
        } else if ("SSE".equals(compass)) {
            value = 157.5f;
        } else if ("S".equals(compass)) {
            value = 180f;
        } else if ("SSW".equals(compass)) {
            value = 202.5f;
        } else if ("SW".equals(compass)) {
            value = 225f;
        } else if ("WSW".equals(compass)) {
            value = 247.5f;
        } else if ("W".equals(compass)) {
            value = 270f;
        } else if ("WNW".equals(compass)) {
            value = 292.5f;
        } else if ("NW".equals(compass)) {
            value = 315f;
        } else if ("NNW".equals(compass)) {
            value = 337.5f;
        } else {
            try {
                value = Float.parseFloat(compass);
                return value;
            } catch (NumberFormatException e) {
                return Float.NaN;
            }
    ...
    public void vectorModeChanged(Parm parm, final VectorMode newMode) {
        VizApp.runAsync(new Runnable() {
            ...
            public void run() {
                for (VectorMode mode : VectorMode.values()) {
                    modeButton[mode.ordinal()]
                            .setSelection(mode.equals(newMode));
                }
