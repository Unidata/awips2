 ...
 * Aug 19, 2019 7914       tgurney     Change Integer lists to Long (Python 3)
 *
 * </pre>
 *
 * @author lvenable
...
public class ProductDataStruct {
    ...
    private static final String nwsTimeStr = "nwstime";
     ...
     *            Product text.
     */
    public ProductDataStruct(Map<String, Object> parsedMap,
            String productText) {
    ...
    private List<List<Integer>> longsToInts(List<List<Long>> other) {
        List<List<Integer>> rval = new ArrayList<>();
        for (List<Long> longs : other) {
            List<Integer> ints = new ArrayList<>();
            for (Long lng : longs) {
                ints.add(Math.toIntExact(lng));
            }
            rval.add(ints);
        }
        return rval;
    ...
    private void extractFrames() {
        if (!parsedMap.containsKey("frames")) {
            return;
        }

        if (!(parsedMap.get("frames") instanceof List)) {
            return;
        }

        frames = new ArrayList<>();

        List<Map<String, List<List<Long>>>> parseFrames = (List<Map<String, List<List<Long>>>>) parsedMap
                .get("frames");

        for (Map<String, List<List<Long>>> frameMap : parseFrames) {
            Set<String> keys = frameMap.keySet();

            for (String key : keys) {
                List<List<Integer>> frameIndexes = longsToInts(
                        frameMap.get(key));
    ...
    private void extractCi() {
        ...
        Map<String, List<List<Long>>> parsedCi = (Map<String, List<List<Long>>>) parsedMap
                .get("ci");

        Set<String> keys = parsedCi.keySet();

        for (String key : keys) {
            tmpArray = longsToInts(parsedCi.get(key));
    ...
    private void extractMnd() {
        ...
        Map<String, List<List<Long>>> parsedMnd = (Map<String, List<List<Long>>>) parsedMap
                .get("mnd");

        Set<String> keys = parsedMnd.keySet();

        for (String key : keys) {
            tmpArray = longsToInts(parsedMnd.get(key));
            tip = createTextIndexPoints(tmpArray);
    ...
    private void extractSegments() {
        ...
        for (Map<String, Object> curSegMap : parsedSegs) {
            ...
            for (String key : keys) {
                if ("headInfo".equals(key)) {
                    List<Map<String, List<List<Long>>>> headInfoArray = (List<Map<String, List<List<Long>>>>) curSegMap
                            .get(key);

                    for (Object hiObj : headInfoArray) {
                        Map<String, List<List<Long>>> headInfoMap = (Map<String, List<List<Long>>>) hiObj;

                        Set<String> headInfoKeys = headInfoMap.keySet();
                        for (String hiKey : headInfoKeys) {
                            tmpArray = longsToInts(headInfoMap.get(hiKey));
                            tip = createTextIndexPoints(tmpArray);
                            segData.addToHeadInfoMap(hiKey, tip);
                        }
                    }
                } else {
                    tmpArray = longsToInts(
                            (List<List<Long>>) curSegMap.get(key));
                    tip = createTextIndexPoints(tmpArray);
     ...
     */
    private TextIndexPoints createTextIndexPoints(
            List<List<Integer>> tmpArray) {
    ...
    public void printData() {
        ...
        for (SegmentData segData : segments) {
            ...
            for (String headInfoKey : headInfoKeys) {
                ...
                System.out.println(
                        "Text = " + headInfoMap.get(headInfoKey).getText());
            }
        }
