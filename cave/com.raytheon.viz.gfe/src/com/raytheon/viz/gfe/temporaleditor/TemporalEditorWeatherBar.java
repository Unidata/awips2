...
import com.raytheon.viz.gfe.temporaleditor.mousehandler.EditorWeatherMouseHandler;
...
public class TemporalEditorWeatherBar extends AbstractTemporalEditorBar {
    private MouseHandler editorMouseHandler;

    private Map<Rectangle, WeatherKey> rectToWeatherKey = new HashMap<>();
     ...
     * @param ts
     */
    public TemporalEditorWeatherBar(TemporalEditor parent,
            TemporalEditorUtil teUtil, Parm parm, TimeSeries ts,
            DataManager dataMgr) {
        super(parent, teUtil, parm, ts, dataMgr);

        editorMouseHandler = new EditorWeatherMouseHandler(this);
    ...
    protected void paintEditorCanvas(PaintEvent event) {
        ...
        if (dataMgr.getRefManager().getActiveRefSet() != null) {
            paintLocks(event);
            paintWeather(event);
        }
     ...
     * @param event
     */
    private void paintWeather(PaintEvent event) {
        GC gc = event.gc;
        rectToWeatherKey.clear();
        displayedParm = null;
        ...
        try {
            paintWeatherBody(event, gc, labelFont);
        } finally {
            gc.setFont(oldFont);
        }
    }
     ...
     * @param gc
     */
    private void paintWeatherBody(PaintEvent event, GC gc, Font labelFont) {
                ...
                for (HistSample sample : samples) {
                    if (sample.numOfPoints() > 0) {
                        ...
                        for (HistPair histPair : sample.histogram()) {
                            ...
                            WeatherKey weatherKey = ((WeatherWxValue) wxv)
                                    .getWeatherKey();
                            rectToWeatherKey.put(rect, weatherKey);
                            ...
                            paintLabel(gc, weatherKey.toPrettyString(), vt,
                                    Math.round((barMax + barMin) / 2.0f), 0,
                                    TextJustify.CENTER,
                                    Math.round(barMax - barMin));
                            // set for next loop iteration
                            barMin = barMax;
                        }
                    }
                }
     ...
     * @return
     */
    public WeatherKey getClickedKey(Point pt) {
        for (Rectangle rect : rectToWeatherKey.keySet()) {
            if (rect.contains(pt)) {
                return rectToWeatherKey.get(rect);
            }
        }
