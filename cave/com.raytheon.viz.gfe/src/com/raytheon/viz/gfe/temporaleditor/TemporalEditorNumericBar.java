...
public class TemporalEditorNumericBar extends AbstractTemporalEditorBar
    ...
    private static float fletchAngle = 60.0f;
     ...
     *
     */
    private Map<Parm, List<Color>> parmColorMap = new HashMap<>();
    ...
    public void addParm(Parm parm, TimeSeries ts) {
        if (!parmList.contains(parm)) {
            ...
            ResourcePair rsc = dataMgr.getSpatialDisplayManager()
            ...
            if (colorMap != null) {
                ...
                for (com.raytheon.uf.common.colormap.Color c : colorMap
                        .getColors()) {
                    colorList.add(convertColor(c, display));
                }
    ...
    protected boolean handleResourceChangeEvent(Parm p, ChangeType type,
            Object eventData) {
        boolean retVal = super.handleResourceChangeEvent(p, type, eventData);

        if ((type == ChangeType.CAPABILITY)
                && (eventData instanceof ColorMapCapability)) {
            IColorMap colorMap = ((ColorMapCapability) eventData)
                    .getColorMapParameters().getColorMap();
            if (colorMap != null) {
                Display display = Display.getCurrent();
                List<Color> newColors = colorMap.getColors().stream()
                        .map((c) -> {
                    return convertColor(c, display);
                }).collect(Collectors.toList());
                List<Color> oldColors = parmColorMap.put(p, newColors);
                if (oldColors != null) {
    ...
    private static Color convertColor(com.raytheon.uf.common.colormap.Color c,
            Display d) {
        return new Color(d, Math.round(c.getRed() * 255f),
                Math.round(c.getGreen() * 255f),
