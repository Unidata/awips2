...
public class GFELegendResource extends
    ...
    private LegendData[] getLegendDataGrids(IDescriptor descriptor) {
        ...
        List<Pair<Parm, ResourcePair>> parms;
        Parm qvParm = null;
        if (qvGrid == null) {
            parms = orderParms(descriptor);
        } else {
            qvParm = qvGrid.getParm();
            parms = new ArrayList<Pair<Parm, ResourcePair>>(1);
            parms.add(new Pair<Parm, ResourcePair>(qvParm, this.dataManager
                    .getSpatialDisplayManager().getResourcePair(qvParm)));
     ...
     * @return
     */
    protected List<Pair<Parm, ResourcePair>> orderParms(IDescriptor descriptor) {
        Iterator<ResourcePair> rl = descriptor.getResourceList().iterator();

        ArrayList<Pair<Parm, ResourcePair>> parms = new ArrayList<Pair<Parm, ResourcePair>>();
        while (rl.hasNext()) {
            ResourcePair rp = rl.next();
            if (rp.getResource() instanceof GFEResource) {
                GFEResource gfeRsc = (GFEResource) rp.getResource();
                parms.add(new Pair<Parm, ResourcePair>(gfeRsc.getParm(), rp));
            }
        }

        // Now sort the parms in the reverse order
