...
public class QueryJob extends Job {
     ...
     */
    protected ArrayBlockingQueue<DbData[]> responseQueue = new ArrayBlockingQueue<DbData[]>(
            QUEUE_SIZE);

    /**
     * @param name
     */
    protected QueryJob(String name) {
        super(name);
    }
    ...
    protected IStatus run(IProgressMonitor monitor) {
        ...
        if (request != null) {
            ...
            if (qResponse != null) {
                ...
                DbData[] unsorted = new DbData[rsps];
                byte[] wkb;
                String tableName;
                String cwa;
                String zone;
                ...
                for (Object[] row : qResponse) {
                    ...
                    zone = (String) row[col++];
                    cwa = (String) row[col++];
                    unsorted[rowNum++] = new DbData(wkb, tableName, cwa, zone);
                }
                long millis = 0L;
