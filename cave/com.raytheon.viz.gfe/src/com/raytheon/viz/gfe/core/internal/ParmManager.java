...
public class ParmManager implements IParmManager, IMessageClient, ISimulatedTimeChangeListener {
    ...
    private List<VCModule> initVirtualCalcParmDefinitions() {
        ...
        LocalizationContext[] contexts = new LocalizationContext[] {
                pathMgr.getContext(LocalizationType.COMMON_STATIC, LocalizationLevel.BASE),
                pathMgr.getContext(LocalizationType.COMMON_STATIC, LocalizationLevel.SITE),
                pathMgr.getContext(LocalizationType.COMMON_STATIC, LocalizationLevel.USER) };

        Map<String, LocalizationFile> modMap = new HashMap<>();
        for (LocalizationContext context : contexts) {
            LocalizationFile[] files = pathMgr.listFiles(context, FileUtil.join("gfe", "vcmodule"),
                    new String[] { "py" }, false, true);
            for (LocalizationFile lf : files) {
                String modName = LocalizationUtil.extractName(lf.getPath()).replace(".py", "");
