...
/**
 * Factory class for construct {@link DataManager} objects for an arbitrary
 * Object discriminator. Manages {@link DataManager}s so only one per
 * discriminator exists. Takes instance of this class when constructing
 * {@link DataManager} to for construction of {@link ISpatialDisplayManager} for
 ...
 * Feb 18, 2020 74905      tjensen     Added getInstance() that takes siteID
 *
 * </pre>
 *
 * @author mschenke
...
public abstract class DataManagerFactory {
    ...
    private static Map<DataManagerKey, DataManager> instanceMap = new ConcurrentHashMap<>();
     ...
     */
    public static DataManager getInstance(DataManagerFactory factory,
            Object discriminator, String siteID) {
        synchronized (DataManagerKey.class) {
            DataManagerKey key = new DataManagerKey(discriminator);
            DataManager dm = instanceMap.get(key);
            if (dm == null) {
                try {
                    dm = new DataManager(factory, discriminator, siteID);
                    instanceMap.put(key, dm);
                    waitForJobs(dm);
                } catch (GFEServerException e) {
                    statusHandler.handle(Priority.PROBLEM,
    ...
    public static void dispose(Object discriminator) {
        synchronized (DataManagerKey.class) {
            DataManager dm = instanceMap
                    .remove(new DataManagerKey(discriminator));
    ...
    /**
     * Dispose all DataManager instances when preparing to shut down
     */
    public static void disposeAll() {
        for (DataManagerKey key : instanceMap.keySet()) {
            DataManager dm = instanceMap.remove(key);
            if (dm != null) {
                dm.dispose();
            }
        }
