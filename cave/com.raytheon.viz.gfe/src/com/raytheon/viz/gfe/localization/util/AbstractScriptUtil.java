...
public abstract class AbstractScriptUtil {
    ...
    public LocalizationFile createNew(String script, Overwrite mode,
        ...
        LocalizationContext context = pathManager
                .getContext(getLocalizationType(), getLocalizationLevel());
        ...
        if (locFile == null) {
            ...
            try (SaveableOutputStream sos = locFile.openOutputStream();
                ...
                throw new GFEException("Error creating new file " + scriptFName,
                        e);
            }
        }
        return locFile;
    ...
    protected void initVelocity() throws GFEException {
        try {
            File templateFile = new File(FileLocator
            ...
            throw new GFEException("Error locating script templates directory.",
                    e);
        }
    }

    /**
     * Add ".py" to name if it doesn't end with a Python file extension, and
     * return the result.
     *
     * @param name
    ...
    public String normalize(String name) {
        String result = null;
        if (name.endsWith(".py")
                || name.endsWith(PyCacheUtil.COMPILED_FILE_EXTENSION)) {
            result = name;
        } else {
            result = name + ".py";
        }
