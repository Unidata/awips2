...
public class SamplerGridSliceCache {
    ...
    private static Map<Long, Map<ParmID, List<IGridSlice>>> cache = new HashMap<>();

    public static ServerResponse<List<IGridSlice>> getData(IFPClient client,
            ParmID parmId, List<TimeRange> gridTimes) {
        List<IGridSlice> result = new ArrayList<>();
        List<TimeRange> needToRequest = new ArrayList<>();
        ...
        if (alreadyRetrieved == null) {
            alreadyRetrieved = new ArrayList<>();
