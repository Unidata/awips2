...
import popen2, string, time, os, pickle
    ...
    def decodeTCMProduct(self, TCMProduct, eyeDiameter):
        ...
        try:
            while self.pos < len(TCMProduct):
                ...
                for k in list(self.keyWordDict.keys()):
...
#        cmd = "/awips2/fxa/bin/textdb -r " + productID
    ...
    def printFcst(self, f, baseTime=None):
        print("==============================================================")
        print("Time:", time.asctime(time.gmtime(f['validTime'])), end=' ')
...
##            return
    ...
    def makeWindDict(self, fcstList):
        ...
        for f in fcstList:
            windValues = list(f["radii"].keys())
            hour = (f["validTime"] - self.baseDecodedTime) / 3600
            maxWind = f["maxWind"]
            minWind = 999999.0
            if len(list(f["radii"].keys())) == 0:
                minWind = 0.0
    ...
    def launchMaxWindGUI(self, fcstList):
        ...
        if newMaxWinds is not None:
            ...
            hourList = list(newMaxWinds.keys())
            hourList.sort()
