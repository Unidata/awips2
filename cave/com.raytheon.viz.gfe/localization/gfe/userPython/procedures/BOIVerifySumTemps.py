...
# ------------- -------- --------- --------------------------------------------
# Nov 21, 2013  16770    ryu       Change name of temporary files for dual 
#                                  domain.
# Nov 26, 2014  649      zhao      changed "dbName" to "dbid"  
# Feb 10, 2016  5283     nabowle   Remove NGM support.
# Apr 19, 2018  7271     randerso  Renamed and/or removed models. Code cleanup.
# Feb 20, 2019  6896     randerso  Fixed various issues
#
##
...
#
MODLIST=["Official","MOSGuideBC","MOSGuide","GFS","GFSBC",
         "ADJMAV","ADJMAVBC","NAM","NAMBC",
...
EMAIL_CMDS=["scp /tmp/{FILE} ldad@ls1:/tmp/{FILE}",
            "ssh ldad@ls1 /usr/sbin/sendmail -t </tmp/{FILE}",
...
class Procedure (SmartScript.SmartScript):
   ...
   def execute(self,):
      ...
      for info in perInfos:
         ...
         if ((name=="unknown")or(name=="multiple")):
            continue
         if name not in individualOutput:
   ...
   def emailMessage(self,to,parm,verifday,output):
      ...
      EMAIL_TEMP_FILE="%s_temp_email.txt" % self.getSiteID()
      EMAIL_FULL_FILE="%s/%s" % (SAVE_DIR, EMAIL_TEMP_FILE)
      outfile=open(EMAIL_FULL_FILE,"w")
      outfile.write("To:%s\n"%to)
   ...
   def getDifficultyRankings(self,parm,vyea,vmon,vday,numdays):
      ...
      for i in range(1,len(obskeys)):
         ...
         #
         anomstr="              "
         cgrid=self.getClimoGrid(parm,pmon,pday)
      ...
      anomkeys=list(absanom.keys())
      if todaykey in anomkeys:
         anomkeys.sort(key=lambda x: absanom[x])
      ...
      chgskeys=list(abschgs.keys())
      if todaykey in chgskeys:
         chgskeys.sort(key=lambda x: abschgs[x])
      ...
      stdskeys=list(stds.keys())
      if todaykey in stdskeys:
         stdskeys.sort(key=lambda x: stds[x])
   ...
   #                 climo grids
   #
   def getClimoGrid(self,parm,vmon,vday):
      dbid = self.findDatabase("D2D_PRISMClimo") # could use NCDCClimo
      if not dbid.isValid():
         return None
   ...
   def getTodayInfo(self,parm,modlist,verifday,fhours):
      ...
      for mod in modlist:
         ...
         statkeys=list(statcases.keys())
         gridkeys=list(gridcases.keys())
         if len(gridkeys)>0:
      ...
      for per in range(14):
         ...
         #  this period
         #
         modkeys=list(modVals.keys())
         modkeys.sort(key=lambda x: modVals[x])
         modkeys.reverse()
         #
         #  Get Official results - if they exist
         #
         officialkey="Official,%2.2d"%per
         if officialkey in bias:
   ...
   def getPastInfo(self,parm,modlist,verifday,numdays,fhours):
      ...
      for per in range(len(allfhrs)):
         ...
         modkeys=list(modVals.keys())
         modkeys.sort(key=lambda x: modVals[x], reverse=True)
         #
         #
         #
         try:
