...
class Procedure (SmartScript.SmartScript):
    ...
    def makeListBox(self):
        ...
        for i in range(len(labelList)):
            listItem = self._listbox.insert(Tkinter.END, labelList[i])
...
#             return
    ...
    def timeButtonClick(self, event):
        ...
        if type(buttonText) is types.TupleType:
            valueText = ""
            for t in buttonText:
    ...
    def pieClick(self, event):
        ...
        dist = math.sqrt((event.x - xCenter) * (event.x - xCenter) + \
                             (event.y - yCenter) * (event.y - yCenter))
    ...
    def cancelCommand(self):
        ...
        if tkMessageBox.askokcancel("High Sea - Cancel", msg, parent=self._bottomFrame):
            self._master.destroy()
            self._tkmaster.destroy()
...
##        for i in range(len(quad)):
    ...
    def latLonStr(self, coord, latOrLon, precision=1.0):
        ...
        coordStr = "%.0f" % abs(round(coord)) + dirStr
    ...
    def makeNamedAreaDesc(self, editArea):
        ...
        for eaName in self._namedAreaMasks.keys():
            overlap = self._namedAreaMasks[eaName] & editAreaMask
...
#                 return ""
    ...
    def writeHSDescriptorToFile(self, highSeasDescriptor, highSeasFileName):
        ...
        try:
            f = open(highSeasFileName, "w")
            cPickle.dump(highSeasDescriptor, f)
            if setPermissions:
    ...
    def saveHSDescToFile(self):
        ...
        saveAreas = []
        if self._multiProductSites.has_key(self._siteID):
...
#             eaMask = self.encodeEditArea(newEditArea)  # Why do this?
    ...
    def writeFeatureList(self, gridBasedFeatures, gridBasedFileName):
        ...
        if not os.path.exists(gridBasedFileName):  # it's a new file set set permissions
            setPermissions = True
        try:   
            with open(gridBasedFileName, "w") as f:
                cPickle.dump(self._sortedFeatures, f)
                if setPermissions:
    ...
    def initializeHSDescriptor(self):
        ...
        try:   
            f = open(highSeasFileName, "r")
            self._hsDesc = cPickle.load(f)
        except:
    ...
    def makeNamedAreaMasks(self, namedAreaDict):
        ...
        namedAreaMasks = {}
        for eaName in namedAreaDict.keys():
    ...
    def defineNamedAreas(self):
        ...
        namedAreaDescrip = {}
        for eaName in allAreas.keys():
            if eaName in allEANames:
                namedAreaDescrip[eaName] = allAreas[eaName]
    ...
    def execute(self, timeRange):
        ...
        latLonTR = TimeRange.TimeRange(startTime, endTime)
