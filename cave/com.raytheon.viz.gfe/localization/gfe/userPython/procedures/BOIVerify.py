...
# Mar 14, 2018  6955     randerso  Added scale to Stats vs. Scale graph
# Mar 14, 2018  7024     randerso  Fixed setting of LogFactor preference
#
# ----------------------------------------------------------------------------
#
...
import tkinter
import TkDefaults
import time,calendar,sys,re,traceback
import SmartScript
...
class Procedure (SmartScript.SmartScript):
   ...
   def __setup(self):
      ...
      try:
         ...
         for k in range(maxk):
            ...
            for digits in range(2):
               ...
               if iscale==rscale:
                  rdig=digits
                  break
            rscale=self.round(scale, "Nearest", 10**-rdig)
         ...
         for i in range(len(editAreaNames)):
            #
            curTxt=wTxt.get()
            last=curTxt[-1]
            rest=curTxt[:-1]
   ...
   def setColorTableAndRange(self,parm,colorTable,displayMinval,displayMaxval):
      ...
      if displayMinval or displayMaxval or colorTable:
         ...
         if colorTable:
            if colorTable in self.__colorMapParams:
   ...
   def ShowGridsStats(self,DialogDict):
      ...
      for parm in parmList:
         ...
         for model in modelList:
            ...
            #
            casekeys=list(cases.keys())
            casekeys.sort(key=lambda x: x.split(",", 1)[1])
            totalcount=len(casekeys)
            self.VU.logMsg("%d cases for %s"%(len(casekeys),model),1)
            count=0
            lastobs=""
   ...
   def errorHistogram(self,parmList,cycleList,modelList,obsmodel,
      ...
      #
      fullkeys=list(self.histograms.keys())
      #
      ...
      #
      histkey1=next(iter(self.histograms))
      maxHist=zeros_like(self.histograms[histkey1])
      for histkey, histval in self.histograms.items():
         histval /= float(self.numCases[histkey])
         maxHist = maximum(maxHist, histval)
      ...
      #  Draw each histogram
      #
      totalcount=len(self.histograms)
      count=0
   ...
   def valueHistogram(self,parmList,cycleList,modelList,obsmodel,
      ...
      for model in modelList:
         ...
         for key in casekeys:
            ...
            if obsSaveKey in self.histograms:
      ...
      for i in range(self.histonumbins):
      ...
      #  Draw each histogram
      #
      totalcount=len(list(self.histograms.keys()))
      count=0
      for key in self.histograms:
         count+=1
         if self.setAndCheckWorking("%s: drawing histogram %d of %d"%(workStart,count,totalcount))==1:
   ...
   def expectedValue(self,parmList,cycleList,modelList,obsmodel,
      ...
      #
      fullkeys=list(self.flists.keys())
      #
      #  if no data could be read - stop here
      #
      if len(fullkeys)<1:
   ...
   def scatterPlot(self,parmList,cycleList,modelList,obsmodel,
      ...
      if maxnum>MaxValuesToShow:
         ...
         totaldcount=len(list(self.flists.keys()))
         dcount=0
         for key in self.flists:
            dcount+=1
   ...
   def makeTimeSeries(self,DialogDict):
      ...
      for parm in parmList:
         ...
         for model in modelList:
            ...
            #
            skeys=list(scases.keys())
            gkeys=list(gcases.keys())
            tkeys=skeys
            for gkey in gkeys:
         ...
         for model in modelList:
            ...
            for key in tkeys:
               ...
               if ((AreaCombine==1)and(len(areaNames)>1)):
                  ...
                  outkey="%s,%s,%3.3d,-01"%(parm,model,fhr)
                  if display!="TwoCat":
                     if outkey not in sumdata:
                        sumdata[outkey]=0.0
                        cntdata[outkey]=0
                     sumdata[outkey]+=valx
                     cntdata[outkey]+=1
                  else:
                     if outkey not in hitsdata:
                        hitsdata[outkey]=0
                        missdata[outkey]=0
                        falrdata[outkey]=0
                        corndata[outkey]=0
   ...
   def ShowScaleStats(self,DialogDict):
      ...
      self.fhouraxes(scalemin,scalemax,valmin,valmax)
      #
      #  Draw timeseries lines
      #
      for key in outdata:
   ...
   def setupBut1(self,butList,numbuttons=5,arrows=0,width=0):
      ...
      for i in range(len(self.but1names)):
   ...
   def setupBut2(self,butList,numbuttons=5,arrows=0,width=0):
      ...
      self.but2={}
      self.but2state={}
      for i in range(len(self.but2names)):
   ...
   def logvalhaxes(self,minval,maxval,tickInterval,logmin,logmax,parm):
      ...
      for j in range(expstart,expend+1):
         a=10.0**j
...
class Verif(BVDialog):
   ...
   def getForecasterListbox(self):
      ...
      try:
         itemnums=list(map(int,itemnums))
   ...
   def getCycleVals(self):
      outlist=[]
      for i in range(len(self.CycleFlags)):
   ...
   def updateGridsScale(self):
      ...
      for i in range(len(self.__scaleList)):
   ...
   def OptionsGridsStats(self,master):
      ...
      for i in range(self.__VU.getCFG('NUMTHRESH')):
