...
class TextProduct(GenericHazards.TextProduct):
    ...
    def _determineTimeRanges(self, argDict):
        ...
        self._extraSampleTimeRange = self.makeTimeRange(startTime3Hour-12*3600, startTime3Hour)
        ...
        # Create a list of 10 periods (GMT time) corresponding to 6AM and 6PM times (local time)
    ...
    def _createTimeRangeList(self, timeRange, hours):
        ...
        for index, tr in enumerate(subRanges):
            self.debug_print("In _createTimeRangeList (%s hour chunks) -> tr = %s" % 
                             (hours, self._pp.pformat(tr)), 1)
    ...
    def _calculateStartTime(self, localCreationTime, resolution):
        ...
        self.debug_print("In _calculateStartTime incoming res %d   localCreationTime = %d" % 
                         (resolution, localCreationTime), 1)
        ...
        startTime = AbsTime(localCreationTime + adjust)
...
#                 partOfDay = "after midnight"
    ...
    def _initializeStormInformation(self):
        ...
        self._stormID = None        # This is a 2-digit string embedded in the storm number ("09" in "AL092016")
...
FORECASTER STEWART"""
    ...
    def _getStormAdvisoryNames(self):
        ...
        stormAdvisories = [filename for filename in allAdvisories if self._getStormNumberStringFromTCP() in filename]
        stormAdvisories = [filename[:-5] for filename in stormAdvisories]
    ...
    def hazardTimeZones(self, areaList):
        ...
        for areaName in areaList:
            if areaName in list(areaDict.keys()):
                ...
                if type(timeZoneList) is not list:  # a single value
