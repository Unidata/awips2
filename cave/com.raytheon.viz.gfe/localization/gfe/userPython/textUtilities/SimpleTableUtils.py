...
class SimpleTableUtils(TextUtils.TextUtils):
    ...
    def singleValue(self, element, stats):
        ...
        if element.dataType() == "Vector":
            ...
            if mag <= self.table_wind_threshold():
                ...
                magStr = magStr.rjust(element.maxWidth())
                dir = stats[1]
            ...
            valStr = valueStr.rjust(element.maxWidth())
    ...
    def avgValue(self, element, stats):
        ...
        if element.dataType() == "Vector":
            ...
            if mag <= self.table_wind_threshold():
                ...
                valStr = valStr.rjust(element.maxWidth())
                # put in the direction
                dir = self.convertDirection(dir)
                valStr = dir.rjust(2) + valStr
        else:
    ...
    def range2Value(self, element, stats):
        ...
        if element.dataType() == "Scalar":
            if val1 == val2:
                valStr = val1Str.rjust(element.maxWidth())
            else:
                valStr = val1Str.rjust(element.maxWidth()) + "-" + \
                             val2Str.rjust(element.maxWidth())
        else:
            ...
            if val1 == val2 and dir1 == dir2:
                valStr = val1Str.rjust(element.maxWidth()) + dir1
            else:
                valStr = dir1 + " " + val1Str.rjust(element.maxWidth()) + \
                         " - " + \
                         dir2 + " " + val2Str.rjust(element.maxWidth())
    ...
    def range4Value(self, element, stats):
        ...
        if element.dataType() == "Vector":
            ...
            val1Str = val1Str.rjust(element.maxWidth())
            val1Str = dir1.rjust(2) + val1Str
            val2Str = self.fformat(mag2, element.roundVal())
            val2Str = val2Str.rjust(element.maxWidth())
            val2Str = dir2.rjust(2) + val2Str
        else:
            val1 = self.average(stats[0], stats[1])
            val2 = self.average(stats[2], stats[3])
            val1 = self.callMethod(val1, element.conversion())
            val2 = self.callMethod(val2, element.conversion())
            val1 = str(self.round(val1,"Nearest", element.roundVal())).rjust(element.maxWidth())
            val2 = str(self.round(val2,"Nearest", element.roundVal())).rjust(element.maxWidth())
            val1Str = self.fformat(val1, element.roundVal())
            val2Str = self.fformat(val2, element.roundVal())
        if val1Str == val2Str:
            valStr = val1Str
