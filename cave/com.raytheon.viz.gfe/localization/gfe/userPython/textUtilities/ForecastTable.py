...
class ForecastTable(TextRules.TextRules, SampleAnalysis.SampleAnalysis):
    ...
    def generateForecast(self, argDict):
        ...
        if type(self.__loopMethod) is str:
            self.__loopMethod = getattr(self, f"__{self.__loopMethod}")
        if type(self.__endMethod) is str:
            self.__endMethod = getattr(self, f"__{self.__endMethod}")

        colJust = self.__ut.set(fcstDef,"columnJustification",
                                       "Center")
        if colJust == "Center":
            self.__alignMethod = lambda s, width: str(s).center(width)
        elif colJust == "Right":
            self.__alignMethod = lambda s, width: str(s).rjust(width)
        else:
            self.__alignMethod = lambda s, width: str(s).ljust(width)
    ...
    def __createRow(self, rowEntries, colLengths):
        ...
        alignMethod = lambda s, width: str(s).ljust(width)
        for rowEntry in rowEntries:            
            # Split entry into list of words
            words = rowEntry.split("|")
...
#                      alignMethod(word,colLengths[index])
    ...
    def __setupVariables(self, fcstDef, argDict):
        ...
        periodLabelMethod = self.__ut.set(fcstDef,"periodLabelMethod",
                                      self.periodLabel)
        if type(periodLabelMethod) is str:
            periodLabelMethod = getattr(self, periodLabelMethod)
    ...
    def __getTableHeading(self, rowType, tableVars):
        ...
        rowHeading = rowType.ljust(colLengths[0])
    ...
    def __getNextEntry(self, tableVars, colValue, rowValue, rowEntries):
        ...
        period = eval(tableVars.setPeriod())
        area = eval(tableVars.setArea())
        element = eval(tableVars.setElement())
