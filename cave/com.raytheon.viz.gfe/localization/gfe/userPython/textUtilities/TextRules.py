...
class TextRules(ConfigurableIssuance.ConfigurableIssuance,
    ...
    def fillSpecial(self, fcst, argDict):
        ...
        if type(trMethod) is str:
            trMethod = getattr(self, trMethod)
        ...
        try:
            ...
            elementName = ""
        fcst = fcst.replace("%TimePeriod",timeperiod)
        fcst = fcst.replace("%EditArea", eaLabel)
        fcst = fcst.replace("%WeatherElement", elementName)
        fcst = fcst.replace("%TimeRange", trName)
        return fcst
