...
#        TimeRangeUtils.TimeRangeUtils.__init__(self)
    ...
    def makeAreaHeader(self, argDict, areaLabel, issueTime, expireTime,
        ...
        if type(expireTime) is int or\
          type(expireTime) is float:
    ...
    def getGeneralAreaList(self, areaList, areaDictName="AreaDictionary"):
        ...
        for state, partState in list(geoAreas.keys()):
        ...
        gaLAND = []
        gaWATER = []
        for g,pg in list(geoAreas.keys()):
            names = geoAreas[(g,pg)]
            words = g.split(' ')
            found = 0
            for w in waters:
    ...
    def timeFromDDHHMM(self, dtgString):
        ...
        s = repr(wmo_year) + "%02i" % wmo_month + "%02i" % wmo_day + \
...
#############################
    ...
    def getAreaDictEntry(self, accessArg, dict, entryName, firstOnly=0):
        ...
        entryValues = []
        for areaLabel in list(dict.keys()):
            if accessArg != "all":
