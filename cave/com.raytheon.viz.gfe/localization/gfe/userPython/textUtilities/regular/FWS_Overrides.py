...
#**********************************************************************
    ...
    def _getFireProduct(self, products):
        ...
        for product in products:
            ...
            if tagFlag:
                fireNameList.append(repr(productNumber) + ") " + fireName + \
                                    " -- " + timeStamp + " -- " + tag)
...
#############################################################################
    ...
    def fire_dayOrNight_words(self, tree, node):
        ...
        if elementName == "MaxT" or elementName == "MinT":
            ...
            igWords =  repr(int(self.getValue(stats, "avg")))
        ...
        if self._withIgnitionTimes == "yes":
            ...
            if self._checkFireTR(node.getTimeRange()):
                ...
                if ignitionStats is not None:
                    ignitionPhrase = repr(int(self.getValue(ignitionStats)))
    ...
    def fireWind_finishUp(self, tree, node):
        ...
        if self._checkFireTR(node.getTimeRange()):
            ...
            if ignitionWindStats is not None:
                igMagStr = repr(int(ignitionWindStats[0]))
    ...
    def smokeDispersal_words(self, tree, node):
        ...
        if self._checkFireTR(node.getTimeRange()):
            ...
            words =  ventType + "..." + vrCat + " " + " /" + repr(vr) + " knot-ft/"
            reqType = self._getRequestWords()                
            ...
            igWords =  vrCat + " /" + repr(int(ignitionDispersal)) + " knot-ft/ at " + reqType + ". \n"
            words = igWords + " " + words
        else:
    ...
    def mixingHgt_words(self, tree, node):
        ...
        if self._checkFireTR(node.getTimeRange()):
            ...
            igWords =  repr(int(ignitionMixStats)) + " " + outUnits + " AGL at " + reqType +", otherwise "
            words = igWords + words
    ...
    def windWave_words(self, tree, node):
        ...
        if height is None:
            return self.setWords(node.parent, "MISSING")
        words =  repr(int(height + 0.5)) + " FEET"
        return self.setWords(node, words)
    ...
    def period_words(self, tree, node):
        ...
        if period is None:
            return self.setWords(node.parent, "MISSING")
        words =  repr(int(period + 0.5)) + " SECONDS"
        return self.setWords(node, words)
    ...
    def swell_words(self, tree, node):
        ...
        if dir >= 22.5 and dir < 67.5:
            ...
            dirWords = "north"
        heightWords = repr(int(height + 0.5))
    ...
    def freezingLevel_words(self, tree, node):
        ...
        # Single Value input
        if  mix1 == mix2:
                words =  repr(mix1) + " " + outUnits
        # Range
        else:
            words =  repr(mix1)+ "-" + repr(mix2) + " " + outUnits
        ...
        if self._checkFireTR(node.getTimeRange()):
            ...
            igWords =  repr(int(ignitionMixStats)) + " " + outUnits + " at " + reqType +", otherwise "
            words = igWords + words
        ...
        for low, hi, shortVal, longVal in self._tableLabels():
            if militaryHour >= low and militaryHour <= hi:
                if colWidth > 4:
                    ...
                    val = shortVal
                val = val.replace("hour", repr(hour))
                break
        return val
    ...
    def _getWindDirSpdStr(self, tree, node, timeRange, element, formatElement=None, units=None):
        ...
        if self._elementFormatDict.get(formatElement, "alpha") == "alpha":
            ...
            if dir < 10:
                dirString = "00" + repr(dir)
            elif dir < 100:
                dirString = "0" + repr(dir)
            else:
                dirString = repr(dir)
        if element == "Wind":
    ...
    def _ventrate_value(self, statDict, timeRange, argList):
        ...
        if ventrate is None:
            return "M"        
        return repr(int(ventrate/1000.0 + 0.5))
...
# the FWS product.
    ...
    def stabilityClass_words(self, tree, node) :
        ...
        if stability is None:
            return self.setWords(node.parent, "MISSING")
        words =  repr(int(self.getValue(stability)))
        return self.setWords(node, words)
    ...
    def marineLayer_words(self, tree, node) :
        ...
        if marineLayer is None:
            return self.setWords(node.parent, "MISSING")
        words =  repr(int(self.getValue(marineLayer)))
        return self.setWords(node, words)
