...
# Jan 15, 2018  6684     randerso  Converted ControlValues dialog to SWT using
#                                  ProcessVariableList so dialog stays above the
#                                  CAVE window. Code cleanup.
#
#
...
from ProcessVariableList import FieldDef
...
keys.extend(SC.Config["Locations"].keys())
...
#
    def execute(self, Topo, variableElement, variableElement_GridInfo, WEname,
        ...
        if (err == 1):
            ...
            self.cancel()
        #
        #  Open the Control Values dialog.
        
        #  The dialog will halt execution of the main processing until the dialog
        #  window is closed. 
        #
        varList = self.createControlValuesDialog(self.guiInfo, SC.Config["MaxPointsInColumn"])
        processVarList = ProcessVariableList.ProcessVariableList("Set Control Values", varList)
        if processVarList.status() != "OK":
            self.cancel()
        
        dialogValueDict = processVarList.varDict()
        values = []
        for label in self.guiInfo['Labels']:
            values.append(dialogValueDict[label])
        ...
        if wxType == 'SCALAR':  # scalar
            return np.clip(variableElement + zval, absmin, absmax)
        if wxType == 'VECTOR':  # vector
            ...
            if (self.guiInfo['vectedit'] == 1):
                newspd = speed
                newdir = direc + zval
                newdir[np.greater(newdir, 360)] -= 360
                newdir[np.less(newdir , 0)] += 360
            elif (self.guiInfo['vectedit'] == 0):
                newspd = np.clip(speed + zval, absmin, absmax)
                newdir = direc
            else:
                newspd = np.clip(speed + zval, absmin, absmax)
...
#
    def getMasterLabel(self, GridTimeRange, WEname):
        ...
        self.guiInfo["masterlabel"] = "Set %s for %d / %2.2dZ --> %d / %2.2dZ" % \
                                    (WEname, startday, starthour, endday, endhour)
        return
#---------------------------------------------------------------------------
#
...
#
    def setInitValues(self, wxType, variableElement, editAreaMask, Topo):
        ...
        for i in range(len(SC.Config["Locations"][self.setname])):
            ...
            #
            xint = int(self.round(x, "Nearest", 1) + 0.5)
            yint = int(self.round(y, "Nearest", 1) + 0.5)
...
#
    def handleEditArea(self, editAreaMask):
        ...
        #
        Topo = self.getTopo()
        allpts = np.add.reduce(np.add.reduce(np.less(Topo * 0.0, 5)))
        numpts = np.add.reduce(np.add.reduce(editAreaMask))
        if numpts == allpts:
            return
        #
        ...
        out1 = np.logical_and(np.greater(smooth1, 0), np.less(mask, 50))
        #
    ...
    def smoothpm(self, grid, k, mask=None):
        ...
        #
            ...
            gridmin1 = np.where(mask, gridmin, np.float32(0))
            mid = grid * 0.0
            midd = grid * 0.0
            ...
            for j in range(xmax):  # handle edges
                ...
                if jk2 == -1:
                    ...
                    out[:, nxm1 - j] = (c[:, nxm1] - c[:, jk2]) / np.maximum((d[:, nxm1] - d[:, jk2]), 1)
            if ((k + 1) <= (nx - k)):  # middle
                out[:, k + 1:nx - k] = (c[:, k2 + 1:] - c[:, :-k2 - 1]) / np.maximum((d[:, k2 + 1:] - d[:, :-k2 - 1]), 1)
            #
            #  Add the minimum back in
            #
            out = np.where(mask, out + fullmin, grid)
        return out

    def createControlValuesDialog(self, guiInfo, maxPointsInColumn):
        varList = [(guiInfo['masterlabel'],"","label"),
            ]
        ...
        numcols = (int((numpoints - 1) / maxPointsInColumn)) + 1
        ...
        for row in range(numrows):
            for col in range(numcols):
                ...
                lab = self.guiInfo['Labels'][num]
                initValue = self.guiInfo['InitValues'][num]
                
                if (self.guiInfo['vectedit'] == 2):
                    varList.append(FieldDef(lab, initValue, "compactText", 
                                            newRow = col==numcols-1))
                else:
                    varList.append(FieldDef(lab, initValue, "deltascale", 
                                            [self.guiInfo['minvalue'], 
                                             self.guiInfo['maxvalue']], 
                                            self.guiInfo['resolution'], 
                                            newRow = col==numcols-1))
