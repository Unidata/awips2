...
# ------------- -------- --------- ---------------------------------------------
# Dec 17, 2018  21046    smoorthy  Adding in change to replace 1.0e-200 which 
#                                  causes NaN values to appear
# Dec 28, 2018  21046    smoorthy  Adding change to replace 1.0e-200 which 
#                                  causes NaN values to appear
...
class ObjAnal(SmartScript.SmartScript):
   ...
   def ObjectiveAnalysis(self,values,guessGrid,analysisType,
      ...
      if ((gridType is not tuple)and(gridType is not list)):
   ...
   def setupScaling(self,xlist,ylist,hlist,elevfactor,Topo,landMask):
      ...
      #
      if landMask is None:
         newlandMask = self.newGrid(True, np.bool)
      else:
         newlandMask=landMask.astype(np.bool)
