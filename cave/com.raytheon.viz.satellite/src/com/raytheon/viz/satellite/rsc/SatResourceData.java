...
public class SatResourceData extends AbstractRequestableResourceData {
        ...
        if (!isShowIncompleteFrames() && (updateData instanceof PluginDataObject
            ...
            if (previousTimes != null) {
                ...
                try {
                    ...
                    List<DataTime> newTimes = new ArrayList<DataTime>(Arrays
                    ...
                    if (newTimes.size() > 0) {
                        PluginDataObject[] objs = requestPluginDataObjects(
                                newTimes);
                        List<PluginDataObject> newArray = new ArrayList<PluginDataObject>();
    ...
    protected DataTime[] getAvailableTimes(
        ...
        /*
         * Don't actually need gid, just need to add it along with distinct so
         * that each time is repeated based off how many distinct gids it has.
         */
        ...
        DataTime[] times = response
                .getFieldObjects(PluginDataObject.DATATIME_ID, DataTime.class);
        int max = 1;
        for (int i = 0; i < times.length; i += 1) {
            Integer gidCount = gidCounts.get(times[i]);
            if (gidCount == null) {
                gidCounts.put(times[i], 1);
            } else {
                gidCounts.put(times[i], gidCount + 1);
        ...
        return uniqueTimes.toArray(new DataTime[0]);
    }
