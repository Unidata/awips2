...
public class SatRenderable implements IRenderable {
    ...
    private Map<SatMapCoverage, SatTileSetRenderable> tileMap = new HashMap<>();
    ...
    /**
     * Performs
     * {@link TileSetRenderable#scheduleImagesWithinExtent(IGraphicsTarget, IExtent, Rectangle)}
     * on all the tilesets in this renderable
     * 
     * @return true if all tilesets are fully loaded, false if any images are
     *         still loading in the background.
     * 
     * @see TileSetRenderable#scheduleImagesWithinExtent(IGraphicsTarget,
     *      IExtent, Rectangle).
     */
    public boolean scheduleImagesWithinExtent(
            IGraphicsTarget target, IExtent extent, Rectangle canvasBounds) {
        boolean result = true;
        synchronized (tileMap) {
            for (SatTileSetRenderable renderable : tileMap.values()) {
                result &= renderable.scheduleImagesWithinExtent(target, extent,
                        canvasBounds);
            }
        }
        return result;
