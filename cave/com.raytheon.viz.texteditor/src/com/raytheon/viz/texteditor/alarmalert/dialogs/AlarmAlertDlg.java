 ...
 * ------------- -------- --------------- --------------------------------------
 * Sep 09, 2009           mnash           Initial creation
 * Oct 31,2011   8510     rferrel         Cleaned up code made more robust
 * Sep 20,2011   1196     rferrel         Change dialogs so they do not block.
 * Mar 05,2013   15173    mgamazaychikov  Set the initial location and dimension
 *                                        of dialog as it is in A1.
 * Jun 23, 2014  3161     lvenable        Added SWT dialog trim to the dialogs
 *                                        for thin client.
...
public class AlarmAlertDlg extends CaveSWTDialog {
    ...
    private java.util.List<AlarmAlertProduct> aapList = new ArrayList<>();

    private java.util.List<AlarmAlertProduct> papList = new ArrayList<>();
    ...
    private void doSaveAs(String fname) {
        ...
        if (fname != null && !fname.isEmpty()) {
            ...
            setLastFile(fname);
            // save alert products
            LocalizationFile lFile = AlarmAlertFunctions
                    .getFile(AlarmAlertFunctions.initUserLocalization(), fname);
            if (lFile != null) {
                ...
                statusHandler.handle(Priority.SIGNIFICANT,
                        ...
                        new VizException("Null localization file"));
            }
        }
    }
                ...
                if (aaList.getSelectionIndex() != -1) {
                    ...
                    proximityAlarmDlg.addCloseCallback(new ICloseCallback() {
                        ...
                        public void dialogClosed(Object returnValue) {
                            ...
                            if (proximityAlarmDlg.haveOkEvent()) {
                                ...
                                aaList.setItem(index,
                                        prod.getProductId() + string);
                                paList.deselectAll();
                                aaList.select(index);
                    ...
                    proximityAlarmDlg.addCloseCallback(new ICloseCallback() {
                        ...
                        public void dialogClosed(Object returnValue) {
                            ...
                            if (proximityAlarmDlg.haveOkEvent()) {
                                paList.setItem(paList.getSelectionIndex(),
                                        prod.getProductId() + " "
                                                + prod.getAlarmType() + " "
                                                + prod.getActionCmd() + " "
                                                + AlarmAlertFunctions
    ...
    private void createPAList() {
        ...
        proximityAlarmLabel.setText(
                "Proximity Alarm Products\n(ProductID, Alarm/Alert, Action Name, Area Definition)");
    ...
    private void createAcknowledgeButtons() {
        ...
        Button saveAndCloseButton = new Button(ackButtons, SWT.PUSH);
    ...
    public boolean shouldClose() {
        if (newChanges) {
            MessageDialog dlg = new MessageDialog(shell, "Save Changes?", null,
                    ...
                    new String[] { "Save", "Discard", "Cancel" }, 1);
    ...
    private AAPACombined loadLastList() {
        ...
        if (AlarmAlertDlg.currentFile == null) {
            ...
            return loadLastList();
                ...
                && !"default.xml".equals(AlarmAlertDlg.currentFile.getName())) {
            setLastFile("default.xml");
            return loadLastList();
        } else {
    ...
    private void setShellText(String fname) {
        shell.setText(
                "( " + fname + " ) Alarm/Alert and Proximity Alarm Products");
    }
}
