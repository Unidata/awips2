 ...
 * Nov 16, 2016 19518   mgamazaychikov Specify font name, improve font scaling mechanism, fix
 *                                     last word printing bug
 * 
 * </pre>
 * 
 * @author rferrel
...
public class PrintDisplay {
    ...
    private PrintDisplay(Printer printer, String text, FontData fontData) {
        ...
        printerFontData.setName(FONT_NAME);
    }
    ...
    private void setFont() {
        ...
        /*
         * Calculate the extent (in pixels) of the longest line of the product
         */
        String[] textLines = textToPrint.split("[\n]");
        int maxLineWidthPixels = gc.stringExtent(textLines[0].toString()).x;
        int maxCount = 0;
        for (int counter = 1; counter < textLines.length; counter++) {
            int currentLineWidthPixels = gc.stringExtent(textLines[counter].toString()).x;
            if (currentLineWidthPixels > maxLineWidthPixels) {
                maxLineWidthPixels = currentLineWidthPixels;
                maxCount = counter;
            }
        }

        /*
         * Create a buffer for computing tab width in pixels.
         */
        int tabChar = 4;
        StringBuilder aBuffer = new StringBuilder(tabChar);

        for (int i = 0; i < tabChar; i++) {
            aBuffer.append(' ');
        }

        /*
         * Calculate the extent (in pixels) of the longest line of the product plus tabs
         */
        int totalLineWidthPixels = maxLineWidthPixels + gc.stringExtent(aBuffer.toString()).x;

        /*
         * Get the device's width in pixels
         */
         ...
         */
        float fontSize = (float) deviceWidthPixels / (float) totalLineWidthPixels
                * (float) origFontSize;
        ...
        while (!isValidated) {
            ...
            gc.setFont(tmpFont);
            totalLineWidthPixels = gc.stringExtent(textLines[maxCount]
                    .toString()).x + gc.stringExtent(aBuffer.toString()).x;
            if (totalLineWidthPixels < deviceWidthPixels) {
                isValidated = true;
            } else {
                fontSize--;
                tmpFont.dispose();
