...
/**
 * Implements a dialog that queries the user for one or more application
 * arguments. When the Run button is clicked, the values are obtained from the
 * text entry fields and returned in a String[] wrapped as an object. The
 * arguments can be made optional. If the arguments are not optional, the dialog
 * can only be canceled until values for all arguments are provided. When the
 * arguments are optional, individual arguments may be left blank.
 *
 * <pre>
 *
 * SOFTWARE HISTORY
...
public class AppArgsDlg extends Dialog {
    ...
    public Object open() {
        ...
        createDataEntryArea();
        shell.pack();
        DialogUtil.centerOnParentShell(getParent(), shell);
        shell.open();
