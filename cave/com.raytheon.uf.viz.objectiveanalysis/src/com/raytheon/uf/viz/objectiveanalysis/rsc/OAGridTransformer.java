...
public class OAGridTransformer {
    ...
    public OAGridTransformer(GeneralGridGeometry geom2D,
        ...
        try {
            ...
            throw new VizException("Error preparing Objective Analysis Grid",
                    e);
        }
    }
    ...
    private float[] getGrid(PointDataContainer pdc, String parameter)
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (Float.isNaN(value) || value <= -9999 || xind[i] < 0
        ...
        float[] grid = null;
        grid = ScalelessAnalysis.scaleless_analysis(xind, yind, values, size,
                nx, ny);
        if (grid != null) {
     ...
     * @param constraints
     */
    public void addLatLonConstraints(
        ...
        longitudeRequestConstraint.setBetweenValueList(
                new String[] { Double.toString(lon0), Double.toString(lon1) });
        ...
        latitudeRequestConstraint.setBetweenValueList(
                new String[] { Double.toString(lat0), Double.toString(lat1) });
