...
public class KmlExportJob extends Job {
    ...
    private final JobPool backgroundPool = new JobPool("Exporting KML", 4, true,
            Job.INTERACTIVE);
    ...
    protected IStatus run(IProgressMonitor monitor) {
        SubMonitor smonitor = SubMonitor.convert(monitor, "Generating KML",
                800);
                ...
                try {
                    ...
                    if (fi.getFrameTimes() != null) {
                        ...
                        if (index > options.getFirstFrameIndex()
                            ...
                            pane.setDisplayedTime(
                                    fi.getFrameTimes()[fi.getFrameIndex()]);
                        }
                    }
                    ...
                    KmlGraphicsTarget target = graphicsAdapter
                            .constructTarget(null, 0.0f, 0.0f);
                    target.setBackgroundColor(display.getBackgroundColor());
                    display.setup(target);
                    pane.setTarget(target);

                    /*
                     * Remove all resources that were not explicitly set to be
                     * exported.
                     */
                    List<ResourcePair> toRemove = new ArrayList<>();
                    for (ResourcePair rp : pane.getDisplay().getDescriptor()
                            .getResourceList()) {
                        if (!pane.getResourcesToExport().contains(rp)) {
                            toRemove.add(rp);
                        }
                    }
                    pane.getDisplay().getDescriptor().getResourceList()
                            .removeAll(toRemove);
    ...
    private void exportPanes(IProgressMonitor monitor, KmlOutputManager out)
        ...
        for (KmlPane pane : options.getPanes()) {
            ...
            IDescriptor descriptor = display.getDescriptor();
            List<ResourcePair> exports = new ArrayList<>();
            for (ResourcePair rp : descriptor.getResourceList()) {
                if (pane.getResourcesToExport().contains(rp)) {
                    exports.add(rp);
                } else {
    ...
    private void exportResources(IProgressMonitor monitor, KmlOutputManager out,
            List<ResourcePair> exports, KmlPane pane) throws IOException {
        ...
        for (int c = 0; c < exports.size(); c++) {
            ...
            if (rsc.hasCapability(BlendableCapability.class)) {
                ...
                lastIndex = Math.min(lastIndex,
                        descriptor.getFramesInfo().getFrameCount());
                // in case there's zero frames (i.e. all time agnostic)
                lastIndex = Math.max(lastIndex, 1);
                rscmonitor.beginTask("Saving " + rsc.getName(),
                ...
                List<DataTime> pastFrames = new ArrayList<>();
                for (int i = startIndex; i < lastIndex; i += 1) {
                    ...
                    if (rsc.isTimeAgnostic()
                        ...
                        timeOut = resourceOut
                                .createFolder(time.getLegendString());
                        timeOut.getContainer()
                                .setTimePrimitive(getTimePrimitive(times, i));
                        pastFrames.add(time);
                    }
                    PaintProperties paintProps = new PaintProperties(1.0f,
                            (float) display.getZoom(), display.getView(),
                            pane.getBounds(), false,
    ...
    private void setView(KmlPane pane, KmlOutputManager out) {
        ...
        try {
            ...
            if (pane.getDisplayedTime() != null) {
                ...
                ts.setWhen(
                        KML_TIME_FORMAT.format(new Date(time.getMatchValid())));
                // At the time of this writing the current api doesn't allow
                // setting time primitive for AbstractView
                lookAt.setAbstractViewObjectExtension(
                        Arrays.asList((AbstractObject) ts));
            }
    ...
    private TimePrimitive getTimePrimitive(DataTime[] times, int index) {
        ...
        case TIME_SPAN: {
            ...
            span.setBegin(
                    KML_TIME_FORMAT.format(new Date(validTime - prevDist)));
     ...
     */
    private void recursiveInvisibility(Feature feature,
    ...
    private void paintResource(IProgressMonitor monitor, KmlOutputManager out,
        ...
        while (target.isNeedsRefresh()
            ...
            if (System.currentTimeMillis()
                    - startTime > options.getMaxRefreshSeconds() * 1000) {
                statusHandler.handle(Priority.PROBLEM,
                        resource.getName() + " took more than "
                                + options.getMaxRefreshSeconds()
        ...
        List<KmlFeatureGenerator> generators = new ArrayList<>(
    ...
    private void addColorMap(KmlOutputManager out, RGB backcolor,
        ...
        for (LabelEntry label : parameters.getLabels()) {
            ...
            Rectangle2D bounds = graphics.getFontMetrics()
                    .getStringBounds(label.getText(), graphics);
        ...
        overlay.createAndSetIcon()
                .setHref(out.addImage(bi, "colormap" + xAnchor + ".png"));
