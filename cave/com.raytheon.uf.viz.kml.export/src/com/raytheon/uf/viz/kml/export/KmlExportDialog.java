 ...
 *                                    use by sub-classes.
 * Apr 24, 2018  6985     njensen     Block selecting disabled resources
 * 
 * </pre>
 * 
 * @author bsteffen
...
public class KmlExportDialog extends CaveSWTDialog {
    ...
    protected void initializeComponents(Shell shell) {
        ...
        leftComposite
                .setLayoutData(new GridData(SWT.NONE, SWT.FILL, false, true));
    ...
    protected void initializeOptionsGroup(Group group) {
        ...
        exportHiddenButton.setToolTipText(
                "Include hidden products in the selection of products to export.");
        ...
        exportMapsButton.setToolTipText(
                "Include maps in the selection of products to export.");
        ...
        fillPlotsButton.setToolTipText(
                "Displays an opaque background tile behind point observations");
    }
    ...
    protected void initializeTimeOptionsGroup(Group group) {
        ...
        setTimesButton.setToolTipText(
                "Causes Google Earth to display a time slider.");
        ...
        timeSpanButton.setSelection(
                options.getTimeMode() != KmlExportTimeMode.TIME_STAMP);
        timeSpanButton.setEnabled(setTimesButton.getSelection());
        timeSpanButton.setToolTipText(
                "Allow products to be visible over a time range.");
        timeStampButton = new Button(timeComposite, SWT.RADIO);
        timeStampButton.setText("Time Stamp");
        timeStampButton.setSelection(
                options.getTimeMode() == KmlExportTimeMode.TIME_STAMP);
        timeStampButton.setEnabled(setTimesButton.getSelection());
        timeStampButton.setToolTipText(
                "Makes each product visible only at its exact valid time.");
        ...
        productTree.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                if (e.detail == SWT.CHECK) {
                    ...
                    if (item.getText().contains(UNSUPPORTED)) {
    ...
    protected void populateProductTree() {
        ...
        List<String> disableList = findResourcesToDisable();
        if (options.isSinglePane()) {
            List<ResourcePair> rscList = options.getSinglePane()
                    .getResources(exportMaps, exportHidden);
            populateProductSubTree(rscList, disableList, null);
        } else {
            int index = 0;
            for (KmlPane pane : options.getPanes()) {
                ...
                if (!rscList.isEmpty()) {
                    if (item == null || item.getData() != pane) {
                        ...
                        item.setData(pane);
                        populateProductSubTree(rscList, disableList, item);
    ...
    protected void populateProductSubTree(List<ResourcePair> rscList,
            List<String> disableList, final TreeItem parent) {
        TreeItem[] items = parent != null ? parent.getItems()
                : productTree.getItems();
        int itemIndex = 0;
        int rscIndex = 0;
        while (itemIndex < items.length || rscIndex < rscList.size()) {
            ...
            if (item != null && item.getData() == pair) {
                ...
                if (parent != null) {
                    item = new TreeItem(parent, SWT.CHECK, rscIndex);
                } else {
                    item = new TreeItem(productTree, SWT.CHECK, rscIndex);
                }
                ...
                item.setData(pair);
                if (disableList
                        .contains(pair.getResource().getClass().getName())) {
                    item.setChecked(false);
                    item.setForeground(productTree.getDisplay()
                            .getSystemColor(SWT.COLOR_GRAY));
                    item.setText(item.getText() + " " + UNSUPPORTED);
                } else {
    ...
    private void treeItemChecked(TreeItem item) {
        for (TreeItem ti : item.getItems()) {
            if (!ti.getText().contains(UNSUPPORTED)) {
    ...
    protected void okPressed() {
        ...
        if (options.isSinglePane()) {
            List<ResourcePair> resourcesToExport = new ArrayList<>();
            for (TreeItem ti : productTree.getItems()) {
                if (ti.getChecked()) {
                    resourcesToExport.add((ResourcePair) ti.getData());
                }
            }
            options.getSinglePane().setResourcesToExport(resourcesToExport);
        } else {
    ...
    protected boolean validate() {
        ...
        if (options.getFirstFrameIndex() > options.getLastFrameIndex()
            ...
            mb.setMessage(
                    "The frame range you entered is invalid, please enter a valid range");
            mb.open();
            return false;
        }
        ...
        if (!file.getParentFile().exists()) {
            ...
            if (result == SWT.YES) {
                if (!file.getParentFile().mkdirs()) {
                    ...
                    mb.setMessage(
                            "An unspecified error has occured creating the directory, please select a new file location.");
                    mb.open();
                    return false;
                }
            } else {
        ...
        if (file.exists()) {
            ...
            mb.setMessage(
                    "The specified file already exist. Would you like to overwrite it?");
            int result = mb.open();
            if (result == SWT.NO) {
                return false;
            }
    ...
    private List<String> findResourcesToDisable() {
        List<String> resourcesToDisable = new ArrayList<>();
        IExtensionRegistry registry = Platform.getExtensionRegistry();
        IExtensionPoint point = registry
                .getExtensionPoint("com.raytheon.uf.viz.kml.export.disable");
        if (point != null) {
            ...
            for (int i = 0; i < extensions.length; i++) {
                ...
                for (IConfigurationElement ele : config) {
                    ...
                    resourcesToDisable.add(clazz);
                }
            }
        }

        return resourcesToDisable;
    }
}
