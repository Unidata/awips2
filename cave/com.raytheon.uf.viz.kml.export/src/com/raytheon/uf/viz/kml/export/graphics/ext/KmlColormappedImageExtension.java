...
public class KmlColormappedImageExtension extends
    ...
    private static class Generator extends KmlGroundOverlayGenerator {
        ...
        private void drawRasters(KmlOutputManager out,
            ...
            Map<GridGeometry2D, DataSource> matches = new HashMap<>();
            List<DrawableImage> others = new ArrayList<>();
            ...
            for (DrawableImage image : images) {
                ...
                GridGeometry2D imageGeometry = null;
                if (mesh == null){
                    try {
                        kmlImage.loadData();
                    ...
                    imageGeometry = new GridGeometry2D(
                            gridRange, userRange);
                } else {
                    imageGeometry = mesh.getImageGeometry();
                }
                try {
                    if (params == null) {
                        params = kmlImage.getColorMapParameters();
                        interpolated = kmlImage.isInterpolated();
                        matches.put(imageGeometry,
                                kmlImage.getData(imageGeometry));
        ...
        private Map<GridGeometry2D, DataSource> mergeTiles(
                Map<GridGeometry2D, DataSource> map) {
            Map<GridGeometry2D, DataSource> newmap = new HashMap<>();
            while (!map.isEmpty()) {
