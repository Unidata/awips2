 ...
 *                                  correct monitor
 * Jun 19, 2018  6915     mduff     Cleared the damStrTree when resetting data map.
...
public class DamLocationResource
        extends AbstractVizResource<DamLocationResourceData, IDescriptor>
        implements IContextMenuContributor {

    private static final IUFStatusHandler statusHandler = UFStatus
            .getHandler(DamLocationResource.class);

    private static final RGB LABEL_COLOR = RGBColors.getRGBColor("white");

    private List<DamMaster> dams = new ArrayList<>();
    ...
    private void populateDamList() {
        ...
        if (damList != null) {
            ...
            displayManager.setDamLocationResource(this);
    ...
    protected void paintInternal(IGraphicsTarget target,
        ...
        List<PointImage> images = new ArrayList<>(dams.size());
        List<DrawableString> strings = new ArrayList<>(dams.size() * 2);
            ...
            if (pixel != null && extent.contains(pixel)) {
                ...
                if (manager.isDisplayDamId()) {
                    Coordinate idCoor = new Coordinate(
                            pixel[0] - getScaleWidth(),
                            pixel[1] - getScaleHeight());
                ...
                if (!elements.isEmpty()) {
                    ...
                    for (Object obj : elements) {
                        ...
                        double distance = Math
                                .pow((dam.getLatitudeDam() - coord.y), 2)
                                + Math.pow((dam.getLongitudeDam() - coord.x),
                                        2);
                        if (distance < curDist) {
                            curDist = distance;
                            foundDam = dam;
                    ...
                    try {
                        ...
                        alh.execute(shell, DC_BUNDLE_LOC, foundDam.getNidid());
                    } catch (ExecutionException e) {
                        statusHandler.error(
                                "Error executing the DamCrest Bundle", e);
                    }
                } else {
