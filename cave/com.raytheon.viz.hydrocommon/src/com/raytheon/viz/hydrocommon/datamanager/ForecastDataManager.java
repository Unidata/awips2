...
public class ForecastDataManager extends HydroDataManager {
    ...
    private static final String INSERT_INTO_STATEMENT = "INSERT INTO %s (lid, pe, dur, ts, extremum, probability, validtime, "
            + "basistime, value, shef_qual_code, quality_code, revision, product_id, producttime, postingtime) VALUES ('%s', "
            + "'%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')";

    private static final String SELECT_STATEMENT = "SELECT lid, pe, dur, ts, extremum, value, shef_qual_code, quality_code, revision, product_id, producttime, postingtime, validtime FROM %s";

    private static final String UPDATE_STATEMENT = "UPDATE %s SET value='%s', shef_qual_code='%s', quality_code='%s', revision='%s', product_id='%s', producttime='%s', postingtime='%s' WHERE %s";
     ...
     * @throws VizException
     */
    public void deleteRecords(List<ForecastData> recordsToDelete)
            throws VizException {
    ...
    public void deleteRecord(ForecastData recordToDelete) throws VizException {
        runStatement(String.format(DELETE_STATEMENT,
                HydroUtils.getTableName(recordToDelete.getPe(),
                        recordToDelete.getTs()),
     ...
     * @throws VizException
     */
    public List<Object[]> getForecastData(ForecastData currData)
            throws VizException {
        ...
        dataQuery.append(String.format(SELECT_STATEMENT,
                HydroUtils.getTableName(currData.getPe(), currData.getTs())));
    ...
    private void updateForecastData(ForecastData currData) throws VizException {
        runStatement(
                String.format(UPDATE_STATEMENT,
                        HydroUtils.getTableName(currData.getPe(),
                                currData.getTs()),
                        currData.getValue(), currData.getShefQualCode(),
                        currData.getQualityCode(), currData.getRevision(),
                        currData.getProductID(),
                        dbFormat.format(currData.getProductTime()),
                        dbFormat.format(currData.getPostingTime()),
    ...
    private void insertForecastData(ForecastData currData) throws VizException {
        ...
        runStatement(String.format(INSERT_INTO_STATEMENT,
                HydroUtils.getTableName(currData.getPe(), currData.getTs()),
                currData.getLid(), currData.getPe(), currData.getDur(),
                currData.getTs(), currData.getExtremum(),
                currData.getProbability(),
                dbFormat.format(currData.getValidTime()),
                dbFormat.format(currData.getBasisTime()), currData.getValue(),
                currData.getShefQualCode(), currData.getQualityCode(),
                currData.getRevision(), currData.getProductID(),
                dbFormat.format(currData.getProductTime()),
                dbFormat.format(currData.getPostingTime())));
    }
    ...
    public void putForecastData(ForecastData currData) throws VizException {
        ...
        if (!getForecastData(currData).isEmpty()) {
            // Do an update
