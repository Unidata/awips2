 ...
 * 
 * If the forecast for highest ranked type-source forecast value is in the past,
 * then this function recomputes the max forecast.
 * 
 * <pre>
 * 
 * SOFTWARE HISTORY
...
public class MaxObsFcst {
    ...
    public HydroDataReport[] calcMaxObsFcst() {
        ...
        Calendar beginBasisTime = Calendar
                .getInstance(TimeZone.getTimeZone("GMT"));
        beginBasisTime.add(Calendar.HOUR, fcstBasisHrsAgo * -1);

        Calendar bestObsValidTime = Calendar
                .getInstance(TimeZone.getTimeZone("GMT"));
        bestObsValidTime.setTimeInMillis(0);

        Calendar bestFcstValidTime = Calendar
                .getInstance(TimeZone.getTimeZone("GMT"));
        ...
        String where = String.format(
                ...
                lid, primaryPe, sdf.format(beginTime.getTime()),
                sdf.format(beginBasisTime.getTime()));
        ...
        try {
             ...
             */
            if (rsList != null) {
                String where2 = String
                        .format(" WHERE lid= '%s' AND pe= '%s' AND ingest= 'T' "
                                + " ORDER BY 1 ", lid, primaryPe);

                uList = HydroDataAccessor.loadUnique("ts_rank||ts",
                        "ingestFilter", where2);
            }

        } catch (VizException e) {
            statusHandler.error("Error getting ts and ts rank data for " + lid,
                    e);
        }
            ...
            for (String[] sa : uList) {
                ...
                if ((useTs.startsWith("R") || useTs.startsWith("P"))
                    ...
                    for (Riverstatus riverStatus : rsList) {
                         ...
                         * current ts and the validtime for the current ts is
                         * more recent than the previous validtime for ts with a
                         * possible matching rank
                         */
                        if (riverStatus.getId().getTs().equals(useTs)
                                && (riverStatus.getValidtime()
                                        .getTime() > bestObsValidTime
                                                .getTimeInMillis())) {
                            ...
                            bestObsValidTime.setTimeInMillis(
                                    riverStatus.getValidtime().getTime());
                            obsFound = true;
                            break;
                        }
                    }
                    ...
                    for (Riverstatus riverStatus : rsList) {
                        if (riverStatus.getId().getTs().equals(useTs)) {
                            ...
                            if (riverStatus.getValidtime().getTime() < now
                                ...
                                // update to use fcstReport object as in
                                // get_curobs_maxfcst.c line 248
                                ...
                                fcstReport.setValidTime(
                                        riverStatus.getValidtime());
                                fcstReport.setBasisTime(
                                        riverStatus.getBasistime());
            ...
            if (recomputeMaxFcst) {
                ...
                where = String.format(
                        " WHERE lid='%s' AND pe='%s' AND ts='%s' "
                                + " AND validtime >= '%s'  "
                                + " AND basistime >= '%s'  ",
                        lid, primaryPe, useTs, sdf.format(now.getTime()),
                        sdf.format(beginBasisTime.getTime()));
                try {
                    rsList = pdcManager.getRiverStatusList(where);
                    ...
                    /*
                     * find the best value, considering the different
                     * type-sources
                     */
                        ...
                        if (useTs.startsWith("F")) {
                            for (Riverstatus riverStatus : rsList) {
                                if (riverStatus.getId().getTs().equals(useTs)) {
                                    fcstReport = new HydroDataReport();
                                    fcstReport
                                            .setPe(riverStatus.getId().getPe());
                                    fcstReport.setDur(riverStatus.getDur());
                                    fcstReport
                                            .setTs(riverStatus.getId().getTs());
                                    fcstReport.setExtremum(
                                            riverStatus.getExtremum());
                                    fcstReport.setValue(riverStatus.getValue());
                                    fcstReport.setValidTime(
                                            riverStatus.getValidtime());
                                    fcstReport.setBasisTime(
                    ...
                    statusHandler
                            .error("Error querying the riverstatus table for lid "
                                    + lid, e);
                }
            }
        }
