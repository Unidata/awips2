...
public class SetTimeVals {
    ...
    private Calendar obsBeginTime = Calendar
            .getInstance(TimeZone.getTimeZone("GMT"));

    private Calendar fcstEndTime = Calendar
            .getInstance(TimeZone.getTimeZone("GMT"));

    private Calendar basisTime = Calendar
            .getInstance(TimeZone.getTimeZone("GMT"));
    ...
    public void setTimeValues() {
        ...
        obsBeginTime.setTimeInMillis(currentMillis
                - (HydroConstants.MILLIS_PER_HOUR * (long) obshrs));
        fcstEndTime.setTimeInMillis(currentMillis
                + (HydroConstants.MILLIS_PER_HOUR * (long) fcsthrs));
        basisTime.setTimeInMillis(currentMillis
                - (HydroConstants.MILLIS_PER_HOUR * (long) basishrs));
    }
    ...
    private void getHrVals() throws VizException {
        ...
        results = (ArrayList<Rpfparams>) HydroDataAccessor.getRpfParams(" ");
        ...
        if ((basisHrs != null) && (basisHrs.length() > 0)) {
            ...
            if ((hrValue <= 0) || (hrValue > 480)) {
                // TODO Log this message
                // fprintf(stderr,
                // "invalid value for basis_hours_filter token:
                // %s\n",basis_hours_str);
            } else {
