...
/**
 * Crest History data manager for CRUD operations on the Crest table in IHFS.
 * 
 * <pre>
 *
 * SOFTWARE HISTORY
 ...
 * Nov 04, 2010   5518        lbousaid    added all/above/below flag to getRiverCrestData
...
public class CrestHistoryDataManager {
    ...
    private static final String INSERT_MARK = "X";
     ...
     */
    public static final synchronized CrestHistoryDataManager getInstance() {
    ...
    public String insertUpdateCrest(CrestData cd, String lid) {
        ...
        if (checkData(cd, lid) == 0) {
            ...
            if (cd.isHighWater()) {
                insertQuery.append("'").append(INSERT_MARK).append("', ");
            } else {
                insertQuery.append("null").append(", ");
            }
            if (cd.isIce()) {
            ...
            if (cd.isHighWater()) {
                updateQuery.append("hw = '").append(INSERT_MARK).append("', ");
            } else {
                updateQuery.append("hw = null, ");
            }

            if (cd.isIce()) {
                updateQuery.append("jam = '").append(INSERT_MARK).append("', ");
            } else {
                updateQuery.append("jam = null, ");
            }

            if (cd.isOldDatum()) {
                updateQuery.append("olddatum = '").append(INSERT_MARK)
                        .append("', ");
            } else {
            ...
            if (cd.isSuppress()) {
                updateQuery.append("suppress = '").append(INSERT_MARK)
                        .append("', ");
            } else {
    ...
    private int checkData(CrestData cd, String lid) {
        int rval = 0;
        ...
        String query = "select lid from crest where lid = '" + lid
                ...
                + cd.getTimeString() + "'";
        try {
            rval = DirectDbQuery.executeMappedQuery(query, HydroConstants.IHFS,
                    QueryLanguage.SQL).getResultCount();
        } catch (VizException e) {
            statusHandler.handle(Priority.ERROR,
