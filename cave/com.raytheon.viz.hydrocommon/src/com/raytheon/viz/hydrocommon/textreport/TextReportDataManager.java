...
/**
 * HydroBase Text Report data access.  Writing individual inner classes
 * because of the Hibernate/Thrift issues/problems with the IHFS.
 * 
 * <pre>
 * 
 * SOFTWARE HISTORY
...
public class TextReportDataManager extends HydroDataManager {
    ...
    private HashMap<String, TextReportData> reportDataMap = 
        new HashMap<String, TextReportData>();
    ...
    /**
     * Get the data for the lid.
     * 
     * @param lid
     *      The site to get data for
     *      
     * @return
     *      The TextReportData object for that lid
     */
    public TextReportData getData(String lid) {
        if (!reportDataMap.containsKey(lid)) {
            TextReportData data = getSiteData(lid);
            reportDataMap.put(lid, data);
        }
        
        return reportDataMap.get(lid);
    }
     ...
     *      The location id
     * @return
     *      The TextReportdata object
     */
    private TextReportData getSiteData(String lid) {
        TextReportData data = new TextReportData();
        data.setLid(lid);

        String locationQuery = "select county, coe, cpm, detail, elev, hdatum, hsa, hu, lat, lon, lremark, lrevise, name, network, rb, rfc, sbd, sn, state, waro, wfo, wsfo, type, des, det, stntype, tzone from location where lid = '" + lid + "'";
        ...
        String stnclassQuery = "select disp_class, dcp, observer, telem_type from Stnclass where lid = '" + lid + "'";
        rs = runQuery(stnclassQuery);
        ...
        String riverstatQuery = "select lat, lon, stream, da, fs, fq, mile, wstg, gsno, zd, bf, cb, pool, tide, por, remark, rrevise, rsource, vdatum, level, rated, action_flow from riverstat where lid = '" + lid + "'";
        ...
        String observerQuery = "select dos, gn, a1, a2, a3, hphone, phone, recip, city, state, zip, comm, email, rprt, spons, ornr, rate, tsk from Observer where lid = '" + lid + "'";
        rs = runQuery(observerQuery);
        ...
        String proximityQuery = "select proximity, bed, reach, res, divert, ice, topo, remark from descrip where lid = '" + lid + "'";
        rs = runQuery(proximityQuery);
        ...
        String dcpQuery = "select goes, owner, rptime, rptfreq, criteria from dcp where lid = '" + lid + "'";
        rs = runQuery(dcpQuery);
        ...
        String floodcatQuery = "select minor_stage, moderate_stage, major_stage from floodcat where lid = '" + lid + "'";
        rs = runQuery(floodcatQuery);
        ...
        String telemQuery = "select type, owner, phone, rptfreq, criteria, cost, payor from telem where lid = '" + lid + "'";
       ...
       String gageQuery = "select gbegin, type, owner, remark, maint, gend from gage where lid = '" + lid + "' order by gbegin,type";
        rs = runQuery(gageQuery);
        ...
        String benchmarkQuery = "select bnum, elev, remark from benchmark where lid = '" + lid + "' order by bnum";
        rs = runQuery(benchmarkQuery);
        ...
        String pubQuery = "select pbegin, ppub, pend from pub where lid = '" + lid + "'";
        rs = runQuery(pubQuery);
        ...
        String datumQuery = "select ddate, elev from datum where lid = '" + lid + "' order by ddate desc";
        ...
        String crestQuery = "select datcrst, cremark, hw, jam, olddatum, q, stage, suppress, timcrst, prelim from crest where lid = '" + lid + "' order by datcrst desc";
        rs = runQuery(crestQuery);
        ...
        String lwQuery = "select lwdat, q, stage, lwrem from lowwater where lid = '" + lid + "' order by lwdat desc";
        rs = runQuery(lwQuery);
        ...
        String crestWhere = " where lid = '" + lid + "' and ((suppress != 'X') or (suppress is null)) order by stage desc";
        ...
        String contactsQuery = "select contact, phone, email, remark, priority from contacts where lid = '" + lid + "' order by priority desc";
        rs = runQuery(contactsQuery);    
     ...
     * @return
     *      ArrayList of Crest objects
     */
    private ArrayList<Crest> getCrestList(String where) {
        String query = "select datcrst, cremark, hw, jam, olddatum, q, stage, suppress, timcrst, prelim from crest " + where;
     ...
     *      The location id
     * @return
     *      The Stnclass data object
     */
    public TextReportData.Stnclass getStnClass(String lid) {
        if (!stnclassMap.containsKey(lid)) {
            String query = "select disp_class, dcp, observer, telem_type from Stnclass where lid = '" + lid + "'";
            TextReportData data = new TextReportData();
     ...
     *      0 = by lid; 1 = by wfo; 2 = by hsa
     * @return
     *      ServiceBackupData array
     */
     ...
     * @param sort
     *      Which type of sorting to perform
     *      0 = lid; 1 = name; 2 = county; 3 = basin;
     *      4 = observer
     * @return
     *      StationList data array
     */
