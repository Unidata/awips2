...
public class E19Report extends TextReport {
    ...
    private String E19Cover() {
        ...
        buffer.append(String.format("   BASIN: %-30s   HSA: %s\n", data.getLocation().getRb(), data.getLocation().getHsa()));
        buffer.append("\n");
        ...
        if (data.getRefer()!= null) {
            count = data.getRefer().size();
        }
        
        if (count > 0) {
            for (String s: data.getRefer()) {
        ...
        for (Benchmark bm: benchList) {
            ...
            if (needed <= avail) {
                ...
                if ((bm.getElev() != HydroConstants.MISSING_VALUE) &&
                        (data.getRiverstat().getZd() != HydroConstants.MISSING_VALUE)) {
                    gageZero = String.format("%8.3f", (bm.getElev() - data.getRiverstat().getZd()));
                } else {
                    gageZero = "        ";
                }
                ...
                // Do footer
                buffer.append(createFooter(data, E19_RREVISE_TYPE, getDate(), "NWS FORM E-19", E19_BENCHMARKS, "BENCHMARKS", null, E19_STANDARD_LEFT_MARGIN));
    ...
    private String E19Gages() {
        ...
        ArrayList<Gage> gageList = data.getGageList();
    ...
    private String E19History() {
        ...
        //  Do footer.
        String footer = createFooter(data, E19_RREVISE_TYPE, sdf.format(new Date()), 
                "NWS FORM E-19", E19_HISTORY, "HISTORY", null, E19_STANDARD_LEFT_MARGIN);
    ...
    private String E19Crests() {
        ...
        for (Crest crest:  data.getCrestList()) {
    ...
    private String createFooter(TextReportData data, int reviseType, String currDate, String formName, int pageType, String pageTypeLabel, String optHeader, int leftMargin) {//, int reason) {
        ...
        String tmp = String.format("%s %s %s, %s", data.getRiverstat().getStream(), data.getDescrip().getProximity(), data.getLocation().getName(), data.getLocation().getState());
        
        String date = null;
        if (reviseType == E19_LREVISE_TYPE) {
            if (data.getLocation().getLrevise() != null) {
                date = sdf.format(data.getLocation().getLrevise());
            }
        ...
        if (subpageNum > 1) {
            buffer.append(String.format("      ID: %-11s                    HSA: %-20s  %s PAGE %d-%d: %s\n", lid, data.getLocation().getHsa(), formName, pageNum, subpageNum, pageTypeLabel));
        } else {
            buffer.append(String.format("      ID: %-11s                    HSA: %-20s  %s PAGE %d: %s\n", lid, data.getLocation().getHsa(), formName, pageNum, pageTypeLabel));
        }
