 ...
 * Jun 27, 2018 #6876      dgilling   Use small text size to fit report to page
 *                                    width.
 *
 * </pre>
 *
...
public class PrintE19TextReportDlg extends PrintTextReportDlg {
    ...
    private Button converBtn = null;

    private Button mapBtn = null;

    private Button benchmarkBtn = null;

    private Button gagesBtn = null;

    private Button historyBtn = null;

    private Button crestsBtn = null;

    private Button lowWaterBtn = null;

    private Button conditionsBtn = null;

    private Button damageBtn = null;

    private Button riverStageBtn = null;

    private Button contactsBtn = null;

    private Button allPageBtn = null;

    protected PrintE19TextReportDlg(Shell parentShell, E19Report rpt) {
        super(parentShell, rpt, 7);
        this.report = rpt;
    ...
    private class E19Selection extends SelectionAdapter {
        ...
        public void widgetSelected(SelectionEvent e) {
            Color yellow = Display.getCurrent()
                    .getSystemColor(SWT.COLOR_YELLOW);
            Color black = Display.getCurrent().getSystemColor(SWT.COLOR_BLACK);
            Color red = Display.getCurrent().getSystemColor(SWT.COLOR_RED);

            Button button = (Button) e.getSource();
            // selected button is not allpage
            if (button.equals(allPageBtn)) {
                ...
                if (button.getSelection()) {
                    if (button.equals(benchmarkBtn) || button.equals(gagesBtn)
                    ...
                    // selected button is off
                    button.setForeground(black);
                    /* every thing is off then allpage must be on */
                    if (!converBtn.getSelection() && !mapBtn.getSelection()
