 ...
 * Feb 23, 2011  5400        lbousaidi    fixed issues in color/value bar
 * Mar 11, 2013  15065       lbousaidi    fixed issue with both color legend
 *                           disappearing after save
 * Jul 01, 2013  2088        rferrel      Changes for non-blocking dialogs.
 * Sep 06, 2013  2342        lvenable     Fixed color memory leaks and a null
 *                                        point exception.
 * Sep 04, 2014  14448       cgobs        Make MPE redisplay after save of color
 *                                        settings in ColorScaleMgr
 * Feb 26, 2015  16848       cgobs        Fix merging of color sets by deleting
 *                                        existing set before saving new set.
 *                                        Updated to include fix of error when
 *                                        saving a new source.
 * Aug 06, 2018  7433        mduff        Remove message box from duration combobox action handler.
 * Apr 08, 2016  5512        bkowal       Minimal updates to fix GUI sizing
 *                                        issues.
 * Oct 06, 2017  6407        bkowal       Cleanup. Updates to support GOES-R
 *                                        SATPRE.
 * Apr 12, 2018  6607        randerso     Added/updated tooltip text
 * Jul 06, 2018  6885        mduff        Fixed issue with loading default colors, cleanup.
 *
 * </pre>
 *
 * @author lvenable
...
public class ColorScaleMgrDlg extends CaveSWTDialog {
    ...
    /**
     * callback to be executed upon saving of a color set generally used to
    ...
    private void createTabFolder() {
        ...
        tabFolder.addSelectionListener(new SelectionAdapter() {
            private Composite browseControl;
    ...
    private Composite createBrowseColorSetComp(Composite parentComp) {
        ...
        browseDataTypeCbo = new Combo(controlComp,
        ...
        browseDurationCbo = new Combo(controlComp,
                SWT.DROP_DOWN | SWT.READ_ONLY);
    ...
    private void createEditColorTopControls(Composite parentComp) {
        ...
        durationCbo.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                boolean good = true;
                try {
                    String dur = durationCbo.getText();
                    if (dur != null && !dur.isEmpty()) {
                        Integer.parseInt(dur);
                    }
                } catch (SWTException | NumberFormatException ex) {
                    good = false;
                }
    ...
    private void createEditColorControls(Composite parentComp) {
        ...
        changeBtn.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent event) {
                if (valueTF.getText() == null || valueTF.getText().isEmpty()) {
                    MessageBox mb = new MessageBox(shell,
        ...
        addBtn.setToolTipText(
                "Puts the edited color-value pair into the set in working memory.");
        ...
        undoBtn.setToolTipText(
                "Undoes the changes in working memory for the currently selected color set, returning it to what is stored in the database.");
        ...
        deleteBtn.setToolTipText(
                "Removes the edited color-value from the set in working memory.");
        ...
        deleteBtn.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent event) {
                if (valueTF.getText().compareTo(ColorScaleData.MISSING) == 0) {
                    ...
                    mb.setText("Warning");
                    mb.setMessage(
                            "Cannot remove 'missing' (MSG) from the list.");
                    mb.open();

                    return;
                } else if (valueTF.getText()
                        .compareTo(ColorScaleData.LESS_THAN_MIN) == 0) {
                    MessageBox mb = new MessageBox(shell,
                            SWT.ICON_ERROR | SWT.OK);
                    mb.setText("Warning");
                    mb.setMessage(
                            "Cannot remove 'less than min' (< Min) from the list.");
                    mb.open();
                    ...
                    try {
                        String source = getSource();
                        double dblValue = Double.parseDouble(valueTF.getText());
                        dblValue = dblValue * 100.0;
                        dblValue = Math.round(dblValue) / 100.0;
                        editColorData
                                .deleteColorValue(source,
                        ...
                        MessageBox mb = new MessageBox(shell,
                                SWT.ICON_ERROR | SWT.OK);
                        mb.setText("Error");
                        mb.setMessage("Not a valid value: " + valueTF.getText()
                                + ".");
                        mb.open();
    ...
    public void updateColor(String source) {
        ...
        if (valueTF.getText().compareTo(ColorScaleData.MISSING) == 0) {
            ...
            try {
                ...
                dblValue = dblValue * 100.0;
                dblValue = Math.round(dblValue) / 100.0;
                editColorData.updateColorValue(source,
                        selectedDurationInSeconds + "_" + dataTypeCbo.getText(),
                        colorLbl.getBackground().getRGB(), dblValue);

                updateColorValueLabelBar();
            } catch (Exception ex) {
                MessageBox mb = new MessageBox(shell, SWT.ICON_ERROR | SWT.OK);
                mb.setText("Error");
                mb.setMessage("Not a valid entry: " + valueTF.getText() + ".");
                mb.open();
    ...
    private void createDbControlsGroup(Composite parentComp) {
        ...
        saveAsUserBtn.setToolTipText(
                "For the current user, saves the currently selected color set to the database.");
        ...
        saveAsUserBtn.addSelectionListener(new SelectionListener() {
            ...
            @Override
            public void widgetSelected(SelectionEvent e) {
                boolean rval = MessageDialog.openConfirm(shell,
                        "Save Confirmation",
        ...
        saveAsOfficeBtn.setToolTipText(
                "For the office default settings, saves the currently selected color set to the database.");
        ...
        deleteAsUserBtn.setToolTipText(
                "Remove the color set with the currently selected DataType/Duration combination from the database for this user.");
        ...
        deleteAsOfficeBtn.setToolTipText(
                "Remove the color set with the currently selected DataType/Duration combination from the database for the office.");
    ...
    private void createEditColorDisplay(Composite parentComp) {
        ...
        colorValLblArray = new ArrayList<>();
    ...
    private void createBrowseEditColorDisplay(Composite parentComp) {
        ...
        browseColorValLblArray = new ArrayList<>();
        ...
        for (int i = 0; i < numCols; i++) {
            createBrowseColorLabel(browseLabelComp,
                    updatedColorSet.get(i).color,
                    browseColorValLblArray.get(i));
        }

        for (int i = 0; i < numCols; i++) {
            createBrowseValueLabel(browseLabelComp,
                    updatedColorSet.get(i).value,
                    browseColorValLblArray.get(i));
        }
    }
    ...
    private void createUsedColorDisplay(Composite parentComp) {
        ...
        usedColorValLblArray = new ArrayList<>();
    ...
    private void updateUsedColorSetGroupText() {
        StringBuilder strBuf = new StringBuilder("Used Color Set - Source: ");
        String source = getSource();
    ...
    private void updateDurationCombo() {
        ...
        if (sourceKey.equals(DEFAULT)) {
            ...
            String dataType = colorManager
                    .getDataTypeName(dataTypeCbo.getText());
            Set<String> durations = editColorData
                    .getColorDataTypeSets(sourceKey)
                    .getDurations(colorManager.getDescription(dataType));
    ...
    private void updateColorValueLabelBar() {
        ...
        if (updatedColorSet.isEmpty()) {
            updatedColorSet = editColorData.getColorScaleDataArray(source,
    ...
    private boolean createColorData(String user) {
        ...
        for (String dataType : userDataTypes) {
            ...
            List<String> durations = colorManager.getDurations(userId,
                    dataType);
            ...
            for (String duration : durations) {
                ...
                try {
                    ...
                    List<ColorScaleData> origList = new ArrayList<>();

                    for (ColorValueData colorValue : data) {
                        ColorScaleData csd = new ColorScaleData();
                        if (colorValue.getThresholdValue()
                                .startsWith("-8888")) {
                            csd.lessThanMinScaleData(RGBColors
                                    .getRGBColor(colorValue.getColorName()));
                        } else if (colorValue.getThresholdValue()
                                .startsWith("-9999")) {
                            csd.missingScaleData(RGBColors
                                    .getRGBColor(colorValue.getColorName()));
                        } else {
                            csd.setColor(RGBColors
                                    .getRGBColor(colorValue.getColorName()));
                            csd.setValueLbl(Double.parseDouble(
                                    colorValue.getThresholdValue()));
                        }
                        origList.add(csd);
                    }
                    List<ColorScaleData> usedList = new ArrayList<>();
    ...
    private boolean updateDataType(String source) {
        ...
        for (String dataType : dataTypes) {
            if (!dataType.contains("null")) {
                String description = colorManager.getDescription(dataType);
    ...
    private void deleteDataFromDb(String applicationName, String colorUseName,
        ...
        try {
            ...
            statusHandler.handle(Priority.DEBUG,
                    "Color set doesn't exist yet, can't delete it. ", e);
        }
    ...
    private String getStringFromColorValueData(ColorValueData cvd) {
        ...
        builder.append("appName: ").append(cvd.getApplicationName());
        builder.append(" colorUseName: ").append(cvd.getColorUseName());
        builder.append(" userId: ").append(cvd.getUserId());
        builder.append(" duration: ").append(cvd.getDuration());
    ...
    private void saveData(String user) {
        ...
        String userId = user;
        String applicationName = colorManager.getApplicationName();
        String colorUseName = colorManager
                .getDataTypeName(saveDataTypeCbo.getText());
    ...
    private void deleteData(String source) {
        ...
        String duration = selectedDurationInSeconds.toString();
        List<ColorScaleData> data = editColorData
    ...
    public String getSource() {
        String sourceColor = sourceCbo.getText();
