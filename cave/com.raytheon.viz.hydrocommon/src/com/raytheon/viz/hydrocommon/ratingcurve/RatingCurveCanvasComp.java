 ...
 * Sep 09, 2013  2349       lvenable    Fixed Font memory leak.
 * Apr 24, 2018  6992       mduff       Fixed drawing of curve where shift values = zero.
 * 
 * </pre>
 * 
 * @author lvenable
...
public class RatingCurveCanvasComp extends Canvas {
     ...
     * Canvas width.
     */
    private static final int CANVAS_WIDTH = 625;
     ...
     */
    private static final int HLINE_YCOORD = CANVAS_HEIGHT - 50;

    /**
     * X coordinate of the vertical graph line.
     */
    private static final int VLINE_XCOORD = 100;

    /**
     * Length of the horizontal graph line.
     */
    private static final int HLINE_LENGTH = 500;

    /**
     * Length of the vertical graph line.
     */
    private static final int VLINE_LENGTH = 400;

    /**
     * Length of the graph line hash mark.
     */
    private static final int HASH_MARK = 5;

    /**
     * Stage feet hash label x coordinate.
     */
    private static final int STAGE_FEET_HASH_LABEL_X = 40;

    /**
     * Stage feet text.
     */
    private static final String STAGE_FEET = "STAGE FEET";

    /**
     * Flow KCFS text.
     */
    private static final String FLOW_KCFS = "FLOW KCFS";
     ...
     */
    private double flowDivisor = 10000.0;
     ...
     */
    private double vNumHashs = 0.0;

    /**
     * Maximum stage feet label value.
     */
    private int maxStageFeetLabelVal = 0;
    ...
    private void calcMaxStageMaxFlow() {
        double maxStageFeet = -999;
        maxFlowKcfs = 0.0;
        double minStageFeet = 0.0;
        minFlowKcfs = 0.0;
        ...
        if (curveDataArray != null) {
            ...
            if (maxFlowKcfs > 100_000) {
                flowDivisor = 100_000;
            } else if (maxFlowKcfs > 10_000) {
                flowDivisor = 10_000;
            } else {
                flowDivisor = 1000;
            }
            maxFlowKcfs /= flowDivisor;
        ...
        maxStageFeetLabelVal = (int) ((vNumHashs * 10)
                - Math.abs(minStageFeetLabelVal));
    }
    ...
    private void calcPixelsPerIncrement() {
        ...
        int maxKcfsLabelVal = (int) (hNumHashs * 10);
    ...
    private void drawRatingCurve(GC gc) {
        ...
        for (char element : charArray) {
            gc.drawString(String.valueOf(element), 5, tmpY, true);
            tmpY += fontHeight;
        }
        ...
        gc.drawLine(VLINE_XCOORD, HLINE_YCOORD, VLINE_XCOORD,
                HLINE_YCOORD - VLINE_LENGTH);
        ...
        int recordYCoord = (int) (HLINE_YCOORD - Math.round(
                ((recordValDbl - minStageFeetLabelVal) * vPixelsPerInc)));

        gc.drawLine(VLINE_XCOORD + 1, recordYCoord, VLINE_XCOORD + HLINE_LENGTH,
                recordYCoord);
        ...
        for (int x = 0; x <= vNumHashs; ++x) {
            int yCoord = (int) (HLINE_YCOORD
                    - Math.round((x * vPixelsPerIncBase10)));
        ...
        for (int x = 0; x <= hNumHashs; ++x) {
            int xCoord = (int) (VLINE_XCOORD
                    + Math.round((x * hPixelsPerInc * 10)));

            if (xCoord <= VLINE_XCOORD + HLINE_LENGTH) {
                gc.drawLine(xCoord, HLINE_YCOORD, xCoord,
                        HLINE_YCOORD + HASH_MARK);

                kcfsStr = String.format("%5.1f",
                        (float) (x * flowDivisor / 1000));
                gc.drawString(kcfsStr,
                        xCoord - ((kcfsStr.length() * fontAveWidth) / 2)
                                - fontAveWidth,
                        HLINE_YCOORD + HASH_MARK * 2, true);
            }
        }
        ...
        for (RatingCurveData curveData : curveDataArray) {
            int xCoord = (int) (VLINE_XCOORD + Math
                    .round((((curveData.getDischarge() / flowDivisor) * 10)
                            * hPixelsPerInc)));

            int yCoord = 0;
            if (shiftAmount <= 0) {
                yCoord = (int) (HLINE_YCOORD - Math
                        .round(((curveData.getStage() - minStageFeetLabelVal)
                                * vPixelsPerInc)));
            } else {
                yCoord = (int) (HLINE_YCOORD
                        - Math.round(((curveData.getStage()) * vPixelsPerInc)));
            }
            ...
            gc.setForeground(
                    parent.getDisplay().getSystemColor(SWT.COLOR_YELLOW));
    ...
    private void updateLabels() {
        if ((currentX > VLINE_XCOORD)
                ...
                && (currentY > HLINE_YCOORD - VLINE_LENGTH)
                && (currentY < HLINE_YCOORD)) {
            double q = (((currentX - VLINE_XCOORD) / (hPixelsPerInc * 100)
                    * flowDivisor)) / 100;
            double stage = (((currentY - (minStageFeetLabelVal * vPixelsPerInc)
                    - HLINE_YCOORD) / vPixelsPerInc * -1));
            parentDlg.getStageDataLbl()
                    .setText(String.valueOf(String.format("%5.1f", stage)));
            parentDlg.getKcfsDataLbl()
                    .setText(String.valueOf(String.format("%5.1f", q)));
        }
    }
