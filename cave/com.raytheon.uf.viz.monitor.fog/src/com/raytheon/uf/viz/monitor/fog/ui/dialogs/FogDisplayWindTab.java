...
public class FogDisplayWindTab extends TabItemComp implements IUpdateDisplayWind
{
    /**
    ...
    {        
        ...
        for (int i = 0; i < fogDataArray.size(); i++)
        {
             ...
             * Wind Speed
             */
            appendIntData(sb, fdwd.getWindWindSpeedR(), fdwd.getWindWindSpeedY());
    ...
    private void createDataArray()
    {
        ...
        for (AreaXML area : areasArray)
        {
            ...
            fdwd.setWindWindSpeedR(ftm.getThresholdValue(duKey, threshKeyR, areaID, xmlKey));
            fdwd.setWindWindSpeedY(ftm.getThresholdValue(duKey, threshKeyY, areaID, xmlKey));

            /*
             * Peak Wind
             */
            xmlKey = FogDisplay.FOG_DISP_WIND_PEAK_WIND.getXmlKey();
            fdwd.setWindPeakR(ftm.getThresholdValue(duKey, threshKeyR, areaID, xmlKey));
            fdwd.setWindPeakY(ftm.getThresholdValue(duKey, threshKeyY, areaID, xmlKey));

            /*
             * Gust Speed
             */
            xmlKey = FogDisplay.FOG_DISP_WIND_GUST_SPEED.getXmlKey();
            fdwd.setWindGustR(ftm.getThresholdValue(duKey, threshKeyR, areaID, xmlKey));
            fdwd.setWindGustY(ftm.getThresholdValue(duKey, threshKeyY, areaID, xmlKey));

            /*
             * Wind Direction: From
             */
            xmlKey = FogDisplay.FOG_DISP_WIND_DIR_FROM.getXmlKey();
            fdwd.setWindDirFromR(ftm.getThresholdValue(duKey, threshKeyR, areaID, xmlKey));
            fdwd.setWindDirFromY(ftm.getThresholdValue(duKey, threshKeyY, areaID, xmlKey));

            /*
             * Wind Direction: To
             */
            xmlKey = FogDisplay.FOG_DISP_WIND_DIR_TO.getXmlKey();
            fdwd.setWindDirToR(ftm.getThresholdValue(duKey, threshKeyR, areaID, xmlKey));
            fdwd.setWindDirToY(ftm.getThresholdValue(duKey, threshKeyY, areaID, xmlKey));
    ...
    public void commitDataToXML()
    {
        ...
        for (FogDisplayWindData fdwd : fogDataArray)
        {
             ...
             */
            xmlKey = FogDisplay.FOG_DISP_WIND_WIND_SPEED.getXmlKey();
            ftm.setThresholdValue(duKey, threshKeyR, areaID, xmlKey, fdwd.getWindWindSpeedR());
            ftm.setThresholdValue(duKey, threshKeyY, areaID, xmlKey, fdwd.getWindWindSpeedY());

            /*
             * Peak Wind
             */
            xmlKey = FogDisplay.FOG_DISP_WIND_PEAK_WIND.getXmlKey();
            ftm.setThresholdValue(duKey, threshKeyR, areaID, xmlKey, fdwd.getWindPeakR());
            ftm.setThresholdValue(duKey, threshKeyY, areaID, xmlKey, fdwd.getWindPeakY());

            /*
             * Gust Speed
             */
            xmlKey = FogDisplay.FOG_DISP_WIND_GUST_SPEED.getXmlKey();
            ftm.setThresholdValue(duKey, threshKeyR, areaID, xmlKey, fdwd.getWindGustR());
            ftm.setThresholdValue(duKey, threshKeyY, areaID, xmlKey, fdwd.getWindGustY());

            /*
             * Wind Direction: From
             */
            xmlKey = FogDisplay.FOG_DISP_WIND_DIR_FROM.getXmlKey();
            ftm.setThresholdValue(duKey, threshKeyR, areaID, xmlKey, fdwd.getWindDirFromR());
            ftm.setThresholdValue(duKey, threshKeyY, areaID, xmlKey, fdwd.getWindDirFromY());

            /*
             * Wind Direction: To
             */
            xmlKey = FogDisplay.FOG_DISP_WIND_DIR_TO.getXmlKey();
            ftm.setThresholdValue(duKey, threshKeyR, areaID, xmlKey, fdwd.getWindDirToR());
            ftm.setThresholdValue(duKey, threshKeyY, areaID, xmlKey, fdwd.getWindDirToY());
        }
    }
    ...
    protected void editDataAction()
    {
        ...
        {
            fogWindEditDlg = new FogDisplayWindEditDlg(getParent().getShell(), fdwd, this);
            fogWindEditDlg.open();            
            fogWindEditDlg = null;
        }
    }
