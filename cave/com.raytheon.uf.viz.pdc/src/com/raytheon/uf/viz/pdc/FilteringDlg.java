    ...
    private void populateList() {
        ...
        try {
            if (dialogType == DialogType.DATA_SOURCE) {
                ...
                for (String element : dataSourceArray) {
                ...
                if ((pcOptions.getDataSourcesChosen() != null)
                    ...
                    for (int i = 0; i < pcOptions
                            .getDataSourcesChosen().length; i++) {
                        for (int j = 0; j < dataList.getItemCount(); j++) {
                            if (dataSourceArray[j].equalsIgnoreCase(
                                    pcOptions.getDataSourcesChosen()[i])) {
                                al.add(j);
                                break;
                            }
                        }
                ...
                List<String> hsaList = dataManager.getHsaList("");
                ...
                if ("Primary".equals(peSelection)) {
                    peIsPrimary = true;
                }

                if (queryMode == 0) {
                    // Ad Hoc Mode
                    List<String[]> peTsBuf = pcPeTs.getOrgBuf();
                    for (int i = 0; i < peTsBuf.size(); i++) {
                        ...
                        if ((peSelection.split(" ")[0]
                                .equals(pcPeTs.getOrgBuf().get(i)[0]))
                                || (peIsPrimary && ("HG".equalsIgnoreCase(
                                        pcPeTs.getOrgBuf().get(i)[0])
                                        || "QR".equalsIgnoreCase(pcPeTs
                                                .getOrgBuf().get(i)[0])))) {
                    ...
                    List<Object[]> rs = dataManager.getUnique("ts",
                            "ingestFilter", where);
    ...
    private void handleApply() {
        ...
        if (selectionIndices.length > 0) {
            ...
            for (int selectionIndice : selectionIndices) {
                al.add(dataList.getItem(selectionIndice));
            }

            if (dialogType == DialogType.DATA_SOURCE) {
                pcOptions.setDataSourcesChosen(
