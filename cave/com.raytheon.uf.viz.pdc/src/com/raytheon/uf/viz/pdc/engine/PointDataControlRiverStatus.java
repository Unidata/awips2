...
public class PointDataControlRiverStatus {
    ...
    public static List<GageData> processRiverThreatIndex(List<GageData> repList,
        ...
        Map<String, GageData> riverStatusMap = new HashMap<>();
        ...
        for (int i = 0; i < repList.size(); i++) {
            if (riverStatusMap.containsKey(repList.get(i).getLid())) {
                ...
                if (riverValue != PDCConstants.MISSING_VALUE) {
                    repList.get(i)
                            .setThreatIndex(ThreatIndex.THREAT_MISSING_STAGE);
    ...
    private static synchronized void getRiverOptions() {
        ...
        if (firstTime) {
            ...
            pcOptionsRiver.setElementType(
                    HydroConstants.AdHocDataElementType.RIVER_AD_HOC_TYPE
            ...
            for (int i = 0; i < pcOptionsRiver
                    .getDataSourceChosenCount(); i++) {
                al.add("dummy");
            }
            pcOptionsRiver
