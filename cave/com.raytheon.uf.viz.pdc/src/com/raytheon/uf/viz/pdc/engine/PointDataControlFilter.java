...
public class PointDataControlFilter {
    ...
    /**
     * Filters the report list as per the user instructions. Reports are
     * filtered by station and/or by value.
     *
     * This function also conveniently filters out data that is for an area, and
     * not for a location.
     *
     * @param repList
     *            The report list
     */
    public static synchronized List<GageData> filterReportsAndAddInfo(
            List<GageData> repList) {
        ...
        /*
         * Load the information from the LocPDC view the first time this routine
         * is called. This information is required for the data source
         * (telem_type column) and service backup filters (hsa column). It is
         * also required for providing the station name, latitude/longitude, and
         * display class for each of the stations in the report list.
         */
        if (locPDCMap == null) {
            /*
             * Set up the where clause so that only stations with a nonzero post
             * value are loaded.
             */
            String where = "WHERE post != 0";

            locPDCMap = manager.getLocPDC(where);

            if ((locPDCMap == null) || locPDCMap.isEmpty()) {
                // TODO log error
                // In routine 'filter_reports_and_add_info':\n"
         ...
         * At the same time, add the necessary information from the LocPDC view.
         */
        for (int i = 0; i < repList.size(); i++) {
            ...
            if (locpdc != null) {
                ...
                if ((pcOptions.getQueryMode() == QueryMode.TIME_STEP_MODE
                    ...
                    for (int k = 0; k < pcOptions
                            .getTypeSourceChosenCount(); k++) {
                        if (pcOptions.getTypeSourceChosenList().get(k)
                                .contains(repList.get(i).getTs())) {
                            useReport = true;
                            break;
                        }
                    }
                ...
                /*
                 * if the user requests that the stations are to be filtered by
                 * the station data source, then apply the filter.
                 */
                if ((pcOptions.getQueryMode() == QueryMode.AD_HOC_MODE
                    ...
                    for (int k = 0; k < pcOptions
                            .getDataSourceChosenCount(); k++) {
                        if (pcOptions.getDataSourcesChosen()[k]
                                .equalsIgnoreCase(
                                        PDCConstants.OBSERVER_STRING)) {
                            if (locpdc.isDcp()) {
                                useReport = true;
                            }
                        } else if (pcOptions.getDataSourcesChosen()[k]
                                .equalsIgnoreCase(PDCConstants.DCP_STRING)) {
                            if (locpdc.isDcp()) {
                                useReport = true;
                            }
                        } else if (pcOptions.getDataSourcesChosen()[k]
                                .equalsIgnoreCase(
                                        PDCConstants.UNDEFINED_STRING)) {
                            if (locpdc.isDcp() && locpdc.isObserver()
                                    && ((locpdc.getTelemType() != null)
                                            && (locpdc.getTelemType()
                                                    .length() == 0))) {
                                useReport = true;
                            }
                            ...
                            if (pcOptions.getDataSourcesChosen()[k]
                                    .equalsIgnoreCase(locpdc.getTelemType())) {
                                useReport = true;
                            }
                        }
                    }
                ...
                /*
                 * if the supress non-fcst point options is enabled, then filter
                 * out any stations that are not forecast points.
                 */
                if (pcOptions.getFcstptsOnly() == 1) {
                    if (!locpdc.getDispClass()
                            .contains(PDCConstants.FCSTPT_STNCLASS)) {
                        useReport = false;
                    }
                }

                /*
                 * Apply the service backup filter if enabled. This test
                 * compares the hsa defined for the current location (as given
                 * in the locPDCPtr) with that selected from the serivce backup
                 * option on the pointcontrol gui. Multiple HSAs may be selected
                 * from this option and are represented as a comma separated
                 * list in the pc_options->hsa_list.
                 */
                if ((pcOptions.getFilterByHSA() == 1) || !useReport) {
                    if (useReport) {
                        if (!pcOptions.getHsaList()
                                .contains(PDCConstants.ALL_AREAS)
                                && !pcOptions.getHsaList()
                                        .contains(locpdc.getHsa())) {
                            useReport = false;
                        }
                    }
                }
                ...
                /*
                 * now suppress missing data and zero data if the suppress
                 * option is enabled
                 */
                ...
                if (useReport || (pcOptions.getQueryMode() == 1)) {
                    ...
                    // set the coordinate
                    Coordinate xy = new Coordinate(locpdc.getLon(),
                            locpdc.getLat());
                    repList.get(i).setCoordinate(xy);

                    /*
                     * Copy the station name into the current report structure
                     * being processed.
                     */
    ...
    /**
     * Determines if a value FAILS the test, so if a value does NOT meet the
     * criteria, the result is TRUE.
     *
     * @param filterOperation
     *            The filter operation SHOW_ALL ("Any Value") 0, SHOW_EQUAL
     *            ("Value =") 1, SHOW_NOT_EQUAL ("Value Not =") 2,
     *            SHOW_GREATER_EQUAL ("Value >=") 3, SHOW_LESS_EQUAL ("Value
     *            <=") 4, SHOW_GREATER ("Value >") 5, SHOW_LESS ("Value <") 6;
     ...
     */
    private static boolean isFilterdOutByValue(int filterOperation,
            double comparisonValue, double valueToExamine) {
        boolean filteredOut = false;
        boolean passesTest = false;

        if (filterOperation != 0) { // SHOW_ALL
            // round to 2 decimal places
            valueToExamine = round(valueToExamine, 2);
