...
public class VizGetTotalPrecip extends GetTotalPrecip {
    ...
    public List<GageData> getTotalRawPrecip(List<Rawpc> rawPCList,
        ...
        Rawpc pc = null;
        if ((rawPCList != null) && (!rawPCList.isEmpty())) {
            pc = rawPCList.get(0);
            prevLid = pc.getLid();
            prevTs = pc.getTs();
        ...
        // process the first record
        Rawpp pp = null;
        if ((rawPPList != null) && (!rawPPList.isEmpty())) {
            pp = rawPPList.get(0);
            prevLid = pp.getLid();
            prevTs = pp.getTs();
        ...
        boolean sumPcReports = appsDefaults
                .getBoolean(CommonHydroConstants.SUM_PC_REPORTS_TOKEN, false);
