    ...
    public void reproject() throws VizException {
        ...
        try {
            for (int level = 0; level < this.levels; level++) {
                ...
                double[] mapCenterLL = mapDescriptor.pixelToWorld(new double[] {
                        mapXCenter, mapYCenter });
                double[] mapCenterLL1 = mapDescriptor
                        .pixelToWorld(new double[] { mapXCenter + 1, mapYCenter });

                double[] input = new double[] { mapCenterLL[0], mapCenterLL[1],
                        mapCenterLL1[0], mapCenterLL1[1] };
                ...
                inverseMathTransform[level].transform(output, 0, output, 0, 2);
