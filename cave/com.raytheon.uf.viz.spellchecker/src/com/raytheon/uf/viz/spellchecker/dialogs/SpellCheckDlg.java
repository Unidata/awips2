 ...
 * Mar 01, 2010  4765     MW Fegan     Moved from GFE plug-in.
 * Sep 24, 2014  16693    lshi         filter out swear words in spelling check
 * Oct 23, 2014  3685     randerso     Changes to support mixed case
 * Oct 30, 2014  16693    lshi         Add more swear words to the filter
 * Mar 30, 2015  4344     dgilling     Make bad word filter configurable.
 * Aug 31, 2015  4781     dgilling     Improve handling of proper nouns in all
 *                                     caps mode, move override dictionary to
 *                                     SITE level.
 * May 25, 2016  16930    MPorricelli  Added suggestionsBlackList to
 *                                     spellCheckJob for flagging of words that
 ...
 * @author lvenable
 */
public class SpellCheckDlg extends CaveSWTDialog
    ...
    public void accept(SpellingProblem problem) {
        ...
        StyleRange styleRange = styledText
                .getStyleRangeAtOffset(problem.getOffset());
        ...
        if ((proposals != null) && (proposals.length > 0)) {
            for (ICompletionProposal proposal : proposals) {
                ...
                Matcher pdMatch = SpellCheckUtil.CHANGE_TO_PATTERN
                        .matcher(pdString);
                if (pdMatch.matches()) {
                    ...
                    if (!BlacklistedWordsUtil.isBlacklisted(replString)) {
                        suggestions.add(replString);
                    }
                }
                Matcher addMatch = SpellCheckUtil.ADD_TO_PATTERN
                        .matcher(pdString);
                if (addMatch.matches()) {
    ...
    private void createMiddleControls(Composite mainComp) {
        ...
        gd.heightHint = 250;
        gd.widthHint = 200;
        suggestionList = new List(middleComp,
        ...
        suggestionList.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent event) {
                replaceWithTF.setText(suggestionList
                        .getItem(suggestionList.getSelectionIndex()));
            }
        });
        ...
        replaceAllBtn.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent event) {
                ...
                Pattern pattern = Pattern
                        .compile("(\\b)(" + Pattern.quote(oldText) + ")(\\b)");
                ...
                Deque<Integer> repList = new LinkedList<>();
        ...
        addWordBtn.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent event) {
                ...
                try {
                    ...
                    LocalizationFile newSiteDictionary = pathManager
                            .getLocalizationFile(siteCtx, SPELLDICT);
    ...
    private void setSpellCheckPreferences() {
        ...
        userDEncodingWasDefault = xstore.isDefault(
                PreferenceConstants.SPELLING_USER_DICTIONARY_ENCODING);
        if (!userDEncodingWasDefault) {
            userDEncodingToRestore = xstore.getString(
                    PreferenceConstants.SPELLING_USER_DICTIONARY_ENCODING);
        }
    ...
    private LocalizationFile getAdditionalDictionary() {
        ...
        LocalizationFile siteDictionary = pathManager
                .getLocalizationFile(siteCtx, SPELLDICT);
        if (!siteDictionary.exists()) {
            ...
            try (SaveableOutputStream outStream = siteDictionary
                    .openOutputStream();
                    InputStream inStream = dictionaryTemplate
                            .openInputStream()) {
     ...
     * @author wldougher
     */
    class CheckWord extends SelectionAdapter
            implements ISpellingProblemCollector {
        ...
        public void widgetSelected(SelectionEvent evt) {
            SpellCheckJob wordCheckJob = new SpellCheckJob("wordCheck");
