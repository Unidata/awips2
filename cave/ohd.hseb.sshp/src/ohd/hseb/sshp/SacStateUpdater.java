 ...
 * @author GobsC
 * 
 *         The purpose of this class is to check the most recent saved
 *         Sacramento Model states and run the model from the last saved state
 *         if the states have aged too much.
 * 
 *         The class also is responsible for creating "background" forecasts for
...
public class SacStateUpdater {
    ...
    public SacStateUpdater(DataMgr dataMgr, int expirationHours,
            int hoursForward, String logFileName) {
        _dataMgr = dataMgr;
        ...
        _backgroundFcstOutputDirectoryNameString = ad
                .getToken(SSHP_BACKGROUND_FORECAST_OUTPUT_DIR);
        _backgroundFcstLengthInHours = ad.getInt(
                SSHP_BACKGROUND_FORECAST_FCST_LENGTH,
    ...
    public void updateAllStates() {
         ...
         * 
         * for each locationId time = _dataMgr.getTimeOfBestRecentState() if
         * more than expirationHours old, then run model for that locationId
         * 
         * save the SacState, and the prior runoff
         */

        List<String> locationIdList = _dataMgr
                .loadAllLocationIdsToAutoUpdateSacStates();
        ...
        if (locationIdList.size() == 0) {
            _logger
                    .log("There are no locations for which auto process has been set.");
        }
    ...
    public void updateStatesByLocation(String locationId, boolean saveStates) {
        ...
        if (bestState != null) {
            ...
            // long bestStateTime =
            // _dataMgr.loadTimeOfBestRecentSacState(locationId);

            long currentTime = System.currentTimeMillis();
            currentTime = TimeHelper.truncateTimeInMillisToNearestHour(
                    currentTime, 1);
            ...
            if (timeDiff > _timeDiffMax) {
                ...
                StreamModel model = new StreamModel(locationId,
                        RainfallRunoffModelType.SAC_SMA, _dataMgr);

                model.setSacSmaStateDescriptor(new SacSmaStateDescriptor(
                        bestState));
                model.setModelStartTime(bestState.getValidTime());

                long modelEndTime = currentTime
                        + (_hoursForward * _millisPerHour);
                model.setModelEndTime(modelEndTime);

                _logger.log("Last saved state time = "
                        + DbTimeHelper
                                .getDateTimeStringFromLongTime(bestStateTime));
                _logger.log("Model run end time = "
                        + DbTimeHelper
                                .getDateTimeStringFromLongTime(modelEndTime));
                ...
                if (saveStates) {
                    ...
                    _logger.log("Saving computed runoff time series for "
                            + locationId + ".");
                    model.saveRunoffTimeSeries(_logger);
                }

            } else {
                _logger.log("Model States for " + locationId
                        + " are up to date.");
            }
        } // end if bestState != null
        else {
            _logger.log("No original model States for " + locationId
                    + " are available.  Unable to update.");
        }
    }
    ...
    public void updateAllBackgroundForecasts() {
         ...
         * 
         * for each locationId run the model and update the latest forecast
         * stored on disk as a flat file
         */

        List<String> locationIdList = _dataMgr.loadLocationIdsForForecasts();

        for (String locationId : locationIdList) {
            _logger.log("Attempting to update a background forecast for "
                    + locationId);
        ...
        if (locationIdList.size() == 0) {
            _logger
                    .log("There are no locations for which to update a background forecast.");
        }
    ...
    public void updateForecastByLocation(String locationId) {
        ...
        // _logger.log(header + " search for valid model states for " +
        // locationId);
        ...
        } else // API_MKC
        {
            AbsTimeMeasurement ffhMeasurement = _dataMgr
                    .loadMostCurrentFfhMeasurement(locationId, config
                            .getBasinId(), currentTime);

            if (ffhMeasurement != null) {
                isStateAvailable = true;

                model = new StreamModel(locationId,
                        RainfallRunoffModelType.API_MKC, _dataMgr);

                model.setModelStartTime(ffhMeasurement.getTime());

                long modelEndTime = currentTime
                        + (_backgroundFcstLengthInHours * _millisPerHour);
                model.setModelEndTime(modelEndTime);
            }
        }
        ...
        if (isStateAvailable) {
            ...
            // _logger.log("Last saved state time = " +
            // DbTimeHelper.getDateTimeStringFromLongTime(bestStateTime));
            // _logger.log("Model run end time = " +
            // DbTimeHelper.getDateTimeStringFromLongTime(modelEndTime));

            model.runModel();

            // for situational awareness applications (e.g. RiverMonitor)
            TimeSeriesFileManager fileManager = new TimeSeriesFileManager();
            RegularTimeSeries fcstStageTimeSeries = model
                    .getForecastStageTimeSeriesHolder().getTimeSeries();
            ...
            if (_backgroundFcstOutputDirectoryNameString != null) {
                ...
                _logger.log(header + "saving forecasts for " + locationId
                        + " to " + filePath);

                fileManager.saveRegularTimeSeries(fcstStageTimeSeries,
                        filePath, MeasuringUnit.feet);
            } else {
                _logger.log(header
                        + "_backgroundFcstOutputDirectoryNameString is null,"
    ...
    public static void main(String[] args) {
        ...
        if (args.length < 4) {
            System.err
                    .println("Usage: ohd.hseb.sshp.StateUpdater connection_string expirationHours hoursForward logFilePath");
//            System.exit(0);
        }
...
//            System.exit(0);
        ...
        SacStateUpdater updater = new SacStateUpdater(dataMgr, expirationHours,
                hoursForward, logFilePath);
