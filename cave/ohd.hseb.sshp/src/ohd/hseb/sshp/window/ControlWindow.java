...
/**
 * @author GobsC This class is the startup window for the SSHP application when
 *         no argument for the location id is entered. It allows for the
...
public class ControlWindow extends JFrame {
    ...
    public static final String MODEL_PREFERENCE = "RR Model Pref"; // rainfall-runoff
                                                                   // model
                                                                   // preference
    ...
    private void initGui() {
        ...
        setTitle("Site Specific Hydrologic Predictor Control Window on "
                + _dataMgr.getDatabaseName());
        ...
        // _mainPanel.setPreferredSize(new Dimension(DEFAULT_WIDTH,
        // DEFAULT_HEIGHT));
    ...
    // ---------------------------------------------------------------------
    private ControlWindowTableAdapter createDescriptorTableAdapter(
            List descriptorList) {
        ...
        JTable table = null;
        String[] columnNameArray = { LID, NAME, BASIN_ID, STREAM_NAME, HSA,
                MODEL_PREFERENCE };
        ...
        tableAdapter = new ControlWindowTableAdapter(comparator,
                columnNameArray, descriptorList);
        ...
        if (tableAdapter.getDescriptorCount() > -1) {
            ...
            LocationDescriptor descriptor = tableAdapter
                    .getDescriptorByIndex(0);
        }

        tableAdapter.getTable().setPreferredScrollableViewportSize(
                scrolledPaneViewportDimension);
    ...
    private List createLocationDescriptorList(List locationIdList) {
        ...
        for (int i = 0; i < locationIdList.size(); i++) {
            ...
            LocationDescriptor descriptor = _dataMgr
                    .getLocationDescriptor(locationId);
    ...
    private void runMAPPreprocessor() {
        ...
        // preprocessor = new MAPPreprocessor(_dataMgr, startTime, endTime,
        // logger);
    ...
    private void runMAPPreprocessorAsSeparateProcess() {
        ...
        System.out
                .println(header + "making system call:" + commandString + ":");
        System.out.flush();

        try {
            // DR#10955
            RunProcess process = RunProcess.getRunProcess().exec(commandString);
            process.waitFor();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void addListeners() {
        ...
        // _mapPreprocessorButton.addActionListener(new
        // MapPreprocessorRunListener());

        // hook up the application launch button and the double click launching
        // from the table
        _launchButton.addActionListener(new AnalysisWindowLaunchListener());
        _tableAdapter.getTable().addMouseListener(
                new DoubleClickAnalysisWindowLaunchListener());
        ...
        _tableAdapter.getListSelectionModel().addListSelectionListener(
                new DescriptorRowSelectionListener());
    ...
    public String getSelectedLocationId() {
        ...
        if ((selectedIndex >= 0) && (listSize > 0)) {
            ...
            LocationDescriptor desc = (LocationDescriptor) _tableAdapter
                    .getDataList().get(selectedIndex);
    ...
    private class AboutLaunchListener implements ActionListener {
        public void actionPerformed(ActionEvent evt) {
            // JOptionPane.showMessageDialog(LaunchWindow.this, _aboutText,
            // "About SSHP", JOptionPane.INFORMATION_MESSAGE );
            String aboutText = AboutInfo.getText();
            DialogHelper.displayMessageDialog(ControlWindow.this, aboutText,
                    "About SSHP");
        }
    }
    ...
    private class ConfigureLaunchListener implements ActionListener {
        public void actionPerformed(ActionEvent evt) {
            SSHPConfigDialog dialog = new SSHPConfigDialog(ControlWindow.this,
                    _dataMgr);
    ...
    private class UnitHydrographLaunchActionListener implements ActionListener {
        public void actionPerformed(ActionEvent evt) {
            ...
            if (lid != null) {
                ...
                boolean isModal = false;
                editor = new UnitHydrographEditor(ControlWindow.this, _dataMgr,
                        lid, isModal);
    ...
    private class DescriptorRowSelectionListener implements
        ...
        public void valueChanged(ListSelectionEvent event) {
            ...
            if (index > -1) {
                _selectedLocationDescriptor = (LocationDescriptor) _descriptorList
                        .get(index);
