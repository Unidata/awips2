 ...
 * Jun 14, 2018  7026     bsteffen  reinit inventory on refresh.
 * Nov 11, 2019  71113    smanoj    Fixing error loading Ensemble data 
 *                                  from the Product Browser
 * 
 * </pre>
 * 
 * @author bsteffen
 */
public class GridProductBrowserDataDefinition
        extends DataListingProductBrowserDefinition {

    private static final String SHOW_DERIVED_PARAMS = "Show Derived Parameters";

    /*
     * This will be the exact same object as super.listing but it is declared
     * here to allow use without casting.
     */
    protected final DerivedPreferenceDataListing listing;

    public GridProductBrowserDataDefinition() {
        this(new DerivedPreferenceDataListing());
    }

    private GridProductBrowserDataDefinition(
            DerivedPreferenceDataListing listing) {
        super(listing);
        this.listing = listing;
        new ProductBrowserPreferenceListener(displayName,
                listing.getDerivedPreference());
    }

    @Override
    public void refresh() {
        if (Boolean.TRUE.equals(listing.getDerivedPreference().getValue())) {
            Object inventory = DataCubeContainer
                    .getInventory(listing.getPluginName());
            if (inventory instanceof CommonGridInventory) {
                ((CommonGridInventory) inventory).reinitTree();
            }
        }
    }
    ...
    protected AbstractRenderableDisplay createRenderableDisplay(
        ...
        if (resourceData instanceof GridResourceData) {
            ...
            List<String> ensembles = inventory
                    .getEnsembles(gridResourceData.getMetadataMap());
            if (ensembles != null && ensembles.size() > 1) {
                Collections.sort(ensembles);
                DisplayType displayType = null;
                if (resourcePair
                        .getLoadProperties() instanceof GridLoadProperties) {
                    displayType = ((GridLoadProperties) resourcePair
                            .getLoadProperties()).getDisplayType();
                }
                ...
                for (String ensemble : ensembles) {
                    ...
                    HashMap<String, RequestConstraint> newParameters = new HashMap<>(
                    ...
                    ResourcePair ensemblePair = createResourcePair(
                            ensembleResourceData, displayType);
                    if (display == null) {
                        display = super.createRenderableDisplay(ensemblePair);
                    } else {
    ...
    public List<ProductBrowserPreference> getPreferences() {
        List<ProductBrowserPreference> prefs = new ArrayList<>(
                super.getPreferences());
        prefs.add(listing.getDerivedPreference());
        return prefs;
    }
    ...
    private static class DerivedPreferenceDataListing implements DataListing {
        ...
        public DerivedPreferenceDataListing() {
            ...
            derivedPreference.setTooltip(
                    "Show derived parameters in the Product Browser");
            derivedPreference.setValue(true);
            this.listing = new GridDataListing(Arrays.asList(
                    GridConstants.DATASET_ID,
                    GridConstants.PARAMETER_ABBREVIATION,
                    GridConstants.MASTER_LEVEL_NAME, GridConstants.LEVEL_ID));
            this.derivedListing = new DerivedGridDataListing(Arrays.asList(
                    GridConstants.DATASET_ID,
                    GridConstants.PARAMETER_ABBREVIATION,
                    GridConstants.MASTER_LEVEL_NAME, GridConstants.LEVEL_ID));
        }
        ...
        @Override
        public Map<String, RequestConstraint> getRequestConstraints(
