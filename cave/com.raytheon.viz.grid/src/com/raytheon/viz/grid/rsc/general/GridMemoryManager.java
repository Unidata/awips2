...
public class GridMemoryManager {
    ...
    private static final String DATA_SIZE_ONLY_PATTERN = "^" + DATA_SIZE_PATTERN
            + "$";
    ...
    private static final int SPARSE_CHECK_THRESHOLD = Integer
            .getInteger("grid.sparse.check.threshold", 100 * 1024 * 1024); // 100
                                                                           // MB
    ...
    private final ReferenceQueue<ByteBuffer> refQueue = new ReferenceQueue<>();
    ...
    public GeneralGridData manage(GeneralGridData data) {
        ...
        if (data instanceof VectorGridData) {
            sizeInBytes *= 2;
        } else if (!(data instanceof ScalarGridData)) {
            // Only Scalar and Vector data use sparse or direct memory
            return data;
        }
    ...
    private GeneralGridData convertToDirect(GeneralGridData data,
        ...
        try {
            ...
            livingRefs.add(
                    new DirectBufferTrackingReference(directBuffer, refQueue));
            ...
            if (data instanceof VectorGridData) {
                ...
                FloatBufferWrapper vWrapper = new FloatBufferWrapper(
                        dataBuffer.slice(), gridRange);
                DataUtilities.copy(vectorData.getUComponent(), uWrapper,
                        gridRange.width, gridRange.height);
                DataUtilities.copy(vectorData.getVComponent(), vWrapper,
                        gridRange.width, gridRange.height);
                data = VectorGridData.createVectorDataUV(gridGeometry, uWrapper,
                        vWrapper, data.getDataUnit());
            } else if (data instanceof ScalarGridData) {
                ScalarGridData scalarData = (ScalarGridData) data;
                FloatBufferWrapper wrapper = new FloatBufferWrapper(dataBuffer,
                        gridRange);
                DataUtilities.copy(scalarData.getScalarData(), wrapper,
                        gridRange.width, gridRange.height);
                data = ScalarGridData.createScalarData(gridGeometry, wrapper,
                        data.getDataUnit());
            }
        } catch (Throwable e) {
    ...
    private boolean isSparse(GeneralGridData data) {
        ...
        if (data instanceof VectorGridData) {
            VectorGridData vectorData = (VectorGridData) data;
            source = vectorData.getUComponent();
        } else if (data instanceof ScalarGridData) {
            ScalarGridData scalarData = (ScalarGridData) data;
            source = scalarData.getScalarData();
        } else {
            source = data.getData();
        }
        ...
        for (int y = 0; y < ny; y += 1) {
            for (int x = 0; x < nx; x += 1) {
                if (Double.isNaN(source.getDataValue(x, y))) {
                    ...
                    if (inRun) {
                        inRun = false;
                        largestNandRun = Math.max(largestNandRun,
                                currentNandRun);
                        currentNandRun = 0;
                    }
                }
    ...
    private GeneralGridData convertToSparse(GeneralGridData data) {
        ...
        if (data instanceof VectorGridData) {
            VectorGridData vectorData = (VectorGridData) data;
            DataSource uSource = vectorData.getUComponent();
            DataSource vSource = vectorData.getVComponent();
            ...
            data = VectorGridData.createVectorDataUV(gridGeometry, uDest, vDest,
                    data.getDataUnit());
        } else if (data instanceof ScalarGridData) {
            ScalarGridData scalarData = (ScalarGridData) data;
            DataSource source = scalarData.getScalarData();
            SparseFloatArray dest = new SparseFloatArray(nx, ny,
                    SPARSE_FILL_VALUE, SPARSE_ARRAY_BLOCK_SIZE);
            DataUtilities.copy(source, dest, nx, ny);
            data = ScalarGridData.createScalarData(gridGeometry, dest,
                    data.getDataUnit());
        }
        return data;
    }
    ...
    private static final class DirectBufferTrackingReference
