...
public class DescriptionDlg extends CaveSWTDialog {
    ...
    private void createMainControls() {
        ...
        gd.widthHint = textControlWidth;
        gd.heightHint = 70;
        remarksTF = new Text(controlComp,
        ...
        gd.widthHint = textControlWidth;
        gd.heightHint = 70;
        freezingTF = new Text(controlComp,
                SWT.BORDER | SWT.MULTI | SWT.WRAP | SWT.V_SCROLL);
        ...
        gd.widthHint = textControlWidth;
        gd.heightHint = 70;
        regulationTF = new Text(controlComp,
                SWT.BORDER | SWT.MULTI | SWT.WRAP | SWT.V_SCROLL);
        ...
        gd.widthHint = textControlWidth;
        gd.heightHint = 70;
        affectedAreaTF = new Text(controlComp,
                SWT.BORDER | SWT.MULTI | SWT.WRAP | SWT.V_SCROLL);
        ...
        ModifyListener listenerA = new ModifyListener() {
            @Override
            public void modifyText(ModifyEvent e) {
                if (affectedAreaTF.getText()
                        .length() > MAX_AFFECTED_AREA_SIZE) {
        ...
        gd = new GridData(SWT.CENTER, SWT.DEFAULT, true, false);
        gd.widthHint = buttonWidth;
        Button affectAreaDeleteBtn = new Button(affectedButtonComp, SWT.PUSH);
        ...
        gd = new GridData(SWT.CENTER, SWT.DEFAULT, true, false);
        gd.widthHint = buttonWidth;
        Button affectAreaSaveBtn = new Button(affectedButtonComp, SWT.PUSH);
    ...
    private void populateProximityCombo() {
        try {
            List<ProximityData> proximityData = ProximityDataManager
    ...
    private void populateControls() {
         ...
         */
        try {
            List<DescriptionData> descriptData = DescriptionDataManager
            ...
            for (DescriptionData descriptionData : descriptData) {
                ...
                proximityCbo.select(
                        proximityCbo.indexOf(descriptionData.getProximity()));
            ...
            java.util.List<LocationAreaData> locData = LocationAreaManager
    ...
    private void saveDescription() {
        try {
            ...
            if (recordExists) {
                DescriptionDataManager.getInstance()
                        .updateDescriptionData(descriptionData);
            } else {
                DescriptionDataManager.getInstance()
                        .insertDescriptionData(descriptionData);
            }
        } catch (VizException e) {
            statusHandler.handle(Priority.PROBLEM,
    ...
    private void saveAffectedArea() {
        try {
            ...
            if (recordExists) {
                LocationAreaManager.getInstance()
                        .updateLocationAreaData(locAreaData);
            } else {
                LocationAreaManager.getInstance()
                        .insertLocationAreaData(locAreaData);
            }
        } catch (VizException e) {
            statusHandler.handle(Priority.PROBLEM,
    ...
    private DescriptionData getDataFromControls() {
        ...
        descriptionData.setProximity(
                proximityCbo.getItem(proximityCbo.getSelectionIndex()));
