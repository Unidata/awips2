 ...
 * @version 1
 */
public class SinglePanelLayoutMenuAction extends AbstractRightClickAction {
    private static final transient IUFStatusHandler statusHandler = UFStatus.getHandler(SinglePanelLayoutMenuAction.class);
     ...
     * Default constructor.
     */
    public SinglePanelLayoutMenuAction(IDisplayPane pane) {
        super("Single Panel Layout");
    ...
    public void run() {
        ...
        IMultiPaneEditor editor = null;
        if (container instanceof IMultiPaneEditor) {
            editor = (IMultiPaneEditor) container;
        }
        for (IDisplayPane pane : container.getDisplayPanes()) {
            if (pane != paneWithFocus && editor != null) {
                editor.removePane(pane);
            }
        }

        try {
            paneWithFocus.getDescriptor().getTimeMatcher().redoTimeMatching(
                    paneWithFocus.getDescriptor());
            HistoryList.getInstance().refreshLatestBundle();
        } catch (VizException e) {
            statusHandler.handle(Priority.PROBLEM,
                    "Error refreshing pane", e);
        }
    }
}
