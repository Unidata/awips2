 ...
 * ------------- -------- --------- --------------------------------------------
 * May 05, 2009  623      bgonzale  Initial creation
 * Oct 05, 2015  4934     bsteffen  Do not re-use datatimes for overlays in
 *                                  VALID_TIME_SEQ
 * Mar 03, 2016  5436     bsteffen  Do not throw illegal state exception when in
 *                                  an unexpected state.
    ...
    public TimeMatchingConfiguration getConfiguration(LoadProperties loadProps,
        ...
        case INVENTORY:
            if (config.getDataTimes() == null) {
                ...
                List<DataTime> bestTimes = new ArrayList<>(
        ...
        case STD:
        case VALID_TIME_SEQ:
            if (TimeOptionsAction.isTimeOptionsSelected()) {
        ...
        if (config != null && !config.isCancel()) {
            TimeOptionsAction.setTimeOptionsSelected(false);
        }
