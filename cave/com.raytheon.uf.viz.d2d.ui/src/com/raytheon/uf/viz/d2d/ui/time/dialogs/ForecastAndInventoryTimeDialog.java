 ...
 * Mar 05, 2018  6900      bsteffen     Set the delta and tolerance on inventory
 *                                      loads to ensure only the selected time is matched.
 * 
 * </pre>
 * 
 * @author bgonzale
...
public class ForecastAndInventoryTimeDialog extends AbstractTimeMatchingDialog {
    ...
    public void init() throws VizException {
        this.forecastTimes = new ArrayList<>();
        this.inventoryTimes = new ArrayList<>();
        ...
        if (availableTimes != null && !availableTimes.isEmpty()) {
            for (DataTime dataTime : availableTimes) {
                ...
                DataTime invTime = new DataTime(
                        dataTime.getRefTimeAsCalendar());
        ...
        Collections.sort(this.inventoryTimes, Comparator.reverseOrder());
    ...
    protected void initializeComponents(Shell shell) {
        ...
        gd.heightHint = 350;
        gd.widthHint = 200;
        inventoryList = new List(composite,
                SWT.BORDER | SWT.SINGLE | SWT.V_SCROLL);
        inventoryList.setLayoutData(gd);
        for (DataTime dataTime : inventoryTimes) {
            inventoryList
                    .add(this.timeFormatter.getTimeMonthDateString(dataTime));
        }
        if (inventoryList.getItemCount() == 0) {
            ...
            if (inventoryTimes != null && !inventoryTimes.isEmpty()) {
                ...
                forecastList.add(
                        this.timeFormatter.getDayTimeForecastHRString(first));
            }
        }
    ...
    protected void setSelectedDataTimes() {
        ...
        java.util.List<DataTime> times = new ArrayList<>();
        ...
        DataTime currentTime = descriptor.getFramesInfo().getCurrentFrame();
        if (currentTime != null) {
            config.setDelta(times.get(0).getValidTime().getTimeInMillis()
                    - currentTime.getValidTime().getTimeInMillis());
