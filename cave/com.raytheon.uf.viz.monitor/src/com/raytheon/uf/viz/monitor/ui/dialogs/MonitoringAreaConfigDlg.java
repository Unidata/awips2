 ...
 * @version 1.0
 */
public abstract class MonitoringAreaConfigDlg extends Dialog implements INewZoneStnAction {
	/**
	 * Dialog shell.
	 */
	...
	/**
	 * monitor area zones/stations; and additional zones/stations in the region
	 */
	private ArrayList<String> maZones = null;
	private ArrayList<String> maStations = null;
	private ArrayList<String> additionalZones = null;
	private ArrayList<String> additionalStns = null;
	...
	private void initZoneStationLists() {
		...
		try {
			for (String zone : maZones) {
				ArrayList<String> stns = configMgr
	...
	public Object open() {
		...
		if (dialogTitle == null) {
			shell.setText("Monitoring Area Configuraton");
		} else {
	...
	private void initFontAndImages() {
		...
		// arrowUpImg = new Image(display,
		// "/home/lvenable/DecisionAidTools/arrowUp.png");
		// arrowDownImg = new Image(display,
		// "/home/lvenable/DecisionAidTools/arrowDn.png");
	}
	...
	private void setShipDistScaleLabel() {
		distanceScaleLBl.setText(String.format("%5d", distanceScale
				.getSelection()));
	}
	...
	private void handleAddNewAction() {
		if (zoneRdo.getSelection() == true) {
			AddNewZoneDlg addNewZoneDlg = new AddNewZoneDlg(shell, appName, this);
			addNewZoneDlg.open();
		} else {
			...
			String area = associatedList.getItem(associatedList
					.getSelectionIndex());
			AddNewStationDlg addNewStnDlg = new AddNewStationDlg(shell,
					appName, area, this);
			addNewStnDlg.open();
		}
	...
	private void handleEditDeleteAction() {
		if (zoneRdo.getSelection() == true) {
			EditNewZoneDlg editDlg = new EditNewZoneDlg(shell, appName);
			editDlg.open();
		} else {
			DeleteStationDlg deleteStnDlg = new DeleteStationDlg(shell, appName);
			deleteStnDlg.open();
		}
	...
	private void populateLeftLists() {
		...
		if (mode == Mode.Zone) {
			...
			Collections.sort(maZones);
			monitorAreaList.setItems(maZones
					.toArray(new String[maZones.size()]));
	...
	private void addZoneStn() {
		...
		if (mode == Mode.Zone) {
			...
			if (!addedZones.contains(entry)) {
				addedZones.add(entry);
			}
	...
	private void removeAssociated() {
		...
		if (mode == Mode.Zone) {
			...
			ArrayList<String> zones = configMgr.getAreaByStationId(entry
	...
	private void handleMonitorAreaListSelection() {
		...
		if (mode == Mode.Zone) {
			...
			ArrayList<String> stations = configMgr
					.getAreaStationsWithType(zone);
			if (stations.size() > 1) {
			...
			ArrayList<String> zones = configMgr.getAreaByStationId(station
	...
	private void closeWithoutSave() {
		int choice = showMessage(shell, SWT.YES | SWT.NO, "Fog Monitor Exit",
	 ...
	 * @param stnWithType (String of station ID with type)
	 */
    ...
    public boolean isExistingStation(String stnWithType) {
		if ( maStations.contains(stnWithType) || additionalStns.contains(stnWithType) ) {
			return true;
		}
    	return false;
    }
