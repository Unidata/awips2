 ...
 * Oct 07, 2013  2436     lvenable  Disposed of the sort color.
 ...
 * Jun 26, 2018  6572     tgurney   Resize table when removing columns
...
public abstract class TableComp extends Composite {
    ...
    public void setToolTipText(String newText) {
        ...
        } else if (!newText.equals(table.getToolTipText())) {
            table.setToolTipText(null);

            /*
             * update the tooltip asynchronously so it moves with the mouse as
             * you move up and down
             */
            table.getDisplay().asyncExec(new Runnable() {
                @Override
                public void run() {
                    table.setToolTipText(newText);
                }
            });
        }
    }
    ...
    private void sortTableData(TableColumn tc) {
        ...
        if ("SSZT_SwellPeriod".equals(sortCol)
                || "SSZT_Swell2Period".equals(sortCol)) {
    ...
    protected void tableMouseHoverAction(MouseEvent event) {
        ...
        if (rc != null) {
            toolTip = tableData.getTableRows().get(rc.y)
                    .getTableCellDataArray()[rc.x].getHoverText();
        }
    ...
    /**
     * Return the table row and column indices under the mouse point
     *
     * @param event
     *            the mouse event
     * @return point containing column and row indices as x and y respectively
     */
    protected Point getRowAndColumnIndex(MouseEvent event) {
        ...
        if (item != null) {
            for (int col = 0; col < table.getColumnCount(); col++) {
                ...
                if (rect.contains(mousePt)) {
                    return new Point(col, table.indexOf(item));
                }
            }
        }
