 ...
 * Dec 06, 2012  1351       skorolev  Changes for non-blocking dialogs.
 * Dec 26, 2015  5114       skorolev  Corrected menu actions according to
 *                                    getThresholdPath.
 * Feb 23, 2016  5393       randerso  Remove hard coded button width
 * May 07, 2019  7689       randerso  Made File->Save and Commit Changes both
 *                                    commit changes and save file. Code cleanup
 *
 * </pre>
 *
 * @author lvenable
...
public abstract class MonitorDisplayThreshDlg extends CaveSWTDialog {
    ...
    protected void menuOpenAction() {
        if (openDlg == null) {
            ...
            openDlg.addCloseCallback(new ICloseCallback() {
                @Override
                public void dialogClosed(Object returnValue) {
                    if (returnValue instanceof LocalizationFile) {
                        LocalizationFile fileName = (LocalizationFile) returnValue;
    ...
    protected void doOpen(LocalizationFile fileName) {
        AbstractThresholdMgr atm = getThresholdMgr();
        atm.loadDisplayThreshold(fileName.getFile().getName());
        reloadThresholds();
    }
