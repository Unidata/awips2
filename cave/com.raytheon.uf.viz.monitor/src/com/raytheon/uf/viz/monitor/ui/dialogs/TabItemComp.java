...
public abstract class TabItemComp extends Composite {
        ...
        editBtn.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent event) {
                if (dataTable.getSelectionCount() == 0) {
                    ...
                    mb.setText("Warning");
                    mb.setMessage(
                            "Please select data from the list to be edited.");
                    mb.open();
    ...
    protected void createGroupHeader(String text, int startCol, int endCol,
            boolean border) {
        Composite header = new Composite(groupComp,
                (border ? SWT.BORDER : SWT.NONE));
    ...
    protected void createHeader(String text, int startCol, int endCol,
        ...
        Composite header = new Composite(headerComp,
                (border ? SWT.BORDER : SWT.NONE));
        GridData gd = new GridData(SWT.DEFAULT, SWT.FILL, false, true);
        int width = 0;
        for (int c = startCol; c <= endCol; c++) {
    ...
    protected void appendIntData(TableItem item, int column, double rValue,
        ...
        item.setText(column, formatTableData(String.valueOf(intVal)));

        intVal = (int) yVal;
        item.setText(column + 1, formatTableData(String.valueOf(intVal)));
    }

    protected void appendDecimalData(TableItem item, int column, double rValue,
            double yVal) {
        item.setText(column, formatTableData(String.valueOf(rValue)));

        item.setText(column + 1, formatTableData(String.valueOf(yVal)));
    }

    protected String formatTableData(String dataStr) {
        return StringUtils.center(dataStr, 6);
    }
