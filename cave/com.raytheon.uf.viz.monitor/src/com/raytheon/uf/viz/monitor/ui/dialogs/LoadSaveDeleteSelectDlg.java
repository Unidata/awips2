 ...
 * ------------- -------- --------- --------------
 * Dec 05, 2012  1351     skorolev  Cleaned code
...
public class LoadSaveDeleteSelectDlg extends CaveSWTDialog {
    ...
    private void createListControl() {
        ...
        gd.heightHint = 400;
        gd.horizontalSpan = 2;
        cfgFileList = new List(controlComp,
        ...
        cfgFileList.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                if (dialogType == DialogType.SAVE_AS) {
                    String selItem = cfgFileList
                            .getItem(cfgFileList.getSelectionIndex());
        ...
        if (dialogType == DialogType.SAVE_AS) {
            ...
            gd.horizontalSpan = ((GridLayout) controlComp
                    .getLayout()).numColumns;
            Label sepLbl = new Label(controlComp,
    ...
    private boolean validateFileName() {
        StringBuilder sb = new StringBuilder(newFileNameTF.getText().trim());
        newFileNameTF.setText(sb.toString());
        if (sb.length() == 0) {
        ...
        if (!sb.toString().matches("[A-Za-z0-9._-]+")) {
            MessageBox mb = new MessageBox(shell, SWT.ICON_ERROR | SWT.OK);
            mb.setText("Warning");
            mb.setMessage(
                    "File name contains invalid charaters.  The file name can only\n"
                            + "contain A-Z, a-z, 0-9, or periods, underscores, or dashes.");
            mb.open();
            return false;
        }
        if (sb.toString().compareTo(excludedNameForSaving) == 0) {
            MessageBox mb = new MessageBox(shell, SWT.ICON_ERROR | SWT.OK);
            mb.setText("Warning");
        ...
        for (String listItem : listItems) {
            ...
            if (fn.compareTo(sb.toString()) == 0) {
                MessageBox mb = new MessageBox(shell,
                        SWT.ICON_ERROR | SWT.YES | SWT.NO);
                mb.setText("Warning");
                mb.setMessage(
                        "File name already exists.  Do you wish to overwrite\n"
                                + "the existing file?.");
                int result = mb.open();
        ...
        if (!sb.toString().endsWith(".xml")) {
            sb.append(".xml");
            newFileNameTF.setText(sb.toString().trim());
        }
        return true;
    }
    ...
    private void getAvailableConfigFiles() {
        ...
        LocalizationFile[] locFiles = PathManagerFactory.getPathManager()
                .listStaticFiles(fileNamePath, extensions, false, true);
        if (locFiles == null) {
            return;
        }
        for (int i = 0; i < locFiles.length; i++) {
            ...
            if (dialogType == DialogType.SAVE_AS
                ...
                if (excludedNameForSaving
                        .compareTo(locFiles[i].getFile().getName()) != 0) {
                    locFileMap.put(locFiles[i].getFile().getName(),
                            locFiles[i]);
                }
            } else {
