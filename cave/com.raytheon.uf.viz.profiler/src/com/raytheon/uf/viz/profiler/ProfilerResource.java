...
public class ProfilerResource
        extends AbstractVizResource<ProfilerResourceData, ProfilerDescriptor> {
    ...
    private Map<Long, List<PlotObject>> plotObjects = null;
    ...
    private void disposePlotObjects() {
        ...
        try {
            if (plotObjects != null) {
                for (List<PlotObject> plotList : plotObjects.values()) {
    ...
    protected void paintInternal(IGraphicsTarget target,
        ...
        if (paintProps.getDataTime() != null && paintProps.getDataTime()
                .getValidTime().getTimeInMillis() < earliestTime) {
            ...
            long earliestRequestTime = earliestTime
                    - NUM_PROFILE_STAFFS * 3600000;
    ...
    private void setScaleWidth(PaintProperties props, Double magnification) {
        ...
        scaleWidthValue = ((ProfilerUtils.BARB_SIZE / 2.0)
                / screenToWorldWidthRatio) * magnification;
    }
    ...
    private void setScaleHeight(PaintProperties props, Double magnification) {
        ...
        scaleHeightValue = ((ProfilerUtils.BARB_SIZE / 2.0)
                / screenToWorldHeightRatio) * magnification;
    }
    ...
    private PixelCoverage getPixelCoverage(Coordinate c) {
        ...
        Coordinate ul = new Coordinate(c.x - getScaleWidth(),
                c.y - getScaleHeight());
        Coordinate ur = new Coordinate(c.x + getScaleWidth(),
                c.y - getScaleHeight());
        Coordinate lr = new Coordinate(c.x + getScaleWidth(),
                c.y + getScaleHeight());
        Coordinate ll = new Coordinate(c.x - getScaleWidth(),
                c.y + getScaleHeight());
    ...
    private void drawProfiles(PaintProperties paintProps) throws VizException {
        ...
        for (Entry<Long, List<PlotObject>> entry : plotObjects
                .entrySet()) {
            ...
            List<PlotObject> plots = entry.getValue();
            if (plots == null) {
                continue;
            }
     ...
     * @throws VizException
     */
    private void createProfiles(List<ProfilerObs> profiles)
            throws VizException {
        plotObsLock.lock();
        try {
            // bottom to top? I hope?
            if (plotObjects == null) {
                plotObjects = new HashMap<Long, List<PlotObject>>();
            }
            ...
            for (ProfilerObs profile : profiles) {
                ...
                List<PlotObject> plots = plotObjects.get(time);
                if (plots == null) {
    ...
    public void drawYAxis(PaintProperties paintProps, Double magnification)
        ...
        for (int i = 0; i < ProfilerUtils.PRESSURES.length; i++) {
            ...
            if (height <= MAX_Y) {
                DrawableString string = new DrawableString(
                        ProfilerUtils.decimalFormat.format(
                                new Double(ProfilerUtils.PRESSURES[i])) + " mb",
                        ProfilerUtils.GRAPH_COLOR);
                string.addTextStyle(TextStyle.BLANKED);
                string.font = font;
                string.verticallAlignment = IGraphicsTarget.VerticalAlignment.MIDDLE;
        ...
        bottom.setCoordinates(ProfilerUtils.profilerRectangle.x,
                (ProfilerUtils.profilerRectangle.y
                        + ProfilerUtils.profilerRectangle.height));
        bottom.addPoint(
                (ProfilerUtils.profilerRectangle.x
    ...
    private final double calcY(double height) {
        return (ProfilerUtils.profilerRectangle.y
                + ProfilerUtils.profilerRectangle.height)
                - (height * incYheight);
    }
