/**
 * 
 * edu.ucar.unidata.ui.ncradarui.dbutil.NcradarDbQuery
 * 
 * This java class performs the NCRADAR GUI database query.
...
public class NcradarDbQuery {

    private final String NCRADAR_DATA_DB_NAME = "metadata";

    private final String NCRADAR_STATIC_DB_NAME = "ncep";

    private final String NCRADAR_STATIC_GP_TABLE_NAME = "nwx.datatypegrouplist";

    private final String NCRADAR_DATA_DB_TABLE = "awips.nctext";
    ...
    /*
     * private static final String OBS_TAF_DATA_TABLE = "nctaf"; private static
     * final String OBS_TAF_DATA_RAW_DATA_KEY = "TAFTEXT"; private static final
     * String OBS_TAF_DATA_TIME_KEY = "START_DATE"; private static final String
     * OBS_TAF_DATA_PRODUCT_NAME = "TAFs Decoded";
     */
    // private final String NCRADAR_STATION_GP_TABLE_NAME =
    // "awips.nctext_stn_group";
    private final String NCRADAR_FILE_TYPE_TABLE_NAME = "awips.nctext_inputfile_type";
    ...
    private static Map<String, List<String>> gpToProductlistMap = new HashMap<String, List<String>>();// <key:
                                                                                                      // dataTypeGpName,
                                                                                                      // value:
                                                                                                      // productList
                                                                                                      // array>
    ...
    private static Map<String, List<NcradarStationInfo>> productAllStationInfoListMap = new HashMap<String, List<NcradarStationInfo>>(); // <key:productName,
                                                                                                                                       // value:List<NcradarStationInfo>>

    // Map, from a data productName to its stationInfo list of a state,
    // key:productname+state, value:its state stn info list
    private static Map<String, List<NcradarStationInfo>> productStateStationInfoListMap = new HashMap<String, List<NcradarStationInfo>>(); // <key:productName,
                                                                                                                                         // value:List<NcradarStationInfo>>

    private static Map<String, String> productNameToTypeMap = new HashMap<String, String>(); // <key:productName,
                                                                                             // value:
                                                                                             // productType>

    private static Map<String, String> fileExtToFileTypeMap = new HashMap<String, String>(); // <key:fileExt,
                                                                                             // value:
                                                                                             // file
                                                                                             // Type>
    ...
    private List<Object[]> readfileTypeInfoList() {
        ...
        String queryStr = new String("Select * FROM "
                + NCRADAR_FILE_TYPE_TABLE_NAME);
        List<Object[]> list = null;
        try {
            list = NcDirectDbQuery.executeQuery(queryStr, NCRADAR_DATA_DB_NAME,
                    QueryLanguage.SQL);
        } catch (Exception e) {
            System.out.println("-----DB exception at readfileTypeInfoList: "
                    + e.getMessage());
        }
    ...
    private List<Object[]> readDataTypeGpInfoList() {
        ...
        String queryStr = new String("Select * FROM "
                + NCRADAR_STATIC_GP_TABLE_NAME + " ORDER BY id");
        List<Object[]> list = null;
        try {
            list = NcDirectDbQuery.executeQuery(queryStr,
                    NCRADAR_STATIC_DB_NAME, QueryLanguage.SQL);
        } catch (Exception e) {
            System.out.println("-----DB exception at readDataTypeGpInfoList: "
                    + e.getMessage());
        }
    ...
    private List<Object[]> readStnInfo(String productTblName) {
        ...
        try {
            ...
            System.out.println("-----DB exception at readStnInfo: "
                    + e.getMessage());
        }
    ...
    private void addToProductAllStationInfoListMap(String productName,
        ...
        List<NcradarStationInfo> listOfStn = new ArrayList<NcradarStationInfo>();
        // System.out.println("Product is adding "+ productName);
        for (Object[] obj : stnInfoLstObj) {
            NcradarStationInfo stn = new NcradarStationInfo();
    ...
    private void addToProductStateStationInfoListMap(String productName,
            List<Object[]> stnInfoLstObj) {
        List<NcradarStationInfo> listOfAllStn = productAllStationInfoListMap
                .get(productName);
        ...
        for (NcradarStationInfo stn : listOfAllStn) {
            state = stn.getState();
        ...
        // add to map
        List<NcradarStationInfo> listOfStateStn;

        for (String sta : setOfState) {
            listOfStateStn = new ArrayList<NcradarStationInfo>();
            for (NcradarStationInfo stn : listOfAllStn) {
    ...
    private List<Object[]> readGpProductInfo(String gpTableName) {
        ...
        try {
            ...
            System.out.println("-----DB exception at readGpProductInfo: "
                    + e.getMessage());
        }
    ...
    private void initTables() {
        ...
        if (dataTypeGpObjList != null) {
            ...
            for (Object[] gpObjArray : dataTypeGpObjList) {
                ...
                // System.out.println("id = "+gpObjArray[0]+
                // " gp name = "+gpName+ " gp tbl name = "+gpTbl);
                ...
                if (gpProObjList != null) {
                    ...
                    for (Object[] proObjArray : gpProObjList) {
                        ...
                        String productType = (String) proObjArray[3];// input
                                                                     // file
                                                                     // extension
                                                                     // used as
                                                                     // product
                                                                     // type
                        // System.out.println("id = "+proObjArray[0]+
                        // " product name = "+productName+
                        // " pro tbl name = "+productTbl);
    ...
    private String createProductDataQuery(String productName,
            NcradarStationInfo sta, EReportTimeRange rptTimeRange) {
        StringBuilder queryStr = null;
        String productType;
        String fileType;
        if ((productName != null) && (sta != null)) {
            productType = productNameToTypeMap.get(productName); // product type
                                                                 // is ingested
                                                                 // text file
                                                                 // extension
            ...
            if ((fileType != null) && (fileType.equals("R"))) {
                ...
                // queryStr = new
                // StringBuilder("Select rawrecord FROM "+NCRADAR_DATA_DB_TABLE+" WHERE issuesite='"+gpStn+"' AND producttype='"+productType+"'");
                queryStr = new StringBuilder(
                        "Select rawrecord, issuesite FROM "
                                + NCRADAR_DATA_DB_TABLE
                ...
                queryStr = new StringBuilder(
                        "Select rawrecord, issuesite FROM "
                                + NCRADAR_DATA_DB_TABLE + " WHERE wmoid='"
                ...
                queryStr = new StringBuilder(
                        "Select rawrecord, issuesite FROM "
                                + NCRADAR_DATA_DB_TABLE + " WHERE issuesite='"
                                + sta.getStnid() + "' AND producttype='"
                                + productType + "'");
            }
            ...
            queryStr = new StringBuilder("Select rawrecord, issuesite FROM "
                    + NCRADAR_DATA_DB_TABLE + " WHERE producttype='"
                    + productType + "'");
        } else if (sta != null) {
            queryStr = new StringBuilder("Select rawrecord, issuesite FROM "
                    + NCRADAR_DATA_DB_TABLE + " WHERE wmoid='"
    ...
    // singleton object
    private NcradarDbQuery() {
        // System.out.println("NcradarDbQuery constructed");
        initTables();
    }

    // create this singleton object
    private static NcradarDbQuery ncradarDbQuery = null;

    public static NcradarDbQuery getAccess() {
        if (ncradarDbQuery == null) {
            ncradarDbQuery = new NcradarDbQuery();
        }
        return ncradarDbQuery;
    }
    ...
    public List<NcradarStationInfo> getProductStaList(String productName,
            EReportTimeRange timeCovered) {
        List<NcradarStationInfo> staList = productAllStationInfoListMap
                .get(productName);

        String productType = productNameToTypeMap.get(productName); // product
                                                                    // type is
                                                                    // ingested
                                                                    // text file
                                                                    // extension
        String fileType = fileExtToFileTypeMap.get(productType);
        // System.out.println("product type "+ productType+ " file type "+
        // fileType );
        if ((fileType != null)
            ...
            StringBuilder queryStr = null;
            List<Object[]> list = null;
            List<NcradarStationInfo> staListFiltered = new ArrayList<NcradarStationInfo>();
            ...
            try {
                list = NcDirectDbQuery.executeQuery(queryStr.toString(),
                        NCRADAR_DATA_DB_NAME, QueryLanguage.SQL);
                ...
                for (Object[] objAr : list) {
                    for (NcradarStationInfo stnInfo : staList) {
                        if (fileType.equals("WRECON")) {
            ...
            List<NcradarStationInfo> rtnstaList = new ArrayList<NcradarStationInfo>();
    ...
    public List<Object[]> getProductDataList(String productName,
            NcradarStationInfo station, EReportTimeRange rptTimeRange,
        ...
        for (NcradarStationInfo sta : listOfStateStn) {
            // System.out.println("Station "+ sta.getStnid() + " -  " +
            // sta.getStnname());
    ...
    private List<List<Object[]>> getOBSSfcHrlyDataListList(
        ...
        rcMap.put("pluginName", new RequestConstraint(OBS_SFC_HRLY_TABLE,
        ...
        if (rptTimeRange.getTimeRange() > 0) {
            ...
            Date date = new Date(cal.getTimeInMillis()
                    - rptTimeRange.getTimeRange() * 3600000);
            String dateStr = timeInSimple.format(date);
            rcMap.put("dataTime.refTime", new RequestConstraint(dateStr,
        ...
        for (NcradarStationInfo sta : listOfStateStn) {
            ...
            String myStnId;
            // Chin Note; in nwx.sfstns table, those US stations' id started
            // with K, its heading 'K' letter has been removed.
            // in "obs" table, US station id started with K is still coded with
            // K at head.
            // so, we have add K in front of station id for query to 'obs'
            // table.
            if (sta.stnid.length() < 4)
                ...
                myStnId = sta.stnid;
            ;
            rcMap.put("location.stationId", new RequestConstraint(myStnId,
                    ConstraintType.EQUALS));
            ...
            /*
             * Chin Note: A typical report from "obs" DB table Header+Str1
             * (starting with station id)\r\r\n +str2+optional \r\r\n....
             * Example: KTPH as station id. A raw report 1 "SAUS70 KWBC 250000
             * METAR KTPH 251456Z AUTO 05007KT 10SM CLR M02/M10 A3009 RMK AO2
            ...
            if (pdc != null) {
                ...
                for (int i = 0; i < pdc.getCurrentSz(); i++) {
                    ...
                    System.out.println("raw data::" + i + "::"
                            + pdv.getString(OBS_SFC_HRLY_RAW_DATA_KEY));
    ...
    private List<List<Object[]>> getOBSSynDataListList(
        ...
        List<List<Object[]>> rtnListList = new ArrayList<List<Object[]>>();
        Map<String, RequestConstraint> rcMap = new HashMap<String, RequestConstraint>();
        rcMap.put("pluginName", new RequestConstraint(OBS_SYN_DATA_TABLE,
                ConstraintType.EQUALS));
        if (rptTimeRange.getTimeRange() > 0) {
            Calendar cal = Calendar.getInstance(TimeZone.getTimeZone("GMT"));
            SimpleDateFormat timeInSimple = new SimpleDateFormat(
        ...
        for (NcradarStationInfo sta : listOfStateStn) {
            ...
            String myProdId;
            /*
             * Chin Note; in nwx.lsfstns table, 'productId' was coded with 6
             * digits (characters) with the last digit always coded as '0'. This
             * id is sent by GUI for data query. However, in OBS_SYN_DATA_TABLE
             ...
             * 
             * TBD::: However, some productId in nwx.lsfstns table only have 5
             * digits. In such case, how do we handle it?? TBD Chin.
             */

            myProdId = sta.productid.substring(0, 5);
            rcMap.put("location.stationId", new RequestConstraint(myProdId,
                    ConstraintType.EQUALS));
            List<String> parameters = new ArrayList<String>();
            ...
            if (pdc != null) {
                ...
                for (int i = 0; i < pdc.getCurrentSz(); i++) {
                    ...
                    // report = report + oneReport+"\n";
                    System.out.println("raw data::" + i + "::"
                            + pdv.getString(OBS_SYN_DATA_RAW_DATA_KEY));
    ...
    void addToSndPdvList(PointDataView pdv, List<PointDataView> pdvlst) {
        ...
        // If pdv's data type is Not found in any pdv in list, then just add it
        // to list.
        // If pdv's data type is already found in one of the pdv in list. Then
        // comparing its
        ...
        // NWX report data in the order of "TTAA, PPAA, TTBB, PPBB, TTCC,
        // ...etc. We will have to do the same way.
    ...
    private List<List<Object[]>> getOBSSndDataListList(
        ...
        List<List<Object[]>> rtnListList = new ArrayList<List<Object[]>>();
        Map<String, RequestConstraint> rcMap = new HashMap<String, RequestConstraint>();
        rcMap.put("pluginName", new RequestConstraint(OBS_SND_DATA_TABLE,
                ConstraintType.EQUALS));
        ...
        for (NcradarStationInfo sta : listOfStateStn) {
            ...
            // Chin Note; in nwx.snstns table, US stations' id are not completed
            // coded.
            // Therefore, use its productId as searching key. ProductId is a
            // miss leading name. It is actually
            // is the station number.
            // The equivalent filed of productId in OBS_SND_DATA_TABLE is
            // 'stnum'.

            rcMap.put("stnum", new RequestConstraint(myProdId,
                    ConstraintType.EQUALS));
            List<String> parameters = new ArrayList<String>();
            ...
            if (pdc != null) {
                ...
                /*
                 * Chin Note: Each TTAA, TTBB...PPAA.. data are stored in its
                 * own record with "correction" We have to keep the latest
                 * corrected data and discard earlier "not correct" data We then
                 * store same obs time data in one pdvList and then put into
                ...
                for (int i = 0; i < pdc.getCurrentSz(); i++) {
                    ...
                    timeDataMap.put(sndTime, pdvList); // replace pdvList with
                                                       // latest one
                    System.out.println("raw data::::" + i + "::"
                            + pdv.getString(OBS_SND_DATA_RAW_DATA_KEY));
                }
                ...
                for (Map.Entry<Object, Object> entry : timeDataMap.entrySet()) {
                    ...
                    Iterator it = pdvList.iterator(); it.hasNext();) {
                        ...
                        oneRefTimeReport = oneRefTimeReport
                                + String.valueOf(recordAry) + "\n";
                    }
                    SimpleDateFormat timeInSimple = new SimpleDateFormat(
                            "yyMMdd/HHmm");// ("yyyy-MM-dd HH:mm:ss");
                ...
                /*
                 * timeDataMap = mapSortByComparator(timeDataMap); for
                 * (Map.Entry<Object, Object> entry : timeDataMap.entrySet()) {
                 * report = report + entry.getValue()+"\n"; }
                 */
                Object[] rtnObj = new Object[2];
    ...
    public List<List<Object[]>> getProductDataListList(String groupName,
            String productName, NcradarStationInfo station,
            EReportTimeRange rptTimeRange, boolean isState,
        ...
        if (groupName.equals(OBS_DATA_GROUP)) {
            // Note: when products need query to its own DB table (NOT to
            // "nctext" table)
            ...
            // Note: Agricurtural Obs (*.AGO), TAF(*.taf) and RADAT (*.FZL) data
    ...
    public Map<String, List<NcradarStationInfo>> getProductStationInfoListMap() {
    ...
    public List<NcradarStationInfo> getStateStationInfoList(String key) {
    ...
    private PointDataContainer requestObsPointData(List<String> parameters,
        ...
        try {
            ...
            System.out
                    .println("requestObsPointData-DataCubeContainer:Error getting raw data from "
                            + tableName + " ::" + e);
        }
        if (pdc == null) {
            try {
                ...
                System.out
                        .println("requestObsPointData-PointDataRequest: Error getting raw data from "
                                + tableName + " ::" + e);
            }
        }
        if (pdc != null) {
