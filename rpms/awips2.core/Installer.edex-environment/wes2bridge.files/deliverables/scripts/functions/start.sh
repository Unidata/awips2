...
function startEnvironmentInternal()
{
   ...
   source "${EDEX_ENV_DIR}/${env_name}/edex-environment/settings"
   ...
   if [ ! -f ${EDEX_ENV_DIR}/${env_name}/edex-environment/postgresql@awips_${env_name_lowercase}.service.d/30-postgresql-setup.conf ]; then
      echo "${EDEX_ENV_DIR}/${env_name}/edex-environment/postgresql@awips_${env_name_lowercase}.service.d/30-postgresql-setup.conf missing"
      echo "ERROR: The ${env_name} environment is corrupt. Recreate it."
      return 1
   fi
   ...
   if [ ! -f ${EDEX_ENV_DIR}/${env_name}/edex-environment/httpd-pypies-logging_${env_name_lowercase}.service ]; then
      echo "${EDEX_ENV_DIR}/${env_name}/edex-environment/httpd-pypies-logging_${env_name_lowercase}.service missing"
      echo "ERROR: The ${env_name} environment is corrupt. Recreate it."
      return 1
   fi
