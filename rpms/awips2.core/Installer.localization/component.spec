...
function removeHydroDbDirectory()
{
   ...
   if [ -d ${hydro_db_directory} ]; then
      rm --recursive --force ${hydro_db_directory}
      if [ $? -ne 0 ]; then
...
function restoreHydroDb()
{
   ...
   sed --in-place "s/${default_damcat}/${DAMCAT_DATABASE}/g" ${pg_hba_conf}
   if [ $? -ne 0 ]; then
   ...
   sed --in-place "s/${default_ihfs}/${IHFS_DATABASE}/g" ${pg_hba_conf}
   if [ $? -ne 0 ]; then
      echo "Failed to update ihfs database in ${pg_hba_conf}!" >> ${log_file}
      return 0
   fi

   systemctl restart postgresql@awips
