# Turn off the brp-python-bytecompile script
%global __os_install_post %(echo '%{__os_install_post}' | sed -e 's!/usr/lib[^[:space:]]*/brp-python-bytecompile[[:space:]].*$!!g')
%define _build_arch %(uname -i)

#
# AWIPS II Python cherrypy Site-Package Spec File
#
Name: awips2-python-cherrypy
Summary: AWIPS II Python cherrypy Site-Package
Version: 3.8.2
Release: 1%{?dist}
Group: AWIPSII
BuildRoot: %{_build_root}
BuildArch: noarch
URL: N/A
License: N/A
Distribution: N/A
Vendor: %{_build_vendor}
Packager: %{_build_site}

AutoReq: no
Requires: awips2-python
Provides: awips2-python-cherrypy

BuildRequires: awips2-python

%description
AWIPS II Python cherrypy Site-Package - Installs the AWIPS II Python
cherrypy Site-Package in the Python installation.

%prep
# Verify That The User Has Specified A BuildRoot.
if [ "%{_build_root}" = "" ]
then
   echo "A Build Root has not been specified."
   echo "Unable To Continue ... Terminating"
   exit 1
fi

rm -rf %{_build_root}
mkdir -p %{_build_root}
if [ $? -ne 0 ]; then
   exit 1
fi

%build
SITE_PACKAGE_SRC="pythonPackages/cherrypy"
SITE_PACKAGE="%{version}.tar.gz"
cd  %{_baseline_workspace}/${SITE_PACKAGE_SRC}
tar -xvzf ${SITE_PACKAGE}
cd cherrypy-%{version}

/awips2/python/bin/python setup.py build

%install
SITE_PACKAGE_SRC="pythonPackages/cherrypy"
SITE_PACKAGE="%{version}.tar.gz"
cd  %{_baseline_workspace}/${SITE_PACKAGE_SRC}/cherrypy-%{version}

/awips2/python/bin/python setup.py install --root=%{_build_root} \
   --prefix=/awips2/python

%pre

%post

%preun

%postun

%files
%defattr(644,awips,fxalpha,755)
/awips2/python/lib/python2.7/site-packages/*
%defattr(755,awips,fxalpha,755)
/awips2/python/bin/*
