...
if [ -f /data/fxa/INSTALL/awips2/scripts/.global ]; then
   ...
   thisSite=$(hostname | cut -d. -f1 | cut -d- -f2 | tr '[:upper:]' '[:lower:]')
   for site in ${disableIngestDatCaseArray[*]};
      do
         # After separation, first/last elements on disableIngestDatCaseArray may have
