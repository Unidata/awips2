...
$config->define('DBname',{ ARGCOUNT => ARGCOUNT_ONE, VALIDATE => '[\w]+',DEFAULT => 0});
$config->define('DBhost',{ ARGCOUNT => ARGCOUNT_ONE, VALIDATE => '[\w]+',DEFAULT => 0});
$config->define('DBuser',{ ARGCOUNT => ARGCOUNT_ONE, DEFAULT => 0});
$config->define('DBsslmode',{ ARGCOUNT => ARGCOUNT_ONE, DEFAULT => 0});
$config->define('DBsslcertDir',{ ARGCOUNT => ARGCOUNT_ONE, DEFAULT => 0});
$config->define('DBport',{ ARGCOUNT => ARGCOUNT_ONE ,DEFAULT => 0});
$config->define('DBdefaultPort',{ ARGCOUNT => ARGCOUNT_ONE, DEFAULT => 5432});
$config->define('master',{ VALIDATE => '[.\w]+',DEFAULT => "HGstation"});
$config->define('basedir',{ VALIDATE => '[- /.\w]+',DEFAULT => $ENV{HYDROGENHOME} . "/bin"});
$config->define('getAppsDefExecPath', { ARGCOUNT => ARGCOUNT_ONE, DEFAULT => "/awips/hydroapps/public/bin/get_apps_defaults.LX"});
...
my $DBsslmode = "";
my $DBsslcertDir = "";
my $appsDefExecPath = $config->get('getAppsDefExecPath');
...
# get variables from the Apps_default file (unless defined in ~/input/hg.cfg file, which takes precedence) 

if (!($config->get('DBname'))) 
{
    if( -e  $appsDefExecPath)
  	{
        $DBname = `$appsDefExecPath db_name`;
        print "(A)dbname: '$DBname'\n";
    }
}
else 
{
...
if ($DBname eq '') {
    print "db_name parameter value not provided in config files. Exiting\n";
    exit -1;
}
...
if (!($config->get('DBhost'))) 
{
    if( -e  $appsDefExecPath)
  	{
        $DBhost = `$appsDefExecPath pghost`;
        print  "(A)dbhost: '$DBhost'\n";
    }
}
else 
{
...
if ($DBhost eq '') {
	print "pghost parameter value not provided in config files. Exiting\n";
	exit -1;
}
...
if (!($config->get('DBport'))) 
{
    if( -e  $appsDefExecPath)
  	{
    	$DBport = `$appsDefExecPath pgport`;
        print  "(A)dbport: '$DBport'\n";
    }
}
else 
{
...
if ($DBport eq '') {
	$DBport = $config->get('DBdefaultPort');
}
...
if(!($config->get('DBuser'))) 
{
  if( -e  $appsDefExecPath)
  {
  	$DBuser = `$appsDefExecPath pguser`;
	print  "(A)dbuser: '$DBuser'\n";
  }
}
else
{
...
if ($DBuser eq '') {
	print "pguser parameter value not provided in config files. Exiting\n";
	exit -1;
}

if(!($config->get('DBsslmode')))
{
  if( -e  $appsDefExecPath)
  {
	$DBsslmode = `$appsDefExecPath DB_ssl_mode`;
	print  "(A)DBsslmode: '$DBsslmode'\n";
  }
}
else
{
  $DBsslmode = $config->get('DBsslmode');
}
if ($DBsslmode eq '') {
	print "DB_ssl_mode parameter value not provided in config files. Exiting\n";
	exit -1;
}

if (!($config->get('DBsslcertDir')))
{
  if( -e  $appsDefExecPath)
  {
	$DBsslcertDir = `$appsDefExecPath DB_ssl_cert_dir`;
   	print  "(A)DB_SSL_certdir: '$DBsslcertDir'\n";
  }
}
else
{
  $DBsslcertDir = $config->get('DBsslcertDir');
}
if ($DBsslcertDir eq '') {
	print "DB_ssl_cert_dir parameter value not provided in config file. Exiting\n";
	exit -1;
}

# make sure no extraneous characters (white space characters) are in the DB parameters
$DBname =~ s/\s+//g;
$DBhost =~ s/\s+//g;
$DBport =~ s/\s+//g;
$DBsslmode =~ s/\s+//g;
...
print "(B)DBname: '$DBname'\n";
print "(B)DBhost: '$DBhost'\n";
print "(B)DBport: '$DBport'\n";
print "(B)DBsslmode: '$DBsslmode'\n";
print "(B)DB_SSL_certdir: '$DBsslcertDir'\n";
print "(B)DBuser: '$DBuser'\n";


#Open a database connection and get the list of LIDs from the IHFS DB
if($DBengine eq "Pg") {
        $DBstr = "dbi:Pg:dbname=$DBname;host=$DBhost;port=$DBport;sslmode=$DBsslmode;sslcert=$DBsslcertDir/$DBuser.crt;sslkey=$DBsslcertDir/$DBuser.key;sslrootcert=$DBsslcertDir/root.crt";
        $wildcard = '%';
} else {
		print "DB other than PostgreSQL is not currently supported. Exiting\n";
		exit -1;
...
print "DBstr: '$DBstr'\n";
my $dbh = DBI->connect("$DBstr","$DBuser",undef,{ChopBlanks => 1}) or warn $DBI::errstr;
