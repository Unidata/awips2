...
#Adding ssl keys to use postgres authentication
export LD_LIBRARY_PATH=/awips2/psql/lib:$LD_LIBRARY_PATH
export PATH=/awips2/psql/bin:$PATH
export NRLDB_LOG=`get_apps_defaults nrldb_log`
export NRLDB_TMP==`get_apps_defaults nrldb_tmp`

SSL_KEYS='sslcert='$DB_SSL_DIR'/'"$PGUSER"'.crt sslkey='$DB_SSL_DIR'/'"$PGUSER".key' sslrootcert='$DB_SSL_DIR'/root.crt'

wfo=`$POSTGRESQLBINDIR/psql "host=$PGHOST dbname=$DB_NAME  user=$PGUSER sslmode=$DB_SSL_MODE $SSL_KEYS" \
-c "select hsa from admin;" | tail -3 | head -1 | sed -e 's/ //g'`
			...
			llid=`echo $lid | wc -m`
        		...
        		do
					...
					ls -l $NRLDB_TMP/update.txt
                                        $POSTGRESQLBINDIR/psql "host=$PGHOST dbname=$DB_NAME  user=$PGUSER sslmode=$DB_SSL_MODE $SSL_KEYS" -c  \
                                          "\copy (select * from $table where lid = '$lid') to '$NRLDB_TMP/update.txt' with delimiter '|';"
					...
					then
						...
						do
                                                        $POSTGRESQLBINDIR/psql "host=$PGHOST dbname=$DB_NAME  user=$PGUSER sslmode=$DB_SSL_MODE $SSL_KEYS" \
                                                         -c "select $col from location where lid = '$lid' and $col is not null;" > ${NRLDB_DATA}/update.txt
					...
					then
						...
						for col in primary_pe bf cb da response_time threshold_runoff fq fs gsno level mile pool por rated lat lon remark rrevise rsource stream tide backwater vdatum action_flow wstg zd ratedat usgs_ratenum uhgdur use_latest_fcst
                                                do
                                                        $POSTGRESQLBINDIR/psql "host=$PGHOST dbname=$DB_NAME  user=$PGUSER sslmode=$DB_SSL_MODE $SSL_KEYS" \
                                                          -c "select $col from riverstat where lid = '$lid' and $col is not null;" > ${NRLDB_DATA}/update.txt
							ct_zero=`grep -c "0 row" ${NRLDB_DATA}/update.txt`
							if [ $ct_zero -eq 0 ]
							then	
								export val=`cat ${NRLDB_DATA}/update.txt | head -3 | tail -1 | cut -c2-80`
