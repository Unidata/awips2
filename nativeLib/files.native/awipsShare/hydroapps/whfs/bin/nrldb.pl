...
$config->define('getAppsDefExecPath', { ARGCOUNT => ARGCOUNT_NONE, DEFAULT => "/awips/hydroapps/public/bin/get_apps_defaults.LX"});
$config->getopt(\@ARGV);
...
our $file_name;
our $conf_dir;
my ($dbname, $host, $port, $user, $sslmode, $sslcertDir, $nrldb_host, $backup_host);
my $DBstr;

my @delete_list;
my $delete_listRef;

print LOG "Version $vNo\n";
...
($dbname, $host, $port, $user, $sslmode, $sslcertDir, $nrldb_host, $office, $backup_host) = read_config_file();
...
#Connect to database
our $db = db_connect($dbname, $host, $user, $port, $sslmode, $sslcertDir);
...
my $appsDefExec = $config->get('getAppsDefExecPath');

if( -e $appsDefExec)
{
	$conf_dir = `$appsDefExec  nrldb_config`;
	
	chomp($conf_dir);
	$conf_file = "${conf_dir}/nrldb.conf";
	
	if ($conf_dir eq '') {
		print "nrldb_config parameter value not provided in Apps_defaults file. Exiting\n";
		exit -1;
	}
	
	$sslmode = `$appsDefExec  DB_ssl_mode`;
	$sslmode =~ s/\s+//g;
	
	if ($sslmode eq '') {
		print "DB_ssl_mode parameter value not provided in Apps_defaults file. Exiting\n";
		exit -1;
	}
	
	$sslcertDir = `$appsDefExec  DB_ssl_cert_dir`;
	$sslcertDir =~ s/\s+//g;
	
	if ($sslcertDir eq '') {
		print "DB_ssl_cert_dir parameter value not provided in Apps_defaults file. Exiting\n";
		exit -1;
	}
	
	$port = `$appsDefExec  pgport`;
	$port =~ s/\s+//g;
	
	if ($port eq '') {
		$port = $config->get('DBdefaultPort');
	}
	
}
else
{
	print "Executable to read Apps_default file cannot be located; unable to proceed. Exiting\n";
	exit -1;
}
...
close(FILE);
  ...
  if ($dbname eq '') {
		if( -e $appsDefExec) 
		{
  			$dbname = `$appsDefExec db_name`;
  		}
  
  		if ($dbname eq '') {
			print "dbname parameter value not provided in config files. Exiting\n";
			exit -1;
		}
  }
  
  if ($user eq '') {
	print "dbuser parameter value not provided in config files. Exiting\n";
	exit -1;
  }
  
  if ($host eq '') {
	print "dbhost parameter value not provided in config files. Exiting\n";
	exit -1;
  }
  
  print LOG "(B)dbname: '$dbname'\n";
  print LOG "(B)host: '$host'\n";
  print LOG "(B)port: '$port'\n";
  print LOG "(B)sslmode: '$sslmode'\n";
  print LOG "(B)sslcertDir: '$sslcertDir'\n";
  print LOG "(B)user: '$user'\n";
  
  return($dbname, $host, $port, $user, $sslmode, $sslcertDir, $nrldb_host, $site_conf, $backup_host);
}
...
$DBstr = "dbi:Pg:dbname=$dbname;host=$host;port=$port;sslmode=$sslmode;sslcert=$sslcertDir/$user.crt;sslkey=$sslcertDir/$user.key;sslrootcert=$sslcertDir/root.crt";

print LOG "DBstr: '$DBstr'\n";
my $db = DBI->connect("$DBstr","$user",undef,{ChopBlanks => 1}) or warn " Can't connect() to database $dbname: $DBI::errstr ";
