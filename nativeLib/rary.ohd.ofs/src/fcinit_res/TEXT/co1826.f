C MEMBER CO1826
C  (from old member FCCO1826)
C
      SUBROUTINE CO1826(WORK,IUSEW,LEFTW,NCO18)
C--------------------------------------------------------------------
C  ROUTINE TO GET AND CHECK CO SPECIFICATIONS FOR S/U #18 -
C  BACKFLOW UTILITY
C---------------------------------------------------------------------
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcinit_res/RCS/co1826.f,v $
     . $',                                                             '
     .$Id: co1826.f,v 1.1 1995/09/17 18:51:28 dws Exp $
     . $' /
C    ===================================================================
C
C
...
C
   15 CONTINUE
      IF (ENDFND.OR.NCARD.GE.LASTCD) GO TO 200
      NUMFLD = 0
...
C
C----------------------------------------------------------------------
C  'ELEV' FOUND. GET THE CO INFO FROM THE REST OF THE LINE.
C
  100 CONTINUE
C
C  GET NEXT FIELD ON LINE
C
      NUMFLD = -2
      CALL UFLD26(NUMFLD,IERF)
      IF (IERF.GT.1) GO TO 9000
      IF (IERF.EQ.1) GO TO 10
C
C  ELEVATION MUST BE REAL VALUE
C
      COOK = .FALSE.
...
C
      REAL = REAL/CONVL
      IF (ELSTOR(1).LE.REAL.AND.REAL.LE.ELSTOR(NSE)) GO TO 150
  140 CONTINUE
C
      CALL STER26(63,1)
      GO TO 10
...
C
  210 CONTINUE
      CALL FLWK26(WORK,IUSEW,LEFTW,COVAL,501)
C
      NCO18 = 1
      GO TO 9999
C
