c
C MEMBER SU0326
C  (from old member FCSU0326)
C
      SUBROUTINE SU0326(WORK,IUSEW,LEFTW,IERR)
C
C---------------------------------------------------------------------
C  SUBROUTINE TO GET PARAMETERS, TIME-SERIES, AND CARRYOVER VALUES
C  FOR SCHEME/UTILITY #03 - PRESCRIBED ELEVATION SCHEME
C---------------------------------------------------------------------
C  ARGS:
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcinit_res/RCS/su0326.f,v $
     . $',                                                             '
     .$Id: su0326.f,v 1.1 1995/09/17 18:52:50 dws Exp $
     . $' /
C    ===================================================================
C
C
C
      DATA ENDSU/4HENDS,4HETH ,4H    /
C
...
C
C--------------------------------------------------------------------
C  NOW PROCESS INPUT UP TO 'ENDSETH ',LOOKING FOR, IN ORDER, KEYWORDS
...
C
C
      CALL PM0326(WORK,IUSEW,LEFTW,NPARXX,GETTS,GETCO,NCOVAL,
     .            LENDSU,JDEST,IERPM)
      IF (GETTS) NTSPXX = 1
C
...
C  IF NEEDED, CALL ROUTINE TO INPUT ALL REQUIRED AND OPTIONAL TIME-SERIE
C
  200 CONTINUE
      ISU(2) = ISU(2) + 1
...
C
  220 CONTINUE
      CALL TS0326(WORK,IUSEW,LEFTW,NTSXX,
     .            LENDSU,JDEST,IERTS)
C
C
...
C
C------------------------------------------------------------------
C  'CARRYOVER' EXPECTED HERE. IF NEEDED,
...
C
      IF (GETCO) GO TO 320
           ...
           GO TO 10
  320 CONTINUE
      CALL CO0326(WORK,IUSEW,LEFTW,NCOVAL,NCOXX)
C
      GO TO 10
C
C---------------------------------------------------------------
