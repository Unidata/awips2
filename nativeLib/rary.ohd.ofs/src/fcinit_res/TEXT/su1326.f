C MEMBER SU1326
C  (from old member FCSU1326)
C
      SUBROUTINE SU1326(WORK,IUSEW,LEFTW,IERR)
C
C---------------------------------------------------------------------
C  SUBROUTINE TO GET PARAMETERS, TIME-SERIES, AND CARRYOVER VALUES
C  FOR SCHEME/UTILITY #13 - POWER GENERATION SCHEME
C---------------------------------------------------------------------
C  ARGS:
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcinit_res/RCS/su1326.f,v $
     . $',                                                             '
     .$Id: su1326.f,v 1.1 1995/09/17 18:53:00 dws Exp $
     . $' /
C    ===================================================================
C
C
      DATA ENDSU/4HENDP,4HOWER,4H    /
C
C
...
C
C--------------------------------------------------------------------
C  NOW PROCESS INPUT UP TO 'ENDPOWER', LOOKING FOR, IN ORDER, KEYWORDS
...
C
C
      CALL PM1326(WORK,IUSEW,LEFTW,NPARXX,
     .            LENDSU,JDEST,IERPM)
C
...
C
C--------------------------------------------------------------
C  'CARRYOVER' FOUND, GO GET CARRYOVER VALUES
C
  300 CONTINUE
C
      ISU2 = ISU2 + 1
      IF (ISU2.GT.1) CALL STER26(39,1)
C
      COLOC = IUSEW + 1
      CALL RFIL26(WORK,LOCCXX,COLOC)
C
      CALL CO1326(WORK,IUSEW,LEFTW,NCOXX)
C
      GO TO 10
C
C---------------------------------------------------------------
...
C
C  STORE DEFAULT CARRYOVER IF 'CARRYOVER' NOT FOUND.
C
      IF (ISU2 .GT. 0) GO TO 9999
C
      COLOC = IUSEW + 1
      CALL RFIL26(WORK,LOCCXX,COLOC)
C
      NTIM24 = 24/MINODT
      DO 920 I=1,NTIM24
      CALL FLWK26(WORK,IUSEW,LEFTW,COQME,501)
  920 CONTINUE
C
      NCOXX = NTIM24
C
