C MEMBER CO0326
C  (from old member FCCO0326)
C
      SUBROUTINE CO0326(WORK,IUSEW,LEFTW,NCOVAL,NCO03)
C--------------------------------------------------------------------
C  ROUTINE TO GET AND CHECK CO SPECIFICATIONS FOR S/U #03 -
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcinit_res/RCS/co0326.f,v $
     . $',                                                             '
     .$Id: co0326.f,v 1.1 1995/09/17 18:51:19 dws Exp $
     . $' /
C    ===================================================================
C
C
...
C
C----------------------------------------------------------------------
C  'OLDH' FOUND. GET THE CO INFO FROM THE REST OF THE LINE.
C
  100 CONTINUE
C
      NVAL=50
      CALL GLST26(1,1,X,VALUE,X,NVAL,COOK)
      IF (.NOT.COOK) GO TO 10
C
C  SEE IF NUMBER READ IN EQUALS THE NUMBER EXPECTED.
C
      IF (NVAL.EQ.NCOVAL) GO TO 110
C
      CALL STER26(62,1)
      GO TO 10
C
...
C
  130 CONTINUE
C
C  SEE IF ELEVATIONS ARE WITHIN BOUNDS OF EL VS STORAGE CURVE
C
      DO 140 J=2,NSE
      IF (ELSTOR(J-1).LE.VALUE(I).AND.VALUE(I).LE.ELSTOR(J)) GO TO 150
  140 CONTINUE
C
      CALL STER26(63,1)
      GO TO 10
