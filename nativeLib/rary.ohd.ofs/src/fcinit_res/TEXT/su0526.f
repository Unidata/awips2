C MEMBER SU0526
C  (from old member FCSU0526)
C
      SUBROUTINE SU0526(WORK,IUSEW,LEFTW,IERR)
C
C---------------------------------------------------------------------
C  SUBROUTINE TO GET PARAMETERS, TIME-SERIES, AND CARRYOVER VALUES
C  FOR SCHEME/UTILITY #05 - FILL AND SPILL SCHEME
C---------------------------------------------------------------------
C  ARGS:
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcinit_res/RCS/su0526.f,v $
     . $',                                                             '
     .$Id: su0526.f,v 1.1 1995/09/17 18:52:52 dws Exp $
     . $' /
C    ===================================================================
C
C
C
      DATA ENDSU/4HENDF,4HILL ,4H    /
      DATA BLANK/4H    /
C
...
C
C--------------------------------------------------------------------
C  NOW PROCESS INPUT UP TO 'ENDFILL ',LOOKING FOR, IN ORDER, KEYWORDS
...
C
C
      CALL PM0526(WORK,IUSEW,LEFTW,NPARXX,
     .            LENDSU,JDEST,IERPM)
C
C
...
C  IF NOT FOUND, SIGNAL ERROR. IN TS0526 WHERE WE STORE ALL BLANKS IF TS
C  IS NOT TO BE USED.
C
  200 CONTINUE
      ISU(2) = ISU(2) + 1
...
C
C
      CALL TS0526(WORK,IUSEW,LEFTW,NTSXX,
     .            LENDSU,JDEST,IERTS)
C
C
...
C
C
      CALL CO0526(WORK,IUSEW,LEFTW,NCOXX)
      GO TO 10
C
C---------------------------------------------------------------
C  SUMMARY
C  IF TS IS NOT FOUND , STORE ALL BLANK ID'S FOR THREE OPTIONAL TS.
C
  900 CONTINUE
