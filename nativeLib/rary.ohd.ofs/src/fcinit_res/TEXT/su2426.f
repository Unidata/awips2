C MEMBER SU2426
C  (from old member FCSU2426)
C
      SUBROUTINE SU2426(WORK,IUSEW,LEFTW,IERR)
C
C---------------------------------------------------------------------
C  IF THIS ROUTINE IS CALLED, SIGNAL AN ERROR. NO INPUT IS NEEDED TO
C  DEFINE THIS S/U - ENTRY INTO FLASH BOARD UTILITY
C---------------------------------------------------------------------
C  ARGS:
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcinit_res/RCS/su2426.f,v $
     . $',                                                             '
     .$Id: su2426.f,v 1.1 1995/09/17 18:53:10 dws Exp $
     . $' /
C    ===================================================================
C
C
      DATA ENDSU/4HENDG,4HFLSH,4H    /
C
C
...
C
C--------------------------------------------------------------------
C  SET ERROR FOR FINDING INPUT TO 'GOFLASH ' AND LOOK FOR 'ENDPASS'
C  KEYWORD OR OTHER SU.
C
      CALL STRN26(53,1,SUID(1,24),3)
C
      USEDUP = .FALSE.
      ENDFND = .FALSE.
C
