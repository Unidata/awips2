C MEMBER SU1026
C  (from old member FCSU1026)
C
      SUBROUTINE SU1026(WORK,IUSEW,LEFTW,IERR)
C
C---------------------------------------------------------------------
C  SUBROUTINE TO GET PARAMETERS, TIME-SERIES, AND CARRYOVER VALUES
C  FOR SCHEME/UTILITY #10 - UPSTREAM STAGE MINIMIZATION SCHEME
C---------------------------------------------------------------------
C  ARGS:
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcinit_res/RCS/su1026.f,v $
     . $',                                                             '
     .$Id: su1026.f,v 1.1 1995/09/17 18:52:57 dws Exp $
     . $' /
C    ===================================================================
C
C
C
      DATA ENDSU/4HENDM,4HINH ,4H    /
C
...
C
C--------------------------------------------------------------------
C  NOW PROCESS INPUT UP TO 'ENDMINH', LOOKING FOR, IN ORDER, KEYWORDS
...
C
C
      CALL PM1026(WORK,IUSEW,LEFTW,NPARXX,
     .            LENDSU,JDEST,IERPM)
C
C
...
C
      CALL TS1026(WORK,IUSEW,LEFTW,NTSXX,
     .            LENDSU,JDEST,IERTS)
C
C
...
C
      CALL CO1026(WORK,IUSEW,LEFTW,NCOXX,
     .            LENDSU,JDEST,IERCO)
C
C
...
C
C  BACK BLOCK STRUCTURE OF CO WHEN IERCO IS 89 WHERE PM OR TS FOUND.
C
      IDEST = JDEST
      GO TO 50
C
