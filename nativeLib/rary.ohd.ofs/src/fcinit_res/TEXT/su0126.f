C MEMBER SU0126
C  (from old member FCSU0126)
C
      SUBROUTINE SU0126(WORK,IUSEW,LEFTW,IERR)
C
C---------------------------------------------------------------------
C  IF THIS ROUTINE IS CALLED, SIGNAL AN ERROR. NO INPUT IS NEEDED TO
C  DEFINE THIS S/U - PASS INFLOW SCHEME
C---------------------------------------------------------------------
C  ARGS:
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcinit_res/RCS/su0126.f,v $
     . $',                                                             '
     .$Id: su0126.f,v 1.1 1995/09/17 18:52:48 dws Exp $
     . $' /
C    ===================================================================
C
C
      DATA ENDSU/4HENDP,4HASS ,4H    /
C
C
...
C
C--------------------------------------------------------------------
C  SET ERROR FOR FINDING INPUT TO 'PASSFLOW' AND LOOK FOR 'ENDPASS'
C  KEYWORD OR OTHER SU.
C
      CALL STRN26(53,1,SUID(1,1),3)
C
      USEDUP = .FALSE.
      ENDFND = .FALSE.
C
