C MEMBER SU1126
C  (from old member FCSU1126)
C
      SUBROUTINE SU1126(WORK,IUSEW,LEFTW,IERR)
C
C---------------------------------------------------------------------
C  SUBROUTINE TO GET PARAMETERS, TIME-SERIES, AND CARRYOVER VALUES
C  FOR SCHEME/UTILITY #11 - INDUCED SURCHARGE SCHEME
C---------------------------------------------------------------------
C  ARGS:
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcinit_res/RCS/su1126.f,v $
     . $',                                                             '
     .$Id: su1126.f,v 1.1 1995/09/17 18:52:58 dws Exp $
     . $' /
C    ===================================================================
C
C
C
      DATA ENDSU/4HENDI,4HNDS ,4H    /
C
...
C
C--------------------------------------------------------------------
C  NOW PROCESS INPUT UP TO 'ENDINDS', LOOKING FOR, IN ORDER, KEYWORDS
...
C
C
      CALL PM1126(WORK,IUSEW,LEFTW,NPARXX,
     .            LENDSU,JDEST,IERPM)
C
C
...
C
      CALL CO1126(WORK,IUSEW,LEFTW,NCOXX,
     .            LENDSU,JDEST,IERCO)
C
C
...
C
C  STORE DEFAULT CARRYOVER IF NO CARRYOVER WAS ENETERED FOR SCHEME
C
      COLOC = IUSEW + 1.01
      CALL RFIL26(WORK,LOCCXX,COLOC)
C
      CALL FLWK26(WORK,IUSEW,LEFTW,COQIN,501)
