C MEMBER SU1226
C  (from old member FCSU1226)
C
      SUBROUTINE SU1226(WORK,IUSEW,LEFTW,IERR)
C
C---------------------------------------------------------------------
C  SUBROUTINE TO GET PARAMETERS, TIME-SERIES, AND CARRYOVER VALUES
C  FOR SCHEME/UTILITY #12 - FLASH BOARD SCHEME
C---------------------------------------------------------------------
C  ARGS:
...
C
      ...
      DIMENSION WORK(1)
C
      LOGICAL ENDFND,SMALL,GATE
C
C    ================================= RCS keyword statements ==========
      CHARACTER*68     RCSKW1,RCSKW2
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcinit_res/RCS/su1226.f,v $
     . $',                                                             '
     .$Id: su1226.f,v 1.1 1995/09/17 18:52:59 dws Exp $
     . $' /
C    ===================================================================
C
C
      DATA ENDSU/4HENDF,4HLASH,4H    /
      DATA BLANK/4H    /
C
...
C
C--------------------------------------------------------------------
C  NOW PROCESS INPUT UP TO 'ENDFLASH',LOOKING FOR, IN ORDER, KEYWORDS
...
C
C
      CALL PM1226(WORK,IUSEW,LEFTW,NPARXX,SMALL,GATE,GATMAX,
     .            LENDSU,JDEST,IERPM)
C
C
...
C
      CALL TS1226(WORK,IUSEW,LEFTW,NTSXX,SMALL,GATE,
     .            LENDSU,JDEST,IERTS)
C
C
...
C
      CALL CO1226(WORK,IUSEW,LEFTW,NCOXX,SMALL,GATE,GATMAX,
     .            LENDSU,JDEST,IERCO)
C
C
...
C
      ...
      CALL RFIL26(WORK,LOCCXX,COLOC)
C
C  STORE DEFAULT CARRYOVER VALUES FOR # LARGE BOARDS DOWN, # SMALL
C  BOARDS DOWN (IF ANY), GATE OPENING AND STATUS (IF GATE USED),
C  AND NON-SPILLWAY DISCHARGE (0.00).
C  FIRST VALUE IS 'FALSE' FLASHBOARD NEED CONDITION.
C
      CALL FLWK26(WORK,IUSEW,LEFTW,0.01,501)
      CALL FLWK26(WORK,IUSEW,LEFTW,0.01,501)
      NCOXX = NCOXX + 2
      IF (.NOT.SMALL) GO TO 940
      CALL FLWK26(WORK,IUSEW,LEFTW,0.01,501)
      NCOXX = NCOXX + 1
C
  940 CONTINUE
...
C
  950 CONTINUE
      CALL FLWK26(WORK,IUSEW,LEFTW,0.00,501)
      NCOXX = NCOXX + 1
      GO TO 9999
C
C---------------------------------------------------------------------
