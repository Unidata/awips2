...
C
C
C     DESC - PERFORMS THE QCSHIFT MOD
C
      SUBROUTINE MCSHFT(NCARDS,MODCRD,IIDATE,LLDATE,IHZERO)
C
      LOGICAL FIRST
C
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcst_mods/RCS/mcshft.f,v $
     . $',                                                             '
     .$Id: mcshft.f,v 1.2 1998/07/02 20:44:50 page Exp $
     . $' /
C    ===================================================================
C
C
      ...
      DATA ALLRC/'ALLRC   '/
C
      CALL FSTWHR('MCSHFT  ',0,OLDOPN,IOLDOP)
C
C     SET DEBUG SWITCH
C
      IBUG=IFBUG(4HMODS)+IFBUG(4HQCSH)
      IFLAG=0
C
      IF(IBUG.GT.0) THEN
         WRITE(IPR,10) IIDATE,LLDATE
10       FORMAT('0** IN MCSHFT - IDATE LDATE = ',I12,2X,I12)
         ENDIF
C
...
15    FORMAT('0**WARNING** THE ENDING DATE AND TIME IS BEFORE ',
     1 'OR AT THE BEGINNING DATE AND TIME.  THESE CHANGES ',
     2 'WILL BE IGNORED.',/,23X,'THE CURRENT MOD CARD IS: ',20A4)
20    FORMAT('0**WARNING** THE DATES FOR CHANGES IN THE ',
     1 'QCSHIFT MOD (',I2,1H/,I2,1H/,I4,1H-,I2,1X,A4,
...
C
      ...
      CALL FCONVT(4HCMS ,4HV   ,ENGUN,CFS2M,DFS2M,IERR)
C
...
70    FORMAT('0**WARNING** NO SUBSEQUENT CARDS FOUND FOR THE ',
     1  'QCSHIFT MOD.  PROCESSING CONTINUES')
...
90    FORMAT('0**WARNING** IN THE QCSHIFT MOD - NOT ENOUGH ',
...
160   FORMAT('0**WARNING** IN THE QCSHIFT MOD - AN INVALID ',
...
190   CONTINUE
C
C     CONVERT TO METRIC UNITS IF NEEDED
C
...
C
C     CHECK FOR BAD DISCHARGE VALUE
C
...
C
230   FORMAT('0**WARNING** IN QCSHIFT MOD - DISCHARGE VALUE ',
...
390   FORMAT(11X,'***LEAVING SUBROUTINE MCSHFT*** NUMRC=',I3)
