...
C
C
C     DESC - SUBROUTINE MUCBFR PERFORMS THE UCBFRATE MOD
C
C  <><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
C
      SUBROUTINE MUCBFR(NCARDS,MODCRD,P,MP,IDATE,ISTATT,IHZERO)
C
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcst_mods/RCS/mucbfr.f,v $
     . $',                                                             '
     .$Id: mucbfr.f,v 1.2 1998/07/02 20:51:52 page Exp $
     . $' /
C    ===================================================================
C
C
      DATA ISLASH/4H/   /,BLANK8/8H        /,UNITS/4HMI. ,4HKM. /
      DATA MODNAM/8HUCBASEF ,8HUCBFRATE/
C
      IMOD=2
C
      CALL FSTWHR(8HMUCBFR  ,0,OLDOPN,IOLDOP)
C
C     SET DEBUG SWITCH
C
      IBUG=IFBUG(4HMODS)+IFBUG(4HUCBF)
C
      IF(IBUG.GT.0)WRITE(IODBUG,900)MODNAM(IMOD),IDATE
  900 FORMAT(11X,'*** ENTERING SUBROUTINE MUCBAS, ',A8,'MOD ***  IDATE='
     1 ,I11)
C
      ISTATT=1
C
...
C               BY AREA OF UHG AS STORED IN P() (I.E., IN SQ KM)
...
C
C     LOOK FOR UNIT-HG OPERATIONS IN THIS SEGMENT WITH NAME OPNAME
C     IF OPNAME BLANK - CHANGE FOR ALL UNIT-HG OPERS IN THIS SEGMENT
C
C     READ NEXT FIELD - IF NONE CHANGE ALL UNIT-HG OPERS IN SEGMENT
...
C
   57 LOCP=0
      CALL FSERCH(2,OPNAME,LOCP,P,MP)
C
...
C
  ...
  605 FORMAT(1H0,10X,'**WARNING** A UNIT-HG OPERATION WITH NAME ',
...
C
      AREAUG=1.0
C
C    FIRST GET AREA OF UHG FROM P() - THIS AREA IN SQ KM
C
      AREAUG=P(LOCP+7)
C
...
C
      AREAUG=AREAUG/(1.609*1.609)
C
   19 P(LOCP+23)=VALUE*AREAUG
      ISTATT=0
C
      IF(IBUG.EQ.0)GO TO 1
      WRITE(IODBUG,902)MODNAM(IMOD),P(LOCP+23),OPNAME
  902 FORMAT(11X,'IN ',A8,'MOD - SUBROUTINE MUCBAS'/11X,
     1 'A VALUE OF ',F8.2,' HAS BEEN STORED IN THE P ARRAY OF ',
     2 'UNIT-HG OPERATION ',A8,' AS THE CONSTANT BASEFLOW IN CMS.')
      WRITE(IODBUG,903)AREAUG,UNITS(IXX)
  903 FORMAT(11X,'THE AREA REPRESENTED BY THE UHG IS ',F10.2,' SQ ',A4)
      GO TO 1
C
   20 CONTINUE
C
C     CHANGE ALL OCCURENCES OF UNIT-HG OPERATION IN THIS SEGMENT
C
      NCHNGE=0
      LOCP=1
   30 CALL FSERCH(2,OPN,LOCP,P,MP)
C
      IF(LOCP.EQ.0.AND.NCHNGE.GT.0)GO TO 1
      IF(LOCP.GT.0)GO TO 35
C
C     IF GET HERE HAVE NOT FOUND ANY UNIT-HG OPERATIONS IN THIS SEGMENT
C
      IF(MODWRN.EQ.1)
     .WRITE(IPR,606)MODNAM(IMOD)
  606 FORMAT(1H0,10X,'**WARNING** A ',A8,'MOD WAS REQUESTED FOR ',
     1 'THIS SEGMENT'/11X,'BUT NO UNIT-HG OPERATIONS WERE FOUND.')
      IF(MODWRN.EQ.1)CALL WARN
      GO TO 999
C
   35 CONTINUE
C
C     HAVE FOUND A UNIT-HG OPERATION - CHANGE P ARRAY
C
      AREAUG=1.0
C
C    FIRST GET AREA OF UHG FROM P() - THIS AREA IS IN SQ KM
C
      AREAUG=P(LOCP+7)
C
C  CONVERT TO SQUARE MI IF NEEDED
...
C
   39 NCHNGE=NCHNGE+1
      P(LOCP+23)=VALUE*AREAUG
      ISTATT=0
C
      IF(IBUG.EQ.0)GO TO 30
      WRITE(IODBUG,902)MODNAM(IMOD),P(LOCP+23),OPN
      WRITE(IODBUG,903)AREAUG,UNITS(IXX)
      GO TO 30
C
  999 CALL FSTWHR(OLDOPN,IOLDOP,OLDOPN,IOLDOP)
      IF(IBUG.GT.0)WRITE(IODBUG,842)MODNAM(IMOD)
  842 FORMAT(11X,'*** LEAVING SUBROUTINE MUCBAS, ',A8,'MOD')
C
      RETURN
      END
