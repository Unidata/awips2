...
C-----------------------------------------------------------------------
C
      SUBROUTINE TAB60 (TO, LEFT, IUSET, NXT, LPO, PO, LCO, TS, MTS,
     1                  LWORK, IDT)
C-----------------------------------------------------------------------
C   THIS IS THE OPERATIONS TABLE ENTRY ROUTINE FOR THE
C   TIDE ADJUSTMENT 'ADJUST-T' OPERATION.
C
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcinit_pntb/RCS/tab60.f,v $
     . $',                                                             '
     .$Id: tab60.f,v 1.1 1998/04/07 11:58:48 page Exp $
     . $' /
C    ===================================================================
C
C.......................................................................
C   DEBUG INITIALIZATION WITH TRACE LEVEL = 1
      CALL FPRBUG ('TAB60   ', 1, 60, IBUG)
C.......................................................................
...
C   SPACE IS AVAILABLE SO MAKE ENTRIES INTO TO().
      ...
      TO(4) = LCO
C
C   TIME INTERVAL OF ALL DWOPER TIME SERIES IS ALWAYS 1 HOUR
      IDELT = 1
C
...
C   TIME SERIES IN THE D().
      ...
      TO(6) = LD
C
C   TIME INTERVAL OF ALL THE TIDE TIME SERIES IS ALWAYS 24 HOUR
      IDELT = 24
C
C   CALL CKINPT AND GET THE LOCATION OF THE TIDE1 BALANCE
C   TIME SERIES IN THE D().
      CALL CKINPT (PO(26), PO(28), IDELT, LD, TS, MTS, IERR)
      TO(7) = LD
C
C   CALL CKINPT AND GET THE LOCATION OF THE TIDE2 BALANCE
C   TIME SERIES IN THE D().
      CALL CKINPT (PO(29), PO(31), IDELT, LD, TS, MTS, IERR)
      TO(8) = LD
C
C   CALL CKINPT AND GET THE LOCATION OF THE TIDE3 BALANCE
C   TIME SERIES IN THE D().
      CALL CKINPT (PO(32), PO(34), IDELT, LD, TS, MTS, IERR)
      TO(9) = LD
C
C   CALL CKINPT AND GET THE LOCATION OF THE TIDE4 BALANCE
C   TIME SERIES IN THE D().
      CALL CKINPT (PO(35), PO(37), IDELT, LD, TS, MTS, IERR)
      TO(10) = LD
C
C   TIME INTERVAL OF THIS DWOPER TIME SERIES IS ALWAYS 1 HOUR
      IDELT = 1
C
C   GET THE LOCATION OF THE DWOPER ADJUSTED STAGE TIME SERIES IN THE D()
...
C  CHECK IF DEBUG OUTPUT IS TO BE PRINTED.
 ...
 1000    FORMAT(1H0,'TIDEREV DEBUG - CONTENTS OF TO()=',11I6)
      END IF
C.......................................................................
C   THE TO ARRAY ENTRIES ARE AS FOLLOWS:
C   POS
C   ---
C    1      I.D. NUMBER OF THE OPERATION (60)
...
C   11      LOCATION OF DWOPER ADJUSTED STAGE TS IN D ARRAY
C.......................................................................
      RETURN
      END
