...
C @PROCESS LVL(77)
C
      SUBROUTINE HPTRDN (ITYPE,NWORDS,BUFFER,ISTAT)
C
C
C          ROUTINE:  HPTRDN
C
...
C
      ...
      INTEGER BUFFER(NWORDS),IARR(4)
C
C    ================================= RCS keyword statements ==========
      CHARACTER*68     RCSKW1,RCSKW2
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/db_hclrw/RCS/hptrdn.f,v $
     . $',                                                             '
     .$Id: hptrdn.f,v 1.1 1995/09/17 18:42:56 dws Exp $
     . $' /
C    ===================================================================
C
C
...
40    FORMAT (' **ERROR** IN HPTRDN. RECORD ',2A4,' HAS INVALID TYPE ',
...
C
      WRITE (LPE,70) NAME
70    FORMAT (' **ERROR** IN HPTRDN. ',2A4,' NOT PREVIOUSLY DEFINED')
      ISTAT=1
      GO TO 160
C
80    CONTINUE
...
C
C        UPDATE INDEX FOR REPLACE
C
      CALL UREADT (IXUNIT,IXREC,IARR,ISTA)
      IF (ISTA.NE.0) GO TO 140
      IARR(3)=ISREC
      CALL UWRITT(IXUNIT,IXREC,IARR,ISTA)
      IF (ISTA.NE.0) GO TO 140
C
