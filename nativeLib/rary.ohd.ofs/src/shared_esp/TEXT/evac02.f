C MEMBER EVAC02
C  (from old member EEVAC02)
C
      SUBROUTINE EVAC02(IDACC,IHACC,LDACC,LHACC,D,A,NDAYS,NHRS,TSID,
     * TYPE,IDT,NPDT,ITSCAL,IVALUE,CO,KODE,VALUE,DAVG,MDAVG)
C
C ......................................................................
C
C      ACCUMULATOR 02 (MIN MEAN DAILY VALUE) SUBROUTINE
C
C THIS IS THE ACCUMULATOR SUBROUTINE FOR FINDING THE MINIMUM MEAN
...
C     A = ACCUMULATOR ARRAY. A(1) IS MIN MEAN DAILY VAL, A(2)IS DAYS TO
C         MIN.
...
C KDAYS = COUNTER OF THE NUMBER OF DAYYS WE GO THRU.
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/shared_esp/RCS/evac02.f,v $
     . $',                                                             '
     .$Id: evac02.f,v 1.1 1995/09/17 19:19:18 dws Exp $
     . $' /
C    ===================================================================
C
C
      DATA MEAN,INST/4HMEAN,4HINST/
      DATA NAME,SBNAME/4HEACC,4HEVAC,4H02  /
      DATA INTL/4HINTL/
C
C
...
115   FORMAT(1H0,14HEVAC02 ENTERED)
      IBUG=IFBUG(NAME)
C
C CHECK A(1)
C
...
C OTHERWISE, A(1) IS SET = TO A VERY LARGE NUMBER(SO THAT IT LOSES
C OUT IN 1ST COMPARISON) AND SUM IS SET = TO 0.
C
...
210   FORMAT(/ 1H0,27HA(1) = MIN MEAN DAILY VALUE /
...
C     2. CHECK IF SUM IS LT A(1). IF SO, A(1)=SUM AND A(2)=KDAYS.
...
C     4. CHECK D/2 FOR POSSIBLE UNDERFLOWS.
C
C CO=SUM AT END
C
...
899   FORMAT(1H0,45H**NOTE** EVAC02 ENCOUNTERED MISSING DATA FOR:,
...
2100  FORMAT(// 1H0,10X,12H** OUTPUT ** / 1H0,29HMIN MEAN DAILY VALUE, A
     *(1) = ,F10.2 / 1H0,24HDAYS TO MINIMUM, A(2) = ,5X,F10.2 //
