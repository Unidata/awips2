...
/***********************************************************************
   ...
   PURPOSE
   In PRACTICE workstation mode. Delete old records in the VTECpractice table.  Purge products older than 
...
void PurgeVTECpractice(int backup_use)
{
   ...
   time(&checktime);
   printf("Begin purge of VTECpractice table at %s", asctime(gmtime(&checktime)));
   ...
   sprintf(where, " WHERE table_name = 'vtecpractice' ");
   ...
   /* determine the unique combinations in the VTECpractice table,
   ...
   ulistHead = LoadUnique("geoid||office_id||phenom||signif",
			  "VTECpractice", where, &ul_count);

   if (ulistHead== NULL )
   {
      printf("  no records in VTECpractice table\n\n");
      return;
   }
   else
   {
   ...
   while (ulistPtr)
   {
	 ...
	 of an infinite loop condition */
      ...
      sprintf(where, 
	      " WHERE producttime = "
	      " (SELECT MAX(producttime) from VTECpractice WHERE "
	      " geoid='%s'  AND office_id = '%s' AND "
	      " phenom='%s' AND signif='%s') ", 
	      geoid, office, phenom, signif);
      
      eventHead = GetVTECpractice(where);
...
*/
      ...
      status = DeleteVTECpractice(where);
   ...
   time(&checktime);
   printf("End purge of VTECpractice table at   %s", asctime(gmtime(&checktime)));
