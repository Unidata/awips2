...
void ShowSelectDS ( Widget w )
{
   ...
   if ( ( selectDS == NULL ) || ( ! XtIsManaged ( selectDS ) ) )
   {
      ...
      wmAtom = XmInternAtom(XtDisplay(selectDS), "WM_DELETE_WINDOW", False);
      XmAddWMProtocolCallback(selectDS, wmAtom, close_select, NULL);
      XtAddCallback(select_cancelPB, XmNactivateCallback, close_select, NULL);	  
      /* single click highlights gage. */
      XtAddCallback(selectLB, XmNbrowseSelectionCallback,
		    selectStationCallback, NULL);
      
      /* double click calls up graph. */
      XtAddCallback(selectLB, XmNdefaultActionCallback,
		    selectTimeSeriesCallback, NULL);
      
      
      /* check on the items typed into the search window. */
      XtAddCallback ( select_searchTxt , XmNmodifyVerifyCallback , lookup_lid , 
                      NULL ) ;		
      XtManageChild(selectFM);
      XtManageChild(selectDS);
      
      /* finally load the actual list */
      loadStationList ( getReportListHead() ) ;
   }
   
   /* if there is a station currently selected, then 
      select it in the list */
   
   highlight_status = get_highlight_flag ( ) ;

   if ( highlight_status == 1 )
   {
      pCurrentStation = get_current_station ( ) ;
      select_list_position ( pCurrentStation->lid ) ;	
   }
