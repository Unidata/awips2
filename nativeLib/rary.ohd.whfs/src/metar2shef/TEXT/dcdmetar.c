 /*
    ...
    an endless loop.        
*/
#include "metar.h"     /* standard header file */ /* dgb:09/16/01 */
/* #include <metar.h>  */   /* standard header file */
#define MAX_LOOP 30000                                      /* dgb:11/28/97 */ 
...
/*                 9/16/01 change  <metar.h> to "mear.h"            */
/*                                                                  */
/*                11/20/04 DGB: add 24DepthNewSnow                  */
/********************************************************************/
...
static char **SaveTokenString ( char *string , char *delimeters )
{
   ...
   while ( token[NDEX] != NULL )
   {
      ...
      if( TOKEN != NULL )
      {
         ...
         token[ NDEX ] = TOKEN;
      ii++;                                                /* dgb:11/28/97 */
      if ( ii > MAX_LOOP ) metar_error_handler("SaveTokenString"); /* dgb:11/28/97 */
   }
...
static void freeTokens( char **token )
{
   ...
   while( *(token+NDEX) != NULL )
   {
      ...
      NDEX++;
      ii++;
      if ( ii > MAX_LOOP )  metar_error_handler("freeTokens"); /* dgb:11/28/97 */
...
static void InitDcdMETAR( Decoded_METAR *Mptr )
{
   ...
   Mptr->DepthNewSnow = (float) MAXINT;                    /* dgb:12/26/96 */ 
   ...
   while( temp_token != NULL && (*next) < MAXWXSYMBOLS )
   {
      ...
      while( WxSymbols[i] != NULL ) {
         if( strncmp(temp_token, WxSymbols[i],
            ...
            break;
         jj++;
         if ( jj > MAX_LOOP ) metar_error_handler("isPresentWX"); /* dgb:11/28/97 */
      } 
      if( WxSymbols[i] == NULL ) {
      ...
      ii++;                                                /* dgb:11/28/97 */
      if ( ii > MAX_LOOP ) metar_error_handler("isPresentWX"); /* dgb:11/28/97 */
   }
...
printf("DcdMETAR:  token[%d] = %s\n",NDEX,token[NDEX]);
         ...
         case( RVR ):
            ...
            while (isRVR( token[NDEX], Mptr, &NDEX, ndex ) &&
               ...
               MetarGroup = presentWX;
               jj++;                                       /* dgb:11/28/97 */
               if ( jj > MAX_LOOP ) metar_error_handler("DcdMETAR"); /* dgb:11/28/97 */
            }
            break;
         case( presentWX ):
            ndex = 0;
...
/*          MetarGroup = SaveStartGroup;   */
      ...
      IS_NOT_RMKS = FALSE;
    ii++;                                                  /* dgb:11/28/97 */
    if ( ii > MAX_LOOP ) metar_error_handler("DcdMETAR");  /* dgb:11/28/97 */
   }
