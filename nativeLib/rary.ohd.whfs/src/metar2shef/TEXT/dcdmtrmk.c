/*
    ...
    05/08/97 DGB:
    in rfunction static bool isP24Precip
    change value to -9999 for group 7////

    11/13/97 DGB:
    In function isVsby2ndSite all "RY" references with 2 as 
    the character count in the strncmp functions were changed 
    to "RWY" and 3, respectively.  Provided by Carl McCalla.

    In function isSectorVsby added many new lines..search for
    11/13/97 in this part for changes.  Provided by Carl McCalla.
  
    In function DcdMTRmk changed line to read:
       Mptr->SFC_VSBY = (float) MAXINT
    Provided by Carl McCalla.
    ...
    12/14/97 DGB:
    Insert more descriptive comments when tracking down endless loop
    errors.  
    
    02/24/97 DGB:
    Fix endless loop problem in the isLTGfreq function...via 
    Carl McCalla.  Change keyword from 'CNS' to 'CONS'.  

                                           
    01/26/00 DGB:
    In function isVsby2ndSite, add check for NULL 
    pointer after bumping up token. 

    09/16/01 DGB:
    Change <metar.h> to "metar.h"

    09/15/05 RAE:
    Fix endless loop problem in the isPeakWind function. 
    Remove 'WND' logic placed there for Change 132 on metar_ver6.9 11/20/04.
    Included 'WND' as part of 'PK' and 'PKWND' logic.  
...
extern bool charcmp(char *str, char *pattern);
...
/*                 None.                                            */
/*                         1/28/96 DGB                              */
/*                         Add a check for a NULL token with the    */
/*                         2nd visibility group                     */
/*                                                                  */
/*                         11/20/04 DGB                             */
/*                         new functions for new 24 hour snow       */
/********************************************************************/
...
/*          printf("isTornadicActivity:  B_stringPtr != NULL"
         ...
         ii++;                                             /* dgb:11/28/97 */
         if ( ii > MAX_LOOP ) metar_error_handler("isTornadicActiv"); /* dgb:11/28/97 */
      }
                     ...
                     &(Mptr->PartialObscurationAmt[ndex][0]) ) == 0 )
         {
            ...
            while( phenom[i] != NULL ) {
               if( strcmp( *string, phenom[i] ) == 0 ) {
                  ...
                  i++;
               ii++;                                       /* dgb:11/28/97 */
               if ( ii > MAX_LOOP ) metar_error_handler("isPartObscur"); /* dgb:11/28/97 */
            }
...
/*  Modification History:                                           */
/*                 None.                                            */
/*                 01/26/00 Check for NULL pointer after            */
/*                          bumping up token.                       */
/*                                                                  */
/*                 12/17/03 Found another null after bumping        */
/*                          token..fixed                            */
/*                 01/16/04 Slightly revised the fix in 12/17/03    */
                           ...
                           int *NDEX)
{
                             ...
                             != NULL )
   {
                             ...
                             (slash-*token)))
      {
         ...
         if ( *(++token) == NULL )                         /* dgb:12/17/03 */
            return FALSE;                                  /* dgb:12/17/03 */

         /* the following line was changed from:
         if( strncmp( *(++token), "RWY", 3 ) == 0) {
	 to the one below...the token in bumped in the 
	 statement about and then checked for NULL..
	 when it was bumped in the *(++token) line
	 if it was null it would go into never land.
         */                                               /* dgb:01/16/04 */
...
bool static isLTGfreq( char **string, Decoded_METAR *Mptr, int *NDEX )
{
   ...
   if( strncmp( *string, "LTG", 3 ) != 0 ) {
      ...
      else if( strcmp( *string, "CONS" ) == 0 ) {    /* dgb:02/24/98 */
         Mptr->CNS_LTG = TRUE;
         LTG_FREQ_FLAG = TRUE;
      }
...
*/
      ...
      ii++;                                                /* dgb:11/28/97 */
      if ( ii > MAX_LOOP ) metar_error_handler("isRecentWX"); /* dgb:11/28/97 */
   }
      ...
      while( temp < numLoc && ndex < 6 ) {
         ...
         while( WxSymbols[i] != NULL && IS_NOT_FOUND ) {
            if( strncmp( WxSymbols[i], temp, strlen(WxSymbols[i]))
               ...
               IS_NOT_FOUND = FALSE;
            jj++;                                          /* dgb:11/28/97 */
            if ( jj > MAX_LOOP ) metar_error_handler("isRecentWX"); /* dgb:11/28/97 */
         }
...
static bool isVrbSky( char **string, Decoded_METAR *Mptr, int *NDEX )
{
   ...
   printf("isVrbSky: *string = %s cldPtr[%d] = %s\n",
      ...
      if( strncmp(*string, cldPtr[i], strlen(cldPtr[i])) != 0 )
         ...
         IS_NOT_FOUND = FALSE;
      ii++;                                                /* dgb:11/28/97 */
      if ( ii > MAX_LOOP ) metar_error_handler("isVrbSky"); /* dgb:11/28/97 */
   }
   ...
   while( *temp != '\0' ) {
      ...
      while( WxSymbols[i] != NULL && IS_NOT_FOUND ) {
         if( strncmp(temp,WxSymbols[i],strlen(WxSymbols[i])) != 0 )
            ...
            IS_NOT_FOUND = FALSE;
         jj++;                                             /* dgb:11/28/97 */
         if ( jj > MAX_LOOP ) metar_error_handler("isObscurAloft"); /* dgb:11/28/97 */
      }
      ...
      if( WxSymbols[i] == NULL ) {
         ...
         temp += strlen(WxSymbols[i]);
      ii++;                                                /* dgb:11/28/97 */
      if ( ii > MAX_LOOP ) metar_error_handler("isObscurAloft"); /* dgb:11/28/97 */
   }
...
/* dgb: 12/26/96 and function isDepthNewSnow */
static bool isDepthNewSnow( char *string,
                               Decoded_METAR *Mptr,
                               int *NDEX )
{
   ...
   if( strlen(string) != 6 )
      ...
      return FALSE;
   else if( strncmp(string, "931", 3) != 0 )
      return FALSE;
   else {
      Mptr->DepthNewSnow = ((float) atoi(string+3))/10.;
      (*NDEX)++;
      return TRUE;
   }
 
}
/* dgb: 11/20/04 isDepthNewSnow24 24/931sss nes snow in the past 24 hours mid to mid*/
static bool isDepthNewSnow24( char *string,
                               Decoded_METAR *Mptr,
                               int *NDEX )
{
   if( string == NULL )
   ...
   if( strlen(string) != 9 )
      return FALSE;
/*   else if( !(nisdigit(string,9)) )
      return FALSE;*/
      
   else if( strncmp(string, "24/931", 6) != 0 )
      return FALSE;
   else {

      Mptr->DepthNewSnow24 = ((float) atoi(string+6))/10.;
      (*NDEX)++;
      return TRUE;
   }
...
void DcdMTRmk( char **token, Decoded_METAR *Mptr )
{
   ...
   int TornadicActvty = 0, A0indicator = 0,
       ...
       DepthNewSnow = 0,        /* dgb: 12/26/96 */
       DepthNewSnow24 = 0,      /* dgb: 11/20/04 */
...
/*   printf("DcdMTRmk:  token[%d] = %s\n",NDEX,token[NDEX]); */
      ...
      if( strcmp(token[ NDEX ], "RMK") != 0 )
         ...
         IS_NOT_RMKS = FALSE;
      ii++;                                                /* dgb:11/28/97 */
      if ( ii > MAX_LOOP ) metar_error_handler("DcdMTRmk:locating start of RMK section"); /* dgb:12/14/97 */
   }
   ...
   while(token[NDEX] != NULL) {
      ...
      /* dgb:12/26/96 add DepthNewSnow */
      else if( isDepthNewSnow( token[NDEX], Mptr, &NDEX ) ) {
         DepthNewSnow++;
         if( DepthNewSnow > 1 )
            Mptr->DepthNewSnow = (float) MAXINT;
      }
      /* dgb:11/20/04 add 24/931xxx new snow in the past 24 hours mid to mid */
      else if( isDepthNewSnow24( token[NDEX], Mptr, &NDEX ) ) { /*dgb:11/20/04 */
         DepthNewSnow24++;                                     /*dgb:11/20/04 */
         if( DepthNewSnow24 > 1 )                              /*dgb:11/20/04 */
            Mptr->DepthNewSnow24 = (float) MAXINT;             /*dgb:11/20/04 */
      }
         ...
         NDEX++;
      ii++;                                                /* dgb:11/28/97 */
      if ( ii > MAX_LOOP ) metar_error_handler("DcdMTRmk:validating RMKs section groups"); /* dgb:12/14/97 */
   }
