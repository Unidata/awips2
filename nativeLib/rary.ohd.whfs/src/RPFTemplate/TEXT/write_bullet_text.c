...
/*********************************************************************
   ...
   PURPOSE
   Writes the product bullet text (after bulletstr: keyword) to the output product.
		       ...
		       FILE 		*outfile_ptr)
{
 ...
 /*    status = fputs("\n", outfile_ptr);*/
				    ...
				    &forced_nl_found)) != (char *)NULL)
   {
	 ...
	 {
	    /*do not fputs token if it is blank*/
	 ...
	 else
	 {
	    ...
	    else
	    {
	       ...
	       {
		  log_msg(EXCEED_STRING, "while writing bullet  output");
	       }
	    }	    
	 }
      }
	 ...
	 bcolumn = 1;
      ...
      {
         log_msg(FREE_NULL_MEMORY, "in write_bullet_text(), token");
      }
   }
   ...
   {
      log_msg(FREE_NULL_MEMORY, "in write_bullet_text(), phrase");
   }
