...
public:
    ...
    QPID_COMMON_EXTERN void setUpdatee(uint64_t _updatee);
    QPID_COMMON_EXTERN uint64_t getUpdatee() const;
    QPID_COMMON_EXTERN bool hasUpdatee() const;
    QPID_COMMON_EXTERN void clearUpdateeFlag();
    typedef void ResultType;

    template <class T> ResultType invoke(T& invocable) const {
        return invocable.updateOffer(getUpdatee());
    }
