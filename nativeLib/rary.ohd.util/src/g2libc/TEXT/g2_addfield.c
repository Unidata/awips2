...
#ifdef USE_JPEG2000
      else if (idrsnum == 40000) {      /*  JPEG2000 encoding   */
        if (ibmap == 255) {
...
#endif  /* USE_JPEG2000 */
#ifdef USE_PNG
      else if (idrsnum == 40010) {      /*  PNG encoding   */
        if (ibmap == 255) {
           getdim(cgrib+lpos3,&width,&height,&iscan);
           if ( width==0 || height==0 ) {
              width=ndpts;
...
{static char rcs_id1[] = "$Source: /fs/hseb/ob72/rfc/util/src/g2libc/RCS/g2_addfield.c,v $";
 static char rcs_id2[] = "$Id: g2_addfield.c,v 1.1 2004/09/16 17:42:10 dsa Exp $";}
/*  ===================================================  */
