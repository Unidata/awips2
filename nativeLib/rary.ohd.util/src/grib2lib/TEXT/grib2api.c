...
#include <string.h>
#include <math.h>
#include "grib2lib_inc/grib2api.h"
#include "grib2lib_inc/grib2.h"
#include "grib2lib_inc/scan.h"
#include "grib2lib_inc/tendian.h"
#include "grib2lib_inc/myassert.h"

/* Declare the external FORTRAN routine
 * gcc has two __ if there is one _ in the procedure name. */

#define UNPK_G2MDL unpk_g2mdl_
extern void UNPK_G2MDL
   (sInt4 * kfildo, sInt4 * jmin, sInt4 * lbit, sInt4 * nov, sInt4 * iwork,
   Float4 * ain, sInt4 * iain, sInt4 * nd2x3, sInt4 * idat, sInt4 * nidat,
   Float4 * rdat, sInt4 * nrdat, sInt4 * is0, sInt4 * ns0, sInt4 * is1,
   ...
   sInt4 * ipack, sInt4 * nd5, Float4 * xmissp, Float4 * xmisss,
   sInt4 * inew, sInt4 * iclean, sInt4 * l3264b, sInt4 * iendpk, sInt4 * jer,
   sInt4 * ndjer, sInt4 * kjer);

#define PK_G2MDL pk_g2mdl_
extern void PK_G2MDL
   (sInt4 * kfildo, sInt4 * jmax, sInt4 * jmin, sInt4 * lbit, sInt4 * nov,
   sInt4 * misslx, Float4 *a, sInt4 *ia, sInt4 * newbox, sInt4 *newboxp,
   Float4 * ain, sInt4 * iain, sInt4 * nx, sInt4 * ny, sInt4 * idat,
   sInt4 * nidat, Float4 * rdat, sInt4 * nrdat, sInt4 * is0, sInt4 * ns0,
   ...
   sInt4 * nd5, sInt4 * missp, Float4 * xmissp, sInt4 * misss,
   Float4 * xmisss, sInt4 * inew, sInt4 * minpk, sInt4 * iclean,
...
static int mdl_LocalUnpack (unsigned char *local, sInt4 locallen,
                            sInt4 * idat, sInt4 * nidat, Float4 * rdat,
                            sInt4 * nrdat)
{
   ...
   Float4 refVal;       /* The reference value in a given group. */
   ...
   int numUsed;         /* Number of bytes used in call to memBitRead. */
   unsigned char bufLoc; /* Where to read for more bits in the data stream. */
   unsigned long int uli_temp; /* Temporary storage to hold unpacked data. */
   ...
   for (i = 0; i < numGroup; i++) {
      ...
      MEMCPY_BIG (&refVal, local + 4, sizeof (Float4));
 ...
 *****************************************************************************
 */
static int TransferInt (Float4 * fld, sInt4 ngrdpts, sInt4 ibitmap,
                        sInt4 * bmap, char f_ignoreScan, sInt4 * scan,
                        sInt4 nx, sInt4 ny, sInt4 iclean, Float4 xmissp,
 ...
 *****************************************************************************
 */
static int TransferFloat (Float4 * fld, sInt4 ngrdpts, sInt4 ibitmap,
                          sInt4 * bmap, char f_ignoreScan, sInt4 * scan,
                          sInt4 nx, sInt4 ny, sInt4 iclean, Float4 xmissp,
                          Float4 * ain, sInt4 nd2x3, sInt4 * ib)
{
   int i;               /* loop counter over all grid points. */
   long int x, y;       /* Where we are in a grid of scan value 0100???? */
 ...
 *****************************************************************************
 */
void unpk_g2ncep (sInt4 * kfildo, Float4 * ain, sInt4 * iain, sInt4 * nd2x3,
                  ...
                  sInt4 * nd5, Float4 * xmissp, Float4 * xmisss,
                  ...
                  sInt4 * iendpk, sInt4 * jer, sInt4 * ndjer, sInt4 * kjer)
{
   ...
   Float4 f_temp;       /* Assist with handling weird MDL behavior in is5[] */
   ...
   *kjer = 8;
   ...
   memcpy (&f_temp, &(is5[11]), sizeof (Float4));
   is5[11] = (sInt4) f_temp;
   if ((is5[9] == 2) || (is5[9] == 3)) {
      if (is5[20] == 0) {
         memcpy (&(f_temp), &(is5[23]), sizeof (Float4));
         *xmissp = f_temp;
         is5[23] = (sInt4) f_temp;
         memcpy (&(f_temp), &(is5[27]), sizeof (Float4));
         *xmisss = f_temp;
         is5[27] = (sInt4) f_temp;
      } else {
         *xmissp = is5[23];
   ...
   f_ignoreScan = 0;
   if ((is5[20] == 1) &&
       ((is5[9] == 0) || (is5[9] == 1) || (is5[9] == 2) || (is5[9] == 3) ||
        (is5[9] == 40000) || (is5[9] == 40010))) {
 ...
 *****************************************************************************
 */
static int validate (char *filename, Float4 * ain, sInt4 * iain,
                     sInt4 * nd2x3, sInt4 * idat, sInt4 * nidat,
                     Float4 * rdat, sInt4 * nrdat, sInt4 * is0, sInt4 * ns0,
                     ...
                     Float4 * xmissp, Float4 * xmisss, sInt4 * iendpk,
 ...
 *****************************************************************************
 */
static void clear (Float4 * ain, sInt4 * iain, sInt4 * nd2x3, sInt4 * idat,
                   sInt4 * nidat, Float4 * rdat, sInt4 * nrdat, sInt4 * is0,
                   ...
                   sInt4 * ibitmap, Float4 * xmissp, Float4 * xmisss,
                   sInt4 * iendpk, sInt4 * jer, sInt4 * ndjer, sInt4 * kjer)
{
   memset ((void *) ain, 0, *nd2x3 * sizeof (Float4));
   memset ((void *) iain, 0, *nd2x3 * sizeof (sInt4));
   memset ((void *) idat, 0, *nidat * sizeof (sInt4));
   memset ((void *) rdat, 0, *nrdat * sizeof (Float4));
 ...
 *****************************************************************************
 */
static void BigByteCpy (sInt4 * dst, uInt4 * ipack, sInt4 nd5,
...
void unpk_grib2 (sInt4 * kfildo, Float4 * ain, sInt4 * iain, sInt4 * nd2x3,
                 ...
                 Float4 * xmissp, Float4 * xmisss, sInt4 * inew,
                 ...
                 sInt4 * ndjer, sInt4 * kjer)
{
   ...
   sInt4 * iwork;
   char f_useMDL = 1;
   char f_noBitmap;        /* 0 if bitmap, else no bitmap. */

   if (FindTemplateIDs (ipack, *nd5, 0, &gdsTmpl, &pdsTmpl, &drsTmpl,
                        &numGrps, &f_noBitmap) != 0) {
      jer[0 + *ndjer] = 2;
...
#ifdef LITTLE_ENDIAN
      memswp (ipack, sizeof (long int), *nd5);
#endif
...
/* gcc has two __ if there is one _ in the procedure name. */
void unpk_grib2__ (sInt4 * kfildo, Float4 * ain, sInt4 * iain,
                   sInt4 * nd2x3, sInt4 * idat, sInt4 * nidat, Float4 * rdat,
...
/*      a = (Float4 *) malloc ((*nx) * (*ny) * sizeof (Float4));*/
      ...
      PK_G2MDL (kfildo, jmax, jmin, lbit, nov, misslx, (Float4 *) ia, ia,
...
{static char rcs_id1[] = "$Source: /fs/hseb/ob72/rfc/util/src/grib2lib/RCS/grib2api.c,v $";
 static char rcs_id2[] = "$Id: grib2api.c,v 1.1 2004/09/16 17:30:40 dsa Exp $";}
/*  ===================================================  */
