...
{
   int i;               /* Counter for use when freeing Wx data. */
 ...
 *****************************************************************************
 */
static int ParseSect0 (long int *is0, long int ns0, long int grib_len,
                       grib_MetaData * meta)
{
   if (ns0 < 9) {
      return -1;
 ...
 *****************************************************************************
 */
static int ParseSect1 (long int *is1, long int ns1, grib_MetaData * meta)
{
   if (ns1 < 21) {
      return -1;
   }
   ...
   meta->pds2.center = (unsigned short int) is1[5];
   meta->pds2.subcenter = (unsigned short int) is1[7];
   meta->pds2.mstrVersion = (uChar) is1[9];
   meta->pds2.lclVersion = (uChar) is1[10];
   if (((meta->pds2.mstrVersion != 1) && (meta->pds2.mstrVersion != 2)) ||
       (meta->pds2.lclVersion > 1)) {
      errSprintf ("Master table version supported (1,2) yours is %d... "
 ...
 *****************************************************************************
 */
static int ParseSect2_Wx (float *rdat, long int nrdat, long int *idat,
                          long int nidat, sect2_WxType * Wx, int simpVer)
{
   int loc;             /* Where we currently are in idat. */
   int groupLen;        /* Length of current group in idat. */
 ...
 *****************************************************************************
 */
static int ParseSect2_Unknown (float *rdat, long int nrdat, long int *idat,
                               long int nidat, grib_MetaData * meta)
{
   /* Used for easier access to answer. */
   int loc;             /* Where we currently are in idat. */
   int ansLoc;          /* Where we are in the answer data structure. */
   long int groupLen;   /* Length of current group in idat. */
   int j;               /* Counter over the length of the current group. */
 ...
 *****************************************************************************
 */
static int ParseSect3 (long int *is3, long int ns3, grib_MetaData * meta)
{
   double unit;         /* Used to convert from stored value to degrees
                         * lat/lon. See GRIB2 Regulation 92.1.6 */
   long int angle;      /* For Lat/Lon, 92.1.6 may not hold, in which case,
                         * angle != 0, and unit = angle/subdivision. */
   long int subdivision; /* see angle explaination. */
   ...
   switch (is3[14]) {
      ...
      case 1:
          ...
          * on: Y * 10^D = R, where Y = original value, D = scale factor, R
          * = scale value. */
 ...
 *
 * PURPOSE
 *    Attempt to parse time data in units provided by table 4.4, to seconds.
 *
 * ARGUMENTS
...
int ParseSect4Time2sec (long int time, int unit, double *ans)
{
   ...
   static long int unit2sec[] = { 60, 3600, 86400L, 0, 0, 0, 0, 0,
      0, 0, 0, 10800, 21600, 43200L, 1
   };
   if ((unit >= 0) && (unit < 15)) {
      if (unit2sec[unit] != 0) {
 ...
 *****************************************************************************
 */
static int ParseSect4 (long int *is4, long int ns4, grib_MetaData * meta)
{
   ...
   if ((is4[7] != GS4_ANALYSIS) && (is4[7] != GS4_ENSEMBLE) &&
       (is4[7] != GS4_DERIVED) && (is4[7] != GS4_STATISTIC) &&
       (is4[7] != GS4_PROBABILITY) && (is4[7] != GS4_SATELLITE)) {
   ...
   if ((is4[24] == GRIB2MISSING_4) || (is4[23] == GRIB2MISSING_1)) {
      meta->pds2.sect4.fstSurfScale = GRIB2MISSING_1;
   ...
   if ((is4[30] == GRIB2MISSING_4) || (is4[29] == GRIB2MISSING_1)) {
      meta->pds2.sect4.sndSurfScale = GRIB2MISSING_1;
   ...
   switch (meta->pds2.sect4.templat) {
      ...
      case GS4_STATISTIC: /* 4.8 */
         if (ParseTime (&(meta->pds2.sect4.validTime), is4[34], is4[36],
            ...
            preErrSprintf ("Error in call to ParseTime from ParseSect4"
                           " (GRIB2)");
            return -1;
         }
         ...
         for (i = 0; i < meta->pds2.sect4.numInterval; i++) {
            meta->pds2.sect4.Interval[i].processID =
               (uChar) is4[46 + i * 12];
            meta->pds2.sect4.Interval[i].incrType = (uChar) is4[47 + i * 12];
            meta->pds2.sect4.Interval[i].timeRangeUnit =
               (uChar) is4[48 + i * 12];
            meta->pds2.sect4.Interval[i].lenTime = is4[49 + i * 12];
            meta->pds2.sect4.Interval[i].incrUnit = (uChar) is4[53 + i * 12];
            meta->pds2.sect4.Interval[i].timeIncr = (uChar) is4[54 + i * 12];
         }
         break;
      case GS4_PROBABILITY: /* 4.9 */
         ...
         if (ParseTime (&(meta->pds2.sect4.validTime), is4[47], is4[49],
            ...
            preErrSprintf
               ("Error in call to ParseTime from ParseSect4 (GRIB2)");
            return -1;
         }
 ...
 *****************************************************************************
 */
static int ParseSect5 (long int *is5, long int ns5, grib_MetaData * meta,
                       float xmissp, float xmisss)
{
   ...
   meta->gridAttrib.missPrim = xmissp;
   ...
   if ((is5[9] == GS5_JPEG2000) || (is5[9] == GS5_PNG)) {
      meta->gridAttrib.missManage = 0;
      return 0;
   }
   if (meta->gridAttrib.packType == 0) {
      ...
      if (is5[22] > 2) {
         errSprintf ("Invalid missing management type, missManage = %ld\n",
                     is5[22]);
         return -2;
      }
      meta->gridAttrib.missManage = (uChar) is5[22];
   }
   return 0;
}
               ...
               float xmissp, float xmisss, int simpVer)
{
   ...
   if (meta->pds2.sect4.numInterval > 0) {
      ParseElemName (meta->pds2.center, meta->pds2.subcenter,
                     meta->pds2.prodType, meta->pds2.sect4.templat,
                     meta->pds2.sect4.cat, meta->pds2.sect4.subcat,
                     meta->pds2.sect4.Interval[0].lenTime,
                     &element, &comment, &unitName, &(meta->convert));
   } else {
      ParseElemName (meta->pds2.center, meta->pds2.subcenter,
                     meta->pds2.prodType, meta->pds2.sect4.templat,
                     meta->pds2.sect4.cat, meta->pds2.sect4.subcat, 0,
                     &element, &comment, &unitName, &(meta->convert));
   }
   if (strcmp (element, "") == 0) {
   ...
   if (meta->pds2.sect4.sndSurfScale == GRIB2MISSING_1) {
      ParseLevelName (meta->pds2.center, meta->pds2.subcenter,
                      meta->pds2.sect4.fstSurfType,
                      meta->pds2.sect4.fstSurfValue, 0, 9999.,
   ...
   for (y = 0; y < subNy; y++) {
      if (((startY + y - 1) < 0) || ((startY + y - 1) >= Ny)) {
         ...
         if (attrib->fieldType) {
            ...
            ftemp = ((Float4 *) iain) + (startY + y - 1) * Nx + (startX - 1);
         }
         for (x = 0; x < subNx; x++) {
            if (((startX + x - 1) < 0) || ((startX + x - 1) >= Nx)) {
               *grib_Data++ = 9999;
               ...
               if (f_wxType) {
                  index = (long int) value;
                  if ((index >= 0) && (index < WxType->dataLen)) {
...
static void ParseGridPrimMiss (gridAttribType * attrib, double *grib_Data,
                               long int Nx, long int Ny, long int *iain,
                               double unitM, double unitB, long int *missCnt,
                  ...
                  if (f_wxType) {
                     ...
                     if ((index >= 0) && (index < WxType->dataLen)) {
                        if (WxType->ugly[index].f_valid) {
                           ...
                           /* Table is not valid here so set value to
                            * missPrim */
                           value = attrib->missPrim;
                           (*missCnt)++;
                        }
                     }
                  }
                  if ((!f_wxType) || (value != attrib->missPrim)) {
                     if (f_maxmin) {
                        if (value < attrib->min) {
                           attrib->min = value;
                        } else if (value > attrib->max) {
         ...
         for (x = 0; x < subNx; x++) {
            if (((startX + x - 1) < 0) || ((startX + x - 1) >= Nx)) {
               *grib_Data++ = attrib->missPrim;
               ...
               /* Make sure value is not a missing value when converting
                * units, and while computing max/min. */
               if ((value == attrib->missPrim) || (value == attrib->missSec)) {
                  (*missCnt)++;
               } else {
                  /* Convert the units. */
                  value = unitM * value + unitB;
...
void ParseGrid (gridAttribType * attrib, double **Grib_Data,
                ...
                uChar f_subGrid, int startX, int startY, int stopX,
                int stopY)
{
   ...
   uChar f_readjust;    /* True if unit conversion caused confusion over
                         * missing values. */
   long int scanIndex;  /* Where we are in the original grid. */
   long int x, y;       /* Where we are in a grid of scan value 0100 */
   long int newIndex;   /* x,y in a 1 dimensional array. */
   double value;        /* The data in the new units. */
   double *grib_Data;   /* A pointer to Grib_Data for ease of manipulation. */
   long int missCnt = 0; /* Number of detected missing values. */
   long int index;      /* Current index into Wx table. */
   Float4 *ain = (Float4 *) iain;
   long int subNx;      /* The Nx dimmension of the subgrid. */
   long int subNy;      /* The Ny dimmension of the subgrid. */
   ...
   if (scan == 64) {
       ...
       * would need a different unpacker library, which is extremely
       * unlikely. */
      for (scanIndex = 0; scanIndex < Nx * Ny; scanIndex++) {
         ...
         /* Make sure value is not a missing value when converting units,
          * and while computing max/min. */
         if ((attrib->missManage == 0) ||
             ((attrib->missManage == 1) && (value != attrib->missPrim)) ||
             ((attrib->missManage == 2) && (value != attrib->missPrim) &&
            ...
            if (f_wxType) {
               ...
               if ((index >= 0) && (index < WxType->dataLen)) {
                  if (WxType->ugly[index].f_valid == 1) {
                     ...
                     /* Table is not valid here so set value to missPrim */
                     if (attrib->missManage != 0) {
                        value = attrib->missPrim;
                        missCnt++;
                     } else {
            ...
            if ((!f_wxType) ||
                ((attrib->missManage == 0) || (value != attrib->missPrim))) {
               if (attrib->f_maxmin) {
                  if (value < attrib->min) {
                     attrib->min = value;
                  } else if (value > attrib->max) {
   ...
   f_readjust = 0;
   xmissp = attrib->missPrim;
   xmisss = attrib->missSec;
   if (attrib->f_maxmin) {
      if ((attrib->missManage == 1) || (attrib->missManage == 2)) {
         if ((attrib->missPrim >= attrib->min) &&
             (attrib->missPrim <= attrib->max)) {
            xmissp = attrib->max + 1;
            f_readjust = 1;
         }
   ...
   if (f_readjust) {
      for (scanIndex = 0; scanIndex < Nx * Ny; scanIndex++) {
         ...
         } else if ((attrib->missManage == 2) && (value == attrib->missSec)) {
            grib_Data[newIndex] = xmisss;
         }
      }
      attrib->missPrim = xmissp;
      if (attrib->missManage == 2) {
   ...
   attrib->numMissing = missCnt;

/*  ==============  Statements containing RCS keywords:  */
{static char rcs_id1[] = "$Source: /fs/hseb/ob72/rfc/util/src/degrib/RCS/metaparse.c,v $";
 static char rcs_id2[] = "$Id: metaparse.c,v 1.1 2004/09/16 17:14:09 dsa Exp $";}
/*  ===================================================  */
