...
c
      ...
      character*132 strng
c
      dimension mdy(6),igds(18)
      parameter (mkptr=20)
      dimension kptr(mkptr)
      parameter (mkpds=25)
      dimension kpds(mkpds)
      parameter (mkgds=20)
      dimension kgds(mkgds)
C
C    ================================= RCS keyword statements ==========
      CHARACTER*68     RCSKW1,RCSKW2
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob81/ohd/pproc/src/gribit_main/RCS/gbitmain.f,v $
     . $',                                                      '
     .$Id: gbitmain.f,v 1.1 2006/10/19 16:06:04 dsa Exp $
     . $' /
C    ===================================================================
C
C
...
10    format (' PROGRAM Gribit (VERSION: OB7.2 - 06/06/2006)' //
...
c  read xmrg format file
      call rdxmrg (xmrgpath,iuxmrg,iupr,xver,mwcol,msrow,ncol,nrow,
...
210   format (' ERROR: wfld and ibmap arrays need ',i7,
     +        ' words but ',i6,' are available.')
          go to 250
        endif
c
...
c  select grid and transform (if needed)
      nwarn = 0
      call griddef (iutw,iupr,igdbug,ngrid,inat,
     +              mwcol,msrow,ncol,nrow,
     +              fld,wfld,igds,mxbmap,ibmap,nwarn,jerr)
...
230   format (' NOTE: wrote GRIB file ',a /
     +        7x,'from  XMRG file ',a /
     +        7x,'with  COMMS HEADER of ''',a4,a3,a4,1x,3i2,
...
330   if (cvgrib.eq.'1') then
         call ungrib (iupr,igdbug,mbuf,kbuf,mfld,fld,
     +                mifld,ifld,kptr,kpds,kgds,kbms,
     +                mwcol,msrow,ncol,nrow,ngrib,jerr)
         else
            call ungrib2 (iutr,iutw,iupr,iud,igdbug,ipack,fld,jerr)
         endif
ccc      write (*,*) 'jerr=',jerr
