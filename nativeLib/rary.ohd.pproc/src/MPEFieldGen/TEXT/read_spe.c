                ...
                int * spe_status )
{
   ...
   * spe_status = 1 ;
   ...
   /* Initialize the satellite precipitation array to missing. */
   for ( j = 0; j < pGeoData->num_rows; ++j )
   {
      for ( i = 0; i < pGeoData->num_cols; ++i )
      {
          pSatPre [j][i] = MOSAIC_DEFAULT ;
      }
   }
   ...
   yoffset = conus_ysize - ( pGeoData->hrap_y ) ;

   for(iy=0; iy < (pGeoData->num_rows); iy++)
   {
      for(ix=0; ix < (pGeoData->num_cols); ix++)
      {
    	 indconus = conus_xsize*(yoffset - iy) + (ix + (pGeoData->hrap_x - 1));
         if(((pGeoData->hrap_y) + iy) <= conus_ysize &&
            ((pGeoData->hrap_x) + ix) <= conus_xsize)
        {
