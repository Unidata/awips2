                ...
                double * cvr, int * errFlag)
{
    ...
    for(i = 0; i < rowSize; i ++)
    {
        for(j = 0; j < colSize; j ++)
        {
            ...
            **/
            if(rad[i][j] < rain_min) continue ;

            /**
             * throw out values subject to beam blockage
             **/
            if(mis[i][j] == 0) continue ;
...
*
*******************************************************************/
void MPEFieldGen_find_nbrsX(const int gageSize ,
                short * iug ,
                short * ivg , 
                const int iu0 ,
                const int iv0 ,
        ...
        for(i0 = 1; i0 <= colSize; i0 ++)
        {
            ...
            else
            {
                ...
                for(k = 1; k <= nl; k ++)
                {
                      i = (i0-1) + il[k-1] ;
                    if(i < 1) continue ;
                    if(i > colSize) continue ;
                    j = j0 + jl[k-1] ;

                    if(j < 1) continue ;
                    if(j > rowSize) continue ;
            ...
            int * num , int * idis , int * jdis)
{
    ...
    for(i = 1; i <= index; i++)
    {
     ...
     MPEFieldGen_heapSortForDoubleAndGeoIndex(dist, idis, jdis, count) ;
            ...
            int * ising)
{
    ...
    MPEFieldGen_ksol(1, neq, 1, a, r,s,ising) ;
...
{static char rcs_id1[] = "$Source: /fs/hseb/ob83/ohd/pproc_lib/src/MPEFieldGen/RCS/statistics.c,v $";
 static char rcs_id2[] = "$Id: statistics.c,v 1.1 2007/10/15 12:19:16 dsa Exp lawrence $";}
/*  ===================================================  */
