...
short
get_intersection_point (double *a1x, double *a1y, double *a2x, double *a2y,
...
short
compute_adjusted_zval (p3_gage_struct * c, P3_CALIBRATE_DATA * cal,
		       TRIANGLE * t,
		       long triidx,
		       double *x, double *y, double *z, double *adj_z)
{
   ...
   if (point_def_count == 3)
   {
	 ...
	 *adj_z = PA.value;
	 return (0);
      ...
      get_intersection_point (&PB.lon, &PB.lat, &PC.lon, &PC.lat,
			      &PA.lon, &PA.lat, &*x, &*y,
			      &x_b_to_c, &y_b_to_c);
	 ...
	 weight_a = (*y - y_b_to_c) / (PA.lat - y_b_to_c);
      ...
      weight_b *= (1 - weight_a);
      weight_c *= (1 - weight_a);
      compute_ratio_and_diff_at_point (PA.value,
				       cal[t[triidx].a].other_dataset_zval,
				       Max_ratio, &ratio_a, &diff_a);
      compute_ratio_and_diff_at_point (PB.value,
				       cal[t[triidx].b].other_dataset_zval,
				       Max_ratio, &ratio_b, &diff_b);
      compute_ratio_and_diff_at_point (PC.value,
				       cal[t[triidx].c].other_dataset_zval,
				       Max_ratio, &ratio_c, &diff_c);
      *adj_z =
	 (*z *
   ...
   if (point_def_count == 2)
   {
	 ...
	 get_intersection_point (&PA.lon, &PA.lat, &PB.lon, &PB.lat,
				 &PC.lon, &PC.lat, &*x, &*y,
				 &x_b_to_c, &y_b_to_c);
	 ...
	 *adj_z =
	 ...
	 /*/printf("\nloc2c %15.5lf %15.5lf", weight_b, weight_c); */
      ...
      {
	 /*/ compute the distance between point a and (x,y) */
	 get_intersection_point (&PC.lon, &PC.lat, &PA.lon, &PA.lat,
				 &PB.lon, &PB.lat, &*x, &*y,
				 &x_b_to_c, &y_b_to_c);
...
short
compute_ratio_and_diff_at_point (double rain_z, double radar_z,
...
short
contour_data_sort_x (p3_gage_struct * c, long numpnt, long *sortx)
{
...
short
closest_pnt_from_array (p3_gage_struct * c, long numpnt, long *sortx,
...
short
point_below_line (double *x, double *y,
...
short
pnt_in_tri (p3_gage_struct * con, TRIANGLE * tri, long triidx, DPOINTL * dptl)
{
	/***********************/
   short count = 0;
...
TRIPNT *
cross_reference_triangles (p3_gage_struct * c, long numpnt, TRIANGLE * t,
			   long numtri)
{
   long i;
   TRIPNT *tripnt = NULL;
...
void
calc_z_value_with_known_triangle (p3_gage_struct * c,
				  TRIANGLE * t,
...
long
determine_dataset_triangle (DPOINTL * pnt,
			    ...
			    TRIANGLE * tri, long numtri)
{
   ...
   if (pnt_in_tri (c, tri, last_triangle, pnt))
   {
   ...
   closest_pnt_from_array (c, numpnt, sortx, &pnt->lon, &pnt->lat,
   ...
   {
      if (pnt_in_tri (c, tri, tripnt[closest_point].tri_pnt[i], pnt))
      {
