	                  ...
	                  int draw_only_persistent )
{
   ...
   for ( i = 0; i < pGeoData->num_rows; ++i )
   {
      ...
      {
         value = (short int) (real_mosaic[i][j] * scale_factor) ;
   ...
   /* Copy the short array to the double output array. */
   for ( i = 0; i < pGeoData->num_rows; ++i )
   {
      for ( j = 0; j < pGeoData->num_cols; ++j )
      {
...
bool eliminateP3Gages ( int hrapx, int hrapy, enum DisplayFieldData field )
{
   return true;
}
