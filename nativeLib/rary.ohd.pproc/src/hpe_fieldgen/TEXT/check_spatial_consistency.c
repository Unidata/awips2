...
void hpe_fieldgen_sort (double * data, const int size, int * index) ;

void subCheckSpatialConsistency(const gage_table_struct * pGageTable,
                            ...
                            int * gageqc)
{
    ...
    int factor = pMPEParams->hrap_grid_factor;
    ...
    /*
     * Consider the HRAP/Quarter HRAP grid
     * 
     * modified by guoxian zhou 11/2006
     */
    /*
    HrapToLatLong(row, col, &lat, &lon);
    */

    ScaledHrapToLatLong(row, col, factor, &lat, &lon);
    ...
    */
    ...
    if(first == 1)
    {
        if(hpe_fieldgen_getAppsDefaults("mpe_sccqc_threshold", tokenvalue) == -1)
        {
    ...
    subCheckSpatialConsistency(pGageTable, minLatitude, maxLatitude, 
           ...
           int * qc_flag)
{
    ...
    hpe_fieldgen_sort (data, arraySize, stnid) ;
    ...
    {
        p75 = (data[n1+1] + data[n1]) / 2.0 ;
        p25 = (data[n2+1] + data[n2]) / 2.0 ;
    }
...
void hpe_fieldgen_sort (double * data, const int size, int * stnid)
{
    int i, j, itemp;
    double temp ;
    ...
    for(j = 1; j < intlat; j++)
    {
        for(i = 1; i < intlon; i++)
        {
            ...
            if(count >= 1)
            {
                if(count < 5)
                {
                    stat1(stndt, count, stnid, threshold, stndtflg) ;
                }
                else
                {
                    stat2(stndt, count, stnid, threshold, stndtflg) ;
                }
