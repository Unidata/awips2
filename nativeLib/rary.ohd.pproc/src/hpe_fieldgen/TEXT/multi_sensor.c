...
*   Dec 11, 2006   Guoxian Zhou      using dynamic memory allocation
*                                    for arrays to avoid memory limit 
*                                    in stack
*
***********************************************************************/
...
static int ** ndgag = NULL ;
static int ** npgag = NULL ;
static int ** ndata = NULL ;
static int ** nposi = NULL ;
static double ** gag = NULL ;


static void initMultiSensorArray(const geo_data_struct * pGeoData) ;

static void freeMultiSensorArray(const geo_data_struct * pGeoData) ;

void multi_sensor(const geo_data_struct * pGeoData ,
...
*/
    ...
    double cvr = 0.0 ;
    coef_var_r(rowSize, colSize, rad, mis, rain_min, &cvr, errFlag) ;
    ...
    cor_scale_r(rowSize, colSize, rad, mis, &rngi, &rngc, errFlag) ;
    ...
    find_indices(iradi,&n,idis,jdis) ;
    ...
    int nfill = 0 ;
    local_stat(rowSize, colSize, 2, iradi, rad, 
        mis, &nfill, ndata, nposi) ;

    int nfgag = 0.0 ;
    if(isrch_g == 2)
    {
        local_stat(rowSize, colSize, 1, iradi, gag,
    ...
    buildNeighborList (pGeoData , pMPEParams,
                 gageSize, iug,  ivg, zg ) ;
        ...
        for(j = 0; j < rowSize; j++)
        {
            ...
            else
            {
                ...
                if(ng > 0)
                {
                    ...
                    srch_nbrs(rowSize, colSize, 1, n, idis, jdis, i, j,
            ...
            else
            {
                ...
                srch_nbrs(rowSize, colSize, 2, n, idis, jdis, i, j,
            ...
            if(nbrs_r == 0)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        ms_soe(rowSize, colSize, isrch_g, cvr, f0_g,
            ...
            else
            {
                ...
                if(nbrs_g == 0)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            ms_soe(rowSize, colSize, isrch_g, cvr, f0_r, 
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        ms_soe(rowSize, colSize, isrch_g, cvr, f0, nbrs_g,
...
void ms_soe(const int rowSize , const int colSize , 
            ...
            short * ilist_g, double * est)
{
    ...
    b = init1DDoubleArray(ZERO_CONSTANT, ndim);
    bi = init1DDoubleArray(ZERO_CONSTANT, ndim);
    w = init1DDoubleArray(ZERO_CONSTANT, ndim);
    cov = init2DDoubleArray(ZERO_CONSTANT, ndim, ndim);
    covi = init2DDoubleArray(ZERO_CONSTANT, ndim, ndim);
    ...
    if (f0 < 1.0)
    {
        ...
        covmatms(isrch_g, 1.0, 1.0, nbrs_g, nbrs_r, dist_min, 
            cor0pig, cor0ig, rngig, cor0pic, cor0ic, rngic, 
        ...
        covvecms(isrch_g, i, j, 1.0, 1.0, nbrs_g, nbrs_r,
    ...
    **/
    ...
    lsolve(nbrs_g + nbrs_r + 1, cov, b, &errFlag);
    ...
    **/ 
    ...
    if(errFlag != 0)
    {
	    free2DDoubleArray(cov, ndim );
	    free2DDoubleArray(covi, ndim );
	    free1DDoubleArray( b );
	    free1DDoubleArray( bi );
	    free1DDoubleArray( w );
...
void covmatms(const int isrch_g, const double varg,
...
void covvecms(const int isrch_g, const int iu0,
...
void cor_scale_r(const double rowSize , const double colSize ,
                double ** rad, int ** mis, 
...
static void initMultiSensorArray(const geo_data_struct * pGeoData)
{
    ...
    ndgag = init2DIntArray(ZERO_CONSTANT, rowSize, colSize );

    npgag = init2DIntArray(ZERO_CONSTANT, rowSize, colSize );

    ndata = init2DIntArray(ZERO_CONSTANT, rowSize, colSize );

    nposi = init2DIntArray(ZERO_CONSTANT, rowSize, colSize );

    gag = init2DDoubleArray(MOSAIC_DEFAULT, rowSize, colSize );
...
static void freeMultiSensorArray(const geo_data_struct * pGeoData) 
{
    ...
    free2DIntArray(ndgag, rowSize );

    free2DIntArray(npgag, rowSize );

    free2DIntArray(ndata, rowSize );

    free2DIntArray(nposi, rowSize );

    free2DDoubleArray(gag, rowSize );
}
