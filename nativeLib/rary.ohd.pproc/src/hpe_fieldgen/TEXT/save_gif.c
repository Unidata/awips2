...
typedef int Colors;
float  dfltclvls[] = {0.00,0.01,0.10,0.20,0.30,0.40,0.50,0.75,1.00,1.25,1.50,1.75,2.0,2.5,3.0};
					...
					long int *irc)
{
  ...
  char                  filename[128] = {'\0'};
  char                  datestamp[128] = {'\0'}, timestamp[128] = {'\0'};
  char                  legendlbl[6] = {'\0'}, buf[8] = {'\0'};
  char                  dates[11] = {'\0'};
...
/* bug here: Array Bounds Read
 * the dfltclvls array has only 15 elements
 * gzhou -- 05/03/2007
 * 
   for(i=0;i<16;i++) clvls[i] = dfltclvls[i];
*/
   for(i=0;i<15;i++) clvls[i] = dfltclvls[i];
   nlvls=i-1;
...
void rfcw_load_static(const int hrap_grid_factor, int *irc_load_stat)
{
...
*/
  ...
  get_apps_defaults("geo_st3_ascii",&len,geoadir,&len);
   ...
   while(!feof(in_file)) 
   {
    ...
    /*
    LatLongToHrap ( r_lat, r_lon, & row, & col);
    */

    LatLongToScaledHrap(r_lat, r_lon,
                        hrap_grid_factor,
                        &row , &col );
