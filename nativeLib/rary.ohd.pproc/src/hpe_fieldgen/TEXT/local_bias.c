                        ...
                        const char * filename, int num_seq )
{
   ...
   int k;
   int status = 0;
   local_bias_values_record local_bias_records [ num_seq ];
...
void find_array_indices( local_bias_params * pLocalBiasParams )
{
...
/**
  ...
  **/
   ...
   qksort32 ( n, dist, pLocalBiasParams->idis, pLocalBiasParams->jdis );
...
void find_nbrs1X ( const gage_radar_pair_table_struct * pGageRadarPair,
                   int * k,
                   int iradi,
                   int iu0, 
                   int iv0, 
...
static void setupLocalBiasParams( )
{
   ...
   if ( mult > 1 )
   {
      find_array_indices( & localBiasParams );
   }
...

{
   ...
   double sum;
   double w;
   float locbias_temp [ pGeoData->num_rows ] [ pGeoData->num_cols ];
   int i;
   int iu0;
   int iv0;
   int j;
   int k;
   int ndat;
   short locspan_temp [ pGeoData->num_rows ] [ pGeoData->num_cols ];
   short iu [ MAXGAGS ];
   short iv [ MAXGAGS ];
   short iz [ MAXGAGS ];
   float rz [ MAXGAGS ];
   ...
   sprintf ( message, "into interpolate_nearby nx=%d, ny=%d, nbory=%d, "
                      "ndat=%d", pGeoData->num_cols, pGeoData->num_rows, 
...
void find_nbrs1 ( int n, short * iu, short * iv, int iu0, int iv0, 
       ...
       else if ( i0 == n - 1 )
       {
        ...
        else
        {
           ...
           for ( i = 0; i <= i0; ++i )
           {
              ...
              if ( m >= NX1 )
              {
                 sprintf ( message, "m gt nx1...stop and reset nx1 to"
                                    " a larger value" );
                 hpe_fieldgen_printMessage( message ); 
                 return;
              }
                                     ...
                                     float dist )
{
   ...
   double sum;
   double w;
   float bias_loc_temp [pGeoData->num_rows] [pGeoData->num_cols];
   float rlist[NDATX];
   float rz[NDATX];
   int iradi;
   int iu0;
   ...
   short iv[NDATX]; 
   short iz[NDATX];
   short loc_span_temp [ pGeoData->num_rows] [pGeoData->num_cols];
   int ndata;
   ...
   for ( iu0 = 0; iu0 < pGeoData->num_cols; ++iu0 )
   {
      for ( iv0 = 0; iv0 < pGeoData->num_rows; ++iv0 )
      {
         if ( locspan [iv0][iu0] >= 0 )
         {
            ...
            if ( ndata >= NDATX )
            {
               sprintf ( message, "ndata greater than ndatx ... stop" );
               hpe_fieldgen_printMessage( message );
               return;
            }
   ...
   if ( ndata == 0)
   {
      sprintf ( message, "ndata = 0...nothing to interpolate on...return" );
      hpe_fieldgen_printMessage( message );
      return;
   }
...
/*            find_nbrs1 ( ndata, iu, iv, iu0, iv0, iradi, &npair, ilist, 
            ...
            for ( k = 0; k < nbor3; ++k )
            {
               ...
               if ( dist2 == 0.0 )
               {
                  sprintf ( message, "dist2 = 0...impossible event" );
                  hpe_fieldgen_printMessage( message );
                  return;
               }
...
void local_bias ( const run_date_struct * pRunDate, 
                  ...
                  int * ierr )
{
   char copy_file [ PATH_LEN ] = {'\0'};
   char * file_error_string = NULL;
   char filename1 [ PATH_LEN ] = {'\0'};
   char filename2 [ PATH_LEN ] = {'\0'};
   char filename3 [ PATH_LEN ] = {'\0'};
   char ctime_b [ YYYYMMDDHH_LEN + 1 ] = {'\0'};
   char year_to_hour_date [ YYYYMMDDHH_LEN + 1 ] = {'\0'};
   double col;
   ...
   sprintf ( message, "   num positive pairs=%d  imult=%d  "
                      "interp=%d", pGageRadarPair->pairNum, 
   ...
   /*
    * need consider HRAP and quarter HRAP cases
    * added by gzhou 01-2007
    */

   col /= pMPEParams->hrap_grid_factor;
   row /= pMPEParams->hrap_grid_factor;
   
   HrapToLatLongByReference ( row, col, & latitude, & longitude ); 

   hpe_fieldgen_hrapsize ( latitude , & rmesh );
   ...
   /* Do 50 */
                   ...
                   iu0 = iu0 + pLocalBiasParams->mult )
    {
                      ...
                      iv0 = iv0 + pLocalBiasParams->mult )
       {
          ...
          if ( lag < 0 )
          {
             sprintf ( message, "itime_p: %d", itime_p );
             hpe_fieldgen_printMessage( message );
