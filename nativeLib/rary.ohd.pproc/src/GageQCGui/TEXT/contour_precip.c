...
void contour_precip (int h, int display_flag, char *file, int num)
{
   ...
   for (i = 0; i < hrap_grid->maxi - 1; i++)
   {
	 ...
	 for (l = 0; l < dqc_precip_numcol; l++)
	 {
            /* Set the color of the contour. */
            color = gridded_precip_colormap[l];
	    ...
	    for (m = 0; m < 4; m++)
	    {
	       ...
	       if ((dqc_precip_delim[kscale][l] >= value[m] &&
		    dqc_precip_delim[kscale][l] < value[m + 1]) ||
		   (dqc_precip_delim[kscale][l] < value[m] &&
		    dqc_precip_delim[kscale][l] >= value[m + 1]))
	       {
		  ...
		  x[pnum] =
		     ...
		     (float) (hrap_grid->coord[i + di[m]][j + dj[m]].x -
			      hrap_grid->coord[i + di[m + 1]][j +
							      dj[m +
								 1]].x) /
		     (value[m] - value[m + 1]) * (dqc_precip_delim[kscale][l] -
						  value[m + 1]);
	    ...
	    else
	    {
	       ...
	       if (dqc_precip_delim[kscale][l] < value[1])
	       {
