...
void resize_px_label(w, data, call_data)
  ...
  XmDrawingAreaCallbackStruct   *call_data;
{
  ...
  char          *axis_label=" PX";            /* axis label character pointer */
  Font          font_id;                      /* font resource id */

  /* printf("in resize_px_label\n"); */
 ...
 */
  if(data->px_label_pix)
     XFreePixmap(XtDisplay(w), data->px_label_pix);

  data->px_label_pix = XCreatePixmap(XtDisplay(w),
			     DefaultRootWindow(XtDisplay(w)),
			     width, height,
			     DefaultDepthOfScreen(XtScreen(w))
			    );
  if(data->px_label_pix == 0)
    {
     printf("No space to create pixmap for px_label drawing area\n");
     exit(1);
    }
/*
 * Create graphics context if needed.
 */
  if(data->px_label_gc == NULL)
    {
     mask = GCForeground | GCBackground;
     gcv.foreground = foreground;
     gcv.background = background;
     data->px_label_gc = XCreateGC(XtDisplay(w), data->px_label_pix, mask, &gcv);
    }
/*
 ...
 */
  XSetForeground(XtDisplay(w), data->px_label_gc, background);
  XFillRectangle(XtDisplay(w), data->px_label_pix, data->px_label_gc,
		  0, 0, width, height);

  XSetForeground(XtDisplay(w), data->px_label_gc, foreground);
  ...
  XSetFont(XtDisplay(w), data->px_label_gc, font_id);
  label_axis(w, axis_label, data->px_label_pix,
	     data->px_label_gc,
	     10, height);

/*  ==============  Statements containing RCS keywords:  */
{static char rcs_id1[] = "$Source: /fs/hseb/ob72/rfc/ifp/src/PlotTulsa/RCS/resize_px_label.c,v $";
 static char rcs_id2[] = "$Id: resize_px_label.c,v 1.2 2002/02/11 19:35:02 dws Exp $";}
/*  ===================================================  */
