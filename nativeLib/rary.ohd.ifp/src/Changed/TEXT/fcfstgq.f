...
C         to the fstgq.f as well. fstgq.f is in fcst_rc directory
C
...
cAV      COMMON /RCSHF/ QSHIFT,QORG(112),HORG(112),NRCPTO
cAV      COMMON /RCNEW/ QMOD(112),HMOD(112),NRCPMD 
cAV
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ifp/src/Changed/RCS/fcfstgq.f,v $
     . $',                                                             '
     .$Id: fcfstgq.f,v 1.6 2002/02/11 19:47:20 dws Exp $
     . $' / 
C    ===================================================================
C
...
C
      ...
      if (rtcvid(1).eq.rcname(1,ncount)) then 
         if (rtcvid(2).eq.rcname(2,ncount)) goto 226
         endif
225   ncount=ncount+1
      call umemov(rtcvid(1),rcname(1,ncount),4)
      call umemov(rtcvid(2),rcname(2,ncount),4)
226   if(iset.eq.0)npoint=laspos
      if(iset.eq.0)nptrc(ncount)=nrcpmd
      numpt=nrcpmd
      numtmp=numpt
      DO 360 I=ITSPOS,LASPOS
      ORGQ=QDATA(I)
      numpt=numtmp
      if(iset.eq.1) goto 229
cc AV debug
      call umemov(rcid(1),rcidx,4)
      do 227  j = (i-1)*TSDELT+1,i*TSDELT
        htemp(ncount,j,1)=numpt
        qtemp(ncount,j,1)=numpt
      do 228 ii = 1,numpt
         qtemp(ncount,j,ii+1)=qmod(ii)
         htemp(ncount,j,ii+1)=hmod(ii)
228   continue
227   continue
C  CHECK TO SEE IF LAST APPLICABLE DAY HAS BEEN EXCEEDED WHEN 
C  RUNNING CALIBRATION PROGRAM
229   IF (MAINUM.LT.3) GO TO 270
...
290   JSHF=JSHF+1
cc      IF (JSHF.EQ.1) CALL FSHFRC(IBUG,NEEDEX,LOWEXT,IUPEXT,NW,NRANGE,
cc     * MISING,CARRYO,ISTYPE(ISHF,IRCS),EMPTY(4),HMOD,QMOD,NRCPMD,
cc     * HNEW(ISHF,IRCS),QNEW(ISHF,IRCS),HL(ISHF,IRCS),HU(ISHF,IRCS))
cc      IUNSHF=0
ccav      print *,'--:fcfstgq: HL(ISHF,IRCS)=',HL(ISHF,IRCS)
ccav      print *,'--:fcfstgq: HU(ISHF,IRCS)=',HU(ISHF,IRCS)

      NRCPMD=NRCPTS
      
      DO 295 JJ=1,NRCPMD
         QMOD(jj)=XRC(LOCQ+jj-1)
         HMOD(jj)=XRC(LOCH+jj-1)
         QORG(jj)=XRC(LOCQ+jj-1)
         HORG(jj)=XRC(LOCH+jj-1)
295      CONTINUE

      CALL FSHFRC(IBUG,NEEDEX,LOWEXT,IUPEXT,NW,NRANGE,
     * MISING,CARRYO,ISTYPE(ISHF,IRCS),EMPTY(4),HMOD,QMOD,numpt,
     * HNEW(ISHF,IRCS),QNEW(ISHF,IRCS),HL(ISHF,IRCS),HU(ISHF,IRCS),i,
     * TSDELT)
      nrcpmd=numpt
      IUNSHF=0
C
...
360   CONTINUE
      if (iset.eq.0.and.noshft.eq.0) iset=1
C
