 ...
 * Jan 31, 2019 6951       dgilling    Code cleanup.
 *
 * </pre>
 *
 * @author mpduff
...
public class GetTotalPrecip {
    ...
    protected PrecipTotal getTotalRawPp(List<PrecipRecord> ppDataList,
        ...
        if (CollectionUtil.isNullOrEmpty(ppDataList)) {
        ...
        boolean[] minuteList = new boolean[numMinutes];
        Arrays.fill(minuteList, false);
    ...
    private PrecipTotal computeRawPpTotal(List<PrecipRecord> ppDataList,
            Date startingTime, Date endingTime, int noAccumFlag,
            int endingTimeMatch, boolean[] minuteList) {
        PrecipTotal precipTotal = new PrecipTotal();
        double total;
        ...
        if (noAccumFlag == 0) {
            for (int i = 0; i < ppDataList.size(); i++) {
                ...
                if ((pRec.getValue() != CommonHydroConstants.MISSING_VALUE)
                    ...
                    /*
                     * Check that the report's time period is completely within
                    ...
                    if (pRec.getValue() >= 0.0) {
                        if (startIsWithin && endIsWithin) {
                            ...
                            if (startIsWithin || endIsWithin) {
                                ...
                                if (endmin > minuteList.length) {
                                    endmin = minuteList.length;
                                }
    ...
    private PrecipTotal subtractRawPc(List<PrecipRecord> pcDataList,
         ...
         */
        int adjStartHrs = AppsDefaults.getInstance().getInt(
                "adjust_PC_startingtime",
                CommonHydroConstants.DEFAULT_ADJUSTED_STARTTIME_HRS);
        if (adjStartHrs <= 0) {
            adjStartHrs = CommonHydroConstants.DEFAULT_ADJUSTED_STARTTIME_HRS;
        }
         ...
         * the starting time.
         */
        if (!CollectionUtil.isNullOrEmpty(pcDataList)) {
