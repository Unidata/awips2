...
public class OgcWmsProvider implements IWmsProvider {
    ...
    protected OgcResponse marshalResponse(Object jaxbobject)
        ...
        OgcResponse rval;
        try {
            String xml = jaxbManager.marshal(jaxbobject, false);
    ...
    public OgcResponse getMap(GetMapRequest req) {
        ...
        try {
            ...
            GetMapProcessor proc = new GetMapProcessor(layerManager, targetCrs,
                    env, elevation, dimensions, width, height, scale, username,
                    roles);
            if (sld == null) {
        ...
        try {
            ...
            for (int i = 0; i < layers.length; ++i) {
                ...
                List<SimpleFeature> res = source.getFeatureInfo(layer,
                        geom.getCoordinateReferenceSystem(), bbox, time,
                        mapReq.getElevation(), mapReq.getDimensions(), coord,
                        scale);
                if (!res.isEmpty()) {
                    features.add(res);
                }
