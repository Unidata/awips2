 ...
 * @version 1.0
 */
public abstract class DefaultWmsSource<D extends SimpleDimension, L extends SimpleLayer<D>>
        extends AbstractWmsSource<D, L> implements IWmsDataRetriever {
    ...
    public WmsImage getImage(String rawLayer, String style,
        ...
        PluginDataObject record = getRecord(layer, time, elevation, dimensions,
                levelUnits);
        ICoverageStyleProvider styler = getStylerInternal(layer);
     ...
     * @throws WmsException
     */
    protected abstract ICoverageStyleProvider getStyleProvider(String layer)
            throws WmsException;
    ...
    protected ICoverageStyleProvider getStylerInternal(String layer)
            throws WmsException {
        ICoverageStyleProvider rval = getStyleProvider(layer);
        return rval;
    }
    ...
    public ReferencedDataRecord getDataRecord(PluginDataObject record,
        ...
        try {
            PluginDao dao = getDao();
            return RecordUtil.getProjected(dao, record, envelope);
        } catch (Exception e) {
    ...
    public GridCoverage2D getGridCoverage(PluginDataObject record,
        ...
        try {
            PluginDao dao = getDao();
            return RecordUtil.getProjectedCoverage(dao, record, envelope);
        } catch (Exception e) {
            log.error("Unable to get reprojected data for record: " + record, e);
            throw new WmsException(Code.InternalServerError);
        }
