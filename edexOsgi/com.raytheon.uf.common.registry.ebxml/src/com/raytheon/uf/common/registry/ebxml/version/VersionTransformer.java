 ...
 * ------------- -------- --------- --------------------------------------------
 * Mar 02, 2014  2789     dhladky   Initial implementation
 * Aug 08, 2016  5744     mapeters  Moved XSLT files from edex_static to
 *                                  common_static
 * 
 * </pre>
 * 
...
public class VersionTransformer implements ILocalizationFileObserver {
    ...
    private VersionTransformer() {
        styleSheetFiles = new ConcurrentHashMap<>();
     ...
     * @throws Exception
     */
    public String transform(String content, String className,
            String targetVersion, String contentVersion) throws Exception {

        StringWriter outputWriter = new StringWriter();
        File styleSheet = getStyleSheet(className);
        try (FileReader styleSheetReader = new FileReader(styleSheet)) {
            Transformer transformer = factory.newTransformer(new StreamSource(
                    styleSheetReader));
            /*
             * Sets version you want to go to, Will use this in the future for
             * scaling our XSLT to convert to target versions.
             */
        ...
        String newXML = outputWriter.toString();
    ...
    public void fileUpdated(FileUpdatedMessage message) {
        if (message.getFileName().startsWith(XSLT_FILE_PREFIX)) {
            try {
                ...
                statusHandler.handle(Priority.WARN, "VersionTransformer: "
                        + message.getFileName() + " File couldn't be updated.",
                        e);
            }
        }
    }
