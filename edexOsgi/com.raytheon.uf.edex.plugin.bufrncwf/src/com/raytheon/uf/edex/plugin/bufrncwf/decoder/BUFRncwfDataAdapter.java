...
public class BUFRncwfDataAdapter extends BUFRPointDataAdapter<BUFRncwf> {
     ...
     * @param pluginName
     */
    public BUFRncwfDataAdapter(PointDataDescription pdd, PointDataPluginDao<BUFRncwf> dao, String pluginName) {
        super(pdd,dao,pluginName);
    }

    /**
     * Not used.
     * @see com.raytheon.uf.edex.bufrtools.BUFRPointDataAdapter#createData(java.util.Iterator, com.raytheon.uf.edex.wmo.message.WMOHeader)
     */
    @Override
     ...
     * 
     * 
     * @see com.raytheon.uf.edex.bufrtools.BUFRPointDataAdapter#createDataList(java.util.Iterator, com.raytheon.uf.edex.wmo.message.WMOHeader)
     */
    @Override
            ...
            for(IBUFRDataPacket p : dataList) {
                if(p != null) {
                    ...
                    for(IBUFRDataPacket pp : sList) {
                        ...
                        if(ncwfReport != null) {
                            ncwfReport.setPluginName(getPluginName());

                            PointDataContainer container = getContainer(ncwfReport);
                            if(container != null) {
                                PointDataView view = container.append();

                                long vt = ncwfReport.getDataTime().getValidTime().getTimeInMillis();
                                view.setLong("validTime",vt);

                                view.setFloat("storm_top", ncwfReport.getStormTop().floatValue());
                                view.setFloat("storm_dir", ncwfReport.getStormDir().floatValue());
                                view.setFloat("storm_speed", ncwfReport.getStormSpeed().floatValue());
                                
                                NCWFFeature detect = ncwfReport.getDetection();
                                view.setFloat("or_centroid_lat",(float) detect.getCentroidLatitude());
                                view.setFloat("or_centroid_lon",(float) detect.getCentroidLongitude());
                                int pos = 0;
                                for(Coordinate dc : detect.getFeatureBoundary()) {
                                    float lat = (float) DecoderTools.getCoordinateLatitude(dc);
                                    float lon = (float) DecoderTools.getCoordinateLongitude(dc);
                                ...
                                view.setFloat("centroid_lat",(float) forecast.getCentroidLatitude());
                                view.setFloat("centroid_lon",(float) forecast.getCentroidLongitude());
                                pos = 0;
    ...
    private BUFRncwf getReport(IBUFRDataPacket packet) {
        ...
        if(packet != null) {
            List<IBUFRDataPacket> sList = (List<IBUFRDataPacket>) packet.getValue();
            ...
            if(c != null) {
                ...
                List<IBUFRDataPacket> features = getPacketSubList(sList.get(FEATURES_POS));
                
                ncwfReport.setDetection(getFeatureData(getPacketSubList(features.get(0))));
                ncwfReport.setForecast(getFeatureData(getPacketSubList(features.get(1))));
                ...
                ncwfReport.setStormDir(getDouble(sList.get(STORM_DIR_POS),INVALID));
                ncwfReport.setStormSpeed(getDouble(sList.get(STORM_SPD_POS),INVALID));
                if(ncwfReport.getDetection() != null) {
                    Coordinate centroid = ncwfReport.getDetection().getCentroidLocation();
                    SurfaceObsLocation loc = new SurfaceObsLocation();
                    loc.assignLocation(DecoderTools.getCoordinateLatitude(centroid),DecoderTools.getCoordinateLongitude(centroid));
...
//            8 1 01 000     :                    Comments: Boundary-Sublist            
    ...
    private Calendar getTimeInfo(List<IBUFRDataPacket> dataList) {
        ...
        int month = getInt(dataList.get(MONTH_POS),IDecoderConstants.VAL_MISSING);
        ...
        int minute = getInt(dataList.get(MINUTE_POS),IDecoderConstants.VAL_MISSING);
        int second = getInt(dataList.get(SECOND_POS),IDecoderConstants.VAL_MISSING);
