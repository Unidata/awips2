...
public class BUFRncwfDecoder extends AbstractBUFRDecoder {
    ...
    public List<PluginDataObject> decodeData(List<BUFRDataDocument> document,
        ...
        if (document != null) {
            ...
            BUFRncwfDataAdapter adapter = new BUFRncwfDataAdapter(pdd, dao,
                    pluginName);

            logger.debug(
                    "List contains " + document.size() + " BUFRDataDocuments");

            Iterator<BUFRDataDocument> iterator = document.iterator();
            while (iterator.hasNext()) {

                logger.debug("Decoding one BUFRDataDocument");
                List<BUFRncwf> ncwfReports = adapter.createDataList(iterator,
                        wmoHeader);
                if (ncwfReports != null) {
