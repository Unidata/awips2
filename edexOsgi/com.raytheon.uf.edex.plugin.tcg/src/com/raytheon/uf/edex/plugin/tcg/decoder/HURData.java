...
public class HURData extends TCGDataAdapter {
    ...
    private final List<DataTime> forecastTimes = new ArrayList<>();

    private final HashMap<String, List<Coordinate>> coordinates = new HashMap<>();
    ...
    public List<TropicalCycloneGuidance> findReports(byte[] message) {
        ...
        List<TropicalCycloneGuidance> reports = new ArrayList<>();
                    ...
                    for (Entry<String, List<Coordinate>> entry : coordinates
                        ...
                        if (list.size() == forecastTimes.size()) {
                            for (int i = 0; i < list.size(); i++) {
                                ...
                                rpt.setInsertTime(Calendar.getInstance(
                                        TimeZone.getTimeZone("GMT")));
                                DataTime dt;
                                if (i != 0) {
                                    int fcstTime = new Double(
                                            (forecastTimes.get(i).getValidTime()
                                                    .getTimeInMillis()
                                                    - refTime.getValidTime()
                                                            .getTimeInMillis())
                                                    / 1000L).intValue();
                                    dt = new DataTime(
    ...
    private void parseModelInfo(String modelInfo) {
        ...
        if (m.find()) {
            ...
            while (m.find()) {
                ...
                double lon = Double.valueOf(
                        latlon[n].trim().substring(0, latlon[n].length() - 1));
                double lat = Double.valueOf(
                        latlon[0].trim().substring(0, latlon[0].length() - 1));
