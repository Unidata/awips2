 ...
 * ------------- -------- --------- --------------------------------------------
 * Mar 13, 2008  1030     randerso  Initial port
 * Apr 24, 2008  1047     randerso  Made all fields private and created getters
 * Apr 24, 2013  1935     randerso  Fixed date line spanning issue
 * Aug 06, 2013  1571     randerso  Added hibernate annotations Removed
 *                                  constructor with int for ProjectionType
...
public class ProjectionData {
    ...
    @Columns(columns = { @Column(nullable = false, name = "latLonLL_x"),
            @Column(nullable = false, name = "latLonLL_y") })
    @Type(type = "com.raytheon.uf.common.dataplugin.gfe.db.type.Coordinate2DType")
    @DynamicSerializeElement
    private Coordinate latLonLL;

    @Columns(columns = { @Column(nullable = false, name = "latLonUR_x"),
            @Column(nullable = false, name = "latLonUR_y") })
    @Type(type = "com.raytheon.uf.common.dataplugin.gfe.db.type.Coordinate2DType")
    @DynamicSerializeElement
    private Coordinate latLonUR;

    @Columns(columns = { @Column(nullable = false, name = "latLonOrigin_x"),
            @Column(nullable = false, name = "latLonOrigin_y") })
    @Type(type = "com.raytheon.uf.common.dataplugin.gfe.db.type.Coordinate2DType")
    @DynamicSerializeElement
    ...
    @Columns(columns = { @Column(nullable = false, name = "gridPointLL_x"),
            @Column(nullable = false, name = "gridPointLL_y") })
    @Type(type = "com.raytheon.uf.common.dataplugin.gfe.db.type.AwtPointType")
    @DynamicSerializeElement
    private Point gridPointLL;

    @Columns(columns = { @Column(nullable = false, name = "gridPointUR_x"),
            @Column(nullable = false, name = "gridPointUR_y") })
    @Type(type = "com.raytheon.uf.common.dataplugin.gfe.db.type.AwtPointType")
    @DynamicSerializeElement
    ...
    public void init() {
        ...
        try {
            ...
            statusHandler.error(
                    "Error initializing ProjectionData: " + projectionID, e);
        }
    }
    ...
    public synchronized CoordinateReferenceSystem getCrs() {
        if (crs == null) {
            ...
            default:
                statusHandler.error(
                        "Unknown projection type: " + this.projectionType);
            }
        }
    ...
    public boolean equals(Object obj) {
        ...
        if ((Math.abs(this.latLonLL.y - rhs.latLonLL.y) > COMPARISON_THRESHOLD)
                ...
                || (Math.abs(this.latLonUR.y
                        - rhs.latLonUR.y) > COMPARISON_THRESHOLD)
                || (Math.abs(this.latLonLL.x
                        - rhs.latLonLL.x) > COMPARISON_THRESHOLD)) {
            return false;
        }
        ...
        switch (this.projectionType) {
        case LAMBERT_CONFORMAL:
            return ((Math.abs(this.latLonOrigin.y
                    - rhs.latLonOrigin.y) < COMPARISON_THRESHOLD)
                    || (Math.abs(this.latLonOrigin.x
                            - rhs.latLonOrigin.x) < COMPARISON_THRESHOLD)
                    || (Math.abs(this.stdParallelOne
                            - rhs.stdParallelOne) < COMPARISON_THRESHOLD)
                    || (Math.abs(this.stdParallelTwo
                            - rhs.stdParallelTwo) < COMPARISON_THRESHOLD));
        case POLAR_STEREOGRAPHIC:
            return (Math.abs(
                    this.lonOrigin - rhs.lonOrigin) < COMPARISON_THRESHOLD);
        case MERCATOR:
            return (Math.abs(
                    this.lonCenter - rhs.lonCenter) < COMPARISON_THRESHOLD);
        case LATLON:
            return true;
        default:
            return false;
    ...
    public Coordinate gridCoordinateToLatLon(Coordinate gridCoord)
        ...
        mt.transform(new double[] { gridCoord.x, gridCoord.y }, 0, output, 0,
                1);
        latLon.x = output[0];
        latLon.y = output[1];
        return latLon;
     ...
     */
    public MathTransform getLatLonToGrid()
