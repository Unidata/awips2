 ...
 * May 17, 2018      #7286 dgilling    Do not allow visibilities to be combined
 *                                     with NoWx.
 * Feb 12, 2019 20735      ryu         Use long to code Wx subkey and enable up
 *                                     to 44 weather attributes to be used.
 *
 * </pre>
 *
 * @author randerso
...
public class WeatherSubKey {
    ...
    public static class Ordering {
        public final long typeMask;

        public final long coverageMask;

        public final long intensityMask;
    ...
    private static final int CoverageShift = 59;

    private static final int TypeShift = 52;

    private static final int IntensityShift = 48;

    private static final int VisibilityShift = 44;

    private static final int AttributesShift = 0;

    private static final long CoverageMask = 0xF800000000000000L;

    private static final long TypeMask = 0x07F0000000000000L;

    private static final long IntensityMask = 0x000F000000000000L;

    private static final long VisibilityMask = 0x0000F00000000000L;

    private static final long AttributesMask = 0x00000FFFFFFFFFFFL;

    private static final Ordering ORDERING = new Ordering();

    // number of possible attributes
    private static final int NumAttributeBits = 44;
    ...
    public WeatherSubKey() {
        ...
        intensity = INVALID;
        visibility = INVALID;
        attributes = Collections.singletonList(INVALID);
        intCached = false;
    }
    ...
    public WeatherSubKey(WeatherSubKey subKey) {
        ...
        this.attributes = new ArrayList<>(subKey.attributes);
     ...
     * @param coded
     */
    public WeatherSubKey(String siteId, long coded) {
        intCached = false;
        ...
        int coverageBit = (int) ((coded & CoverageMask) >> CoverageShift);
        int typeBit = (int) ((coded & TypeMask) >> TypeShift);
        int intensityBit = (int) ((coded & IntensityMask) >> IntensityShift);
        int visibilityBit = (int) ((coded & VisibilityMask) >> VisibilityShift);
        long attrBit = (coded & AttributesMask) >> AttributesShift;
     ...
     *         0xFFFFFFFFFFFFFFFFL on an error.
     */
    public long subKeyAsInt() {
        if (intCached) {
            return intCache;
        }
        ...
        for (int i = 0; i < indices.attrIndex.size(); i++) {
            attrMask |= (1L << indices.attrIndex.get(i));
        }

        this.intCache = (((long) indices.typeIndex) << TypeShift)
                | (((long) indices.covIndex) << CoverageShift)
                | (((long) indices.intenIndex) << IntensityShift)
                | (((long) indices.visIndex) << VisibilityShift)
    ...
    private void setInvalid() {
        ...
        attributes = Collections.singletonList(invalidStr());
        intCached = false;
    }
    ...
    private Indices okay() {
        ...
        List<Integer> attrIndex = attributes.stream()
                .mapToInt(a -> wxDefinition.attributeIndex(type, a)).boxed()
        ...
        if (typeIndex == -1 || covIndex == -1 || intenIndex == -1
            ...
            return null;
        } else if (("<NoWx>".equals(type)) && (!"<NoVis>".equals(visibility))) {
            return null;
        } else {
