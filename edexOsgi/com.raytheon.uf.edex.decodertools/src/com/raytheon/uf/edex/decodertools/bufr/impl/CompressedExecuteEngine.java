 ...
 **/
package com.raytheon.uf.edex.decodertools.bufr.impl;
...
import com.raytheon.uf.edex.decodertools.bufr.descriptors.BUFRReplicationDescriptor;
import com.raytheon.uf.edex.decodertools.bufr.descriptors.BUFRSublistDescriptor;
import com.raytheon.uf.edex.decodertools.bufr.descriptors.BUFRTableB;
import com.raytheon.uf.edex.decodertools.bufr.descriptors.BUFRTableC;
...
import com.raytheon.uf.edex.decodertools.bufr.packets.BUFROperatorPacket;
import com.raytheon.uf.edex.decodertools.bufr.packets.BUFRSublistPacket;
import com.raytheon.uf.edex.decodertools.bufr.packets.IBUFRDataPacket;
import com.raytheon.uf.edex.decodertools.bufr.EngineData;
import com.raytheon.uf.edex.decodertools.bufr.ExecuteEngine;
...
public class CompressedExecuteEngine extends ExecuteEngine {
    ...
    /**
     * Construct an instance of a BUFR decode engine for data stored in compressed
     * format. After construction, the processBUFR method is called to actually
     * decode the data.
     * @param subList
    ...
    /**
     * 
     * @param bitStrm The bit input stream to read data from.
        ...
        for (BUFRDescriptor descriptor : descriptors) {
            ...
            try {
                if (descriptor instanceof BUFRTableB) {
                    ...
                    if (readOverBits > 0) {
                        ((BUFRNullDescriptor) descriptor).setBitWidth(readOverBits);
    ...
    private List<IBUFRDataPacket> processTableB(BUFRTableB descriptor,
        ...
        //
        // NCEP/NCO/SIB added boolean argument for compressed flag and repetition count
        // (number of subsets) to method execute. (4/2010)
        IBUFRDataPacket packet = descriptor.execute(bitStream,
                getDataScaleOverride(), getDataBitOverride(), getSkipBits(), true, repetitionCount);
     ...
     * @param packets List of data packets to receive the decoded data. 
     */
     ...
     * @param descriptor The Table C descriptors to "execute".
     * @param packets List of Data Packets that may receive the descriptor.
     */
    ...
    private void normalizeList(List<IBUFRDataPacket> packetData,
        ...
        for (IBUFRDataPacket p : packetData) {
            if (p instanceof BUFRSublistPacket) {
                ...
                BUFRSublistPacket packet = new BUFRSublistPacket(nReceiver, p
                        .getUnits(), p.getReferencingDescriptor());
                receiver.add(packet);
