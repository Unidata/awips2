 ...
 * ------------ ---------- ----------- --------------------------
 * ?            ?          ?           Initial creation.
 * Mar 02, 2016 5434       bkowal      Eliminated deprecation warnings.
 * 
 * </pre>
 * 
 * @author dhladky
...
public class SpatialLine {
    ...
    private void makeSynopticSpatial(String[] line) {
        ...
        if (!line[0].trim().equals("")) {
            ...
            if (line[0].trim().equals(
                    AWIPSCommonObsSQLGenerator.SYNOPTIC_BLANK_POS)) {
        ...
        if (wmo.equals(AWIPSCommonObsSQLGenerator.NULL)
                && !icao.equals(AWIPSCommonObsSQLGenerator.NULL)) {
            setGid("null-" + icao);
        } else if (!wmo.equals(AWIPSCommonObsSQLGenerator.NULL)
                && icao.equals(AWIPSCommonObsSQLGenerator.NULL)) {
            setGid(wmo + "-null");
        } else {
            setGid(wmo + "-" + icao);
        ...
        if (!line[5].equals("") && !line[6].equals("")) {
            ...
            try {
                ...
                geometry = (Point) wktReader.read("POINT ("
                        + AWIPSCommonObsSQLGenerator.FORMAT.format(lon) + " "
                        + AWIPSCommonObsSQLGenerator.FORMAT.format(lat) + ")");
            } catch (ParseException pe) {
                pe.printStackTrace();
            }

            geom = WKBWriter.toHex(wkbWriter.write(geometry));
        }
