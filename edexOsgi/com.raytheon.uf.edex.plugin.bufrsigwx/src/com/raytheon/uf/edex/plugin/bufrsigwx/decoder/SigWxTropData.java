...
public class SigWxTropData extends SigWxDataAdapter {
     ...
     * @param container
     */
    public SigWxTropData(PointDataDescription pdd, PointDataPluginDao<SigWxData> dao, String pluginName) {
    ...
    List<SigWxData> getSigWxData(SigWxData sigWx, List<IBUFRDataPacket> dataList) {
        ...
        for(SigWxData s : sList) {
            ...
            if(container != null) {
                ...
                if(size < la.size()) {
                    logger.info("Tropopause data truncated at " + size + " needed " + la.size());
                }
                int numOfPoints = 0;
                try {
