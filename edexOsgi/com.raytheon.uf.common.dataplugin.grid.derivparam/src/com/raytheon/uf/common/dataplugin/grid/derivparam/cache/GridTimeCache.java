 ...
 * ------------- -------- --------- --------------------------------------------
 * Jun 10, 2010  6276     bsteffen  Initial creation
...
public class GridTimeCache implements GridUpdateListener {
    ...
    private final Map<GridMapKey, CacheEntry<TimeAndSpace>> cache = new LinkedHashMap<GridMapKey, CacheEntry<TimeAndSpace>>(
         ...
         * cache recently.
         */
        private long minTimeInCache = cacheTime;
        ...
        protected boolean removeEldestEntry(
            ...
            if (this.size() > CACHE_SIZE) {
                ...
                if (timeInCache < cacheTime
                        /*
                         * Don't print more than once a minute (60000ms) unless
                         * this entry has been purged faster(was in the cache
                         * for less time) than the last output message.
                         */
     ...
     */
    private final Map<String, CacheEntry<DataTime>> modelCache = new HashMap<>();
    ...
    public Set<TimeAndSpace> getTimes(GridRequestableNode gNode) {
        synchronized (cache) {
            ...
            if (entry.insertTime + cacheTime < System.currentTimeMillis()) {
                cache.remove(key);
                return null;
            }
            return entry.times;
    ...
    public void setModelTimes(String modelName, Set<DataTime> times) {
        synchronized (modelCache) {
            modelCache.put(modelName, new CacheEntry<>(times));
        }
    }
    ...
    public void update(GridRecord record) {
        GridMapKey key = new GridMapKey(record);
        clearTimes(key);
    }
