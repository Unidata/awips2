...
public class FFMPRunConfigurationManager implements ILocalizationPathObserver {
    ...
    private FFMPRunConfigurationManager() {
        ...
        try {
            ...
            throw new IllegalStateException("Error reading " + CONFIG_FILE_NAME
     ...
     * @return
     */
    public FFMPRunXML getFFMPRunner() {
        if (!this.isPopulated) {
            return null;
        }
    ...
    public void setFFMPRunner(FFMPRunXML runner) {
        if (configXml == null) {
            configXml = new FFMPRunConfigXML();
        }
        configXml.setFFMPRun(runner);
    }

    public DomainXML getDomain(String domainname) {
        for (DomainXML domain : getFFMPRunner().getDomains()) {
    ...
    public FFMPRunXML getRunner(String domainName) {
        FFMPRunXML runner = getFFMPRunner();
    ...
    public List<DomainXML> getDomains() {
        ...
        domains.addAll(getFFMPRunner().getDomains());
        return domains;
    }

    public List<ProductRunXML> getProducts() {
        List<ProductRunXML> products = new ArrayList<>();
        products.addAll(getFFMPRunner().getProducts());
        return products;
    }
    ...
    public void applySourceConfigOverrides() {
        ...
        if (runner.isOverride()) {
            for (String source : runner.getOverrideSources()) {
                ...
                SourceOverrideXML override = runner.getSourceOverride(source);
                osource.setOverrideDataKeys(
                        override.getSourceOverrideDataKeys());
            }
        }
    }
}
