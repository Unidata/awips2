 ...
 *                                  SITE
 * Aug 15, 2016  5819     mapeters  Undid last change since code that saves
 *                                  config file is called from CAVE
 * 
 * </pre>
 * 
 * @author lvenable
...
public class FFTIDataManager implements ILocalizationPathObserver {
    ...
    private static final String CONFIG_FILE_NAME = "ffmp"
            + IPathManager.SEPARATOR + "FFTIData.xml";
    ...
    public synchronized void readConfigXml() {
        ...
        if (lf == null || !lf.exists()) {
            statusHandler.warn(CONFIG_FILE_NAME + " does not exist.");
            return;
        }
        ...
        try (InputStream is = lf.openInputStream()) {
            ...
            statusHandler.handle(Priority.WARN, "Error reading file: "
                    + CONFIG_FILE_NAME, e);
        }
    }
