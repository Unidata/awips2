...
public class FFFGDataMgr {
     ...
     */
    private static final String FFFG_MASTER_FILENAME = "FFFGMasterData.xml";
    ...
    private void readInMasterXmlManager() {
        masterXmlMgr = new FFFGXmlMgr(FFFG_MASTER_FILENAME, getFFFGDataFilePath(),
                FFFGXmlType.MASTER);
        try {
            masterXmlMgr.readThresholdXml();
        } catch (Exception ex) {
            statusHandler.debug("Error reading threshold xml", ex);
            masterXmlMgr = null;
        }
    }
    ...
    public boolean isExpired() {
        ...
        if (Calendar.getInstance(TimeZone.getTimeZone("GMT"))
                .getTimeInMillis() < expTime) {
            return false;
        }
    ...
    public void saveUpdateUserXML(long expTime,
            List<SourceCompData> srcCompData,
            List<List<FFFGBasinIdXML>> basinList) {
        if (userXmlMgr == null) {
            statusHandler.debug("User XML manager is null");
            return;
        }
     ...
     *            Array of source component data.
     */
    public void updateMasterXML(long expTime, List<SourceCompData> srcCompData,
            List<List<FFFGBasinIdXML>> basinList) {
        FFFGDataXML newXmlData = new FFFGDataXML();
    ...
    private void createSourceXml(FFFGDataXML newXmlData,
            List<SourceCompData> srcCompData,
            List<List<FFFGBasinIdXML>> basinList) {
        ArrayList<FFFGSourceXML> sources = new ArrayList<>();
        ...
        for (SourceCompData scd : srcCompData) {
            List<FFFGBasinIdXML> list = basinList.get(idx);
            ...
            List<ValueNameIdData> cntyBasinData = scd.getCountyBasinData();
            ArrayList<FFFGSourceItemXML> sourceItems = new ArrayList<>();
    ...
    public double adjustValue(double value, String sourceName, Long basinPfaf,
        ...
        if (!masterXmlMgr.getXMLData().containsSource(sourceName)) {
            return value;
        }

        // Check basin
        FFFGSourceXML srcXML = masterXmlMgr.getXMLData()
                .getSourceData(sourceName);

        if (srcXML.containsID(basinPfaf)) {
            return adjustValue(value, srcXML.getAssociatedValue(basinPfaf));
        }

        // Check county
        if (srcXML.containsID(countyGID)) {
    ...
    public boolean isForced(String sourceName, Long id) {
        if (masterXmlMgr != null) {
            ...
            if (srcXML.containsID(id) && (!isExpired())) {
                return true;
            }
        }
     ...
     */
    public List<SourceCompData> getUserXMLData() {
        List<SourceCompData> srcDataArray = new ArrayList<>();

        SourceCompData srcCompData;
        if ((userXmlMgr == null) || (userXmlMgr.getXMLData() == null)) {
            return null;
        }
        List<FFFGSourceXML> sources = userXmlMgr.getXMLData().getSources();
     ...
     * @return An array of source component data.
     */
    public List<SourceCompData> getMasterXMLData() {
        List<SourceCompData> srcDataArray = new ArrayList<>();

        SourceCompData srcCompData;
        List<FFFGSourceXML> sources = null;

        try {
            sources = masterXmlMgr.getXMLData().getSources();
        } catch (Exception e) {
            statusHandler.handle(Priority.DEBUG,
                    "No FFFG Data sources in file", e);
        }
    ...
    public boolean isBasinForced(String sourceName, long id) {
        if ((masterXmlMgr != null) && (sourceName != null)) {
            ...
            try {
                ...
                /*
                 * FIXME Leave catch in place until we can figure out what is
                 * null and correct the code
                 */
                statusHandler.error(
                        "NPE for sourceName=" + sourceName + ", id=" + id, npe);
            }
        }
