...
nohup bash -c set_nfs_perms > /data/fxa/INSTALL/a2logs/set_nfs_perms_dx1_${now}.log 2>&1 & disown
exit
EOF
...
export -f set_nfs_perms
nohup bash -c set_nfs_perms >>/data/fxa/INSTALL/a2logs/set_nfs_perms_dx3_${now}.log 2>&1 & disown
exit
EOF
...
# qpid message store - px1 edge case
...
# The following is a copy of dx3 commands, covering /awips2/edex/data tree;
# these are also run on px1 because of weird setup between sites ALR/SJU.
...
export -f set_nfs_perms
nohup bash -c set_nfs_perms > /data/fxa/INSTALL/a2logs/set_nfs_perms_px1_${now}.log 2>&1 & disown
exit
EOF


ssh cpsbn1 << EOF & disown
bash
set -x
...
export -f set_nfs_perms
nohup bash -c set_nfs_perms > /data/fxa/INSTALL/a2logs/set_nfs_perms_cpsbn1_${now}.log 2>&1 & disown
exit
EOF
