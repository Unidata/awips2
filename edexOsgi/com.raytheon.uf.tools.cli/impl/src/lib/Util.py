...
#    12/12/19        7995          dgiling        Add time_limit.
##############################################################################
...
def convListToDict(list):
    if len(list) % 2 != 0:
        raise ValueError("Invalid input list " + str(list))
    retVal = [] 
    while len(list) > 0:
        temp = []
        key = list.pop(0)
...
def doWithinTime(target_function, description='complete the operation', 
        max_tries = 3, max_try_time = 10.0, args=(), kwargs=None):
    if kwargs is None:
        kwargs = {}
    q = queue.Queue()
    ...
    raise Exception("Failed to %s%s" % (description, reason))
