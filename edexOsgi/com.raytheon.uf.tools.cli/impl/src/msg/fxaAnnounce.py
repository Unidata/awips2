...
if len(sys.argv) > 4:
   ...
   if arg4 is not None:
      if not arg4:
         ...
         fltrs=dict(item.strip().split("=") for item in arg4.split(","))
...
if len(fltrs):
    ...
    msg = NotificationMessage.NotificationMessage(host=hostname, port=portnum, message=announcement, source=source, category=cat, priority=pri, audioFile=arg4)
