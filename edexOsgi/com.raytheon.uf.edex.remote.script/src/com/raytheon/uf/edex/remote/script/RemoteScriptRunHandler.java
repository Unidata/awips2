...
public class RemoteScriptRunHandler extends AbstractRemoteScriptHandler {
    ...
    /**
     * Constructor setup permission Id and resource property values.
     */
    public RemoteScriptRunHandler() {
        // The permission id in the common remoteScript.ini
        super(PermissionUtils.buildPermissionString("remoteScript", "run"));
        ...
        try {
            ...
            if (defaultSetupExit <= 0) {
                defaultSetupExit = Integer.parseInt(
                        RemoteScriptConstants.scriptSetUpErrorDefault);
            }
    ...
    public Object performRequest(RemoteScriptRequest request) {
        ...
        if (maxTimeout <= 0) {
            String message = String.format(
                    "HttpClient's socket timeout of %d msec not enough time to run a remote script.",
    ...
    public Class<?> getRequestType() {
        return RemoteScriptRunRequest.class;
    }
}
