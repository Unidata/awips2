 ...
 * ------------- -------- --------- --------------------------------------------
 * Jan 10, 2010  4112     jsanchez  Initial creation
 * Apr 10, 2014  2971     skorolev  Cleaned code.
 * Jun 25, 2014  3008     nabowle   Refactor for EventReport.
 * Apr 12, 2016  4913     bsteffen  Return specific type when there are no
 *                                  reports
 * 
 * </pre>
 * 
...
public class InternalReport {
     ...
     * </pre>
     */
    private static final Pattern NO_REPORT_LN_PTRN = Pattern
            .compile("\\s*\\*{10} NO SEVERE REPORTS RECEIVED BY SPC \\*{10}\\s*");
    ...
    public static List<InternalReport> identifyMessage(byte[] message) {
        ...
        if (lines != null) {
            ...
            Map<InternalType, Pattern> patternsToTest = new EnumMap<>(
                    InternalType.class);
            patternsToTest.put(InternalType.REPORT_TYPE, REPORT_TYPE_LN_PTRN);
            patternsToTest.put(InternalType.TIME_RANGE, TIME_RANGE_LN_PTRN);
            patternsToTest.put(InternalType.NO_REPORT, NO_REPORT_LN_PTRN);
            ...
            for (String s : lines) {
                found = false;
                for (Entry<InternalType, Pattern> entry : patternsToTest
                        .entrySet()) {
                    m = entry.getValue().matcher(s);
                    if (m.matches()) {
