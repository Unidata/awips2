...
import com.raytheon.uf.edex.plugin.mpe.gather.radar.MpeRadarDecoderConfig;
 ...
 * Dec 01, 2016 5588       nabowle     Move localization config class for reuse.
...
public class DecodeDPA {
    ...
    private static final float PRECIP_OUTSIDE_RADAR = -99f;

    private static final float PRECIP_ZERO = -98f;
    ...
    private SingleTypeJAXBManager<MpeRadarDecoderConfig> jaxbManager;
    ...
    private void decodeProductData(byte[] productFileBytes, String radarId,
        ...
        float bias = dmaHeaderData[37] / (float) 100;
        float maxvalh = dmaHeaderData[36] / (float) 10;
        ...
        int operationalWeatherMode = dmaHeaderData[6];
            ...
            if (decon) {
                ...
                try {
                    Files.write(fileLEPath, littleEndianBytes.array(),
                            optionOption);
                    IOPermissionsHelper.applyFilePermissions(fileLEPath,
        ...
        if (xmax > 0.0) {
            ...
            try {
                IOPermissionsHelper.applyFilePermissions(
                        Paths.get(decodedFilePath),
                        FilePermissionConstants.POSIX_FILE_SET);
            } catch (IOException e) {
                logger.warn(
                        "Failed to update the permissions of stage-1 decoded file: "
                                + decodedFilePath + ".",
                        e);
            }
        }
    ...
    private boolean getLocalizedXMLParameter() throws DecodeDPAException {
        ...
        MpeRadarDecoderConfig defaultConfigValues = null;
