...
public class FFMPURIFilter extends URIFilter {
    ...
    /** date differential, basically 30 minutes */
    private static final long DIFFERENTIAL = TimeUtil.MILLIS_PER_MINUTE * 30;

    /** create a date formatter for XMRG **/
    private static final SimpleDateFormat XMRG_DATE_FMT = new SimpleDateFormat(
            "yyyyMMddHHmm");
    ...
    protected String applyWildCards(String key) {
        ...
        for (int i = 1; i < comps.length; i++) {
            if (i == 2) {
                ...
                newKey.append(uriSeperator).append(wildCard);
            } else if (i == 0) {
                // do nothing
            } else {
                newKey.append(uriSeperator).append(comps[i]);
            }
        }
    ...
    protected String removeWildCards(String key) {
        ...
        String time = getDateFormatter()
                .format(getMatchedURIs().get(key).getTime());

        for (int i = 1; i < comps.length; i++) {
            if (i == 2) {
                // second comp is time, wildcarded for most
                newKey.append(uriSeperator).append(time);
            } else if (i == 0) {
                // do nothing
            } else {
                newKey.append(uriSeperator).append(comps[i]);
    ...
    private String replaceWildCard(String site, String dataPath, int pos) {
        ...
        for (int i = 1; i < comps.length; i++) {
            if (i == pos) {
                ...
                newKey.append(uriSeperator).append(site);
            } else if (i == 0) {
                // do nothing
            } else {
                newKey.append(uriSeperator).append(comps[i]);
    ...
    public void setMatchURIs() {
        ...
        for (SourceXML source : sourceConfig.getSources()) {
            SourceIngestConfigXML sicx = runner
                    .getSourceIngest(source.getSourceName());
    ...
    private Date getXmrgDate(final SourceXML xml, final File xmrgFile,
        ...
        SimpleDateFormat formatter = XMRG_DATE_FMT;
    ...
    public boolean isMatched(DataURINotificationMessage message) {
        ...
        if (!sourceFileTimes.isEmpty()) {
            for (SourceXML source : sourceFileTimes.keySet()) {
                // cursory check
                if (getLastModified(source)
                        .getTime() > (getCurrentTime().getTime()
                                - (DIFFERENTIAL))) {
