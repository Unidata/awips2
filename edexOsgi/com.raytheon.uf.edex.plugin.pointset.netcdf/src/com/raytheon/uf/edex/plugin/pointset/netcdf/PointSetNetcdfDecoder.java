...
public class PointSetNetcdfDecoder extends AbstractNetcdfDecoder {
    ...
    protected List<NetcdfRecordInfo> processDeferredDescriptions(
        ...
        while (iter.hasNext()) {
            info = iter.next();
            dataVariable = NetcdfDecoderUtils.getDataVariable(info, netcdfFile);
            if (dataVariable == null) {
                logger.warn(
                        "Record discarded because the data variable could not be found.");
                iter.remove();
                continue;
            }
            ...
            VariableDescription latVariableDesc = (VariableDescription) info
                    .getDeferredDescription(
                            PointSetNetcdfProductDescription.LATITUDE_KEY);
            ...
            VariableDescription lonVariableDesc = (VariableDescription) info
                    .getDeferredDescription(
                            PointSetNetcdfProductDescription.LONGITUDE_KEY);
            ...
            if (lonVariable == null || latVariable == null) {
                ...
                for (int i = 0; i < dims.size(); i++) {
                    dim = dims.get(i);
                    if (info.getDimensionIndexMap()
                            .containsKey(dim.getName())) {
                        expectedShape[i] = 1;
                    } else {
            ...
            if (location == null) {
                float[] lonVals;
                try {
                    lonVals = (float[]) lonVariable.read()
                ...
                float[] latVals;
                try {
                    latVals = (float[]) latVariable.read()
                            .get1DJavaArray(float.class);
                } catch (IOException e) {
                    throw new NetcdfDecoderException(
                ...
                if (triangulation != null) {
                    ...
                    if (type != null) {
                        if (type == TriangulationType.GRID) {
                            ...
                            try {
                                location.setTriangles(
                                        triangulator.triangulate(location));
            ...
            if (!storedPaths.contains(path)) {
                try {
                    IMetadataIdentifier metaId = new DataUriMetadataIdentifier(
                            record);
                    location.save(path.toFile(), metaId);
    ...
    private static Variable checkCoordinate(NetcdfFile file, String coordinate,
        ...
        if (pattern.matcher(coordinate).find()) {
            ...
            if (variable != null && !Arrays.equals(expectedShape,
