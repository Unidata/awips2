...
public class GetBackupHostCapabilitiesMapHandler
    ...
    public Object handleRequest(GetBackupHostCapabilitiesMapRequest request)
            throws Exception {
        BackupServiceConfigManager configMgr =
                BackupServiceConfigManager.getInstance();
        configMgr.reload();
        List<BackupHost> hosts =
                BackupServiceConfigManager.getInstance().getBackupHosts();
        Map<String, Set<String>> hostMap = new HashMap<>();
        for (BackupHost host : hosts) {
            Set<String> capabilities = host.getCapabilities();
            if (capabilities != null) {
                hostMap.put(host.getHostName(), capabilities);
            }
        }
        return hostMap;
    }
