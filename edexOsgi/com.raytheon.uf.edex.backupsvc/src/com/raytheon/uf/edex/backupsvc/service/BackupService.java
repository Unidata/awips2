 ...
 * Jul 26, 2023 2035783    Lisa.Singh  Improved error handling and messages.
 *
 * </pre>
 *
 * @author tgurney
...
public class BackupService extends EdexTimerBasedThread {
    ...
    public void process() throws Exception {
        ...
        for (BackupHost host : hosts) {
            ...
            if (host.getEDEXVersion() == null
                    || host.getEDEXVersion().isBlank()) {
                logger.warn("Unable to reach recipient site '"
                        + host.getHostName() + "'.");
                continue;
            }

            for (BackupJob job : dao.poll(host.getHostName())) {
                if (EDEXUtil.isShuttingDown()) {
                    break;
                }
                ...
                if (versionMatches == null) {
                    logger.warn("Unable to contact host " + job.getHost()
                            + " for version information. Skipping job with ID"
                            + job.getId() + " for this host. "
                            + "Will try again next time");
                } else if (versionMatches != 0) {
                    logger.info("Skipping job with ID" + job.getId()
                            + " intended for backup site" + job.getHost()
                            + " as that server's version ("
                            + host.getEDEXVersion()
