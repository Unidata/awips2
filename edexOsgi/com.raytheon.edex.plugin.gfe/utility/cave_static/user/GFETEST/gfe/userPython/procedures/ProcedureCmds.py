...
class Procedure (SmartScript.SmartScript):
    ...
    def execute(self, editArea, timeRange, varDict):
        ...
        # findDatabase -- Find databaseID for GFS model
        GFS_databaseID = self.findDatabase("GFS")

        # copy Commands
        print("Copy commands")
        copyTimeRange = self.getTimeRange(beginHour, 120, 'GFS')
        self.copyCmd(elements, GFS_databaseID, copyTimeRange)
        self.copyCmd(elements, GFS_databaseID, timeRange_120_240)
        ...
        # zero
        print("Zero")
        zeroTimeRange = self.getTimeRange(beginHour, 6, 'GFS')
        self.zeroCmd(['MixHgt'], zeroTimeRange)
        self.zeroCmd(['MixHgt'], timeRange_begin_60)

        # assignValue
        print("AssignValue")
        assignTimeRange = self.getTimeRange(beginHour, 6, 'GFS')
        ...
        # timeShift
        print("TimeShift")
        shiftTimeRange = self.getTimeRange(beginHour, 24, 'GFS')
        self.timeShiftCmd(['T', 'Wind'], 1, 3, shiftTimeRange)
        self.timeShiftCmd(['T', 'Wind'], 1, 3, timeRange_begin_24)
        
        # interpolate
        print("Interpolate")
        interpolateTimeRange = self.getTimeRange(beginHour, 6, 'GFS')
