...
class gfsLRForecaster(Forecaster):
##-------------------------------------------------------------------------
## Module that calculates surface weather elements from gfsLR model
## output.
##-------------------------------------------------------------------------
    def __init__(self):
        Forecaster.__init__(self, "gfsLR")
...
##-------------------------------------------------------------------------
    def levels(self):
        return ["MB1000", "MB850", "MB700", "MB500", "MB300"]
...
## fields.
##-------------------------------------------------------------------------
    def calcTd(self, gh_c, T, t_BL030, stopo, topo, rh_BL030):
        p_SFC = self.getSFCP(gh_c, stopo) - 15 * 100
        t_FHAG2 = t_BL030
        rh_FHAG2 = rh_BL030
...
##-------------------------------------------------------------------------
    ...
    def calcSky(self, rh_c, gh_c, topo, pmsl_MSL):
        return self.skyFromRH(rh_c, gh_c, topo, pmsl_MSL)
...
##-------------------------------------------------------------------------
##  Internal function that returns a pressure from the geopotential
##  height cube at an elevation specified by the topo grid.
##-------------------------------------------------------------------------
...
##-------------------------------------------------------------------------
    def calcWx(self, QPF, T, t_c, gh_c, stopo, topo, tp_SFC, cp_SFC):
        p_SFC = self.getSFCP(gh_c, stopo)
        gh_c = gh_c[:13, :, :]
        t_c = t_c[:13, :, :]
        T = self.FtoK(T)
...
##-------------------------------------------------------------------------
##  Calculates Lightning Activity Level based on total precip.
##  convenctivePrecip, and 3-D relative humidity.
##-------------------------------------------------------------------------
    def calcLAL(self, tp_SFC, cp_SFC, rh_c, rh_BL030):
        cp_SFC = where(equal(cp_SFC, 0), 0.00001, cp_SFC)
        lal = self._empty + 1
        ...
        lal = where(greater(cp_SFC, 2.0), lal + 1, lal)
        lal = where(greater(cp_SFC, 8.0), lal + 1, lal)
