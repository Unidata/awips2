...
# Jul 31, 3017  6342     randerso  Removed unused imports. Fixed long standing
...
import string, time, gzip, os, LogStream, stat, traceback
import calendar
...
def getDims(file, dimSizes, dimNames):
    ...
    actDimNames = []
    existingDimList = list(file.dimensions.keys())
...
###-------------------------------------------------------------------------###
def storeLatLonGrids(ifpServer, file, databaseID, invMask, krunch, clipArea):
...
###-------------------------------------------------------------------------###
def storeTopoGrid(ifpServer, file, databaseID, invMask, clipArea):
...
def storeGridDataHistory(file, we, histDict):
    ...
    maxHistSize = 0
    histList = []
    for (tr, his) in list(histDict.items()):
        hisString = ''
...
def storeProjectionAttributes(var, projectionData):
    ...
    setattr(var, "projectionType", str(projectionType))
...
def storeWEAttributes(var, we, timeList, databaseID, clipArea, siteIdOverride):
    ...
    # data type
    setattr(var, "gridType", str(we.getGpi().getGridType()))
    # database ID
...
def getSamplingDefinition(configName, siteId):
...
#      AFPS.DBSubsystem_getBuildVersion())
    ...
    samplingDef = getSamplingDefinition(argDict['configFileName'], siteId)
    ...
    if argDict["geoInfo"]:
        storeTopoGrid(ifpServer, file, argDict['databaseID'], invMask, clipArea)
        storeLatLonGrids(ifpServer, file, argDict['databaseID'], invMask,
