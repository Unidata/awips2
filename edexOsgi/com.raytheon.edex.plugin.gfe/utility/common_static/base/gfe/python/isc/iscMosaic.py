...
# Mar 12, 2013 1759       dgilling    Remove unnecessary command line
#                                     processing.
...
# Jun 05, 2013  2063     dgilling  Change __siteInDbGrid() to
...
# Jul 22, 2014 17484      randerso    Update cluster lock time to prevent time
#                                     out
...
def initLogger(logName):
    import logging
    global logger
    logger = iscUtil.getLogger("iscMosaic", logName=logName, logLevel=logging.DEBUG)
...
class WECache(object):
    ...
    def keys(self):
        if not self._invCache:
            self._invCache = tuple(sorted(list(self._inv.keys()), key=lambda t: t[0]))
    ...
    def flush(self):
        ...
        self.__flushGrids(list(self.keys()))
...
class IscMosaic:
    ...
    def __processInputFile(self, filename):
        ...
        areaMask = None
        inFileVars = list(file.variables.keys())   #parm names
        ...
        if len(self.__announce) and totalGrids > 0:
            msg = self.__announce + self.__siteID + ' ' + repr(pParms) + ' ' + printShortTR(totalTimeRange) + ' #Grids=' + repr(totalGrids)
            if totalFails:
    ...
    def __processParm(self, parmName, vars, history, filename):
        ...
        if self.__blankOtherPeriods or self.__eraseFirst or len(inTimes) > 0:
            while retryAttempt != retries:
                ...
                logger.debug("iscMosaic: Attempting to acquire cluster lock for: %s", lockName)
                startTime = time.time()
                clusterLock = ClusterLockUtils.lock("ISC Write Lock", lockName , 120000, True)
                elapsedTime = (time.time() - startTime) * 1000
                logger.debug("iscMosaic: Request for %s took %d ms", lockName, elapsedTime)
                if str(clusterLock.getLockState()) == "SUCCESSFUL":
                    logger.debug("iscMosaic: Successfully acquired cluster lock for: %s", lockName)
                    try:
                        ...
                        self.__dbinv = list(self._wec.keys())
                        try:
                        ...
                        logger.debug("iscMosaic: Attempting to release cluster lock for: %s", lockName)
                        ClusterLockUtils.unlock(clusterLock, False)
                        logger.debug("iscMosaic: Successfully released cluster lock for: %s", lockName)
