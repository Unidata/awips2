...
import numpy
import  tempfile, os, stat, getopt, sys, pickle, siteConfig
import LogStream, time, traceback, string, IrtAccess, urllib.request, urllib.parse, urllib.error, urllib.request, urllib.error, urllib.parse
    ...
    traffic routing information is old and possibly incorrect."""
        ...
        for addressE in destE:
            if addressE.tag == "doclip":
                for name, value in list(addressE.items()):
                    if name == "clip":
                        if value == "1":
                            doClip = 1
                        elif value == "0":
