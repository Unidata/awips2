...
# ------------- -------- --------- --------------------------------------------
# May 09, 2018  7293     randerso  Fixed casing of weather types
#
##
##
...
class TextProduct(TextRules.TextRules, SampleAnalysis.SampleAnalysis):
    ...
    def _mxmnValue(self, statDict, timeRange, argList):
        ...
        if maxval is None:
            ...
            maxString = repr(int(self.round(maxval, "Nearest", 1)))
        minval = self.getStats(statDict, "MinT")
        if minval is None:
            minString = "MM"
        else:
            minString = repr(int(self.round(minval, "Nearest", 1)))
    ...
    def _calcPeriodLabels(self, periods, colWidth, startPoint):
        fcstString = " " * startPoint
        dayNumString = " " * startPoint
        dateString = " " * startPoint

        for index in range(len(periods)):
            fcstString += "FCST".ljust(colWidth)
            dayNumString += self._periodNumString(index, periods[index][0]).ljust(colWidth)
            timeRange, label = periods[index]
            dateString += self._formatDate(timeRange).ljust(colWidth)
