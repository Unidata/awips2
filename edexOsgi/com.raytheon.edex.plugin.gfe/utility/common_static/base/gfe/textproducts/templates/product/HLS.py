...
#    Definition["debug"] = 1         #  turn on ALL debug messages
    ...
    def _determineHazardStates(self):
        ...
        self.debug_print("*"*80)
        keys = list(self._previousAdvisory.keys())
        keys.sort()
        for key in keys:
    ...
    def _determineHazards(self, segments):
        ...
        for hazard, segment in hazAreaList:
            ...
            if key not in list(hazardDict.keys()):
...
##        gmtHour = int(gmt)/100
    ...
    def _getTimeZoneList(self, areaList):
        ...
        for areaName in areaList:
            if areaName in list(self._areaDict.keys()):
                ...
                if type(timeZoneList) is bytes:  # a single value
...
##            stormReference = coordPtnMatch.group(11)
    ...
    def _calcReference(self, lat0, lon0, lat1, lon1):
        ...
        distMph_str = repr(int((distMph/10)*10))
...
class LegacyFormatter():
    ...
    def processHazards(self, hazardsList, isChangesHazards):
        ...
        if len(hazardsList) == 0:
            ...
            for (sortedAreas, hazards) in list(groupedHazards.items()):
            ...
            if status == "UPG":
                ...
                for action in groupedHazards[sortedAreas]:
                    ...
                    for hl in sortedHeadlines:
                        if hl in VTECTable.upgradeHazardsDict and \
    ...
    def _consolidateGroupedHazards(self, groupedHazards, isChangesHazards):
        ...
        for sortedAreas1, hazards1 in list(groupedHazards.items()):
            ...
            if not isChangesHazards:
                ...
                for (action, (hazard, upgrades)) in list(hazards1.items()):
...
#                 print "Done with merging EXA and CON"
            ...
            for sortedAreas2, hazards2 in list(newGroupedHazards.items()):
    ...
    def _createChangesTextParts(self, hazards):
        ...
        hazardTextParts = []
        for status, (sortedPhensigs, upgrades) in list(hazards.items()):
            ...
            if status == "CAN":
                ...
                if len(upgrades) > 0:
                    upgradeTextParts = []
                    for phensig, previousPhenSig in list(upgrades.items()):
