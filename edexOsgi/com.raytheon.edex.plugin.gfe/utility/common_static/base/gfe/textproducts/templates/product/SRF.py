...
# ------------    ----------    -----------    --------------------------
# 06/29/2020      DR20759       NFTF           Fix erroneous 600 AM issuance time.
#
##
##
...
##### SURF PERIOD AND ISSUANCE LISTS
    ...
    def _getSurfPeriod_additionalAreas(self):
        ...
        surfList = []
        for (combo, value) in self._combinedAreaDict.items():
            surfList += value["surfAreas"]
        areas = []
    ...
    def _getAdditionalAreas(self, component):
        ...
        elementDict = {}
        for (areaType, value) in areaDict.items():
            for entry in value["analysisList"]:
                element = entry[0]
        ...
        #  e.g. ("MaxT", ["SRF_586"])
        for (element, areaTypes) in elementDict.items():
            addAreaList = []
            for areaType in areaTypes:
                for (combo, value) in self._combinedAreaDict.items():
                    addAreaList += [value[areaType]]
    ...
    def _getAdditionalAnalysisList(self, component):
        ...
        for (key, value) in areaDict.items():
            analysisList += value["analysisList"]
        return analysisList
    ...
    def _getSurfPeriod_intersectWithAreas(self):
        ...
        intersectWithAreas = []
        for (key, value) in self._combinedAreaDict.items():
            if 'landSeaArea' in value:
                intersectWithAreas.append(value['landSeaArea'])
    ...
    def _setSurfAreaLabel(self, tree, node, phrase):
        ...
        useAreaType = None
        for (areaType, value) in areasDict.items():
...
### Generate Product
    ...
    def _determineTimeRanges(self, argDict):
        ...
        if self._periodCombining:
            self._definition["methodList"] = [self.combineComponentStats, self.assembleChildWords]
        else:
            self._definition["methodList"] = [self.assembleChildWords]

        # Calculate current times
        self._ddhhmmTime = self.getCurrentTime(argDict, "%d%H%M", shiftToLocal=0, stripLeading=0)
    ...
    def _getCombinedAdditionalAreas(self, argDict, components):
        ...
        for comp in components:
            areaTypes += list(self._additionalAreasDict(comp).keys())
    ...
    def _getCombinedSurfAreas(self, argDict, components, comboList, comboLabel):
        ...
        #print "\nLabelDict", labelDict
        # Make combinations
        for (surfLabel, surfAreas) in labelDict.items():
            if len(surfAreas) > 1:
...
#########        tide locations and tides.   
    ...
    def _uvi_setUp(self, tree, node):
        ...
        self._uviLines = uviProduct.split("\n")
        return self.DONE()
