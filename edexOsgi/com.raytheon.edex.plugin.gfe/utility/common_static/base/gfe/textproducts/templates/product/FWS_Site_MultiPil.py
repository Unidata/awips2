...
# ---------------------------------------------------------------------
#
# FWS_<site>_<MultiPil>
#
#  This file should not be edited by the site.
#  Site changes should go in FWS_<site>_Overrides for methods and
#       FWS_<site>_<MultiPil>_Definition to set up Product Definition Settings
#FWS_Site_MultiPil_Baseline.py
#
# ---------------------------------------------------------------------
#
# SOFTWARE HISTORY
...
# These statements get the class object for the national, regional, and
# site overrides class. The class and the module name (the file name) must
...
regionOverrides_modObject = importlib.import_module(regionOverrides)
siteOverrides_modObject = importlib.import_module(siteOverrides)
...
regionOverrides_classObject = getattr(regionOverrides_modObject,regionOverrides,None)
siteOverrides_classObject = getattr(siteOverrides_modObject,siteOverrides,None)
...
regionDefinition = getattr(regionOverrides_modObject,"Definition",{})
nationalDefinition = getattr(nationalOverrides_modObject,"Definition",{})

class TextProduct(
                  siteOverrides_classObject,
                  regionOverrides_classObject,
    ...
    # Get the VariableList if overridden in Region FWS Overrides
    regionVariableList = getattr(regionOverrides_modObject,"VariableList",{})
    if regionVariableList:
        VariableList = regionVariableList

    # Get the VariableList if overridden in Site FWS Overrides
    siteVariableList = getattr(siteOverrides_modObject,"VariableList",{})
    if siteVariableList:
        VariableList = siteVariableList

    # Definition overrides should really go in FWS_<site>_<MultiPil>_Definition
    # but may be put here for testing.
    # Most common would be need to set unique display name
    ##Definition["displayName"] = "Test_FWS_<MultiPil>"
