...
#    11/30/2018      7542          dgilling       Don't save CTAs when CAN-ing hazards
#                                                 with S significance.
#
    ...
    def hazardBodyText(self, hazardList, argDict):
      ...
      #
        ...
        if incTextFlag and bulletProd:
            for eachHazard in sortedHazardList:
                if 'prevText' not in eachHazard:
                    newBullets = self._bulletDict().get(eachHazard['phen']).split(",")
                        ...
                        if re.search("\* " + bullet + "\.\.\.", segmentText, flags=re.IGNORECASE) is None:
                            ...
                            for i in range(start,end):
                                if (re.search("\* " + self._bulletOrder()[i] + "\.\.\.", segmentText, flags=re.IGNORECASE) is not None) and bulletFlag:
                                    ...
                                    segmentText = "* " + bullet.upper() + "...|* Enter bullet text *|\n\n* " + self._bulletOrder()[i] + "...".join(segmentTextSplit)
                                    bulletFlag = 0
                            if bulletFlag:
                                ...
                                segmentText = "\n" + "* " + bullet.upper() + "...|* Enter bullet text *|\n\nPRECAUTIONARY/PREPAREDNESS ACTIONS...".join(segmentTextSplit)
                                bulletFlag = 0
        #
        ...
        if incTextFlag and bulletProd:
            ...
            for eachHazard in sortedHazardList:
                if eachHazard['act'] not in ["CAN","EXP"]:
                    saveBullets = self._bulletDict().get(eachHazard['phen']).split(",")
            ...
            for eachHazard in sortedHazardList:
                if eachHazard['act'] in ["CAN","EXP"]:
                    canBullets = self._bulletDict().get(eachHazard['phen']).split(",")
            ...
            for bullet in removeBulletList:
                if re.search("\* "+ bullet + "\.\.\.", segmentText, flags=re.IGNORECASE) is not None: 
                    ...
                    segmentTextSplit2 = segmentTextSplit[1].split("*", 1)
                    ...
                    segmentText = "".join(segmentTextSplit)
            if removeBulletList:
                segmentText = "|*\n" + segmentText + "*|"
...
##            print "bulletFlag is: ",bulletFlag
    ...
    def overviewText(self, hazardList, pil):
        ...
        for each in hazardList:
            if ('prevOverviewText' in each and
                'pil' in each and
                'endTime' in each and
                'act' in each):
                if (each['pil'] == pil and
        ...
        if self.__procCTA is None:
            ...
            for (k, v) in d.items():
                ...
                for it in items:
                    ...
                    for cta in ctaParas:
                        self.__procCTA.append((k, cta.replace(' ', '')))
            d = ctao.ctaPilDict()
            for (k, v) in d.items():
                func = v
                items = func()
            ...
            for it in ctas:
                ...
                ctaParas = self.convertSingleParas(it)
                for cta in ctaParas:
                    self.__procCTA.append(("GENERIC", cta.replace(' ', '')))
