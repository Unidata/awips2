    ...
    def _byStateTextFromCountyTuple(self, countyTuple, lineLength,
      colWidth, mainFormatSingular, mainFormatPlural, subFormat):
        ...
        for state, partStateNames in geoList:
            ...
            header = header.replace('{area}', state)
            header = header.replace('{number}', str(ccount))
            if state == "Louisiana":
                header = header.replace('{placeType}', "parish")
                header = header.replace('{placeTypes}', "parishes")
            else:
                header = header.replace('{placeType}', "county")
                header = header.replace('{placeTypes}', "counties")
            ...
            for partState, names in partStateNames:
                subheader = subFormat
                subheader = subheader.replace('{area}', partState)
                result += subheader + '\n\n'
      ...
      colWidth, mainFormatSingular, mainFormatPlural):
        ...
        for partState, names in geoList:
            ...
            if ccount > 1:
                ...
                header = mainFormatSingular
            header = header.replace('{area}', partState)
            header = header.replace(header, '{number}', str(ccount))
            if "Louisiana" in partState:
                header = header.replace('{placeType}', "parish")
                header = header.replace('{placeTypes}', "parishes")
            else:
                header = header.replace('{placeType}', "county")
