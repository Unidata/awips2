...
#
# VCModule for IceAccum ISC Parm
#   
# SOFTWARE HISTORY
#
# Date          Ticket#  Engineer  Description
# ------------- -------- --------- --------------------------------------------
# Jun 24, 2021  8541     randerso  Initial Creation.
#
##
...
class VCParm:
    ...
    def getInventory(self, IceAccum, IceAccum_ISC):
        # all Fcst grids get included if they overlap an ISC grid
        rval = []
        for tr in IceAccum:
            lst = []
            for itr in IceAccum_ISC:
        ...
        # goes in as well
        for tr in IceAccum_ISC:
            for ntr in self.splitTR(tr, IceAccum):
                for ftr in IceAccum:
    ...
    def calcGrid(self, IceAccum, IceAccum_ISC):
        # If getInventory() works right
        # we either get 0 or 1 IceAccum grids.
        # And if 0 then only 1 IceAccum_ISC
        if len(IceAccum) == 0:
            return IceAccum_ISC[0][1]
        rval = IceAccum[0][1] * 0.0
        for i in IceAccum_ISC:
            rval = rval + i[1] * self.poverlap(i[0], IceAccum[0][0])
        return rval

    def calcHistory(self, IceAccum_ISC):
        rval = []
        for t in IceAccum_ISC:
            for i in t[1]:
                rval.append(i)
        return rval
    ...
    def getWEInfo(self):
        ...
        return (("IceAccum", SCALAR, "in", "Ice Accumulation", 5.0, 0.0, 2, YES),
                ("ISC", "V"), (3600, 3600, 0))
