...
# Dec 92, 2019  7881     randerso  Added additional None checks in calcMax/Min methods
#                                  Changed calcQPF to use tp6hr to better since
#                                  tp is not defined in the base parameter info file    
#
##
...
##--------------------------------------------------------------------------
## Module that calculates surface weather elements from GFS model
## output.
##
##--------------------------------------------------------------------------
class GFSForecaster(Forecaster):
    def __init__(self):
        Forecaster.__init__(self, "GFS", "GFS")
...
####-------------------------------------------------------------------------
    def calcQPF(self, tp6hr_SFC):
        qpf = tp6hr_SFC / 25.4   # convert from millimeters to inches
        return qpf
...
####-------------------------------------------------------------------------
    ...
    def calcSky(self, rh_c, gh_c, topo, p_SFC, rh_BL030, rh_BL3060, rh_BL6090,
        ...
        #  See which boundary layer levels have pressures > lowest "significant"
...
####-------------------------------------------------------------------------
    def calcSnowLevel(self, gh_c, t_c, rh_c):
        # Only use the levels that are >= freezing (plus one level)
...
##########################################################
    ...
    ### Given a grid of temperature in Celsius, this method computes
    ...
    def baseSnowRatio(self, tGrid):
        ...
        # Initialize the coefficient grids
        ...
        # Do the calculation using the grids of spline coefficients
...
###########################################################
    def calcSnowRatio(self, gh_c, t_c, rh_c, pvv_c, gh_MB925, gh_MB800, gh_MB850, gh_MB750, gh_MB700, gh_MB650, gh_MB600, gh_MB550):
        ...
        #for i in range(len(gh_c)):
            ...
            pvvAvg[i][less(pvvAvg[i], 0.0)] = 0.0
            # Scale vertical velocity as a function of the square of RH.
            # This scaling will effectively negate a snowratio contribution in
            # layers that are dry.
            pvvAvg[i] = where(less(avgRH, 80.0), pvvAvg[i] * ((avgRH * avgRH) / 6400.0), pvvAvg[i])
            pvvSum = pvvSum + pvvAvg[i]
...
####-------------------------------------------------------------------------
    def calcTransWind(self, MixHgt, wind_c, gh_c, topo):
        ...
        mmask = mask + 0.0001
        # calculate the average value in the mixed layer
