...
##-------------------------------------------------------------------------
    def calcPoP(self, rh_c, pvv_MB700):
...
##-------------------------------------------------------------------------
    def calcWind(self, wind_FHAG10, wind_MB850, wind_MB800, wind_MB750, wind_MB700, wind_MB650, topo):
        ...
        winddir = wind_FHAG10[1]         # get the wind direction

        mag = where(logical_and(greater_equal(topo, 1327), less(topo, 1828)), wind_MB850[0] * 1.94, mag)
        winddir = where(logical_and(greater_equal(topo, 1327), less(topo, 1828)), wind_MB850[1], winddir)

        mag = where(logical_and(greater_equal(topo, 1828), less(topo, 2438)), wind_MB800[0] * 1.94, mag)
        winddir = where(logical_and(greater_equal(topo, 1828), less(topo, 2438)), wind_MB800[1], winddir)

        mag = where(logical_and(greater_equal(topo, 2438), less(topo, 3048)), wind_MB750[0] * 1.94, mag)
        winddir = where(logical_and(greater_equal(topo, 2438), less(topo, 3048)), wind_MB750[1], winddir)

        mag = where(logical_and(greater_equal(topo, 3048), less(topo, 3657)), wind_MB700[0] * 1.94, mag)
        winddir = where(logical_and(greater_equal(topo, 3048), less(topo, 3657)), wind_MB700[1], winddir)

        mag = where(logical_and(greater_equal(topo, 3657), less(topo, 4267)), wind_MB650[0] * 1.94, mag)
        winddir = where(logical_and(greater_equal(topo, 3657), less(topo, 4267)), wind_MB650[1], winddir)
