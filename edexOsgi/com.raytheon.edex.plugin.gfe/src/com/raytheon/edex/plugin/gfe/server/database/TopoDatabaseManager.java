 ...
 * Feb 15, 2013  1638     mschenke  Deleted topo edex plugin, moved code into
 *                                  common topo
 * Jun 13, 2013  2044     randerso  Refactored to use non-singleton
 *                                  GridParmManager, code cleanup
 ...
 *                                  Standarad Terrain WA
 * Sep 23, 2021  8608     mapeters  Add metadata id handling
 * Jan 18, 2022  8740     randerso  Log exceptions in fileList()
 *
 * </pre>
 *
 * @author njensen
...
public class TopoDatabaseManager {
    ...
    public TopoDatabaseManager(String siteID, IFPServerConfig config)
        ...
        try {
            ...
            DatabaseID dbId = dao.getDatabaseId(
                    new DatabaseID(siteID, DataType.GRID, "EditTopo", "Topo"));

            SimpleModelConfig smc = new SimpleModelConfig(siteID, "GRID",
                    "EditTopo", "Topo", gloc.getProjection().getProjectionID(),
                    true, false, 1, 0);
            smc.grids = Arrays.asList(new SimpleGridParmConfig("Topo", "Scalar",
                    "ft", "Topography", 50000.0f, -32000.0f, 1, true,
     ...
     */
    public ServerResponse<ScalarGridSlice> getTopoData(
            final GridLocation gloc) {
        ServerResponse<ScalarGridSlice> sr = new ServerResponse<>();
        ...
        try {
            ...
            String msg = "Unable to retrieve topo record for " + gloc + ": ";
    ...
    private FloatDataRecord processTopography(final GridLocation gloc,
        ...
        float[] heights = TopoQuery.getInstance()
                .getHeight(MapUtil.getGridGeometry(gloc));
        UnitConverter cvt = SI.METRE.getConverterTo(USCustomary.FOOT);
    ...
    private void createDiskCache(final GridLocation gloc) throws GfeException {
        ...
        if (needToCreate) {
            ...
            ServerResponse<?> sr = this.topoDatabase.saveGridData(parmId,
                    timeRange, Arrays.asList(record),
                    new WsId(null, "EDEX", "GFE"));
            if (sr.isOkay()) {
    ...
    /**
     * Returns a list of TOPO records in the disk cache. If an exception occurs
     * when querying for the file names, an empty list is just returned.
     *
     * @return the TOPO record file names (guaranteed non-null and unmodifiable)
     */
    private List<String> fileList() {
        List<String> rval = Collections.emptyList();

        try {
            String[] names = dataStore.getDatasets("/");
            rval = names == null ? List.of() : List.of(names);
        } catch (FileNotFoundException e) {
            statusHandler.warn("No Topo data found for " + config.getSiteID(),
                    e);
        } catch (StorageException e) {
            statusHandler.error(
                    "Error getting Topo dataset list for " + config.getSiteID(),
                    e);
        }
    ...
    private String calcGroupName(final GridLocation gloc) {
        ...
        StringBuilder o = new StringBuilder("TOPO_").append(gloc.getNx())
                .append('_').append(gloc.getNy()).append('_').append(proj)
    ...
    public void saveTopoData(final GridLocation gloc, final IGridSlice slice,
            GFERecord grec) throws IllegalArgumentException, GfeException {
        ...
        String name = calcGroupName(gloc);
        FloatDataRecord fdr = new FloatDataRecord("Data", name, data, 2,
                new long[] { gloc.getNx().longValue(),
                        gloc.getNy().longValue() });

        writeTopoData(gloc, fdr, grec);
    }
    ...
    private void writeTopoData(final GridLocation gloc,
            final FloatDataRecord fdr, GFERecord grec) throws GfeException {
        try {
            IMetadataIdentifier metaId = grec == null
                    ? new NoMetadataIdentifier()
                    : new DataUriMetadataIdentifier(grec);
            dataStore.addDataRecord(fdr, metaId);
