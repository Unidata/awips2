 ...
 * Dec 14, 2020  8303     randerso  Added additional error logging
 *
 * </pre>
 *
 * @author randerso
 */
public class SendWFOMessageHandler extends BaseGfeRequestHandler
        implements IRequestHandler<SendWFOMessageRequest> {
    private static final Logger logger = LoggerFactory
            .getLogger(SendWFOMessageHandler.class);

    private static final String SCRIPT_PATH = LocalizationUtil.join("gfe",
            "python", "isc", "sendWFOMessage.py");
    ...
    public ServerResponse<?> handleRequest(SendWFOMessageRequest request) {
        ...
        try {
            ...
            try (PythonScript script = new PythonScript(scriptPath, includePath,
                    SendWFOMessageHandler.class.getClassLoader())) {
                try {
