 ...
 * Feb 16, 2022  8608     mapeters  Add "traceId" to excluded fields
 *
 * </pre>
 *
 * @author nabowle
...
public abstract class AbstractNetcdfDecoder {
    ...
    static {
        ...
        /*
         * Excluded since different records need different trace IDs for data
         * storage auditing to work correctly.
         */
        fields.add("traceId");
    ...
    public Iterator<NetcdfRecordInfo> split(File file) {
        ...
        try {
            ...
            for (NetcdfProductDescriptions descs : theDescs) {
                ...
                } catch (NetcdfDecoderException
                        | InvalidDescriptionException e) {
                    logger.error("Error generating records from a description.",
                            e);
                }
            }
            ...
            if (desc.matches(netcdfFile, this.logger)) {
                if (desc.getData() != null) {
                    if (desc.getData().isPresent(netcdfFile)) {
                        ...
                        logger.debug(
                                "description skipped because the data variable "
                ...
                if (desc.isDebug() && this.logger.isDebugEnabled()) {
                    logger.debug(
                            "description skipped because the file does not match the configured pattern(s).");
                }
            }
        }
        return records;
    ...
    protected List<NetcdfRecordInfo> processDeferredDescriptions(
        ...
        for (NetcdfRecordInfo info : currentInfos) {
            ...
            Variable dataVar = NetcdfDecoderUtils.getDataVariable(info,
                    netcdfFile);
            Parameter param = (Parameter) info.getBeanMap()
                    .get(ParameterDescription.PARAMETER_KEY);
                    ...
                    if (deferred.isPresent(netcdfFile)) {
                        ...
                        while (iter.hasNext()) {
                            ...
                            for (int i = 0; i < length; i++) {
                                if (isNumeric) {
                                    fieldVal = deferred.getNumber(netcdfFile,
                                            i);
                                } else {
                                    fieldVal = deferred.getString(netcdfFile,
                                            i);
                                }
                        ...
                        logger.warn(
                                "Unable to decode deferred field description: "
                                        + deferred.getName() + " for field "
                                        + entry.getKey());
                    }
                }
            }
     ...
     * @throws NetcdfDecoderException
     */
    protected void extractLevel(NetcdfFile netcdfFile, NetcdfRecordInfo info,
            Variable dataVar, List<NetcdfRecordInfo> outInfos)
            throws InvalidDescriptionException, NetcdfDecoderException {
     ...
     * @throws NetcdfDecoderException
     */
    protected void extractDataTime(NetcdfFile netcdfFile, NetcdfRecordInfo info,
            Variable dataVar, List<NetcdfRecordInfo> outInfos)
            throws InvalidDescriptionException, NetcdfDecoderException {
        ...
        if (dataTimeDesc != null) {
            DataTime dataTime;
            Dimension dataTimeDim = NetcdfDecoderUtils
                    .getDataTimeDimension(dataVar, dataTimeDesc);
            int numDataTimes = dataTimeDim == null ? 1
                    : dataTimeDim.getLength();
            if (numDataTimes > 1) {
                NetcdfRecordInfo infoCopy;
    ...
    public PersistablePluginDataObject[] decode(NetcdfRecordInfo recordInfo) {
        ...
        try {
            ...
            logger.error("Unable to read the file, {}",
                    recordInfo.getFile().getAbsolutePath(), e);
        } finally {
            if (netcdfFile != null) {
                try {
                    netcdfFile.close();
    ...
    protected void extractData(NetcdfFile netcdfFile, NetcdfRecordInfo info)
        ...
        Variable dataVariable = netcdfFile
                .findVariable(dataDesc.getVariable().getName());
        if (dataVariable == null) {
            throw new NetcdfDecoderException("Cannot find the data variable.");
        }

        Section sect = NetcdfDecoderUtils.getRecordSection(dataVariable,
                info.getDimensionIndexMap());

        Number fillValue = NetcdfDecoderUtils.getNoDataValue(dataVariable,
                null);
