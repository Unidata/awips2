 ...
 * Aug 03, 2016  #5747     dgilling    Move backups from edex_static to non-localization 
 *                                     path.
 * 
 * </pre>
 * 
...
public class ActiveTableBackup {
    ...
    private static BlockingQueue<BackupRequest> queue = new LinkedBlockingQueue<>(
            QUEUE_LIMIT);
    ...
    private static Runnable target = new Runnable() {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    final BackupRequest request = queue.take();
                    timer.reset();
                    timer.start();

                    String siteId = EDEXUtil.getEdexSite();
                    Path backupDir = Paths.get(EDEXUtil.getEdexData(),
                            "activetable", siteId, "backup");
                    Files.createDirectories(backupDir, PosixFilePermissions
                    ...
                    try {
                        ...
                        HashMap<String, Object> args = new HashMap<>(4, 1.0f);
                    ...
                    if ((now - lastPurge) > TimeUtil.MILLIS_PER_DAY) {
                        ...
                        for (File file : backupDir.toFile().listFiles(filter)) {
