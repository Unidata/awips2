...
class ActiveTableRecord(object):
    ...
    def __init__(self, javaRecord, state="Decoded"):
        self.atr = javaRecord
        self.state = state
        self.pil = ActiveTableVtec.remapPil(
            self.atr.getPhen(), self.atr.getSig(), self.atr.getPil())
    ...
    def __setitem__(self, key, value):
        if key == 'vtecstr':
            ...
            start = Date(int(value * 1000))
            self.atr.setStartTime(start)
        elif key == 'endTime':
            end = Date(int(value * 1000))
            self.atr.setEndTime(end)
        elif key == 'purgeTime':
            purge = Date(int(value * 1000))
            self.atr.setPurgeTime(purge)
        elif key == 'issueTime':
            issue = Date(int(value * 1000))
