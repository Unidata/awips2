...
//    private static final String TEMP_FROM_TENTHS = "tempFromTenths";
    ...
    public static FSSObsRecord fromMetarRecord(PointDataContainer container) throws JAXBException{
        ...
        fssr.setDataTime(new DataTime(new Date(pdv.getNumber(TIME_OBS).longValue())));
...
//        fssr.setPressureAltimeter(pa.floatValue());
        ...
        if(pdv.getNumber(VERT_VISIBILITY).floatValue() >=0 && pdv.getNumber(VERT_VISIBILITY).floatValue()< 1e20f && pdv.getNumber(VERT_VISIBILITY).floatValue() != MISSING){
            fssr.setCeiling(pdv.getNumber(VERT_VISIBILITY).floatValue() / 100f);
        }else{    
        // in feet
            float ceiling = FSSObsUtils.findMetarCeilingFromLayers(skyCov, levels);
...
//        fssr.setWindDir(Float.valueOf(pdv.getString(WIND_DIR_STR)));
    ...
    private static final String[] MESOWEST_PARAMS = { PRESS_CHANGE3_HOUR,
            ...
            STATION_NAME, STORAGE_TYPE, ELEVATION,
    ...
    public static FSSObsRecord fromMaritimeRecord(PointDataContainer container) throws JAXBException {
        container.setCurrentSz(container.getAllocatedSz());
        PointDataView pdv = container.readRandom(0);
