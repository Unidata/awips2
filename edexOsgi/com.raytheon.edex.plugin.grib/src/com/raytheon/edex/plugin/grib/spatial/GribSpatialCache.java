...
import com.raytheon.uf.common.grib.GribCoverageNameLookup;
 ...
 * Jul 21, 2014  3373     bclement      JAXB managers only live during
 *                                      initializeGrids()
 ...
 * Feb 26, 2016  5414     rjpeter       Fix subgrids along boundary and add
 *                                      check for disabled subgrid.
 ...
 * Apr 23, 2018  6981     bsteffen      Default to no shift west in subgrids
 *                                      that specify a center point.
 * Apr 23, 2020  21070    dhaines       Add method purgeCaches for piggybacking 
 *                                      purging of orphaned grid coverages onto the 
 *                                      existing grid info purge job in GridDao.
 * Nov 11, 2020  8278     rjpeter       Updated purgeCaches to no longer change instance
 *                                      reference.
 * 
 * </pre>
 * 
 * @author bphillip
 */
public class GribSpatialCache implements GribCoverageNameLookup {
    ...
    private static final String GRIB_ROOT_PATH = IPathManager.SEPARATOR + "grib"
            + IPathManager.SEPARATOR;

    private static final String GRIDS_PATH = GRIB_ROOT_PATH + "grids";

    private static final String SUBGRIDS_PATH = GRIB_ROOT_PATH + "subgrids";

    private static final String DEFAULT_CENTER_POINT_PATH = GRIB_ROOT_PATH
            + "defaultSubGridCenterPoint.xml";
    ...
    private boolean shiftSubGridWest = false;

    static {
        float minCoveragePercent = 20;
        try {
            minCoveragePercent = Float.parseFloat(
                    System.getProperty("SUB_GRID_COVERAGE_PERCENT", "20"));
            ...
            statusHandler.error(
                    "SUB_GRID_COVERAGE_PERCENT must be a value from 0-100.  Defaulting to 20",
                    e);
        }
    ...
    private GribSpatialCache() {
        gridNameMap = new HashMap<>();
        spatialNameMap = new HashMap<>();
        definedSubGridMap = new HashMap<>();
        subGridCoverageMap = new HashMap<>();
    ...
    public GridCoverage getGrid(GridCoverage coverage) {
        GridCoverage rval = GridCoverageLookup.getInstance()
                .getCoverage(coverage, false);
        if (rval == null) {
            scanFiles();
            rval = GridCoverageLookup.getInstance().getCoverage(coverage,
                    false);
        }
        return rval;
    }
    ...
    public GridCoverage getSubGridCoverage(String modelName,
        ...
        GridCoverage subGrid = subGridCoverageMap
                .get(subGridKey(modelName, coverage));
        if (subGrid == null) {
            if (loadSubGrid(modelName, coverage)) {
    ...
    private boolean loadSubGrid(String modelName, GridCoverage coverage) {
        SubGridDef subGridDef = subGridDefMap.getSubGridDef(modelName,
                coverage);

        try {
            if ((subGridDef != null) && !subGridDef.isSubGridDisabled()) {
                ...
                double coveragePercent = ((double) subGrid.getNX()
                        * subGrid.getNY()) / (nx * ny);
                if (coveragePercent < MIN_SUBGRID_COVERAGE) {
                    statusHandler.warn(String.format(
                            "Rejecting data from model [%s]. SubGrid only covers %.2f%%, minimum coverage is %.2f%%",
                ...
                if ((wrapCount > 0) && (wrapCount < coverage.getNx())) {
                    ...
                    insertSubGrib(modelName, coverage, subGridCoverage,
                            subGrid);
                } else {
                    return false;
                }
            ...
            statusHandler.error(
                    "Failed to generate sub grid for model " + modelName, e);
            return false;
        }
    ...
    private SubGridDef loadSubGridDef(
        ...
        if (f.length() > 0) {
            try {
                ...
                boolean noGrid = StringUtil
                        .isEmptyString(rval.getReferenceGrid());
                if (noGrid) {
                    ...
                    } else if (rval.getShiftWest() == null) {
                        rval.setShiftWest(false);
                    }
                }
            } catch (SerializationException e) {
                statusHandler.error("Failed reading sub grid file: " + filePath,
                        e);
            }
        }
    ...
    /**
     * scan the grib grid definition for changes, this will only scan if we have
     * not scanned in the last 60 seconds.
     * 
     * @param force
     * @return
     */
     ...
     * @return
     */
    private synchronized void scanFiles(boolean force) {
        if (!force && (fileScanTime + TimeUtil.MILLIS_PER_MINUTE) > System
                .currentTimeMillis()) {
            return;
        }
        FileDataList currentFDL = generateFileDataList();
        fileScanTime = System.currentTimeMillis();
        if (force || !currentFDL.equals(this.fileDataList)) {
    ...
    private void initializeGrids(FileDataList fdl) {
        ...
        try {
            subGridDefJaxb = new SingleTypeJAXBManager<>(true,
                    SubGridDef.class);
            gridCovJaxb = new SingleTypeJAXBManager<>(true, GridCoverage.class);
    ...
    private FileDataList generateFileDataList() {
        ...
        fileList.addCoverageFiles(pm.listFiles(contexts, GRIDS_PATH,
                new String[] { "xml" }, true, true));
        fileList.addSubGridFiles(pm.listFiles(contexts, SUBGRIDS_PATH,
                new String[] { "xml" }, true, true));
    ...
    private Coordinate getDefaultSubGridCenterPoint() throws Exception {
        ...
        ILocalizationFile defaultSubGridLocationFile = pm
                .getStaticLocalizationFile(DEFAULT_CENTER_POINT_PATH);
        SingleTypeJAXBManager<DefaultSubGridCenterPoint> subGridCenterJaxb = new SingleTypeJAXBManager<>(
                DefaultSubGridCenterPoint.class);
        if ((defaultSubGridLocationFile != null)
                && defaultSubGridLocationFile.exists()) {
            try (InputStream is = defaultSubGridLocationFile
                    .openInputStream()) {
                ...
                if ((defaultSubGridLocation != null)
                        ...
                        && (defaultSubGridLocation
                                .getCenterLongitude() != null)) {
                ...
                statusHandler
                        .error("Unable to load default sub grid location from file: "
        ...
        if (defaultCenterPoint == null) {
            ...
            statusHandler.info("Default sub grid location is wfo center point ["
    ...
    private class SubGridMapping {
        private Map<String, SubGridDef> subGridsByCoverageName = new HashMap<>();

        private Map<String, SubGridModelEntry> subGridsByModel = new HashMap<>();
        ...
        public SubGridDef getSubGridDef(String modelName,
                GridCoverage coverage) {
            SubGridDef rval = null;
    ...
    /**
     * Purges from the local instance of the GridCoverageLookup, then calls
     * scanFiles to regenerate any of crucial gridcoverages used for ingest that
     * may have been purged
     * 
     * @param idsToPurge
     */
    public synchronized void purgeCaches(Collection<Integer> idsToPurge) {
        GridCoverageLookup gcl = GridCoverageLookup.getInstance();
        gcl.purgeCaches(idsToPurge);
        statusHandler.info(
                "Reinitializing GribSpatialCache after completion of gridcoverage purge job...");

        // force everything to be reinitialized internally
        scanFiles(true);
    }
}
