...
public class AAGForecastGenerator {
    ...
    private static final UnitConverter SM_TO_KM = NonSI.MILE
            .getConverterTo(SI.KILOMETER);

    private static final UnitConverter KT_TO_MPH = NonSI.KNOT
            .getConverterTo(NonSI.MILES_PER_HOUR);

    private static final UnitConverter KT_TO_MPS = NonSI.KNOT
    ...
    private String[] getSkyText(String sky) {
        ...
        if (sky != null) {
            if (sky.length() <= 3) {
                 ...
                 */
                ceilPart = Integer
                        .toString(Integer.parseInt(sky.substring(3, 6)) * 100);
            }
        }
        switch (cldsPart) {
        case "OVC":
    ...
    private Map<String, String> getFriendlyStationNames(
        ...
        for (QueryResultRow row : qresult.getRows()) {
            rval.put(row.getColumn(0).toString(), row.getColumn(1).toString());
        }
        return rval;
    }
    ...
    public Map<String, String> generateForecasts(
        ...
        if (!aagParts.isEmpty()) {
            ...
            logger.info("Generated products for " + rval.size() + " stations");
        }
        return rval;
    }
}
