...
public class FFMPGeometryFactory extends AbstractDataPluginFactory {
    ...
    protected IGeometryData[] getGeometryData(IDataRequest request,
        ...
        // Ensures that all records have been added to the cache before
        // calculations
        ...
        try {
            ...
            throw new DataRetrievalException(
                    "Unable to create Geometry Data: sourceName: " + sourceName,
                    e);
        }
    ...
    private Map<Long, DefaultGeometryData> makeGeometryData(String sourceName,
        ...
        String siteKey = (String) request.getIdentifiers().get(SITE_KEY);
        String cwa = (String) request.getIdentifiers().get(WFO);
        Number accumulationTime = (Number) request.getIdentifiers().get(
                ACCUM_HRS);
        FFMPDataCache cache = getCache(cwa);
        ...
        HucLevelGeometriesFactory geomFactory = HucLevelGeometriesFactory
                .getInstance();
        Map<Long, Geometry> geomMap = geomFactory.getGeometries(getCache(cwa)
                .getTemplates(siteKey), siteKey, cwa, huc);
    ...
    private List<Long> getAvailableLocationPfafs(IDataRequest request) {
        ...
        return cache.getTemplates(siteKey).getHucKeyList(siteKey,
                FFMPRecord.ALL, domains);
    }
    ...
    public String[] getIdentifierValues(IDataRequest request,
        ...
        List<String> allowedIds = Arrays.asList(new String[] { SITE_KEY, WFO,
                DATA_KEY });
        List<String> disallowedIds = Arrays.asList(new String[] { HUC,
                ACCUM_HRS });
        if (allowedIds.contains(identifierKey)) {
            return getAvailableValues(request, identifierKey, String.class);
        } else if (disallowedIds.contains(identifierKey)) {
            throw new IncompatibleRequestException(String.format(
                    "%s does not support getting values for identifier \"%s\"",
                    request.getDatatype(), identifierKey));
        } else {
