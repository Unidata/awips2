...
public class FFMPGuidanceInterpolation {
    ...
    // all hours
    public List<Double> orderedHours = new ArrayList<>();

    private List<String> interpolationSources = new ArrayList<>();
    ...
    private String getSource(double i) {
        String rsource = null;
        List<SourceXML> sources = productRun
                .getGuidanceSourcesBySourceFamily(product, guidType);
    ...
    public void setInterpolationSources(double time) {
        ...
        // max
        if (!skip && previous != null && (time > previous)) {
     ...
     * @return
     */
    public List<String> getInterpolationSources() {
    ...
    public String getSource1() {
        if (!interpolationSources.isEmpty()) {
            return interpolationSources.get(0);
        } else {
            return null;
        }
