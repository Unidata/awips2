...
if test -n "$site"; then
    ca_cn=dv1-$site
    # Clients connect with "dv1" and not the full host name
    db_server_host=dv1
fi
...
usage() {
    ...
    echo "     server.tgz  - bundle for database server (Extract in /awips2/database/ssl on dv1.)"
    ...
    echo ""
    echo "  Options:"
    echo "   -c {name}   Specify the root certificate CN [dv1-\$SITE_IDENTIFIER]"
    echo "   -d {name}   Specify the database server host name [dv1-\$SITE_IDENTIFIER]"
