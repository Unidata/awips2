...
psql="${PSQLBINDIR}psql"
shp2pgsql="${PGBINDIR}shp2pgsql"
vacuumdb="${PGBINDIR}vacuumdb"

echo "  Importing `basename ${SHAPEFILE}` into ${SCHEMA}.${TABLE} ..."
if [ -n "$LOGFILE" ] ; then
    ${psql} -d ncep -U ${PGUSER} -q -p ${PGPORT} -c "
        DELETE FROM public.geometry_columns WHERE f_table_schema = '${SCHEMA}' AND f_table_name = '${TABLE}';
        DROP TABLE IF EXISTS ${SCHEMA}.${TABLE}
    " >> $LOGFILE 2>&1
    ${shp2pgsql} -s 4326 -g the_geom -I ${SHAPEFILE} ${SCHEMA}.${TABLE} 2>> $LOGFILE | ${psql} -d ncep -U ${PGUSER} -q -p ${PGPORT} -f -  >> $LOGFILE 2>&1
    ${psql} -d ncep -U ${PGUSER} -q -p ${PGPORT} -c "
    ...
    ${psql} -d ncep -U ${PGUSER} -q -p ${PGPORT} -c "
        ...
        DROP TABLE IF EXISTS ${SCHEMA}.${TABLE}
    "
    ${shp2pgsql} -s 4326 -g the_geom -I ${SHAPEFILE} ${SCHEMA}.${TABLE} | ${psql} -d ncep -U ${PGUSER} -q -p ${PGPORT} -f -
    ${psql} -d ncep -U ${PGUSER} -q -p ${PGPORT} -c "
        SELECT AddGeometryColumn('${SCHEMA}','${TABLE}','the_geom_0','4326',(SELECT type FROM public.geometry_columns WHERE f_table_schema='${SCHEMA}' and f_table_name='${TABLE}' and f_geometry_column='the_geom'),2);
        UPDATE ${SCHEMA}.${TABLE} SET the_geom_0=ST_Segmentize(the_geom,0.1);
        CREATE INDEX ${TABLE}_the_geom_0_gist ON ${SCHEMA}.${TABLE} USING gist(the_geom_0);
...
if [ -n "$LOGFILE" ] ; then
    ${vacuumdb} -d ncep -t ${SCHEMA}.${TABLE} -U ${PGUSER} -p ${PGPORT} -qz >> $LOGFILE 2>&1
else
    ${vacuumdb} -d ncep -t ${SCHEMA}.${TABLE} -U ${PGUSER} -p ${PGPORT} -qz
fi
