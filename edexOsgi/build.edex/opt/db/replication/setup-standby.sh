...
# Jan  6, 2025 2038205    jsebahar    Update DDS replication scripts to work with
#                                     PostgreSQL 12
#
##
...
db_rep_user='*.awips.noaa.gov' # for connecting to master
...
is_recovery=$(${as_awips} a2dbauth psql \
  -U "${db_superuser}" \
  --port=${local_port} \
  --db=metadata \
  --host="$(hostname -s)" \
