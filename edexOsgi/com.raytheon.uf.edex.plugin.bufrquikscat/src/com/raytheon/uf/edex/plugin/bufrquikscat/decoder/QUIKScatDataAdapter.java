...
public class QUIKScatDataAdapter extends BUFRPointDataAdapter<QUIKScatObs> {
     ...
     * @param pluginName
     */
    public QUIKScatDataAdapter(PointDataDescription pdd, PointDataPluginDao<QUIKScatObs> dao, String pluginName) {
        ...
        if (dataDoc != null) {
            ...
            if(obsData != null) {
                ...
                if(container != null) {
                    ...
                    view.setLong("validTime",obsData.getTimeObs().getTimeInMillis());
                    
                    double lat = getDouble(dataList.get(8), IDecoderConstants.VAL_MISSING);
                    double lon = getDouble(dataList.get(9), IDecoderConstants.VAL_MISSING);
                    ...
                    int rainFlag = getInt(dataList.get(10),IDecoderConstants.VAL_MISSING);
                    ...
                    // Wind data
                    float val = (float) getDouble(dataList.get(12), PDV_FILL_DBL);
                    if(val < 0) {
    ...
    private QUIKScatObs getHeaderData(List<IBUFRDataPacket> dataList) {
        ...
        if(obsTime != null) {
            ...
            int satId = getInt(dataList.get(SAT_ID_POS), IDecoderConstants.VAL_MISSING);
            int orbNo = getInt(dataList.get(ORBIT_N_POS), IDecoderConstants.VAL_MISSING);
            
            logger.debug("getHeaderData(" + satId + "," + orbNo + ")" + obsData.getDataTime());
