...
def _getProducts(direct):
    ...
    """
    try:
        plist = sorted(os.path.splitext(f)[0]
                 for f in os.listdir(direct)
                 if f.endswith('.cfg')
                 )
        with open(os.path.join(direct, 'DEFAULT')) as f:
            default = f.read().rstrip()
...
Strips leading and trailing blanks'''
    return [_f for _f in [s.strip() for s in strng.split(sep)] if _f]
        ...
        for tag in allitems:
            ...
            for k in opts:
                if k in ['items', 'labels']:
                    opts[k] = [_f for _f in [x.strip() for x in
                        opts[k].split(',')] if _f]
                else:
                    opts[k] = _guess(opts[k])
    ...
    try:
        ...
        if 'qc' in cp.sections():
            for x in ['currentwx', 'impact', 'climate']:
                if x in cp.options('qc'):
                    d['qc'][x] = int(cp.get('qc', x))
...
def getTafTemplate(ident, hour):
    fpath = os.path.join('etc', 'tafs', ident, '%02d.template' % hour)
...
def getAllSiteIds():
    ...
    return {k: sorted(v) for (k, v) in iddict.items()}
