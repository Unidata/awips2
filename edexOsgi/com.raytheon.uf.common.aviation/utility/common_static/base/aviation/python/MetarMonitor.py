...
#    Mar 05, 2018    7087          njensen        Tempo msgs always show time since tempo start
#    Sep 13, 2018    7445          tgurney        Log missing metar message at debug level
#
...
# For DR15153: use 'maxSize=0' to indicate that the latest record is to be retrieved
        ...
        if not metars:
            msg = 'Missing METAR'
            _Logger.debug('%s for %s', msg, self.info['sites']['metar'])
        else:
            rpt = metars[0]
        ...
        if 'ocnl' in g and g['ocnl']['type'] == 'TEMPO' and now < g['ocnl']['time']['to']:
            ...
            if delta >= 3600. or delta >= halflife:
                ...
                while (dt > tf):
                    ...
                    if d:
                        tdict.update(d)
                        # this gets the time since the tempo started
                        delta = now - tf
                        #
