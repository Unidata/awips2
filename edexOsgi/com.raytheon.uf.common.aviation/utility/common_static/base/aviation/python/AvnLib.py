...
# ------------- -------- --------- --------------------------------------------
# Aug 03, 2015 17540     zhao      Modified to make default issue time configurable
# Jan 16, 2018 6989      tgurney   Change AMD line indent to 5 spaces
...
def getFmtHeaderTime(bbb, t=None):
...
def getFmtValidTime(bbb, t=None, tafDuration=24, evtime=None):
...
def getIssueTime(bbb, t=None):
    if t is None:
        t = time.time()
    if not bbb or bbb[0] == ' ': # regular issue forecast
        itime = Avn.string2time('%s00' % getFmtValidTime(bbb, t)[:8])
...
def getFmtIssueTime(bbb, t=None):
    if t is None:
        t = time.time()
    return time.strftime('%d%H%MZ', time.gmtime(getIssueTime(bbb, t)))
...
def indentTaf(lines):
    taf = []
    lines = [_f for _f in [x.strip() for x in lines] if _f]
    if not lines:
        return taf
    k = 0
    if lines[0][:3] == 'TAF':
...
def formatRecForMAIN(rec, ident, bbb, itime=None, tafDuration=24, evtime=None):
    ...
    lst = [ident, getFmtIssueTime(bbb, itime),
           getFmtValidTime(bbb, itime, tafDuration, evtime)[4:]]
    if not rec:
...
def formatRecForOCNL(rec):
    ...
    start = time.gmtime(rec['time']['from'])
    end = list(time.gmtime(rec['time']['to']))
...
def _filterPeriods(bbb, periods, t, tafDuration):
    starttime = getValidTime(bbb, t)
...
def makeTafFromPeriods(ident, bbb, periods, t=None, tafDuration=24, evtime=None):
    ...
    inperiods = _filterPeriods(bbb, periods, t, tafDuration)
    try:
        p = next(inperiods)
        if getValidTime(bbb, t) < p['prev']['time']['from'] - 1800.0:
...
def fixTafSky(sky):
    ...
    nsky = []
    s = sky['str']
    if s.find('CLR') != -1 or s.find('SKC') != -1:
...
def getTafPeriods(taf):
    ...
    return list(zip(indices, periods))
