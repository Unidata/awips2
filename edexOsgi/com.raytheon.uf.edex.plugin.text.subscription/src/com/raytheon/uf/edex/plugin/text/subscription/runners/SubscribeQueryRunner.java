...
public class SubscribeQueryRunner extends ASubscribeRunner {
    ...
    /**
     * Filters the list of records received, limiting to those that match the
     * trigger condition.
     * 
     * @param records
     ...
     */
    private List<SubscriptionRecord> filterRecords(
    ...
    /**
     * Packages the subscriptions for return to the client. Each record is
     * bundled within the value of a {@link Property} object with a name of
     * <em>subscription</em>.
     * 
     * @param records
     *            the list of records to bundle
     * 
        ...
        if (records.size() == 0) {
            ...
            for (SubscriptionRecord record : records) {
                try {
                    ...
                    logger.warn("Unable to serialize subscription record "
                            + record.toString() + " - skipping", e);
                }
            }
        }
        return retVal;
    ...
    /**
     * Modifies the list of {@link Property} objects by removing the
     * <em>trigger</em> property and adding a <em>active</em> property with a
     * value of <em>TRUE</em>.
     * 
     * @param properties
     *            the list of properties to modify
     * 
