 ...
 * Dec  7, 2018 7674       tgurney     decodeTrigger() check for null prodId
 *
 * </pre>
 *
 * @author mfegan
    ...
    private List<String> decodeTrigger(Object event) throws TriggerException {
        ...
        case DATA:
            if (event != null) {
                ...
                if (event instanceof DataURINotificationMessage) {
                    ...
                    logger.warn(
                            "notification contained invalid message, type was "
                ...
                if (event instanceof String) {
                    ...
                    for (PluginDataObject pdo : pdos) {
                        try {
                            ...
                            logger.warn(
                                    "Unable to extract product information from ["
                    ...
                    try {
                        String prodID = ReflectionUtil.getter(String.class, pdo,
                                "productId");
                        if (logger.isDebugEnabled()) {
                            logger.debug("Processing trigger: " + prodID
                                    + ", class = "
        ...
        default:
            throw new TriggerException(
                    "Invalid scriptrunner type [" + this.type + "] configured");
        }
        if (logger.isDebugEnabled()) {
    ...
    private void executeScript(SubscriptionRecord record, String trigger) {
        ...
        if (script != null) {
            ...
            if (StringUtil.isEmptyString(path)
                ...
                logger.warn(
                        "Unable to execute script for " + record.toString());
                return;
            }
        ...
        if ("ldad".equals(runner.toLowerCase())
                || "pil".equals(runner.toLowerCase())) {
