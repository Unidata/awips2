 ...
 * Apr 20, 2017 5569       tgurney     Change endpoint to one inside radar
 *                                     plugin
 * Jul 18, 2017 5569       tgurney     sendAsync
 *
 * </pre>
 *
 * @author dgilling
...
public class RadarEdexTextProductUtil {
    ...
    private static final String TEXT_ENDPOINT = "radarTextProductRoute";
    ...
    public static void storeTextProduct(AFOSProductId afosId, WMOHeader wmoId,
        ...
        if (wmoId != null) {
            isValid = wmoId.isValid();
        }
        try {
            if (afosId != null && text != null && !afosId.toString().isEmpty()
                    && !text.isEmpty()) {
                StdTextProduct textProd;
                if (operationalMode) {
                    textProd = new OperationalStdTextProduct();
                } else {
                    textProd = new PracticeStdTextProduct();
                }
                if (!isValid) {
                    textProd.setWmoid(" ");
                    textProd.setSite(" ");
                ...
                EDEXUtil.getMessageProducer().sendAsync(TEXT_ENDPOINT,
                        textProd);
            }
        } catch (Exception e) {
