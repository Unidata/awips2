...
public class Level3BaseRadar {
    ...
    private void parseCommandParameterMessage() throws IOException,
            MalformedDataException {
        int divider = theRadarData.readShort();
        if ((divider != -1)) {
        ...
        theRadarData.readShort();  // skip version number
        theRadarData.readShort();  // skip block length

        byte[] buf = RadarUtil.subArray(theRawRadarByteArray, 24, 46);
        DataInputStream in = new DataInputStream(new ByteArrayInputStream(buf));
