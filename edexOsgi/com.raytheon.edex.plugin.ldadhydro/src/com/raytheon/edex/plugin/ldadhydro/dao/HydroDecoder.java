 ...
 * Oct 16, 2013  16685    M.Porricelli  Add error checking for date format
 * Jul 23, 2014  3410     bclement      location changed to floats
 * Dec 17, 2015  5166     kbisanz       Update logging to use SLF4J by adding
...
public class HydroDecoder<E> implements IBinaryDecoder {
    ...
    public PluginDataObject[] decode(byte[] data) throws DecoderException {
        /*
         * TODO use inheritance for code that is shared with other ldad decoders
         * (ManualDecoder and MesonetDecoder)
         */
        ...
        if (data != null) {
            ...
            String filePath = "ldad" + IPathManager.SEPARATOR
                    + "ldadUnitsMap.txt";
            try {
                ILocalizationFile confile = pathMgr.getLocalizationFile(
                        commonStaticBase, filePath);
                if (confile != null && confile.exists()) {
                    try (InputStream is = confile.openInputStream()) {
                        configFile.load(is);
                    }
                } else {
