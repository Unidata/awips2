 ...
 * Mar 26, 2019  21070    dhaines     Add method purgeCaches for piggybacking 
...
public class GridCoverageLookup {
    ...
    /**
     * Purges records from both caches with ids supplied
     * @param idsToPurge
     */
    public void purgeCaches(Collection<Integer> idsToPurge) {
        int i2cDels = 0;
        int c2iDels = 0;
        for (Integer id : idsToPurge) {
            GridCoverage gc = null;
            if (idToCoverage.containsKey(id)) {
                gc = idToCoverage.get(id);
                idToCoverage.remove(id);
                i2cDels++;
            }
            if (gc != null && coverageToId.containsKey(gc)) {
                coverageToId.remove(gc);
                c2iDels++;
            }
        }
        statusHandler.info("Purged " + i2cDels + " coverages from cache idToCoverage and " 
            + c2iDels + " coverages from cache coverageToId.");
    }
}
