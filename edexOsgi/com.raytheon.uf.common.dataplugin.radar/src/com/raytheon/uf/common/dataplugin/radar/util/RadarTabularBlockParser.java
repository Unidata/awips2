...
public class RadarTabularBlockParser {
    ...
    public static void parseTabularBlock(TabularBlock tabularBlock,
            int productNum,
            Map<MapValues, Map<String, Map<MapValues, String>>> bigMap,
            Map<MapValues, Map<MapValues, String>> recordMap) {
        Map<String, Map<MapValues, String>> data = new HashMap<>();
         ...
         */
            ...
            for (int i = 1; i < eachPage.length - 1; i++) {
                String[] temp = RadarConstants.sti_headerPattern
                        .split(eachPage[i].trim());
                Matcher m = RadarConstants.sti_headerValues
                        .matcher(temp[0].trim());
                while (m.find()) {
                    sti_spd = m.group(1);
                    sti_dir = m.group(2);
                }
                temp = RadarConstants.sti_headerPattern
                        .split(eachPage[i].trim());
            ...
            String[] temp = eachPage[eachPage.length - 1].trim()
                    .split("DATA ")[1].split("\\s+");
         ...
         */
            ...
            for (int i = 1; i < eachPage.length - 1; i++) {
                String[] temp = RadarConstants.tvs_headerPattern
                        .split(eachPage[i].trim());
                temp[1] = temp[1].replaceFirst("\\s+\n\t\\s+\n\t\\s+", " ");
                Matcher m = RadarConstants.tvs_productValues
                        .matcher(temp[1].trim());
         ...
         */
            ...
            for (int i = 1; i < eachPage.length - 2; i++) {
                String[] temp = RadarConstants.vad_headerPattern
                        .split(eachPage[i].trim());
                temp[1] = temp[1].replaceFirst("\\s+\n\t\\s+", " ");
                Matcher m = RadarConstants.vad_productValues
                        .matcher(temp[1].trim());
                while (m.find()) {
         ...
         */
            ...
            for (int i = 1; i < eachPage.length; i++) {
                String[] temp = RadarConstants.meso_headerPattern
                        .split(eachPage[i].trim());
                temp[1] = temp[1].replaceFirst("\\)\\s+\n\t\\s+\n\t", " ");
                Matcher m = RadarConstants.meso_productValues
                        .matcher(temp[1].trim());
                while (m.find()) {
    ...
    private static void parseStiToken(String token, Map<MapValues, String> map,
        ...
        if (RadarConstants.NO_DATA.equals(trimToken)
                || "NEW".equals(trimToken)) {
            ...
            statusHandler.error(String.format(
                    "Error parsing radar tabular block: [%s] is not a valid token for %s/%s",
