 ...
 * Sep 01, 2016 4628       bkowal      Updated to allow Apps Defaults overrides.
 * Sep 09, 2016 4628       bkowal      Fixed subcenter logic and valid hour calculations.
 * 
 * </pre>
 * 
 * @author bkowal
...
public class XmrgToGribConverter {
    ...
    public void execute(final Path xmrgInputPath, final Path gribOutputPath)
        ...
        execute(xmrgInputPath, gribOutputPath, null);
    }

    public void execute(final Path xmrgInputPath, final Path gribOutputPath,
            final AppsDefaultsOverrides overrides)
                    throws XmrgToGribInitializationException,
                    XmrgToGribConversionException {
        ...
        try {
            ...
            throw new XmrgToGribConversionException("Failed to load xmrg file: "
                    + xmrgInputPath.toString() + ".", e);
        }
        ...
        if (processFlag.contains(XmrgToGribConstants.PROC_CONTAINS_DHR)
                || processFlag
                        .contains(XmrgToGribConstants.PROC_CONTAINS_DSP)) {
        ...
        final Boolean gribConvertData = AppsDefaultsConversionWrapper
                .getPropertyAsBoolean(
                        XmrgToGribConstants.AppsDefaults.GRIB_CONVERT_DATA);
         ...
         * divisor.
         */
        final float convertDivisor = (Boolean.FALSE.equals(gribConvertData))
                ? 1.0f : 100.0f;
        ...
        XmrgToGridParameters conversionParams = completeProcessLookups(
                xmrgFile.getHeader(), xmrgFile.getHrapExtent(), overrides);
        ...
        logger.debug("mwcol = {}, msrow = {}, ncol = {}, nrow = {}, mxval = {}",
                xmrgFile.getHrapExtent().x, xmrgFile.getHrapExtent().y,
                xmrgFile.getHrapExtent().width, xmrgFile.getHrapExtent().height,
                xmrgFile.getHeader().getMaxValue());
        /*
        ...
        logger.debug("Bit Packing = {}", bitPacking.toString());
         ...
         */
        bds.setNumberBytes(
                pbds.getLength() + BinaryDataSection.PREFIX_BYTES_LENGTH);
        /*
    ...
    private void convertData(final XmrgFile xmrgFile,
        ...
        if (conversionParams.getNgrid() != XmrgToGribConstants.DEFAULT_NGRID
                && conversionParams
                        .getNgrid() != XmrgToGribConstants.SUPPORTED_GRID_218) {
    ...
    private ProductDefinitionSection constructGribPDS(
            final XmrgToGridParameters parameters,
            final XmrgHeader xmrgHeader) {
        ...
        final int ngrid = (parameters
                .getNgrid() == XmrgToGribConstants.DEFAULT_NGRID)
                        ? XmrgToGribConstants.PDS_NGRID : parameters.getNgrid();
        pds.setGrid(ngrid);
        ...
        if (parameters.getXmrgToGridSubCenter().isSubCenter0()) {
            pds.setSubCenter(0);
        } else {
    ...
    private String constructHeaderForUnformatted(
        ...
        final String office = (parameters.getXmrgToGridSubCenter() == null
                || parameters.getXmrgToGridSubCenter().getOffice() == null)
                        ? StringUtils.EMPTY
                        : parameters.getXmrgToGridSubCenter().getOffice();
        return String.format(GRIB_HEADER_FMT, wmo, office,
                gribHeaderDF.get().format(saveDate));
    }
     ...
     * @param overrides
     *            optional {@link AppsDefaultsOverrides} that can be used to
     *            override specific properties relevant to xmrg -> grib in Apps
     *            Defaults
    ...
    private XmrgToGridParameters completeProcessLookups(
         ...
         */
        XmrgToGridSubCenter xmrgToGridSubCenter = lookupSubCenterCode(
                hrapExtent.x, overrides);
        if (xmrgToGridSubCenter == null) {
            throw new XmrgToGribConversionException(
                    "Failed to determine the Sub Center for: "
                            + xmrgHeader.getUserId() + ".");
        }
        logger.info("Using sub-center: {}.", xmrgToGridSubCenter.toString());
        ...
        final int ngrid = (foundParameter.getNgrid() == 0)
                ? XmrgToGribConstants.DEFAULT_NGRID : foundParameter.getNgrid();
         ...
         */
        final Calendar validCalendar = TimeUtil
                .newCalendar(xmrgHeader.getValidDate());

        /*
         * Based on how the date is formatted in the XMRG header, no 'f' present
         * in the date String, the valid hour flag will never be set. So, the
         * valid time will need to be adjusted based on the accumulation hours.
         */

        final int nturef = (foundParameter.getNturef() >= 0)
                ? foundParameter.getNturef() : 0;
        final int ntufc = (foundParameter.getNtufc() >= 0)
                ? foundParameter.getNtufc() : accumulationHours;
        validCalendar.roll(Calendar.HOUR_OF_DAY, -accumulationHours);
        xmrgHeader.setValidDate(validCalendar.getTime());
        ...
        if (!processFlag.startsWith(XmrgToGribConstants.PROC_STARTS_QPA)
                || !processFlag
                        .startsWith(XmrgToGribConstants.PROC_STARTS_QPM)) {
            /*
        ...
        if (gribParam.endsWith(XmrgToGridParameter.WILDCARD_CHAR)) {
            gribParam = gribParam.replaceAll(
                    Pattern.quote(XmrgToGridParameter.WILDCARD_CHAR),
                    StringUtils.leftPad(Integer.toString(accumulationHours), 2,
                            "0"));
        }
        ...
        if (parameter == null) {
            ...
            final Boolean ptblSearch = AppsDefaultsConversionWrapper
                    .getPropertyAsBoolean(
                            XmrgToGribConstants.AppsDefaults.GRIB_PTBL_SEARCH);
            gribParamNumberFoundTable = Table128.TABLE_NUMBER;
            if (parameter2Override != null
                    && !Boolean.TRUE.equals(ptblSearch)) {
                gribParamNumberFoundTable = Table2.TABLE_NUMBER;
     ...
     */
    private XmrgToGridSubCenter lookupSubCenterCode(
            final int mostWestHRAPColumn,
            final AppsDefaultsOverrides overrides) {
        /*
         * First, determine if the grib sub center should be set to 0.
         */
        final Boolean subCenter0;
        if (overrides == null || overrides.getSubCenter0() == null) {
            subCenter0 = AppsDefaultsConversionWrapper.getPropertyAsBoolean(
                    XmrgToGribConstants.AppsDefaults.GRIB_SET_SUBCENTER_0);
        } else {
            subCenter0 = overrides.getSubCenter0();
        }
        ...
        String siteId = AppsDefaults.getInstance()
                .getToken(XmrgToGribConstants.AppsDefaults.AWIPS_RFC_ID, null);
        if (siteId == null) {
            /*
        ...
        return new XmrgToGridSubCenter(center.getNum(), fourCharSiteId, false);
    }
    ...
    private void initializeLookupTables()
            throws LUTReadException, LUTContentException {
    ...
    private <T> T readLookupTable(final String lutFileName,
        ...
        try (InputStream is = localizationFile.openInputStream()) {
            T lut = lutClass
                    .cast(jaxbManager.unmarshalFromInputStream(lutClass, is));
