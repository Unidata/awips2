 ...
 * Sep 09, 2016 4628       bkowal      Round packed data length up in the case that
 *                                     calculation yields a floating point number.
 * 
 * </pre>
 * 
 * @author bkowal
...
public class BinaryDataSectionPacker {
    ...
    public PackBinaryDataResults run(final Boolean tossData,
        ...
        if (bitPacking.getBitsToPack() == 0) {
             ...
             */
            double scale = Math.rint(Math
                    .log(maxDiff / Math.pow(2.,
                            (double) bitPacking.getBitsToPack() - 0.5))
                    / ALOG2 + HPEPS);
            /*
            ...
            for (int i = 0; i < floatWorkGridArray.length; i++) {
                intWorkGridArray[i] = (int) Math
                        .round((floatWorkGridArray[i] - dataMin) * twon);
            }

            length = (int) Math.ceil(
                    ((double) (xmrgData.length * bitPacking.getBitsToPack())
                            + 7) / 8.0);
            final short[] packedData = new short[length];
            ...
            return new PackBinaryDataResults(packedData, length, scale,
                    dataMin);
        }
    }
}