...
public class PointSetLocation {
     ...
     * @param metaId
     */
    public void save(File file, IMetadataIdentifier metaId)
        ...
        store.addDataRecord(new FloatDataRecord(LONGITUDE, getGroup(id),
                longitudes.array()), metaId);
        store.addDataRecord(
                new FloatDataRecord(LATITUDE, getGroup(id), latitudes.array()),
                metaId);
        if (triangles != null) {
            store.addDataRecord(new IntegerDataRecord(TRIANGLES, getGroup(id),
                    triangles.array()), metaId);
        }
