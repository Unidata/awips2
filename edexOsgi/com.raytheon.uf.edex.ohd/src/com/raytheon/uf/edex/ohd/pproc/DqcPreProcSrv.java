 ...
 * Jul 08, 2016 4623       skorolev    Added parallelExecEnabled key
 * Mar 22, 2017 6180       bkowal      Break dependencies between the new and legacy
 *                                     DQC Pre Processors.
 * </pre>
 * 
 * @author snaples
...
public class DqcPreProcSrv {
    ...
    public Object process(String dqcArg) throws EdexException {
        ...
        if (AppsDefaults.getInstance().setAppContext(this)) {
            ...
            if (dqcArg.matches(DAYS_ARG)) {
                logger.info(
                        "Executing DQC Preprocessor with argument: " + dqcArg);
                days = dqcArg;
            } else if (DAYS_ARG == null) {
                logger.info(
                        "Executing DQC Preprocessor with default number of days:  "
                                + defaultNumDays);
                days = defaultNumDays;
            }
            ...
            if (exitValue == 0) {
                ...
                logger.error(
                        "DQC Preprocessor terminated abnormally with exit code: "
                                + exitValue);
            }
