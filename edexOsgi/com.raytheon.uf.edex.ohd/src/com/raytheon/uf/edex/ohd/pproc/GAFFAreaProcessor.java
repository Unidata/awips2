 ...
 * June 28, 2016 19081      xwei       Fixed casting in computeAvgFfg() method
 * 
 * </pre>
 * 
 * @author mpduff
...
public class GAFFAreaProcessor {
	private static final int MISSING_VALUE_INT = -99;
    ...
    private void computeAvgFfg(String areaId, int numRows, int[] rows,
        ...
        for (int i = 0; i < numRows; i++) {
            ...
            for (int jcol = begCol[i]; jcol <= endCol[i]; jcol++) {
                ...
                rawVal = (double) ( data[row * width + col] ) / (double) 100;

                if ( data[row * width + col] != MISSING_VALUE_INT ) {
                    sum += rawVal;
                    if (rawVal > max) {
                        max = rawVal;
                    }
