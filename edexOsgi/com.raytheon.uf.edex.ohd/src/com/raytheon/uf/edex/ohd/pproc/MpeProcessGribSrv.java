...
import com.raytheon.uf.edex.plugin.mpe.grib.ProcessGribFilesRunner;
 ...
 * Sep 01, 2016   4628     bkowal      Updates to use {@link ProcessGribFilesRunner} in
 *                                     parallel.
 * </pre>
 * 
 * @author snaples
 */
public class MpeProcessGribSrv
        implements IRequestHandler<MpeGribProcessRequest> {
     ...
     */
    @Override
    public Object handleRequest(MpeGribProcessRequest request)
            throws Exception {
        ...
        if (AppsDefaultsConversionWrapper.parallelExecEnabled()) {
            ProcessGribFilesRunner processGribFiles = new ProcessGribFilesRunner(
                    xmrg, grib);
            try {
                processGribFiles.execute();
            } catch (Exception e) {
                logger.error(
                        "Failed to execute: "
                                + ProcessGribFilesRunner.class.getName() + ".",
                        e);
            }
        }
        ...
        int exitValue = 0;
        logger.info(
                "Executing process_grib_files with arguments: xmrgfilename: "
