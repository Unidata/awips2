 ...
 * Apr 19, 2016  5572     bsteffen    Move to common, remove deprecated fields.
 * Jul 08, 2016  5748     bsteffen    Fix COVERAGE names when name is not in DB.
 * Mar 13, 2018  7052     bsteffen    Attempt to resolve COVERAGE conflicts by checking defined options.
 * 
 * </pre>
 * 
 * @see GribModelLookup
...
public class GridModel {
    ...
    public String getTransformedName(GridCoverage gc, String filePath,
            GribCoverageNameLookup coverageLookup) {
        String retval = name;
        /*
        ...
        if (covMatch.find()) {
            Set<String> names = coverageLookup.getGribCoverageNames(gc);
            if (names.size() != 1) {
                /*
                 * If there are multiple possible names, hopefully only one is
                 * listed in the definition.
                 */
                names = new HashSet<>(names);
                names.retainAll(getAllGrids());
                if (names.size() != 1) {
                    /*
                     * Sort so the log message and the choice of which name to
                     * use is deterministic.
                     */
                    names = new TreeSet<>(names);
                    statusHandler.warn("Multiple matching coverages for "
                            + this.name + ": " + names);
                }
            }
        ...
        if (resMatch.find()) {
            ...
            while (resMatch.find()) {
                 ...
                 */
                if (resMatch.group(4) != null
                        && "1".equals(resMatch.group(4))) {
