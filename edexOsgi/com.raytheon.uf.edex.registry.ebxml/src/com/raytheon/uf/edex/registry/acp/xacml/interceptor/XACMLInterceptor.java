            ...
            try {
                ...
                for (int i = 0; i < requestList.size() && accessPermitted; i++) {
                    ResponseType response = performRequest(requestList.get(i),
                            resources.get(i));
                    List<ResultType> resultList = response.getResults();
                    ResultType result = null;
                    if (resultList.size() > 1) {
                        statusHandler
                                .warn("Multiple results returned from XACML response when only 1 expected. "
                                        + "All except the first will be ignored.");
                    }
