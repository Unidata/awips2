 ...
 * Jun 21, 2012  736      djohnson  Change handleRequest() to call
 *                                  RegistryManager.
 * Sep 14, 2012  1169     djohnson  Add use of create only mode.
 * Sep 27, 2012  1187     djohnson  Simplify the session management for a
 *                                  registry interaction.
 * Mar 18, 2013           bphillip  Modified to use proper transaction
 *                                  management and spring injection of objects
 * Jun 24, 2013  2106     djohnson  Separate factory functionality into its own
 *                                  class, use registryHandler, start
 *                                  transaction if not already open.
 ...
 * @author jspinks
 */
public class EDEXRegistryManager
        extends AbstractPrivilegedRequestHandler<IRegistryRequest<?>> {

    protected static IUFStatusHandler statusHandler = UFStatus
            .getHandler(EDEXRegistryManager.class);

    protected static final String CAN_ONLY_STORE_SINGLE_OBJECT = "Only one object can be stored at a time, ignoring all but the first item in the list!";
    ...
    public Object handleRequest(IRegistryRequest<?> request) throws Exception {
        ...
        case REMOVE:
            if (query == null) {
                response = registryHandler.removeObjects(username, objects);
        ...
        case STORE_OR_REPLACE:
            ...
            response = registryHandler.storeOrReplaceObject(
                    request.getUsername(), objects.get(0));
            break;
        case STORE:
            if (objects.size() > 1) {
                statusHandler.error(CAN_ONLY_STORE_SINGLE_OBJECT);
            }
            response = registryHandler.storeObject(request.getUsername(),
                    objects.get(0));
            break;
        }
    ...
    @Override
    @Transactional
    public AuthorizationResponse authorized(IRegistryRequest<?> request)
