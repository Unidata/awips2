 ...
 * ------------- -------- --------- ------------------------------------------------------------
 * Aug 17, 2012  724      bphillip  Initial Coding
 * Mar 18, 2013  1802     bphillip  Modified to use transaction boundaries and spring injection
 * Apr 09, 2013  1802     bphillip  Import changes due to moved constant classes
 * May 21, 2013  2022     bphillip  Implemented RegistryInitializedListener
 * Mar 31, 2014  2889     dhladky   Added username for notification center tracking.
 * Jul 10, 2014  1717     bphillip  No longer implements RegistryInitializedListener
...
public class XACMLPolicyAdministrator {
    ...
    private final Map<String, PolicyType> policyMap = new HashMap<>();

    /** The map of policy sets known to the system */
    private final Map<String, PolicySetType> policySetMap = new HashMap<>();
    ...
    public void loadAccessControlPolicies() throws MsgRegistryException {
        ...
        List<RegistryObjectType> regObjs = new ArrayList<>();
        for (File element : fileList) {
            ...
            try {
                ...
                byte[] serializedPolicy = XACMLParser.getInstance()
                        .readFileContents(element)
                        .getBytes(Charset.forName("UTF-8"));
                ExtrinsicObjectType regObj = new ExtrinsicObjectType();
                regObj.setId(lid);
                regObj.setLid(lid);
                regObj.setDescription(description);
                regObj.setName(name);
