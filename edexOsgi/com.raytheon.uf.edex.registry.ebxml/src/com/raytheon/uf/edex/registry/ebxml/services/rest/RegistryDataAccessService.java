 ...
 * 12/13/2018   7238        skabasele   added getRegistryObjectIdsOwnerUpdateTimeOfType
 *
 * </pre>
 * 
 * @author bphillip
...
public class RegistryDataAccessService implements IRegistryDataAccessService {
    ...
    /**
     * @see com.raytheon.uf.common.registry.services.rest.IRegistryDataAccessService
    ...
    public RestCollectionResponse<String> getRegistryObjectIdsOfType(
        ...
        statusHandler.info(
                "Getting registry object ids of type [" + objectType + "]...");
        RestCollectionResponse<String> response = new RestCollectionResponse<>();
        response.setPayload(
                registryObjectDao.getRegistryObjectIdsOfType(objectType));
        return response;
    }

    /**
     * @see com.raytheon.uf.common.registry.services.rest.IRegistryDataAccessService
     *      .getRegistryObjectIdsOwnerUpdateTimeOfType(String)
     */
    @Override
    @GET
    @Path("getRegistryObjectIdsOwnerUpdateTimeOfType/{objectType}")
    public List<RegObjectSubset> getRegistryObjectIdsOwnerUpdateTimeOfType(
            @PathParam("objectType") String objectType) {
        statusHandler
                .info("Getting registry object's id, owner, and updateTime for type ["
                        + objectType + "]...");
        List<RegObjectSubset> result = registryObjectDao
    ...
    public String getParametersForQuery(@PathParam("queryId") String queryId) {
        statusHandler.debug(
                "Getting query parameters for query: " + queryId + "...");
