 ...
 * Nov 26, 2018  7238     skabasele Minor syntax change to resolve sonarQube gerrit issue  
 * 
 * </pre>
 * 
 * @author bphillip
...
public class CredentialCache {
    ...
    public static final boolean centralRegistry = "centralRegistry".equals(System
            .getProperty("edex.run.mode"));
                                ...
                                public String[] doInTransaction(
                                    ...
                                    if (centralRegistry) {
                                        ...
                                        if (isFederationEnabled) {
                                            String compareLocalUser = securityConfig
                                                    .getProperty(
                                                            RegistryFederationManager.EDEX_SECURITY_AUTH_USER);
                                            if (!compareLocalUser
                                                    .equals(userName)) {
                                                statusHandler
                                            ...
                                            try {
                                                user = restServices
                                                        .getRegistryObject(
                                                                ...
                                                                userName + RegistryUsers.USER_SUFFIX);
                                            } catch (Exception e) {
                                    ...
                                    if (isFederationEnabled) {
                                        ...
                                        if (user == null) {
                                            ...
                                            localUserName = user.getSlotValue(
                                                    RegistryUsers.USER_SLOT_NAME);

                                            try {
                                                password = getEncryption()
                                                        .decrypt((String) user
                                                                .getSlotValue(
                                                                        RegistryUsers.PASSWORD_SLOT_NAME));
                                            ...
                                            role = user.getSlotValue(
                                                    RegistryUsers.ROLE_SLOT_NAME);
    ...
    public void processEvent(CreateAuditTrailEvent event) {
        ...
        for (RegistryObjectType affectedObj : objsAffected) {
            if (RegistryObjectTypes.PERSON
                    .equals(affectedObj.getObjectType())) {
    ...
    public String[] getUser(String userName) throws RegistryServiceException {
        try {
            ...
            statusHandler
            .error("Error occured while retrieving user : "+ userName, e);
            throw new RegistryServiceException(
                    "Error retrieving user " + userName, e);
        }
    }
    ...
    public String getUserRole(String userName) throws RegistryServiceException {
        try {
            ...
            statusHandler
            .error("Error occured while retrieving userRole for user : "+ userName, e);
            throw new RegistryServiceException(
                    "Error retrieving role for user " + userName,e);
        }
    }
    ...
    public String getUserPassword(String userName)
        ...
        try {
            ...
            statusHandler
            .error("Error occured while retrieving password for user : "+ userName, e);
            throw new RegistryServiceException(
                    "Error retrieving password for user " + userName, e);
        }
    }
