...
public class RegistryFederationManager
     ...
     */
    private static final String NOTIFICATION_SERVERS_FILE = "ebxml"
            + IPathManager.SEPARATOR + "registry" + IPathManager.SEPARATOR
            + "notificationServers.xml";
    ...
    @Override
    @Transactional
    public synchronized void executeAfterRegistryInit()
            ...
            try {
                if (federationProperties == null) {
                    ...
                    ILocalizationFile federationPropertiesFile = PathManagerFactory
                            .getPathManager()
                            .getStaticLocalizationFile(FEDERATION_CONFIG_FILE);
                    if (federationPropertiesFile == null
                        ...
                        try (InputStream is = federationPropertiesFile
                            ...
                            statusHandler
                                    .error("Error reading federation configuration ["
                                            + federationPropertiesFile.getPath()
                                            + "]", e);
                        }
                    }
                }
                ...
                if (replicateToServers == null) {
                    ILocalizationFile notificationServerConfigFile = PathManagerFactory
                    ...
                    if (notificationServerConfigFile == null
                            || !notificationServerConfigFile.exists()) {
                        throw new EbxmlRegistryException(
                    ...
                    try (InputStream is = notificationServerConfigFile
                            .openInputStream()) {
                        replicateToServers = jaxbManager
                                .unmarshalFromInputStream(
                                        NotificationServers.class, is);
                        ...
                        statusHandler
                                .error("Error reading replication configuration ["
                                        + notificationServerConfigFile.getPath()
                                        + "]", e);
                    }
                    statusHandler.info("Found " + replicateToServers
        ...
        try {
            if (centralRegistry) {
                if (!registryUsers.userExists(RegistryUtil.registryUser)) {
                    ...
                    statusHandler.info(
                            REGISTRY_ADMIN + " present in Central Registry: "
                                    + REGISTRY_ADMIN);
                }
                ...
                if (!registryUsers
                    ...
                    registryUsers.addUser(
                            securityConfig.getSecurityProperties()
                                    .getProperty(EDEX_SECURITY_AUTH_USER),
                            securityConfig.getSecurityProperties()
                                    .getProperty(EDEX_SECURITY_AUTH_PASSWORD),
    ...
    public void updateRegistryEvents(@PathParam("registryId") String registryId,
        ...
        try {
            ...
            statusHandler.error(
                    "Error occurred updating registry uptime for " + registryId,
                    e);
        }
    }
     ...
     *            Event to be removed
     */
    private void removeRegistryFromEvent(String registryId,
            List<Long> eventIds) {
        try {
            replicationSiteEventDao.deleteMapping(eventIds, registryId);
        } catch (DataAccessLayerException e) {
            statusHandler
                    .error("Error occurred removing events from replication for site "
                            + registryId, e);
        }
    }
    ...
    public String getReplicatingTo() {
        return RegistryQueryUtil.formatArrayString(
                replicateToServers.getRegistryReplicationServers().toArray());
    }
    ...
    private void saveNotificationServers() {
        ...
        try (SaveableOutputStream os = lf.openOutputStream()) {
            ...
            statusHandler.error("Error occurred saving notification servers",
                    e);
        }
    }
        ...
        for (RegistryObjectType affectedObj : objsAffected) {
            if (replicatedObjectTypes.contains(affectedObj.getObjectType())) {
                ...
                if (!replicationEvent.getReplicateTo().isEmpty()) {
                    ...
                    statusHandler.debug("Event from source '"
            ...
            txTemplate.execute(new TransactionCallbackWithoutResult() {
                ...
                protected void doInTransactionWithoutResult(
                    ...
                    try {
                        ...
                        statusHandler.error(
                                "Sending events to [" + remoteRegistryId
                                        + "] has failed due to connection loss.",
                                e);
                    } catch (Exception e) {
    ...
    private int gatherEventsFor(String remoteRegistryId)
        ...
        RegistryType remoteRegistry = registryDao.getById(
                remoteRegistryId + FederationProperties.REGISTRY_SUFFIX);
        if (remoteRegistry == null) {
        ...
        if (!restClient.isRegistryAvailable(remoteRegistry.getBaseURL())) {
            statusHandler
                    .warn("Unable to gather events for [" + remoteRegistryId
                            + "]. Registry is currently unavailable.");
            return eventsGathered;
        }

        long t0 = TimeUtil.currentTimeMillis();
        List<Long> eventIds = replicationSiteEventDao
                .getEventsBatch(remoteRegistryId, SYNC_BATCH_SIZE);
        ...
        if (eventIds != null && !eventIds.isEmpty()) {
            ...
            for (ReplicationEvent event : events) {
                boolean isDelete = ActionTypes.delete
            ...
            LifecycleManager lcm = soapService
                    .getLifecycleManagerServiceForHost(
            ...
            for (List<ReplicationEvent> batchedEvents : orderedBatchedEvents) {
                ...
                if (!currentEventType.equals(ActionTypes.delete)) {
                    ...
                    if (batchSize == dbSize) {
                        statusHandler.info(
                                "Retrieved {} registry objects from local registry in {}",
                                batchSize, TimeUtil.prettyDuration(t1 - t0));
                    } else {
                        statusHandler.info(
                                "Retrieved {} registry objects from local registry in {}. Expected {} objects, some objects have already been deleted from registry before they could be synced.",
                                dbSize, TimeUtil.prettyDuration(t1 - t0),
                                batchSize);
                    }
                    ...
                    if (dbSize > 0) {
                        ...
                        while (restClient.isRegistryAvailable(
                            ...
                            try {
                                ...
                                t1 = TimeUtil.currentTimeMillis();
                                statusHandler.error("Failed after "
                                        + TimeUtil.prettyDuration(t1 - t0)
                                        + " submitting " + batchSize
                                        + " objects to " + remoteRegistryId, e);
                    ...
                    while (restClient
                        ...
                        try {
                            ...
                            t1 = TimeUtil.currentTimeMillis();
                            statusHandler.info(
                                    "Removed {} registry objects from {} in {}",
                                    batchSize, remoteRegistryId,
                                    TimeUtil.prettyDuration(t1 - t0));
                            // Successfully sent request, so stop looping.
                            break;
                            ...
                            statusHandler.error("Failed after "
                                    + TimeUtil.prettyDuration(t1 - t0)
                                    + " removing " + batchSize
                                    + " objects from " + remoteRegistryId, e);
                            handleGatherSoapFailure(remoteRegistryId,
                ...
                t1 = TimeUtil.currentTimeMillis();
                statusHandler.info("Removed {} replication events for {} in {}",
                        batchSize, remoteRegistryId,
                        TimeUtil.prettyDuration(t1 - t0));
            }
        } else {
    ...
    private void handleGatherSoapFailure(String remoteRegistryId,
        ...
        if (restClient.isRegistryAvailable(remoteRegistry.getBaseURL())
            ...
            int failCount = gatherFails.containsKey(remoteRegistryId)
                    ? gatherFails.get(remoteRegistryId) : 0;
            failCount = failCount + batchSize;
            gatherFails.put(remoteRegistryId, failCount);
            statusHandler.error("Unable to successfully send request to "
                    + remoteRegistryId + ". " + batchSize
            ...
            throw new ConnectException(
                    "Lost connection to " + remoteRegistryId);
        }
    }
        ...
        if (federationEnabled && initialized.get() && EDEXUtil.isRunning()) {
            ...
            if (downTime > MAX_DOWN_TIME_DURATION && !centralRegistry) {
                if (!SYNC_IN_PROGRESS.get()) {
                    ...
                    statusHandler.warn(
                            "Registry is out of sync with federation. Attempting to automatically synchronize");
                    try {
                        synchronize();
                        monitorHandler
                                .info("Registry successfully synchronized!");
                    } catch (EbxmlRegistryException e) {
                        monitorHandler.error("Error synchronizing registry!",
                                e);
                        throw e;
                    }
                }
    ...
    public void deleteExpiredEvents() {
        ...
        do {
            try {
                ...
                statusHandler.error("Error occurred deleting expired events",
                        e);
            }
    ...
    public List<String> getExpiredSites() {
        ...
        List<String> registries = replicationSiteEventDao.getExpiredEventSites(
                (gmtTime.getTimeInMillis() - MAX_DOWN_TIME_DURATION));
        return registries;
    }
    ...
    public void stopTrackingEventsForSite(String registryId) {
        ...
        try {
            ...
            statusHandler.error("Error ocurred removing replication events for "
                    + registryId, e);
        }
    }
    ...
    public void verifyReplication() throws Exception {
        if (federationEnabled && replicateToServers != null
                && !replicateToServers.getRegistryReplicationServers()
                        .isEmpty()) {
            statusHandler
    ...
    private void verifyReplicationFor(String registryId) throws Exception {
        ...
        if (remoteRegistry != null) {
            String replicatingTo = restClient
            ...
            if (replicatingTo.contains(RegistryIdUtil.getId())) {
                ...
                subscribeToRegistry(registryId);
                statusHandler.info(
                        "Replication with [" + registryId + "] established");
            }
        } else {
