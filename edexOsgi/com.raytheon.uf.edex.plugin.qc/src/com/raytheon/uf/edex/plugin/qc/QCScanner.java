...
/**
 * Scans NetCDF files generated by A1 legacy applications and generates
 * QCRecord instances that refer to their records.
 * 
 * <pre>
 * 
 * SOFTWARE HISTORY
            ...
            for (String fn : directory.list()) {
                if (filePattern.matcher(fn).matches()) {
                    try {
                        ...
                        statusHandler.error(String.format("error reading %s/%s: %s",
        ...
        private void scanFile(String fn) throws Exception {
            ...
            try {
                ...
                while (index < nRecords) {
                    PluginDataObject[] records = new PluginDataObject[Math.min(maxRecordsInChunk, nRecords - index)];
                    ...
                    while (ri < records.length) {
                        ...
                        if (obsTime != vObsTimeFillValue &&
                            ...
                            r.setDataTime(new DataTime(new Date((long)(obsTime * 1000))));
                            SurfaceObsLocation loc = new SurfaceObsLocation();
                            loc.assignLocation(dLat.getDouble(ri), dLon.getDouble(ri));
                            loc.setElevation(dElev.getInt(ri));
                            StringBuilder stationId = new StringBuilder(ID_LENGTH);
