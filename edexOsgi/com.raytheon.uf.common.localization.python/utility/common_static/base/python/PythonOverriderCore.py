...
def _internalOverride(files):
    ...
    """
    themodule = types.ModuleType('tmpmodule')
...
def _combineMembers(tocombine, combinationresult):    
    for attr in dir(tocombine):
        ...
        if isinstance(getattr(tocombine, attr), type):
...
def _mergeClasses(source, target, className):
    ...
    for attr in dir(sourceClass):
         ...
         if isinstance(getattr(sourceClass, attr), type):
    ...
    abstractMethods = set()
    if '__abstractmethods__' in sourceClass.__dict__:
