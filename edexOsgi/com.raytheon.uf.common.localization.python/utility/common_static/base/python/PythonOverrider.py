...
#    04/22/20  7883     tgurney   Python 3 fix - convert subprocess.check_output
#                                 result from bytes to string

import os.path
import sys

JEP_AVAILABLE = 'jep' in sys.modules
...
if JEP_AVAILABLE:
    ...
    if os.path.isfile(SETUP_FILE):
        ...
        test_host = subprocess.check_output(format.format(f'source {SETUP_FILE}; echo $DEFAULT_HOST', shell=True).strip().decode()
        if test_host:
            THRIFT_HOST = test_host
        test_port = subprocess.check_output(format.format(f'source {SETUP_FILE}; echo $DEFAULT_PORT', shell=True).strip().decode()
        if test_port:
