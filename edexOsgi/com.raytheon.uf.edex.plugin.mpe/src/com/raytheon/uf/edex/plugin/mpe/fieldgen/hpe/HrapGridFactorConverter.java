 ...
 * property value to a {@link HrapGridFactor}.
 * 
 * <pre>
 * 
 * SOFTWARE HISTORY
...
public class HrapGridFactorConverter implements
        ICustomValueConverter<HrapGridFactor> {

    @Override
    public HrapGridFactor convertValue(String value)
            throws ValueConverterException {
        int lookupNum;
        try {
            lookupNum = Integer.parseInt(value);
        } catch (NumberFormatException e) {
            throw new ValueConverterException("Failed to parse: " + value
                    + " as an Integer.", e);
        }

        return HrapGridFactor.lookupGridFactorByNum(lookupNum);
    }
}