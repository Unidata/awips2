 ...
 * Dec 15, 2016 4622       bkowal      Added {@link #findPartialRecordByNamedQueryAndNamedParam(String, String, Object)}.
 * 
 * </pre>
 * 
 * @author bkowal
...
public abstract class AbstractMPEDao<T, I extends Serializable>
    ...
    public long countByNamedQueryAndNamedParam(final String queryName,
        ...
        Long countResult = txTemplate.execute(new TransactionCallback<Long>() {
            public Long doInTransaction(final TransactionStatus txStatus) {
                ...
                Number countNum = (Number) query.uniqueResult();
                if (null == countNum) {
                    return null;
                }
                return new Long(countNum.longValue());
            }
        });
        if (null == countResult) {
            return 0;
    ...
    public Date getDateByNamedQueryAndNamedParam(final String queryName,
            final String[] names, final Object[] parameters) {
        if ((names == null) || (parameters == null)
                || (names.length != parameters.length)) {
            throw new IllegalArgumentException(
        ...
        Date queryResult = txTemplate.execute(new TransactionCallback<Date>() {
            public Date doInTransaction(final TransactionStatus txStatus) {
                Session session = getCurrentSession();
                Query query = session.getNamedQuery(queryName);
                for (int i = 0; i < names.length; i++) {
                    if (parameters[i] instanceof Collection) {
    ...
    public List<?> findPartialRecordByNamedQueryAndNamedParam(
            final String queryName, final String name, final Object parameter) {
        return txTemplate.execute(new TransactionCallback<List<?>>() {
            @Override
            public List<?> doInTransaction(TransactionStatus status) {
                Session session = getCurrentSession();
