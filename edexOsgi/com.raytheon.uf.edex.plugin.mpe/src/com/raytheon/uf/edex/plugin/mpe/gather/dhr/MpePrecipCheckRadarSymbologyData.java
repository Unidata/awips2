...
public final class MpePrecipCheckRadarSymbologyData
    ...
    public boolean readPrecipIndicationParams(final ByteBuffer buf) {
        try {
             ...
             * and files without the proper structure in the DHR Gather file
             * directory. So, if a file with the wrong structure is encountered,
             * it will just be ignored to match the legacy implementation.
             */
            return false;
        }
        return (getPsmParams() != null
                && getPsmParams().length > DHRGatherConstants.DHR_PARAM_PRECIP);
    }
}