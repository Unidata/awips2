 ...
 * Sep 04, 2014  3220     skorolev  Removed parameter currentSite from FSSObs
 *                                  configuration managers.
 * Oct 12, 2015  4911     rjpeter   Updated to reload all location data and diff
 *                                  table as a whole.
 * Jul 14, 2016  5744     mapeters  Use common_static instead of edex_static for
 *                                  storing/retrieving NDM files.
 * Mar  5, 2018  7109     tgurney   Allow partial update of tables. Log failures
 *                                  for individual rows
 *
 * </pre>
 *
...
public class LocationTablesIngest implements INationalDatasetSubscriber {
    ...
    private static final String NDM_LOC_DIR = "spatialTables"
        ...
        try {
            ...
            logger.error(
                    "Update of common_obs_spatial cancelled.  Failed to store "
                            + file.getPath() + " to localization",
                    e);
            return;
        }

        try {
            Map<String, ObStationRow> gidMap = new HashMap<>();
            ...
            proc.doUpdate();
            logger.info("Processing of file [" + file.getName() + "] complete");
            logger.info(
                    String.format("Stations Added/Updated/Deleted: [%d/%d/%d]",
                            proc.getStationsAdded(), proc.getStationsUpdated(),
                            proc.getStationsDeleted()));
            if (proc.getStationsAddedFailed() > 0
                    || proc.getStationsUpdatedFailed() > 0
                    || proc.getStationsDeletedFailed() > 0) {
                logger.warn(
                        String.format("Failed to Add/Update/Delete: [%d/%d/%d]",
                                proc.getStationsAddedFailed(),
                                proc.getStationsUpdatedFailed(),
                                proc.getStationsDeletedFailed()));
            }
        } catch (Exception e) {
            logger.error("Error occurred processing file: " + file.getName(),
                    e);
        }
    }
            ...
            if (station != null) {
                ...
                if (ObStation.CAT_TYPE_SFC_RAOB
                    ...
                    if (gidMap.containsKey(key)) {
                        ...
                        aggregate.setUpperAirElevation(
                                station.getUpperAirElevation());
                        aggregate.setUpperAirGeometry(
                                station.getUpperAirGeometry());
                        if (aggregate.getIcao() == null) {
                            aggregate.setIcao(station.getIcao());
                        }
                    }
                } else if (ObStation.CAT_TYPE_SFC_FXD
                        .equals(station.getCatalogType())) {
                    ...
                    if (gidMap.containsKey(key)) {
                        ObStationRow aggregate = gidMap.get(key);
                        station.setUpperAirElevation(
                                aggregate.getUpperAirElevation());
                        station.setUpperAirGeometry(
                                aggregate.getUpperAirGeometry());
     ...
     */
    protected void storeNdmFile(File file)
